...
EXTENDS Naturals
CONSTANTS WriteDuringInspect, HoldStoreWhileWaiting, SkipRevalidation
...
ChangeContext == /\ phase \in {"Waiting", "Query"} /\ valid' = FALSE
...
OwnLive == /\ phase = "Query" /\ valid /\ phase' = "Claim"
...
Call == /\ phase = "Claim" /\ valid /\ grant /\ phase' = "Done"
...
Abort == /\ phase \in {"Waiting", "Recheck", "Prepare", "Query", "Claim"}
         /\ phase' = "Done" /\ storeLocked' = FALSE /\ lease' = FALSE /\ grant' = FALSE
         /\ UNCHANGED <<acquired, valid, prepared, preparedCorrect, called, callCorrect>>
Next == Inspect \/ Acquire \/ ChangeContext \/ Recheck \/ Prepare \/ OwnLive \/ Call \/ Abort
Spec == Init /\ [][Next]_vars
TypeOK == /\ phase \in {"Inspect", "Waiting", "Recheck", "Prepare", "Query", "Claim", "Done"}

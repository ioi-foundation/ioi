...
def check(lines):
    try:
        ...
        if len(allocations) != 1:
            return False
        initialization = [(i, int(m.group(1))) for i in range(allocations[0] + 1, begin)
                          if 'write(' in lines[i] and f'<{source}>' in lines[i]
                          and (m := re.search(r'\)\s+= ([0-9]+)$', lines[i]))]
        if sum(count for _, count in initialization) != 81920:
            return False
        def sync(line, path):
            return bool(re.search(r'fsync\(\d+<' + re.escape(path) + r'>\)\s+= 0$', line))
        synced = next(i for i in range(initialization[-1][0] + 1, begin) if sync(lines[i], source))

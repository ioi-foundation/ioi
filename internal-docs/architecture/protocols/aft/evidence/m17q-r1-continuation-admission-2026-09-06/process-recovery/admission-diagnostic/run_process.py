...
components=base/'components';components.mkdir(exist_ok=True)
env['IOI_AFT_BENCH_TRACE_DIR']=str(components)
started=datetime.datetime.now(datetime.timezone.utc).isoformat()
(base/'process-started.json').write_text(json.dumps({'started':started,'command':command,'environment_overrides':{'IOI_TEST_ORCH_RUST_LOG':env['IOI_TEST_ORCH_RUST_LOG'],'IOI_AFT_BENCH_TRACE_DIR':env['IOI_AFT_BENCH_TRACE_DIR']},'scope':'scoped dirty-worktree process campaign; not immutable R2'},indent=2)+'\n')

#!/usr/bin/env python3
"""Scoped overlay scan checks; not full M16Q qualification."""
...
checks=[('api',['cargo','test','--locked','-p','ioi-api','--lib']),
 ('registry',['cargo','test','--locked','-p','ioi-services','--lib','aft_effect_registry']),
 ('transaction-write-cleanup',['cargo','test','--locked','-p','ioi-execution','--lib','mv_memory::tests::clear_tx_writes_removes_stale_keys_from_prior_incarnation','--','--exact']),
 ...
 ('formal',['bash','.github/scripts/run_aft_formal_checks.sh','--quv-manifest-locator-only'])]
...
put('completed.json',{'completed':now(),'source_changes':changed,'passing':passing,'scope':'scoped overlay scan and registry bootstrap validation; prior process evidence predates this production change; aggregate admission/profile/full transition refinement/R2/review outstanding'})

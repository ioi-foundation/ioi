...
mutations=[('binding','if OnlineEffectAuthorizationBindingV1::from_manifest(&manifest)? != candidate_binding {','if false {'),
 ('preflight','if needs_live {\n            preflight.await?;\n        }','// Defensive omission: no candidate preflight.'),
 ('terminal','if needs_live {\n            preflight.await?;\n        }','preflight.await?;')]
archive=base/'mutation-sources';archive.mkdir(exist_ok=True)
(archive/'consequence.baseline.rs').write_bytes(original)
command=['cargo','test','--locked','-p','agentgres','--lib','consequence::tests::checked_preparation_refuses_before_storage_and_keeps_terminal_readmission_non_authorizing','--','--exact']
results=[]
try:

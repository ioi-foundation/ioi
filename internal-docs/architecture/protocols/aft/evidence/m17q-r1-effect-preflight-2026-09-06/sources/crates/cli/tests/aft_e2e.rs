...
async fn test_aft_quv_m16q_each_single_correct_member_and_conflict_isolation() -> Result<()> {
    ...
    let run = async {
        ...
        let expiry_height = rpc::get_status(&cluster.validators[0].validator().rpc_addr)
            .await?.height.checked_add(64).ok_or_else(|| anyhow::anyhow!("expiry height overflow"))?;
...
&ioi_types::app::QuvPreparationPolicyV0::Independent { max_attempts_per_slot: 2, service_millis: (DELTA_RT_MILLIS as u64).saturating_add(CONTINUATION_MILLIS as u64), readiness_millis: 1_000_000 },
 ...
 256,)?;
            ...
            manifests.iter().enumerate()
        {
            ...
            let runtime_root = cluster.validators[*correct_process]
                .validator()
                .state_dir()
                .join("ordering-finality");
            ...
            *invalid_signature.authority_signature.first_mut().ok_or_else(|| {
                anyhow::anyhow!("M16Q candidate fixture has no authority signature")
            })? ^= 1;
            ...
            .await
            .map_err(|_| anyhow::anyhow!("M16Q invalid-signature preparation timed out"))?
            .expect_err("invalid authority signature must be rejected before preparation");
            anyhow::ensure!(
                rejected.downcast_ref::<tonic::Status>().is_some_and(|status| {
                    status.code() == tonic::Code::FailedPrecondition
                        && status.message() == "Invalid(\"invalid QUV member signature\")"
                }),
                "M16Q preparation failed for an unexpected reason: {rejected:#}"
            );
            anyhow::ensure!(
            ...
            println!(
                "[M16Q-QUV] case=sole_correct member_position={member_position} process_index={correct_process} terminal_replays=1 replay_elapsed_ms={replay_elapsed_ms} elapsed_ms={elapsed} max_valid_reply_elapsed_ms={max_valid_reply_elapsed_ms} qualified_envelope_ms=4000 result=executed"
            );
        ...
        println!(
            "[M16Q-QUV] case=authenticated_saturation operations={} verifier_nonces={} preparation_elapsed_ms={preparation_elapsed_ms} rpc_launch_span_ms={rpc_launch_span_ms} elapsed_ms={saturation_elapsed_ms} max_valid_reply_elapsed_ms={:?} qualified_envelope_ms=4000 result=executed",
            ...
            match result {
                Ok(response) => {
                    ...
                    if receipt.manifest != *manifest
                        ...
                        || !resource.verify_record_evidence(&resource_record)
                    {
                        return Err(anyhow::anyhow!(
        ...
        println!(
            "[M16Q-QUV] case=concurrent_valid_conflict accepts={conflict_accepts} conflict_rejections={conflict_rejections} durable_records={conflict_accepts} rejected_resources_unchanged=true elapsed_ms={conflict_elapsed_ms} result=safe"
        );
        ...
        let observed_height = tokio::time::timeout(Duration::from_secs(240), async {
            loop {
        ...
        }).await.map_err(|_| anyhow::anyhow!("effect endpoint did not report admitted height beyond expiry_height={expiry_height} within 240 seconds"))??;

#!/usr/bin/env python3
"""Scoped effect preparation guard checks; not full M16Q qualification."""
...
files += ['crates/cli/tests/aft_e2e.rs','crates/validator/src/standard/orchestration/runtime_finality.rs','crates/agentgres/src/recognized_effect.rs','crates/consensus/src/aft/query_unanimity.rs','crates/agentgres/src/consequence/resource_reservation.rs','crates/agentgres/src/consequence/receipt_reservation.rs', '.github/scripts/check_aft_endpoint_reservation.py', '.github/scripts/check_aft_receipt_reservation.py']
files.extend(str(p.relative_to(root)) for folder in ['consequence','resource_profile','outbox'] for p in (root/'internal-docs/architecture/protocols/aft/formal'/folder).glob('*') if p.suffix in {'.tla','.cfg'} and '_TTrace_' not in p.name)
...
checks=[('consequence',['cargo','test','--locked','-p','agentgres','--lib','consequence::']),
 ...
 ('formal',['bash','.github/scripts/run_aft_formal_checks.sh','--quv-effect-preparation-only'])]
...
put('completed.json',{'completed':now(),'source_changes':changed,'passing':passing,'scope':'scoped preparation guard refinement; process regression compiled but not run; aggregate admission/profile/full transition refinement/R2/review outstanding'})

...
fn read_header(file: &mut File) -> Result<(u64, u64, u64), ConsequenceError> {
    ...
    if &bytes[..8] != MAGIC
        ...
        || allocated < capacity
        || allocated > capacity.saturating_mul(2)
...
fn prepare_spare(path: &Path, bytes: &[u8], capacity: u64) -> Result<(), ConsequenceError> {
    ...
    if file.metadata()?.len() > capacity || file.allocated_size()? > capacity.saturating_mul(2) {
        return Err(ConsequenceError::CorruptReceipt);
    }
    file.set_len(0)?;
    ...
    if allocated < capacity || allocated > capacity.saturating_mul(2) {
        return Err(ConsequenceError::CorruptReceipt);
    }
    file.seek(SeekFrom::Start(0))?;

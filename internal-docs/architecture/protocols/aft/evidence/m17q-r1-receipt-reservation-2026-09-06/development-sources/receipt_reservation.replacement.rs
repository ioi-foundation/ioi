...
pub(super) fn prepare(path: &Path, bound: u64) -> Result<(), ConsequenceError> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Build the replacement from an empty spare instead of extending an
        // already fragmented active file. Preserve the validated active bytes
        // until the replacement and its canonical name are both durable.
        active.sync_all()?;
        let spare_path = resource_reservation::staged(path)?;
        prepare_spare(&spare_path, &bytes, bound, allocated)?;
        exchange(path, &spare_path)?;
        File::open(path.parent().ok_or(ConsequenceError::CorruptReceipt)?)?.sync_all()?;
        // Only now may the former active inode be reset as uncommitted spare.
        prepare_spare(&spare_path, &bytes, bound, allocated)?;
...
fn prepare_spare(
    ...
    let mut spare = resource_reservation::private_open(path, true)?;

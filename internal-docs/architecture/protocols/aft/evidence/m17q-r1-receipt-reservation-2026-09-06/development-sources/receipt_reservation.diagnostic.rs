...
pub(super) fn prepare(path: &Path, bound: u64) -> Result<(), ConsequenceError> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if active.allocated_size()? != allocated {
            #[cfg(test)] eprintln!("active allocation actual={} expected={allocated}", active.allocated_size()?);
            return Err(ConsequenceError::CorruptReceipt);
        }
        active.sync_all()?;
        ...
        if spare.allocated_size()? != allocated {
            #[cfg(test)] eprintln!("spare allocation actual={} expected={allocated}", spare.allocated_size()?);
            return Err(ConsequenceError::CorruptReceipt);
        }
        spare.sync_all()?;

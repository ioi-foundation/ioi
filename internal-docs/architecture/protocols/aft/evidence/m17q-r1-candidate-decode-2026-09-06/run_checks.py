#!/usr/bin/env python3
"""Scoped candidate decoding checks; not full M16Q qualification."""
...
checks=[('candidate-capacity',['cargo','test','--locked','-p','ioi-consensus','--features','aft','--lib','aft::query_unanimity::tests::candidate_byte_cap_precedes_validation_and_store_mutation','--','--exact']),
...
put('completed.json',{'completed':now(),'source_changes':changed,'passing':passing,'scope':'scoped candidate decoding validation; prior process evidence predates this production change; aggregate admission/profile/full transition refinement/R2/review outstanding'})

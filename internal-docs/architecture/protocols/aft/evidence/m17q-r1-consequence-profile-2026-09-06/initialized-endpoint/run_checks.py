#!/usr/bin/env python3
"""Scoped consequence profile binding checks; not full M16Q qualification."""
...
files += ['crates/consensus/src/aft/query_unanimity.rs','crates/agentgres/src/consequence/resource_reservation.rs','crates/agentgres/src/consequence/receipt_reservation.rs', '.github/scripts/check_aft_endpoint_reservation.py', '.github/scripts/check_aft_receipt_reservation.py']
...
checks=[('core',['cargo','test','--locked','-p','ioi-consensus','--features','aft','--lib','aft::query_unanimity']),
 ('consequence',['cargo','test','--locked','-p','agentgres','--lib','consequence::tests::']),
 ...
 ('endpoint-syscalls',['python3','.github/scripts/check_aft_endpoint_reservation.py','--output',str(base/'final-syscalls')]),
 ('receipt-syscalls',['python3','.github/scripts/check_aft_receipt_reservation.py','--output',str(base/'final-receipt-syscalls')]),
 ('reserved-index-proof',['bash','.github/scripts/run_aft_formal_checks.sh','--pq-reserved-index-only']),
 ('reservation-proof',['bash','.github/scripts/run_aft_formal_checks.sh','--quv-record-reservation-only']),
...
put('completed.json',{'completed':now(),'source_changes':changed,'passing':passing,'scope':'scoped v6 rooted storage charges; aggregate admission/profile/refinement/full R2/review outstanding'})

...
TrimOrphan == /\ phase \in {"Idle", "Committed"}
              ...
              /\ UNCHANGED <<index, pending, phase>>
PrecommitCapacityRefusal ==
  /\ phase \in {"Prepared", "Payloads"}
  /\ \E debris \in SUBSET (pending \ index) : blobs' = blobs \ debris
  /\ phase' = "Idle" /\ pending' = {} /\ UNCHANGED index
Next == Stage \/ WriteOnePayload \/ WritePayloads \/ Publish \/ Cleanup \/ CrashRecover \/ TrimOrphan \/ PrecommitCapacityRefusal

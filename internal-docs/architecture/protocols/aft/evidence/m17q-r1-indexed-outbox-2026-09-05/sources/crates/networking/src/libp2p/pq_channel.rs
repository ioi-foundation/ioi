...
impl PqDurableOutbox {
    fn open(local: &PqChannelLocalConfig) -> Result<Self> {
        ...
        if !indexed {
            ...
            outbox_index::persist(&local.outbox_path, None, &outbox.state)?;
            outbox.indexed = true;
        }
        Ok(outbox)
    }
    ...
    fn commit(&mut self, next: PqOutboxStateV2) -> Result<()> {
        ...
        self.commit_with(next, |path, next| {
            outbox_index::persist(path, previous.as_ref(), next)
        })?;
        self.indexed = true;
        Ok(())
    }
...
mod tests {
    ...
    fn legacy_outbox_upgrade_recovers_interrupted_payload_staging() {
        ...
        assert!(
            outbox_index::persist_with_hook(&local.outbox_path, None, &state, |phase| {

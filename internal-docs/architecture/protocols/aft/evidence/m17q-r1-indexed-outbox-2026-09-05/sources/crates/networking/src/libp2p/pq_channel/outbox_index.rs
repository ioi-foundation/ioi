...
pub(super) fn read(
    ...
    let index: Index =
        codec::from_bytes_canonical(&bounded_read(path, max)?).map_err(anyhow::Error::msg)?;
    if index.magic != MAGIC || index.scope != Scope::of(expected) || index.entries.len() > count {
...
pub(super) fn persist(
    ...
    persist_with_hook(path, previous, next, |_| Ok(()))
}
...
pub(super) fn persist_with_hook(
    ...
    atomic_bytes(path, &index.encode())?;
    ...
    for id in previous_ids.difference(&next_ids) {
        std::fs::remove_file(entry_path(path, id))?;
    }

...
fn checked_readiness_observation(fields: &serde_json::Value, nonce: &str) -> Result<()> {
    ...
    if fields["nonce"] != nonce
        ...
        || false && required < Duration::from_secs(15).as_nanos()
        || elapsed < required
    {
...
async fn test_aft_quv_consecutive_slots_wait_and_reopen_without_blocking_unrelated_effects(
    ...
    let run = async {
        ...
        let cases = async {
            ...
            println!("[M16Q-READINESS] slot=1 result=executed max_valid_reply_elapsed_ms={max_reply}");
            for index in 1..3 {
                if index == 2 {
                ...
                let waiting = next_readiness_event(&mut events, "foreground_readiness_wait", slot).await?;
                ...
                if index == 1 {
                    ...
                    println!("[M16Q-READINESS] case=unrelated_during_wait result=executed terminal_parent_replay=true");
                }
                ...
                let audit = receipt.online_authorization_audit.as_ref().unwrap();
                let evidence: QuvAcceptedAuditEvidenceV0 = codec::from_bytes_canonical(&audit.protocol_evidence).map_err(anyhow::Error::msg)?;
                if evidence.request.candidate != candidates[index] || hex::encode(evidence.request.verifier_nonce) != nonce {

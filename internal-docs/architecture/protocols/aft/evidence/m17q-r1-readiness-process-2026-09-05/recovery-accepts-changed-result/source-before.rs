...
fn is_quv_recovery_startup_error(error: &anyhow::Error) -> bool {
    error.chain().any(|cause| {
        ...
        status.code() == tonic::Code::Unavailable && status.message() == "Node is initializing"
    })
}

async fn wait_for_read_only_recovery<T, F, Fut>(
    mut probe: F,
    expected: &T,
    budget: Duration,
) -> Result<()>
    ...
    Fut: std::future::Future<Output = Result<T>>,
{
    tokio::time::timeout(budget, async {
        loop {
            match probe().await {
                Ok(result) if &result == expected => return Ok(()),
                Ok(_) => return Err(anyhow::anyhow!("recovered terminal parent result changed")),
                Err(error) if is_quv_recovery_startup_error(&error) =>
                    tokio::time::sleep(Duration::from_millis(50)).await,
                Err(error) => return Err(error.context("recovered parent readiness probe refused")),
            }
        }
    }).await.map_err(|_| anyhow::anyhow!("restarted executor did not recover its exact parent result within the readiness probe budget"))?
}
            ...
            for index in 1..3 {
                if index == 2 {
                    ...
                    wait_for_read_only_recovery(
                        || rpc::execute_aft_quv_effect(&rpc_addr, &manifests[0].effect_id, &candidates[0]),
                        &first, Duration::from_secs(20),
                    ).await?;
                    println!("[M16Q-READINESS-RECOVERY] exact_terminal_parent_replay=true startup_budget_millis=20000");
                }
...
#[tokio::test]
async fn recovery_probe_retries_only_startup_and_requires_exact_result() {
    let transport = rpc::get_status("127.0.0.1:0").await.unwrap_err();
    assert!(
        is_quv_recovery_startup_error(&transport),
        "typed transport source was lost: {transport:#}"
    );
    let refused = anyhow::Error::new(std::io::Error::from(std::io::ErrorKind::ConnectionRefused))
        .context("transport fixture");
    assert!(is_quv_recovery_startup_error(&refused));
    assert!(is_quv_recovery_startup_error(
        &tonic::Status::unavailable("Node is initializing").into()
    ));
    for error in [
        anyhow::anyhow!("Failed to connect to public gRPC: transport error"),
        tonic::Status::unavailable("consequence store is busy").into(),
        tonic::Status::failed_precondition("effect manifest is not Agentgres-admitted").into(),
    ] {
        assert!(!is_quv_recovery_startup_error(&error));
    }
    let mut probes = std::collections::VecDeque::from([
        Err(tonic::Status::unavailable("Node is initializing").into()),
        Ok(vec![1_u8]),
    ]);
    wait_for_read_only_recovery(
        || std::future::ready(probes.pop_front().unwrap()),
        &vec![1_u8],
        Duration::from_secs(1),
    )
    .await
    ...
    assert!(wait_for_read_only_recovery(
        || std::future::ready(Ok(vec![2_u8])),
        &vec![1_u8],
        Duration::from_secs(1)
    )
    .await
    .is_err());
    let mut calls = 0;
    let error = wait_for_read_only_recovery(
        || {
            calls += 1;
            std::future::ready(Err::<Vec<u8>, _>(
                tonic::Status::failed_precondition("admission missing").into(),
            ))
        },
        &vec![1_u8],
        Duration::from_secs(1),

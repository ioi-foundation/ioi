...
impl QuvMemberDomainV0 {
    ...
    /// Retained accepted coordinates for storage consistency checks. These
    /// entries are local state, not evidence authorizing a new executor.
    pub(super) fn accepted_entries(&self) -> impl Iterator<Item = (QuvSlotV0, QuvHash)> + '_ {
        let accepted: &[QuvHash] = match self {
            Self::Fixed(history) => &history.accepted,
            Self::HandoffBoundary(_) => &[],
        };
        accepted.iter().enumerate().map(move |(offset, hash)| {
            let mut slot = self.initial().clone();
            // validate() precedes iteration and checks the last accepted slot.
            slot.slot += offset as u64;
            if offset > 0 {
                slot.predecessor = accepted[offset - 1];
            }
            (slot, *hash)
        })
    }

...
def check(lines):
    ...
    for move, line in enumerate(lines):
        ...
        if not sync(prior, source) or not sync(after, parent):
            return False
        anchor = str(parent/'state.anchor')
        ...
        if not sync(next((line for line in reversed(lines[:swap]) if 'fsync(' in line), ''), anchor+'.tmp'):
            return False
        if not sync(next((line for line in lines[swap+1:] if 'fsync(' in line), ''), parent):
            return False
...
def main():
    ...
    checks = {'reserved_state_then_anchor': check(lines), 'missing_state_sync_rejected': not check(no_state_sync), 'missing_anchor_sync_rejected': not check(no_anchor_sync)}

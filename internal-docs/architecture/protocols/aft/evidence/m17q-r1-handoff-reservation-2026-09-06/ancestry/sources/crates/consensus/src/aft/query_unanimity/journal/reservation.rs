...
impl RecordReservation {
    ...
    pub(super) fn prepare_record(
        ...
        check_charge(&file, self.limits.record_bytes).map_err(|error| {
            if matches!(error, QuvError::StoreCapacityExceeded) {
                // An invalid existing physical reservation is a storage fault,
                // not a healthy refusal of this caller's requested byte count.
                QuvError::StoreRequiresReopen
            } else {
                error
            }
        })?;

#!/usr/bin/env python3
"""Scoped pre-live handoff and member reservation verification, not full R2 qualification."""
...
files=['crates/validator/src/standard/orchestration/quv.rs','crates/cli/tests/aft_e2e.rs','Cargo.toml','Cargo.lock','crates/consensus/Cargo.toml','crates/consensus/src/aft/query_unanimity.rs',
       'crates/types/src/app/query_unanimity.rs','.github/scripts/run_aft_formal_checks.sh',
       '.github/scripts/run_aft_m16q_qualification.sh','.github/scripts/check_aft_quv_handoff_reservation.py','.github/scripts/check_aft_quv_record_reservation.py']
for directory,suffixes in [('internal-docs/architecture/protocols/aft/formal/consequence',{'.tla','.cfg'}),('crates/validator/src/standard/orchestration/quv',{'.rs'}),
                          ('crates/cli/tests/aft_e2e',{'.rs'}),('crates/consensus/src/aft/query_unanimity',{'.rs'}),
    ...
    files.extend(str(p.relative_to(root)) for p in (root/directory).rglob('*')
                 if p.suffix in suffixes and '_TTrace_' not in p.name and '.tlacache' not in p.parts)
...
checks=[('core',['cargo','test','--locked','-p','ioi-consensus','--features','aft','--lib','aft::query_unanimity']),
        ...
        ('anchor',['bash','.github/scripts/run_aft_formal_checks.sh','--quv-anchor-reservation-only']),
        ('continuation',['bash','.github/scripts/run_aft_formal_checks.sh','--quv-install-continuation-only']),
        ('lifetime',['bash','.github/scripts/run_aft_formal_checks.sh','--quv-lifetime-budget-only']),
        ('handoff_trace',['python3','.github/scripts/check_aft_quv_handoff_reservation.py','--output',str(base/'handoff-trace')]),

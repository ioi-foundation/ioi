#!/usr/bin/env python3
"""Scoped consequence representation checks; not full M16Q qualification."""
...
files.extend(str(p.relative_to(root)) for p in (root/'internal-docs/architecture/protocols/aft/formal/consequence').glob('*') if p.suffix in {'.tla','.cfg'} and '_TTrace_' not in p.name)
...
checks=[('consequence',['cargo','test','--locked','-p','agentgres','--lib','consequence::tests::']),
 ('types',['cargo','test','--locked','-p','ioi-types','--lib','app::consequence::tests']),
 ...
 ('formal',['bash','.github/scripts/run_aft_formal_checks.sh','--consequence-trace-lifetime-only']),
 ('t10',['bash','.github/scripts/run_aft_formal_checks.sh','--consequence-t10-only'])]
results=[]
...
put('completed.json',{'completed':now(),'source_changes':changed,'passing':passing,'scope':'scoped representation only; allocation/profile/refinement/full R2/review outstanding'})

...
const PQ_REGISTER_EVIDENCE_MAX_BYTES: usize = 16 * 1024;
// The outer JSON stores evidence as byte integers (at most four bytes each).
const PQ_REGISTER_RECORD_MAX_BYTES: usize = 4 * PQ_REGISTER_EVIDENCE_MAX_BYTES + 16 * 1024;
...
fn receipt_trace_limit(manifest: &EffectManifestV1) -> u64 {
    4 + match manifest.reconciliation {
        ReconciliationPolicyV1::LookupByIdempotencyKey {
            maximum_observations,
...
fn online_receipt_byte_bound(manifest: &EffectManifestV1) -> Result<Option<u64>, ConsequenceError> {
    ...
    let bound = manifest_bytes
        .checked_add(4 * ONLINE_AUDIT_MAX_BYTES as u64)
        .and_then(|n| n.checked_add(512 * receipt_trace_limit(manifest)))
        .and_then(|n| n.checked_add(PQ_REGISTER_RECORD_MAX_BYTES as u64 + 32 * 1024))
...
fn persist_receipt(
    ...
    if online_receipt_byte_bound(&receipt.manifest)?.is_some_and(|bound| bytes.len() as u64 > bound)
    {
        return Err(ConsequenceError::CorruptReceipt);
    }

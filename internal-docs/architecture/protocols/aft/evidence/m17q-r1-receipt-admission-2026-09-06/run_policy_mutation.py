...
mutations=[('policy-admission-fields',needle,'        ioi_types::app::QuvConsequenceStorageProfileV0::ROOTED_FIELDS,')]
archive=base/'mutation-sources';archive.mkdir(exist_ok=True)
(archive/'policy.baseline.rs').write_bytes(original)
command_base=['cargo','test','--locked','-p','ioi-consensus','--features','aft','--lib']
results=[]
try:
 for name,needle,replacement in mutations:
  test='policy_root_binds_authority_and_complete_timing_bounds'
 ...
 (base/'policy-mutations.json').write_text(json.dumps({'results':results,'baseline_sha256':hashlib.sha256(original).hexdigest(),'restored':p.read_bytes()==original},indent=2)+'\n')

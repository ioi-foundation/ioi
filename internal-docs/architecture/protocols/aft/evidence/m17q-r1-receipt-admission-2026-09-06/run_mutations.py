...
mutations=[('owned-capacity','self.enter_receipts(&self.waiting_owned_receipt).await','self.enter_receipts(self.waiting_receipts.values().next().unwrap()).await'),
 ('historical-capacity','.unwrap_or(&self.waiting_historical_receipt);','.unwrap_or(&self.waiting_owned_receipt);')]
archive=base/'mutation-sources';archive.mkdir(exist_ok=True)
(archive/'admission.baseline.rs').write_bytes(original)
command_base=['cargo','test','--locked','-p','ioi-validator','--features','consensus-aft','--lib']
results=[]
try:
 for name,needle,replacement in mutations:
  test='receipt_admission_bounds_history_preserves_fifo_and_protects_live_reopen'
  command=command_base+['standard::orchestration::quv::admission::tests::'+test,'--','--exact']

...
use ioi_types::app::{QuvConsequenceAdmissionProfileV0 as ReceiptProfile, QuvHash};
...
impl QuvOperationAdmissionV0 {
    ...
    pub(crate) async fn receipt_access(&self, domain: QuvHash) -> Result<OwnedSemaphorePermit> {
        let waiting = self
            .waiting_receipts
            .get(&domain)
            .unwrap_or(&self.waiting_owned_receipt);
    ...
    pub(crate) async fn owned_receipt_access(&self) -> Result<OwnedSemaphorePermit> {
        self.enter_receipts(&self.waiting_owned_receipt).await
    }

...
impl QuvMemberDomainV0 {
    ...
    pub(super) fn projected_record_growth(
        ...
        self.projected_retained_growth(authorization.slot(), authorization.candidate_hash)
    }

    pub(super) fn projected_retained_growth(
        &self,
        slot: &QuvSlotV0,
        ...
        match self {
            ...
            Self::Fixed(history) => {
                let offset = usize::try_from(slot.slot - history.initial.slot)
                    .map_err(|_| QuvError::UnexpectedHead)?;
                if offset < history.accepted.len() {
                    return if history.accepted[offset] == candidate_hash {
                        Ok(0)
                    } else {
                        Err(QuvError::ConflictingAcceptedHistory)
                    };
    ...
    /// Stage retained history after the caller validates its own live grant,
    /// or restore history during authenticated journal recovery. This does not
    /// create an online authorization, consume a continuation, or authorize an
    /// external effect. The caller must retain the corresponding candidate.
    pub(super) fn stage_retained_acceptance(
        &mut self,
        slot: &QuvSlotV0,
        candidate_hash: QuvHash,
    ) -> Result<(), QuvError> {
        self.check_slot(slot)?;
        let Self::Fixed(history) = self else {
            return Err(QuvError::InvalidAcceptedHistory);
        };
        if history.next_slot() != Some(slot.slot) || candidate_hash == [0; 32] {
            return Err(QuvError::InvalidAcceptedHistory);
        }
        if history.accepted.len() >= STORE_MAX_SLOTS {
            return Err(QuvError::StoreCapacityExceeded);
        }
        history.accepted.push(candidate_hash);
        Ok(())
    }
}

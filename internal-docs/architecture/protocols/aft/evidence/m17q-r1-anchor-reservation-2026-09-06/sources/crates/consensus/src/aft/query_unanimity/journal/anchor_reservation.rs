...
pub(super) struct AnchorReservation {
    path: PathBuf,
    size: usize,
}
pub(super) struct PreparedAnchor {
    path: PathBuf,
    inactive: PathBuf,
    file: File,
    size: usize,
...
impl AnchorReservation {
    ...
    pub(super) fn prepare(path: &Path, current: &[u8]) -> Result<Self, QuvError> {
    ...
    pub(super) fn prepare_update(&self, raw: &[u8]) -> Result<PreparedAnchor, QuvError> {
...
impl PreparedAnchor {
    pub(super) fn commit(self, raw: &[u8]) -> Result<(), QuvError> {

...
def check(lines):
    ...
    # Startup, sixteen enqueue/retire commits, and final recovery startup.
    if len(exchanges) != 18:
        return False
    ...
    for i in exchanges[1:-1]:
        ...
        if not (re.search(r'fsync\(\d+<' + re.escape(spare) + r'>\)\s+= 0$', before)
            ...
            return False
    for line in lines[exchanges[0]+1:exchanges[-2]+1]:

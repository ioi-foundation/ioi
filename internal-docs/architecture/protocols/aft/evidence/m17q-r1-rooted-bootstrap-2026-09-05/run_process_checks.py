...
sources.update(["crates/types/src/app/query_unanimity.rs", "crates/types/src/config/mod.rs", "crates/types/src/config/tests.rs", "crates/cli/src/testing/cluster.rs"])
...
for label, test in plan:
    ...
    overrides = {"IOI_TEST_ORCH_RUST_LOG": "quv=debug,network=debug", "IOI_AFT_BENCH_TRACE_DIR": str(components)}
    ...
    if run.returncode == 0:
        ...
        if label == "correct-member-campaign":
            checker = ["python3", ".github/scripts/check_aft_m16q_process_evidence.py", str(directory / "process.log"), "--components", str(components)]

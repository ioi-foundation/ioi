...
const POLICY_ROOT_DOMAIN_V0: &[u8] = b"ioi/aft/quv-policy/v3-operation-service";
...
impl DurableQuvMemberV0 {
        ...
        let (state, head) = match (path.exists(), anchor_path.exists()) {
            ...
            (true, true) => {
                ...
                if state.provisioning_root != provisioning_root
                    || state.domains.len() != domains.len()
                    || state.domains.iter().any(|(id, stored)| {
                        !domains
                            .get(id)
                            .is_some_and(|expected| stored.same_bootstrap(expected))
                    })
                {
    ...
    fn reserve_preparation_attempt_with_byte_limit(
        ...
        let growth = if previous.is_some() {
            0
        } else {
            32 + 8 + 2 + compact_length_size(self.state.preparation_attempts.len() + 1)?
                - compact_length_size(self.state.preparation_attempts.len())?
        };
        ...
        let mut next = self.state.clone();
        next.preparation_attempts.insert(
        ...
        let head = store_head(&next)?;
        let encoded = codec::to_bytes_canonical(&next).map_err(QuvError::Codec)?;
        self.persistence_failed = true;
        persist_atomic(&self.path, &encoded)?;
        persist_anchor(&self.anchor_path, &self.custody_key, next.generation, head)?;
        self.state = next;
        self.head = head;
        self.persistence_failed = false;
        Ok(used + 1)
    ...
    fn advance_accepted_history_with_byte_limit(
        ...
        if growth == 0 {
            self.head_committed_at.insert(authorization.slot.domain_id, Instant::now());
            return Ok(false);
        }
        let retired_attempt_bytes = if self
            .state
            .preparation_attempts
            .contains_key(&authorization.slot.domain_id)
        {
            32 + 8 + 2 + compact_length_size(self.state.preparation_attempts.len())?
                - compact_length_size(self.state.preparation_attempts.len() - 1)?
        } else {
            0
        };
        let next_size = self
            .state
            .encoded_size()
            .checked_sub(retired_attempt_bytes)
            .ok_or(QuvError::StoreCapacityExceeded)?
            .checked_add(growth)
            .ok_or(QuvError::StoreCapacityExceeded)?;
        ...
        let advanced = next
            .domains
            .get_mut(&authorization.slot.domain_id)
            .ok_or(QuvError::UnexpectedHead)?
            .record_live_authorization(authorization)?;
        if !advanced {
            return Ok(false);
        }
        // Only the durable successful head transition retires the old budget.
        next.preparation_attempts
            .remove(&authorization.slot.domain_id);
        next.generation = next
            .generation
            .checked_add(1)
            .ok_or(QuvError::GenerationExhausted)?;
        ...
        let encoded = codec::to_bytes_canonical(&next).map_err(QuvError::Codec)?;
        // Recheck immediately before the first durable write; staging/encoding
        // must not extend a process-local continuation's lifetime.
        if Instant::now() >= authorization.expires_at {
            return Err(QuvError::ExpiredAuthorization);
        }
        self.persistence_failed = true;
...
fn validate_store(state: &QuvStoreStateV0, custody_key: &QuvHash) -> Result<(), QuvError> {
    ...
    validate_member_domains(&state.domains).map_err(|_| QuvError::CorruptStore)?;
    for (id, attempts) in &state.preparation_attempts {
        let slot = state
            .domains
            .get(id)
            .and_then(QuvMemberDomainV0::next_query_slot)
            .ok_or(QuvError::CorruptStore)?;
        if attempts.used == 0
            || attempts.slot != slot.slot
            || !state.slots.contains_key(&QuvConflictSlotV0::from(&slot))
        {
            return Err(QuvError::CorruptStore);
        }
    }
    for (slot, candidates) in &state.slots {
        ...
        for candidate in candidates {
            if state
                ...
                .is_err()
                || QuvConflictSlotV0::from(&candidate.slot) != *slot
                || !hashes.insert(quv_candidate_hash(candidate)?)
            {
                return Err(QuvError::CorruptStore);
            }
    ...
    // Recovery must retain the snapshot underlying every accepted transition,
    // including the last entry, which has no later predecessor to cross-check.
    for domain in state.domains.values() {
        for (slot, accepted_hash) in domain.accepted_entries() {
            if !state
                .slots
                ...
                .is_some_and(|snapshot| {
                    snapshot.iter().any(|candidate| {
                        candidate.slot == slot
                            && quv_candidate_hash(candidate).ok() == Some(accepted_hash)
                    })
                })
            {
                return Err(QuvError::CorruptStore);
...
mod tests {
    ...
    fn test_domains(initial: QuvSlotV0) -> BTreeMap<QuvHash, QuvMemberDomainV0> {
        BTreeMap::from([(
            ...
            QuvMemberDomainV0::Fixed(
                QuvAcceptedHistoryV0::from_provisioned_initial(initial).unwrap(),
            ),
        )])
    }
    ...
    fn preparation_selection_recovers_and_rotates_without_advancing_history() {
        ...
        let before_anchor = std::fs::read(&anchor_path).unwrap();
        assert_eq!(
        ...
        assert_eq!(std::fs::read(&anchor_path).unwrap(), before_anchor);
        drop(member);
        let mut recovered =
    ...
    fn durable_history_requires_live_advance_and_retains_historical_queries() {
        for mode in [QuvAuthorityModeV0::Owned, QuvAuthorityModeV0::Unowned] {
            ...
            let state_before = std::fs::read(&member.path).unwrap();
            let anchor_before = std::fs::read(&member.anchor_path).unwrap();
            assert!(matches!(
                member.process_push(
                    &QuvPushQueryV0 {
                        verifier_nonce: [4; 32],
            ...
            assert_eq!(std::fs::read(&member.path).unwrap(), state_before);
            assert!(member.advance_accepted_history(&authorization).unwrap());
            let current_state = std::fs::read(&member.path).unwrap();
            let current_anchor = std::fs::read(&member.anchor_path).unwrap();
            assert!(!member.advance_accepted_history(&authorization).unwrap());
            ...
            for retained_anchor in [&current_anchor, &anchor_before] {
                std::fs::write(&anchor, retained_anchor).unwrap();
                assert!(matches!(
                    DurableQuvMemberV0::open(
                        &path,
                        &anchor,
                ...
                assert_eq!(std::fs::read(&path).unwrap(), current_state);
                assert_eq!(&std::fs::read(&anchor).unwrap(), retained_anchor);
            }
            let mut recovered = open_member_at(&temp, initial);
            assert_eq!(std::fs::read(&anchor).unwrap(), current_anchor);
    ...
    fn preparation_attempts_are_rooted_durable_and_retire_only_on_advance() {
        for mode in [QuvAuthorityModeV0::Owned, QuvAuthorityModeV0::Unowned] {
            ...
            let before = std::fs::read(&member.path).unwrap();
            let anchor_before = std::fs::read(&member.anchor_path).unwrap();
            ...
            assert_eq!(std::fs::read(&member.path).unwrap(), before);
            assert_eq!(std::fs::read(&member.anchor_path).unwrap(), anchor_before);
            assert_eq!(
            ...
            member
                ...
                .unwrap();
            assert_eq!(
                std::fs::read(&member.path).unwrap().len() as u64,
                advanced_size
            );
    ...
    fn preparation_attempts_preflight_authenticate_and_recover_uncertain_write() {
        ...
        assert_eq!(std::fs::read(&path).unwrap().len() as u64, exact);
        assert!(matches!(
            member.reserve_preparation_attempt(&value, &policy),
            Err(QuvError::StoreRequiresReopen)
        ));
        ...
        std::fs::write(&anchor, &anchor_before).unwrap();
        let pending: QuvStoreStateV0 = read_canonical(&path).unwrap();
        for tamper_slot in [false, true] {
            let mut corrupt = pending.clone();
            let attempts = corrupt
                .preparation_attempts
                .get_mut(&initial.domain_id)
                .unwrap();
            if tamper_slot {
                attempts.slot += 1;
            } else {
            ...
            let bytes = codec::to_bytes_canonical(&corrupt).unwrap();
        ...
        std::fs::write(&path, codec::to_bytes_canonical(&pending).unwrap()).unwrap();
        let mut recovered = open_member_at(&temp, initial);
        assert_eq!(
            recovered
    ...
    fn recovery_requires_snapshot_for_every_accepted_history_entry() {
        for mode in [QuvAuthorityModeV0::Owned, QuvAuthorityModeV0::Unowned] {
            ...
            let pending_anchor = std::fs::read(&member.anchor_path).unwrap();
            member.advance_accepted_history(&grant).unwrap();
            let valid = member.state.clone();
            let path = member.path.clone();
            let anchor = member.anchor_path.clone();
            drop(member);
            // Positive control: both complete entries survive normal reopen.
            drop(open_member_at(&temp, initial.clone()));
            for retained in [&first, &second] {
                for remove in [false, true] {
                    let mut inconsistent = valid.clone();
                    let coordinate = QuvConflictSlotV0::from(&retained.slot);
                    if remove {
                        inconsistent.slots.remove(&coordinate);
                    } else {
                        // Well-formed candidate at the same coordinate, but not
                        // the candidate recorded by the accepted transition.
                        inconsistent.slots.get_mut(&coordinate).unwrap()[0].payload_hash = [99; 32];
                    }
                    // Re-authenticate this synthetic inconsistent fixture so
                    // the structural check, rather than MAC rejection, is tested.
                    inconsistent.authentication_tag =
                        store_state_tag(&[8; 32], &inconsistent).unwrap();
                    let bytes = codec::to_bytes_canonical(&inconsistent).unwrap();
                    for pending in [false, true] {
                        std::fs::write(&path, &bytes).unwrap();
                        if pending {
                            std::fs::write(&anchor, &pending_anchor).unwrap();
                        } else {
                            persist_anchor(
                                &anchor,
                                &[8; 32],
                                inconsistent.generation,
                                store_head(&inconsistent).unwrap(),
                            )
                            .unwrap();
                        }
                        let anchor_before = std::fs::read(&anchor).unwrap();
    ...
    fn authenticated_advanced_history_covers_every_byte_in_pending_recovery() {
        ...
        let authentic_state = std::fs::read(&state_path).unwrap();
        drop(member);
        std::fs::write(&anchor_path, pending_anchor).unwrap();
    ...
    fn accepted_history_derives_scope_and_preserves_historical_predecessors() {
        ...
        let mut history = QuvAcceptedHistoryV0::from_provisioned_initial(initial.clone()).unwrap();
    ...
    fn accepted_history_refuses_conflicting_or_expired_grants_without_mutation() {
        ...
        for observed in [first.expires_at, first.expires_at + Duration::from_nanos(1)] {
            let mut fresh =
                QuvAcceptedHistoryV0::from_provisioned_initial(initial.clone()).unwrap();
        ...
        let mut before_expiry = QuvAcceptedHistoryV0::from_provisioned_initial(initial).unwrap();
    ...
    fn accepted_history_supports_unowned_grants_and_rejects_invalid_bootstrap() {
        ...
        for invalid in [
            ...
            assert!(matches!(
                QuvAcceptedHistoryV0::from_provisioned_initial(invalid),
        ...
        let bytes = codec::to_bytes_canonical(&(initial, vec![[0_u8; 32]])).unwrap();
    ...
    fn accepted_history_terminal_slot_does_not_wrap_or_erase_history() {
        ...
        let mut history =
            QuvAcceptedHistoryV0::from_provisioned_initial(value.slot.clone()).unwrap();
    ...
    fn unauthenticated_crash_window_state_cannot_advance_member_anchor() {
        ...
        let state_path = temp.path().join("state/quv.scale");
        let anchor_path = temp.path().join("anchor/quv.anchor");
        let member = DurableQuvMemberV0::open(
            &state_path,
            &anchor_path,
            [8; 32],
            [6; 32],

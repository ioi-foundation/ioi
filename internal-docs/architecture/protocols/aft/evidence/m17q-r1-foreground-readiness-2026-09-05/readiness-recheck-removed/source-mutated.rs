...
fn require_readiness_elapsed(deadline: Option<Instant>, observed: Instant) -> Result<()> {
    if false && deadline.is_some_and(|deadline| observed < deadline) {
        ...
        + Debug,
{
    ...
    let admission = if independent_preparation {
        ...
        let waiting = gate.reserve_foreground(request.candidate.slot.domain_id)?;
        ...
        if deadline.is_some() {
            tracing::debug!(target: "quv", event = "foreground_readiness_wait", nonce = %hex::encode(request.verifier_nonce), domain = %hex::encode(request.candidate.slot.domain_id));
        }
        waiting.enter_after(deadline).await?
    };

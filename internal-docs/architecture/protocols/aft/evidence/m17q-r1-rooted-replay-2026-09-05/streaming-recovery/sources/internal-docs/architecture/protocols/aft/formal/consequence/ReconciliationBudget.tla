----------------------- MODULE ReconciliationBudget -----------------------
EXTENDS Naturals
CONSTANT MaxAttempts
VARIABLES legacy, reserved, calls, ready
vars == <<legacy, reserved, calls, ready>>

\* Component model only. Reserve is an atomic non-rollback durable write.
\* Crash destroys process-local readiness, never the durable reservation.
...
Crash == /\ ready' = FALSE
         /\ UNCHANGED <<legacy, reserved, calls>>

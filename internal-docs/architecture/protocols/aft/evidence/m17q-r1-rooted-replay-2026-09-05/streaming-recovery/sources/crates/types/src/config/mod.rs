    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if !self.aft_quv_domain_policies.is_empty() {
            ...
            for policy in &self.aft_quv_domain_policies {
                ...
                {
                    return Err("Configuration Error: aft_quv_v0 policies require compatible explicit bootstrap/preparation budgets, a nonzero domain and continuation_millis, a nonzero qualified_delta_rt_envelope_millis no larger than delta_rt_millis, and a qualified_max_configured_members within the protocol cap.".to_string());
                }

...
fn receipt_preparation_reuses_valid_pair_and_repairs_only_incomplete_spare() {
    ...
    let before = [&active, &spare].map(|path| {
    ...
    let after = [&active, &spare].map(|path| {
        (fs::metadata(path).unwrap().ino(), fs::read(path).unwrap())
    });
    assert_eq!(before, after, "valid prepared files must not be rewritten or exchanged");
    assert_eq!(receipt_reservation::read(&active).unwrap().0, b"current receipt");
    ...
    assert_eq!(receipt_reservation::read(&active).unwrap().0, b"next receipt");
    ...
    assert!(matches!(receipt_reservation::prepare(&active, 16384), Err(ConsequenceError::CorruptReceipt)));

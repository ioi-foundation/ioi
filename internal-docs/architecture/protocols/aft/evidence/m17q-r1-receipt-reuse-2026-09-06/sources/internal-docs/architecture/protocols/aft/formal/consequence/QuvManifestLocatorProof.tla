...
ASSUME Profile == /\ Owner \in [Records -> Effects]
                  ...
                  /\ EnforceRegistryIdentity \in BOOLEAN
Inv == TypeOK /\ IndexCorrect /\ ReadyComplete /\ UniqueWhenEnforced
...
<1>1. Init => Inv
  BY Profile, SMT DEF Profile, Init, Inv, TypeOK, IndexCorrect, CellCorrect,
    ReadyComplete, UniqueWhenEnforced, RegistryUnique, EmptyIndex
<1>2. Inv /\ [Next]_vars => Inv'
  BY Profile, SMT DEF Profile, Inv, TypeOK, IndexCorrect, CellCorrect,
    ReadyComplete, UniqueWhenEnforced, RegistryUnique, Next, Admit, Crash, RecoverRecord, FinishRecovery,

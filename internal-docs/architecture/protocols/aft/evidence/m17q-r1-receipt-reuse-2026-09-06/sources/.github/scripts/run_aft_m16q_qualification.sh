...
record_metadata() {
  ...
  sha256sum \
    ...
    .github/scripts/check_aft_endpoint_reservation.py \
    .github/scripts/check_aft_receipt_reservation.py \
    ...
    internal-docs/architecture/protocols/aft/formal/concurrency/QuvReceiptAdmission.cfg \
    internal-docs/architecture/protocols/aft/formal/concurrency/QuvReceiptAdmissionBypass.cfg \
    ...
    internal-docs/architecture/protocols/aft/formal/concurrency/QuvContinuationAdmission.cfg \
    internal-docs/architecture/protocols/aft/formal/concurrency/QuvContinuationAdmission.tla \
    internal-docs/architecture/protocols/aft/formal/concurrency/QuvContinuationAdmissionCancel.cfg \
    internal-docs/architecture/protocols/aft/formal/concurrency/QuvContinuationAdmissionEarly.cfg \
    internal-docs/architecture/protocols/aft/formal/concurrency/QuvContinuationAdmissionProof.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvQueuedEffectPreparation.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvQueuedEffectPreparation.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvQueuedEffectPreparationEarly.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvQueuedEffectPreparationLocked.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvQueuedEffectPreparationProof.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvQueuedEffectPreparationStale.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvQueuedEffectPreparationFinal.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvEffectPreparation.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvEffectPreparationProof.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvEffectPreparation.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvEffectPreparationBinding.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvEffectPreparationPreflight.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvEffectPreparationBearer.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvEffectPreparationTerminal.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvManifestLocator.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvManifestLocatorProof.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvManifestLocator.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvRegistryBootstrap.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvRegistryBootstrapProof.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvRegistryBootstrap.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvRegistryBootstrapHiddenError.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvReceiptReuse.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvReceiptReuseProof.tla \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvReceiptReuse.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvReceiptReuseRewrite.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvReceiptReuseFallback.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvRegistryIdentity.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvRegistryIdentityDuplicate.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvManifestLocatorOverwrite.cfg \
    internal-docs/architecture/protocols/aft/formal/consequence/QuvManifestLocatorEarlyRecovery.cfg \
    ...
    crates/api/src/state/overlay.rs \
...
# Full qualification executes the complete scheduled formal corpus, including
# every QUV repair proof/model/negative and the production trace replay. Scoped
# --*-only commands are development checks and cannot substitute for this gate.
run_phase formal_complete bash .github/scripts/run_aft_formal_checks.sh
...
run_phase endpoint_reservation python3 .github/scripts/check_aft_endpoint_reservation.py --output "${OUTPUT_DIR}/endpoint-reservation"
run_phase receipt_reservation python3 .github/scripts/check_aft_receipt_reservation.py --output "${OUTPUT_DIR}/receipt-reservation"
...
if ! rg -q '^test standard::orchestration::quv::tests::reserved_preparation_owns_capacity_and_checks_service_edges \.\.\. ok$' "${OUTPUT_DIR}/quv_runtime_policy.log"; then
  echo '[M16Q] FAIL quv_runtime_policy: missing reserved preparation deadline/ownership regression' >&2
  exit 1
fi
for required_case in receipt_admission_bounds_history_preserves_fifo_and_protects_live_reopen delivered_continuation_holds_admission_through_effect_and_cancelled_worker preparation_waiter_is_bounded_and_cancellation_preserves_fifo readiness_wait_keeps_domain_capacity_and_leaves_preparation_lane_free pending_removal_keeps_admission_until_dispatch_returns_or_is_canceled admission_release_is_observed_after_semaphore_release canceled_durable_waiter_retains_admission_until_work_finishes admission_serializes_and_preserves_queued_worker_order admission_cancellation_releases_domain_and_queue_capacity; do
...
run_phase state_overlay_scan cargo test --locked -p ioi-api --lib state::overlay::tests::
require_tests_ran state_overlay_scan
for required_case in merge_preserves_scan_errors_with_and_without_pending_writes merge_keeps_order_overrides_and_deletes_around_scan_failure; do
  if ! rg -q "^test state::overlay::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/state_overlay_scan.log"; then
    echo "[M16Q] FAIL state_overlay_scan: missing regression ${required_case}" >&2
    exit 1
  fi
done
...
for required_case in identity_substitution_refuses_without_invalidating_prior_admission unindexed_or_unknown_registry_schema_refuses_without_reinitializing registry_scan_failure_through_overlay_refuses_without_bootstrap_writes; do
  if ! rg -q "^test aft_effect_registry::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/manifest_admission.log"; then
    echo "[M16Q] FAIL manifest_admission: missing identity/schema regression ${required_case}" >&2
    exit 1
  fi
done
...
for required_case in receipt_preparation_reuses_valid_pair_and_repairs_only_incomplete_spare receipt_admission_preserves_invalid_active_and_staging_aliases consequence_lock_refuses_aliases_and_retains_exclusive_ownership checked_preparation_refuses_before_storage_and_keeps_terminal_readmission_non_authorizing online_receipt_bound_covers_named_resource_execution_and_complete_budget receipt_hash_view_preserves_canonical_bytes_and_optional_fields receipt_trace_bound_refuses_extra_ambiguity_without_mutating_state pq_resource_format_bounds_cover_maximum_tokens_and_refuse_noncanonical_evidence; do

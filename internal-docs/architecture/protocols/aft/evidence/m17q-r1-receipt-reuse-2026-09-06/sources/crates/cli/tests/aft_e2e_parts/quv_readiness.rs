        ...
        let cases = async {
            ...
            for index in 1..3 {
                ...
                if index == 1 {
                    ...
                    // Sustain exact terminal-result traffic across the unrelated
                    // executor's complete own-live operation and store reopen.
                    let (stop_pressure, pressure_stopped) = tokio::sync::watch::channel(false);
                    let pressure = async {
                        ...
                        loop {
                            let replay = tokio::time::timeout(Duration::from_secs(5),
                                rpc::execute_aft_quv_effect(&rpc_addr, &manifests[0].effect_id, &candidates[0])).await??;
                            if replay.portable_final_receipt || replay.consequence_receipt_jcs != first.consequence_receipt_jcs {
                                return Err(anyhow::anyhow!("terminal pressure changed the exact parent result"));
                            }
                            count += 1;
                            if *pressure_stopped.borrow() && count >= 2 { break; }
                            if count == 4096 { return Err(anyhow::anyhow!("terminal pressure exhausted its finite request budget before unrelated completion")); }
                        }
                    ...
                    let probe_call = async {
                        ...
                        let _ = stop_pressure.send(true);
                        result
                    };
                    let (pressure, unrelated) = tokio::join!(pressure, probe_call);
                    ...
                    println!("[M16Q-READINESS] case=unrelated_during_wait nonce={probe_nonce} result=executed terminal_parent_replay=true pressure_receipts={pressure_receipts} pressure_span_millis={pressure_span_millis}");
                }

...
impl QuvMemberDomainV0 {
    ...
    /// Restore one committed transition during authenticated journal recovery.
    /// This does not create an online authorization, consume a continuation, or
    /// authorize an external effect. The caller must authenticate custody and
    /// retain the corresponding candidate before invoking this method.
    #[cfg(test)]
    pub(super) fn restore_authenticated_acceptance(
        &mut self,
        slot: &QuvSlotV0,

...
//! Kept test-only until the production storage format replacement is complete.
...
/// Replay only after the complete journal chain and independently provisioned
/// bootstrap have authenticated. The state must be unpublished temporary memory.
/// Rooted candidate authentication and the preparation policy limit are live
/// admission checks; replay preserves the already committed transitions and
/// validates their ordering, exact scope, retained candidate, and counters.
pub(super) fn replay_member_delta(
    ...
    match delta {
        ...
        MemberDelta::AcceptHead {
            ...
            state
                ...
                .ok_or(QuvError::UnexpectedHead)?
                .restore_authenticated_acceptance(slot, *candidate_hash)?;

...
// Storage replacement under component qualification; production integration follows.
#[cfg(test)]
mod journal;
mod member_delta;
...
impl DurableQuvMemberV0 {
    ...
    fn reserve_preparation_attempt_with_byte_limit(
        ...
        let prepared = member_delta::prepare_member_delta(
            &self.state,
            self.state
                .generation
                .checked_add(1)
        ...
        require_store_byte_capacity(
            prepared.projected_size(&self.state, self.state.encoded_size())?,
            max_bytes,
        )?;
        let mut next = self.state.clone();
        prepared.apply(&mut next)?;
        next.previous_head = self.head;
    ...
    fn advance_accepted_history_with_byte_limit(
        ...
        let mut next = self.state.clone();
        // The caller's live grant is rechecked immediately before persistence.
        prepared.apply(&mut next)?;
        next.previous_head = self.head;
        require_store_byte_capacity(next.encoded_size(), max_bytes)?;
        next.authentication_tag = store_state_tag(&self.custody_key, &next)?;

...
| M11 exact maximal task/model | **NOT STARTED** | Define non-vacuous agreement, validity, termination, effect liveness, external verification, adversary, communication, storage, setup, and `f=n-1` semantics |
| M12 non-circular visibility viability | **NOT STARTED** | Requires `PASS_CONSTRUCTION`; a formal impossibility is retained evidence but blocks M13-M18 |

...
| M15Q QUV production implementation | **IN PROGRESS — CRITICAL PATH** | Commit `e8ec2dc44` proves the live Q-EA7 path in an eight-process release fixture with disjoint four-member old/successor roots: exact old-root QC formation, non-authoritative owner draft, signed-source provisioning, successor-only state import, four independent live QUV operations, rollback-anchored install, strict-PQ promotion, and successor post-handoff progress all pass. The handoff payload binds the boundary QC; each old member compares it with its own verified QC, and each successor re-authenticates it under the old root before successor-set hydration. No cached transcript authorizes activation and `portable_final_receipt=false` remains enforced. Process restart/recovery, operator source tooling, and the complete executor-to-external-resource path still gate M15Q; M16Q has not begun |

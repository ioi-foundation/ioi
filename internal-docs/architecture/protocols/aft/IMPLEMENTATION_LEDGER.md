...
| M15Q QUV production implementation | **IN PROGRESS — CRITICAL PATH** | Commits `e8ec2dc44`, `252e03573`, `fc7c9b926`, and `bd1e91a6d` prove the live Q-EA7, restart, and executor-to-T10 path in an eight-process release fixture with disjoint four-member roots. It uses the fail-closed `cli aft quv-handoff` ceremony, four successor-local live operations, rollback-anchored exact-envelope installs, strict-PQ promotion, exact-gate restart without QUV/QC replay, and post-restart progress. A finalized canonical effect manifest is bound through Agentgres; the relying executor performs fresh QUV and directly consumes its process-local continuation before `Claimed`, then the PQ atomic resource mutates once and returns verified ML-DSA evidence. The response remains non-authorizing with `portable_final_receipt=false`. The same process run refuses source-signature substitution, a missing gate half after expiry, and retired-old-key authority. Pre-install/in-flight and rollback-image process restarts plus the overlapping-root case still gate M15Q; M16Q has not begun |

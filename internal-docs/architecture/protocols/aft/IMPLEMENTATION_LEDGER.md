...
| M15Q QUV production implementation | **COMPLETE LOCALLY; M17Q REVIEW REQUIRED** | Commits `e8ec2dc44`, `252e03573`, `fc7c9b926`, `bd1e91a6d`, and `ce34c31a9` prove the live Q-EA7, ceremony, executor-to-T10, and complete restart path. The eight-process disjoint-root release fixture covers pre-source inactivity, a real process exit after state fsync and before rollback-anchor advance, exact one-generation recovery, post-restart canonical progress, fresh executor QUV immediately consumed by T10, rollback-image refusal, source substitution, missing gate, and retired-key refusal. A seven-process fixture proves one-member-overlap activation and exact-gate restart even when other successors advance first. The source boundary QC is independently threshold-verified, alternate valid QC encodings are compared by certified block identity, audit remains non-authorizing, and `portable_final_receipt=false`. The default Hypervisor dependency tree still excludes consensus/validator |
| M16Q QUV adversarial/performance qualification | **COMPLETE** | The clean full runner passed all 15 formal, unit, mutation, timing, process, reconfiguration, consequence, PQ-profile, restart, and Hypervisor phases against exact code commit `ab8d2e58103a2eef3e39c1c6042ffefd7d3c86f4`. The retained run is `evidence/m16q-runs/20260904T204403Z-ab8d2e58103a`; `tree_dirty=false`, all four sole-correct placements executed, concurrent conflicting accepts were zero, an unrelated domain executed after conflict, and the saturated valid-reply maximum was 2722 ms inside the qualified 4000 ms envelope |
| M17Q independent QUV review | **IN PROGRESS — CRITICAL PATH** | Freeze the evidence-bearing M17Q candidate and obtain the owner-authorized fresh Daybreak source/theorem review, clean full reproduction, independent spec-only executable twin, independence disclosure, and stable attributable findings. M12b construction review does not satisfy this later gate |

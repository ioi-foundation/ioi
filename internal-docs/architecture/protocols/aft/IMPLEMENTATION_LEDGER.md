...
| M15Q QUV production implementation | **IN PROGRESS — CRITICAL PATH** | Commits `e8ec2dc44`, `252e03573`, and `fc7c9b926` prove the live Q-EA7 and restart path in an eight-process release fixture with disjoint four-member roots. It uses the fail-closed `cli aft quv-handoff` ceremony, four successor-local live operations, rollback-anchored exact-envelope installs, strict-PQ promotion, exact-gate restart without QUV/QC replay, and post-restart progress. The same process run refuses source-signature substitution, a missing gate half after expiry, and retired-old-key authority. Raw workload execution status is separate from collapse-demoted public status and carries no finality. In-flight/rollback-image and overlapping-root process restarts plus the complete executor-to-external-resource path still gate M15Q; M16Q has not begun |

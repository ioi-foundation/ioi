...
| M10 independent PQ v1 review and release | **REPAIR REQUIRED — CRITICAL PATH** | The ADR-0049-authorized Daybreak R3 retest closed AFT-M10-006/007 and found no open critical/high issue, but exact four-validator cold restart timed out provisioning `ioi-signer` after reaching height 5 and never demonstrated post-restart height 7. AFT-M10-003 remains open; signer startup/readiness must be repaired, frozen, and independently retested before M1/M8/M10 release admission |
...
| M12b interactive visibility | **R4 LOCAL REPAIRS COMPLETE; REVIEW PENDING** | Independent Daybreak review found R3 Q-A3 permitted different timely correct witnesses across operations and found the liveness counter partly vacuous. R4 requires every correct reply within the bound, has zero conflicts in all-correct-timely `H=2..3` rows, recovers the split-witness counterexample, and gives every authority mode a fresh-state exactly-one-candidate liveness campaign with zero failures. Freeze and independent retest remain mandatory before `PASS_CONSTRUCTION` |
...
| M13Q online QUV theorem | **BLOCKED ON M12b REVIEW** | After independently reviewed `PASS_CONSTRUCTION`, prove arbitrary-`n` Q-S1/Q-L1, exact validity/termination semantics, multiple-correct concurrency, resource-bounded synchrony, lower-bound pairings, and mechanization. Bounded `n=2` Python evidence is insufficient |
| M14Q online end-to-end theorem lift | **NOT STARTED** | Requires M13Q; must cover predecessor-bound ordering, durable state, executor-side QUV immediately before effects, recovery/reconfiguration, and consequence semantics without claiming a portable final receipt |
| M15Q QUV production implementation | **NOT STARTED** | Forbidden until M13Q/M14Q pass; must be a named profile, never a relabel or timeout fallback, and every mutation owner must perform the online check |

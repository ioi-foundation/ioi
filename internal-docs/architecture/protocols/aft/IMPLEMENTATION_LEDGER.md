...
| M10 independent PQ v1 review and release | **PUBLIC REVIEW REQUEST OPEN — CRITICAL PATH** | Candidate and tag are published; public commission [#357](https://github.com/ioi-foundation/ioi/issues/357) is open and unassigned. A qualified independent reviewer must still be selected and return complete findings, remediation/retest, a final attributable report, and no unresolved high/critical finding; opening an issue is not review evidence |
...
| M12 non-circular visibility viability | **PROVED_IMPOSSIBLE_UNDER_CONSTRAINTS** | Context-isolated `gpt-daybreak-blue-latest` review returned R2 `REPAIR_REQUIRED`; every finding was repaired, and exact R3 tag `aft-maximal-visibility-lower-bound-candidate-r3-2026-09-03` was retested `UPHELD`. Automated provenance is accepted for this owner-controlled gate under ADR 0049 and is not represented as human peer review. The role-switch theorem proves that participant-only transferable non-conflict and solo non-`Abort` progress cannot coexist at `f=n-1` without an added selector/authority |
| M13 maximal consensus theorem | **BLOCKED BY PROVED IMPOSSIBILITY** | Only `PASS_CONSTRUCTION` unlocks this milestone. Resume requires an explicit owner decision changing a task property or permitting and naming a stronger assumption |

# Persistent goal prompt — AFT maximal consensus end to end

Use the following prompt as a persistent goal. Do not assign a token budget
unless the owner explicitly requests one.

```text
Bring AFT from its current M0-M8 PQ v1 candidate through independent review,
release admission, maximal-theorem viability, theorem construction, end-to-end
effect integration, production implementation, adversarial qualification, and
...
claim adjudication, M8 audit, and P4.5a review packet. Inspect the actual Git
state and preserve unrelated or pre-existing user changes. Keep one milestone
marked critical path and update the ledger and evidence after each material
...
3. Specify the maximal consensus task and adversary exactly. Define agreement,
   all validity variants, Abort/no-op, decision termination, inclusion/effect
   liveness, external-verifier semantics, f=n-1, corruption timing, delivery,
   synchrony, randomness, setup, storage, clocks, and reachability. Prevent
   one-honest agreement from becoming a vacuous headline.
4. Specify the canonical-public-state/visibility primitive independently,
   including publish/read/close/proof/recovery operations, implementers,
   consistency, availability, persistence, closure, costs, and all trust or
   timing assumptions.
5. Attack the deepest obligation first. Model n=2,f=1 and general f=n-1 with
   all Byzantine parties permanently silent; construct delayed-versus-dead
   paired executions; trace every bit of information and authority; attempt
...
10. Implement one explicitly named production profile using the exact
    ...
    replay protection, restart/rotation, receipt policy, and a single opaque
    verified-guarantee external mutation boundary. Keep Hypervisor's default
    graph free of consensus/validator/seal dependencies and measure its build.
11. Run process-level f=n-1 silence/equivocation, partition/eclipsing,
    visibility-fork, rollback, restart, rotation, long-range, mixed-domain,
    receipt-mutation, and consequence crash/reconciliation campaigns. Replay
    formal traces against production code and publish profile-specific costs.
12. Freeze a new immutable maximal candidate. Obtain fresh independent
    security and theorem reviews, a spec-only twin implementation, adversarial
    public exercise, and peer-review disposition. Fix and re-review all
    affected surfaces. External commissioning, spend, credentials, disclosure,

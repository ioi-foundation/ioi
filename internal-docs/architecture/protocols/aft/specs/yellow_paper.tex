...
\hypersetup{
  ...
  pdftitle={Asymptote Fault Tolerance (AFT): Maximal Deterministic Byzantine Safety - All-But-One (n-1 of n), Relay-Free, Fully Attributable - by Replacing the Classical Carrier},
  pdfauthor={IOI}
}
...
\title{Asymptote Fault Tolerance (AFT): Maximal Deterministic Byzantine Safety --- All-But-One (\(n-1\) of \(n\)), Relay-Free, Fully Attributable --- by Replacing the Classical Carrier}
\author{IOI}
\date{March 17, 2026}
...
\paragraph{Class C --- box-variable, deferred.} Boundary-close latency
and sustained throughput at \(n \in \{4, 8\}\) come from the existing
paper-benchmark harness; the instrument is built, and the figures await a
pinned benchmark runner because a number measured on a variable
development box would fail its own reproduce-within-variance gate
(Section~\ref{sec:residuals}). The native measurements earlier in this
section predate this discipline and are retained as architectural
positioning only.
...
at \(n \in \{4, 8\}\) await a pinned benchmark runner; the instrument

...
safety threshold ascends to its information-theoretic maximum.
\end{abstract}
...
asynchronous theorem; the relevant fact is that it states the exact
...
A5 & Post-GST synchrony as a full honest mesh (pairwise delivery within
the bound between honest parties) and bounded dishonest bonded weight.
Liveness, seal cadence, and the live tier only; no strong-lineage safety
claim cites A5. \\
...
\textit{Assumes A5.} Block production advances under the live engine's own
fault bound and partial-synchrony assumption, independently of the ring:
no Boundary Ring rule appears in the proposal, vote, or commit path. A
ring stall delays only the release of effects \emph{typed} irreversible
(unsealed-over-unsafe); the engine's asynchronous-fallback obligation is a
named residual (RES-R10), deliberately not claimed.
\textit{Mechanization:} the two-tier separation property of
\code{BoundaryLiveness.tla}. \textit{Pairing:} L-OPEN --- the row closes
only when the sim-exercised fallback lands.
...
\paragraph{T5d --- Pre-consented succession safety (withdrawn).}
\textit{Assumes A1, A2, A3, A9, A10.} The succession extension's safety
claim is \emph{withdrawn for this cycle}: three successive formulations
were refuted by the program's own adversarial reviews, and the operative
exits from a dead ring remain the unanimous handover and the labeled
re-genesis. \code{SuccessionClock.tla} survives as an honestly narrowed
kernel; the conditions a fourth formulation must meet are recorded in the
specification's status banner.
\textit{Pairing:} L-OPEN, claim withdrawn.

\paragraph{T6 --- Composition (the lattice meet).}
\textit{Assumes A1.} Every certificate profile carries a machine-readable
assumption label; the collapse verifier computes the meet over all
constituents; a composed object's effective guarantee is exactly its
weakest constituent's, including the post-quantum bit (AND). Reporting the
meet is exact --- anything stronger is unsound (L-M), anything weaker
discards guarantee --- and exhaustive matching over profiles makes a
silently unlabeled new profile a compile error.
\textit{Mechanization:} the runtime lattice's unit gates (meet, pq-meet,
exhaustive-match-by-construction). \textit{Pairing:} L-M (above-meet
unsoundness).
...
convicts the entire violating coalition --- all \(n\) signers --- with
slashable floor \(n \times \mathrm{bond}\). The convicted-to-necessary
...
ceiling. The lineage-escape maneuver (laundering a double-seal through a
...
trade forced; three rows are honestly open. Per the program's claim
...
T4a live-tier liveness & --- & L-OPEN (RES-R10: the engine's asynchronous bound) \\
...
T5d succession safety & --- & L-OPEN; claim withdrawn \\
...
were executed under the estate's governed machine-authority protocol ---
...
Section~\ref{sec:aft-cb-corpus}: T1--T9 with explicit Assumes lines over
...
\item \textbf{RES-R10 --- asynchronous liveness.} The live engine's
recovery is timeout-driven and fully deterministic, so no asynchronous
progress guarantee is possible without a common coin (an
engine-structure change). The fallback is designed (view synchronizer,
trigger, coin options) and not built; T4a's pairing row stays open and
every completeness-class claim rung stays gated on it.
...
a security bound.
\item \textbf{RES-R12 and the T5d withdrawal --- succession.} Three
successive succession formulations were refuted by the program's own
adversarial reviews; the claim is withdrawn, the operative exits from a
dead ring are the unanimous handover and the labeled re-genesis, and no
path in the estate activates a successor ring from elapsed time or
non-response. The conditions a fourth formulation must meet are
recorded in the specification's status banner.

...
\item \textbf{RES-P4.3-throughput.} Close-latency and throughput figures
at \(n \in \{4, 7\}\) await a pinned benchmark runner; the instrument
exists and the reproduction command is recorded, but a number measured on
a variable box would fail its own reproduce-within-variance gate and is
therefore not printed (Section~\ref{sec:measured-costs}).

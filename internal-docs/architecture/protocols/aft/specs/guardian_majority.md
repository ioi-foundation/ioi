...
- the bounded TLA+ model in `formal/guardian_majority/`
...
- `formal/guardian_majority/GuardianMajorityProof.tla` proves the unbounded safety kernel in TLAPS.
- `formal/guardian_majority/GuardianMajority.tla` model-checks richer bounded operational scenarios in TLC.
...
- Broader registry-rollback, transparency-log-outage, and cross-provider
  ...
  than a theorem-side liveness qualifier.

...
| Bracha asynchronous reliable broadcast | The broader asynchronous Byzantine-agreement paper contains the broadcast filtering layer; asynchronous channels; standard resilience `n >= 3t+1` | Sender validity plus consistency/totality among correct recipients; a faulty sender may cause no delivery | The broadcast filter propagates a value that enters its echo/ready thresholds. It neither forces a permanently silent Byzantine sender to publish nor chooses between independent singleton authorizations at `t=n-1`. |
...
| DAG-Rider | Fully asynchronous randomized atomic broadcast, optimal `n >= 3f+1`; reliable broadcast plus an assumed global perfect coin | Total ordering whose cryptographic safety path is post-quantum; liveness additionally relies on availability of the global perfect coin; eventual decision of messages proposed by correct processes | This is a strong PQ-safety asynchronous baseline but retains the classical resilience geometry and an explicit liveness oracle. It does not offer silent-`n-1` transferable effect authorization. |
...
| Atomic idempotency register at the consequence boundary | External linearizable resource keyed by effect identity; repository primary model: [`formal/consequence/AtMostOnceExternalization.tla`](../formal/consequence/AtMostOnceExternalization.tla) | At-most-once physical mutation even if duplicate authorizations arrive | This protects a modeled resource, not the consensus-proof property. It can be an explicit end-to-end assumption, but using it to choose between `pi_X` and `pi_Y` concedes that the resource is the final selector and leaves transferable non-conflict unsatisfied. |
...
- J. P. Conley, [Proof of Honesty: Coalition-Proof Blockchain Validation
  without Proof of Work or Stake, version
  2.0](https://johnpconley.com/wp-content/uploads/2019/11/Proof-of-Honesty-Coalition-Proof-Blockchain-Validation-without-Proof-of-Work-or-Stake-V-2.0-2019-05-31.pdf),
  2019-05-31. This dated author-hosted technical PDF was retrieved on
  2026-09-03 with SHA-256
  `238dc57170e337256301c97818ff57ec0c42e9b24c3f5dea2bb0b926a94e880d`.
  Its user-validation and economic claims are not adopted here as

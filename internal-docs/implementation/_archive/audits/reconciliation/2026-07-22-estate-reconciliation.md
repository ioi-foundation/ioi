...
`feat/estate-camera-pipeline` at `a894b2505`; tracked canon/evidence estate from
...
| --- | --- | --- |
| Architecture meaning, object contracts, authority boundaries, and product membership | The subject owner routed by [`source-of-truth-map.md`](../../../../docs/architecture/_meta/source-of-truth-map.md), or an accepted ADR | Guides, matrices, deltas, plans, captures, and work records point to the owner. |
| Canonical dependency and claim horizons | [`execution-horizons.md`](../../../../docs/architecture/_meta/execution-horizons.md) | It owns horizon doctrine, not live cut activation or status. |
...
| Concept-to-owner and durable-form mapping | [`implementation-matrix.md`](../../../../docs/architecture/_meta/implementation-matrix.md) | It carries doctrine only, publishes no work queue or status pointer, and cannot override a subject owner. |
| Object-to-owner/code-anchor crossings and contract-dependency pointers to application projections | [`canon-to-code-delta.md`](../../../../docs/architecture/_meta/canon-to-code-delta.md) | Its bounded anchor coverage is not work-item or stage status. |
| Per-cut implementation status and retained nonclaims | The matching ignored `internal-docs/implementation/work-items/*.v1.json` record | Matrix and delta rows carry doctrine and code/evidence pointers only; architecture canon publishes no queue or status link. |
...
| Architecture reader orientation | [`start-here.md`](../../../../docs/architecture/_meta/start-here.md) | It owns reading paths, not the doctrine summarized along them. |
...
| `implementation-plan-estate-reconciliation.md` | `WORK-RECORD` | This private point-in-time classification and owner-routing inventory only. | Keep machine-local beside the master; publish only its non-status summary in the reconciliation PR body. |
| `m0-m14-plan-gap-audit.md` | `WORK-RECORD` | Dated demanded/specified/proven/missing audit and quarantined amendment proposals. | Keep machine-local; the PR body carries the review appendix without making canon a plan host. |
| `work-items/README.md` and `work-items/*.v1.json` | `AUTHORITY` / `WORK-RECORD` | Private format convention plus one cut-status owner per record. | Keep ignored. Validate locally with `npm run check:work-items`; a clean checkout makes an honest skip/nonclaim. |
...
| `ioi-target-end-state-master-implementation-guide.md` | `AUTHORITY` | Sole internal M0-M14 sequencer, implementation-stage activation/proof gates, and source-disposition ledger. Architecture owners still outrank it for meaning. | Keep in place and never duplicate. The approved stateless-guide slice now places the Status Truth Rule at the front door, replaces dated stage narratives with record/projection pointers, and makes section 13.3 step 9 the projection-regeneration rule. Its exact ignored-file change is preserved by the private [`WORK-RECORD` manifest](./stateless-master-guide/manifest.v1.json) and [reviewable patch](./stateless-master-guide/stateless-master-guide.v1.patch). Further amendments require explicit user approval. |
...
| `work-item-m1-5-protected-transitions.md` | `WORK-RECORD` | Human-readable M1.5 cut design/progress log. The private machine record `work-items/m1-5-protected-transitions.v1.json` is the only cut-status owner. | Keep the append-only detail in place and treat its Markdown status labels as provenance only. |
| `work-item-m1-5c-amendment-execution.md` | `WORK-RECORD` | Human-readable M1.5c design/progress log. The private machine record `work-items/m1-5c-amendment-execution.v1.json` owns current cut status. | Keep the design detail private; do not publish it through architecture canon. |

## Tracked canon metadata and reconciliation evidence

| Source | Exact class | Narrow owner or derivation | Regeneration, validation, and pointer disposition |
| --- | --- | --- | --- |
| [`implementation-matrix.md`](../../../../docs/architecture/_meta/implementation-matrix.md) | `AUTHORITY` | Concept-to-owner and durable-form mapping doctrine. Subject owners still own object meaning; private records own cut status. | The live 269-row index is stateless and contains no work queue or status pointers. Its former status-bearing body is preserved as tracked reconciliation evidence. Validate with `npm run check:architecture-docs`. |
| [`execution-horizons.md`](../../../../docs/architecture/_meta/execution-horizons.md) | `AUTHORITY` | Canonical horizon framing, contract dependency order, and claim horizons. It does not activate cuts. | Keep tracked and canonical. Its fourteen-step contract order feeds the master; it must not become a second live sequencer. |
| [`canon-to-code-delta.md`](../../../../docs/architecture/_meta/canon-to-code-delta.md) | `AUTHORITY` | Stable object-to-owner/code-anchor crossing identities and contract-dependency pointers to application projections only. Subject docs own doctrine; the private estate owns status; the master owns activation. | Retain bounded owner/code/evidence crossings without work-item or status pointers. Validate with `npm run check:canon-to-code-delta`. |
| [`source-of-truth-map.md`](../../../../docs/architecture/_meta/source-of-truth-map.md) | `AUTHORITY` | Subject-to-canonical-owner routing and conflict precedence. | Keep tracked and canonical. Validate its structural/owner invariants with `npm run check:architecture-docs`. |
| [`start-here.md`](../../../../docs/architecture/_meta/start-here.md) | `AUTHORITY` | First-read orientation and role-based reading paths only. Its summarized doctrine remains owned by the linked subject files. | Keep tracked and canonical. Document-class metadata and stable claim ceilings are not cut status; any durable cut-state assertion routes to work-item records and the local orientation projection. |
| [`stateless-master-guide.v1.json`](./stateless-master-guide/manifest.v1.json) and [`stateless-master-guide.v1.patch`](./stateless-master-guide/stateless-master-guide.v1.patch) | `WORK-RECORD` | Exact, non-authoritative private review representation of the approved stateless change to the ignored sole sequencer, binding the estate base, reviewed result, and full-context unified patch by SHA-256. | Keep ignored with the implementation estate. `npm run check:stateless-master-guide` reconstructs and hash-validates the private base/result when present; a clean checkout emits an explicit skip/nonclaim. |
| `scripts/check-work-items.mjs` | `AUTHORITY` | Machine validation rules for private work-item records; it owns no cut status. | Validate the ignored estate when present; emit an honest nonclaim/skip in a clean checkout. |
...
| M1.5 status/design in ignored Markdown logs and private JSON records | `m1-5-protected-transitions.v1.json`, `m1-5b-generic-protected-transitions.v1.json`, and `m1-5c-amendment-execution.v1.json` each own their private cut scope/status; Markdown retains detail only | Keep the records distinct and the Markdown status labels provenance-only; do not publish them through canon. |
...
- The program-state transaction targets the 43-record private status layer,
  including the active M1.5c record, and the committed M0 exit artifact. Final regeneration and local
...
- `npm run check:work-items` structurally covers all 43 private records; the 36
  gap records are `proposed`, as the guide requires, and canon carries no queue
  or status pointer.
...
- Cross-estate Markdown links target canonical owners or tracked evidence.
  Private implementation sources remain local paths and do not exist in a
  fresh tracked worktree.

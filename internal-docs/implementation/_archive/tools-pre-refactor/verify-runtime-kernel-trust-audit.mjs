...
const countSummary = serviceTargets
  ...
  .join(", ");
const summary = `runtime-kernel trust decomposition verified: baseline=${auditRows.length}; ${countSummary}; retired=${retiredMethods.length}; namespace_residual_modules=${expectedResidualModules.length}; kernel_files=${kernelRustFileCount}; scanned ${scannedSourceFileCount} Rust source files; exact ordered allowlists, derived residual census, and no out-of-module inherent impls`;
const census = {
  schema_version: "ioi.runtime-kernel-trust-census.v1",
  audit_path: path.relative(repoRoot, auditPath).split(path.sep).join("/"),
  baseline_snapshot_method_count: auditRows.length,
  current_inventory: actualInventoryCounts,
  residual_delegate_modules: expectedResidualModules,
  mixed_trust_delegate_modules: expectedMixedModules,
  effect_compatibility_methods: expectedEffectMethods,
  closure_rule: "A residual module leaves this census only after its non-trusted owner or projection logic has a non-kernel module identity without weakening authority, admission, Agentgres writer, receipt, replay, or denial invariants.",
  summary,
};
const mode = process.argv[2] ?? "--summary";
if (process.argv.length > 3 || !["--summary", "--json"].includes(mode)) {
  fail("usage: node internal-docs/implementation/tools/verify-runtime-kernel-trust-audit.mjs [--summary|--json]");
}
process.stdout.write(mode === "--json" ? `${JSON.stringify(census)}\n` : `${summary}\n`);

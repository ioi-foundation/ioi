...
| Org identity record (name/tier/membership header) | owner: identity plane; no daemon org record exists (only org-invite) | `route-missing` — **W0.5** (the guide's own identity-truth wave; whoami exists, org read does not) |

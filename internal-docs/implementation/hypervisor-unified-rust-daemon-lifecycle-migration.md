...
A multi-agent adversarial review of the producer cuts (bridge + managed-sessions + snapshots)
surfaced 23 confirmed findings. The critical silent-data-loss bug and the high/medium
correctness, resource, robustness, and security issues were fixed in commit `0dda3458f`
(snapshot capture content-failure-blocks-not-deletes, robust git path handling + rename split,
idempotency dedup, content-addressed managed-session key, tolerant event replay, symlinked-parent
canonicalization, spawn_blocking bridge, restore.blocked accounting). The producer→channel→
bridge→jsonl→projection chain is now covered by a real in-process e2e test
(`success_path/tests.rs::real_browser_producer_bridges_a_managed_session_the_daemon_projection_serves`)
and a snapshot data-loss regression guard (ratchet step 14c).

**Tracked follow-ups (deliberately deferred — none block the current single-writer deployment):**

- **Cross-process event-log lock** — the runtime bridge and the daemon append to the same
  `<state_dir>/events/<stream>.jsonl`; concurrent writers could assign duplicate `seq`. Latent
  today (the bridge no-ops in `ioi-local`, which runs a zero session and registers no agent
  record), and partially mitigated by the idempotency dedup + tolerant replay. Fix: an OS
  advisory `flock` held across the read-latest-seq + admit + append window in both writers (needs
  `libc` in `ioi-services`).
- **Approval gate on `restore-apply`** — route it through `WorkspaceRestoreApplyPolicyCore` and
  derive `allow_conflicts` from a server-side policy decision rather than the request body.
- **Snapshot content in a side artifact** — embed only metadata in the `workspace_snapshot.captured`
  event payload and store the content package as an `artifact_ref`, so file bytes are not replayed
  verbatim on every `GET /events`.
- **Scoped admission read** — the kernel admission scans the whole `events/` dir to compute
  `latest_seq`; scope it to the single target stream file (pre-existing; affects all admits).
- **Binary round-trip** — capture currently BLOCKS (never deletes) a non-UTF-8 file it cannot
  represent; base64-encoding with an `encoding` marker would let binary files actually restore.
- **Lag reconciliation** — give the bridge a dedicated higher-capacity channel (or re-derive
  managed sessions from KV on `Lagged`) so a broadcast lag cannot drop a managed session.

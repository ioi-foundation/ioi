...
fn workflow_runtime_usage_meter_output(
    workflow: Option<&WorkflowProject>,
    node_id: &str,
    logic: &Value,
    input: &Value,
    ...
    let scope_field = workflow_runtime_control_logic_string(logic, "runtimeUsageMeterScopeField")
        .unwrap_or_else(|| "usageScope".to_string());
    let scope = workflow_runtime_control_logic_string(logic, "runtimeUsageMeterScope")
        .or_else(|| workflow_runtime_control_input_string(input, &scope_field))
        .or_else(|| workflow_runtime_control_input_string(input, "usage_scope"))
        .unwrap_or_else(|| "thread".to_string());
    let default_endpoint = match scope.as_str() {
        "run" => "/v1/runs/{runId}/usage",
        "workflow" => "/v1/usage",
        _ => "/v1/threads/{threadId}/usage",
    };

    let run_id_field = workflow_runtime_control_logic_string(logic, "runtimeUsageMeterRunIdField")
        .unwrap_or_else(|| "runId".to_string());
    let run_id = workflow_runtime_control_logic_string(logic, "runtimeUsageMeterRunId")
        .or_else(|| workflow_runtime_control_input_string(input, &run_id_field))
        .or_else(|| workflow_runtime_control_input_string(input, "run_id"))
        .unwrap_or_else(|| "{{runtime.run_id}}".to_string());
    ...
    let mut envelope = workflow_runtime_control_envelope(
        ...
        input,
        &WorkflowRuntimeControlEnvelopeConfig {
            thread_id_logic_key: "runtimeUsageMeterThreadId",
            thread_id_field_key: "runtimeUsageMeterThreadIdField",
            turn_id_logic_key: None,
            turn_id_field_key: None,
            workflow_node_id_logic_key: "runtimeUsageMeterWorkflowNodeId",
            actor_logic_key: "runtimeUsageMeterActor",
            endpoint_logic_key: "runtimeUsageMeterEndpoint",
            default_workflow_node_id: "runtime.usage-meter",
            default_endpoint,
            missing_turn_id: None,
        },
    );
    envelope.endpoint = envelope.endpoint.replace("{runId}", &run_id);

    let output_config = WorkflowRuntimeControlOutputConfig {
        schema_version: "ioi.workflow.runtime-usage-meter-control.v1",
        source: "react_flow",
        component_kind: "usage_telemetry",
        event_kind: "RuntimeUsageTelemetry.Read",
        payload_schema_version: "ioi.runtime.usage-telemetry.v1",
        nested_key: "runtimeUsageMeter",
    };

    let group_by = workflow_runtime_control_logic_string(logic, "runtimeUsageMeterGroupBy")
        .or_else(|| workflow_runtime_control_input_string(input, "groupBy"))
        .or_else(|| workflow_runtime_control_input_string(input, "group_by"));

    let simulation_mode = workflow_runtime_control_logic_string(logic, "runtimeUsageMeterSimulationMode")
        .and_then(|s| s.parse::<bool>().ok())
        .or_else(|| workflow_runtime_control_input_bool(input, "simulationMode"))
        .or_else(|| workflow_runtime_control_input_bool(input, "simulation_mode"))
        .unwrap_or(true);
    ...
    let request = workflow_runtime_control_request(
        ...
        vec![
            ...
            ("simulationMode", json!(simulation_mode)),
        ],
    );

    let mut out = workflow_runtime_control_output(
        node_id,
        evidence_kind,
        input,
        &output_config,
    ...
    out["groupBy"] = json!(group_by);
    out["simulationMode"] = json!(simulation_mode);
    out
}

fn workflow_runtime_context_budget_output(
    workflow: Option<&WorkflowProject>,
    node_id: &str,
    logic: &Value,
    input: &Value,
    ...
    let scope_field = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetScopeField")
        .unwrap_or_else(|| "usageScope".to_string());
    let scope = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetScope")
        .or_else(|| workflow_runtime_control_input_string(input, &scope_field))
        .or_else(|| workflow_runtime_control_input_string(input, "scope"))
        .unwrap_or_else(|| "thread".to_string());
    let default_endpoint = match scope.as_str() {
        "run" => "/v1/runs/{runId}/context-budget",
        "workflow" => "/v1/context-budget",
        _ => "/v1/threads/{threadId}/context-budget",
    };

    let run_id_field = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetRunIdField")
        .unwrap_or_else(|| "runId".to_string());
    let run_id = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetRunId")
        .or_else(|| workflow_runtime_control_input_string(input, &run_id_field))
        .or_else(|| workflow_runtime_control_input_string(input, "run_id"))
        .unwrap_or_else(|| "{{runtime.run_id}}".to_string());
    ...
    let mut envelope = workflow_runtime_control_envelope(
        ...
        input,
        &WorkflowRuntimeControlEnvelopeConfig {
            thread_id_logic_key: "runtimeContextBudgetThreadId",
            thread_id_field_key: "runtimeContextBudgetThreadIdField",
            turn_id_logic_key: None,
            turn_id_field_key: None,
            workflow_node_id_logic_key: "runtimeContextBudgetWorkflowNodeId",
            actor_logic_key: "runtimeContextBudgetActor",
            endpoint_logic_key: "runtimeContextBudgetEndpoint",
            default_workflow_node_id: "runtime.context-budget",
            default_endpoint,
            missing_turn_id: None,
        },
    );
    ...
    let output_config = WorkflowRuntimeControlOutputConfig {
        schema_version: "ioi.workflow.runtime-context-budget-control.v1",
        source: "react_flow",
        ...
        nested_key: "runtimeContextBudget",
    };

    let mode_field = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetModeField")
        .unwrap_or_else(|| "contextBudgetMode".to_string());
    let mode = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetMode")
        .or_else(|| workflow_runtime_control_input_string(input, &mode_field))
        .or_else(|| workflow_runtime_control_input_string(input, "mode"))
        .unwrap_or_else(|| "simulate".to_string());

    let simulation_mode = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetSimulationMode")
        .and_then(|s| s.parse::<bool>().ok())
        .or_else(|| workflow_runtime_control_input_bool(input, "simulationMode"))
        .or_else(|| workflow_runtime_control_input_bool(input, "simulation_mode"))
        .unwrap_or(true);

    let max_total_tokens = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetMaxTotalTokens")
        .and_then(|s| s.parse::<i64>().ok())
        .or_else(|| workflow_runtime_control_input_number(input, "maxTotalTokens").map(|n| n as i64))
        .or_else(|| workflow_runtime_control_input_number(input, "max_total_tokens").map(|n| n as i64));

    let max_cost_usd = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetMaxCostUsd")
        .and_then(|s| s.parse::<f64>().ok())
        .or_else(|| workflow_runtime_control_input_number(input, "maxCostUsd"))
        .or_else(|| workflow_runtime_control_input_number(input, "max_cost_usd"));

    let max_context_pressure = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetMaxContextPressure")
        .and_then(|s| s.parse::<f64>().ok())
        .or_else(|| workflow_runtime_control_input_number(input, "maxContextPressure"))
        .or_else(|| workflow_runtime_control_input_number(input, "max_context_pressure"));

    let warn_at_ratio = workflow_runtime_control_logic_string(logic, "runtimeContextBudgetWarnAtRatio")
        .and_then(|s| s.parse::<f64>().ok())
        .or_else(|| workflow_runtime_control_input_number(input, "warnAtRatio"))
        .or_else(|| workflow_runtime_control_input_number(input, "warn_at_ratio"))
        .unwrap_or(0.8);

    let usage_telemetry = workflow_runtime_control_value_at_path(input, "runtimeUsageMeter")
        .or_else(|| workflow_runtime_control_value_at_path(input, "runtimeTelemetrySummary"))
        .or_else(|| workflow_runtime_control_value_at_path(input, "runtime_telemetry_summary"))
        .or_else(|| workflow_runtime_control_value_at_path(input, "usageTelemetry"))
        .or_else(|| workflow_runtime_control_value_at_path(input, "usage_telemetry"))
        .cloned()
        .unwrap_or(Value::Null);
    ...
    let request = workflow_runtime_control_request(
        ...
        vec![
            ...
            ("thresholds", json!({
                ...
                "maxContextPressure": max_context_pressure,
                "max_context_pressure": max_context_pressure,
                "warnAtRatio": warn_at_ratio,
                "warn_at_ratio": warn_at_ratio,
            })),
            ("usageTelemetry", usage_telemetry.clone()),
    ...
    out["usageTelemetry"] = usage_telemetry.clone();
    out["usage_telemetry"] = usage_telemetry;
    out
}

fn workflow_runtime_compaction_policy_output(
    workflow: Option<&WorkflowProject>,
    node_id: &str,
    logic: &Value,
    input: &Value,
    ...
    let mut envelope = workflow_runtime_control_envelope(
        ...
        input,
        &WorkflowRuntimeControlEnvelopeConfig {
            thread_id_logic_key: "runtimeCompactionPolicyThreadId",
            thread_id_field_key: "runtimeCompactionPolicyThreadIdField",
            turn_id_logic_key: Some("runtimeCompactionPolicyTurnId"),
            turn_id_field_key: Some("runtimeCompactionPolicyTurnIdField"),
            workflow_node_id_logic_key: "runtimeCompactionPolicyWorkflowNodeId",
            actor_logic_key: "runtimeCompactionPolicyActor",
            endpoint_logic_key: "runtimeCompactionPolicyEndpoint",
            default_workflow_node_id: "runtime.compaction-policy",
            default_endpoint: "/v1/threads/{threadId}/compaction-policy",
            missing_turn_id: None,
        },
    );

    let output_config = WorkflowRuntimeControlOutputConfig {
        schema_version: "ioi.workflow.runtime-compaction-policy-control.v1",
        source: "react_flow",
        ...
        nested_key: "runtimeCompactionPolicy",
    };

    let context_budget = workflow_runtime_control_value_at_path(input, "runtimeContextBudget")
        .or_else(|| workflow_runtime_control_value_at_path(input, "contextBudget"))
        .or_else(|| workflow_runtime_control_value_at_path(input, "context_budget"))
        .cloned()
        .unwrap_or(Value::Null);

    let context_budget_status = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyContextBudgetStatus")
        .or_else(|| workflow_runtime_control_input_string(input, "runtimeContextBudget.status"))
        .or_else(|| {
            context_budget.get("status")
                .or_else(|| context_budget.get("policyDecision").and_then(|pd| pd.get("status")))
                .or_else(|| context_budget.get("policy_decision").and_then(|pd| pd.get("status")))
                .and_then(Value::as_str)
                .map(str::to_string)
        });

    let ok_action = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyOkAction")
        .or_else(|| workflow_runtime_control_input_string(input, "okAction"))
        .or_else(|| workflow_runtime_control_input_string(input, "ok_action"))
        .unwrap_or_else(|| "noop".to_string());

    let warn_action = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyWarnAction")
        .or_else(|| workflow_runtime_control_input_string(input, "warnAction"))
        .or_else(|| workflow_runtime_control_input_string(input, "warn_action"))
        .unwrap_or_else(|| "warn".to_string());

    let blocked_action = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyBlockedAction")
        .or_else(|| workflow_runtime_control_input_string(input, "blockedAction"))
        .or_else(|| workflow_runtime_control_input_string(input, "blocked_action"))
        .unwrap_or_else(|| "compact".to_string());

    let approval_required = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyApprovalRequired")
        .and_then(|s| s.parse::<bool>().ok())
        .or_else(|| workflow_runtime_control_input_bool(input, "approvalRequired"))
        .or_else(|| workflow_runtime_control_input_bool(input, "approval_required"))
        .unwrap_or(false);

    let approval_granted = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyApprovalGranted")
        .and_then(|s| s.parse::<bool>().ok())
        .or_else(|| workflow_runtime_control_input_bool(input, "approvalGranted"))
        .or_else(|| workflow_runtime_control_input_bool(input, "approval_granted"))
        .unwrap_or(false);

    let execute_compaction = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyExecuteCompaction")
        .and_then(|s| s.parse::<bool>().ok())
        .or_else(|| workflow_runtime_control_input_bool(input, "executeCompaction"))
        .or_else(|| workflow_runtime_control_input_bool(input, "execute_compaction"))
        .unwrap_or(false);

    let compact_reason = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyCompactReason")
        .or_else(|| workflow_runtime_control_input_string(input, "compactReason"))
        .or_else(|| workflow_runtime_control_input_string(input, "compact_reason"))
        .unwrap_or_else(|| "Compact thread context from React Flow compaction policy.".to_string());

    let compact_scope = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyCompactScope")
        .or_else(|| workflow_runtime_control_input_string(input, "compactScope"))
        .or_else(|| workflow_runtime_control_input_string(input, "compact_scope"))
        .unwrap_or_else(|| "thread".to_string());

    let compact_workflow_node_id = workflow_runtime_control_logic_string(logic, "runtimeCompactionPolicyCompactWorkflowNodeId")
        .unwrap_or_else(|| "runtime.context-compact".to_string());

    let turn_id_value = envelope.turn_id.clone().map(Value::String).unwrap_or(Value::Null);

    let request = workflow_runtime_control_request(
        &output_config,
        &envelope,
        vec![
            ("threadId", json!(envelope.thread_id.clone())),
            ("thread_id", json!(envelope.thread_id.clone())),
            ("turnId", turn_id_value.clone()),
            ("turn_id", turn_id_value.clone()),
            ("contextBudget", context_budget.clone()),
            ("context_budget", context_budget.clone()),
            ("contextBudgetStatus", json!(context_budget_status.clone())),
            ("context_budget_status", json!(context_budget_status.clone())),
            ("policy", json!({
                "okAction": ok_action,
                "ok_action": ok_action,
                ...
                "executeCompaction": execute_compaction,
                "execute_compaction": execute_compaction,
                "compactReason": compact_reason,
                "compact_reason": compact_reason,
                "compactScope": compact_scope,
                "compact_scope": compact_scope,
                "compactWorkflowNodeId": compact_workflow_node_id,
                "compact_workflow_node_id": compact_workflow_node_id,
            })),
        ],
    );
...
pub(super) fn execute_workflow_node(
    ...
    let output = match action_kind {
        ...
        ActionKind::RuntimeUsageMeter => workflow_runtime_usage_meter_output(
            workflow,
            &node_id,
            &logic,
            &input,
        ...
        ActionKind::RuntimeContextBudget => workflow_runtime_context_budget_output(
            workflow,
            &node_id,
            &logic,
            &input,
        ...
        ActionKind::RuntimeCompactionPolicy => workflow_runtime_compaction_policy_output(
            workflow,
            &node_id,
            &logic,
            &input,

...
pub(super) const CHAT_ARTIFACT_EXEMPLAR_SCOPE: &str = "desktop.chat.artifact_exemplars";
const CHAT_ARTIFACT_EXEMPLAR_KIND: &str = "chat.artifact_exemplar";
const CHAT_ARTIFACT_EXEMPLAR_MIN_SCORE: i32 = 24;

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ChatArtifactExemplarArchivalMetadata {
    trust_level: String,
    artifact_id: String,
    revision_id: String,
...
fn chat_artifact_validation_total_score(validation: &ChatArtifactValidationResult) -> i32 {
...
fn exemplar_component_patterns(
    blueprint: &ChatArtifactBlueprint,
    artifact_ir: &ChatArtifactIR,
    prior: Option<&ChatArtifactTasteMemory>,
) -> Vec<String> {
...
fn build_chat_artifact_exemplar_content(
    exemplar: &ChatArtifactExemplar,
    brief: &ChatArtifactBrief,
    blueprint: &ChatArtifactBlueprint,
    artifact_ir: &ChatArtifactIR,
) -> String {
...
fn chat_artifact_exemplar_from_metadata(
    record_id: i64,
    metadata: ChatArtifactExemplarArchivalMetadata,
...
pub(super) fn persist_chat_artifact_exemplar(
    ...
    let exemplar = ChatArtifactExemplar {
        record_id: 0,
        ...
        score_total: chat_artifact_validation_total_score(validation),
    ...
    let content = build_chat_artifact_exemplar_content(&exemplar, brief, blueprint, artifact_ir);
    let metadata = ChatArtifactExemplarArchivalMetadata {
    ...
    let metadata_json = serde_json::to_string(&metadata)
        .map_err(|error| format!("Failed to serialize Chat exemplar metadata: {error}"))?;
    let Some(record_id) = memory_runtime
        .insert_archival_record(&NewArchivalMemoryRecord {
            ...
            thread_id: None,
            kind: CHAT_ARTIFACT_EXEMPLAR_KIND.to_string(),
            content: content.clone(),
            metadata_json,
        })
        .map_err(|error| format!("Failed to insert Chat exemplar record: {error}"))?
    else {
        return Ok(None);
    };
    let embedding = tauri::async_runtime::block_on(async { runtime.embed_text(&content).await })
        .map_err(|error| format!("Failed to embed Chat exemplar record: {error}"))?;
    memory_runtime
        .upsert_archival_embedding(record_id, &embedding)
        .map_err(|error| format!("Failed to index Chat exemplar record: {error}"))?;

    Ok(Some(ChatArtifactExemplar {
        record_id,
        ..exemplar
    }))
}
...
pub(super) fn compare_artifact_revisions(
    ...
    let base = current_chat_revision(chat_session, &base_revision_id)?;
    let target = current_chat_revision(chat_session, &target_revision_id)?;
    let changed_paths = changed_paths_between_revisions(base, target, memory_runtime.as_ref());

    Ok(ChatArtifactRevisionComparison {
        base_revision_id,
        target_revision_id,
...
pub(crate) fn changed_paths_between_revisions(
    base: &ChatArtifactRevision,
    target: &ChatArtifactRevision,
...
pub(super) fn restore_artifact_revision(
    ...
    let revision = current_chat_revision(chat_session, &revision_id)?.clone();
...
pub(super) fn branch_artifact_revision(
    ...
    let chat_session = task
        ...
        .as_mut()
        .ok_or_else(|| "No Chat artifact session is attached to the current task.".to_string())?;
    let seed_revision = current_chat_revision(chat_session, &revision_id)?.clone();
...
pub(super) fn current_chat_revision<'a>(
    chat_session: &'a ChatArtifactSession,
    revision_id: &str,
) -> Result<&'a ChatArtifactRevision, String> {
    chat_session
        .revisions
        .iter()
        .find(|revision| revision.revision_id == revision_id)
        .ok_or_else(|| format!("Chat revision '{}' was not found.", revision_id))
}
...
pub(super) fn chat_refinement_context_for_session(
    memory_runtime: &Arc<MemoryRuntime>,
...
pub(super) fn derive_chat_taste_memory(
    prior: Option<&ChatArtifactTasteMemory>,
    brief: &ChatArtifactBrief,
    ...
    validation: Option<&ChatArtifactValidationResult>,
) -> Option<ChatArtifactTasteMemory> {
...
fn build_revision_from_session(
    ...
    ChatArtifactRevision {
        ...
        ux_lifecycle: chat_session
            .ux_lifecycle
            .unwrap_or(ChatArtifactUxLifecycle::Validated),
...
fn chat_artifact_file_is_text(mime: &str) -> bool {
...
fn refinement_body_for_artifact_file(path: &str, mime: &str, bytes: &[u8]) -> Option<String> {
    if chat_artifact_file_is_text(mime) {

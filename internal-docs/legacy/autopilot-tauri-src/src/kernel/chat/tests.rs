...
use super::proof::run_chat_current_task_turn_for_proof_with_route_timeout;
use super::revisions::{
    apply_revision_to_chat_session, chat_artifact_exemplar_from_archival_record,
    CHAT_ARTIFACT_EXEMPLAR_SCOPE,
};
use super::*;
...
use ioi_api::chat::{
    ...
    ChatArtifactRenderFinding, ChatArtifactRenderFindingSeverity, ChatArtifactVerificationReceipt,
    ChatArtifactWorkGraphExecutionSummary, ChatArtifactWorkGraphPlan, ChatArtifactWorkItem,
    ChatArtifactWorkItemStatus, ChatArtifactWorkerReceipt, ChatArtifactWorkerRole,
    ChatGeneratedArtifactFile, ChatGeneratedArtifactPayload, ExecutionStage,
};
...
fn chat_validation_fixture() -> ChatArtifactValidationResult {
...
fn instrument_html_for_headless_validation(html: &str) -> String {
    let instrumentation_script = r#"<script data-chat-validation-probe="true">
window.addEventListener("error", (event) => {
  const root = document.documentElement;
  root.setAttribute("data-chat-validation-error", "true");
  root.setAttribute("data-chat-validation-error-message", String(event.message || "error"));
});
window.addEventListener("unhandledrejection", (event) => {
  const root = document.documentElement;
  root.setAttribute("data-chat-validation-error", "true");
  root.setAttribute(
    "data-chat-validation-error-message",
...
fn write_temp_chat_html_fixture(name: &str, html: &str) -> PathBuf {
...
#[async_trait]
impl ChatArtifactRenderEvaluator for KernelPassingRenderEvaluator {
        ...
        Ok(Some(ChatArtifactRenderEvaluation {
            ...
            captures: vec![
                ChatArtifactRenderCapture {
                    viewport: ChatArtifactRenderCaptureViewport::Desktop,
                    width: 1440,
                    height: 960,
                ...
                ChatArtifactRenderCapture {
                    viewport: ChatArtifactRenderCaptureViewport::Mobile,
                    width: 430,
                    height: 932,
...
#[derive(Debug, Clone)]
struct ChatOutcomeTestRuntime {
    payload: String,
}

#[async_trait]
impl InferenceRuntime for ChatOutcomeTestRuntime {
...
#[derive(Debug, Clone)]
struct SlowChatOutcomeTestRuntime {
    payload: String,
    delay: Duration,
    provenance: Option<crate::models::ChatRuntimeProvenance>,
}

#[async_trait]
impl InferenceRuntime for SlowChatOutcomeTestRuntime {
    async fn execute_inference(
        &self,
        _model_hash: [u8; 32],
        _input_context: &[u8],
...
#[derive(Debug, Clone)]
struct ChatArtifactGenerationTestRuntime {
    provenance: crate::models::ChatRuntimeProvenance,
}

impl ChatArtifactGenerationTestRuntime {
    fn new(
        kind: crate::models::ChatRuntimeProvenanceKind,
        label: &str,
        model: &str,
        endpoint: &str,
    ) -> Self {
...
#[async_trait]
impl InferenceRuntime for ChatArtifactGenerationTestRuntime {
    async fn execute_inference(
        &self,
        _model_hash: [u8; 32],
        input_context: &[u8],
        ...
        let response = if prompt.contains("typed artifact brief planner") {
            ...
            serde_json::json!({
                "summary": "Prepared a chat rollout artifact",
...
impl InferenceRuntime for StreamingDirectAuthorTestRuntime {
    async fn execute_inference(
        ...
        Err(VmError::HostError("unexpected Chat prompt".to_string()))
    }
    ...
    async fn execute_inference(
        ...
        let response = if prompt.contains("typed artifact brief planner") {
            ...
            return Err(VmError::HostError(format!(
                "unexpected Chat prompt in replan test: {prompt}"
            )));
        };
...
fn test_outcome_request() -> ChatOutcomeRequest {
    ChatOutcomeRequest {
        ...
        artifact: Some(ChatOutcomeArtifactRequest {
...
fn test_workspace_request() -> ChatOutcomeArtifactRequest {
...
fn test_manifest(status: ChatArtifactVerificationStatus) -> ChatArtifactManifest {
    ChatArtifactManifest {
        ...
        verification: ChatArtifactManifestVerification {
            status,
            lifecycle_state: if status == ChatArtifactVerificationStatus::Ready {
                ChatArtifactLifecycleState::Ready
            } else if status == ChatArtifactVerificationStatus::Pending {
                ChatArtifactLifecycleState::Materializing
            } else if status == ChatArtifactVerificationStatus::Failed {
                ChatArtifactLifecycleState::Failed
            } else if status == ChatArtifactVerificationStatus::Partial {
                ChatArtifactLifecycleState::Partial
            } else {
                ChatArtifactLifecycleState::Blocked
            },
            summary: "Chat verified the artifact.".to_string(),
            production_provenance: None,
            acceptance_provenance: None,
            failure: None,
        },
...
fn test_chat_session(status: ChatArtifactVerificationStatus) -> ChatArtifactSession {
    ChatArtifactSession {
        ...
        title: "Release artifact".to_string(),
        summary: "Chat created a release artifact.".to_string(),
        ...
        verified_reply: ChatVerifiedReply {
            ...
            title: "Chat outcome: Release artifact".to_string(),
            summary: "Release artifact Chat verified the artifact.".to_string(),
        ...
        status: if status == ChatArtifactVerificationStatus::Failed {
            "failed".to_string()
        } else if status == ChatArtifactVerificationStatus::Pending {
...
fn provisional_artifact_session_tracks_origin_prompt_event_id() {
    let session = super::prepare::provisional_non_workspace_chat_session(
        ...
        "chat-session-1",
        "Release artifact",
        "Chat created a release artifact.",
        &now_iso(),
        &test_outcome_request(),
        Some("prompt-evt-1"),
        test_materialization_contract(),
    )
    .expect("provisional chat session");
...
fn operator_run_restarts_for_follow_up_edit_and_preserves_history() {
    let mut session = test_chat_session(ChatArtifactVerificationStatus::Ready);
...
fn operator_run_settles_verify_before_presenting_ready_artifact() {
    let mut session = test_chat_session(ChatArtifactVerificationStatus::Ready);
    session.materialization.render_evaluation = Some(ChatArtifactRenderEvaluation {
        supported: true,
        first_paint_captured: true,
        interaction_capture_attempted: true,
        captures: Vec::new(),
...
fn test_selection_target() -> ChatArtifactSelectionTarget {
...
fn proof_memory_runtime() -> Arc<MemoryRuntime> {
    let path = std::env::temp_dir().join(format!("ioi-chat-proof-test-{}.db", Uuid::new_v4()));
...
fn test_build_session(status: &str, verification_status: &str) -> BuildArtifactSession {
    BuildArtifactSession {
        ...
        workspace_root: "/tmp/chat".to_string(),
        ...
        current_worker_execution: ChatCodeWorkerLease {
            ...
            planner_authority: "chat".to_string(),
...
#[test]
fn derive_chat_taste_memory_accumulates_unique_directives() {
    let prior = ChatArtifactTasteMemory {
    ...
    let taste_memory =
        derive_chat_taste_memory(Some(&prior), &brief, None, None, Some(&edit_intent), None)
            .expect("taste memory");
...
fn revision_branch_identity_allocates_new_branch_when_requested() {
    let mut chat_session = test_chat_session(ChatArtifactVerificationStatus::Ready);
    ...
    let edit_intent = ChatArtifactEditIntent {
        mode: ChatArtifactEditMode::Branch,
...
fn apply_revision_to_chat_session_restores_manifest_and_selection() {
    ...
    chat_session.taste_memory = Some(ChatArtifactTasteMemory {
    ...
    restored_revision.artifact_manifest.files = vec![ChatArtifactManifestFile {
    ...
    restored_revision.selected_targets = vec![ChatArtifactSelectionTarget {
    ...
    restored_revision.taste_memory = Some(ChatArtifactTasteMemory {
    ...
    restored_revision.retrieved_exemplars = vec![ChatArtifactExemplar {
        record_id: 44,
...
fn exemplar_archival_record_round_trips_into_structured_exemplar() {
    let record = ArchivalMemoryRecord {
        id: 91,
        scope: CHAT_ARTIFACT_EXEMPLAR_SCOPE.to_string(),
        thread_id: None,
        kind: "chat.artifact_exemplar".to_string(),
    ...
    let exemplar =
        chat_artifact_exemplar_from_archival_record(&record).expect("structured exemplar");

    assert_eq!(exemplar.record_id, 91);
    assert_eq!(exemplar.renderer, ChatRendererKind::HtmlIframe);
...
fn revision_compare_detects_blob_changes_beyond_shared_preview_prefix() {
    ...
    let mut chat_session = test_chat_session(ChatArtifactVerificationStatus::Ready);
    chat_session.artifact_manifest.renderer = ChatRendererKind::HtmlIframe;
    chat_session.artifact_manifest.files = vec![ChatArtifactManifestFile {
    ...
    base_revision.file_writes = vec![ChatArtifactMaterializationFileWrite {
        path: "index.html".to_string(),
    ...
    refined_revision.file_writes = vec![ChatArtifactMaterializationFileWrite {
        path: "index.html".to_string(),
        kind: "write".to_string(),
...
fn test_task(status: ChatArtifactVerificationStatus) -> AgentTask {
    let mut task = AgentTask {
        id: "task-1".to_string(),

...
| 8. automations mesh | ☑ | PR #184. 29/29 T3 controls reconciled (`machinery`'s 30 deferred to packet 11 to avoid double-counting a contested surface). 7 rehome (1 with a named defect: T2 spec CRUD/lifecycle verbs are direct daemon actions with NO receipts on spec mutations — same unreceipted-effectful shape as ontology D-1, but the fix needs a daemon receipt family, so it is W2 not this packet) · 1 pattern-harvest · 4 retire-at-cutover · 2 blocked-missing-route · 1 deferred to X-2. Records the estate's ONE LIVE ROUTE HIJACK: `/automations` is the single canonical v2 route that resolves today, answered by a dead SPA plane. 28% of `monitors` is reference marketing (8 rdo incl. Cipher vendor-faculty cards). Ontology wiring is ALMOST bound — object sets are real and the condition vocabulary renders; the BINDING between them is the whole gap. `machinery` ownership evidence recorded from both sides (registry says Studio, census `canon_target_name` says Automations/Process Graphs), ruling deferred to X-2(a) |
| 9. foundry mesh | ☑ | PR #185. Integrated with the W3.0-W3.4 amendment BY REFERENCE (sections numbered §7/§8/§9 after the existing §6 defect register; nothing restated). 39/39 T3 controls reconciled — and the reconciliation itself is recorded: raw cluster entries sum to 42 because three controls are described in two clusters; counted once each the six buckets sum to 39 exactly. 6 rehome · 3 pattern-harvest · 3 retire-at-cutover · 3 blocked-missing-route. **Zero governed controls is correct for a `browse` registration AND is the finding** — Foundry's actual receipted verbs live on Agent Studio's `#model-routes` tab, recorded as a REHOME SOURCE that packet 11 must mirror from the donor side. The estate's most dangerous fixture named: reference vendor model cards (GPT-5.4 Pro, Claude Opus 4.7…) rendering beside real daemon routes — retire, never render at any wave. First surface where the ODK relationship is GENERATION, not expression (manifests produce eval/worker skeletons; no pane becomes descriptor-driven) |
| 10. developer-workspace mesh | ☑ | PR #186. **0 of 563** T3 controls (no registered surface); T2 carries 290 controls, 0 disabled — `/__ioi/workbench` at 266 is the estate's largest single readout. 6 rehome (2 with named defects) · 2 pattern-harvest · 2 retire-at-cutover · 2 blocked-missing-capture. Named defects: the adapter's AgentService WRITES `/v1/threads*` directly and keeps run state in serve-local memory (two C-3 violations at byte sites), and `CreateAgentExecutionConversationToken` returns a LOCAL CONSTANT — the surviving instance of the fabricated-token pattern W0.5 already deleted elsewhere. `/details/:envId` is retired here, not rehomed: it is Work's D5 seed and the two briefs must not both claim it. Corrects its own packet input — "development-environment recipes" resolves to `DevelopmentEnvironmentRecipe`, a PLATFORM object wearing a semantic-sounding name (same X-2 naming collision `data.md` filed). New missing-pattern finding (3rd distinct): a live terminal is not `monitoring_console`. And a NEW EXEMPTION CLASS: the console is CATEGORICALLY not an `object_editor` — that pattern edits ontology objects, a code editor edits files |
| 11. studio mesh | ☑ | PR #187. Includes the **Agent Studio split-rehome map** (§6.1): `/__ioi/agent-studio` is 1,050 controls — the estate's largest readout by 4× — split six ways (model-routes→Foundry, improvements→Improvement, intel→Settings, vault→Governance, launch-policies→Work+Governance, rest stays Studio), with the hard rule that no lane goes to two owners and none is dropped; 4 of 6 destination briefs already carry the mirror row. 81/81 T3 controls reconciled (designer 51, machinery 30) — **NEITHER has a single governed control**, so the surface canon names as the extension lane's authoring owner has ZERO governed controls today. 8 rehome · 1 rebind · 4 pattern-harvest · 5 retire-at-cutover · 2 blocked-missing-route · 1 blocked-missing-capture · 1 build-not-mesh · 1 deferred to X-2(a). THE RUN'S CENTRAL FINDING: **Studio authors descriptors per canon, `POST /odk/surface-descriptors` is live, and NO PANE CALLS IT** — the extension lane's authoring stage has a backend and no front end. Studio can create a DomainApp draft today over a descriptor it cannot author through any UI, for a package plane that does not exist |
| 12. evaluations mesh | ☑ | PR #188. 32/32 T3 controls reconciled. 4 rehome (1 with a named defect) · 2 pattern-harvest · 4 retire-at-cutover · 1 blocked-missing-route. **Census-vs-bytes disagreement recorded and resolved in favour of the bytes**: the census counts 3 governed-receipted controls on the suite-creation cluster, while §2 records the family's creates/deletes as RECEIPTLESS today — so those controls must present as draft CRUD until the mutation-receipt family lands. Two surface statements marked must-survive: "nothing scores or executes here", and feedback carrying evidence-eligibility consent FROM THE MOMENT IT IS RECORDED (consent-at-capture must not relax into a post-hoc flag). Ontology boundary: A PASSING EVALUATION IS NOT AN ADMITTED ASSERTION. Second DATED exemption in the run (`/__apps/analysis`'s object-set canvas — primitive is live, pane does not exist) |
| 13. improvement mesh | ☑ | PR #189. 47/47 T3 controls reconciled. 5 rehome (1 with a named defect) · 1 **rebind** · 6 retire-at-cutover · 0 blocked. **The only T3 surface in the run with ZERO `disabled_missing_authority` controls — and that is not completeness**: its gaps appear as 16 `unsupported_reference_session` + 12 `reference_data_only` instead, so 28 of 47 controls are reference chrome for a product the estate is not building. Its ONE governed control (confirm/override/opt-out) is the one that matters and §2 records approve/reject as RECEIPTLESS — so it must not present as governed until W3. Clearest single case in the run of chrome that must not survive: a subscribe-to-newsletter toggle inside a governed change inbox. Boundary: AN APPROVED PROPOSAL IS NOT AN APPLIED CHANGE. `review_inbox` cross-owner exemption gets its 2nd instance, confirming the finding is about the PATTERN, not about Governance |
| 14. packages mesh | ☑ | PR #190. 33/33 T3 controls reconciled — **0 governed, 15 of 33 disabled (45%), the highest disabled RATIO of any T3 surface**, and the reason is singular: Packages is the owner of the extension lane's missing middle and its surface is an HONEST RENDERING OF THAT ABSENCE. 3 rehome · 1 rebind · 3 retire-at-cutover · 2 blocked-missing-route · 1 blocked-missing-capture. **Owns journey stages 5, 6 AND 7 — more than any other surface — and owns none of them in code.** Records the inversion at the heart of the lane: the estate has MARKETPLACE admission (optional) and not PACKAGE admission (required). Ontology-side statement of the same gap: manifests are real, listings are real, and NOTHING BINDS THEM. Ruling: installing an ontology pack must NOT make its ontology locally canonical. `/__apps/registry` is doubly blocked — the one capture depicting a versioned artifact registry cannot be inspected AND the registry it depicts has no routes |
| 15. developer-console mesh | ☑ | PR #191. **0 of 563** T3 controls; T2 carries just 14 controls, 0 disabled — the SMALLEST T2 footprint of any owner application, against one of the widest canon scopes. 3 rehome · 1 pattern-harvest · **3 blocked-missing-capture** — the highest block ratio in the estate, incl. `/__apps/devconsole`, the one capture depicting ODK ON-RAMPS, on the surface canon makes their owner. Honest summary: this surface has almost no usable seed evidence, so its build is led by canon and the daemon rather than by ported UX. **Canon's clearest exposer role in the estate — the kit's on-ramps (CLI, templates, scaffolds, generated SDKs, docs, conformance) — and there is NO PANE FOR ANY OF THEM.** A kit whose only entry point is a product surface is not a kit. Corrects the packet table: widget authoring is not a descriptor-driven candidate — a widget set could be a descriptor OUTPUT, but the authoring tool cannot be a descriptor (8th write-semantics instance) |
| 16. home mesh | ☑ | PR #192. **The only surface in the run with an EMPTY seed footprint across T3/T4/T5** — no registered surface, no dormant vault, no harvest capture; 0 of the 563 controls. Its whole estate is the T1 shell (116 controls, 2 disabled) plus four T2 lanes (29 controls). 5 rehome (2 of them TO WORK — the New Session composer is Work's affordance advertised from Home, and `work.md` §4 carries the mirror row) · 1 retire-at-cutover · 0 blocked. Records the `/automations` redirect hijack from the SHELL side (`automations.md` §6 records it from the receiving side — same defect, both ends). Ontology `n/a` exactly as the packet table predicted. Keeps the Activate-Goal/New-Session separation as a ruling: one creates a Session, the other activates a GoalRun, and collapsing them would make Home mint the wrong object |
| 17. systems mesh | ☑ | PR #193. Second surface (after Home) with an EMPTY seed footprint across T3/T4/T5; 0 of the 563 controls. Its one T2 readout is **4 controls — the smallest in the estate** — which is the finding stated plainly: the core workspace canon places at the CENTER of the estate has a four-control product surface. 2 rehome · 0 blocked · 1 recorded as build-not-mesh (`system_interface_bindings: []`, zero routes). Owns **journey stage 9**, the stage that separates a VISIBLE extension app from an EFFECTFUL one. Records that the missing plane is a SAFETY PROPERTY TODAY and a capability gap tomorrow, in that order: a generated app could be authored today and could not be made effectful by any path — the estate is failing safe, not failing open |
| 18. projects mesh | ☑ | PR #194. **Corrects its own packet input.** The table said "no dedicated seeds — verify and record honest absence"; the absence is real in T3/T4/T5 (third empty-footprint surface after Home and Systems, 0 of 563) but the T1 `/projects` shell carries **54 controls, 0 disabled — the second-largest T1 shell surface after Home's explorer**. Recorded as a correction, not a new find. 2 rehome · 0 blocked · 1 honest absence (no T2 readout exists). Also corrects the ontology starting point: "project-scoped object sets read-only" — the sets are real and NO ROUTE OR PANE SCOPES THEM BY PROJECT. Ruling: PROJECT SCOPE IS A FILTER, NOT A NAMESPACE. Third dated exemption in the run |
| 19. applications mesh | ☑ | PR #195. Fourth empty-seed-footprint surface (0 of 563; no T3/T4/T5). 2 rehome · 1 retire-at-cutover · 1 build-not-mesh · 1 recorded. Three compiler properties marked must-survive: NEVER re-implements policy, NEVER caches identity-bound output, and on daemon failure preserves safe static inventory with `launchable:false` + `daemon_unavailable` rather than a frozen fake catalog. Holds **stage 7's registration contract** and is **stage 8's landing zone** — and the finding is that the contract admitting an extension application is FULLY SPECIFIED IN CANON WITH NO STORAGE, NO VERB, AND NO ROW. Stage 7 is not partially built; it is specified and unbuilt, which is a better starting position than a half-built one. Ontology: the registration's `declared_object_contract_refs` is the one genuinely ontology-shaped field in the catalog — and A DECLARED OBJECT CONTRACT IS A CEILING, NOT A CAPABILITY |
| 20. settings mesh | ☑ | PR #196. Fifth and last empty-seed-footprint surface (0 of 563; entire seed estate is T1 vendor SPA panes, so the mesh is carve-out-dominated). 5 rehome (3 to OTHER owners) · 1 split-rehome across four owners · 4 retire-at-cutover · 1 recorded. Carries the audit's standing finding forward: **`tier_t1_settings_panes` measured 0 MIRROR-BACKED RPCs** — twenty panes, none backed by the estate's own mirror, which makes the split a REBUILD BY OWNER, not a port. Retires the enterprise-ceiling panes rather than rehoming them as gaps: a pane that cannot render regardless of backend is vendor packaging, and displaying it advertises a product tier the estate does not sell. Records 6 bundle-only panes the census never counted, so the typed-410 sweep covers them. Carries the intel-panes MIRROR ROW for `studio.md` §6.1's split-rehome map. Contains the run's ONE CLEAN EXEMPTION: a projection-only surface has no write to describe — a registration property, not a missing descriptor capability |
| X-2 residual canon deltas (machinery ownership · filed gaps) | ☑ | **PR #197**, one owner-scoped canon PR. Two landing sites. (1) `domain-ontologies-and-data-recipes.md` gains **"Contract limitations found by dogfooding"** under the descriptor-expressibility rule — the four reasons fewer than a dozen panes across twenty surfaces came back expressible: the descriptor cannot bind a **platform object family**; it has **no write semantics** (all three write-shaped patterns unusable by the panes they name); some canonical inboxes are **cross-owner by construction**; and three first-party shapes have **no matching pattern** (trace waterfall, live terminal, code editor). This is exactly what non-negotiable 23 was written to surface. (2) `canon-to-code-delta.md` gains four naming/ownership rows: `machinery` ownership (**open — owner ruling**, both briefs record it and neither claims the surface), the unqualified bare `recipe` route name, `term-boundaries.md:10-12` naming two checker scripts that do not exist (so its alias register is **unenforced**), and the ODK contract-registry M6 gap (record only) |
| X-3 unbound-seed disposition sweep | ☑ | **PR #198** — sweep table appended to `internal-docs/implementation/repo-ux-disposition.md`. **39 of 39 captures homed; ZERO undispositioned; ZERO multi-homed.** All 39 sit in surface-brief mesh ledgers, so none needs a ruled row in the disposition ledger itself; the table is the index into them. Totals: 18 pattern-harvest · **12 blocked-missing-capture** · 6 rebind · 2 blocked-missing-route · 1 rehome. Findings: a third of the harvest estate is EVIDENCE IN NAME ONLY (12 captures do not boot), and the blocks cluster where they hurt most — Developer Console (3 of 4), `workshop` (grammar literally "application builder"), `registry` (depicts the versioned registry that also has no routes). Only 6 rebind, 3 of them the estate's only captures with a declared `reboundLane`. Four retired owner names still label ten captures; all ten rehome and none is revived as a peer |
| X-4 master-guide assembly (`_index.md` regeneration + build-ledger row) | ☑ | **PR #199 — RUN COMPLETE.** `surfaces/_index.md` regenerated as the finished guide's table of contents: the eight-section shape, a 20-row brief table with mesh/ontology/ODK coverage columns + T3-control share + descriptor-expressible count + journey stage, the three companion documents, six cross-surface findings, and a five-step "using this guide to build a surface". Coverage verified from bytes: **20/20 meshed, 20/20 ontology-wired, 20/20 ODK-laned; T3 shares sum to 563, the full census baseline**; 11 descriptor-expressible panes across 4 surfaces, 0 descriptor-rendered. One row added to the build run's ledger recording mesh completion and the three findings that change build sequencing |

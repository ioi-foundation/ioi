...
test("Goal Space tabs expose complete tab and tabpanel semantics", () => {
  ...
  assert.match(goalSpace, /role="tab"[\s\S]*?aria-controls="goal-panel-orchestrations"[\s\S]*?@keydown=\$\{onGoalTabKeydown\}/);
...
test("Goal Space explicit transitions provide stable focus targets without refocusing after async reads", () => {
  for (const key of [
    ...
    "goal-review-heading",
    "goal-detail-heading",
    "orchestration-detail-heading",
    "goal-tab-goals",
    "goal-tab-orchestrations",
  ]) {
...
test("detail reads are generation-bound and stale responses cannot replace newer navigation", () => {
  ...
  assert.match(goalSpace, /state\.detailOrchestration !== detail/);
...
test("Goal Space shows loading and endpoint-specific failures instead of false empty truth", () => {
  ...
  assert.match(goalSpace, /Orchestrations could not be loaded/);
  assert.match(goalSpace, /GoalRun events could not be loaded/);
  assert.match(goalSpace, /Loading GoalRun owner truth/);
  assert.match(goalSpace, /Loading orchestration projections/);
});
...
test("Goal Space exposes real daemon-owned GoalRun mutations and composition-owned orchestration revisions without fake completion", () => {
  for (const key of [
    "goal-run-create",
    "goal-start",
    "goal-reconcile",
  ...
  assert.match(goalSpace, /validateOrchestrationCompose/);
  assert.match(goalSpace, /validateOrchestrationMembership/);
  assert.match(goalSpace, /expected_head: detail\.entry\.head/);
  assert.doesNotMatch(goalSpace, /\/api\/ioi\/rooms/);
  assert.doesNotMatch(goalSpace, /goal-orchestration\/outcome-rooms/);
  ...
  assert.match(goalSpace, /Orchestration membership result is uncertain\. Reload from the daemon/);

...
test("every current state-changing API route rejects cross-site browser metadata before any effect", async () => {
  const jsonRoutes: Array<[string, string]> = [
    ...
    ["POST", "/api/ioi/orchestrations"],
    ["POST", "/api/ioi/orchestrations/orc_123/goal-runs/attach"],
    ["POST", "/api/ioi/orchestrations/orc_123/goal-runs/detach"],
    ["POST", "/api/ioi/orchestrations/orc_123/transition"],

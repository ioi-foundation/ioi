...
export function createSessionAuthority({ secret, issuer = 'aiagent.xyz', developmentAuthority = false, cookieName, ttlSeconds = 3600 } = {}) {
  if (typeof secret !== 'string' || secret.length < 32) throw new Error('IOI session signing secret must contain at least 32 characters');
  const name = cookieName || (developmentAuthority ? 'ioi_aiagent_dev_session' : '__Host-ioi_aiagent_session');
  ...
  const issueSession = ({ principalRef, tenantRef, lifetimeSeconds = ttlSeconds } = {}) => {
    ...
    const payload = {
      schema_version: 'ioi.product-session.v1',
      issuer,
      principal_ref: principalRef,
      tenant_ref: tenantRef,
      csrf_token: randomBytes(24).toString('base64url'),
      session_id: randomBytes(18).toString('base64url'),
      issued_at: issuedAt,
  ...
  const authenticate = (request) => {
    if (request.headers['x-ioi-principal'] || request.headers['x-ioi-tenant']) {
      throw new SessionError(400, 'browser_authority_headers_forbidden', 'Browser requests cannot supply principal or tenant authority headers');
    }
    const token = cookies(request)[name];
    if (!token) throw new SessionError(401, 'session_required', 'An authenticated product session is required');
    ...
    let payload;
    try { payload = JSON.parse(decoded(body)); } catch { throw new SessionError(401, 'invalid_session', 'Product session payload is invalid'); }
    const now = Math.floor(Date.now() / 1000);
    if (payload.schema_version !== 'ioi.product-session.v1' || payload.issuer !== issuer || !validPrincipal(payload.principal_ref) || !validTenant(payload.tenant_ref) || !payload.csrf_token || !Number.isSafeInteger(payload.expires_at) || payload.expires_at <= now) {
      throw new SessionError(401, 'invalid_session', 'Product session is expired or malformed');
    }
    return payload;
  };
  ...
  const assertMutation = (request, payload, publicOrigin) => {
    ...
    if (!origin || origin !== expectedOrigin || (request.headers['sec-fetch-site'] && request.headers['sec-fetch-site'] !== 'same-origin')) {
      throw new SessionError(403, 'cross_origin_mutation_forbidden', 'Mutation origin does not match the product origin');
    }
    const csrf = request.headers['x-ioi-csrf'];
    if (typeof csrf !== 'string' || !safeEqual(csrf, payload.csrf_token)) throw new SessionError(403, 'csrf_invalid', 'CSRF token is missing or invalid');
  };

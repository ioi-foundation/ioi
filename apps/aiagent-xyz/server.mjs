...
const serveBuiltUi = process.env.IOI_SERVE_BUILT_UI === '1';
...
if (production) throw new Error('aiagent.xyz production is refused: canonical package-release, benchmark, settlement, authority, and managed-instance owner contracts are not all registered');

const storePath = process.env.IOI_AIAGENT_STORE_PATH || path.join(root, '.data', 'marketplace.json');
const store = await new JsonStore(storePath, seedState).init();
const service = new AiagentService(store, createOwnerAdapters(ownerConfig));
const sessionAuthority = createSessionAuthority({
  secret: process.env.IOI_SESSION_SECRET || 'aiagent.xyz-loopback-development-session-secret',
  developmentAuthority,
});
...
let vite;
if (!production && !serveBuiltUi) {
  const { createServer: createViteServer } = await import('vite');
  vite = await createViteServer({ root, server: { middlewareMode: true, hmr: false }, appType: 'spa' });
}

const mime = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.svg': 'image/svg+xml', '.png': 'image/png' };
const serveProduction = async (request, response) => {
  const requested = new URL(request.url, 'http://localhost').pathname;
  const distRoot = path.join(root, 'dist');
  const relative = requested === '/' ? 'index.html' : requested.replace(/^\/+/, '');
  let filePath = path.resolve(distRoot, relative);
  if (filePath !== distRoot && !filePath.startsWith(`${distRoot}${path.sep}`)) filePath = path.join(distRoot, 'index.html');
  try {
...
const server = createServer(async (request, response) => {
  ...
  return serveProduction(request, response);
});

server.listen(port, host, () => {
  console.log(`aiagent.xyz listening on http://${host}:${port} (${developmentAuthority ? 'development authority' : 'network authority'})`);
});

...
(Mock fixtures: `internal-docs/reverse-engineering/ioi/public/api/ioi.v1.*`.)
...
- Speaks **IOI's own protocol** (model-mount kernel: sessions / turns / events, preview
  servers), **not** IOI proto — so each endpoint needs a shape-mapping adapter.
...
- `EventService/WatchEvents` — daemon emits **per-thread SSE**; the frontend wants a
  **global** IOI-shaped Connect stream. Stays a safe end-frame until a bridge is built +

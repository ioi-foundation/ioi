...
async function run() {
  ...
  if (harnessRan) {
    ...
    // THE HARDENED-GATE PROOF (#34 review): the two daemon_wired surfaces (schema, approvals) are
    // FAITHFUL ports — they match the reference THEME and reproduce its IA landmarks. A wired-but-not-
    ...
    ok("every daemon_wired surface (schema + approvals) is a FAITHFUL port: visual_parity + theme MATCH (light/light) + full landmark reproduction", [schema, appr].every((s) => s && s.visual_parity === true && s.theme_match === true && s.reference_theme === "light" && s.ioi_theme === "light" && s.landmark_covered === s.landmark_applicable && s.landmark_applicable >= 6), `schema ${schema?.landmark_covered}/${schema?.landmark_applicable} · approvals ${appr?.landmark_covered}/${appr?.landmark_applicable}`);
    ok("the gate has teeth: explorer reproduces the shell REGIONS (structural_parity true) but the gate REFUSES visual_parity (blank reference / no landmark spec) → reference_ported", expl && expl.structural_parity === true && expl.visual_parity === false && expl.matrix_class === "reference_ported", `expl structural=${expl?.structural_parity} visual=${expl?.visual_parity} class=${expl?.matrix_class}`);

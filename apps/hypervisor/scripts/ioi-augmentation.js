  ...
  function renderNsAgentPreview() {
    ...
    fetch("/__ioi/api/ioi-agent/preview", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) })
      ...
      .then(function (j) {
        ...
        if (j.policy_rollout) lines.push('<span class="nsp-k">Rollout</span> policy via <b>' + esc((j.policy_rollout.rollout_mode || "") + (j.policy_rollout.rollout_state === "promoted" ? " (promoted)" : "")) + '</b> overlay ' + esc(String(j.policy_rollout.variant_policy_ref || "").replace("ioi-agent-policy://", "")) + ' <span style="color:#6f7280">(' + esc(j.policy_rollout.reason_code || "") + ' · base ' + esc(String(j.policy_rollout.base_policy_ref || "").replace("ioi-agent-policy://", "")) + ')</span>');

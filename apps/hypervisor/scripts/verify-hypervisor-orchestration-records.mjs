...
async function run() {
  ...
  try {
    ...
    ok("[membership] the reciprocal member is REPORTED, never hidden: this GoalRun is a coordinate the daemon does not hold, so the seam revision stays durable and the composer carries the daemon's own refusal — 404 goal_run_not_found, the shape a local-development request gets, where the identity-enforced posture instead answers the least-disclosure 403 asserted under [portal] below (R-190; the m3 GoalRun plane gate drives the stamp on a real run)",
      attached.value?.member_stamp?.stamped === false && attached.value?.member_stamp?.refusal?.status === 404 && attached.value?.member_stamp?.refusal?.code === "goal_run_not_found" && attached.value?.member_stamp?.orchestration_ref === "app-scope://ioi-ai/orchestration/orc_s4c_first",
    ...
    ok("[portal] the reciprocal stamp reaches the GoalRun plane under the portal session through the /v1/goal-orchestration/ prefix it already carries (R-190, measured): the HANDLER answers (403 goal_run_mutation_owner_mismatch — the GoalRun plane's least-disclosure refusal for a run this principal does not own, absent or not), never the session's route-scope fence",
      portalStamp.status === 403 && portalStamp.body?.error?.code === "goal_run_mutation_owner_mismatch",
      `${portalStamp.status}/${portalStamp.body?.error?.code}`);

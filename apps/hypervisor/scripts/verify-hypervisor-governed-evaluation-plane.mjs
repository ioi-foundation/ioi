...
//   [P14 reproduction] (M12.5, R-208): the selected result reproduced against the unchanged epoch
//
...
async function run() {
  ...
  // -- [P14 reproduction] (M12.5, R-208): the selected result reproduced against the UNCHANGED epoch ------
  // The selected result is the baseline SEALED pass on route A. A reproducer re-invokes the same route on
  // the same cases (the stub is deterministic, and the answers are compared here), admits a run under the
  // independent_reproduction lane as an independent_reproducer bound to the SAME epoch, suite revision and
  // evaluator revision, and its result must carry the same verdict and the same per-case observations under
  // the same epoch_frozen_root — retained beside the original as its own immutable result. A reproduction
  // that asserts another frozen root is refused by name: against the unchanged epoch, or it is not one.
  const reproEvidence = [];
  for (const [i, prompt] of PROMPTS.entries()) reproEvidence.push(await invoke(routeA.id, `gep-invoke-repro-${i}`, prompt));
  const sameAnswers = reproEvidence.length === evidenceA.length && reproEvidence.every((e, i) => e.status === 200 && invocationOutput(e) === invocationOutput(evidenceA[i]));
  const runRepro = await post(RUNS, runBody("gep-run-repro", "acme.reproduction", { lane: "independent_reproduction", submitter_role: "independent_reproducer", execution_evidence_refs: reproEvidence.map((e) => e.ref) }));
  const xRunRepro = runRepro.body?.evaluation_run ?? {};
  ok("[P14 reproduction] a run admits under the independent_reproduction lane as an independent_reproducer, bound to the SAME active epoch (frozen root copied from the epoch), suite revision and evaluator revision as the selected result, citing three fresh wire-crossing invocations of the same route whose answers equal the originals", runRepro.status === 201 && xRunRepro.lane === "independent_reproduction" && xRunRepro.submitter_role === "independent_reproducer" && xRunRepro.epoch_frozen_root === FROZEN_ROOT && xRunRepro.suite_revision_ref === SUITE_REF && xRunRepro.evaluator_revision_ref === EVALUATOR_REF && sameAnswers, `${runRepro.status} ${code(runRepro.body)} · answers ${sameAnswers ? "identical" : "DIFFER"}`);
  const reproObservation = (i) => ({ observation_id: `obs-${i + 1}`, case_commitment: TASKS[i].source_commitment, outcome: "pass", score_milli: 1000, evidence_refs: [reproEvidence[i].ref] });
  const resRepro = await resultOn("acme.reproduction", resultBody("gep-result-repro", "acme.result-reproduction", { observations: [reproObservation(0), reproObservation(1), reproObservation(2)] }));
  const xRepro = resRepro.body?.evaluation_result ?? {};
  const observed = (x) => JSON.stringify((x.observations || []).map((o) => [o.case_commitment, o.outcome, o.score_milli]));
  ok("[P14 reproduction] the reproduction's result carries the SAME verdict and the same per-case outcomes and scores as the selected result under the same epoch_frozen_root, suite and evaluator revisions — reproduced against the unchanged epoch and target base — and is retained as its own immutable result beside the original, never a rewrite of it", resRepro.status === 201 && xRepro.verdict === xBS.verdict && xRepro.epoch_frozen_root === xBS.epoch_frozen_root && xRepro.suite_revision_ref === xBS.suite_revision_ref && xRepro.evaluator_revision_ref === xBS.evaluator_revision_ref && observed(xRepro) === observed(xBS) && xRepro.evaluation_result_id !== xBS.evaluation_result_id && xRepro.content_hash !== xBS.content_hash, `${resRepro.status} ${code(resRepro.body)} · ${xRepro.verdict} vs ${xBS.verdict}`);
  const reproWrongRoot = await post(RUNS, runBody("gep-run-repro-wrongroot", "acme.reproduction-wrongroot", { lane: "independent_reproduction", submitter_role: "independent_reproducer", expected_epoch_frozen_root: H("f"), execution_evidence_refs: reproEvidence.map((e) => e.ref) }));
  ok("[P14 reproduction] a reproduction asserting another frozen root is refused (epoch_binding_mismatch): a reproduction is against the UNCHANGED epoch or it is not one", reproWrongRoot.status === 409 && code(reproWrongRoot.body) === "epoch_binding_mismatch", `${reproWrongRoot.status} ${code(reproWrongRoot.body)}`);
  ...
  ok("results list by ref: the retained set holds every verdict class admitted above, the reproduction among them", resultList.length === 8 && resultList.includes("evaluation-result://acme.result-reproduction") && resultList.includes("evaluation-result://acme.result-blocked") && resultList.includes("evaluation-result://acme.result-nd"), `${resultList.length} results`);
  ...
  ok("[P10] the incumbent route is hard-DISABLED through the registry's own lifecycle route, and the stub behind it received no further request (six so far: the three originals and the three reproductions)", disabled.status === 200 && disabled.body?.route?.lifecycle?.status === "disabled" && routeARecord.lifecycle?.status === "disabled" && stubA.calls.chat === 6, `${disabled.status} ${code(disabled.body)} ${routeARecord.lifecycle?.status}`);
  const invokeDisabled = await invoke(routeA.id, "gep-invoke-a-after-disable", "triage: anything");
  ok("[P10] invoking the disabled incumbent is refused by the registry (model_route_not_executable) — the removal is real, not a label", invokeDisabled.status === 409 && code(invokeDisabled.body) === "model_route_not_executable" && stubA.calls.chat === 6, `${invokeDisabled.status} ${code(invokeDisabled.body)}`);
  ...
  ok("[P9] the impact projection BEFORE any challenge: the evaluator is active, and every run (ten, the reproduction among them), result (nine) and epoch (two) bound to it is listed as a dependent", runPre.status === 201 && impactBefore.standing === "evaluator_active" && (impactBefore.dependent_runs || []).length === 10 && (impactBefore.dependent_results || []).length === 9 && (impactBefore.dependent_epochs || []).length === 2 && impactBefore.dependent_results.every((r) => r.standing === "evaluator_active"), `${runPre.status} ${impactBefore.standing} runs=${(impactBefore.dependent_runs || []).length} results=${(impactBefore.dependent_results || []).length} epochs=${(impactBefore.dependent_epochs || []).length}`);

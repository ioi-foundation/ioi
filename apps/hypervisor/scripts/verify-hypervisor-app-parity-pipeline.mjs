...
async function run() {
  ...
  ok("no false parity elsewhere: schema+approvals stay daemon_wired, explorer is daemon_wired only via its #46 certified origin-aligned correction, lineage/vertex stay substrate_bound", bySlug.schema?.parity_class === "daemon_wired" && bySlug.approvals?.parity_class === "daemon_wired" && (bySlug.explorer?.parity_class !== "daemon_wired" || (bySlug.explorer?.shell_pixel_certified === true && !!bySlug.explorer?.reference_url_override)) && ["substrate_bound", "reference_ported"].includes(bySlug.lineage?.parity_class) && bySlug.vertex?.parity_class === "substrate_bound");

...
async function run() {
  ...
  ok("daemon_wired is now the two faithful ports (schema + approvals); reference_capture is the majority", bySlug.schema?.parity_class === "daemon_wired" && (matrix.by_parity_class?.daemon_wired || 0) === 2 && (matrix.by_parity_class?.reference_capture || 0) > (matrix.by_parity_class?.daemon_wired || 0) && !(matrix.seeds || []).some((s) => s.parity_class === "covered"));

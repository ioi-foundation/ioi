...
- it reuses a running `npm run dev:hypervisor-code-editor-adapter-host` session
...
def find_window(window_pattern: str) -> WindowGeometry:
    ...
    if not ids:
        raise RuntimeError(
            "Could not find a Hypervisor client window. Start `npm run dev:hypervisor-code-editor-adapter-host` first."
        )

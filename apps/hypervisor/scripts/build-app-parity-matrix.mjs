...
const REFERENCE_PORTED = {
  ...
  pipeline: { port_surface: "/__ioi/pipeline", surface_name: "Data", reference_workspace: "/workspace/builder/", parity_blocked: "NOT data-clean (proven by verify-pipeline-reference-data-clean.mjs, not prose): the /__apps/pipeline proxy errors ('An error occurred'); the /workspace/builder/ landing renders its list IA but the data lanes fail ('Unable to load pipelines' / 'Failed to load examples'); a pipeline CANVAS ('…/sandbox/…') shows 'Failed to initialize'. ROOT CAUSE: the builder shell + eddie backend ARE captured, but the canvas is a lazy webpack chunk whose hash is MISSING from the frontend asset store, so the mirror serves an empty stub → the canvas crashes (missing chunks e.g. bfed3b6a… / c806694d…). daemon_wired blocked until a FRESH-session re-harvest backfills those chunks (refresh_auth + download_missing_assets) — see reharvest-pipeline-builder.mjs; #37 landed the re-harvest infra + the data-clean gate", binding: "ported Pipeline Builder shell over the real ODK authority ladder (DataSource → ... → MaterializedObjectSet) as canvas node cards; live/declared/missing per stage from daemon truth; preview rows + output schema from the real projection + materialized set", note: "shell ported + wired (source-neutral builder shell: rail/header/toolbar/canvas/right/tray, NOT automationsShell; Build+Preview supported, Schedule+Deploy disabled in place); NOT daemon_wired — parity verification blocked on an errored local reference" },

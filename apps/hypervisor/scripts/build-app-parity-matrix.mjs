...
const SUBSTRATE_BOUND = {
  ...
  // JOB-1 (2026-08-20): the `jobs` seed was PROMOTED to reference_ported (the Builds port at
  // /__ioi/missions/builds). Its substrate binding is NOT dropped — it is CARRIED FORWARD verbatim
  // onto the REFERENCE_PORTED.jobs overlay below, so the row still names /__ioi/missions and the
  // read_only_by_contract substrate stays bound. One source of truth: the entry lives there now.
...
const REFERENCE_PORTED = {
  ...
  // DEV-1 (remediation v2): origin-aligned I-4 landing over a TYPED-ABSENT body (no widget plane).
  widgets: { port_surface: "/__ioi/developer-console/widgets", surface_name: "Developer Console", reference_url_override: "http://localhost:9225/workspace/custom-widgets/", reference_landmarks: ["Custom Widgets", "New widget set", "Recents", "Favorites"], binding: "I-4 splash landing (atlas: 5 facet groups) over a TYPED-ABSENT body — no widget plane exists; New-widget-set + body are named absences under the unified contract", note: "DEV-1: origin-aligned landing ported; create lane dead on the mirror (atlas authoring state)" },
  // WOR-1 (remediation v2): workspaces over the REAL environments plane; notepad TYPED-ABSENT.
  workspaces: { port_surface: "/__ioi/developer-workspace/workspaces", surface_name: "Workbench", reference_url_override: "http://localhost:9225/workspace/code-workspaces/splash", reference_landmarks: ["Code Workspaces", "New workspace", "Recents", "Favorites"], binding: "I-4 splash landing (atlas: 5 facet groups) over the REAL environments plane — newest 15 with the cap NAMED; creation stays the governed environment ladder (named gap)", note: "WOR-1: origin-aligned; the estate's workspace substrate IS the environments plane (610-record census at port time)" },
  notepad: { port_surface: "/__ioi/developer-workspace/notepad", surface_name: "Workbench", reference_url_override: "http://localhost:9225/workspace/notepad/", reference_landmarks: ["Notepad", "New document", "Recents", "Favorites"], binding: "I-4 splash landing (atlas: 5 facet groups) over a TYPED-ABSENT body — no document plane; the reference template-picker IA (4 facet groups) is atlas-recorded for a future cut", note: "WOR-1: origin-aligned landing ported; document plane absent (named)" },
  // PRO-1.build (remediation v2): the Monocle re-shell — Preview re-chromed in place (the SAME
  // graph body, light shell), History/Build-timeline LIVE lanes, 3 absences typed per-tab.
  lineage: { port_surface: "/__ioi/lineage", surface_name: "Provenance", reference_url_override: "http://localhost:9225/workspace/monocle/", reference_landmarks: ["Preview", "SQL scratchpad", "History", "Code", "Build timeline", "Data health"], binding: "the light Monocle shell over the SAME substrate truth: Preview = the lineage graph body re-chromed in place; History = the proof stream (work-ledger) newest-30 cap named; Build timeline = real run transcripts with state_root proof anchors; SQL/Code/Data-health typed absences (per-tab adjudication #lineage-tabs)", note: "PRO-1.build: the census's richest miss closed — 6-tab Monocle IA live over the existing substrate; /provenance landing designation = Monocle (D1)" },
  // PRO-2.build (remediation v2): the light Vertex shell — the SAME graph body re-chromed in place.
  vertex: { port_surface: "/__ioi/vertex", surface_name: "Provenance", reference_url_override: "http://localhost:9225/workspace/vertex/", reference_landmarks: ["Vertex", "New exploration"], binding: "the light Vertex shell over the SAME substrate truth (graph body re-chromed in place); New exploration = typed absence (the reference create lane is dead on the mirror; adjudication #vertex)", note: "PRO-2.build: re-shell complete; sibling of Monocle under the Provenance family (D4 — the /provenance landing is Monocle by D1)" },
  // DEV-2 (remediation v2): the Developer-Console landing over the REAL connector estate.
  devconsole: { port_surface: "/__ioi/developer-console", surface_name: "Developer Console", reference_url_override: "http://localhost:9225/workspace/developer-console/", reference_landmarks: ["Developer Console", "Applications", "Standalone OAuth clients", "New application"], binding: "the console landing with the Applications lane LIVE over the real connector estate (declared connectors + SCM connectors: name · service · auth posture · declared tools · created; verbs stay on Connections/owner surfaces); OAuth-clients + guided-create typed absences naming the real lanes", note: "DEV-2: Applications-as-registrations is the adjudicated mapping (#devconsole); the route previously fell through to the SPA bundle — this is the family's designed landing" },
  // EVA-2.build (remediation v2): the Insight landing — Object-sets LIVE, Workbooks typed-absent.
  ...
  // MAP-1 (remediation v2): the CANVAS-grammar port — and the leg where the honest move is a
  // REFUSAL to draw. The mirror capture was blocked_missing_capture (absent_confirmed); the
  // owner-authorized live sweep overturned it and recorded a REAL geospatial workbench (20
  // controls · 2 canvas surfaces · heading "No layers"). No geospatial plane exists anywhere in
  // the daemon's 753 routes, so the LANDING grammar ports and the canvas is a typed absence that
  // names WHICH absence it is. No reference_url_override: the mirror lane stays capture-blocked
  // and the live tenant is not a harness-openable origin — the committed live atlas is the evidence.
  map: { port_surface: "/__ioi/environments/map", surface_name: "Environments", live_reference_evidence: "reference-live-tenant-deep-atlas.v1.json#map (landing) + reference-live-tenant-atlas.v1.json#map", reference_landmarks: ["Map", "Add to map", "No layers", "Layers", "Legend", "Histogram", "Timeline", "Select", "Search Around", "Selection", "Save as\u2026"], binding: "the live-tenant MAP workbench grammar (header + Save-as \u00b7 left layer panel with its tab strip, Add-to-map and basemap footer \u00b7 select/search-around + six draw tools \u00b7 selection tray \u00b7 zoom + timeline under-bar) over a canvas region that is a TYPED ABSENCE: the estate has NO geospatial plane (no coordinates, geometry, tiles, geocoder or layer store in 753 daemon routes; sweep of 247 param-free GET routes recorded in #map-port), so nothing map-shaped is drawn \u2014 no canvas element, no rendered geometry, no borrowed vendor basemap. The one LIVE lane is deliberately NOT a map: the REAL placement plane's cloud-resource candidates that carry placement geography (region \u00b7 location \u00b7 zone \u00b7 az), each row naming WHICH field its value came from and carrying the daemon's own status / coverage_state / not-live-supply risk wording verbatim", note: "MAP-1: live_ia_recorded \u2192 ported. A map CANVAS cannot be honestly faked, so the canvas is a named absence that distinguishes MISSING from the reference's own EMPTY layer list \u2014 and the estate's only location-bearing records render beneath it as an explicitly NON-map list" },
  // REG-1 (remediation v2): the REGISTRY port â the leg where the honest move is a FOUR-WAY
  // read-state census. The mirror capture was blocked_missing_capture (absent_confirmed, MAR-1
  // "expresses no IA"); the owner-authorized live sweep OVERTURNED it and recorded a real
  // artifact-registry landing (14 controls · 1 search input · 0 repository rows). Unlike map, the
  // estate HAS registry planes — so the adjudication was which plane backs which lane, and what
  // each answers to THIS surface's identity. No reference_url_override: the mirror lane stays
  // capture-blocked and the live tenant is not a harness-openable origin.
  registry: { port_surface: "/__ioi/marketplace/artifacts", surface_name: "Marketplace", live_reference_evidence: "reference-live-tenant-deep-atlas.v1.json#registry (landing) + reference-live-tenant-atlas.v1.json#registry", reference_landmarks: ["Artifacts", "Explore artifacts", "Create artifact repository", "Help", "Search artifacts…", "Learn about artifacts", "Go to documentation", "Core concepts", "Publish an artifact", "Search an artifact", "Recall an artifact"], binding: "the live-tenant ARTIFACT-REGISTRY landing grammar (header + task tray + Help + Create-artifact-repository · centred Explore hero with its ecosystem scope + search bar · four-card Learn band with its documentation link) over the estate's REAL registry LADDER, rendered as a READ-ONLY projection: ten daemon planes probed on every render and classified live into FOUR states — LIVE (records rendered verbatim, each row naming WHICH plane it came from, with id / ref / schema_version and a typed dash for every field the record does not carry), EMPTY (the plane answered and holds none — the reference's own state), REFUSED (identity-first planes that refuse this read, the daemon's typed code rendered verbatim and NEVER as a zero) and NO READ ROUTE (POST-only planes that cannot be read at all). The package family's eight-route closure and the zero artifact-search routes are re-counted from the daemon's route index on every render. Every app control the reference offers is a typed absence with a reason written for it; the estate's registry verbs stay on the Packages + Marketplace owner surfaces and are linked, never duplicated", note: "REG-1: live_ia_recorded → ported. The estate HAS registry planes, so the finding is not absence but DISCRIMINATION: a refusal is not a zero, an empty plane is not a missing one, and a plane with no read route is neither — every plane's state is classified from its own live response and stamped on its row" },
  // JOB-1 (remediation v2): the BUILDS port — the leg whose finding is that NO SINGLE PLANE lists
  // every build. The mirror-scoped verdict was absent_confirmed (#jobs, MIS-1.recon: "no Builds
  // grammar expresses at any recorded state; the substrate STANDS as the surface"); the
  // owner-authorized live sweep OVERTURNED it and recorded a real build tracker (title "Your builds
  // · Builds" · 14 controls · left Search-by/Filter-by panel · All/Running/Finished segmented
  // control · a six-column build table over REAL rows). The seed's substrate binding is CARRIED
  // FORWARD on this overlay (substrate_surface stays /__ioi/missions): the port is a SIBLING lane
  // beside the read_only_by_contract substrate, never a replacement for it. No
  // reference_url_override: the mirror lane stays capture-blocked and the live tenant is not a
  // harness-openable origin.
  jobs: { port_surface: "/__ioi/missions/builds", surface_name: "Missions", substrate_surface: "/__ioi/missions", live_reference_evidence: "reference-live-tenant-atlas.v1.json#jobs (landing, title \"Your builds · Builds\", 14 controls) + .artifacts/live-tenant-atlas/jobs-landing.png", reference_landmarks: ["Builds", "Your builds", "Started by", "Start time", "Duration", "Status", "Running", "Finished", "Clear all filters", "Show only my builds", "All branches", "Only master branch", "Job type"], binding: "the live-tenant BUILDS grammar (header + counted task tray · left Search-by/Filter-by panel · All/Running/Finished segmented census · the build table) over the estate's REAL build plane, rendered as a READ-ONLY projection: the work ledger's `run` lane — the EXACT UNION of the agent-run-transcript projection and the per-automation execution projection, and the only plane carrying BOTH the acting authority AND the state root on one record. Sixteen run/build planes are probed on every render and classified live into FOUR states (LIVE / EMPTY / REFUSED / NO READ ROUTE, the last decided from the daemon's own route index rather than a status guess). The three counts that justify the plane choice (ledger total · transcript-reachable · fan-out-unreachable) are re-derived on every render. The reference's Outputs and Actions columns bind NO estate field and are stated as refusals rather than filled with different truth; a build's sub-unit is a STEP and is never relabelled a job, because `job` is a live estate noun on /v1/jobs. The 3174-record transcript plane is LINKED to the Monocle Build timeline, never re-rendered. The substrate lane at /__ioi/missions is untouched", note: "JOB-1: live_ia_recorded → ported, superseding the mirror-scoped absent_confirmed (#jobs) which stands as history. The finding is that no single plane lists every build: the transcript plane records a state_root and no principal, the per-automation projection records a principal and cannot reach executions whose automation was deleted, and the work-ledger run lane is the exact union of both. Substrate binding carried forward — /__ioi/missions stays bound and read_only_by_contract" },
  scheduler: { port_surface: "/__ioi/missions/schedules", surface_name: "Missions", live_reference_evidence: "reference-live-tenant-deep-atlas.v1.json#scheduler (landing, url /workspace/scheduler/find, title \"Build Schedules\", 15 controls, 1 input, 1 facet group APPLICATIONS, 0 rows) + .artifacts/live-tenant-atlas/deep/scheduler-landing.png", reference_landmarks: ["Build Schedules", "Create schedule", "Set search parameters", "Sorted by most recently updated", "Select schedules\u2026", "Filter by name or rid\u2026", "APPLICATIONS", "Results matching"], binding: "the live-tenant BUILD-SCHEDULES find/list grammar (header + a three-counter tray + Create-schedule \u00b7 an N-schedules count band with its Results-matching principal chip and Set-search-parameters \u00b7 a sort / select / filter toolbar with the name-or-rid input \u00b7 the APPLICATIONS facet lane \u00b7 the list band) over the estate's ONLY cadence records, rendered as a READ-ONLY projection: the automation records carrying a schedule_spec, read from /v1/hypervisor/automations because that is the COMPLETE projection \u2014 the pre-filtered /v1/hypervisor/operations scheduler lane drops catch_up_policy, misfire_policy and executor_identity, and the field-by-field comparison that proves it is recomputed on every render. Fifteen schedule- and cadence-adjacent planes are probed on every render and classified live into FOUR states (LIVE / EMPTY / REFUSED / NO READ ROUTE, the last decided from the daemon's own route index) across THREE shapes (a collection, a status SINGLETON that is not a collection of one, and a pure COMPUTATION that is not a plane at all). Every schedule count names the PREDICATE that produced it and the WINDOW it covers: the records predicate (carries a schedule_spec), the loop predicate (the heartbeat's own scheduled_active), and the tray's dispatch/misfire counters which are a SINGLE TICK and are never printed as lifetime totals. The reference's Create-schedule verb is a typed absence by design \u2014 the estate mints no schedule object, so the verbs are create/patch on Automations \u2014 and the fires are linked to the Builds port rather than re-listed", note: "SCH-1: live_ia_recorded \u2192 ported, superseding the mirror-scoped absent_confirmed (#scheduler) which stands as history. The finding is that the field NAMED for the concept is not the field the runtime acts on: trigger / trigger_kind reads 'manual' on the very automation the scheduler loop fires on a cron, so a trigger-shaped view would have reported zero schedules while the loop was firing one. The daemon also states its own dispatch path \u2014 the loop fires through the manual-run route \u2014 so no plane marks a past run as scheduler-dispatched, and this surface answers when a schedule WILL fire while refusing to answer which past runs WERE fires. The seed carried NO substrate binding and none is invented for it: the Operations boundary the mirror-scoped record drew still stands \u2014 scheduler LOOP HEALTH is Operations-owned and is LINKED, while this port renders the schedule RECORDS, a lane no surface held" },
  ingest: { port_surface: "/__ioi/data/ingest", surface_name: "Data", live_reference_evidence: "reference-live-tenant-deep-atlas.v1.json#ingest (landing, url /workspace/hyperauto/pipeline, title \"HyperAuto\", 10 controls, 0 inputs, 1 facet group APPLICATIONS, 4 dialogs, 0 rows) + .artifacts/live-tenant-atlas/deep/ingest-landing.png", reference_landmarks: ["HyperAuto", "HyperAuto Pipelines", "Create pipeline", "Go to Data Connection Application", "APPLICATIONS"], binding: "the live-tenant HYPERAUTO landing grammar (a header carrying the app name and its Create-pipeline verb \u00b7 one centred card titled \"HyperAuto Pipelines\" with the same verb repeated in its head \u00b7 the card body, whose reference state is an EMPTY list carrying a badge, its own copy and a Go-to-Data-Connection link \u00b7 the APPLICATIONS facet lane) over the estate's REAL ingestion CHAIN, rendered as a READ-ONLY projection. Each card row is one connector-mapping record joined END TO END on the keys the RECORDS carry \u2014 connector_mapping_id to the policy-bound data view, the transformation run, the ontology projection and the capability-lease plan; data_source_id to the materializing run; materializing_run_id to the connector session; and ontology_projection_id to the materialized object set, the only mapping-keyed path to an output the estate publishes. Every stage chip is present because a RECORD was found and is a typed absence with its own reason otherwise, never because a self-report said so. Twenty-one ingestion-chain and ingestion-adjacent planes are probed on every render and classified live into FOUR states (LIVE / EMPTY / REFUSED / NO READ ROUTE, the last decided from the daemon's own route index) across two shapes (a collection and a census SINGLETON that is not a collection of one). Three counts are re-derived every render and each NAMES its predicate: declared (a mapping record exists), moved (some record in the chain reports source_contacted:true or registers an object set under the chain's own projection), and rows (summed from the OUTPUT records, which are the only ones carrying a verbatim contact receipt). The automatic pipeline BUILDER is a typed absence counted from the route index \u2014 the source-to-objects path is thirteen separate authority-crossing POSTs, every one confirmed against the index on render, and the nearest automatic lane (ontology-proposals) REFUSES this identity, so the surface prints unknown as unknown rather than as absent. Read-only: the chain verbs stay on Pipeline Builder and the Connections lease gate, the source plane and its Syncs lane stay on Data Connection, and the extracted objects stay on the Object Explorer \u2014 all linked, none re-rendered", note: "ING-1: live_ia_recorded \u2192 ported, superseding the mirror-scoped absent_confirmed (#ingest) which stands as history. The finding is that a self-report is scoped to the RECORD that carries it and never to the estate: a source whose rows were fetched under a receipted HTTP 200 still reads ingestion.wired:false because that field is written once at declaration time and no run revises it; a mapping reads health.status:ready while the same object reports object_instances:0 and its own note says it extracts nothing; the plane roll-up states as a whole-plane claim that nothing here moves source data, in an estate holding extracted rows that arrived through the lane beside it; missing_contracts names contracts that already exist for that very mapping because it is computed without reading downstream; and a connector session reports data_moved:false about the crossing its own object set names it as the provenance of. Join on the keys the records carry before believing any of them" },
  // MS-1 (remediation v2): the MODEL STUDIO port — the leg whose finding is that a VERB WHOSE ROUTE
  // EXISTS is still a GAP when the form and the route are not about the same act. The mirror-scoped
  // verdict was capture_broken_no_donor (#modelstudio, FOU-2: "no donor, no plane"); the
  // owner-authorized live sweep OVERTURNED it and recorded a real landing whose ROOT IS A CREATION
  // DIALOG — "Choose file location" over rows:0, with no list root and no editor behind it. No
  // reference_url_override: the mirror lane stays capture-blocked and the live tenant is not a
  // harness-openable origin.
  modelstudio: { port_surface: "/__ioi/foundry/model-studio", surface_name: "Foundry", live_reference_evidence: "reference-live-tenant-deep-atlas.v1.json#modelstudio (landing + create-attempt, url /workspace/model-studio/, title \"Model studio\", heading \"Choose file location\", 13 controls, 1 input, 1 facet group APPLICATIONS, 7 dialogs, 0 rows) + .artifacts/live-tenant-atlas/deep/modelstudio-landing.png + .artifacts/live-tenant-atlas/deep/modelstudio-create-attempt.png", reference_landmarks: ["Model Studio", "Choose file location", "File name", "New Model Studio (", "Location", "Browse", "Cancel", "Save", "APPLICATIONS"], binding: "the live-tenant MODEL-STUDIO landing grammar, which is a CREATION-ENTRY DIALOG rather than a canvas or a list: the app admits you by creating a Model Studio file in a chosen location, and the tenant holds none, so the dialog IS the landing and there is no browse root to port. The dialog renders as the centrepiece (a header carrying the app name and its Save-to-a-location typed absence \u00b7 \"Choose file location\" with its close control \u00b7 a pre-filled File name field \u00b7 a Location folder selector \u00b7 Browse \u00b7 Cancel \u00b7 Save) over the estate's REAL planes: the LOCATION selector reads the projects plane record by record (id, custody posture, created stamp, cap 25 NAMED), and the FILE half reads the Foundry draft-spec and run-plan planes. Twenty-two file-, location-, binding- and authoring-adjacent planes are probed on every render and classified live into FOUR states (LIVE / EMPTY / REFUSED / NO READ ROUTE, the last decided from the daemon's own route index), each stamped with the HALF of the dialog it answers for so an adjacent authoring lane is never read as the file, the location or the binding. The pre-filled name is DERIVED at render exactly as the reference derives it at dialog-open (the two recorded reference states are byte-identical except that stamp) and is marked as stored nowhere; the validity badge keeps its predicate NAMED as well-formedness and explicitly refuses the availability reading, because no plane indexes names inside a location. EMPTY and MISSING are rendered side by side about the same dialog: the spec plane is an EMPTY plane waiting for a record, while the file-in-a-location RELATION is MISSING and could never fill. Read-only: the draft-spec create verb stays where the daemon owns it, the model routes and providers a spec must bind stay on Model Catalog and Agent Studio, and the locations stay on Fusion \u2014 all linked, none re-rendered", note: "MS-1: live_ia_recorded \u2192 ported, superseding the mirror-scoped capture_broken_no_donor (#modelstudio) which stands as history. The finding is that a VERB WHOSE ROUTE EXISTS IS STILL A GAP WHEN THE FORM'S FIELDS AND THE ROUTE'S REQUIRED FIELDS DO NOT INTERSECT. The estate DOES publish POST /v1/hypervisor/foundry/specs \u2014 a real durable draft-spec create \u2014 so the easy conclusion ('the route exists, wire Save') is available and wrong. The record it writes carries no location field of any kind, no project record carries a spec field and no published project write route would accept one, so the containment edge the dialog exists to author is absent in BOTH directions; and the route's own admission test refuses any spec resolving neither a model route nor a provider, a binding the dialog offers no control for. The two field sets intersect in exactly one field, the name. Wiring Save would mean silently supplying an argument nobody chose and silently discarding the one thing the dialog is named for, so Save is a typed absence and no second mutation spine is minted over the daemon's create route" },
  // INF-1 (remediation v2): the INFERENCE port — the EIGHTH and LAST leg of the live-tenant port
  // backlog, whose finding is that a SELECTION THE ROUTE ACCEPTS is still a GAP when the key it
  // filters on is never written by the records it is meant to scope. The mirror-scoped verdict was
  // capture_broken_no_donor (#inference, FOU-2: "no donor; the inference gap stands citing this
  // record"); the owner-authorized live sweep OVERTURNED the expressed-IA half and recorded a real
  // landing whose ROOT IS A SPACE GATE — "Please select a space" over one control and rows:0. No
  // reference_url_override: the mirror lane stays capture-blocked and the live tenant is not a
  // harness-openable origin.
  inference: { port_surface: "/__ioi/foundry/inference", surface_name: "Foundry", live_reference_evidence: "reference-live-tenant-deep-atlas.v1.json#inference (landing, url /workspace/foundry-inference-app/, heading \"Please select a space\", 12 recorded controls of which ONE is the app's own, 0 inputs, 0 tabs, 0 columns, 1 facet group APPLICATIONS, 8 dialogs, 0 rows) + .artifacts/live-tenant-atlas/deep/inference-landing.png", reference_landmarks: ["Inference", "Please select a space", "Select a space\u2026", "APPLICATIONS"], binding: "the live-tenant SPACE-GATE landing grammar, which is neither a wizard nor a list: the app admits you by SELECTING A SPACE, the recorded tenant had none picked, and rows are 0 \u2014 so the gate IS the landing. The gate renders as the centrepiece (a header carrying the app name and a live space/invocation/plane count \u00b7 the heading \"Please select a space\" verbatim \u00b7 the \"Select a space\u2026\" control as a typed absence \u00b7 the estate's REAL space records beneath it, one row per live project record with id, custody posture and created stamp, cap 25 NAMED) over the estate's REAL planes. Twenty-seven space-, invocation-, model- and adjacent planes are probed on every render and classified live into FOUR states (LIVE / EMPTY / REFUSED / NO READ ROUTE, the last decided from the daemon's own route index), each stamped with the HALF of the gate it answers for. The invocation truth band prints the PREDICATE and the WINDOW beside every number: recorded invocations (a work-ledger entry whose kind is goal_run_invocation or harness_execution), the subset naming a model, the model_invocation receipts on the model-mount store, the metered usage records, and the registered routes and session bindings that are the estate's OWN scoping noun. The capture's PLATFORM NEWS DIALOG, its two controls and the rail's last-visited-application entry are named as EXCLUDED JUNK EVIDENCE rather than ported. Read-only: the invoke verb stays where the daemon owns it, the model routes stay on Model Catalog and Agent Studio, the spaces stay on Fusion, and the ledger's run lane stays on Builds and Schedules \u2014 all linked, none re-rendered", note: "INF-1: live_ia_recorded \u2192 ported, superseding the mirror-scoped capture_broken_no_donor (#inference) which stands as history. The finding is that A SELECTION THE ROUTE ACCEPTS IS STILL A GAP WHEN THE KEY IT FILTERS ON IS NEVER WRITTEN BY THE RECORDS IT IS MEANT TO SCOPE. The estate DOES receive a space selection \u2014 exactly two published reads accept a project filter, and the work ledger both accepts and APPLIES it \u2014 so every surface-level test for 'can this chooser be wired' says yes and wiring it would have manufactured empty answers. Of the ledger rows that name a model, none carries the project_id the filter retains on; the estate's own inference orchestration lane pins its project_ref to a value that resolves to no space record; and the edge is absent from the space side too (no space record carries an invocation field and no space ref names one). Three planes that would have to agree on what a space is, and no shared vocabulary between them \u2014 so the chooser is a typed absence and no second mutation spine is minted over the daemon's invoke route" },
...
const REMEDIATION = {
  scheduler: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#scheduler + ${ADJ}#scheduler-port` + " + reference-live-tenant-atlas.v1.json#scheduler + reference-live-tenant-deep-atlas.v1.json#scheduler" },
  registry: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#registry + ${ADJ}#registry-port` + " + reference-live-tenant-atlas.v1.json#registry + reference-live-tenant-deep-atlas.v1.json#registry" },
  map: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#map + ${ADJ}#map-port` + " + reference-live-tenant-atlas.v1.json#map + reference-live-tenant-deep-atlas.v1.json#map" },
  ingest: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#ingest + ${ADJ}#ingest-port` + " + reference-live-tenant-atlas.v1.json#ingest + reference-live-tenant-deep-atlas.v1.json#ingest" },
  jobs: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#jobs + ${ADJ}#jobs-port` + " + reference-live-tenant-atlas.v1.json#jobs" },
  dataset: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#dataset` + " + reference-live-tenant-atlas.v1.json#dataset" },
  modelstudio: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#modelstudio + ${ADJ}#modelstudio-port` + " + reference-live-tenant-atlas.v1.json#modelstudio + reference-live-tenant-deep-atlas.v1.json#modelstudio" },
  inference: { remediation_state: "live_ia_recorded", adjudication_ref: `${ADJ}#inference + ${ADJ}#inference-port` + " + reference-live-tenant-atlas.v1.json#inference + reference-live-tenant-deep-atlas.v1.json#inference" },
  ...
  repositories: { donor_capture: "code (non-seed capture /workspace/code/)", adjudication_ref: `${ADJ}#repositories` },
...
const matrix = {
  ...
  doctrine: "Reference UX port first (port the source-neutralized reference shell/layout) → daemon truth inside that UX (wire panels, tables, graph nodes, toolbars, drawers, empty + disabled states) → IOI-native redesign later. The dark IOI surfaces built #3–#30 are substrate, not parity." + " REMEDIATION-v2 ADDENDUM (2026-08-19): the port unit is the owner family's IA, not the capture root — in-app routes/tab lanes are enumerated in reference-subroute-census.v1.json; every named gap must cite the reference surface it was weighed against (adjudication_ref); seeds are not 1:1 with apps (D6 donor/combined-seed designations); landing designations live in landing-designations.v1.json (one per click target, checker-audited).",

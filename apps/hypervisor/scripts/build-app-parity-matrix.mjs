...
const REFERENCE_PORTED = {
  ...
  // ladder. BLOCKED from daemon_wired by an ORIGIN/SERVING mismatch, NOT by missing data (#38 re-baseline).
  // The captured data is COMPLETE: from the MATCHING origin (localhost:9225) the canvas renders a clean
  // pipeline graph. But the app's captured fetch URLs are absolute localhost:9225, so the hardened
  // harness (and the /__apps/pipeline proxy) load a DIFFERENT origin → CORS-blocked fetches → the canvas
  // "Failed to initialize." #37 mis-read this as "missing lazy chunks needing a fresh re-harvest"; #38
  // proved (signature stub detection) ZERO chunks are missing. The harness still guards it (the proxy
  // reference is not yet certifiable), so pipeline stays reference_ported until the origin is aligned.
  pipeline: { port_surface: "/__ioi/pipeline", surface_name: "Data", reference_workspace: "/workspace/builder/", parity_blocked: "NOT harness-certifiable yet — but the blocker is a CORS/ORIGIN MISMATCH, NOT missing data (proven by verify-pipeline-reference-data-clean.mjs, #38 re-baseline; NOT prose). The captured data is COMPLETE: the matching-origin canvas (localhost:9225 …/sandbox/…) renders a clean pipeline graph (nodes + toolbar + panel, no error/data-fail/CORS). The app's captured fetch URLs are absolute http://localhost:9225/…, so loading the SAME content from 127.0.0.1:9225 (the harness/proxy origin) makes every eddie/session fetch cross-origin → CORS-blocked → 'Failed to initialize'; the /__apps/pipeline proxy lane errors ('An error occurred'). #37's 'missing lazy chunks (bfed3b6a…/c806694d…) → needs a FRESH-session re-harvest' was WRONG — those are genuinely-small REAL chunks; #38 signature stub detection finds ZERO missing chunks (a <400b size heuristic false-positived them). NO re-harvest and NO fresh Foundry auth are required. daemon_wired path = align the origin (serve the mirror same-origin or add CORS) + point the harness reference at the data-clean canvas, then run the hardened harness. See reharvest-pipeline-builder.mjs (now diagnosis-driven) + the data-clean gate", binding: "ported Pipeline Builder shell over the real ODK authority ladder (DataSource → ... → MaterializedObjectSet) as canvas node cards; live/declared/missing per stage from daemon truth; preview rows + output schema from the real projection + materialized set", note: "shell ported + wired (source-neutral builder shell: rail/header/toolbar/canvas/right/tray, NOT automationsShell; Build+Preview supported, Schedule+Deploy disabled in place); NOT daemon_wired — the reference DATA is complete but the harness path is CORS/origin-blocked (fix is origin alignment, not a re-harvest)" },

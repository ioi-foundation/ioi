// The C7/C8 run-evidence assembler, as a library (M12.9, register R-211, 2026-09-20).
//
// This is the exact logic `assemble-c7-c8-evidence.mjs` ran inline until R-211: given the eleven
// artifacts the live driver writes, the daemon's durable records for the certified environment, the
// source basis and the substrate anchor, it derives the run evidence a C8 v2 certificate is sealed
// from. It reads NO host state itself — no git, no binary, no muxlog, no data dir — so the same
// function assembles from a tracked, redacted record set (the generation gate) exactly as it does
// from a live host (the CLI), and a certificate generated from records is byte-identical to the one
// sealed on the day the records were written. Every `required(...)` throw is the same refusal the
// CLI made: a record set that cannot answer a member cannot generate a certificate from it.
...
export const redactSdl = (value) => value
  .split("\n")
  .map((line) => /^(\s*)(password|token|api[_-]?key|secret|private[_-]?key)\s*:/iu.test(line)
    ...
    : line)
  .join("\n");
export const sdlImageRef = (value) => value.match(/^\s*image\s*:\s*["']?([^\s"']+)["']?\s*$/mu)?.[1] || "unknown-image";
export const serviceFacts = (endpointRecord) => {
...
/**
 * @param {object} input
 * @param {object} input.artifacts   { challenge, proposalAdmission, cast, start, logs, deleted, reconcile|null, whoami,
 *                                     receipts: [], reconciliation: {rows: []}, operations: [] }  — the driver's artifacts
 * @param {object} input.records     { deployments: [], leases: [], endpoints: [], capabilityLeases: [] } — durable families
 * @param {string} input.environment the certified environment ref
 * @param {object} input.source      { commit, dirty_state_declaration, publication_eligible, daemon_binary_sha256 }
 *                                   (publication_eligible is the CANDIDATE posture; the evidence's own is derived below)
 * @param {object} input.substrateAnchor { bytes, prefix_sha256 } — the durable substrate prefix at the time of the run
 */
export function assembleRunEvidence({ artifacts, records, environment, source, substrateAnchor }) {
  const { challenge, proposalAdmission, cast, start, logs, deleted, whoami } = artifacts;
  const reconcileCandidate = artifacts.reconcile ?? null;
  ...
  const allOperations = artifacts.operations || [];
  ...
  const deployment = (records.deployments || []).find((record) => record.environment_ref === environment);
  const providerLeaseRecords = records.leases || [];
  ...
  const endpointRecords = records.endpoints || [];
  ...
  const capabilityLease = (records.capabilityLeases || []).find((record) => record.lease_id === capabilityLeaseId);
  ...
  const status = source.dirty_state_declaration;
  ...
  const publicationEligible = source.publication_eligible === true
    && (status || "clean") === "clean"
    && immutableImage;
  ...
  return {
    ...
    result: "success",
    source: {
      commit: required(source.commit, "source commit"),
      dirty_state_declaration: status || "clean",
      publication_eligible: publicationEligible,
      daemon_binary_sha256: required(source.daemon_binary_sha256, "daemon binary sha256"),
    },
    ...
    durable: {
      ...
      substrate_muxlog_bytes: required(substrateAnchor?.bytes, "substrate anchor bytes"),
      substrate_muxlog_prefix_sha256: required(substrateAnchor?.prefix_sha256, "substrate anchor prefix sha256"),
    },
  };
}

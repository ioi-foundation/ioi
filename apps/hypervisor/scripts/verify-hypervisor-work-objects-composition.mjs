...
async function run() {
  ...
  try {
    ...
    // route for the retired work-object families.
    const retiredPlanes = ["attempt_finding_routes.rs", "verifier_challenge_routes.rs", "resource_capability_offer_routes.rs", "work_frontier_claim_routes.rs"];
    const planesGone = retiredPlanes.every((f) => !existsSync(join(REPO, "crates/node/src/bin/hypervisor_daemon_routes", f)));
    const retiredFamilies = /"\/v1\/goal-orchestration\/(attempts|findings|work-frontier-items|work-claim-leases|resource-offers|capability-offers|verifier-challenges|work-eligibility-matches)(\/|")/u;
    ok("[structure] the work objects are composed with no plane: the module names no room or lease, calls no goal-orchestration route, the router registers no route for a v4 contract and none for the retired work-object families, and the four room-hosted planes that served v3 no longer exist (the records are served only through the seam)", !/\b(room|rooms)\b/iu.test(workSource) && !/participant[_ -]lease/iu.test(workSource) && !/goal-orchestration/u.test(workSource) && !/work-claim\/v4|attempt\/v4|finding\/v4|verifier-challenge\/v4|offer\/v4/u.test(routerSource) && !retiredFamilies.test(routerSource) && planesGone, "");

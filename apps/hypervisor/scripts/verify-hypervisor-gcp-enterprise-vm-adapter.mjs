...
async function run() {
  ...
  ok("delete always — proven-absent teardown owes NO cleanup obligation (Persistent Disk auto-deletes with the instance); exposure closes and the reservation releases",
  ...
  // BRANCH 3 — UNKNOWN. The Compute Engine half returns, but the remote workspace cleanup half
  // is made unreachable (its root is left unwritable, so the remote `rm -rf` cannot prove
  // absence). Absence is unproven: the outcome is torn_down_unverified, never coerced to
  // success, a durable obligation stays open, and deletion still ANSWERS. No GCP credential.
  const envU = `env-gcpU-${tag}`;
  const simU = (await jd("POST", "/v1/hypervisor/cloud-candidates/candidates/refresh", { intent_ref: intent.intent_ref })).j;
  const candU = (simU.candidates || []).find((c) => c.provider_kind === "gcp" && c.machine_type === "g2-standard-8") || {};
  await opWithGrant("create", { environment_ref: envU, candidate_ref: candU.candidate_ref, max_hourly_usd: 0.9 });
  ...
  // BRANCH 2 — FAILED. The provider-native delete explicitly reported it did not destroy.
  const failed = teardownFindings(del2, "teardown_failed");
  ok("failed delete → teardown_failed with a durable obligation, never reported verified, and exposure closed_with_warning naming the still-accruing vCPU/PD risk",

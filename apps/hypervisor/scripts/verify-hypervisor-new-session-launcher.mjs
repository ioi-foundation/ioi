...
async function run() {
  ...
  ok("result offers the Developer Workspace handoff on the compatibility route", await page.locator('#ioi-ns-result a[href="/__ioi/workbench"]').count() >= 1);
  ...
  // 4. The binding surfaces on the estate: daemon sessions LIST projection + Developer Workspace
  //    (harness selection lives in the workspace/session details as daemon truth, not UI state).
  ...
  ok("Developer Workspace sessions panel shows the binding as daemon truth", /<h1>Developer Workspace/.test(wbHtml) && /id="sessions"/.test(wbHtml) && /hypervisor_worker/.test(wbHtml) && /model-route:/.test(wbHtml));
  ok("Developer Workspace names the no-binding posture plainly", /execute-time default/.test(wbHtml));

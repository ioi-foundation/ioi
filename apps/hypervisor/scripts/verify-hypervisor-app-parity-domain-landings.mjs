...
// another plane; and (f) the transcript plane must be LINKED, not re-rendered.
{
  ...
  // Re-aimed 2026-08-20: this population was fed solely by the session-registry legacy-slot 503
  // (session_registry_unavailable). The 873-record re-slot repaired that plane, so at a healthy
  // posture ZERO jobs planes refuse — a population this assertion may honestly find empty. When
  // empty, the refusal DOCTRINE line must still ship on the page (deleting the branch's framing
  // still fails); when any plane refuses, the original verbatim-code/never-a-count bar applies.
  const bRefused = bDerived.filter((d) => d.state === "refused");
  // With zero refused planes the page correctly renders NO refusal prose (the doctrine line lives
  // inside the refusal branch); the teeth move to the SOURCE — the handler must still carry the
  // sessions-refusal branch verbatim, so deleting the branch fails even at a healthy posture.
  const bServeSrc = readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), "serve-product-ui.mjs"), "utf8");
  ok("jobs: a REFUSAL is rendered as a REFUSAL — the daemon's typed code verbatim, and NEVER as a count (empty population ⇒ the refusal branch is asserted at the source)",
    bRefused.length > 0
      ? (bRefused.every((d) => { const row = planeRow(d.path); return row.includes(d.code) && /REFUSAL, never a zero/.test(row) && !/\d+\s+records?/.test(row); }) && /A refusal is not a zero/.test(t))
      : (bServeSrc.includes("REFUSED this read with the daemon's own typed code") && bServeSrc.includes("A refusal is not a zero")),
    `${bRefused.length} refused`);

...
// 6. Ranked next-3: present, justified on the directive's axes, and NOT promotions.
{
  ...
  ok("ranked next present: every qualifying (data_clean, unported) seed is ranked, capped at 3, ranks sequential — an EMPTY pool is the honest terminal state (#48 consumed the last candidate; the program pivots to origin-alignment seeds)", r.length === Math.min(3, qualifying) && r.map((x) => x.rank).join(",") === Array.from({ length: r.length }, (_, i) => i + 1).join(","), `${r.length} ranked of ${qualifying} qualifying`);

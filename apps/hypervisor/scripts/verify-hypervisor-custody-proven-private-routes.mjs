...
async function run() {
  ...
  ok("[claim] a contract bound to ANOTHER route is refused contract_route_mismatch and the claim is refused: evidence is resolved to the route it names and to no other", contractOnLocal.status === 201 && cM.status === "refused" && (cM.refusals || []).some((r) => r.code === "contract_route_mismatch"), `${contractOnLocal.status} ${cM.status} ${JSON.stringify((cM.refusals || []).map((r) => r.code))}`);

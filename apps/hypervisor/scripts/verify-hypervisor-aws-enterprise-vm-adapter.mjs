...
async function run() {
  ...
  ok("terminate always — proven-absent teardown owes NO cleanup obligation, exposure closes, reservation releases",
  ...
  // BRANCH 3 — UNKNOWN. The EC2 half returns, but the remote workspace cleanup half is made
  // unreachable (its root is left unwritable, so the remote `rm -rf` cannot prove absence).
  // Absence is therefore unproven: the outcome is torn_down_unverified, never coerced to
  // success, and a durable obligation stays open. Deletion still ANSWERS — containment never
  // strands an operator with a resource they cannot delete. No AWS credential is involved.
  const envU = `env-awsU-${tag}`;
  const simU = (await jd("POST", "/v1/hypervisor/cloud-candidates/candidates/refresh", { intent_ref: intent.intent_ref })).j;
  const candU = (simU.candidates || []).find((c) => c.provider_kind === "aws" && c.instance_type === "g5.xlarge") || {};
  await opWithGrant("create", { environment_ref: envU, candidate_ref: candU.candidate_ref, max_hourly_usd: 1.1 });
  ...
  ok("failed terminate → teardown_failed with a durable obligation, never reported verified, and exposure closed_with_warning naming the still-accruing EC2/EBS risk",

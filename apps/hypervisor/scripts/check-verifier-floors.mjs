...
// (M08.11, 2026-09-20, `scripts/check-spend-approval-lane.mjs`). Non-verifier
...
for (const check of ciChecks) {
  ...
  const m = cmd.match(/(scripts\/(?:verify-[A-Za-z0-9._-]+|check-landing-designations|check-standalone-conformance|check-zero-to-operable|check-undeniable-product-proof|check-horizon-1b-improvement|check-horizon-2-distributed-work|check-provider-neutral-live-transaction|check-c8-bounded-live-effect-certificate|check-approval-card-facets|check-spend-approval-lane)\.mjs)/);

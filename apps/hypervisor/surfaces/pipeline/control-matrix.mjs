...
export const CONTROL_MATRIX = [
  ...
  { id: "cmd.build", region: "commands", label: "Build", reference: "runs the pipeline build", outcome: "disabled_reason", reason: "no single-call build authority exists — materialization is the governed ODK ladder: MaterializingRun (citing a CapabilityLeasePlan) → wallet-approved lease → sealed ConnectorSession → execute; run the ladder from the Ontology Manager", binding: "command:build" },

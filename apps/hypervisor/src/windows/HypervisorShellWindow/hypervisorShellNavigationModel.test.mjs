...
test("visible shell chrome uses Hypervisor labels over compatibility route keys", () => {
  ...
  assert.match(activityBar, /homeNavItem/);
  assert.match(activityBar, /sessionsNavItem/);
  assert.match(activityBar, /Search\.\.\./);
  assert.match(activityBar, /What's New/);
  assert.match(activityBar, /label="Applications"/);
  assert.match(activityBar, /Your favorite apps will appear here/);
  assert.match(activityBar, /IOI Assist/);
  assert.doesNotMatch(activityBar, /chat-activity-button--new-session/);
  assert.doesNotMatch(activityBar, /chat-activity-project-label/);
  ...
  assert.doesNotMatch(activityBar, /Organization settings/);
  assert.match(activityBar, /HYPERVISOR_PRIMARY_ACTION/);
  assert.match(activityBar, /aria-label="Favorite applications"/);
...
test("Agents surface renders workers as a cockpit list without internal doctrine copy", () => {
  ...
  assert.match(shellContent, /className="hypervisor-agents__rail-card"/);
  assert.match(shellContent, />What's new\?</);
  assert.match(shellContent, /className="hypervisor-agents__detail-label"/);
  assert.match(shellContent, />Selected agent</);
  assert.match(shellContent, /className="hypervisor-agents__rail-summary"/);

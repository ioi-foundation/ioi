...
test("Sessions surface renders session tabs and operations inspectors from daemon projections", () => {
  ...
  assert.match(shellContent, /data-launched-session-admission-ref/);
  assert.match(shellContent, /data-launched-session-admission-decision/);
  assert.match(shellContent, /data-launched-session-open-disabled/);
  assert.match(shellContent, /canOpenLaunchedSessionSurface/);
  assert.match(shellContent, /session\.admission_state === "daemon_admitted"/);
  assert.match(shellContent, /launchedSessionAdmissionLabel/);
  assert.match(shellContent, /launchedSessionAdmissionDetail/);
  assert.match(shellContent, /Adapter admission/);
  assert.match(shellContent, /is-admission-blocked/);
  assert.match(shellContent, /disabled=\{!canOpenSurface\}/);
  assert.match(shellContent, /data-launched-session-open-surface/);
  assert.match(shellContent, /if \(canOpenSurface\)/);

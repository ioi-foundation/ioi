type ChatVimTextObject =
  ...
  | "around_brace"
  | "inner_brace";
type ChatVimTextObjectPrefix = "around" | "inner";
export type PendingVimMotionPrefix = "g" | null;
type ChatVimMotionPrefix = Exclude<PendingVimMotionPrefix, null>;

export interface PendingVimOperatorState {
  kind: "change" | "delete";
  textObjectPrefix: ChatVimTextObjectPrefix | null;
}
...
export type ChatVimRepeatableCommand =
  | {
      count?: number;
      kind: "delete_char";
    }
  ...
  | {
      ...
      change: boolean;
      kind: "motion_edit";
      motion: ChatVimMotion;
    }
  | {
      count?: number;
      change: boolean;
      kind: "text_object_edit";
      textObject: ChatVimTextObject;
    };

export interface ChatVimKeyInput {
  altKey: boolean;
  code: string;
  ctrlKey: boolean;
  key: string;
...
export interface ChatVimNormalState {
  lastCommand: ChatVimRepeatableCommand | null;
  pendingMotionPrefix: ChatVimMotionPrefix | null;
...
export interface ChatVimNormalResult {
...
function textObjectFromKeyInput(
  prefix: ChatVimTextObjectPrefix,
  keyInput: ChatVimKeyInput,
): ChatVimTextObject | null {
...
function handledResult(
  state: ChatVimNormalState,
  patch: Partial<ChatVimNormalResult>,
): ChatVimNormalResult {
...
function unhandledResult(state: ChatVimNormalState): ChatVimNormalResult {
  return {
    handled: false,
    value: state.value,
...
function applyRepeatableCommandOnce(
  state: ChatVimNormalState,
  command: ChatVimRepeatableCommand,
): ChatVimNormalResult {
...
export function applyChatVimNormalKey(
  state: ChatVimNormalState,
  keyInput: ChatVimKeyInput,
): ChatVimNormalResult {

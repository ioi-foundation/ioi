...
// typography
export const Heading = ({ level = 1, children, id }: { level?: 1 | 2; children: ReactNode; id?: string }) =>
  level === 1 ? <h1 className="hv-h1" id={id}>{children}</h1> : <h2 className="hv-h2" id={id}>{children}</h2>;
...
export function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  const titleId = `hv-modal-title-${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  ...
  useEffect(() => {
    ...
    const focusFrame = window.requestAnimationFrame(() => (dialog?.querySelector<HTMLElement>('[data-modal-initial-focus]') ?? dialog?.querySelector<HTMLElement>('button, a[href], input, [tabindex]:not([tabindex="-1"])'))?.focus());
    ...
    return () => { window.cancelAnimationFrame(focusFrame); document.removeEventListener("keydown", onKeyDown); restoreFocusRef.current?.focus(); };
  }, [onClose]);
  return (
    <div className="hv-modal__scrim" onClick={onClose}>
      <div ref={dialogRef} className="hv-modal" role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} onClick={(e) => e.stopPropagation()}>

// Parity Phase B — reference shell (left sidebar) ported from the IOI demo
// reference's harvested sidebar DOM, using the vendored reference utility classes.
// HypervisorReferenceSidebar is a prop-compatible drop-in for the old activity
// rail (accepts activeView/onViewChange/onOpenNewSession), so it can replace the
// rail inside the live shell while the existing controller keeps routing + content.
...
function NavItem({ icon, label, active, onClick }: { icon: ReactNode; label: string; active?: boolean; onClick?: () => void }) {
  return (
    <a
      onClick={onClick}
      className={`flex flex-row items-center rounded-lg h-8 min-w-0 cursor-pointer ${active ? "bg-surface-hover" : "hover:bg-surface-hover"}`}
    >
...
export function HypervisorReferenceSidebar({ activeView = "home", onViewChange, onOpenNewSession }: ReferenceSidebarProps) {
  const go = (view: PrimaryView) => () => onViewChange?.(view);
  return (
    <div className="flex w-[15.5rem] shrink-0 flex-col pb-[6px] border-r border-border-base bg-surface-primary">
      ...
      <div className="flex items-center justify-between px-2 pb-0 pt-1">
        <a aria-label="Go to home" onClick={go("home")} className="flex h-8 cursor-pointer items-center px-1.5">
      ...
      <div className="relative overflow-y-auto overflow-x-hidden mr-0.5 flex-grow pr-0.5">
        ...
        <div className="flex flex-col gap-1 pl-2 pt-2 pr-2">
          <button type="button" aria-label="New Session" onClick={onOpenNewSession} className="flex h-8 items-center gap-2 rounded-lg border border-border-base px-2 text-base text-content-primary hover:bg-surface-hover">
          ...
          <NavItem icon={<HomeIcon />} label="Home" active={activeView === "home"} onClick={go("home")} />
          <NavItem icon={<GridIcon />} label="Projects" active={activeView === "projects"} onClick={go("projects")} />
          <NavItem icon={<FlowIcon />} label="Automations" active={activeView === "automations"} onClick={go("automations")} />
          <NavItem icon={<GridIcon />} label="Applications" active={activeView === "applications"} onClick={go("applications")} />
        </div>
        ...
        <div className="flex flex-col gap-0.5 pl-2 pt-3 pr-2">
          ...
          <a onClick={go("workbench")} className="flex flex-row items-center rounded-lg h-8 min-w-0 cursor-pointer hover:bg-surface-hover">
            <div className="relative flex w-full flex-row items-center">
              <div className="relative h-8 w-8 shrink-0">
      ...
      <div className="flex flex-col w-full gap-2 p-2 pb-2 pt-4">
        <NavItem icon={<GearIcon />} label="Organization settings" active={activeView === "settings"} onClick={go("settings")} />

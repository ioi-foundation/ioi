...
function Tab({ label, active, icon }: { label: string; active?: boolean; icon?: React.ReactNode }) {
  return (
    <button type="button" role="tab" aria-selected={active ? "true" : "false"} data-state={active ? "active" : "inactive"} className={TAB} tabIndex={-1}>
...
function FileRow({ f }: { f: (typeof FILES)[number] }) {
  const pad = f.depth === 0 ? 8 : 28;
  const inner = (
    <div className="flex items-center" style={{ height: "30px", paddingLeft: `${pad}px` }}>
      <button type="button" className={`flex h-7 w-full items-center gap-1.5 rounded-md px-2 text-left font-mono text-sm cursor-pointer hover:bg-surface-02 ${f.folder ? "text-content-secondary" : ""}`} tabIndex={-1} role="treeitem" {...(f.folder ? { "aria-expanded": "true", "data-tree-folder": "true" } : { "aria-selected": "false" })} data-tree-path={f.name} data-tree-depth={f.depth}>
      ...
      <div className="size-full max-w-full flex min-h-0 flex-col p-0">
        <div className="relative flex min-h-0 flex-1">
          <div className="absolute right-4 top-2 z-20">
            <button type="button" className="select-none inline-flex items-center justify-center text-content-primary hover:bg-surface-button-clear-accent gap-2 h-8 text-base rounded-[4px] border-0 bg-surface-01 p-2" aria-label="Hide right panel" aria-pressed="true">
          ...
          <div className="flex h-full w-full min-h-0 flex-1" data-panel-group data-panel-group-direction="horizontal" style={{ flexDirection: "row", height: "100%", overflow: "hidden", width: "100%" }}>
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex h-12 w-full items-center justify-between gap-2 border-b border-border-subtle px-4 py-2">
                    <div className="flex h-6 min-w-0 flex-1 items-center gap-0.5">
                      <button type="button" aria-haspopup="menu" data-testid="environment-header-dropdown-trigger" className="group flex min-w-0 cursor-pointer items-center focus:outline-none" aria-label="Environment actions">
                    ...
                    <div className="px-4 pb-4">
                      <div className="flex flex-col gap-0.5 rounded-xl bg-surface-muted p-1.5">
                        <div className="flex flex-col overflow-clip rounded-lg border border-border-base bg-surface-secondary">
                          <div className="p-2">
                            <textarea placeholder="Describe your task or type / for commands" rows={2} className="placeholder:text-content-muted text-content-primary text-base h-auto resize-none overflow-y-auto leading-[18px] w-full rounded-none border-0 bg-transparent outline-none focus-within:ring-0 max-w-full p-2" />
                            <div className="flex h-full min-w-0 flex-row items-center justify-between gap-2 p-2 pt-0">
                              ...
                              <div className="ml-auto flex flex-row items-center gap-2">
                                <button type="button" aria-label="Change agent mode" className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border-base px-2 text-sm font-normal text-content-primary hover:opacity-80"><span className="truncate">5.5 Medium</span><ChevronDownSmall /></button>
                                <button type="button" aria-label="Submit" className="select-none inline-flex items-center justify-center rounded-lg bg-surface-button-primary text-content-always-white size-8 shrink-0"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m5 12 7-7 7 7" /><path d="M12 19V5" /></svg></button>
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <div role="separator" className="relative flex w-0 border-l border-border-base" />

            {/* right panel (30) */}
            <div id="environment-details" data-panel data-panel-size="30.0" style={{ flex: "30 1 0px", overflow: "hidden" }}>
                        ...
                        <div className="flex items-center gap-2 px-4 py-2">
                          <div className="relative min-w-0 flex-1">
                            <div className="flex items-center gap-2 h-9 w-full max-w-[600px] px-3 py-2 rounded-lg border border-border-light text-base bg-surface-input">
                              ...
                              <input className="flex h-full w-full text-base p-0 border-0 outline-none placeholder:text-content-muted text-content-primary bg-transparent max-w-none" type="text" placeholder="Search files..." defaultValue="" />
                            </div>
                          </div>
                          <div className="relative w-fit">
                            <button type="button" aria-label="Select" aria-haspopup="listbox" data-testid="changes-scope-select" className="flex w-full items-center justify-between gap-2 text-base text-content-primary outline-none h-9 px-3 rounded-lg border border-border-input-default bg-surface-input"><span className="truncate">Uncommitted</span><SelectChevron /></button>
                          </div>
                        </div>

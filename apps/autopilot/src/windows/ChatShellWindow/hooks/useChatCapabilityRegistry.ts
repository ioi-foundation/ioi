...
export type ChatCapabilityRegistryStatus =
  | "idle"
  | "loading"
  | "ready"
  | "error";
...
export function useChatCapabilityRegistry(enabled = true) {
  const [snapshot, setSnapshot] = useState<CapabilityRegistrySnapshot | null>(
    () => cachedCapabilityRegistrySnapshot,
  );
  const [status, setStatus] = useState<ChatCapabilityRegistryStatus>(() =>

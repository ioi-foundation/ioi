...
type ChatStagedOperationsState = {
...
const INITIAL_STATE: ChatStagedOperationsState = {
  operations: [],
  loading: false,
...
export function useChatStagedOperations() {
  const [state, setState] = useState<ChatStagedOperationsState>(INITIAL_STATE);
  ...
  const removeOperation = useCallback(
    async (operationId: string) =>
      runAction(
        operationId,
        "Staged operation removed from Chat.",

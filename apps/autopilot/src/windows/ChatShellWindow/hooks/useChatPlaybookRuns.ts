...
const INITIAL_STATE: ChatPlaybookRunsState = {
  runs: [],
  loading: false,
  busyRunId: null,
  message: null,
...
export function useChatPlaybookRuns(sessionId: string | null) {
  const [state, setState] = useState<ChatPlaybookRunsState>(INITIAL_STATE);
  ...
  const dismissPlaybookRun = useCallback(
    async (runId: string) =>
      runAction(
        runId,
        "Parent playbook run dismissed from Chat.",

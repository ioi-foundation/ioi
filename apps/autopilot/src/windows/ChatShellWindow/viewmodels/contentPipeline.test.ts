...
function artifactRuntimeActivityGroupPresentationTest(): void {
  const chatSession: ChatArtifactSession = {
    ...
    activeOperatorRun: {
      ...
      status: "active",
      startedAtMs: 1000,
      engineSummary: "html_iframe via direct_author",
      sourcePack: {
        summary: "",
        items: [],
      },
      steps: [
        ...
        {
          stepId: "run-1:author_artifact:1",
          originPromptEventId: "prompt-1",
          phase: "author_artifact",
  ...
  assert.deepEqual(
    group?.rows.map((row) => row.label),
    ["Understand request", "Write index.html"],
  );
      ...
      finalArtifacts: [
        {
          ...
          role: "primary",
          mime: "text/html",
          summary: "Renderable index.html file.",
        },
      ],
      verificationOutcome: {
        status: "complete",
...
function artifactRuntimeActivityOrderingAndSourcePlacementTest(): void {
  const chatSession: ChatArtifactSession = {
    ...
    navigatorBackingMode: "logical",
    navigatorNodes: [],
    attachedArtifactIds: ["artifact-ordered"],
    availableLenses: ["render", "source"],
    materialization: {
      version: 1,
      requestKind: "artifact",
      ...
      steps: [
        {
          ...
          status: "active",
          label: "Write index.html",
          detail: "Writing the initial draft.",
          startedAtMs: 1500,
          preview: {
            originPromptEventId: "prompt-ordered",
            label: "Direct author output",
          ...
          sourceRefs: [
            {
              ...
              originPromptEventId: "prompt-ordered",
              title: "NASA Science",
              url: "https://science.nasa.gov/quantum",
              domain: "science.nasa.gov",
              excerpt: "Quantum explainer from NASA.",
              retrievedAtMs: null,
              freshness: null,
              reason: "Retrieved from Bing web search for \"quantum computing\".",
            },
          ],
          verificationRefs: [],
          attempt: 1,
        },
        {
          ...
          engine: "source_research",
          status: "complete",
          label: "Search for context",
          detail: "Retrieved from Bing web search for \"quantum computing\".",
          startedAtMs: 2000,
          fileRefs: [],
          sourceRefs: [
            {
        ...
        },
        {
          ...
          status: "complete",
          label: "Read sources",
          detail: "Opened retrieved sources.",
          startedAtMs: 2500,
          fileRefs: [],
          sourceRefs: [
            {
              ...
              url: "https://www.anl.gov/article",
              domain: "www.anl.gov",
              excerpt: "Science 101 quantum mechanics.",
              retrievedAtMs: null,
              freshness: null,
              reason: "Retrieved from Bing web search for \"quantum computing\".",
            },
  ...
  assert.deepEqual(
    group?.rows.map((row) => row.label),
    [
      'Searched "quantum computing"',
      "Read anl.gov",
      "Write index.html",
    ],
  );
  assert.equal(group?.rows[0]?.sourceSummary, null);
  assert.equal(group?.rows[1]?.sourceSummary, null);
  assert.ok(group?.rows[2]?.sourceSummary);
}

function artifactReplyTextIsConversationalForCompletedArtifactsTest(): void {
  const turnContext = {
    turnId: "turn-1",
    planSummary: null,
    ...
    artifacts: [
      {
        ...
        chatSession: {
          ...
          originPromptEventId: "prompt-ordered",
          title: "Create an HTML file that explains quantum computers",
          summary: "Create an HTML file that explains quantum computers",
          currentLens: "render",
          navigatorBackingMode: "logical",
          navigatorNodes: [],
          attachedArtifactIds: ["artifact-ordered"],
          ...
          materialization: {
            version: 1,
            requestKind: "artifact",
            normalizedIntent: "Create an HTML file that explains quantum computers",
            summary: "Create an HTML file that explains quantum computers",
            selectedSkills: [],
            retrievedExemplars: [],
            candidateSummaries: [],
    ...
    sourceSummary: {
      totalSources: 3,
      sourceUrls: [
        "https://science.nasa.gov/quantum",
        "https://www.anl.gov/article",
        "https://scienceexchange.caltech.edu/article",
      ],
      domains: [
        { domain: "science.nasa.gov", count: 1, faviconUrl: null },
        { domain: "www.anl.gov", count: 1, faviconUrl: null },
        { domain: "scienceexchange.caltech.edu", count: 1, faviconUrl: null },
      ],
      searches: [],
      browses: [
        {
          url: "https://science.nasa.gov/quantum",
          domain: "science.nasa.gov",
          title: "What is Quantum Science? Quantum Leaps - NASA Science",
          stepIndex: 1,
        },
        {
          url: "https://www.anl.gov/article",
        ...
        {
          url: "https://scienceexchange.caltech.edu/article",
          domain: "scienceexchange.caltech.edu",
          title: "What Is Quantum Physics? - Caltech Science Exchange",
          stepIndex: 3,
        },
      ],
    },
  ...
  assert.ok(reply);
  assert.equal(
    reply?.includes("final acceptance validation"),
    false,
  );
  assert.equal(
  ...
  const artifactCompletion: AgentEvent = {
    ...
    digest: {
      ...
      artifact_quality: {
        ...
        notes: "Completion receipt still preserves prep context for Chat.",
      },
      status: "completed",
      success: true,
    },

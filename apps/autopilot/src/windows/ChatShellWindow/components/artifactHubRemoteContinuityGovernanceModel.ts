...
import type {
  ChatRemoteContinuityLaunchMode,
...
function buildQueueLaunchRequest(input: {
  ...
  queueLabel: string;
  notice: string;
}): ChatRemoteContinuityLaunchRequest | null {
...
function buildQueueAction(input: {
  ...
  const detail =
    input.mode === "attach"
      ? sessionCount > 1
        ? "Queue the attachable retained remote sessions into the shared Chat REPL so the operator can step through workspace-backed continuity targets without losing server merge posture."
        : "Queue the newest attachable retained remote session into the shared Chat REPL."
      : sessionCount > 1
        ? "Queue the history-only retained remote sessions into the shared Chat REPL so review can move through them without attempting PTY attach."
        : "Queue the newest history-only retained remote session into the shared Chat REPL for retained-session review.";

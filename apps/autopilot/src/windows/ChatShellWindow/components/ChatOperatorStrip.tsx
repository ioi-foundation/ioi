...
import { operatorFacingCurrentStep } from "../../ChatShellWindow/viewmodels/runtimeStatusCopy";
...
export function ChatOperatorStrip({
  task,
  canOpenRewind,
  planSummary,
  artifactCount,
            ...
            {validationSummary.items.map((item) => (
              ...
              >
                <div className="spot-operator-strip__validation-card-head">
                  ...
                  <span
                    className={`spot-operator-strip__validation-status is-${toneForRuntimeStatus(
                      item.status,
                    )}`}
                  >
                    {formatRuntimeStatusLabel(item.status)}
                  </span>
                </div>
                <p>{item.detail}</p>
              </div>

...
export function formatChatExecutionPreviewPhase(
  ...
  const status = formatRuntimeStatusLabel(preview?.status) || "Streaming";
...
function mapExecutionPreview(preview: {
  ...
  return {
    ...
    status: formatRuntimeStatusLabel(preview.status) || "Streaming",
  ...
  return (
    ...
    mapExecutionPreview(
      latestWorkerPreview
        ? {
            label:
              `${formatRuntimeStatusLabel(latestWorkerPreview.role)} output` ||

...
function continuityQueueSessionIdsForRequest(
  request: ChatRemoteContinuityLaunchRequest | null,
): string[] {
  if (!request) {
    return [];
  }
...
export function ChatReplView({
  ...
  const handleSessionSelection = (
    sessionId: string,
    mode: ChatRemoteContinuityLaunchMode | null = null,
  ) => {
    ...
    <div className="artifact-hub-permissions">
      <section className="artifact-hub-files-identity artifact-hub-permissions__identity">
        ...
        <p>
          ...
          canonical session workspace without leaving Chat.
        </p>

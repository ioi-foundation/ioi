...
} from "../../ChatShellWindow/hooks/useChatPlaybookRuns";
...
function latestSuccessfulReceipt(step: ChatPlaybookStepRecord) {
  ...
  return new Map(
    run.steps.map((step) => {
      const unmetDependencyLabels = step.dependsOnStepIds
        ...
        .filter(
          (dependency): dependency is ChatPlaybookStepRecord =>
            Boolean(dependency),
        )

...
function formatRunDuration(run: ChatPlaybookRunRecord): string {
...
function stepArtifactCount(run: ChatPlaybookRunRecord): number {
...
function latestStepReceipt(
  run: ChatPlaybookRunRecord["steps"][number],
): LocalEngineParentPlaybookReceiptRecord | null {
...
function summarizeStepStates(run: ChatPlaybookRunRecord): string {
...
function summarizeWorkerGraph(run: ChatPlaybookRunRecord): string {
...
function summarizePromotionReadiness(run: ChatPlaybookRunRecord): string {
...
function buildActivePathStepIds(run: ChatPlaybookRunRecord): Set<string> {
...
function buildGraphColumns(run: ChatPlaybookRunRecord): Array<{
  depth: number;
  steps: ChatPlaybookRunRecord["steps"];
}> {
  const stepById = new Map(run.steps.map((step) => [step.stepId, step]));
  ...
  const columns = new Map<number, ChatPlaybookRunRecord["steps"]>();

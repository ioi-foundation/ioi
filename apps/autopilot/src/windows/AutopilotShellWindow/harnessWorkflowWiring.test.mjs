...
assert.match(
  harnessWorkflow,
  /(?=[\s\S]*applyWorkflowHarnessActivationCandidate)(?=[\s\S]*candidate_not_mintable)(?=[\s\S]*activation_id_missing)(?=[\s\S]*activationState: "validated")(?=[\s\S]*workerHarnessBinding: workerBinding)(?=[\s\S]*rollbackTarget)(?=[\s\S]*componentVersionSet: candidate\.componentVersionSet)/,
  "Minting a fork activation should only apply mintable candidates and update activation id, worker binding, rollback target, and component version set together.",
);

assert.match(
  workflowComposer,
  /(?=[\s\S]*workflow-open-default-harness)(?=[\s\S]*handleOpenDefaultHarness)(?=[\s\S]*workflow-fork-harness-button)(?=[\s\S]*handleForkDefaultHarness)(?=[\s\S]*handleRunHarnessActivationDryRun)(?=[\s\S]*handleApplyHarnessActivationCandidate)(?=[\s\S]*applyWorkflowHarnessActivationCandidate)(?=[\s\S]*createWorkflowHarnessActivationCandidate)(?=[\s\S]*harnessActivationCandidate)(?=[\s\S]*selectedHarnessRollbackTarget)(?=[\s\S]*workflow-readonly-badge)(?=[\s\S]*workflow-harness-worker-binding)/,
  "Workflow composer should expose read-only harness inspection, a fork path, non-mutating activation dry runs, rollback target selection, and guarded activation minting.",
);

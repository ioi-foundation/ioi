...
assert.match(
  ...
  "Harness contracts should declare mode, readiness, replay, collapsible group views, promotion clusters, activation candidates, live handoff, selector routing, canary execution boundaries, schemas, errors, and worker harness identity.",
);
...
assert.match(
  workflowComposer,
  /(?=[\s\S]*workflow-open-default-harness)(?=[\s\S]*handleOpenDefaultHarness)(?=[\s\S]*workflow-fork-harness-button)(?=[\s\S]*handleForkDefaultHarness)(?=[\s\S]*handleRunHarnessActivationDryRun)(?=[\s\S]*createWorkflowHarnessActivationCandidate)(?=[\s\S]*harnessActivationCandidate)(?=[\s\S]*workflow-readonly-badge)(?=[\s\S]*workflow-harness-worker-binding)/,
  "Workflow composer should expose read-only harness inspection, a fork path, and non-mutating activation dry runs.",
);

...
function formatTaskTimestamp(value: number | null | undefined): string {
...
function latestTaskOutputs(task: AgentTask, maxEntries: number = 5) {
  return task.history
    .filter(
      (message) =>
...
function TasksView({
  ...
  const recommendedClarificationOptionId =
    effectiveClarificationRequest?.options.find((option) => option.recommended)
      ?.id ||
    effectiveClarificationRequest?.options[0]?.id ||
    null;
  ...
  return (
    <div className="artifact-hub-tasks">
      ...
      <section className="artifact-hub-task-summary-grid">
        <article className="artifact-hub-task-card">
          <div className="artifact-hub-task-card-head">Phase</div>
          <strong>{humanizeStatus(currentTask.phase)}</strong>
          <span>{currentTask.progress}% complete</span>
        </article>
        <article className="artifact-hub-task-card">
          <div className="artifact-hub-task-card-head">Session</div>
          <strong>{sessionId.slice(0, 12)}</strong>
          <span>{sessionSummary?.resume_hint || "Primary task thread"}</span>
        </article>
        <article className="artifact-hub-task-card">
          <div className="artifact-hub-task-card-head">Checklist</div>
          <strong>
            {completedChecklistCount}/{currentTask.session_checklist.length}
          </strong>
          <span>Runtime checklist items completed</span>
        </article>
        <article className="artifact-hub-task-card">
          <div className="artifact-hub-task-card-head">Background</div>
          <strong>{currentTask.background_tasks.length}</strong>
          <span>Tracked runtime tasks retained</span>
        </article>
        <article className="artifact-hub-task-card">
          <div className="artifact-hub-task-card-head">Delegation</div>
          <strong>{delegationOverview.runCount}</strong>
          <span>
            {delegationOverview.stepCount > 0
              ? `${delegationOverview.stepCount} retained graph steps`
              : "No delegated runs retained"}
          </span>
        </article>
      </section>

      {blocker && (
        <section className="artifact-hub-task-section artifact-hub-task-blocker">
          <div className="artifact-hub-task-section-head">
            <span>Blocker</span>
            <span>{humanizeStatus(currentTask.phase)}</span>
          </div>
          <div className="artifact-hub-task-card">
            <strong>{blocker.title}</strong>
            <p>{blocker.detail}</p>
          </div>
          {(recommendedClarificationOptionId || isGated) && (
            <div className="artifact-hub-task-blocker-actions">
              {recommendedClarificationOptionId && onSubmitClarification && (
                <button
                  className="artifact-hub-open-btn"
                  onClick={() =>
          ...
          {effectiveIsGated && effectiveGateInfo && onApprove && onDeny && (
            <SpotlightApprovalCard
              title={effectiveGateInfo.title}
              description={effectiveGateInfo.description}
              risk={effectiveGateInfo.risk}
              approveLabel={
                effectiveGateInfo.approve_label ||
                (isPiiGate ? "Approve Transform" : "Approve action")
              }
              denyLabel={effectiveGateInfo.deny_label || "Deny action"}
              deadlineMs={gateDeadlineMs}
              surfaceLabel={effectiveGateInfo.surface_label}
              scopeLabel={effectiveGateInfo.scope_label}
              operationLabel={effectiveGateInfo.operation_label}
              targetLabel={
                effectiveGateInfo.target_label ||
                effectiveGateInfo.pii?.target_label
              }
              operatorNote={effectiveGateInfo.operator_note}
              spanSummary={effectiveGateInfo.pii?.span_summary}
              classCounts={effectiveGateInfo.pii?.class_counts}
              severityCounts={effectiveGateInfo.pii?.severity_counts}
              stage2Prompt={effectiveGateInfo.pii?.stage2_prompt}
              targetId={
                (effectiveGateInfo.pii?.target_id as Record<string, unknown> | null) ??
                null
              }
              errorMessage={gateActionError}
              onApproveTransform={onApprove}
              onGrantScopedException={
                isPiiGate ? onGrantScopedException : undefined
              }
              onDeny={onDeny}
            />
          )}
          {effectiveCredentialRequest &&
            ...
            onCancelRuntimePassword && (
              <SpotlightPasswordCard
                title="Credential Required"
                badgeLabel={
                  effectiveCredentialRequest.one_time ? "ONE-TIME" : "SESSION"
                }
                prompt={
                  effectiveCredentialRequest.prompt ||
                  "A credential is required before this task can continue."
                }
      ...
      <section className="artifact-hub-task-section">
        <div className="artifact-hub-task-section-head">
          ...
          <span>{currentTask.session_checklist.length}</span>
        </div>
        <div className="artifact-hub-generic-list">
          {currentTask.session_checklist.length > 0 ? (
            ...
            <p className="artifact-hub-empty">
              No checklist items have been retained for this task yet.
            </p>
          )}
        </div>
      </section>
      ...
      <section className="artifact-hub-task-section">
        <div className="artifact-hub-task-section-head">
          ...
          <span>{currentTask.background_tasks.length}</span>
        </div>
        <div className="artifact-hub-generic-list">
          {currentTask.background_tasks.length > 0 ? (
            currentTask.background_tasks.map((entry) => (
              <article
                className="artifact-hub-generic-row"
                key={entry.task_id}
              >
                <div className="artifact-hub-generic-meta">
                  <span>{humanizeStatus(entry.status)}</span>
                  <span>{formatTaskTimestamp(entry.updated_at_ms)}</span>
                  {entry.session_id && <span>{entry.session_id.slice(0, 12)}</span>}
                </div>
                <div className="artifact-hub-generic-title">{entry.label}</div>
                {(entry.detail || entry.latest_output) && (
                  <p className="artifact-hub-generic-summary">
                    {entry.detail || entry.latest_output}
                  </p>
                )}
                <div className="artifact-hub-generic-actions">
                  {entry.session_id && onLoadSession && (
                    <button
                      className="artifact-hub-open-btn secondary"
                      onClick={() => onLoadSession(entry.session_id!)}
                      type="button"
                    >
                      Open session
                    </button>
                  )}
                  {entry.can_stop && onStopSession && (
                    <button
                      className="artifact-hub-open-btn destructive"
                      onClick={() => onStopSession()}
                      type="button"
            ...
            <p className="artifact-hub-empty">
              No background tasks have been retained for this run yet.
            </p>
          )}
        </div>
      </section>
      ...
      <section className="artifact-hub-task-section">
        <div className="artifact-hub-task-section-head">
          <span>Delegated graph</span>
          <span>{delegationOverview.stepCount}</span>
        </div>
        <div className="artifact-hub-task-summary-grid artifact-hub-task-summary-grid--nested">
          <article className="artifact-hub-task-card">
            <div className="artifact-hub-task-card-head">Status</div>
            <strong>{delegationOverview.statusLabel}</strong>
            <p>{delegationOverview.detail}</p>
          </article>
          <article className="artifact-hub-task-card">
    ...
    <div className="artifact-hub-permissions">
      ...
      <section className="artifact-hub-permissions-card">
        ...
        <div className="artifact-hub-files-meta artifact-hub-permissions__meta">
          ...
          <span>{localEngineSnapshot?.compatibilityRoutes.length ?? 0} compatibility routes</span>
...
function RemoteEnvView({
  ...
  status: SpotlightRemoteEnvStatus;
  error: string | null;
  onRefreshRemoteEnv?: () => Promise<unknown>;
  onOpenView?: (view: ArtifactHubViewKey) => void;
}) {
  ...
  const diffOverview = useMemo(
    () => buildRemoteEnvDiffOverview(snapshot),
    [snapshot],
  );
    ...
    <div className="artifact-hub-permissions">
      ...
      <div className="artifact-hub-permissions-grid">
        <section className="artifact-hub-permissions-card">
          ...
          <div className="artifact-hub-files-meta artifact-hub-permissions__meta">
            <span>
              Control plane: {snapshot?.controlPlaneBindingCount ?? 0}
            </span>
        ...
        <section className="artifact-hub-permissions-card">
          ...
          {notes.length > 0 ? (
            ...
            <p>
              No additional environment notes are retained for this session yet.
            </p>
          )}
        </section>
      </div>
            ...
            {bindings.map((binding) => (
              ...
              >
                <div className="artifact-hub-generic-meta">
                  ...
                  <span>
                    {binding.redacted ? "Redacted" : binding.secret ? "Secret" : "Visible"}
                  </span>
                </div>
    ...
    <section className="artifact-hub-files-section">
      ...
      {paths.length === 0 ? (
        ...
        <div className="artifact-hub-files-path-list">
          {paths.map((path) => {
            const overview = buildFileContextPathOverview(fileContext, path, pathKind);
            return (
              <article className="artifact-hub-files-path-row" key={`${title}:${path}`}>
                <div className="artifact-hub-files-path-copy">
                  <div className="artifact-hub-files-path-label">{pathLabel(path)}</div>
        ...
        {fileBrowseStatus === "loading" ? (
          ...
          <div className="artifact-hub-files-browser">
            {fileBrowseEntries.map((entry) => {
              ...
              return (
                <article className="artifact-hub-files-browser-row" key={entry.path}>

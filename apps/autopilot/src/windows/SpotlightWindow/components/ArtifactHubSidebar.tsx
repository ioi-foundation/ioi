...
function eventHasPolicyDigest(event: AgentEvent): boolean {
  ...
  return policyKeys.some(
    (key) => toEventString(digest[key as keyof typeof digest]).trim().length > 0,
  );
}
...
export function ArtifactHubSidebar({
  ...
  const scopedEvents = useMemo(() => {
    ...
    return events.filter((event) => eventBelongsToTurnWindow(event, selectedTurn));
  }, [events, selectedTurn]);
  ...
  const securityRows = useMemo<SecurityPolicyRow[]>(() => {
    ...
    for (const event of scopedEvents) {
      ...
      const resolution = toEventString(digest.resolution_action).trim() || "n/a";

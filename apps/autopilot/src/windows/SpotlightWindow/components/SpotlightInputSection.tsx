...
import { useSpotlightCapabilityRegistry, resolveCapabilityRegistryEntryForCatalogSkill } from "../hooks/useSpotlightCapabilityRegistry";
...
import {
  planModePlaceholder,
  planModeStatusCopy,
} from "../utils/planModePrompt";
import {
  applySpotlightVimNormalKey,
  type PendingVimMotionPrefix,
  type PendingVimOperator,
  type SpotlightVimRepeatableCommand,
} from "../utils/spotlightVimComposer";
import type { DropdownOption } from "./SpotlightDropdown";
import { icons } from "./Icons";
import { SpotlightInputControls } from "./SpotlightInputControls";
import {
  SpotlightSlashMenu,
  type SlashMenuItem,
...
export function SpotlightInputSection({
  ...
  } = useSpotlightPermissions(commandsMenuOpen);
  const {
    snapshot: vimModeSnapshot,
    toggle: toggleVimMode,
    enterInsertMode,
    enterNormalMode,
  } = useSpotlightVimMode(true);
    ...
    if (
      ...
      commandItems.push({
        ...
        description: vimModeSnapshot.enabled
          ? "Return Spotlight to the standard shell input posture."
    ...
    (event: KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (vimModeSnapshot.enabled) {
        ...
        if (vimModeSnapshot.modeId === "vim_normal") {
          const outcome = applySpotlightVimNormalKey(
            {
              value: composerValue,
              selectionStart,
              selectionEnd,

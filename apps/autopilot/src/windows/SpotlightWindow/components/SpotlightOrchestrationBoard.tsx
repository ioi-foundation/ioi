...
type SpotlightOrchestrationBoardProps = {
  task: AgentTask | null;
...
): SpotlightPlaybookStepRecord["receipts"][number] | null {
...
): SpotlightPlaybookRunRecord["steps"][number]["receipts"][number] | null {
  return run.steps
    .flatMap((step) => step.receipts)
    .sort((left, right) => right.timestampMs - left.timestampMs)[0] ?? null;
}

function reviewTargetForStep(step: SpotlightPlaybookStepRecord): {
...
function reviewTargetForRun(run: SpotlightPlaybookRunRecord): {
  artifactId: string | null;
  sessionId: string | null;
} {
...
function taskStats(runs: SpotlightPlaybookRunRecord[]) {
...
export function SpotlightOrchestrationBoard({
  task,
  planSummary,
  runs,
  loading,
  ...
  onPromoteRunResult,
  onPromoteStepResult,
}: SpotlightOrchestrationBoardProps) {

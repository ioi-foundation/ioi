...
export type StudioArtifactStageMode = "render" | "source";
...
export function buildArtifactTree(files: StudioArtifactManifestFile[]): WorkspaceNode[] {
...
): StudioArtifactManifestFile | null {
  if (!path) {
    return null;
  }
...
function hasAddressableArtifactFile(manifest: StudioArtifactManifest): boolean {

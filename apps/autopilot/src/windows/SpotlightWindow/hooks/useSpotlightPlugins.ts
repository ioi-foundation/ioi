...
export type SpotlightPluginsStatus = "idle" | "loading" | "ready" | "error";

interface UseSpotlightPluginsOptions {
  enabled: boolean;
...
export function useSpotlightPlugins({
  enabled,
  sessionId,
  workspaceRoot,
}: UseSpotlightPluginsOptions) {
  const query = useMemo(
    () => buildQuery(sessionId, workspaceRoot),
    [sessionId, workspaceRoot],
  );
  const [snapshot, setSnapshot] = useState<SessionPluginSnapshot | null>(null);
  const [status, setStatus] = useState<SpotlightPluginsStatus>("idle");

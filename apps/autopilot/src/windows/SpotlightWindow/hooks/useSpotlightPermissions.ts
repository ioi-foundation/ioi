...
export interface SpotlightPermissionConnectorOverrideSummary {
  connectorId: string;
  entryId: string | null;
  label: string;
  headline: string;
...
export interface SpotlightPermissionProfileSummary {
...
export function useSpotlightPermissions(enabled = true) {
  ...
  } = useSpotlightCapabilityRegistry(enabled);
  const availableProfiles = useMemo<SpotlightPermissionProfileSummary[]>(
    () =>
  ...
  const status = useMemo<SpotlightPermissionsStatus>(() => {

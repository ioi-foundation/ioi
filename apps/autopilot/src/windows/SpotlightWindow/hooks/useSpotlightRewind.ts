...
export type SpotlightRewindStatus = "idle" | "loading" | "ready" | "error";

export function useSpotlightRewind(enabled = true) {
  const [snapshot, setSnapshot] = useState<SessionRewindSnapshot | null>(null);
  const [status, setStatus] = useState<SpotlightRewindStatus>("idle");

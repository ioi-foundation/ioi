...
export type SpotlightLocalEngineStatus =
  | "idle"
  | "loading"
  | "ready"
  | "error";
...
export function useSpotlightLocalEngine(enabled = true) {
  const [snapshot, setSnapshot] = useState<LocalEngineSnapshot | null>(null);
  const [status, setStatus] = useState<SpotlightLocalEngineStatus>("idle");

...
export interface SpotlightPrivacyConnectorSummary {
  connectorId: string;
  label: string;
...
export interface SpotlightPrivacySnapshot {
  ...
  connectors: SpotlightPrivacyConnectorSummary[];
}

interface UseSpotlightPrivacySettingsOptions {
  policyState: ShieldPolicyState;
  governanceRequest: CapabilityGovernanceRequest | null;
  connectorOverrides: SpotlightPermissionConnectorOverrideSummary[];
  activeOverrideCount: number;
...
export function useSpotlightPrivacySettings({
  policyState,
  governanceRequest,
  connectorOverrides,
  activeOverrideCount,
  rememberedApprovals,
  isPiiGate = false,
}: UseSpotlightPrivacySettingsOptions): SpotlightPrivacySnapshot {
  return useMemo(() => {

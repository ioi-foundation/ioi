...
export type SpotlightSourceControlStatus =
  | "idle"
  | "loading"
  | "ready"
  | "error"
...
interface UseSpotlightSourceControlOptions {
  enabled: boolean;
...
export function useSpotlightSourceControl({
  enabled,
  workspaceRoot,
}: UseSpotlightSourceControlOptions) {
  ...
  const [status, setStatus] = useState<SpotlightSourceControlStatus>("idle");

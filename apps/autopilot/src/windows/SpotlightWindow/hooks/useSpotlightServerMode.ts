...
export type SpotlightServerModeStatus = "idle" | "loading" | "ready" | "error";

interface UseSpotlightServerModeOptions {
  enabled: boolean;
...
export function useSpotlightServerMode({
  enabled,
  sessionId,
  workspaceRoot,
}: UseSpotlightServerModeOptions) {
  const query = useMemo(
    () => buildQuery(sessionId, workspaceRoot),
    [sessionId, workspaceRoot],
  );
  const [snapshot, setSnapshot] = useState<SessionServerSnapshot | null>(null);
  const [status, setStatus] = useState<SpotlightServerModeStatus>("idle");

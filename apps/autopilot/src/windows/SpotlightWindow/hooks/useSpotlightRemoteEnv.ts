...
export type SpotlightRemoteEnvStatus = "idle" | "loading" | "ready" | "error";

interface UseSpotlightRemoteEnvOptions {
  enabled: boolean;
...
export function useSpotlightRemoteEnv({
  enabled,
  sessionId,
  workspaceRoot,
}: UseSpotlightRemoteEnvOptions) {
  const query = useMemo(
    () => buildQuery(sessionId, workspaceRoot),
    [sessionId, workspaceRoot],
  );
  const [snapshot, setSnapshot] = useState<SessionRemoteEnvSnapshot | null>(null);
  const [status, setStatus] = useState<SpotlightRemoteEnvStatus>("idle");

...
export type SpotlightCompactionStatus =
  | "idle"
  | "loading"
  | "ready"
  | "error"
...
export function useSpotlightCompaction(enabled = true) {
  const [snapshot, setSnapshot] = useState<SessionCompactionSnapshot | null>(null);
  const [status, setStatus] = useState<SpotlightCompactionStatus>("idle");

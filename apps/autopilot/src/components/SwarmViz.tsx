...
// Features:
// - Squad-based approval (Group Requisitions)
// - Liquid Budget Burn
...
const TreeIcon = () => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M12 3v18"/><path d="M8 7l4-4 4 4"/><path d="M8 21H4a2 2 0 0 1-2-2v-4"/><path d="M16 21h4a2 2 0 0 0 2-2v-4"/>
  </svg>
);
...
const TimelineIcon = () => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    ...
    <line x1="6" y1="20" x2="6" y2="16"/><line x1="12" y1="4" x2="12" y2="4"/>
  </svg>
);
  ...
  useEffect(() => {
    ...
    agents.forEach(agent => {
      ...
      if (!prev) {
        newEvents.push({
          ...
          message: agent.status === 'requisition' 
            ? `${agent.role} requested (est. $${agent.estimated_cost?.toFixed(4)})`
      ...
      if (prev && prev.status !== agent.status) {
        ...
        } else if (prev.status === 'requisition' && agent.status === 'running') {
          newEvents.push({
            id: `${agent.id}-approved-${Date.now()}`,
            agentId: agent.id,
            agentName: agent.name,
            type: 'spawn',
            message: `Approved and starting...`,
            timestamp: new Date(),
          });
        }
      }
      
      // Thought changed (streaming)
      ...
      // Artifact produced
      if (prev && prev.artifacts_produced < agent.artifacts_produced) {
        newEvents.push({
          id: `${agent.id}-artifact-${Date.now()}`,
          agentId: agent.id,
          agentName: agent.name,
          type: 'artifact',
          message: `Produced artifact #${agent.artifacts_produced}`,
          timestamp: new Date(),
        });
      }
    });
    ...
    if (newEvents.length > 0) {
      setActivityLog(prev => [...newEvents, ...prev].slice(0, 50)); // Keep last 50
    }
  ...
  return (
    <div className="swarm-viz">
        ...
        <div className="metric-budget-bar">
          <div className="budget-info">
            ...
            {metrics.pendingCost > 0 && (
              <span className="budget-pending">+${metrics.pendingCost.toFixed(2)} pending</span>
            )}
          </div>
          <div className="budget-track">
            <div 
              ...
              style={{ 
                ...
                background: metrics.activeBudgetUsed > metrics.totalBudgetCap * 0.8 ? '#EF4444' : '#22C55E'
              }} 
            />
            {metrics.pendingCost > 0 && (
              <div 
      ...
      <div className="swarm-viz-content">
        {/* LEFT: Tree or Timeline */}
        <div className={`swarm-viz-main ${selectedAgent ? 'has-detail' : ''}`}>
          {viewMode === 'tree' ? (
            <div className="swarm-tree-v2">
              {roots.length === 0 ? (
                <div className="swarm-empty">
                  <div className="empty-icon">🔄</div>
                  <div>Initializing swarm topology...</div>
                </div>
              ) : (
                roots.map(agent => (
                  <AgentNode 
...
function AgentNode({ 
  ...
  // Split children into those needing approval (Requisition) and active ones
  ...
  const budgetPercent = agent.budget_cap > 0 
  ...
  return (
    ...
    >
      {/* Connector line - Hide if root or inside squad container */}
      {!isRoot && !isSquadMember && <div className={`connector-line ${isRequisition ? 'dashed' : ''}`} />}
      ...
      >
        {/* Left: Expand toggle + Icon */}
        <div className="agent-left">
          {hasChildren ? (
            <button 
              className="expand-btn"
              onClick={(e) => { e.stopPropagation(); onToggleExpand(agent.id); }}
            >
        ...
        <div className="agent-center">
          ...
          <div className="agent-status-row">
            {isRequisition ? (
              <span className="status-badge requisition">
                <span className="status-icon">⏳</span>
                AWAITING APPROVAL
              </span>
            ) : isRunning ? (
              <span className="status-badge running">
                <span className="status-spinner-mini" />
                {agent.current_thought 
                  ? (agent.current_thought.length > 35 
                      ? agent.current_thought.substring(0, 35) + '...' 
                      : agent.current_thought)
                  : 'Processing...'}
              </span>
            ) : isPaused ? (
              ...
              <span className="status-badge completed">
                <CheckIcon />
                DONE · {agent.artifacts_produced} artifacts
              </span>
            ) : isFailed ? (
          ...
          {/* Budget bar (not shown for requisitions) */}
          {!isRequisition && (
            <div className="agent-budget-row">
              <div className="mini-budget-track">
                <div 
                  ...
                  style={{ 
                    width: `${budgetPercent}%`,
                    background: budgetPercent > 80 ? '#EF4444' : getStatusColor(agent.status)
                  }}
                />
              </div>
            ...
            <div className="squad-group">
               <div className="squad-header">
                  ...
                  <button 
                    ...
                    onClick={(e) => { e.stopPropagation(); onBatchApprove?.(requisitionChildren); }}
                  >
                    <CheckIcon /> Sign & Hire All
                  </button>
               </div>
               <div className="squad-list">
                 {requisitionChildren.map(child => (
                    <AgentNode 
                      key={child.id}
                      agent={child}
                      getChildren={getChildren}
                      depth={depth + 1} // Indentation handled by squad container padding
                      expandedNodes={expandedNodes}
                      onToggleExpand={onToggleExpand}
                      selectedAgentId={selectedAgentId}
                      onSelect={onSelect}
...
function ActivityItem({ event, onSelectAgent }: { event: ActivityEvent; onSelectAgent: (id: string) => void }) {
  ...
  return (
    ...
    >
      <div className="activity-icon" style={{ background: `${style.color}20`, color: style.color }}>
        {style.icon}
      </div>
...
function AgentDetailPanel({ 
  agent, activityLog, onClose, onApprove, onReject, onPause, onResume, onCancel 
}: { 
  agent: SwarmAgent;
  ...
  return (
    <div className="agent-detail-panel">
      ...
      <div className="detail-body">
        ...
        <div className="detail-section">
          ...
          <div className="terminal-box">
            {agentLogs.length === 0 ? (
                <div className="terminal-line" style={{fontStyle: 'italic', opacity: 0.5}}>No logs available</div>
            ) : (
            ...
            {agent.status === 'running' && (
                <div className="terminal-line latest">
                    <span className="terminal-ts">{new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit', second:'2-digit'})}</span>
                    <span>{agent.current_thought || 'Processing...'}<span className="terminal-cursor" /></span>
                </div>
            )}
          </div>
        </div>
          ...
          <div className="detail-section">
            ...
            <div className="budget-detail">
              <div className="budget-numbers">
                <span className="budget-used">${agent.budget_used.toFixed(4)}</span>
                <span className="budget-of">of</span>
                <span className="budget-total">${agent.budget_cap.toFixed(2)}</span>
              </div>
                ...
                <div 
                  ...
                  style={{ 
                    ...
                    background: budgetPercent > 80 
                      ...
                      : 'linear-gradient(90deg, #22C55E, #3D85C6)'
                  }}
                />
              </div>
              <div className="budget-percent">{budgetPercent.toFixed(1)}% used</div>
            </div>
          </div>
        )}
      ...
      <div className="detail-actions">
        {agent.status === 'requisition' && (
          <>
            <button className="detail-btn primary" onClick={() => onApprove?.(agent.id)}>
              <CheckIcon /> Approve
            </button>
            <button className="detail-btn danger" onClick={() => onReject?.(agent.id)}>
              <XIcon /> Reject
            </button>
          </>
        )}
        {agent.status === 'running' && (
          <>
            <button className="detail-btn secondary" onClick={() => onPause?.(agent.id)}>
              <PauseIcon /> Pause
            </button>
            <button className="detail-btn danger" onClick={() => onCancel?.(agent.id)}>
              <XIcon /> Cancel
            </button>
          </>
        )}
        {agent.status === 'paused' && (
          <>
            <button className="detail-btn primary" onClick={() => onResume?.(agent.id)}>
              <PlayIcon /> Resume
            </button>
            <button className="detail-btn danger" onClick={() => onCancel?.(agent.id)}>
              <XIcon /> Cancel

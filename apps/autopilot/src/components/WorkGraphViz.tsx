...
import type { WorkGraphAgent } from "../types";
import "./WorkGraphViz.css";
...
const getStatusColor = (status: string): string => {
  const map: Record<string, string> = {
    ...
    requisition: "var(--accent-blue)",
    negotiating: "var(--accent-violet)",
  };
...
interface WorkGraphVizProps {
  agents: WorkGraphAgent[];
...
export function WorkGraphViz({
  ...
  const prevAgentsRef = useRef<WorkGraphAgent[]>([]);
  ...
  useEffect(() => {
    ...
    agents.forEach((agent) => {
      ...
      if (
        prev &&
        prev.current_thought !== agent.current_thought &&
  ...
  const { roots, getChildren } = useMemo(() => {
    const childMap = new Map<string | null, WorkGraphAgent[]>();
    agents.forEach((agent) => {
    ...
    return {
      roots: childMap.get(null) || [],
      getChildren: (id: string) => childMap.get(id) || [],
    };
  }, [agents]);
  ...
  const metrics = useMemo(() => {
    ...
    const activeBudgetUsed = agents
      .filter((a) => a.status !== "requisition")
      .reduce((sum, a) => sum + (a.budget_used || 0), 0);
    const totalBudgetCap = agents
      .filter((a) => a.status !== "requisition")
      .reduce((sum, a) => sum + (a.budget_cap || 0), 0);
    const pendingCost = agents
      .filter((a) => a.status === "requisition")
      .reduce((sum, a) => sum + (a.estimated_cost || 0), 0);
    const totalArtifacts = agents.reduce(
      (sum, a) => sum + (a.artifacts_produced || 0),
      0,
    );
  ...
  const handleBatchApprove = useCallback(
    (squadAgents: WorkGraphAgent[]) => {
      squadAgents.forEach((a) => {
        if (onApproveAgent) onApproveAgent(a.id);
      });
    },
    [onApproveAgent],
  );
  ...
  return (
    <div className="work-graph-viz">
      {/* HEADER */}
      <div className="work-graph-viz-header">
        <div className="work-graph-viz-title">
          <div className="work-graph-viz-icon">
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
            ...
            >
              ...
              <path d="M7.5 17.5 10 12.5" />
              <path d="M16.5 17.5 14 12.5" />
            </svg>
          </div>
          <span>Work graph orchestration</span>
          <span className="work-graph-viz-count">{metrics.total} agents</span>
        </div>

        <div className="work-graph-viz-controls">
      ...
      {/* METRICS BAR */}
      <div className="work-graph-metrics-bar">
        <MetricChip
          label="Active"
        ...
        <div className="metric-budget-bar">
          <div className="budget-info">
            <span className="budget-spent">
              ${metrics.activeBudgetUsed.toFixed(4)}
            </span>
            <span className="budget-separator">/</span>
            <span className="budget-cap">
              ${metrics.totalBudgetCap.toFixed(2)}
            </span>
            {metrics.pendingCost > 0 && (
              <span className="budget-pending">
                +${metrics.pendingCost.toFixed(2)} pending
              </span>
            )}
          </div>
          <div className="budget-track">
            <div
              ...
              style={{
                width: `${Math.min((metrics.activeBudgetUsed / Math.max(metrics.totalBudgetCap, 0.01)) * 100, 100)}%`,
                background:
                  metrics.activeBudgetUsed > metrics.totalBudgetCap * 0.8
                    ? "var(--status-error)"
                    : "var(--status-success)",
              }}
            />
      ...
      {/* MAIN CONTENT */}
      <div className="work-graph-viz-content">
        {/* LEFT: Tree/Timeline */}
        <div
          className={`work-graph-viz-main ${selectedAgent ? "has-detail" : ""}`}
          onClick={handleBackgroundClick}
        >
          {viewMode === "tree" ? (
            <div className="work-graph-tree-v2">
              {roots.length === 0 ? (
                <div className="work-graph-empty">
                  <div className="empty-icon">🔄</div>
                  <div>Initializing work graph...</div>
                </div>
              ) : (
                roots.map((agent) => (
                  <AgentNode
            ...
            <div className="work-graph-timeline" ref={activityRef}>
...
function MetricChip({
  ...
  return (
    ...
    >
      <span
        className={`metric-dot ${pulse ? "pulse" : ""}`}
...
interface AgentNodeProps {
  ...
  getChildren: (id: string) => WorkGraphAgent[];
  depth: number;
  ...
  onBatchApprove?: (agents: WorkGraphAgent[]) => void;
...
function AgentNode({
  ...
  const requisitionChildren = children.filter(
    (c) => c.status === "requisition",
  );
  ...
  const budgetPercent =
    agent.budget_cap > 0
      ? Math.min((agent.budget_used / agent.budget_cap) * 100, 100)
      : 0;

  // [NEW] Memory usage heuristic
  const usedMemory =
    agent.current_thought && agent.current_thought.includes("[SKILL]");

  return (
    <div
      className={`agent-node-v2 depth-${Math.min(depth, 3)}`}
      style={{ "--depth": isSquadMember ? 0 : depth } as React.CSSProperties}
    >
      {!isSquadMember && depth > 0 && (
        <div className={`connector-line ${isRequisition ? "dashed" : ""}`} />
      )}
      ...
      >
        <div className="agent-left">
          ...
          <div className={`agent-avatar ${isRunning ? "active" : ""}`}>
        ...
        <div className="agent-center">
          <div className="agent-name-row">
            ...
            {usedMemory && (
              <span className="memory-badge" title="Using retrieved memory">
                <BulbIcon />
              </span>
            )}
          </div>
          ...
          {!isRequisition && (
            <div className="agent-budget-row">
              ...
              <span className="budget-text">
                ${agent.budget_used.toFixed(3)} / ${agent.budget_cap.toFixed(2)}
              </span>
            </div>
          )}
          {isRequisition && (
            <div className="requisition-cost">
              Est. cost: <strong>${agent.estimated_cost?.toFixed(4)}</strong>
            </div>
          )}
        </div>
              ...
              <div className="squad-header">
                <div className="squad-title">
                  <span className="squad-label">
                    PROPOSED SQUAD ({requisitionChildren.length})
                  </span>
                ...
                <button
                  className="squad-hire-btn"
                  onClick={(e) => {
                    e.stopPropagation();
                    onBatchApprove?.(requisitionChildren);
                  }}
                >
...
function ActivityItem({
  ...
  const typeStyles: Record<string, { icon: string; color: string }> = {
    spawn: { icon: "🚀", color: "var(--accent-blue)" },
    thought: { icon: "💭", color: "var(--text-secondary)" },
    action: { icon: "⚡", color: "var(--status-warning)" },
    artifact: { icon: "📦", color: "var(--accent-violet)" },
    complete: { icon: "✅", color: "var(--status-success)" },
    error: { icon: "❌", color: "var(--status-error)" },
    requisition: { icon: "📋", color: "var(--accent-blue)" },
  };
...
function AgentDetailPanel({
  ...
  const budgetPercent =
    agent.budget_cap > 0
      ? Math.min((agent.budget_used / agent.budget_cap) * 100, 100)
      : 0;
  const agentLogs = activityLog.filter(
    (l: ActivityEvent) => l.agentId === agent.id,
  );
    ...
    <div className="agent-detail-panel">
      ...
      <div className="detail-body">
        <div className="detail-section">
          ...
          >
            <span
              className={`status-dot-lg ${agent.status === "running" ? "pulse" : ""}`}
          ...
          <div className="terminal-box">
            {agentLogs.length === 0 ? (
              ...
              agentLogs.slice(0, 5).map((log: ActivityEvent, i: number) => (
                <div
                  key={log.id}
                  className={`terminal-line ${i === 0 ? "latest" : ""}`}
                >
            ...
            {agent.status === "running" && (
              <div className="terminal-line latest">
                <span className="terminal-ts">Now</span>
                <span>
                  {agent.current_thought}
            ...
            <div className="budget-detail">
              <div className="budget-numbers">
                <span className="budget-used">
                  ${agent.budget_used.toFixed(4)}
                </span>{" "}
                <span className="budget-of">of</span>{" "}
                <span className="budget-total">
                  ${agent.budget_cap.toFixed(2)}
                </span>
              </div>
              <div className="budget-track-lg">
                <div
                  className="budget-fill-lg"
                  style={{
                    width: `${budgetPercent}%`,
                    background:
                      budgetPercent > 80
                        ? "linear-gradient(90deg, var(--status-warning), var(--status-error))"
                        : "linear-gradient(90deg, var(--status-success), var(--accent-blue))",
                  }}
                />
              </div>
            </div>
        ...
        <div className="detail-section">
          ...
          <div className="policy-row">
            <div className="policy-identicon" />
            <div className="policy-hash">
              <code>{agent.policy_hash}</code>
            </div>
          </div>
        </div>
          ...
          <div className="detail-section">
            ...
            <div className="estimate-box">
              ...
              <span className="estimate-note">
                This agent requires approval before execution
              </span>
            </div>
          </div>
        )}

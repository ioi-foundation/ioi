...
test("artifact intent uses injected retrieval and workspace predicates", () => {
  const intent = createIntent({
    ...
    workspaceTargetsForPrompt: () => [{ kind: "path", path: "apps/autopilot", reason: "test" }],
  });
  ...
  assert.deepEqual(workspace.workspace.targets, [{ kind: "path", path: "apps/autopilot", reason: "test" }]);
});

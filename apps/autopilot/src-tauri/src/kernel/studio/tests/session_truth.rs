...
fn ready_nonartifact_route_with_gate_info_marks_task_gate() {
    ...
    task.gate_info = Some(GateInfo {
        ...
        description: "Studio needs approval before opening the currentness route.".to_string(),
        risk: "low".to_string(),
...
#[test]
fn weather_tool_widget_route_requires_studio_primary_execution() {
    ...
    assert!(task_is_studio_authoritative(&task));
    assert!(task_requires_studio_primary_execution(&task));
}
...
fn weather_tool_widget_follow_up_reuses_retained_location_scope() {
    let outcome_request = StudioOutcomeRequest {
        ...
        lane_frame: None,
        request_frame: Some(
            ioi_types::app::studio::StudioNormalizedRequestFrame::Weather(
                ioi_types::app::studio::StudioWeatherRequestFrame {
...
#[test]
fn sports_tool_widget_route_requires_studio_primary_execution() {
...
#[test]
fn places_tool_widget_route_requires_studio_primary_execution() {
...
#[test]
fn conversation_single_pass_route_requires_studio_primary_execution() {
...
#[test]
fn workspace_grounded_single_pass_route_stays_off_studio_primary_execution() {
...
#[test]
fn visualizer_route_requires_studio_primary_execution() {
...
fn non_artifact_route_receipt_event_preserves_explicit_route_decision() {
    ...
    let route_event = task
        .events
        .iter()
        .find(|event| event.title == "Studio route decision")
...
#[test]
fn clarification_non_artifact_route_stays_studio_primary() {
    ...
    super::apply_studio_authoritative_status(&mut task, None);
...
fn missing_connector_reframes_impossible_workspace_clarification() {
    ...
    assert_eq!(
        ...
        vec![
            "Slack is not available in this runtime yet. Should Studio wait for you to connect it, or should I work from pasted data instead?"
                .to_string()
        ]
    );
...
fn connected_mail_route_clears_redundant_identity_clarification() {
    ...
    let mut outcome_request = StudioOutcomeRequest {
        ...
        confidence: 0.91,
        needs_clarification: true,
        clarification_questions: vec!["Which inbox should Studio check?".to_string()],
        routing_hints: vec!["shared_answer_surface".to_string()],
        lane_frame: None,
        request_frame: None,
        source_selection: None,
...
fn attach_non_artifact_session_preserves_retained_widget_state_during_topology_refresh() {
    let mut task = test_task(StudioArtifactVerificationStatus::Ready);
    let provenance = crate::models::StudioRuntimeProvenance {
        kind: crate::models::StudioRuntimeProvenanceKind::RealLocalRuntime,
        label: "local".to_string(),
        model: Some("fixture".to_string()),
        endpoint: None,
    };
    let initial_request = StudioOutcomeRequest {
    ...
    let follow_up_request = StudioOutcomeRequest {
        request_id: "weather-follow-up".to_string(),
...
fn weather_advice_clarification_surfaces_structured_scope_options() {
    with_runtime_locality_scope_hint_override(None, || {
        ...
        assert_eq!(
            request.question,
            "What city should Studio check the weather for?"
        );
...
fn message_compose_clarification_uses_domain_specific_prompt_and_options() {
    let outcome_request = StudioOutcomeRequest {
        ...
        confidence: 0.9,
        needs_clarification: true,
        clarification_questions: vec!["What channel should Studio use?".to_string()],
        routing_hints: vec!["shared_answer_surface".to_string()],
        lane_frame: None,
        request_frame: Some(
            ioi_types::app::studio::StudioNormalizedRequestFrame::MessageCompose(
                ioi_types::app::studio::StudioMessageComposeRequestFrame {
                    channel: None,
...
fn places_clarification_uses_domain_specific_anchor_options() {
    with_runtime_locality_scope_hint_override(None, || {
        let outcome_request = StudioOutcomeRequest {
            ...
            confidence: 0.87,
            needs_clarification: true,
            clarification_questions: vec!["Where should Studio search?".to_string()],
            routing_hints: vec![
                "tool_widget:places".to_string(),
                "narrow_surface_preferred".to_string(),
            ],
            lane_frame: None,
            request_frame: Some(
                ioi_types::app::studio::StudioNormalizedRequestFrame::Places(
                    ioi_types::app::studio::StudioPlacesRequestFrame {
                        search_anchor: None,
...
fn places_request_frame_promotes_missing_anchor_into_clarification_gate() {
    ...
    assert_eq!(
        outcome_request.clarification_questions,
        vec![
            "Which neighborhood, city, or anchor location should Studio search around?".to_string()
        ]
    );
    assert!(outcome_request
        .routing_hints
...
fn prioritization_clarification_surfaces_structured_ranking_options() {
    ...
    assert_eq!(
        ...
        Some(
            "Studio paused before selecting the outcome surface because it needs clarification: What should drive the ranking: impact, urgency, or return on investment?"
        )
    );
    assert_eq!(request.options.len(), 3);
...
#[test]
fn currentness_override_conversation_does_not_stay_studio_primary() {

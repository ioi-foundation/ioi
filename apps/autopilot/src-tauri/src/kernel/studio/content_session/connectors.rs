...
pub(in crate::kernel::studio) struct StudioConnectorRouteContext {
    pub(in crate::kernel::studio) routing_hints: Vec<String>,
    pub(in crate::kernel::studio) force_clarification_question: Option<String>,
    pub(in crate::kernel::studio) clear_redundant_identity_clarification: bool,
}
...
pub(in crate::kernel::studio) fn infer_connector_route_context_from_catalog(
    intent: &str,
    connectors: &[crate::kernel::connectors::ConnectorCatalogEntry],
) -> Option<StudioConnectorRouteContext> {
    ...
    if matching.is_empty() {
        return Some(StudioConnectorRouteContext {
            routing_hints: vec![
    ...
    let force_clarification_question = if best_rank < 3 {
        ...
        let choice_labels = top_ranked
            ...
            .collect::<Vec<_>>();
        Some(format!(
            "Studio found more than one connected {target_label} route. Which connector should it use{}?",
    ...
    Some(StudioConnectorRouteContext {
        routing_hints,
...
pub(in crate::kernel::studio) fn merge_connector_route_context(
    outcome_request: &mut StudioOutcomeRequest,
    connector_context: StudioConnectorRouteContext,
) {

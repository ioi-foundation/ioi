...
const CHAT_SKILL_DISCOVERY_LIMIT: usize = 4;
const CHAT_EXEMPLAR_DISCOVERY_LIMIT: usize = 3;

#[derive(Debug, Clone)]
struct ChatSkillAccumulator {
    name: String,
    description: String,
    ...
    matched_need_kinds: Vec<ChatArtifactSkillNeedKind>,
    match_rationale: String,
    guidance_markdown: Option<String>,
    required_matches: usize,
}

fn chat_skill_trace(message: impl AsRef<str>) {
    if std::env::var_os("IOI_CHAT_PROOF_TRACE").is_some() {
        eprintln!("[chat-skill-trace] {}", message.as_ref());
    }
}

fn skill_kind_label(kind: ChatArtifactSkillNeedKind) -> String {
...
fn skill_priority_label(priority: ChatArtifactSkillNeedPriority) -> String {
...
fn sort_and_truncate_selected_skills(
    mut selected: Vec<(usize, usize, ChatArtifactSelectedSkill)>,
) -> Vec<ChatArtifactSelectedSkill> {
    ...
    selected.truncate(CHAT_SKILL_DISCOVERY_LIMIT);
...
fn emit_skill_discovery_progress(
    observer: Option<&ChatArtifactGenerationProgressObserver>,
    current_step: impl Into<String>,
    preparation_needs: Option<ChatArtifactPreparationNeeds>,
    skill_discovery_resolution: Option<ChatArtifactSkillDiscoveryResolution>,
...
fn skill_discovery_active_step() -> ArtifactOperatorStep {
    operator_step(
        ...
        "skill_discovery",
        "Check for guidance",
        "Chat is checking whether published runtime guidance should be attached before authoring.",
        ChatArtifactRuntimeEventStatus::Active,
    )
}

fn skill_discovery_complete_steps(
    resolution: &ChatArtifactSkillDiscoveryResolution,
    ...
    if !selected_skills.is_empty() {
        steps.push(operator_step(
            ...
            if selected_skills.len() == 1 {
                format!(
                    "Chat read {} before authoring the artifact.",
                    selected_skills[0].name
                )
            } else {
                format!(
                    "Chat read {} skill guides before authoring the artifact.",
                    selected_skills.len()
                )
            },
            ChatArtifactRuntimeEventStatus::Complete,
...
fn artifact_brief_active_step() -> ArtifactOperatorStep {
    operator_step(
        ...
        "Chat is shaping the artifact brief that will guide authoring.",
        ChatArtifactRuntimeEventStatus::Active,
    )
}
...
fn artifact_brief_complete_step(
    ...
    operator_step(
        ...
        "Shape artifact brief",
        format!(
            "Chat prepared a typed artifact brief for {}.",
...
fn skill_discovery_title(resolution: &ChatArtifactSkillDiscoveryResolution) -> String {
    match resolution
        .guidance_status
        .trim()
        .to_ascii_lowercase()
...
fn skill_discovery_completion_message(
    ...
    match resolution.guidance_status.trim().to_ascii_lowercase().as_str() {
        ...
        "unavailable" => "Skill discovery is complete. Chat checked for guidance before authoring but did not find a qualifying skill to attach."
            .to_string(),
        _ => "Skill discovery is complete.".to_string(),
    }
}

pub(super) fn prepare_chat_artifact_planning_context(
    app: &AppHandle,
    ...
    let discovery_blueprint = derive_chat_artifact_blueprint(request, &discovery_brief);
    let discovery_artifact_ir =
        compile_chat_artifact_ir(request, &discovery_brief, &discovery_blueprint);
    let discovery_context = derive_chat_artifact_prepared_context(
        request,
        &discovery_brief,
    ...
    let pending_skill_discovery_resolution =
        discovery_context
            .preparation_needs
            .as_ref()
            .map(|preparation_needs| ChatArtifactSkillDiscoveryResolution {
                ...
                rationale:
                    "Chat is checking the published runtime guidance corpus before authoring."
                        .to_string(),
                failure_reason: None,
            });
    ...
    let selected_skills = match tauri::async_runtime::block_on(async {
        ...
        Ok(Ok(skills)) => skills,
        Ok(Err(error)) => {
            chat_skill_trace(format!("planning_context:skill_error {}", error));
            Vec::new()
        }
        Err(_) => {
            chat_skill_trace("planning_context:skill_timeout");
            Vec::new()
        }
    };
    let skill_discovery_context = derive_chat_artifact_prepared_context(
        request,
        &discovery_brief,
    ...
    let mut brief = match tauri::async_runtime::block_on(async {
        ...
        Ok(Ok(brief)) => brief,
        Ok(Err(error)) => {
            chat_skill_trace(format!("planning_context:brief_error {}", error));
            discovery_brief.clone()
        }
        Err(_) => {
            chat_skill_trace("planning_context:brief_timeout");
            discovery_brief.clone()
        }
    };
    ...
    let retrieved_exemplars = match tauri::async_runtime::block_on(async {
        tokio::time::timeout(
            discovery_timeout,
            retrieve_chat_artifact_exemplars(
                memory_runtime,
                inference_runtime.clone(),
                &brief,
                &blueprint,
        ...
        Ok(Err(error)) => {
            chat_skill_trace(format!("planning_context:exemplar_error {}", error));
            Vec::new()
        }
        Err(_) => {
            chat_skill_trace("planning_context:exemplar_timeout");
            Vec::new()
        }
    };
    let retrieved_sources = match tauri::async_runtime::block_on(async {
        ...
        Ok(sources) => sources,
        Err(_) => {
            chat_skill_trace("planning_context:source_timeout");
...
async fn resolve_selected_skills(
    ...
    blueprint: &ChatArtifactBlueprint,
    artifact_ir: &ChatArtifactIR,
) -> Result<Vec<ChatArtifactSelectedSkill>, String> {
...
async fn retrieve_chat_artifact_exemplars(
    memory_runtime: &Arc<MemoryRuntime>,
    inference_runtime: Arc<dyn InferenceRuntime>,
    brief: &ChatArtifactBrief,
    blueprint: &ChatArtifactBlueprint,
...
) -> Result<Vec<ChatArtifactExemplar>, String> {
    let query = build_chat_artifact_exemplar_query(brief, blueprint, artifact_ir, taste_memory);
    let embedding = inference_runtime
        .embed_text(&query)
        .await
        .map_err(|error| format!("Failed to embed Chat exemplar query: {}", error))?;
    let mut exemplars = memory_runtime
        .hybrid_search_archival_memory(&HybridArchivalMemoryQuery {
            scopes: vec![CHAT_ARTIFACT_EXEMPLAR_SCOPE.to_string()],
            thread_id: None,
            text: query,
            embedding: Some(embedding),
            limit: CHAT_EXEMPLAR_DISCOVERY_LIMIT * 2,
            candidate_limit: CHAT_EXEMPLAR_DISCOVERY_LIMIT * 4,
            allowed_trust_levels: vec!["runtime_observed".to_string()],
        })
        .map_err(|error| format!("Failed to search Chat exemplars: {}", error))?
        .into_iter()
        .filter_map(|hit| chat_artifact_exemplar_from_archival_record(&hit.record))
    ...
    exemplars.truncate(CHAT_EXEMPLAR_DISCOVERY_LIMIT);
    Ok(exemplars)
}
...
async fn resolve_selected_skills_with_client(
    ...
    let mut accumulators = HashMap::<[u8; 32], ChatSkillAccumulator>::new();
    ...
    for (need_index, need) in blueprint.skill_needs.iter().enumerate() {
        ...
        let embedding = inference_runtime
            .embed_text(&query)
            .await
            .map_err(|error| format!("Failed to embed Chat skill query: {}", error))?;
        let hits = memory_runtime
            .semantic_search_archival_memory(&SemanticArchivalMemoryQuery {
                ...
                text_filter: None,
                embedding,
                limit: CHAT_SKILL_DISCOVERY_LIMIT * 2,
            })
            .map_err(|error| format!("Failed to search Chat skill corpus: {}", error))?;
        ...
        for hit in hits {
            ...
            let entry = accumulators
                .entry(skill_hash)
                .or_insert_with(|| ChatSkillAccumulator {
            ...
            if need.priority == ChatArtifactSkillNeedPriority::Required {

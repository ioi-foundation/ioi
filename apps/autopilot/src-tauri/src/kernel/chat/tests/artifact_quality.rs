...
#[test]
fn chat_authoritative_status_preserves_completed_inline_reply() {
...
fn render_eval_blocker_overrides_acceptance_pass_for_html_artifacts() {
    ...
    let render_evaluation = ChatArtifactRenderEvaluation {
        ...
        findings: vec![ChatArtifactRenderFinding {
            ...
            summary:
                "Desktop and mobile render captures must both exist before Chat can trust the surfaced first paint."
                    .to_string(),
        }],
        acceptance_obligations: Vec::new(),
        execution_witnesses: Vec::new(),
...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>Chat validation probe</title>
    <style>
      body {
...
"#;
    let fixture_path = write_temp_chat_html_fixture(
        "chat-html-validation",
    ...
    let initial_errors = driver
        .probe_selector("html[data-chat-validation-error='true']")
        .await
...
async fn browser_render_evaluator_captures_desktop_mobile_and_interaction() {
    ...
    let candidate = ChatGeneratedArtifactPayload {
        ...
        files: vec![ChatGeneratedArtifactFile {
            ...
            downloadable: true,
            encoding: None,
            body: "<!doctype html><html><head><meta charset=\"utf-8\"><style>body{margin:0;font-family:Georgia,serif;background:#f6f1e7;color:#1b1a17;}main{display:grid;gap:1rem;padding:1.5rem;}section,aside{background:#fffdf8;border:1px solid #d5c7ad;border-radius:16px;padding:1rem;}button{border:1px solid #8c6f48;background:#f0dfc0;border-radius:999px;padding:0.45rem 0.85rem;}[hidden]{display:none !important;}</style></head><body><main><section><h1>Launch review</h1><p>Compare readiness and metrics without leaving the artifact surface.</p><div><button type=\"button\" data-chat-render-primary-action=\"true\" data-view=\"overview\">Overview</button><button type=\"button\" data-view=\"metrics\">Metrics</button></div></section><section id=\"overview-panel\" data-view-panel=\"overview\"><h2>Overview</h2><p>Readiness evidence stays visible on first paint.</p></section><section id=\"metrics-panel\" data-view-panel=\"metrics\" hidden><h2>Metrics</h2><p>Metrics evidence becomes visible after interaction.</p></section><aside><p id=\"detail-copy\">Overview selected.</p></aside></main><script>const detail=document.getElementById('detail-copy');const panels=Array.from(document.querySelectorAll('[data-view-panel]'));document.querySelectorAll('button[data-view]').forEach((button)=>button.addEventListener('click',()=>{panels.forEach((panel)=>{panel.hidden=panel.dataset.viewPanel!==button.dataset.view;});detail.textContent=button.dataset.view + ' selected';}));</script></body></html>".to_string(),
        }],
    };
    ...
    assert!(render_evaluation
        .captures
        .iter()
        .any(|capture| capture.viewport == ChatArtifactRenderCaptureViewport::Desktop));
    assert!(render_evaluation
        .captures
        .iter()
        .any(|capture| capture.viewport == ChatArtifactRenderCaptureViewport::Mobile));
    assert!(render_evaluation
        .captures
        .iter()
...
fn mermaid_prefilter_does_not_block_valid_compact_diagram_documents() {
    let request = ChatOutcomeArtifactRequest {
        ...
        deliverable_shape: ChatArtifactDeliverableShape::SingleFile,
        renderer: ChatRendererKind::Mermaid,
        presentation_surface: ChatPresentationSurface::SidePanel,
        persistence: ChatArtifactPersistenceMode::ArtifactScoped,
        execution_substrate: ChatExecutionSubstrate::ClientSandbox,
        workspace_recipe_id: None,

use super::*;
use ioi_api::runtime_harness::ConnectorGrounding;

#[test]
fn connector_grounding_is_carried_into_artifact_brief() {
    let mut brief = ChatArtifactBrief {
...
#[test]
fn chat_skill_query_is_structural_and_not_skill_name_routed() {
    ...
    let need = blueprint
        .skill_needs
        .iter()
        .find(|need| need.kind == ChatArtifactSkillNeedKind::VisualArtDirection)
        .cloned()
...
fn unrelated_lower_ranked_skill_does_not_change_primary_selection_order() {
    let primary = ChatArtifactSelectedSkill {
    ...
    let unrelated = ChatArtifactSelectedSkill {
        ...
        matched_need_kinds: vec![ChatArtifactSkillNeedKind::DataStorytelling],

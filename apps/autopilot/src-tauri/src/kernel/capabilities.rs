...
mod tests {
    ...
    fn test_extension_manifest(
        ...
        ExtensionManifestRecord {
            ...
            manifest_path: "/tmp/governed/family/extension-alpha/.codex-plugin/plugin.json"
                .to_string(),
    ...
    fn runtime_profile_fallback_entries_link_interactive_extension_to_concrete_runtime_members() {
        ...
        let backend_slug = backend
            ...
            .to_string();
        let model = runtime_profile_model_entry(&runtime, Some(&backend_slug))
            .expect("runtime model entry");
        let mut entries = vec![
    ...
    fn proposal_targets_skip_filesystem_skills_as_governing_targets() {
        ...
        assert!(ids.contains(&"skill_source:alpha".to_string()));
        assert!(
            !ids.contains(&"filesystem_skill:extension:alpha:skills/alpha/SKILL.md".to_string())
        );
    }
    ...
    fn filesystem_skill_subject_prefers_governing_family_targets_over_self() {
        ...
        let subject = snapshot
            ...
            .iter()
            .find(|entry| {
                entry.entry_id == "filesystem_skill:extension:alpha:skills/alpha/SKILL.md"
            })
            .expect("subject entry")
            .clone();

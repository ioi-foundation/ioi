...
pub struct CanonicalTraceBundle {
    ...
    #[serde(default)]
    pub projection_receipts: Vec<AgentEvent>,
    #[serde(default)]
    ...
    #[serde(default)]
    pub trace_authority: String,
    #[serde(default)]
    pub settlement_backed: bool,
    #[serde(default)]
...
struct CanonicalTraceBundleSource {
    ...
    projection_receipts: Vec<AgentEvent>,
    settlement_receipts: Vec<Value>,
    missing_settlement_refs: Vec<String>,
...
fn export_trace_bundle_impl(
    ...
    write_zip_json(
        &mut zip,
        "projection_receipts.json",
        &source.projection_receipts,
    )?;
    files.push("projection_receipts.json".to_string());

    write_zip_json(
        &mut zip,
        "settlement_receipts.json",
        &source.settlement_receipts,
    )?;
    files.push("settlement_receipts.json".to_string());

    write_zip_json(
        &mut zip,
        "missing_settlement_refs.json",
        &source.missing_settlement_refs,
    )?;
    files.push("missing_settlement_refs.json".to_string());
...
fn load_canonical_trace_bundle_source(
    ...
    let projection_receipts = events
        .iter()
        .filter(|event| event.event_type == EventType::Receipt)
        .cloned()
        .collect::<Vec<_>>();
    let settlement_receipts = Vec::<Value>::new();
    let missing_settlement_refs = if projection_receipts.is_empty() {
        Vec::new()
    } else {
        vec!["settlement_trace_loader_not_configured_for_chat_artifact_projection".to_string()]
    };
...
fn build_canonical_trace_bundle(
    ...
    let stats = TraceBundleStats {
        ...
        receipt_count: source.projection_receipts.len(),
    ...
    let settlement_backed =
        !source.settlement_receipts.is_empty() && source.missing_settlement_refs.is_empty();
    let mut evidence_tiers = Vec::new();
    if !source.projection_receipts.is_empty() {
        evidence_tiers.push("Projection".to_string());
        evidence_tiers.push("Runtime event receipt".to_string());
    }
    if !source.settlement_receipts.is_empty() {
        evidence_tiers.push("Settlement receipt".to_string());
    }
    if !source.missing_settlement_refs.is_empty() {
        evidence_tiers.push("Missing settlement".to_string());
    }
    evidence_tiers.sort();
    evidence_tiers.dedup();
    let trace_authority = if settlement_backed {
        "settlement".to_string()
    } else if !source.settlement_receipts.is_empty() {
        "partial_settlement".to_string()
    } else {
    ...
    CanonicalTraceBundle {
        ...
        projection_receipts: source.projection_receipts,
        settlement_receipts: source.settlement_receipts,
        missing_settlement_refs: source.missing_settlement_refs,
        evidence_tiers,
        trace_authority,
        settlement_backed,
...
fn compare_canonical_trace_bundles(
    ...
    changed_detail(
        &mut runtime_details,
        &mut runtime_changed_labels,
        "Projection receipt count",
        left.projection_receipts.len().to_string(),
        right.projection_receipts.len().to_string(),
    );
    changed_detail(
        &mut runtime_details,
        &mut runtime_changed_labels,
        "Settlement receipt count",
        left.settlement_receipts.len().to_string(),
        right.settlement_receipts.len().to_string(),
    );
    changed_detail(
        &mut runtime_details,
        &mut runtime_changed_labels,
    ...
    let runtime_section = TraceBundleDiffSection {
        ...
        summary: if runtime_changed {
            ...
            "Event counts and receipt tiers matched.".to_string()
        },
        left_value: Some(format!(
            "{} events · {} projection receipts · {} settlement receipts",
            left.events.len(),
            left.projection_receipts.len(),
            left.settlement_receipts.len()
        )),
        right_value: Some(format!(
            "{} events · {} projection receipts · {} settlement receipts",
            right.events.len(),
            right.projection_receipts.len(),
            right.settlement_receipts.len()
        )),
    ...
    let stats = vec![
        ...
        TraceBundleDiffStat {
            label: "Projection receipts".to_string(),
            left_value: left.projection_receipts.len().to_string(),
            right_value: right.projection_receipts.len().to_string(),
        },
        TraceBundleDiffStat {
            label: "Settlement receipts".to_string(),
            left_value: left.settlement_receipts.len().to_string(),
            right_value: right.settlement_receipts.len().to_string(),
        },
        TraceBundleDiffStat {

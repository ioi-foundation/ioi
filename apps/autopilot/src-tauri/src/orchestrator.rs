...
use ioi_scs::{FrameType, RetentionClass, SovereignContextStore}; // [FIX] Import RetentionClass
...
static GLOBAL_EXECUTION_CACHE: once_cell::sync::Lazy<Mutex<HashMap<String, ExecutionResult>>> =
...
pub fn append_artifact(
    ...
    if let Ok(mut store) = scs.lock() {
        let mut artifacts =
            read_latest_json::<Vec<Artifact>>(&store, &index_key).unwrap_or_default();
    ...
    F: Fn(GraphEvent) + Send + 'static,
{
    ...
    while let Some(node_id) = queue.pop() {
        ...
        if let Some(mut result) = result_opt {
            let output_val: Value = if let Some(data) = &result.data {
                data.clone()
            } else {
        ...
        .await
        {
            Ok(mut res) => {
                ...
                let output_val: Value = if let Some(data) = &res.data {
                    data.clone()
                } else {
                    json!({"raw": res.output})
                };

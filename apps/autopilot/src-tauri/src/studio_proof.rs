use crate::kernel::studio::{
    changed_paths_between_revisions, run_studio_current_task_turn_for_proof,
};
...
fn studio_proof_trace(message: impl AsRef<str>) {
    if env::var_os("IOI_STUDIO_PROOF_TRACE").is_some() {
...
struct ProofGenerationEvidence {
    ...
    revisions: Vec<StudioArtifactRevision>,
    active_revision_id: Option<String>,
    full_studio_path: bool,
}
...
fn run_turn(
    ...
    studio_proof_trace("run_turn:start");
    ...
    let workspace_root_base = state_root.join("studio-workspaces");
    ...
    studio_proof_trace("run_turn:before_current_task_turn");
    ...
    studio_proof_trace("run_turn:after_current_task_turn");
    save_task(state_root, &task)?;
    studio_proof_trace("run_turn:after_save_task");
    let evidence = materialize_current_task_output(&task, &memory_runtime, output, prompt)?;
    studio_proof_trace("run_turn:after_materialize_output");
    if json_output {
        println!(
            "{}",
    ...
    studio_proof_trace("run_turn:return");
    Ok(())
}
...
fn apply_revision_to_session(
    ...
    chat_session.navigator_nodes = chat_session
        ...
        .files
        .iter()
        .map(|file| crate::models::StudioArtifactNavigatorNode {
            id: file.path.clone(),
    ...
    chat_session.status = match chat_session.lifecycle_state {
        ...
        crate::models::StudioArtifactLifecycleState::Failed => "failed",
        crate::models::StudioArtifactLifecycleState::Draft => "draft",
        crate::models::StudioArtifactLifecycleState::Planned => "planned",
        crate::models::StudioArtifactLifecycleState::Materializing => "materializing",
        crate::models::StudioArtifactLifecycleState::Rendering => "rendering",
        crate::models::StudioArtifactLifecycleState::Implementing => "implementing",
...
fn parse_selected_targets(
    raw_targets: &[String],
) -> Result<Vec<StudioArtifactSelectionTarget>, String> {
    raw_targets
        .iter()
        .enumerate()
        .map(|(index, raw)| {
            serde_json::from_str::<StudioArtifactSelectionTarget>(raw).map_err(|error| {
                format!(

...
#[test]
fn live_mcp_tool_catalog_consumes_provider_catalog_without_tool_execution() {
    ...
    let tool_output = execute_workflow_harness_live_default_node(
        &tool_node,
        json!({
            "mode": "test",
    ...
    assert_eq!(
        tool_output.get("mockBinding").and_then(Value::as_bool),
        Some(false)
    );
    assert_eq!(
        tool_output.get("credentialReady").and_then(Value::as_bool),
        Some(true)
    );
    let catalog = tool_output
        .get("mcpToolCatalog")
        .expect("tool catalog payload should be attached");
    assert_eq!(
        catalog.get("schemaVersion").and_then(Value::as_str),
        Some("workflow.mcp-tool.catalog-read.v1")
    );
    assert_eq!(
        catalog.get("executionMode").and_then(Value::as_str),
    ...
    assert_eq!(
        catalog.get("providerCatalogLinked").and_then(Value::as_bool),
        Some(true)
    );
    assert_eq!(
        catalog.get("sideEffectsExecuted").and_then(Value::as_bool),
...
fn live_connector_catalog_describe_consumes_mcp_tool_catalog_without_connector_execution() {
    ...
    .expect("live read-only MCP tool catalog should execute");
    let connector_node = json!({
        "id": "connector-catalog",
        "type": "adapter",
        "name": "Connector catalog describe",
        "config": {
            "logic": {
                "connectorBinding": {
    ...
    let connector_output = execute_workflow_harness_live_default_node(
        &connector_node,
        json!({
            "mode": "test",
            "previousAuthorityOutput": tool_output,
            "mutation": false
        }),
        1,
    )
    .expect("live read-only connector catalog should execute");

    assert_eq!(
        connector_output.get("mockBinding").and_then(Value::as_bool),
        Some(false)
    );
    assert_eq!(
        connector_output
    ...
    let catalog = connector_output
        .get("connectorCatalog")
        .expect("connector catalog payload should be attached");
    assert_eq!(
        catalog.get("schemaVersion").and_then(Value::as_str),
        Some("workflow.connector.catalog-describe.v1")
    );
    assert_eq!(
        catalog.get("executionMode").and_then(Value::as_str),
    ...
    assert_eq!(
        catalog
            .get("connectorExecutionEnabled")
            .and_then(Value::as_bool),
        Some(false)
    );
    assert_eq!(
        catalog.get("externalRequestEnabled").and_then(Value::as_bool),
        Some(false)
    );
    assert!(catalog

...
fn workflow_native_browser_runtime_thread_events(
    ...
    let proposal_ref = format!("proposal_{}_browser_inspect", run_id);
    let action_ref = format!("action_{}_browser_inspect", run_id);
    ...
    let environment_selection = json!({
        ...
        "rejected_options": [
            ...
            },
            {
                "lane": "sandboxed_hosted",
                "session_mode": "hosted_sandbox",
                "reason": "This workflow run is local and read-only; sandbox isolation remains available for risky or reproducible tasks."
            }
        ],
        "reasons": [

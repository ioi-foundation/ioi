...
export function CodeBlock({
  ...
  return (
    <div
      className={`relative min-w-0 max-w-full overflow-hidden rounded-3xl border ${isDark ? "border-stone-800 bg-stone-950/85" : "border-stone-200 bg-stone-950 text-stone-100"}`}
    >
      <div
        className={`flex items-center justify-between border-b px-4 py-3 ${isDark ? "border-stone-800 bg-stone-950/95" : "border-stone-800/70 bg-stone-950/95"}`}
      >
      ...
      <pre className="min-w-0 max-w-full overflow-x-auto p-4 text-sm leading-6 text-stone-200">
        <code>{code}</code>
      </pre>
    </div>
  );
...
export function Table({
  ...
  return (
    <div
      className={`min-w-0 max-w-full overflow-hidden rounded-3xl border ${isDark ? "border-stone-800 bg-stone-950/70" : "border-stone-200 bg-white"}`}
    >
        ...
        {rows.map((row, rowIndex) => (
          ...
          >
            {row.map((cell, cellIndex) => (
              <div key={cellIndex} className="min-w-0 space-y-1.5">
                <dt
                  className={`text-xs font-medium uppercase tracking-[0.12em] ${isDark ? "text-stone-500" : "text-stone-500"}`}
                >
                  {headers[cellIndex] ?? `Field ${cellIndex + 1}`}
                </dt>
                <dd
                  className={`min-w-0 break-words text-sm leading-6 [overflow-wrap:anywhere] ${
                    cellIndex === 0
                      ? isDark
          ...
          >
            <tr>
              {headers.map((header) => (
                <th
                  key={header}
                  className={`border-b px-4 py-4 font-medium ${isDark ? "border-stone-800" : "border-stone-200"}`}
                >
                  {header}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row, rowIndex) => (
              <tr
                ...
                className={
                  isDark
                    ? "border-b border-stone-900/80 text-stone-300 last:border-b-0"
                    : "border-b border-stone-100 text-stone-700 last:border-b-0"
                }
              >
                {row.map((cell, cellIndex) => (
                  <td
                    key={cellIndex}
                    className={`px-4 py-4 align-top leading-6 ${cellIndex === 0 ? (isDark ? "font-medium text-stone-100" : "font-medium text-stone-900") : ""}`}
                  >
                    {cell}
                  </td>
                ))}
      ...
      {steps.map((step, index) => (
        ...
        >
          ...
          <div className="min-w-0 space-y-3">
            <div
              className={`text-base font-semibold tracking-tight ${isDark ? "text-stone-100" : "text-stone-900"}`}
            >
              {step.title}
            </div>
            {step.body ? (
              <p
                className={
                  isDark
                    ? "text-[15px] leading-7 text-stone-300/88"
                    : "text-[15px] leading-7 text-stone-700"
                }
              >
                {step.body}
              </p>
  ...
  return (
    <div className="hidden w-60 shrink-0 self-start pl-6 xl:block xl:sticky xl:top-8">
      <div className="space-y-4">
        <div
          className={`text-[15px] font-semibold leading-6 tracking-[-0.01em] ${isDark ? "text-stone-500" : "text-stone-500"}`}
        >
          On this page
        </div>
        <div
          className={`border-l ${isDark ? "border-stone-800" : "border-stone-200"}`}
        >
          {items.map((item) => {

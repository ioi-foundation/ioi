...
async function summarizeArtifactCase(
  ...
  return {
    ...
    fullStudioPath: options.fullStudioPath,
  };
}
...
export async function executeLiveCase(
  ...
  const evidence = runStudioProofJson(
    [
      "run-turn",
      "--state-root",
      stateRoot,
...
export async function summarizeLiveCase(
  ...
  if (evidence.fullStudioPath !== true) {
    classification = "blocked";
    contradiction ??= "Live case did not record full Studio-path provenance.";
  }
  ...
  if (evidence.fallbackUsed) {
    classification = "blocked";
    contradiction ??=
      "A fallback was surfaced as the primary artifact in the live Studio lane.";
  }
...
export async function buildLiveRevisionFlow(
  ...
  const compare = runStudioProofJson([
    "compare",
    "--state-root",
    refined.stateRoot,
    "--base-revision-id",
  ...
  const restoreEvidence = runStudioProofJson(
    [
      "restore",
      "--state-root",
      restoreStateRoot,
  ...
  const branchEvidence = runStudioProofJson(
    [
      "branch",
      "--state-root",
      branchStateRoot,
...
export async function runLiveStudioLane(options?: {
  ...
  if (selectedCases.length === 0) {
    return summarizeLiveStudioResults(
      liveCases,
      runtimeEndpoint,
  ...
  if (!runtimeEndpoint) {
    ...
    return {
      ...
      strongestContradiction:
        "No LOCAL_LLM_URL or AUTOPILOT_LOCAL_RUNTIME_URL is configured for the live Studio parity lane.",
      notes: [
        "The contract lane still ran and produced evidence, but it cannot certify desktop parity.",
        "The live Studio lane exercised the kernel-owned failure path and surfaced a blocked artifact instead of substituting mock output.",
      ],
      unavailableProof,
      evidenceRoot: liveEvidenceRoot,
  ...
  return summarizeLiveStudioResults(liveCases, runtimeEndpoint, true, unavailableProof);
}

...
const CLAUSES = [
  ...
  { id: "4", clause: "MCP is a transport: a tool arriving over MCP normalizes to the same contract; resources, prompts, elicitation, tasks and Apps normalize to their canonical owners or fail typed-unavailable", unit: "M01.4 · M01.10", checks: [app("check:mcp-transport-normalization"), bounded(rootScript("check:mcp-non-tool-normalization", ["--", "--drills"]), 15)], battery: { ...rootScript("mutate:mcp-non-tool-normalization"), cost: "minutes" } },
  ...
  { id: "8", clause: "The outward gateway is least privilege: one immutable versioned HypervisorMCPGatewayProfile binds subject, key, origin, use, refs, exposure hash, privacy, budget, rate, expiry, revocation and receipt obligations", unit: "M01.11", checks: [bounded(rootScript("check:hypervisor-mcp-gateway-profile", ["--", "--drills"]), 15)], battery: { ...rootScript("mutate:hypervisor-mcp-gateway-profile"), cost: "minutes" }, absences: [{ what: "the LIVE admission lane against a running daemon — a requirement resolved without issuing, a genesis profile admitted, a narrowing successor admitted under the same decision and a widening one refused without a fresh admission, and native-versus-MCP parity measured on a fresh idempotency namespace. The contracts are registered, the plane is mounted and the oracle and source are drilled here; what this clause does NOT yet execute is the isolated-daemon leg that proves the lane end to end", owner: "M01.11 slice 2c — the gate's full mode (R-220)" }] },
  ...
  // R-192: goal pursuit is the ioi.ai application's composition, so the task clause names what the task
  // handle must not become in this estate's own vocabulary rather than a plane the Hypervisor deleted.
  { id: "N5", negative: true, clause: "A resource URI cannot grant access, a prompt cannot become trusted instruction, elicitation cannot approve, an MCP task cannot become a run identity or receipt identity, an App cannot acquire host/runtime truth", unit: "M01.10", provenBy: "4" },
  // The mutation matrix is proven at the ORACLE by clause 8's battery (35 planted defects, each refused by
  // name) and at the SOURCE by the same gate's pins. What it does not yet do is drive those mutations
  // against a running daemon, which is the same absence clause 8 carries and is named once there rather
  // than restated here as though it were a second gap.
  { id: "N6", negative: true, clause: "Missing, unknown, cross-kind, unauthenticated, candidate-key/origin/basis, budget/rate/expiry, upstream-state and widened-successor gateway mutations refuse; no profile exposes a master surface, ambient context, raw secret or cross-tenant capability", unit: "M01.11", provenBy: "8" },

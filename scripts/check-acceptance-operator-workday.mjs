...
const CLAUSES = [
  ...
  { id: "9", clause: "Collective work is an artifact ecology, not agent theater: ioi.ai and Hypervisor show the same room, ancestry, definitions, installations, health, dependencies, caretaker coverage, leases, effects and receipts", unit: "M08.17", checks: [bounded(rootScript("check:collective-artifact-ecology-surface"), 40), app("mutate:collective-artifact-ecology-surface")], absences: [{ what: "THE ioi.ai HALF HAS NO SURFACE TO BE HALF OF: no Goal Space surface exists anywhere in this estate (`apps/ioi-ai/src/surfaces` holds three Slack files, `apps/ioi-ai/src/api` carries zero goal or collective routes, and the core taxonomy's 29 first-party registrations include no collective, rooms or ecology surface), so the cross-view parity this clause names is proven between the DEEP and EMBEDDED renderings of the one view that exists and the ioi.ai comparison is NAMED rather than constructed. ACC-9, this clause's other acceptance, additionally has no runner script at all", owner: "the ioi.ai application's own client, and ACC-9's runner (M08.17, R-224/R-226)" }] },
  ...
  // M08.16 (R-221). ACC-10 named this unit in its acceptance and carried no clause for it — not even a
  // typed absence — so a reader who assumed "re-type the absence" for both acceptances would have
  // shipped an unproven ACC-10 half. The row is authored here rather than re-typed.
  { id: "12", clause: "Connected is not authorized on the operator's own surface: the connected-access cockpit renders the ADMITTED connection's posture, reachability, revocation epoch and required next action rather than a credential-presence stamp, keeps the scopes a connector requested apart from the scopes a provider returned, keeps connection, grant, integration and provider-account actions distinct, and renders no sealed member", unit: "M08.16", checks: [app("check:connected-access-cockpit"), app("mutate:connected-access-cockpit")], absences: [{ what: "the embedded product-settings view and the advanced wallet view do not exist, so the three-way live comparison this clause implies runs over constructed views in the oracle; the live leg drives the ONE view that exists against a real daemon, a stub provider and a completed ceremony", owner: "M08.16 — M08's embedded settings surface and the advanced wallet view (R-221)" }] },
  ...
  { id: "N4", negative: true, clause: "No chat, board or artifact-ecology projection becomes room, runtime, installation, authority, health or evaluation truth", unit: "M08.17", provenBy: "9" },

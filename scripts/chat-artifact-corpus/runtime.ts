...
import type { CommandCapture, RendererKind, ChatRuntimeRuntimeEnv } from "./types";

let cachedArtifactCommandEnv: NodeJS.ProcessEnv | null = null;
let cachedAutoConfiguredRuntime: ChatRuntimeRuntimeEnv | null = null;
...
function buildChatRuntimeRuntimeEnv(
  availableModels: string[],
  productionPreferences: string[],
  acceptancePreferences: string[],
): ChatRuntimeRuntimeEnv | null {
...
function buildChatRuntimeRuntimeEnvFromModels(
  productionModel: string,
  acceptanceModel: string,
): ChatRuntimeRuntimeEnv {
  return {
    endpoint: DEFAULT_OLLAMA_CHAT_ENDPOINT,
    healthEndpoint: DEFAULT_OLLAMA_HEALTH_ENDPOINT,
    productionModel,
...
function chooseSingleModelLaneOverrideRuntime(
  ...
  if (model) {
    return buildChatRuntimeRuntimeEnvFromModels(model, model);
  }
...
export function chooseAvailableOllamaRuntime(
  ...
  return buildChatRuntimeRuntimeEnv(
    availableModels,
    PREFERRED_OLLAMA_PRODUCTION_MODELS,
    PREFERRED_OLLAMA_ACCEPTANCE_MODELS,
  );
...
function chooseAvailableOllamaContractRuntimeForRenderer(
  ...
  if (documentRenderer && rendererUsesFastDocumentProofLane(renderer)) {
    ...
    return buildChatRuntimeRuntimeEnvFromModels(productionModel, acceptanceModel);
  }

  return buildChatRuntimeRuntimeEnvFromModels(productionModel, productionModel);
}
...
export function detectLocalOllamaRuntime(): ChatRuntimeRuntimeEnv | null {
...
export function ensureChatRuntimeProofBinary() {
  if (
    spawnSync(
      process.platform === "win32" ? "where" : "test",
      process.platform === "win32" ? [chatArtifactProofBinary] : ["-x", chatArtifactProofBinary],
      { encoding: "utf8" },
    ).status === 0
  ) {
    return;
...
export function runChatRuntimeProofJson(
  args: string[],
  options?: {
    cwd?: string;
    allowFailure?: boolean;
  ...
  const capture = runCommand(chatArtifactProofBinary, [...args, "--json"], options);
  return JSON.parse(capture.stdout);
}
...
export function autoConfiguredRuntime(): ChatRuntimeRuntimeEnv | null {

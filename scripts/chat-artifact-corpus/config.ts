...
const configuredEvidenceDate =
  process.env.CHAT_ARTIFACT_EVIDENCE_DATE?.trim() ||
  new Date().toISOString().slice(0, 10);
export const cliBinary = path.join(repoRoot, "target", "debug", "cli");
export const chatArtifactProofBinary = path.join(
  repoRoot,
  "target",
  "debug",
  process.platform === "win32" ? "chat_artifact_proof.exe" : "chat_artifact_proof",
);
...
export const liveEvidenceRoot = path.join(evidenceRoot, "live-chat-artifact-lane");

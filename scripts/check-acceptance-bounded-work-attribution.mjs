...
const CLAUSES = [
  ...
  { id: "4", clause: "A GoalRun admits with its bindings: admitted state root, receipt obligations, source context and authority scopes are on the admitted record for the general surface", unit: "M04.4", checks: [bounded(app("check:goal-run-composition"), 40), bounded(app("mutate:goal-run-composition"), 40)] },
  ...
  { id: "9", clause: "Context is leased application state, not copied authority: GoalRun owns ContextCell, ContextLease and ContextHandoff revisions; restart reproduces the least-context view; handoff creates a candidate re-evaluated under receiver policy", unit: "M04.11", checks: [bounded(app("check:context-family-composition"), 40), bounded(app("mutate:context-family-composition"), 40)] },
  { id: "10", clause: "A composition instantiates only existing owners, and lineage outlives its creator: a daemon-admitted collective-resolution receipt (composed by ioi.ai; its closure root re-derived by registered invariant at the seam) freezes the exact dependency closure without registering a new profile owner; lifecycle.status = active satisfies no check; removal transfers no ownership", unit: "M04.12", checks: [bounded(app("check:collective-artifact-runtime-lifecycle"), 40), bounded(app("mutate:collective-artifact-runtime-lifecycle"), 40)] },
  ...
  // The native half is proven inside clause 9's composition gate (R-202): widening, stale, revoked,
  // wrong-recipient and unknown-holder reads refuse by name over the ioi.ai composition. The MCP half
  // stays a TYPED ABSENCE owned by M01.10/M01.11 until a tool reaches these families. provenBy now
  // reads clause 9's real outcomes (fixed 2026-09-19), so this row cannot be PASS over a red clause 9.
  { id: "N3", negative: true, clause: "No context handoff copies raw payload truth, executable/kernel state or authority; widening, stale, revoked, wrong-recipient and cross-tenant reads refuse identically through native and MCP paths", unit: "M04.11", provenBy: "9", absences: [absent("M01.10 · M01.11", "check:mcp-non-tool-normalization", "the MCP half of this matrix: no MCP tool reaches the context families today, so there is no MCP path on which to refuse anything; the MCP matrix arrives with the MCP gateway's own units")] },
  { id: "N4", negative: true, clause: "A resolved composition registers no new profile owner; an active artifact, chat, participant or surviving process is never execution or authority truth", unit: "M04.12", provenBy: "10" },

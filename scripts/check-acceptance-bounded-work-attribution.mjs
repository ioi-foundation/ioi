...
const CLAUSES = [
  ...
  // Both halves are TYPED ABSENCES rather than claims (2026-09-19, R-197 commit): the native gate went
  // with the daemon plane in S5-1 and returns with the S5-3 composition; the MCP matrix belongs to
  // M01.10/M01.11 when a tool reaches these families. Until 2026-09-19 this row said `provenBy: "9"`
  // over a clause whose scripts no longer existed, and the harness reported it PASS anyway.
  { id: "N3", negative: true, clause: "No context handoff copies raw payload truth, executable/kernel state or authority; widening, stale, revoked, wrong-recipient and cross-tenant reads refuse identically through native and MCP paths", unit: "M04.11", absences: [absent("M04.11", "check:context-lease-handoff-lifecycle", "the native half: the gate that proved widening, stale, revoked, wrong-recipient and cross-tenant reads refuse identically was deleted with the daemon plane in S5-1 and is re-authored over the ioi.ai composition in S5-3"), absent("M01.10 · M01.11", "check:mcp-non-tool-normalization", "the MCP half of this matrix: no MCP tool reaches the context families today, so there is no MCP path on which to refuse anything; the MCP matrix arrives with the MCP gateway's own units")] },

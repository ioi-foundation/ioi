...
const QUALIFICATION = { ...bounded(rootScript("check:machine-product-profile-qualification", ["--", "--drills"]), 15), allowsFixture: true };
const QUALIFICATION_BATTERY = { ...rootScript("mutate:machine-product-profile-qualification"), cost: "minutes" };
...
const CLAUSES = [
  ...
  { id: "10", clause: "Claims are scoped: separate offline certificates bind the exact hosted Workstation and attached-Infrastructure release/profile/backend matrices, unsupported cells, freshness and limitations (the registered certificate contract with its reads-down invariant; workstation_hosted_v1 and infrastructure_attached_v1 GENERATED from the tracked record set, regenerating their attested hashes, reading not_qualified on a simulated basis with their typed reasons, verified offline, independent, withdrawing on expiry and drift — M12.15, R-216)", unit: "M12.15", checks: [QUALIFICATION], battery: QUALIFICATION_BATTERY, scheduled: [{ what: "the hosted Workstation and attached-Infrastructure POSITIVE crossings: a live ordinary-OS backend on this host's KVM registered against the machine plane, and an independently implemented attached-estate backend, each driven through the same gate with its certificate reading qualified", prerequisite: "an ordinary-OS backend for the MACHINE plane with a `live` declaration (the environment plane's microVM is a different object) and an independently implemented attached-estate backend (IOI_ACC20_SOAK_EVIDENCE with hosted-backend-evidence.json and attached-backend-evidence.json)", ruling: "the journey's own text (simulated-only evidence validates the contract and never qualifies a public host or attached-estate matrix), R-121, R-139 and R-216: a missing backend or host blocks a RUN, never the unit" }] },
  { id: "N1", negative: true, clause: "A VM boot, hostile-guest test, downloadable binary, bootable image, generated dashboard, backend declaration or autonomy proof is not profile qualification (every refused evidence kind refused by kind and never entering the runs; a certificate relabelled qualified on a simulated basis refused by schema and verifier; the other profile's declaration refused as cross-profile evidence)", unit: "M12.15", provenBy: "10" },

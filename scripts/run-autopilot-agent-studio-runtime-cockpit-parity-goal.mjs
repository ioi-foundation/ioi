...
function checkPackageScripts() {
  ...
  return {
    id: "package:scripts",
    ok: missing.length === 0,
    summary: missing.length === 0 ? "Agent Studio runtime cockpit parity scripts are registered" : "Goal scripts are missing",
    evidence: { missing },
  };
}
...
function checkSource() {
  ...
  return {
    id: "source:agent-studio-runtime-cockpit-parity",
    ok: missing.length === 0 && forbidden.length === 0,
    summary:
      missing.length === 0 && forbidden.length === 0
        ? "Studio source contains runtime cockpit projections and daemon-backed controls"
        : "Studio source still has runtime cockpit gaps",
...
function preflightChecks() {
  ...
  const checks = [
    checkFile(MASTER_GUIDE, "Agent Studio runtime cockpit parity master guide"),
  ...
  return {
    schemaVersion: "ioi.autopilot-agent-studio-runtime-cockpit-parity.preflight.v1",
...
async function cleanupValidationProcesses({ outputDir, phase }) {
  ...
  const proof = {
    schemaVersion: "ioi.autopilot-agent-studio-runtime-cockpit-parity.process-cleanup.v1",
    phase,
...
function updateGuideStatus({ outputDir, proof, blocker = null }) {
  ...
  const status = proof?.targetStudioRuntimeCockpitAchieved ? "Achieved" : "Blocked";
  const lines = [
    "## Latest Validation",
    "",
    `Status: ${status}`,
    ...
    "",
    `Runtime cockpit summary: ${proof?.targetStudioRuntimeCockpitAchieved ? "model streaming, daemon tool proposal, policy lease denial, command output, native diff/hunk controls, diagnostics gate, receipts/replay, browser status, worker status, and stop/resume were validated through the Electron GUI." : "runtime cockpit validation is still blocked."}`,
    "",
...
async function runValidation(outputDir) {
  ...
  try {
    ...
    queueCommand(commands, "ioi.studio.open", { phase: "runtime-cockpit-parity" });
    ...
    const proof = {
      schemaVersion: "ioi.autopilot-agent-studio-runtime-cockpit-parity.proof.v1",
      ...
      remainingBlockers: cockpitAttributes.targetStudioRuntimeCockpitAchieved === "true" ? "none" : "Runtime cockpit root attribute did not report achieved after GUI interactions.",
    };
    if (!proof.targetStudioRuntimeCockpitAchieved) {
      partialProof = proof;
      throw new Error(`Runtime cockpit target not achieved: ${JSON.stringify(cockpitAttributes)}`);
    }
    partialProof = proof;
...
async function main() {
  ...
  console.log(JSON.stringify({
    schemaVersion: "ioi.autopilot-agent-studio-runtime-cockpit-parity.goal.v1",
    ok: true,
    evidenceDir: outputDir,
    proof,
  }, null, 2));

...
import { app, bounded, rootScript, runJourney } from "./lib/acceptance-journey.mjs";

const absent = (unit, check, what) => ({ what: `${what} — ${check} (${unit}) is To be authored`, owner: unit });

const CLAUSES = [
  { id: "1", clause: "Terms precede the crossing: CollaborationTerms with an exact accepted root; nothing crosses before every required party has accepted", unit: "M11.1", checks: [bounded(app("check:aiip-collaboration-discovery"), 40)] },
  { id: "2", clause: "Discovery is a policy-bound projection, not database access", unit: "M11.1", provenBy: "1" },
  { id: "3", clause: "The handoff carries refs, not tables: bounded signed handoffs and refs cross; each domain's truth stays local", unit: "M11.1", provenBy: "1" },
  ...
  { id: "5", clause: "Bindings are replaceable: A2A, MCP, directory, HTTP/RPC and chain/escrow are transports; no completion state, task state or registry entry is read as IOI verification, acceptance or authority", unit: "M11.3", checks: [bounded(app("check:aiip-binding-replaceability"), 40)] },
  { id: "6", clause: "Exit is portable and does not end the room: a receipted, policy-filtered ParticipantStateBundle verifies without hosted database access; a participant leaves with lineage, credit and dispute history intact", unit: "M11.2", checks: [bounded(app("check:aiip-portable-exit"), 40)] },
  { id: "7", clause: "Federated admission is declared and enforced, distinct from hosted admission, and unreachable without terms acceptance", unit: "M11.2", provenBy: "6" },
  ...
  { id: "N1", negative: true, clause: "No AIIP path is used inside one system_id", unit: "M11.1", provenBy: "1" },

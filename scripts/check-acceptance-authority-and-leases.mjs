...
const CLAUSES = [
  ...
  { id: "10", clause: "The wallet renders the exact signed facets: amount/denomination, deposit, selector, artifact/config hash, teardown policy and stage derive from the canonical challenge bytes", unit: "M03.9", checks: [{ ...bounded(rootScript("check:approval-card-facets", ["--", "--drills"]), 15), allowsFixture: true }, { ...bounded(rootScript("check:spend-approval-lane", ["--", "--drills"]), 15), allowsFixture: true }], battery: [{ ...rootScript("mutate:approval-card-facets"), cost: "minutes" }, { ...rootScript("mutate:spend-approval-lane"), cost: "minutes" }], cited: [{ label: "the alpha journey's approval card rendering the daemon's exact policy/request commitments for session execute and credential bind (deployment-local custody tier)", evidence: ALPHA_DEPLOYMENT_EVIDENCE }], absences: [{ what: "the graduated wallet app's handoff and the passkey step-up as custody tiers (the App lane that parks a provider operation landed as M08.11's check:spend-approval-lane, R-213), and a signed denial receipt (no deny act exists on the authority node or the fixture)", owner: "the graduated wallet app · the passkey tier · a follow-on slice of M03.9 (R-212)" }], scheduled: [{ what: "the session-execute and custody cards LIVE under the deployment-local tier — check:alpha-journey with IOI_ALPHA_JOURNEY_AUTHORITY=deployment", prerequisite: "a local model (Ollama) and the deployment-local authority node", ruling: "R-139 (2026-09-14) and R-212 (2026-09-20): a missing local model or authority node blocks that RUN, never the unit" }] },
  ...
  { id: "N6", negative: true, clause: "Provider-granted scopes, an active connection badge, an installed connector, credential presence or a prior provider success cannot mint action authority; asynchronous dependent cleanup cannot delay disconnect fencing", unit: "M03.16 · M08.16", checks: [app("check:provider-connection-lifecycle"), app("check:connected-access-cockpit"), app("mutate:connected-access-cockpit")] },
  { id: "E", clause: "Journey evidence the document names beyond the clauses: session authority, model-route authority and admission evidence", unit: "M03", checks: [app("check:session-authority"), app("check:model-route-authority"), rootScript("check:admission-evidence")], absences: [{ what: "the EMBEDDED and ADVANCED connected-access views: only the Hypervisor cockpit exists, and it is re-plumbed onto the connection plane and driven by check:connected-access-cockpit; there is no embedded product-settings view and no advanced wallet view, so the three-way live comparison the clause names runs over constructed views in the oracle and is named here", owner: "M08.16 — M08's embedded settings surface and the advanced wallet view (R-221)" }] },
];

...
const ALPHA_DEPLOYMENT_EVIDENCE = "docs/architecture/_meta/evidence/m12-alpha-journey-release-no-checkout-2026-09-10.v1.json";
...
const CLAUSES = [
  ...
  { id: "10", clause: "The wallet renders the exact signed facets: amount/denomination, deposit, selector, artifact/config hash, teardown policy and stage derive from the canonical challenge bytes", unit: "M03.9", cited: [{ label: "the alpha journey's approval card rendering the daemon's exact policy/request commitments for session execute and credential bind (deployment-local custody tier)", evidence: ALPHA_DEPLOYMENT_EVIDENCE }], absences: [absent("M03.9", "the byte-derived lease_request_facets rendering", "provider-operation facets (ceiling+denom, deposit, provider_selector, sdl_hash, teardown_policy, stage) rendered byte-derived from the 428 challenge and the wallet-app diff harness; no surface in either repository reads lease_request_facets today")] },
  ...
  { id: "14", clause: "Connected is not authorized: a replay-safe provider ceremony creates a versioned ProviderConnectionBinding and brokered credential relationship, not a grant; disconnect, provider revocation, expiry and rotation fence the next use; reconnect creates successor lineage", unit: "M03.16", absences: [absent("M03.16", "check:provider-connection-lifecycle", "ProviderConnectionCeremony/ProviderConnectionBinding are canon prose with no registered schema, no /v1/connections route, no SDK/CLI path")] },

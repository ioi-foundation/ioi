...
const SHIM_MARKERS = [
  "data-chat-normalized",
  "data-chat-view-switch-repair",
  "data-chat-rollover-repair",
];
...
const LANE_ALIASES = new Map([
  ["live-chat-artifact-lane", "live_chat_runtime"],
  ["contract-lane", "contract"],
  ["fixture-lane", "fixture"],
  ["root", "root"],
]);
const LANE_LABELS = new Map([
  ["live_chat_runtime", "Live Chat Runtime"],
  ["contract", "Contract"],
  ["fixture", "Fixture"],
  ["root", "Root"],
]);
...
function repeatedRunVariationFlow(summary) {
  return (
    ...
    summary?.lanes?.liveChatRuntime?.repeatedRunVariationFlow ??
...
function summarizeTotals(entries) {
  ...
  for (const entry of entries) {
    ...
    if (entry.fullChatRuntimePath) {
      totals.fullChatRuntimePathCount += 1;
    }
  ...
  return {
    ...
    proofPath:
      typeof summary?.proofPath === "string"
        ...
        : null,
    fullChatRuntimePath: summary?.fullChatRuntimePath === true,
...
export function collectChatArtifactBenchmarkSuite(options = {}) {
  const repoRoot =
    options.repoRoot ??
  ...
  return {
    ...
    metrics: {
      ...
      shimRequiredRate: metricReading({
        ...
        method: "share of executed benchmark cases that still relied on Chat runtime normalization repair shims",
...
export function collectChatArtifactCorpusIndex(options = {}) {
  const repoRoot =
    options.repoRoot ??
    path.resolve(path.dirname(new URL(import.meta.url).pathname), "..", "..");
  const evidenceRoot =
  ...
  return {
    ...
    auxiliaryCases,
    dateRoots,
    benchmarkSuite: collectChatArtifactBenchmarkSuite({
      repoRoot,
      evidenceRoot,
      cases,
      allEntries: entries,
...
export function writeChatArtifactCorpusIndex(options = {}) {
  const summary = collectChatArtifactCorpusIndex(options);
  const outputPath =

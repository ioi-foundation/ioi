...
test("TS harness fork activation contract records blocked and canary-validated paths", () => {
  ...
  assert.match(workflow, /makeHarnessDefaultRuntimeDispatchProof[\s\S]*authorityToolingReadOnlyLiveAttemptIds:[\s\S]*authority_tooling_mcp_provider_read_only[\s\S]*authority_tooling_mcp_tool_call_read_only[\s\S]*authority_tooling_tool_call_read_only[\s\S]*authority_tooling_connector_call_read_only[\s\S]*authority_tooling_wallet_capability_read_only[\s\S]*authorityToolingReadOnlyAuthorityCanaryReady: true[\s\S]*readOnlyAuthorityCanaryReady: true[\s\S]*mutationDeferredComponentKinds: authorityToolingMutationDeferredComponentKinds/);

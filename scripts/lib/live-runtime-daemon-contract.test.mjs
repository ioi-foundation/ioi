...
test("React Flow memory, authority/tooling, doctor, skill, hook, and package node contracts remain workflow-addressable", () => {
  ...
  const runtimeActionSchema = fs.readFileSync(
    path.join(root, "docs/architecture/_meta/schemas/runtime-action-schema.json"),
    "utf8",
  );
  ...
  const generatedRustActionSchema = fs.readFileSync(
    path.join(root, "crates/types/src/app/generated/runtime_action_schema.rs"),
    "utf8",
  );
  ...
  assert.match(workflowRuntimeControlNodes, /operator_override_approved/);
  assert.match(workflowRuntimeControlNodes, /approval_granted/);
  assert.match(workflowRuntimeControlNodes, /allow_conflicts/);

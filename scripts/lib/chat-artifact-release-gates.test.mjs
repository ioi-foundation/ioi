...
import {
  collectChatArtifactReleaseGates,
  collectChatArtifactReleaseGatesView,
  writeChatArtifactReleaseGates,
} from "./chat-artifact-release-gates.mjs";
...
test("release gates writer persists a report and exposes a summarized view", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-release-gates-"));
  try {
    ...
    const { reportPath, report } = writeChatArtifactReleaseGates({
      repoRoot: root,
      evidenceRoot,
    ...
    const view = collectChatArtifactReleaseGatesView({
      repoRoot: root,
      evidenceRoot,
      reportPath,
    });

...
const evidenceRoot =
  process.env.AUTOPILOT_STAGE5_STOP_CANCEL_RECOVER_LIVE_GUI_EVIDENCE_ROOT ||
  "docs/evidence/autopilot-agent-runtime-parity-plus/stage-5-stop-cancel-recover-stop-hook/live-gui-stop-cancel-recover";
...
async function createDaemonModelInvocationToken(endpoint) {
  const response = await fetch(`${endpoint}/api/v1/tokens`, {
    ...
    body: JSON.stringify({
      audience: "autopilot-stage5-stop-cancel-recover-live-gui",
      allowed: [
        "model.chat:*",
        "model.responses:*",
        "model.import:*",
...
function bridgeState(daemonEndpoint, runtimeModelRoute) {
  ...
  const endpointId = runtimeModelRoute.endpointId || "endpoint.stage5-stop-cancel-recover";
  return {
...
function makeRuntimeBridgeProofAdapter(runtimeCalls) {
  const state = {
    turnId: `turn_stage5_stop_cancel_recover_${Date.now().toString(36)}`,
    runId: `run_stage5_stop_cancel_recover_${Date.now().toString(36)}`,
  ...
  async function waitForResume(timeoutMs = 45_000) {
    if (state.resumeSeen) return true;
    await Promise.race([
      new Promise((resolve) => waiters.push(resolve)),
      wait(timeoutMs),
    ]);
    return state.resumeSeen;
  }
  return {
    bridgeId: "stage5-stop-cancel-recover-live-gui-bridge",
    async startThread(input) {
      runtimeCalls.push({ operation: "start_thread", input, atMs: Date.now() });
      return {
        session_id: "session_stage5_stop_cancel_recover",
        status: "active",
        source: "runtime_service",
        events: [
          {
            event_id: "evt_stage5_stop_cancel_recover_thread_started",
            event_kind: "thread.started",
            status: "completed",
            created_at: new Date().toISOString(),
            payload: {
              summary: "Stage 5 stop/cancel/recover runtime thread started.",
            },
          },
        ],
      };
    ...
    async submitTurn(input, options = {}) {
      state.submitStartedAtMs = Date.now();
      runtimeCalls.push({ operation: "submit_turn", input, atMs: state.submitStartedAtMs });
      const started = {
        event_id: "evt_stage5_stop_cancel_recover_turn_started",
        ...
        created_at: new Date().toISOString(),
        payload: {
          summary: "Stage 5 stop/cancel/recover model stream is active.",
        },
        receipt_refs: ["receipt_stage5_model_stream_started"],
      };
      options.onRuntimeEvent?.(started);
      await waitForResume();
      await wait(500);
      const completed = {
        event_id: "evt_stage5_stop_cancel_recover_turn_completed",
        ...
        created_at: new Date().toISOString(),
        payload: {
          summary: "Stage 5 stop/cancel/recover turn resumed and completed.",
        },
        receipt_refs: ["receipt_stage5_recovered_turn_completed"],
      };
      state.submitCompletedAtMs = Date.now();
      return {
        turn_id: state.turnId,
        run_id: state.runId,
        status: "completed",
        result: "Stage 5 stop/cancel/recover proof completed after operator stop and resume.",
        stop_reason: "runtime_bridge_completed_after_operator_resume",
    ...
    async controlThread(input) {
      const atMs = Date.now();
      const cleanup = input.action === "stop"
        ? {
            status: "completed",
            receipt_refs: ["receipt_stage5_process_tree_cleanup"],
            killed_process_trees: [
              { kind: "model_stream", status: "terminated" },
              { kind: "retained_shell", status: "terminated" },
              { kind: "browser_session", status: "terminated" },
              { kind: "computer_use", status: "terminated" },
            ],
          }
        : null;
      const result = {
        schema_version: "ioi.runtime.control-thread.stage5-live-gui.v1",
        action: input.action,
        status: input.action === "resume" ? "running" : "paused",
        cleanup,
        receipt_refs: cleanup?.receipt_refs ?? [`receipt_stage5_${input.action}_control`],
        input,
        controlled_at: new Date(atMs).toISOString(),
      };
      runtimeCalls.push({ operation: "control_thread", input, result, atMs });
...
async function seedRuntimeModelRoute(daemon) {
  const modelId = "stage5-stop-cancel-recover-model";
  const endpointId = "endpoint.stage5-stop-cancel-recover";
...
async function domEvidence(studioFrame) {
  return studioFrame.evaluate(() => {
    ...
    return {
      ...
      hasStopButton: document.querySelectorAll('[data-testid="studio-stop-icon"]').length > 0,
      hasResumeButton: document.querySelectorAll('[data-testid="studio-resume-icon"]').length > 0,
      hasInterruptedStatus: /interrupted/i.test(text),
      hasRuntimeStopAck: /Runtime stop control|control_thread stop acknowledged/i.test(text),
      hasRuntimeResumeAck: /Runtime resume control|control_thread resume acknowledged/i.test(text),
      hasFinalRecoveredAnswer:
        /completed after operator stop and resume|stop\/cancel\/recover proof completed|stop\/cancel\/recover turn resumed and completed/i.test(text),
    };
  });
}
...
async function runProof(outputDir) {
  ...
  const runtimeCalls = [];
  const runtimeBridge = makeRuntimeBridgeProofAdapter(runtimeCalls);
  const daemonStateDir = mkdtempSync(join(tmpdir(), "autopilot-stage5-stop-cancel-recover-daemon-"));
  ...
  try {
    ...
    userDataDir = mkdtempSync(join(tmpdir(), "autopilot-stage5-stop-cancel-recover-user-"));
    const extensionsDir = mkdtempSync(join(tmpdir(), "autopilot-stage5-stop-cancel-recover-ext-"));
    ...
    queueCommand(commands, "ioi.studio.exerciseStage5StopCancelRecoverLifecycle", {
      prompt,
      routeId: runtimeModelRoute.routeId,
      modelId: runtimeModelRoute.modelId,
    });
    await waitForPredicate(
      () => runtimeCalls.some((call) => call.operation === "submit_turn"),
      45_000,
      250,
    );
    const stopControl = await waitForPredicate(
      () => runtimeCalls.find((call) => call.operation === "control_thread" && call.input?.action === "stop"),
      45_000,
      250,
    );
    if (!stopControl) throw new Error("Studio stop did not route to runtime control_thread stop.");
    const resumeControl = await waitForPredicate(
      () => runtimeCalls.find((call) => call.operation === "control_thread" && call.input?.action === "resume"),
      45_000,
      250,
    );
    if (!resumeControl) throw new Error("Studio resume did not route to runtime control_thread resume.");
    const lifecycleRequest = await requireRequest(
      requests,
      (request) => request?.requestType === "studio.stage5StopCancelRecover.exercised",
      "studio.stage5StopCancelRecover.exercised",
      45_000,
    );
    ...
    await screenshot(page, outputDir, "stage5-stop-cancel-after-resume.png", screenshots);

    const chatStopRequest = await requireRequest(
      requests,
      (request) => request?.requestType === "chat.stop",
      "chat.stop",
      10_000,
    );
    const chatResumeRequest = await requireRequest(
      requests,
      (request) => request?.requestType === "chat.resume",
      "chat.resume",
      10_000,
    );

    const cleanupKinds = new Set(
      stopControl.result?.cleanup?.killed_process_trees?.map((entry) => entry.kind) ?? [],
    );
    const checks = {
      electronLaunched: true,
      studioOpened: true,
      submitTurnStarted: runtimeCalls.some((call) => call.operation === "submit_turn"),
      stopClickedDuringSubmit:
        runtimeBridge.state.submitStartedAtMs &&
        stopControl.atMs > runtimeBridge.state.submitStartedAtMs &&
        (!runtimeBridge.state.submitCompletedAtMs || stopControl.atMs < runtimeBridge.state.submitCompletedAtMs),
      runtimeControlStopObserved: Boolean(stopControl),
      runtimeControlResumeObserved: Boolean(resumeControl),
      stopUsedRuntimeSession: stopControl.input?.sessionId === "session_stage5_stop_cancel_recover",
      resumeUsedRuntimeSession: resumeControl.input?.sessionId === "session_stage5_stop_cancel_recover",
      processTreeCleanupReceipt:
        stopControl.result?.cleanup?.status === "completed" &&
        cleanupKinds.has("model_stream") &&
        cleanupKinds.has("retained_shell") &&
        cleanupKinds.has("browser_session") &&
        cleanupKinds.has("computer_use"),
      bridgeStopRequestObserved: Boolean(chatStopRequest),
      bridgeResumeRequestObserved: Boolean(chatResumeRequest),
      lifecycleProofRequestObserved: lifecycleRequest?.payload?.passed === true,
      lifecycleStopControlObserved: lifecycleRequest?.payload?.checks?.stopControlObserved === true,
      lifecycleResumeControlObserved: lifecycleRequest?.payload?.checks?.resumeControlObserved === true,
      domResumeStateVisible: Boolean(finalEvidence?.hasRuntimeResumeAck || finalEvidence?.hasFinalRecoveredAnswer),
      screenshotsCaptured: screenshots.length >= 2 && screenshots.every((entry) => entry.exists),
    };
    const passed = Object.values(checks).every(Boolean);
    const proof = {
      schemaVersion: "ioi.autopilot.stage5.stop-cancel-recover.live-gui-proof.v1",
      passed,
      ...
      screenshots,
      productLeakAudit: {
        rawRuntimePayloadLeakObserved: /IOI_RUNTIME_AGENT_SERVICE_BRIDGE_COMMAND|runtime-agent-service-bridge|session_stage5_stop_cancel_recover/.test(
    ...
    writeFileSync(join(outputDir, "runtime-calls.json"), `${JSON.stringify(runtimeCalls, null, 2)}\n`);
    ...
    if (!proof.passed) {
      throw new Error(`Stage 5 stop/cancel/recover live GUI proof failed: ${JSON.stringify(checks)}`);
    }
    return proof;
  } finally {
    writeFileSync(join(outputDir, "runtime-calls.json"), `${JSON.stringify(runtimeCalls, null, 2)}\n`);

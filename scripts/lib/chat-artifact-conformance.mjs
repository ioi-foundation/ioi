...
const DEFAULT_PRODUCTION_SOURCE_PATHS = [
  path.join("crates", "api", "src", "chat", "planning.rs"),
  path.join("crates", "api", "src", "chat", "generation", "mod.rs"),
...
export function collectChatArtifactConformanceReport(options = {}) {
  ...
  const checks = [
    buildCheck(
      ...
      benchmarkLeaks.length === 0
        ? "Production Chat runtime sources do not contain benchmark ids or case-binding ids."
        : "Production Chat runtime sources contain benchmark-specific ids.",
    ...
    buildCheck(
      ...
      skillNameLeaks.length === 0
        ? "Production Chat runtime sources do not route on retained concrete skill names."
        : "Production Chat runtime sources contain retained concrete skill names.",

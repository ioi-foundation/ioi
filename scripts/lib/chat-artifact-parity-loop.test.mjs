...
import {
  collectChatArtifactParitySnapshot,
  createEmptyChatArtifactParityLedger,
  loadChatArtifactParityLoopLedger,
  planChatArtifactParityIteration,
} from "./chat-artifact-parity-loop.mjs";
...
test("parity loop selects scaffold for html distinctness failure", () => {
  ...
  const receipt = planChatArtifactParityIteration({
    corpusSummary,
...
test("parity loop keeps improving changes and resets no-improvement streak", () => {
  ...
  const ledger = createEmptyChatArtifactParityLedger();
  ledger.receipts.push({
    createdAt: "2026-03-31T11:00:00.000Z",
    snapshot: collectChatArtifactParitySnapshot(previousCorpus),
...
test("parity loop stops after the configured no-improvement plateau", () => {
  const corpusSummary = buildCorpusSummary();
  const snapshot = collectChatArtifactParitySnapshot(corpusSummary);
  const ledger = createEmptyChatArtifactParityLedger({
...
test("parity loop anchors wall-clock budget to the first recorded intervention", () => {
  ...
  const firstReceipt = planChatArtifactParityIteration({
    corpusSummary,
    ledger,
    now: "2026-03-31T12:00:00.000Z",
  });
  ...
  ledger.receipts.push({
    createdAt: "2026-03-31T11:00:00.000Z",
    snapshot: collectChatArtifactParitySnapshot(corpusSummary),
    noImprovementStreak: 0,
  });

  const secondReceipt = planChatArtifactParityIteration({
    corpusSummary,
    ledger,
...
test("parity loop normalizes legacy live chat runtime lane receipts on load", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-parity-ledger-"));
  try {
          ...
          receipts: [
            {
              ...
              snapshot: {
                invariantFailures: [
                  {
                    id: "live_chat_runtime_lane",
                    label: "Live Chat Runtime lane",
                    summary: "Live Chat Runtime lane is not yet passing.",
                    family: "evidence_ux",
                    caseIds: [],
                  },
                ],
              ...
              decision: {
                kind: "continue",
                reason: "Address Live Chat Runtime lane.",
              },
            },
          ],
        },
    ...
    const { ledger } = loadChatArtifactParityLoopLedger({
      repoRoot: root,
      ledgerPath,
    });

    assert.equal(ledger.receipts[0].snapshot.invariantFailures[0].id, "live_chat_runtime");
    assert.equal(ledger.receipts[0].snapshot.invariantFailures[0].label, "Live Chat Runtime");
    assert.equal(ledger.receipts[0].weakestTarget.id, "live_chat_runtime");
    assert.equal(ledger.receipts[0].decision.reason, "Address Live Chat Runtime.");
  } finally {

  ...
  return {
    ...
    method:
      Array.isArray(checkIds) && checkIds.length > 0
        ...
        : "count of non-pass ChatRuntime artifact conformance checks",
...
export function collectChatArtifactReleaseGates(options = {}) {
...
export function writeChatArtifactReleaseGates(options = {}) {
  const repoRoot = options.repoRoot ?? process.cwd();
  const evidenceRoot =
    options.evidenceRoot ??
    path.join(repoRoot, "docs", "evidence", "chat-artifact-surface");
  const report = collectChatArtifactReleaseGates({
    ...options,
    repoRoot,
    evidenceRoot,
  });
...
function loadChatArtifactReleaseGates(options = {}) {
  const repoRoot = options.repoRoot ?? process.cwd();
  const evidenceRoot =
    options.evidenceRoot ??
    path.join(repoRoot, "docs", "evidence", "chat-artifact-surface");
  const reportPath =
    options.reportPath ?? path.join(evidenceRoot, DEFAULT_RELEASE_GATE_REPORT_PATH);
  const report = readJsonIfExists(reportPath) ?? collectChatArtifactReleaseGates(options);
  return { reportPath, report: deepClone(report) };
}

export function collectChatArtifactReleaseGatesView(options = {}) {
  const { reportPath, report } = loadChatArtifactReleaseGates(options);
  return {

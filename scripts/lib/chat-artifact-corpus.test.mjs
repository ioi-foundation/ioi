...
import {
  collectChatArtifactBenchmarkSuite,
  collectChatArtifactCorpusIndex,
  writeChatArtifactCorpusIndex,
} from "./chat-artifact-corpus.mjs";
...
test("collectChatArtifactCorpusIndex detects shim dependency and excludes auxiliary flows", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-corpus-"));
  try {
    const evidenceRoot = path.join(root, "docs", "evidence", "chat-artifact-surface");
    const primarySummaryPath = path.join(
      evidenceRoot,
      "2026-03-30",
      "live-chat-artifact-lane",
    ...
    fs.writeFileSync(
      ...
      "<!doctype html><main data-chat-normalized=\"true\">shimmed</main>",
    );
    fs.mkdirSync(auxiliaryArtifactDir, { recursive: true });
    fs.writeFileSync(path.join(auxiliaryArtifactDir, "hero.svg"), "<svg />");

    const summary = collectChatArtifactCorpusIndex({ repoRoot: root, evidenceRoot });
    ...
    assert.equal(
      ...
      "2026-03-30/live_chat_runtime/html-quantum-explainer-baseline",
    );
    assert.equal(
      summary.cases[0].summaryPath,
      "2026-03-30/live_chat_runtime/html-quantum-explainer-baseline/case-summary.json",
    );
...
test("collectChatArtifactBenchmarkSuite reports applied distillation gain when ledger evidence exists", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-distillation-"));
  try {
    const evidenceRoot = path.join(root, "docs", "evidence", "chat-artifact-surface");
    ...
    const suite = collectChatArtifactBenchmarkSuite({
      repoRoot: root,
      evidenceRoot,
...
test("collectChatArtifactBenchmarkSuite derives screenshot and responsiveness metrics from render evaluation", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-render-metrics-"));
  try {
    const evidenceRoot = path.join(root, "docs", "evidence", "chat-artifact-surface");
    const summaryPath = path.join(
      evidenceRoot,
...
test("writeChatArtifactCorpusIndex persists the aggregated summary", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-corpus-write-"));
  try {
    const evidenceRoot = path.join(root, "docs", "evidence", "chat-artifact-surface");
    const summaryPath = path.join(
      evidenceRoot,
    ...
    const { summary } = writeChatArtifactCorpusIndex({
      repoRoot: root,
      evidenceRoot,
      outputPath,
    });
...
test("collectChatArtifactBenchmarkSuite joins the catalog to retained evidence and pairwise arena data", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-benchmark-suite-"));
  try {
    const evidenceRoot = path.join(root, "docs", "evidence", "chat-artifact-surface");
    const summaryPath = path.join(
      evidenceRoot,
...
test("collectChatArtifactCorpusIndex derives variance across retained repeated runs", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-repeat-variance-"));
  try {
    const evidenceRoot = path.join(root, "docs", "evidence", "chat-artifact-surface");

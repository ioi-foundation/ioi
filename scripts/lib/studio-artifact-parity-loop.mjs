...
function normalizeHistoricalLabel(value) {
  ...
  return value.replace(/\bLive Studio lane\b/g, "Live Studio");
}
...
function normalizeInvariantFailure(failure) {
  ...
  const normalizedId = failure.id === "live_studio_lane" ? "live_studio" : failure.id;
  const normalizedLabel =
    failure.id === "live_studio_lane" || failure.label === "Live Studio lane"
      ? "Live Studio"
...
function normalizeWeakestTarget(target) {
  ...
  return {
    ...target,
    id: target.id === "live_studio_lane" ? "live_studio" : target.id,
    label:
      target.id === "live_studio_lane" || target.label === "Live Studio lane"
        ? "Live Studio"
...
function collectInvariantFailures(corpusSummary) {
  ...
  if (corpusSummary?.lanes?.liveStudio?.status !== "pass") {
    failures.push({
      id: "live_studio",
      label: "Live Studio",
      summary:
        corpusSummary?.lanes?.liveStudio?.strongestContradiction ??
        "Live Studio is not yet passing.",
      family: "evidence_ux",
      caseIds: Array.isArray(corpusSummary?.lanes?.liveStudio?.cases)
        ? corpusSummary.lanes.liveStudio.cases.map((entry) => entry.id).filter(Boolean)
        : [],
    });
  }
...
export function compareStudioArtifactParitySnapshots(
  previousSnapshot,
  currentSnapshot,
) {
...
export function selectStudioArtifactInterventionFamily(snapshot) {
...
export function createEmptyStudioArtifactParityLedger(options = {}) {
  return {
    version: 1,
...
export function loadStudioArtifactParityLoopLedger(options = {}) {
...
export function loadStudioArtifactParityCorpusSummary(options = {}) {
  const repoRoot = options.repoRoot ?? process.cwd();
  const evidenceRoot =
    options.evidenceRoot ??
    path.join(repoRoot, "docs", "evidence", "studio-artifact-surface");
...
export function planStudioArtifactParityIteration(options = {}) {
  const nowIso =
  ...
  const snapshot = collectStudioArtifactParitySnapshot(options.corpusSummary, {
    thresholds,
  });
  ...
  const comparison = previousReceipt?.snapshot
    ? compareStudioArtifactParitySnapshots(previousReceipt.snapshot, snapshot)
    : null;
  ...
  const interventionFamily = selectStudioArtifactInterventionFamily(snapshot);
...
export function writeStudioArtifactParityLoopLedger(options = {}) {
  const { ledgerPath, ledger } = loadStudioArtifactParityLoopLedger(options);
...
export function collectStudioArtifactParityLoopView(options = {}) {
  const repoRoot = options.repoRoot ?? process.cwd();
  const evidenceRoot =
    options.evidenceRoot ??
    path.join(repoRoot, "docs", "evidence", "studio-artifact-surface");
  const { corpusSummary, summaryPath } = loadStudioArtifactParityCorpusSummary({
    repoRoot,
    evidenceRoot,
  ...
  const { ledgerPath, ledger } = loadStudioArtifactParityLoopLedger({
    repoRoot,
    evidenceRoot,
    ledgerPath: options.ledgerPath,
  });
  const currentPlan = planStudioArtifactParityIteration({
    corpusSummary,
    ledger,
    budgets: options.budgets,
    thresholds: options.thresholds,

...
const evidenceRoot =
  process.env.AUTOPILOT_SESSION_BRAIN_LIVE_GUI_EVIDENCE_ROOT ||
  "docs/evidence/autopilot-agent-runtime-parity-plus/stage-1-trajectory-brain/live-gui-session-brain";
...
async function createDaemonModelInvocationToken(endpoint) {
  const response = await fetch(`${endpoint}/v1/model-mount/tokens`, {
    ...
    body: JSON.stringify({
      audience: "autopilot-session-brain-live-gui",
      allowed: [
        "model.chat:*",
        "model.responses:*",
        "model.embeddings:*",
      ...
      denied: ["connector.*"],
      source: "session-brain-live-gui-proof",
    }),
  });
...
async function sessionBrainDomEvidence(frame) {
  const panel = frame.locator('[data-testid="studio-session-brain-panel"]').first();
  const rowLocator = panel.locator('[data-testid="studio-session-brain-artifact-row"]');
  const rows = await rowLocator.evaluateAll((nodes) =>
    nodes.map((node) => ({
      kind: node.getAttribute("data-brain-artifact-kind"),
      status: node.getAttribute("data-brain-artifact-status"),
  ...
  return {
    panel: {
      ...
      implementationPlan: await panel.getAttribute("data-brain-implementation-plan-observed").catch(() => null),
      taskChecklist: await panel.getAttribute("data-brain-task-checklist-observed").catch(() => null),
      walkthrough: await panel.getAttribute("data-brain-walkthrough-observed").catch(() => null),
      scratchRefs: await panel.getAttribute("data-brain-scratch-refs-observed").catch(() => null),
      artifactRefs: await panel.getAttribute("data-brain-artifact-refs-observed").catch(() => null),
      replayCursor: await panel.getAttribute("data-brain-replay-cursor-observed").catch(() => null),
      brainOutsideWorkspace: await panel.getAttribute("data-brain-outside-workspace").catch(() => null),
      readOnlyAuditMode: await panel.getAttribute("data-brain-read-only-audit-mode").catch(() => null),
...
function domReady(evidence) {
  ...
  return (
    ...
    evidence.panel.implementationPlan === "true" &&
    evidence.panel.taskChecklist === "true" &&
    evidence.panel.walkthrough === "true" &&
    evidence.panel.scratchRefs === "true" &&
    evidence.panel.artifactRefs === "true" &&
    evidence.panel.replayCursor === "true" &&
    evidence.panel.brainOutsideWorkspace === "true" &&
    evidence.panel.readOnlyAuditMode === "true" &&
    evidence.panel.traceLinkCount > 0 &&
    evidence.panel.verifiedBadgeCount > 0 &&
    rowKinds.has("implementation_plan") &&
    rowKinds.has("task") &&
    rowKinds.has("walkthrough") &&
    rowKinds.has("scratch") &&
...
async function runProof(outputDir) {
  ...
  const daemonStateDir = mkdtempSync(join(tmpdir(), "autopilot-session-brain-gui-daemon-"));
  ...
  try {
    ...
    userDataDir = mkdtempSync(join(tmpdir(), "autopilot-session-brain-gui-user-"));
    const extensionsDir = mkdtempSync(join(tmpdir(), "autopilot-session-brain-gui-ext-"));
    ...
    queueCommand(commands, "ioi.studio.exerciseSessionBrainLifecycle", {});
    const lifecycleRequest = await requireRequest(
      requests,
      (request) => request?.requestType === "studio.sessionBrainLifecycle.exercised",
    ...
    if (!drawerExpanded) throw new Error("Studio utility drawer did not expand for session brain proof.");

    const domEvidence = await waitForPredicate(async () => {
      try {
        studioFrame = await findFrameWithTestId(page, "agent-studio-operational-chat", 1000);
        const evidence = await sessionBrainDomEvidence(studioFrame);
    ...
    if (!domEvidence) {
      const latest = await sessionBrainDomEvidence(studioFrame).catch((error) => ({ error: String(error) }));
      throw new Error(`Session brain panel did not hydrate in the live GUI: ${JSON.stringify(latest)}`);
    }
    await screenshot(page, outputDir, "studio-session-brain.png", screenshots);

    await studioFrame.locator('[data-testid="studio-session-brain-panel"] [data-testid="studio-view-trace-link"]').first().click();
    const traceRequest = await requireRequest(
      requests,
      (request) =>
        request?.requestType === "runs.open" &&
        (request?.traceTarget?.kind || request?.payload?.traceTarget?.kind) === "session.brain",
      "runs.open session brain trace target",
    );
    await screenshot(page, outputDir, "studio-session-brain-trace-link.png", screenshots);
    ...
    const checks = {
      ...
      domPanelReady: domReady(domEvidence),
    ...
    const proof = {
      schemaVersion: "ioi.autopilot.stage1.session-brain-live-gui-proof.v1",
    ...
    if (!proof.passed) {
      throw new Error(`Session brain live GUI proof failed checks: ${JSON.stringify(checks)}`);
    }
    return proof;
  } finally {
    writeFileSync(join(outputDir, "bridge-requests.json"), `${JSON.stringify(requests, null, 2)}\n`);

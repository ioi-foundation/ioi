...
import { collectStudioArtifactCorpusIndex } from "./studio-artifact-corpus.mjs";
...
export function collectStudioArtifactArenaLedger(options = {}) {
  ...
  const corpusSummary =
    ...
    (fs.existsSync(summaryPath)
      ...
      : collectStudioArtifactCorpusIndex({ repoRoot, evidenceRoot }));
...
export function writeStudioArtifactArenaLedger(options = {}) {
  const repoRoot = options.repoRoot ?? process.cwd();
  const evidenceRoot =
    options.evidenceRoot ??
    path.join(repoRoot, "docs", "evidence", "studio-artifact-surface");
  const ledger = collectStudioArtifactArenaLedger({
    ...options,
    repoRoot,
    evidenceRoot,
  });
...
function loadStudioArtifactArenaLedger(options = {}) {
  const repoRoot = options.repoRoot ?? process.cwd();
  const evidenceRoot =
    options.evidenceRoot ??
    path.join(repoRoot, "docs", "evidence", "studio-artifact-surface");
...
export function collectStudioArtifactArenaView(options = {}) {
  const repoRoot = options.repoRoot ?? process.cwd();
  const evidenceRoot =
    options.evidenceRoot ??
    path.join(repoRoot, "docs", "evidence", "studio-artifact-surface");
  const { ledgerPath, ledger } = loadStudioArtifactArenaLedger({
    repoRoot,
    evidenceRoot,

...
import { collectChatArtifactConformanceReport } from "./chat-artifact-conformance.mjs";
...
test("conformance report flags benchmark ids and retained skill names in production sources", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chat-artifact-conformance-"));
  try {
    ...
    writeFile(
      path.join(root, "crates", "api", "src", "chat", "planning.rs"),
    ...
    const report = collectChatArtifactConformanceReport({
      repoRoot: root,
      evidenceRoot,
      corpusSummary,
      benchmarkCatalog,

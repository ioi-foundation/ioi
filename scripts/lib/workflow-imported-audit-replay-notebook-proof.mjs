...
const events = importedPanels.map((panel, index) => ({
  ...
  receiptRefs: panel.receiptRefs ?? [`receipt:stage57:panel:${index + 1}`],
  artifactRefs: [`artifact:stage57:panel:${index + 1}`],
  policyDecisionRefs: ["policy:stage57:historical_only"],
}));
const notebook = buildWorkflowSignedReplayNotebook({
  events: [
    ...events,
    {
      ...
      receiptRefs: ["receipt:stage57:snapshot"],
      artifactRefs: ["artifact:stage57:snapshot"],
  ...
  snapshots: [
    {
      ...
      changedFiles: [{ path: auditPath }],
      fileCount: 1,
      receiptRefs: ["receipt:stage57:snapshot-list"],
      artifactRefs: ["artifact:stage57:snapshot-list"],
    },
  ],
  restoreResults: [
    {
      ...
      receiptRefs: ["receipt:stage57:restore-preview"],
      artifactRefs: ["artifact:stage57:restore-preview"],
      rollbackRefs: [snapshotId],
      policyDecisionRefs: ["policy:stage57:restore-preview"],
    },
    {
      ...
      receiptRefs: ["receipt:stage57:restore-apply-blocked"],
      rollbackRefs: [snapshotId],
      policyDecisionRefs: ["policy:stage57:restore-apply-blocked"],
    },
  ],
});
...
const proof = {
  ...
  checks: {
    signedReplayBuilderUsed: notebook.schemaVersion === "ioi.workflow.signed-replay-notebook.v1",
    ...
    restoreApplyBlocked: notebook.restoreApplyBlockedCount === 1,
    tamperUpdateDenied:

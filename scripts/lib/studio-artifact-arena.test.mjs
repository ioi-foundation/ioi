...
import {
  collectStudioArtifactArenaLedger,
  collectStudioArtifactArenaView,
  writeStudioArtifactArenaLedger,
} from "./studio-artifact-arena.mjs";
...
test("arena writer persists a ledger that the arena view can surface", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "studio-artifact-arena-"));
  try {
    ...
    const { ledgerPath, ledger } = writeStudioArtifactArenaLedger({
      repoRoot: root,
      evidenceRoot,
    ...
    const view = collectStudioArtifactArenaView({
      repoRoot: root,
      evidenceRoot,
      ledgerPath,
    });

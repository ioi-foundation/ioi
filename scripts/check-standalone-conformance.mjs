...
async function drills(profileLoaded) {
  ...
  // One stable name for both hosts: the refusal half is ASSERTED where namespaces exist and typed
  // absent where they do not; the branch taken is in the detail, never in the name (a name that
  // varies by host moves the floor's names sha between hosts, which the floors gate reports).
  ok("the refusal half is typed by the host: where unprivileged namespaces exist the KERNEL refused every attempt to the non-loopback host (ENETUNREACH, before any packet existed); where the host refuses namespaces the attempt is recorded and the refusal half is typed absent, not claimed", isolation.network_namespace ? reachAttempts.length >= 1 && reachAttempts.every((a) => a.errno === "ENETUNREACH") : true, `${isolation.network_namespace ? "refused by the kernel" : "recorded only (namespaces unavailable)"} · ${reachAttempts.map((a) => a.errno).join(",")}`);

...
function runReceipts() {
  ...
  assertCheck(
    result,
    "runtime-subagent-state-storage-write-rust-admitted",
    /RUNTIME_SUBAGENT_STATE_COMMIT_SCHEMA_VERSION/.test(agentgresAdmissionCore) &&
      /RuntimeSubagentStateCommitRequest/.test(agentgresAdmissionCore) &&
      /RuntimeSubagentStateCommitRecord/.test(agentgresAdmissionCore) &&
      ...
      /rust_agentgres_runtime_subagent_state_commit_command/.test(bridgeModule) &&
      ...
      /"subagents\/subagent_1\.json"/.test(
        read("packages/runtime-daemon/src/threads/thread-persistence.test.mjs"),
      ),
    [
      "crates/services/src/agentic/runtime/kernel/agentgres_admission.rs",
    ...
    "Phase 5/11 is pending: subagent lifecycle records must be persisted through Rust Agentgres storage admission instead of direct JS file writes",
  );
  assertCheck(
    result,
...
function runCompositor() {
  ...
  assertCheck(
    ...
    !/legacy_event_(?:id|type)/.test(
      ...
      runtimeBranchPolicyPayloadSummaryBlock.length > 0 &&
      /event_kind:\s*event\.data\?\.event_kind \?\? "BranchPolicyDecision"/.test(
        runtimeBranchPolicyPayloadSummaryBlock,
      ) &&
      /policy_id:\s*event\.data\?\.policy_id \?\? null/.test(
        runtimeBranchPolicyPayloadSummaryBlock,
      ) &&
      /repository_context_id:\s*event\.data\?\.repository_context_id \?\? null/.test(
        runtimeBranchPolicyPayloadSummaryBlock,
      ) &&
      /mutation_allowed:\s*Boolean\(event\.data\?\.mutation_allowed\)/.test(
        runtimeBranchPolicyPayloadSummaryBlock,
      ) &&
      ...
      runtimeIssueContextPayloadSummaryBlock.length > 0 &&
      /event_kind:\s*event\.data\?\.event_kind \?\? "IssueContext"/.test(
        runtimeIssueContextPayloadSummaryBlock,
      ) &&
      /context_id:\s*event\.data\?\.context_id \?\? null/.test(
        runtimeIssueContextPayloadSummaryBlock,
      ) &&
      /github_context_id:\s*event\.data\?\.github_context_id \?\? null/.test(
        runtimeIssueContextPayloadSummaryBlock,
      ) &&
      /issue_number:\s*event\.data\?\.issue_number \?\? null/.test(
        runtimeIssueContextPayloadSummaryBlock,
      ) &&
      ...
      runtimeGithubPrCreatePlanPayloadSummaryBlock.length > 0 &&
      /event_kind:\s*event\.data\?\.event_kind \?\? "GitHubPrCreatePlan"/.test(
        runtimeGithubPrCreatePlanPayloadSummaryBlock,
      ) &&
      /plan_id:\s*event\.data\?\.plan_id \?\? null/.test(
        runtimeGithubPrCreatePlanPayloadSummaryBlock,
      ) &&
      /request_payload_hash:\s*event\.data\?\.request\?\.payload_hash \?\? null/.test(
        runtimeGithubPrCreatePlanPayloadSummaryBlock,
      ) &&
      /required_authority_scopes:\s*normalizeArray\(event\.data\?\.authority\?\.required_scopes\)/.test(
        runtimeGithubPrCreatePlanPayloadSummaryBlock,
      ) &&
      !/event\.data\?\.(?:eventKind|planId|repositoryContextId|branchPolicyId|githubContextId|issueContextId|prAttemptId|reviewGateId|dryRun|toolName|repoFullName|baseBranch|headBranch|issueNumber|reviewGateStatus|reviewSatisfied|mutationAttempted|mutationExecuted|networkLookupPerformed|workflowNodeId)\b/.test(
        runtimeGithubPrCreatePlanPayloadSummaryBlock,
      ) &&
      !/event\.data\?\.request\?\.(?:payloadHash|bodyIncluded|tokenIncluded)\b/.test(
        runtimeGithubPrCreatePlanPayloadSummaryBlock,
      ) &&
      !/event\.data\?\.authority\?\.(?:requiredScopes|missingScopes|scopeGranted)\b/.test(
        runtimeGithubPrCreatePlanPayloadSummaryBlock,
      ...
      runtimeBranchPolicyEventProducerBlock.length > 0 &&
      /event_kind:\s*"BranchPolicyDecision"/.test(runtimeBranchPolicyEventProducerBlock) &&
      /policy_id:\s*branchPolicy\.policyId \?\? null/.test(
        runtimeBranchPolicyEventProducerBlock,
      ) &&
      /repository_context_id:\s*branchPolicy\.repositoryContextId \?\? null/.test(
        runtimeBranchPolicyEventProducerBlock,
      ) &&
      /workflow_node_id:\s*"runtime\.branch-policy"/.test(
        runtimeBranchPolicyEventProducerBlock,
      ) &&
      ...
      runtimeIssueContextEventProducerBlock.length > 0 &&
      /event_kind:\s*"IssueContext"/.test(runtimeIssueContextEventProducerBlock) &&
      /context_id:\s*issueContext\.contextId \?\? null/.test(
        runtimeIssueContextEventProducerBlock,
      ) &&
      /github_context_id:\s*issueContext\.githubContextId \?\? null/.test(
        runtimeIssueContextEventProducerBlock,
      ) &&
      /workflow_node_id:\s*"runtime\.issue-context"/.test(
        runtimeIssueContextEventProducerBlock,
      ) &&
      ...
      runtimeGithubPrCreatePlanEventProducerBlock.length > 0 &&
      /event_kind:\s*"GitHubPrCreatePlan"/.test(runtimeGithubPrCreatePlanEventProducerBlock) &&
      /plan_id:\s*githubPrCreatePlan\.planId \?\? null/.test(
        runtimeGithubPrCreatePlanEventProducerBlock,
      ) &&
      /payload_hash:\s*githubPrCreatePlan\.request\?\.payloadHash \?\? null/.test(
        runtimeGithubPrCreatePlanEventProducerBlock,
      ) &&
      /required_scopes:\s*normalizeArray\(githubPrCreatePlan\.authority\?\.requiredScopes\)/.test(
        runtimeGithubPrCreatePlanEventProducerBlock,
      ) &&
      /workflow_node_id:\s*"runtime\.github-pr-create"/.test(
        runtimeGithubPrCreatePlanEventProducerBlock,
      ) &&

...
BACKEND_START="${HYPERVISOR_OLLAMA_OPENAI_BACKEND_START:-${HYPERVISOR_OLLAMA_OPENAI_BACKEND_START:-}}"

EIGHT_GB_CLASS_MAX_MIB=9000

detect_nvidia_total_memory_mib() {
  if [ -n "${HYPERVISOR_LOCAL_GPU_TOTAL_MEMORY_MIB:-}" ]; then
    printf '%s\n' "${HYPERVISOR_LOCAL_GPU_TOTAL_MEMORY_MIB}"
    return 0
  fi
...
resolve_hardware_profile() {
  if [ -n "${HYPERVISOR_LOCAL_HARDWARE_PROFILE:-}" ]; then
    printf '%s\n' "${HYPERVISOR_LOCAL_HARDWARE_PROFILE}"
    return 0
  fi
...
warm_models() {
  ...
  if [ "$#" -eq 0 ]; then
    set -- \
      "${OLLAMA_DEFAULT_MODEL:-${HYPERVISOR_LOCAL_RUNTIME_MODEL:-}}" \
      "${OLLAMA_ROUTING_MODEL:-${HYPERVISOR_CHAT_ARTIFACT_ROUTING_RUNTIME_MODEL:-}}" \
      "${OLLAMA_ACCEPTANCE_MODEL:-${HYPERVISOR_ACCEPTANCE_RUNTIME_MODEL:-}}" \
      "${OLLAMA_EMBEDDING_MODEL:-${HYPERVISOR_LOCAL_EMBEDDING_MODEL:-}}"
  fi

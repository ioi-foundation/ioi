...
const PROCESS_PATTERN = "/tmp/autopilot-agent-studio-chat-hardening-user-";
...
function checkPackageScripts() {
  ...
  const required = [
    "goal:autopilot-agent-studio-chat-ux-hardening",
    "goal:autopilot-agent-studio-chat-ux-hardening:run",
  ];
  const missing = required.filter((script) => !scripts[script]);
  return {
    id: "package:scripts",
    ok: missing.length === 0,
    summary: missing.length === 0 ? "Agent Studio chat UX hardening scripts are registered" : "Goal scripts are missing",
    evidence: { missing },
  };
}
...
function checkSource() {
  ...
  return {
    id: "source:agent-studio-chat-ux-hardening",
...
function preflightChecks() {
  ...
  const checks = [
    checkFile(MASTER_GUIDE, "Agent Studio chat UX hardening master guide"),
  ...
  return {
    schemaVersion: "ioi.autopilot-agent-studio-chat-ux-hardening.preflight.v1",
...
async function createDaemonModelInvocationToken(endpoint) {
  const response = await fetch(`${endpoint}/api/v1/tokens`, {
    ...
    body: JSON.stringify({
      ...
      source: "agent-studio-chat-ux-hardening",
    }),
  });
...
async function latestAssistantText(page) {
  return withStudioFrame(page, async (frame) => {
    const paragraphs = frame.locator('[data-testid="studio-assistant-answer-card"] p');
...
async function submitPrompt(page, requests, prompt, mode = "button", timingPath = null, options = {}) {
  ...
  if (streamRequired && !streamProbe?.streamText) {
  ...
  markPromptPhase(timing, "stream-probe", phaseStartedAtMs, { streamRequired, observed: Boolean(streamProbe?.streamText) });
  phaseStartedAtMs = Date.now();
  const completed = await waitForPredicate(async () => {
    try {
      return await withStudioFrame(page, async (frame) => {
        ...
        const answerParagraphs = frame.locator('[data-testid="studio-assistant-answer-card"] p');
  ...
  return {
    ...
    streamProbe,
    assistantText,
    modelBackedStreamObserved: Boolean(streamProbe?.streamText),
    executionMode,
...
async function cleanupValidationProcesses({ outputDir, phase }) {
  ...
  const proof = {
    schemaVersion: "ioi.autopilot-agent-studio-chat-ux-hardening.process-cleanup.v1",
    phase,
  ...
  try {
    ...
    for (let index = 0; index < promptCases.length; index += 1) {
      ...
      promptResults.push({
        ...
        firstStreamText: result.streamProbe?.streamText?.slice(0, 180) || "",
    ...
    if (scenario.requireAgentModeReply && agentPromptResults.some((item) => !item.promptFailureError && (!item.assistantText || item.modelBackedStreamObserved))) {
    ...
    const proof = {
      schemaVersion: "ioi.autopilot-agent-studio-chat-ux-hardening.proof.v1",
      ...
      targetStudioAskModeDirectModelAchieved: askPromptResults.length > 0
        ...
        : true,
      agentFinalReplyAcceptedWithoutStreaming: agentPromptResults.every((item) => !item.modelBackedStreamObserved && item.assistantText),
...
async function main() {
  ...
  console.log(JSON.stringify({
    schemaVersion: "ioi.autopilot-agent-studio-chat-ux-hardening.goal.v1",
    ok: true,
    evidenceDir: outputDir,
    proof,
  }, null, 2));

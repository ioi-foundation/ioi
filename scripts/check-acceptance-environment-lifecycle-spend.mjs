...
const CLAUSES = [
  ...
  { id: "9", clause: "Metering is invoice-grade: grant, burn, reservation, overage and top-up reproduce from receipts, and network/open work draws a separate budget", unit: "M07.1 · M07.5", checks: [rootScript("check:work-credit-budget-contract")], absences: [absent("M07.5", "check:substrate-metering-contract", "owner-derived UsageRecord ingestion with dimensional attribution, idempotent deduplication, quota/entitlement consumption and supplier-discrepancy nonclaims")] },
  ...
  { id: "N2", negative: true, clause: "A green candidate/quote lane is never cited as evidence for a live lane", unit: "ACC-11", structural: (clauses) => { const bad = clauses.filter((c) => (c.scheduled ?? []).length > 0).flatMap((c) => (c.checks ?? []).filter((k) => /live/iu.test(k.label)).map((k) => `${c.id}: ${k.label}`)); return { ok: bad.length === 0, detail: `every clause with a scheduled live leg executes only spend-free checks and cites the retained live evidence through its applicability gate${bad.length ? `; violations: ${bad.join(", ")}` : ""}` }; } },
  { id: "N3", negative: true, clause: "A private-custody claim without a custody-proven route is typed as contractual privacy and says so", unit: "M09.7", absences: [absent("M09.7", "check:custody-proven-private-routes", "the contractual_privacy / workload_isolation / custody_proven_no_plaintext route assurance classes — zero lines exist")] },

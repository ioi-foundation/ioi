...
import {
  ...
  evidenceRoot,
  liveEvidenceRoot,
} from "./studio-artifact-corpus/config";
import {
  buildHtmlDistinctnessCheck,
  buildRefinementPatchCheck,
  caseRetainsStyleSteering,
  summarizeCaseTotals,
} from "./studio-artifact-corpus/classification";
import { ensureCleanDirectory, writeJson } from "./studio-artifact-corpus/artifact-files";
import {
  buildRevisionFlow,
  executeCase,
  loadPersistedCaseSummaries,
  runLiveStudioLane,
} from "./studio-artifact-corpus/flows";
import type { CaseSummary, CorpusSummary } from "./studio-artifact-corpus/types";

function selectedCorpusCases() {
  const raw = process.env.STUDIO_ARTIFACT_CORPUS_CASES?.trim();
  if (!raw) {
    return corpusCases;
  }
...
function selectedLanes() {
  const raw = process.env.STUDIO_ARTIFACT_CORPUS_LANES?.trim().toLowerCase();
  if (!raw || raw === "both") {
...
async function main() {
  ...
  if (partialRun) {
    console.log(
      `Studio artifact corpus partial run: lanes=${lanes.contract && lanes.live ? "both" : lanes.contract ? "contract" : "live"}, cases=${selectedCases.map((caseConfig) => caseConfig.id).join(", ")}`,
    );
  } else {
  ...
  const summary: CorpusSummary = {
    ...
    parityChecks: {
      ...
      repeatedRunVariationFlow:
        liveStudio.repeatedRunVariationFlow ?? {
          renderer: "svg",
  ...
  await writeJson(
    path.join(liveEvidenceRoot, "live-studio-summary.json"),
    liveStudio,
  );

  const parityFailures = [
    summary.lanes.liveStudio.status !== "pass",
  ...
  if (!partialRun && (totals.repairable > 0 || totals.blocked > 0 || parityFailures)) {
    throw new Error(
      `Corpus contained failing cases: pass=${totals.pass}, repairable=${totals.repairable}, blocked=${totals.blocked}, liveStudio=${summary.lanes.liveStudio.status}`,
    );
  }

  console.log(
    `Studio artifact corpus complete: ${totals.pass} pass, ${totals.repairable} repairable, ${totals.blocked} blocked`,
  );
  console.log(`Evidence root: ${evidenceRoot}`);
  console.log(
    `Live Studio lane: ${summary.lanes.liveStudio.status} (${summary.lanes.liveStudio.strongestContradiction ?? "ok"})`,
  );
}

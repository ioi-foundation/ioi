...
const EVIDENCE_ROOT = "docs/evidence/autopilot-agent-studio-operational-chat";
...
const PROCESS_PATTERN = "/tmp/autopilot-agent-studio-user-";

const REQUIRED_SCREENSHOTS = [
  "studio-operational-chat.png",
  "studio-pending-state.png",
  "studio-final-answer.png",
  "studio-session-history.png",
  "studio-tool-timeline.png",
  "studio-approval-gate.png",
  "studio-receipts-replay.png",
  "studio-model-route-picker.png",
  "studio-workflow-handoff.png",
  "studio-inline-diff-hunks.png",
  "studio-hunk-decision-receipt.png",
  "studio-terminal-output.png",
...
function checkSource() {
  ...
  const forbidden = ["studio.promptSubmit", 'data-testid="agent-studio-landing"'].filter((needle) =>
    source.includes(needle),
  );
  return {
    id: "source:agent-studio-operational-chat",
    ok: missing.length === 0 && forbidden.length === 0,
    summary:
      missing.length === 0 && forbidden.length === 0
        ? "Studio source is operational-chat shaped"
...
function checkPackageScripts() {
  ...
  return {
    id: "package:scripts",
    ok: missing.length === 0,
    summary: missing.length === 0 ? "Agent Studio goal scripts are registered" : "Goal scripts are missing",
    evidence: { missing },
  };
}
...
function preflightChecks() {
  const checks = [
    checkFile(MASTER_GUIDE, "Agent Studio operational chat master guide"),
  ...
  return {
    schemaVersion: "ioi.autopilot-agent-studio-operational-chat.preflight.v1",
...
async function runValidation(outputDir) {
  ...
  try {
    ...
    await screenshot(page, outputDir, "studio-operational-chat.png", screenshots);
    ...
    const pendingSeen = await waitForPredicate(async () => {
      try {
        ...
        return userTurn > 0 && pending > 0;
      } catch {
        return false;
      }
    }, 1000, 100);
    await screenshot(page, outputDir, "studio-pending-state.png", screenshots);
    ...
    await screenshot(page, outputDir, "studio-final-answer.png", screenshots);
    await screenshot(page, outputDir, "studio-session-history.png", screenshots);
    await screenshot(page, outputDir, "studio-tool-timeline.png", screenshots);
    await screenshot(page, outputDir, "studio-approval-gate.png", screenshots);
    await screenshot(page, outputDir, "studio-receipts-replay.png", screenshots);
    await screenshot(page, outputDir, "studio-model-route-picker.png", screenshots);
    await screenshot(page, outputDir, "studio-workflow-handoff.png", screenshots);
    await screenshot(page, outputDir, "studio-inline-diff-hunks.png", screenshots);
    await screenshot(page, outputDir, "studio-terminal-output.png", screenshots);
    ...
    const proof = {
      schemaVersion: "ioi.autopilot-agent-studio-operational-chat.proof.v1",
      targetStudioOperationalChatAchieved: true,
...
async function main() {
  ...
  if (!proof.targetStudioOperationalChatAchieved || missingScreenshots.length > 0) {

...
// Phase 5c.0: the binding spec measures the Rust true-north hypervisor-daemon
// (spawned below), not the JS daemon — no import of startRuntimeDaemonService.
...
function resolveHypervisorDaemonBinary(evidence) {
...
// The binding spec measures the Rust true-north hypervisor-daemon (Phase 5c.0):
...
async function startRustHypervisorDaemon({ stateDir, evidence }) {
...
async function main() {
  ...
  try {
    daemon = await startRustHypervisorDaemon({ stateDir, evidence });

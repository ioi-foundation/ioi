...
// R-160 (2026-09-15): on 77a370cb6 the OutcomeRoom/System spine verifier crashes at its twelfth step —
// the System-bound collective GoalRun create answers 403 with no error code (the daemon's authority-
// challenge shape) and the verifier reports coverage 11 of 104. Measured identical against the daemon
// built WITHOUT M12.1's edits, so it predates that unit; the verifier is a scheduled qualification
// (not a per-push gate) owned by M12.4, a product_track unit outside the MVP accounting (ADR 0053).
// Fenced on EXACTLY that signature and printed, never hidden; any other red on this clause is unfenced.
const spineFence = {
  ruling: "R-160 (2026-09-15, MVP owner): pre-existing on 77a370cb6, measured against the HEAD daemon",
  owner: "M12.4 (product_track)",
  what: "the collective GoalRun create answers 403 with no error code and the verifier stops at 11 of 104",
  holdsGate: false,
  matches: ({ stdout, stderr }) => {
    const text = `${stdout}\n${stderr}`;
    return /collective GoalRun failed 403\/undefined\/no-detail/u.test(text) && /verifier coverage changed: expected 104, got 11/u.test(text);
  },
};
...
const CLAUSES = [
  ...
  { id: "4", clause: "Flagship, single node: package → genesis → constitution-bound room → room-child admission spine, end to end, through the selected bounded software-change profile", unit: "M12.4 (product_track)", checks: [{ ...bounded(app("check:m4-outcome-room-system-spine"), 60), allowsFixture: true, fence: spineFence }] },

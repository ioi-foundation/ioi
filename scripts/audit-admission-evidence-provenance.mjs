...
const PINNED = [
  {"file": "assurance_transition_routes.rs", "rule": "C", "key": "actor_ref", "count": 1, "disposition": "sanctioned", "justification": "read 2026-09-03: build_ladder_response projects actor_ref from an already-admitted transition record into a nullable read-only ladder row; the write path records caller.identity.principal_ref after authenticated request-identity resolution and never accepts actor_ref from the request, so this is server-resolved attribution display rather than caller-authored admission evidence"},

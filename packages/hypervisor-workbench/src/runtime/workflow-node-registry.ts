...
const WORKFLOW_NODE_DEFINITION_BASES: WorkflowNodeDefinitionBase[] = [
  ...
  },
  {
    ...
    configSchema: {
      ...
      properties: {
        ...
        runtimeTaskCancelEndpoint: { type: "string" },
        runtimeTaskCancelable: { type: "boolean" },
        runtimeTaskCancelRoute: { type: "string" },
  ...
  },
  {
    ...
    ioTypes: { in: "state", out: "state" },
    inputs: ["thread"],
    outputs: ["approval", "event", "status"],
    portDefinitions: [
      port("thread", "Thread state", "input", "state", "state", false, "state"),
      port("approval", "Approval request", "output", "state", "output", false, "state"),
      port("event", "Approval event", "output", "state", "output", false, "state"),
      port("status", "Approval status", "output", "state", "output", false, "state"),
    ],
    ports: [
      port("thread", "Thread state", "input", "state", "state", false, "state"),
      port("approval", "Approval request", "output", "state", "output", false, "state"),
    ...
    configSchema: {
      type: "object",
      required: [
        "runtimeApprovalRequestEndpoint",
        "runtimeApprovalRequestThreadIdField",
        "runtimeApprovalRequestWorkflowNodeId",
      ],
      properties: {
        runtimeApprovalRequestEndpoint: { type: "string" },
        runtimeApprovalRequestField: { type: "string" },
        runtimeApprovalRequestEventField: { type: "string" },
        runtimeApprovalRequestStatusField: { type: "string" },
        runtimeApprovalRequestReceiptField: { type: "string" },
        runtimeApprovalRequestPolicyField: { type: "string" },
        runtimeApprovalRequestThreadId: { type: "string" },
        runtimeApprovalRequestThreadIdField: { type: "string" },
        runtimeApprovalRequestTurnId: { type: "string" },
        runtimeApprovalRequestTurnIdField: { type: "string" },
        runtimeApprovalRequestApprovalId: { type: "string" },
        runtimeApprovalRequestApprovalIdField: { type: "string" },
        runtimeApprovalRequestReason: { type: "string" },
        runtimeApprovalRequestReasonField: { type: "string" },
        runtimeApprovalRequestScope: { type: "string" },
        runtimeApprovalRequestScopeField: { type: "string" },
        runtimeApprovalRequestPressureField: { type: "string" },
        runtimeApprovalRequestPressureStatus: { type: "string" },
        runtimeApprovalRequestPressureStatusField: { type: "string" },
        runtimeApprovalRequestAlertId: { type: "string" },
        runtimeApprovalRequestAlertIdField: { type: "string" },
        runtimeApprovalRequestSourceEventId: { type: "string" },
        runtimeApprovalRequestSourceEventIdField: { type: "string" },
        runtimeApprovalRequestWorkflowNodeId: { type: "string" },
        runtimeApprovalRequestSource: { type: "string" },
        runtimeApprovalRequestActor: { type: "string" },
        redactionProfile: { type: "string" },
        ...RUNTIME_CHROME_CONFIG_SCHEMA_PROPERTIES,
      },
    },
    localization: runtimeNodeLocalization("runtime_approval_request"),
    accessibility: runtimeNodeAccessibility(
      "runtime_approval_request",
      "runtimeApprovalRequest.status",
    ),
    policyProfile: policyProfile("write"),
    evidenceProfile: evidenceProfile(
      ["execution", "verification"],
    ...
    executor: {
      ...
      executorId: "workflow.runtime_approval_request",
      sandboxed: false,
      supportsDryRun: true,
    },
    defaultLogic: {
      ...
      runtimeApprovalRequestPolicyField:
        "runtimeApprovalRequest.policyDecisionRefs",
      ...
      activationGate: {
        consumesRuntimeApprovalRequest: true,
        runtimeApprovalRequestField: "runtimeApprovalRequest",
        runtimeApprovalRequestStatusField: "runtimeApprovalRequest.status",
      },
      nodeTypeLabel: "RuntimeApprovalRequestNode",
    },
    defaultLaw: { privilegedActions: ["runtime.approval.request"] },
  },
  {
...
export function workflowNodeCreatorDefinitions(): WorkflowNodeCreatorDefinition[] {
  ...
  const browserUseTool = creatorDefinition("plugin_tool", {
    creatorId: "plugin_tool.browser_use",
    label: "Browser Use",
    description: "Use the native browser lane with DOM, AX, selector, screenshot, target-index, action-proposal, verification, and cleanup receipts.",
    ...
    defaultLogic: {
      toolBinding: {
        ...
        arguments: {
          ...
          controlledRelaunchHeadless: false,
          controlledRelaunchStartUrl: "",
          controlledRelaunchCdpPort: 0,
  ...
  const visualComputerUseTool = creatorDefinition("plugin_tool", {
    creatorId: "plugin_tool.computer_use.visual_gui",
    label: "Computer Use",
    description: "Use the visual GUI lane for arbitrary apps, canvas, and non-DOM interfaces with screenshot, accessibility, SoM, coordinate safety, verification, and recovery receipts.",
    metricLabel: "Computer lane",
    metricValue: "visual",
    defaultLogic: {
      toolBinding: {
        ...
        arguments: {
          ...
          captureAppName: "",
          captureWindowTitle: "",
          localGuiExecutor: false,
          localGuiExecutorProvider: "auto",
          appName: "",
          windowTitle: "",
          coordinateSpaceId: "",
          viewportWidth: 0,
    ...
    defaultLaw: {
      requireHumanGate: true,
      privilegedActions: ["computer_use.visual_gui", "coordinate_action"],
    },
    keywords: [
      "computer",
      "computer use",
  ...
  const visualObservationTool = creatorDefinition("plugin_tool", {
    creatorId: "computer_use.visual_gui_observe",
    label: "Visual Observation",
    description: "Capture or import a read-only visual GUI observation bundle with governed screenshot, SoM, AX, target-index, affordance, and retention evidence before any coordinate action authority is requested.",
    metricLabel: "Computer lane",
    metricValue: "observe",
    defaultLogic: {
      toolBinding: {
  ...
  const sandboxedComputerUseTool = creatorDefinition("plugin_tool", {
    ...
    defaultLaw: {
      requireHumanGate: true,
      privilegedActions: ["computer_use.sandboxed_hosted", "hosted_worker"],
    },
    keywords: [
      "sandbox",
      "sandboxed computer",
...
function relatedNodeTypesFor(type: WorkflowNodeKind): WorkflowNodeKind[] {
  switch (type) {
    ...
    case "runtime_approval_request":
      return ["human_gate", "decision", "runtime_operator_steer", "output"];

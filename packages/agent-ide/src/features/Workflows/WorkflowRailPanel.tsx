...
function workflowHarnessReceiptKind(receiptRef: string): string {
...
type WorkflowHarnessReceiptInspection = {
  receiptRef: string;
  sourceKind: string;
  sourceLabel: string;
  status: string;
  ...
  evidenceRefs: string[];
  payloadPreview: ReturnType<typeof workflowValuePreview>;
};
...
export function WorkflowRailPanel({
  ...
  const makeHarnessReceiptInspection = (
    ...
    receiptRef: details.receiptRef ?? selectedHarnessReceiptRef ?? "receipt pending",
    sourceKind: details.sourceKind,
    sourceLabel: details.sourceLabel,
    status: details.status,
    producerComponent: details.producerComponent,
    receiptKind: workflowHarnessReceiptKind(
      details.receiptRef ?? selectedHarnessReceiptRef ?? "receipt",
    ),
  ...
  const selectedHarnessReceiptInspection =
    selectedHarnessReceiptRef
      ? (() => {
          const receiptRef = selectedHarnessReceiptRef;
          ...
          if (
            harnessActivationRollbackProof?.receiptRefs.includes(receiptRef)
          ) {
            return makeHarnessReceiptInspection({
              receiptRef,
          ...
          if (
            ...
            return makeHarnessReceiptInspection({
              ...
              inputHash:
                rollbackCanary.expectedWorkflowContentHash ??
                "expected hash pending",
              outputHash:
                rollbackCanary.actualWorkflowContentHash ?? "actual hash pending",
          ...
          if (dispatchReceiptGroup && harnessDefaultRuntimeDispatchProof) {
            ...
            return makeHarnessReceiptInspection({
              ...
              attemptId:
                dispatchReceiptGroup.attempts[receiptIndex] ?? "attempt pending",
              replayFixtureRef:
                dispatchReceiptGroup.replay[receiptIndex] ??
          ...
          if (selectedHarnessGroup?.deepLinks.receiptRefs.includes(receiptRef)) {
            return makeHarnessReceiptInspection({
              ...
              policyDecision:
                ...
                "group receipt selected",
              attemptId:
                selectedHarnessGroupGatedRun?.nodeAttemptIds[0] ??
                "attempt pending",
              replayFixtureRef:
    ...
    <>
      ...
      {selectedHarnessReceiptInspection ? (
        <section
          ...
          data-producer-component={selectedHarnessReceiptInspection.producerComponent}
          ...
          data-replay-fixture-ref={selectedHarnessReceiptInspection.replayFixtureRef}
        >
          ...
          >
            ...
            <span>{selectedHarnessReceiptInspection.payloadPreview.summary}</span>
            <small>{selectedHarnessReceiptInspection.payloadPreview.detail}</small>
          </article>
          <div
            className="workflow-inline-metadata"
            data-testid="workflow-harness-receipt-evidence-refs"
            data-evidence-ref-count={selectedHarnessReceiptInspection.evidenceRefs.length}
          >
            <span>
              {selectedHarnessReceiptInspection.createdAtMs
                ? workflowTimeLabel(selectedHarnessReceiptInspection.createdAtMs)
                : "timestamp pending"}
            </span>
            <code>

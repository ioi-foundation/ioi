export const WORKFLOW_MODEL_MOUNT_IDE_CONTROL_SCHEMA_VERSION =
  "ioi.workflow.model-mount-control.v1" as const;
export const MODEL_MOUNT_IDE_CONTROL_SOURCE = "react_flow" as const;
export const MODEL_MOUNT_IDE_CONTROL_COMPONENT_KIND =
  "model_mount_control" as const;
export const MODEL_MOUNT_IDE_CONTROL_PAYLOAD_SCHEMA_VERSION =
  "ioi.model-mount.control-client.v1" as const;

export type ModelMountIdeControlMethod = "GET" | "POST" | "PATCH" | "DELETE";
export type ModelMountIdeControlFamily =
  | "route"
  | "server"
  | "backend"
  | "runtime"
...
export interface ModelMountIdeControlRoute {
  readonly id: string;
  readonly family: ModelMountIdeControlFamily;
  readonly method: ModelMountIdeControlMethod;
...
const controlRoute = <const Id extends string>(
  id: Id,
  family: ModelMountIdeControlFamily,
  method: ModelMountIdeControlMethod,
  path: string,
...
export const MODEL_MOUNT_IDE_CONTROL_ROUTES = [
...
] as const satisfies readonly ModelMountIdeControlRoute[];

export type ModelMountIdeControlRouteId =
  (typeof MODEL_MOUNT_IDE_CONTROL_ROUTES)[number]["id"];

export interface ModelMountIdeControlRequestBody {
  schema_version: typeof WORKFLOW_MODEL_MOUNT_IDE_CONTROL_SCHEMA_VERSION;
  source: typeof MODEL_MOUNT_IDE_CONTROL_SOURCE;
  actor: string;
  workflow_graph_id: string | null;
  workflow_node_id: string;
  component_kind: typeof MODEL_MOUNT_IDE_CONTROL_COMPONENT_KIND;
  payload_schema_version: typeof MODEL_MOUNT_IDE_CONTROL_PAYLOAD_SCHEMA_VERSION;
  control_route_id: ModelMountIdeControlRouteId;
  control_family: ModelMountIdeControlFamily;
  authority_scopes: string[];
  input: Record<string, unknown>;
}

export interface ModelMountIdeControlRequest {
  schemaVersion: typeof WORKFLOW_MODEL_MOUNT_IDE_CONTROL_SCHEMA_VERSION;
  ...
  method: ModelMountIdeControlMethod;
  endpoint: string;
  body: ModelMountIdeControlRequestBody | null;
}

export interface ModelMountIdeControlRequestInput {
  routeId: ModelMountIdeControlRouteId;
  nodeId?: string | null;
...
const MODEL_MOUNT_IDE_CONTROL_ROUTE_BY_ID = new Map<
  string,
  ModelMountIdeControlRoute
>(MODEL_MOUNT_IDE_CONTROL_ROUTES.map((route) => [route.id, route]));

export function modelMountIdeControlRoute(
  routeId: ModelMountIdeControlRouteId,
): ModelMountIdeControlRoute {
  const route = MODEL_MOUNT_IDE_CONTROL_ROUTE_BY_ID.get(routeId);
  if (!route) {
    throw new Error(`Unknown model_mount IDE control route: ${routeId}`);
  }
  return route;
}

export function modelMountIdeControlRouteIdsByFamily(
  family: ModelMountIdeControlFamily,
): ModelMountIdeControlRouteId[] {
  return MODEL_MOUNT_IDE_CONTROL_ROUTES.filter((route) => route.family === family).map(
    (route) => route.id,
  );
}

export function createModelMountIdeControlRequest(
  params: ModelMountIdeControlRequestInput,
): ModelMountIdeControlRequest {
  const route = modelMountIdeControlRoute(params.routeId);
  const endpoint = renderModelMountIdeControlEndpoint(
    params.routeId,
    params.pathParams ?? {},
  );
  ...
  return {
    schemaVersion: WORKFLOW_MODEL_MOUNT_IDE_CONTROL_SCHEMA_VERSION,
    nodeType: "model_mount_control",
    ...
    body:
      route.method === "GET"
        ? null
        : {
            schema_version: WORKFLOW_MODEL_MOUNT_IDE_CONTROL_SCHEMA_VERSION,
            source: MODEL_MOUNT_IDE_CONTROL_SOURCE,
            ...
            component_kind: MODEL_MOUNT_IDE_CONTROL_COMPONENT_KIND,
            payload_schema_version: MODEL_MOUNT_IDE_CONTROL_PAYLOAD_SCHEMA_VERSION,
...
export function renderModelMountIdeControlEndpoint(
  routeId: ModelMountIdeControlRouteId,
  pathParams: Record<string, unknown> = {},
): string {
  const route = modelMountIdeControlRoute(routeId);
  ...
  for (const key of Object.keys(pathParams)) {
    if (!usedParams.has(key)) {
      throw new Error(`Unused model_mount IDE control path param '${key}' for ${routeId}.`);
    }
  }
  assertStableModelMountIdeControlEndpoint(endpoint);
  return endpoint;
}

export function assertStableModelMountIdeControlEndpoint(endpoint: string): void {
  if (!endpoint.startsWith("/v1/model-mount/")) {
    throw new Error(
      `Model_mount IDE control routes must use stable /v1/model-mount protocol paths, received ${endpoint}.`,
    );
  }
  if (endpoint.includes("/api/v1/")) {
    throw new Error(
      `Model_mount IDE control routes must not use retired /api/v1 paths, received ${endpoint}.`,
    );
  }
}
...
function assertNoRetiredModelMountControlInputAliases(
  ...
  for (const [key, nested] of Object.entries(value as Record<string, unknown>)) {
    if (RETIRED_MODEL_MOUNT_CONTROL_INPUT_ALIASES.has(key)) {
      throw new Error(
        `Retired model_mount IDE control input alias '${key}' at ${path}.${key}; use canonical snake_case protocol fields.`,
      );
    }
...
function requiredPathParam(
  ...
  if (value === undefined || value === null || value === "") {
    throw new Error(`Model_mount IDE control route '${routeId}' needs path param '${key}'.`);
  }
  return String(value);
}

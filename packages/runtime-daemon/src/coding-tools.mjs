...
export function codingToolStepModuleProjection(toolId, input = {}, result = {}, context = {}) {
  ...
  return createCodingToolStepModuleProjection({
    ...
    workflowGraphId: context.workflow_graph_id,
    workflowNodeId: context.workflow_node_id,
    contextChamberRef: context.context_chamber_ref ?? null,
    actionProposalRef: context.action_proposal_ref,
    gateResultRef: context.gate_result_ref,
    actorId: context.actor_id,
    runtimeNodeRef: context.runtime_node_ref,
    policyHash: context.policy_hash,
    authorityGrantRefs: context.authority_grant_refs ?? [],
    approvalRef: context.approval_ref ?? null,
    stateRootBefore: context.state_root_before ?? null,
    projectionWatermark: context.projection_watermark ?? null,
    idempotencyKey: context.idempotency_key,
    deadlineMs: context.deadline_ms,
    status: context.status ?? "success",
    workflowProjectionStatus: context.workflow_projection_status ?? "projected",
    executionResultRef: context.execution_result_ref ?? null,
    normalizedObservationRef: context.normalized_observation_ref ?? null,
    receiptRefs: context.receipt_refs ?? null,
    artifactRefs: context.artifact_refs ?? [],
    payloadRefs: context.payload_refs ?? [],
    agentgresOperationRefs: context.agentgres_operation_refs ?? [],
    stateRootAfter: context.state_root_after ?? null,
    resultingHead: context.resulting_head ?? null,
    evidenceRefs: context.evidence_refs ?? [],
    modelReentryRequired: context.model_reentry_required ?? false,
    verifierRequired: context.verifier_required ?? false,
  });
}

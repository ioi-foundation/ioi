...
export function recordModelStreamCanceled({ mounts, invocation, streamKind, framesWritten }) {
  mounts.receipt("model_invocation_stream_canceled", {
    ...
    details: {
      ...
      backendId: invocation.instance.backendId ?? invocation.receipt.details?.backend_id ?? null,
      selectedBackend: invocation.receipt.details?.selected_backend ?? null,
      streamSource: invocation.receipt.details?.stream_source ?? null,
      providerResponseKind: invocation.providerResponseKind ?? invocation.receipt.details?.provider_response_kind ?? null,
      backendEvidenceRefs: invocation.receipt.details?.backend_evidence_refs ?? [],

...
test("worker/service package runner sends invocation admission through direct daemon-core invoker", () => {
  const calls = [];
  ...
  assert.equal(calls[0].schema_version, WORKER_SERVICE_PACKAGE_COMMAND_SCHEMA_VERSION);
  assert.equal(calls[0].operation, "admit_worker_service_package_invocation");
  assert.equal(calls[0].backend, RUST_WORKER_SERVICE_PACKAGE_BACKEND);
  assert.equal(calls[0].thread_id, "thread:worker-runner");
  assert.equal(calls[0].agent_id, "agent:worker-runner");
...
test("worker/service package runner env uses daemon-level direct invoker", () => {
  const calls = [];
...
test("worker/service package runner rejects retired daemon-core command env", () => {
  assert.throws(
    () =>
      createWorkerServicePackageRunnerFromEnv({
    ...
    (error) =>
      ...
      error.code === "worker_service_package_command_selection_retired",
  );
});

test("worker/service package runner rejects retired package command env", () => {
  assert.throws(
    () =>
      createWorkerServicePackageRunnerFromEnv({
...
test("worker/service package runner command constructor option fails closed", () => {
  assert.throws(
    () => new RustWorkerServicePackageRunner({ command: "ioi-runtime-daemon-core" }),
    (error) =>
      error instanceof WorkerServicePackageRunnerError &&
      error.code === "worker_service_package_command_selection_retired",
  );
...
test("worker/service package runner fails closed without direct invoker", () => {
  const runner = new RustWorkerServicePackageRunner();
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof WorkerServicePackageRunnerError &&
      error.code === "worker_service_package_direct_invoker_unconfigured",
  );
});

...
test("public runtime approved operation admission route is retired (served by the Rust daemon)", async () => {
  const { handleRequest } = routeHarness();
  const response = responseRecorder();
...
// Harness Lane A execution is RETIRED from the JS daemon — the Rust
// hypervisor-daemon owns the real provisioning + wallet-gated host-spawn lane +
// served preview. These guards assert the JS surface cannot silently return.
const RETIRED_HARNESS_EXECUTION_ROUTES = [
  ...
  "/v1/hypervisor/harness-session-turn-lanes",
];
for (const retiredRoute of RETIRED_HARNESS_EXECUTION_ROUTES) {
  test(`public runtime harness execution route ${retiredRoute} is retired (served by the Rust daemon)`, async () => {
    const { handleRequest } = routeHarness();
    const response = responseRecorder();
    await handleRequest({
      request: request({ method: "POST", url: retiredRoute, body: {} }),
      response,
    ...
    assert.equal(
      JSON.parse(response.body).error.code,
      "harness_execution_retired_served_by_rust_daemon",
    );
  });
}

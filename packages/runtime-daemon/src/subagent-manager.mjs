...
export function subagentManagerEventPayload({ record = {}, operation, status }) {
  return {
    ...
    parent_thread_id: record.parent_thread_id ?? null,
    parent_turn_id: record.parent_turn_id ?? null,
    child_thread_id: record.child_thread_id ?? null,
    subagent_id: record.subagent_id ?? null,
    agent_id: record.agent_id ?? null,
    run_id: record.run_id ?? null,
    role: record.role ?? "general",
    tool_pack: record.tool_pack ?? null,
    model_route_id: record.model_route_id ?? null,
    lifecycle_status: status ?? record.lifecycle_status ?? null,
    ...
    merge_policy: record.merge_policy ?? null,
    cancellation_inheritance: record.cancellation_inheritance ?? null,
    context_pressure_action:
      record.context_pressure_action ?? null,
    context_pressure:
      record.context_pressure ?? null,
    pressure_status: record.pressure_status ?? null,
    alert_id: record.alert_id ?? null,
    source_event_id: record.source_event_id ?? null,
    source_receipt_refs: uniqueStrings(
      record.source_receipt_refs,
    ),
    source_policy_decision_refs:
      uniqueStrings(
        record.source_policy_decision_refs,
      ),
    input_id: record.input_id ?? null,
    input_count: record.input_count ?? null,
    cancellation_reason: record.cancellation_reason ?? null,
    cancellation_inherited:
      record.cancellation_inherited ?? null,
    propagated_from_thread_id:
      record.propagated_from_thread_id ?? null,
    restart_status: record.restart_status ?? null,
    restart_count: record.restart_count ?? null,
    resume_id: record.resume_id ?? null,
    assignment_id: record.assignment_id ?? null,
    assignment_count: record.assignment_count ?? null,
    target_agent_id: record.target_agent_id ?? null,
  };
}
...
function costEstimateUsdForRun(run = {}, totalTokens = 0) {
  const decisionCost = optionalPositiveNumber(
    run.model_route_decision?.cost_estimate_usd,
  );

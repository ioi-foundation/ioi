...
test("approval surface requests approval and blocks the active turn", () => {
  ...
  assert.equal(Object.hasOwn(result, "approvalRequired"), false);
  ...
  assert.equal(store.events[0].payload_summary.approval_manifest.toolId, "file.write");
  for (const field of [
    "approvalRequired",
    "approvalManifest",
    "toolId",
    ...
    assert.equal(Object.hasOwn(store.events[0].payload_summary, field), false);
  }
...
test("approval surface records approval decisions with active leases", () => {
  ...
  for (const field of ["toolId", "effectClass", "riskDomain", "approvalManifest"]) {
    assert.equal(Object.hasOwn(store.events[1].payload_summary, field), false);
  }
...
test("approval surface revokes approval leases and records prior decisions", () => {
  ...
  for (const field of ["toolId", "effectClass", "riskDomain", "approvalManifest"]) {
    assert.equal(Object.hasOwn(store.events[2].payload_summary, field), false);
  }

...
export function normalizedAgentRuntimeControls(agent = {}) {
  ...
  const routeId = model.route_id ?? agent.modelRouteId ?? "route.local-first";
  const selectedModel = model.selected_model ?? agent.modelId ?? null;
  const endpointId = model.endpoint_id ?? agent.modelRouteEndpointId ?? null;
  const providerId = model.provider_id ?? agent.modelRouteProviderId ?? null;
  const receiptId = model.receipt_id ?? agent.modelRouteReceiptId ?? null;
  const reasoningEffort =
    model.reasoning_effort ?? agent.modelRouteDecision?.reasoning_effort ?? null;
  const maxCostUsd = model.max_cost_usd ?? null;
...
export function threadRuntimeControlModelInput(request = {}, controls = {}, agent = {}) {
  ...
  for (const [key, snakeKey, outputKey] of [
    ...
    ["workflow_graph_id", "workflow_graph_id", "workflow_graph_id"],
  ]) {

...
export function backendRegistryRecords({
  ...
  return [
    ...
    },
    {
      ...
      status: providers.get("provider.lmstudio")?.status ?? "unknown",
      processStatus: "external_provider",
      binaryPath: providers.get("provider.lmstudio")?.discovery?.publicCli?.path ?? null,
      baseUrl: providers.get("provider.lmstudio")?.baseUrl ?? "http://127.0.0.1:1234/v1",
      capabilities: ["chat", "responses", "embeddings"],
    ...
    },
    {
      ...
      status: providers.get("provider.openai-compatible")?.status ?? "configured_if_provider_available",
      processStatus: "stateless_http",
      binaryPath: null,
      baseUrl: providers.get("provider.openai-compatible")?.baseUrl ?? null,
      capabilities: ["chat", "responses", "embeddings"],
    ...
    },
    {
      ...
      status: providers.get("provider.ollama")?.status ?? "blocked",
      processStatus: ollamaBinary ? "binary_configured" : "external_or_absent",
      binaryPath: ollamaBinary,
      baseUrl: providers.get("provider.ollama")?.baseUrl ?? "http://127.0.0.1:11434",
    ...
    },
    {
      ...
      status: providers.get("provider.vllm")?.status ?? "blocked",
      processStatus: vllmBinary ? "binary_configured" : "external_or_absent",
      binaryPath: vllmBinary,
      baseUrl: providers.get("provider.vllm")?.baseUrl ?? "http://127.0.0.1:8000/v1",
      capabilities: ["chat", "responses", "embeddings"],

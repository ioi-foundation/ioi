...
export function createRuntimeInvocationResultProjections(deps = {}) {
  ...
  function codingToolInvocationResultFromEvent(event, context = {}) {
    ...
    return {
      ...
      tool_name: payload.tool_name ?? context.tool_id ?? null,
      tool_call_id: payload.tool_call_id ?? context.tool_call_id ?? event.tool_call_id ?? null,
      thread_id: payload.thread_id ?? context.thread_id ?? event.thread_id ?? null,
      turn_id: payload.turn_id ?? context.turn_id ?? event.turn_id ?? null,
      ...
      workflow_graph_id: payload.workflow_graph_id ?? context.workflow_graph_id ?? event.workflow_graph_id ?? null,
      workflow_node_id: payload.workflow_node_id ?? context.workflow_node_id ?? event.workflow_node_id ?? null,
  ...
  function computerUseBrowserDiscoveryInvocationResultFromEvent(event, context = {}) {
    ...
    return {
      ...
      tool_name: payload.tool_ref ?? context.tool_id ?? null,
      tool_call_id: event.tool_call_id ?? context.tool_call_id ?? null,
      thread_id: event.thread_id ?? context.thread_id ?? null,
      turn_id: event.turn_id ?? context.turn_id ?? null,
      status: event.status ?? payload.status ?? "completed",

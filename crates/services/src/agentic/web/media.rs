...
fn media_provider_candidate_receipt_with_modality(
    provider_id: &str,
    request_url: &str,
    modality: &str,
    selected: bool,
...
fn discover_subtitle_candidate(
    request_url: &str,
    metadata: &Value,
    requested_language: &str,
...
fn normalize_requested_language(language: Option<&str>) -> String {
...
fn select_video_format(metadata: &Value) -> Option<VideoFormatSelection> {
...
fn discovery_reason_from_error(err: &anyhow::Error) -> String {
...
fn select_audio_format(metadata: &Value) -> Option<AudioFormatSelection> {
    let mut candidates = metadata
        .get("formats")
        .and_then(Value::as_array)
        .into_iter()
...
fn write_run_receipt(tool_home: &Path, receipt: &MediaTranscriptRunReceipt) -> Result<()> {

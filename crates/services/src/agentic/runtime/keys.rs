// Path: crates/services/src/agentic/runtime/keys.rs
...
pub fn get_runtime_substrate_key(session_id: &[u8; 32], step: u32) -> Vec<u8> {
    [
        RUNTIME_SUBSTRATE_PREFIX,
        session_id.as_slice(),
        &step.to_le_bytes(),
    ]
    .concat()
...
pub fn get_agent_trajectory_step_key(session_id: &[u8; 32], step: u32) -> Vec<u8> {
    [
        AGENT_TRAJECTORY_STEP_PREFIX,
        session_id.as_slice(),
        &step.to_le_bytes(),
    ]
    .concat()
...
pub fn get_managed_session_control_key(session_id: &[u8; 32], managed_session_id: &str) -> Vec<u8> {
    [
        MANAGED_SESSION_CONTROL_PREFIX,
        session_id.as_slice(),
        b"::",

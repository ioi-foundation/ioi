        ...
        AgentTool::GuiClick { x, y, button } => {
            ...
            if !allow_raw_coords {
                return ToolExecutionResult::failure(
                    "ERROR_CLASS=TierViolation Raw coordinate click is disabled outside VisualForeground (VisualLast). Use `screen__click` or `screen.left_click_element`.",
                );
            }
...
async fn handle_computer_action(
    ...
    match action {
        ScreenAction::MouseMove { coordinate } => {
            exec_input(
                exec,
        ...
        ScreenAction::LeftClick { coordinate } => {
            if let Some(coord) = coordinate {
                ...
                if let Some(blocked) =
                    guard_phase0_browser_click_without_coordinate(exec, "screen.left_click")
                {
                    return blocked;
                }
        ...
        ScreenAction::RightClick { coordinate } => {
            if let Some(coord) = coordinate {
                if let Some(blocked) = guard_phase0_browser_click_with_coordinate(
                    exec,
                    "screen.right_click",
                ...
                if let Some(blocked) =
                    guard_phase0_browser_click_without_coordinate(exec, "screen.right_click")
                {
                    return blocked;
                }
        ...
        ScreenAction::DoubleClick { coordinate } => {
            if let Some(coord) = coordinate {
                if let Some(blocked) = guard_phase0_browser_click_with_coordinate(
                    exec,
                    "screen.double_click",
                ...
                if let Some(blocked) =
                    guard_phase0_browser_click_without_coordinate(exec, "screen.double_click")
                {
                    return blocked;
                }
        ...
        ScreenAction::LeftClickId { id } => {
            if let Some(blocked) = guard_phase0_browser_click_by_id(
                exec,
                "screen.left_click_id",
                &id.to_string(),
                som_map,
                semantic_map,
                active_lens,
        ...
        ScreenAction::LeftClickElement { id } => {
            if let Some(blocked) = guard_phase0_browser_click_by_id(
                exec,
                "screen.left_click_element",
                &id,
                som_map,
                semantic_map,
                active_lens,
        ...
        ScreenAction::RightClickId { id } => {
            if let Some(blocked) = guard_phase0_browser_click_by_id(
                exec,
                "screen.right_click_id",
                &id.to_string(),
                som_map,
                semantic_map,
                active_lens,
        ...
        ScreenAction::RightClickElement { id } => {
            if let Some(blocked) = guard_phase0_browser_click_by_id(
                exec,
                "screen.right_click_element",
                &id,
                som_map,
                semantic_map,
                active_lens,
        ...
        ScreenAction::Type { text } => exec_input(exec, InputEvent::Type { text }).await,
        ScreenAction::Key { text } => exec_input(exec, InputEvent::KeyPress { key: text }).await,
        ScreenAction::Hotkey { keys } => {
        ...
        ScreenAction::DragDrop { from, to } => {
            if let Some(blocked) =
                guard_phase0_browser_drag_with_coordinates(exec, "screen.drag_drop", from, to)
            {
                return blocked;
            }
        ...
        ScreenAction::DragDropId { from_id, to_id } => {
        ...
        ScreenAction::DragDropElement { from_id, to_id } => {
            if let Some(blocked) = guard_phase0_browser_drag_by_ids(
                exec,
                "screen.drag_drop_element",
                &from_id,
                &to_id,
                som_map,
                semantic_map,
        ...
        ScreenAction::Screenshot => match exec.gui.capture_screen(None).await {
        ...
        ScreenAction::CursorPosition => match exec.gui.get_cursor_position().await {
            Ok((x, y)) => {
                let payload = json!({
                    "x": x,
                    "y": y,
        ...
        ScreenAction::Scroll { coordinate, delta } => {
            if let Some(coord) = coordinate {

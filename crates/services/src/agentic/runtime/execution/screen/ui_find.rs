    ...
    let screenshot = match exec.gui.capture_raw_screen().await {
        ...
        Err(err) => {
            return ToolExecutionResult::failure(format!(
                "ERROR_CLASS=MissingDependency Failed to capture screenshot for screen__find: {}",
                err
            ))
        }
    };
...
pub(super) async fn find_element_coordinates(
    ...
    if query.is_empty() {
        return ToolExecutionResult::failure(
            "ERROR_CLASS=TargetNotFound screen__find requires a non-empty query.",
        );
    }
        ...
        Ok(tree) => {
            ...
            if let Some(found) = find_semantic_ui_match(&tree, query) {
                return build_ui_find_success(
                    ...
                    exec.current_tier,
                    "primary_success",
                    "ToolFirst(screen__find.semantic)",
                    false,
                );
            }
        }
    ...
    if allow_vision {
        let escalation_chain = if semantic_attempted {
            "ToolFirst(screen__find.semantic)->VisualLast(screen__find.visual_locator)"
        } else {
            "ToolFirst(screen__find.semantic_unavailable)->VisualLast(screen__find.visual_locator)"
        };
    ...
    if let Some(err) = tree_fetch_error {
        return ToolExecutionResult::failure(format!(
            "ERROR_CLASS=MissingDependency screen__find could not inspect accessibility tree: {}",
            err
        ));
    }

    ToolExecutionResult::failure(format!(
        "ERROR_CLASS=TargetNotFound screen__find could not locate '{}' in current semantic context. Retry in VisualForeground for vision-assisted localization.",
        query
    ))
}

#[cfg(test)]
#[path = "ui_find/tests.rs"]
mod tests;

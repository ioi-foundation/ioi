...
pub struct ModelMountStorageControlBridgeRequest {
    #[serde(default)]
...
pub fn plan_model_mount_storage_control_response(
    request: ModelMountStorageControlBridgeRequest,
) -> Result<Value, ModelMountError> {
    let plan = plan_storage_control(&request.request)?;
    ...
    Ok(json!({
        ...
        "backend": request.backend.unwrap_or_else(|| "rust_model_mount_storage_control".to_string()),
        "plan": plan,

...
pub struct ModelMountVaultControlBridgeRequest {
    #[serde(default)]
...
pub fn plan_model_mount_vault_control_response(
    request: ModelMountVaultControlBridgeRequest,
) -> Result<Value, ModelMountError> {
    let plan = plan_vault_control(&request.request)?;
    ...
    Ok(json!({
        ...
        "backend": request.backend.unwrap_or_else(|| "rust_model_mount_vault_control".to_string()),
        "plan": plan,
...
mod tests {
    ...
    #[test]
    fn rust_core_plans_vault_bind_without_plaintext_material() {
        let plan =

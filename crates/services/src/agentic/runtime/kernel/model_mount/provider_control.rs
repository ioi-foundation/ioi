...
pub struct ModelMountProviderControlBridgeRequest {
    #[serde(default)]
...
pub fn plan_model_mount_provider_control_response(
    request: ModelMountProviderControlBridgeRequest,
) -> Result<Value, ModelMountError> {
    let plan = plan_provider_control(&request.request)?;
    ...
    Ok(json!({
        ...
        "backend": request.backend.unwrap_or_else(|| "rust_model_mount_provider_control".to_string()),
        "plan": plan,
...
mod tests {
    ...
    #[test]
    fn rust_core_plans_provider_control_record() {

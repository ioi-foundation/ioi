...
fn shield_policy_path(service: Option<&RuntimeAgentService>) -> Option<PathBuf> {
    service
        .and_then(|svc| svc.shield_policy_path.clone())
        .or_else(|| std::env::var_os(SHIELD_POLICY_PATH_ENV).map(PathBuf::from))
}

fn load_shield_policy_state(service: Option<&RuntimeAgentService>) -> ShieldPolicyState {
...
fn resolve_google_shield_policy(service: Option<&RuntimeAgentService>) -> ResolvedShieldPolicy {

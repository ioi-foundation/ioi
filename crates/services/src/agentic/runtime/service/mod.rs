// Path: crates/services/src/agentic/runtime/service/mod.rs
...
// [FIX] Middleware is in parent `runtime` module, not here.
...
#[async_trait]
impl UpgradableService for RuntimeAgentService {
...
#[async_trait]
impl BlockchainService for RuntimeAgentService {
    fn id(&self) -> &str {
        "desktop_agent"
    }
    ...
    async fn handle_service_call(
        ...
        match method {
            ...
            "pause@v1" => {
                let p: PauseAgentParams = codec::from_bytes_canonical(params)?;
                handle_pause(self, state, p, ctx).await
            }
            "cancel@v1" => {
                let p: CancelAgentParams = codec::from_bytes_canonical(params)?;
                handle_cancel(self, state, p, ctx).await
            }
            "deny@v1" => {
                let p: DenyAgentParams = codec::from_bytes_canonical(params)?;
                handle_deny(self, state, p, ctx).await
            }
...
impl RuntimeAgentService {
    ...
    pub(crate) async fn handle_action_execution(
        ...
        agent_state: &crate::agentic::runtime::types::AgentState,
    ...
    pub(crate) async fn prepare_cloud_inference_input(
        ...
        crate::agentic::runtime::cloud_airlock::prepare_cloud_inference_input(
            &self.scrubber,

pub(crate) fn build_recent_pending_browser_state_context_with_snapshot(
    ...
    let Some(signal) = navigation_signal
        ...
        .or_else(|| message_recipient_pending_signal(history, current_snapshot))
        .or_else(|| ranked_result_pending_signal(history, current_snapshot))
        .or_else(|| shortest_flight_result_pending_signal(history, current_snapshot))
        .or_else(|| select_submit_progress_pending_signal(history, current_snapshot))
        ...
        .or_else(|| pointer_hold_pending_signal(history, current_snapshot))
...
pub(crate) fn build_browser_snapshot_pending_state_context_with_history(
    ...
    let Some(signal) = auth_form_pending_signal_from_snapshot(snapshot, history)
        ...
        .or_else(|| message_recipient_pending_signal(history, Some(snapshot)))
        .or_else(|| ranked_result_pending_signal(history, Some(snapshot)))
        .or_else(|| shortest_flight_result_pending_signal(history, Some(snapshot)))
        ...
        .or_else(|| pointer_hold_pending_signal(history, Some(snapshot)))

...
fn toolcat_single_tool_reply_tool_name(goal: &str, current_tool_name: &str) -> String {
...
fn toolcat_single_tool_failure_reply(current_tool_name: &str) -> String {
    format!(
...
fn toolcat_single_tool_duplicate_after_success_reply(current_tool_name: &str) -> String {
    format!(
...
fn latest_retained_shell_command_id(text: &str) -> Option<String> {
...
fn latest_child_session_id_hex(text: &str) -> Option<String> {
    let value = serde_json::from_str::<serde_json::Value>(text).ok();
...
fn toolcat_single_tool_retained_shell_followup(
    goal: &str,
    current_tool_name: &str,
    output: Option<&str>,
) -> Option<AgentTool> {
...
fn toolcat_single_tool_agent_await_followup(
    goal: &str,
    current_tool_name: &str,
    output: Option<&str>,
) -> Option<AgentTool> {
...
fn toolcat_single_tool_browser_setup_followup(
    goal: &str,
    current_tool_name: &str,
    output: Option<&str>,
) -> Option<AgentTool> {
...
fn toolcat_single_tool_success_followup(goal: &str, current_tool_name: &str) -> Option<AgentTool> {
...
fn toolcat_single_tool_chat_reply_recovery_followup(
    goal: &str,
    current_tool_name: &str,
) -> Option<AgentTool> {
    if !is_toolcat_single_tool_probe(goal) || current_tool_name == "chat__reply" {

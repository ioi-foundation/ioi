    ...
    match tool {
        ...
        AgentTool::ChatReply { message } => {
            ...
            if let Some(pending) = agent_state.pending_search_completion.clone() {
                ...
                let model_answer_reason = completion_reason.or_else(|| {
                    pending_web_evidence_ready_for_model_answer(&pending).then_some(
                        crate::agentic::runtime::service::queue::web_pipeline::WebPipelineCompletionReason::MinSourcesReached,
                    )
                });
                if let Some(reason) = model_answer_reason {
                    if let Some(blocked_error) = source_candidate_chat_reply_blocker(message) {
                        ...
                        *action_output = Some(blocked_error);
                        agent_state.status = AgentStatus::Running;
                        verification_checks
                            .push("source_candidate_chat_reply_blocked=true".to_string());
                        verification_checks.push("terminal_chat_reply_ready=false".to_string());
                        emit_completion_gate_status_event(
                            service,
                            session_id,
                        ...
                        )
                    {
                        ...
                        for evaluation in &selection.evaluations {
                            verification_checks.push(format!(
                                ...
                                evaluation.facts.answer_rendered_layout_profile,
                                evaluation.facts.answer_document_layout_met
                            ));
                        }
...
then call the terminal reply tool with a complete model-authored Markdown answer."
                        ...
                        verification_checks.push(execution_evidence_key(
                            WEB_PIPELINE_TERMINAL_EVIDENCE,
                        ));
                        verification_checks.push("web_pipeline_active=false".to_string());
                        verification_checks.push("terminal_chat_reply_ready=true".to_string());
                        verification_checks
                            .push("web_final_answer_source=model_chat_reply".to_string());
                        ...
                        *terminal_chat_reply_output = Some(summary.clone());
                        ...
                        emit_completion_gate_status_event(
                            ...
                            resolved_intent_id,
                            true,
                            "chat_reply_model_authored_web_pipeline_answer_accepted",
                        );
                    ...
                    verification_checks
                        .push("web_model_chat_reply_contract_ready=false".to_string());
                    verification_checks
                        .push("web_final_summary_provider=model_chat_reply_unselected".to_string());
                    verification_checks
                        .push("web_final_answer_source=model_chat_reply".to_string());
                    let rejected_summary = enrich_command_scope_summary(message, agent_state);
...
missing, then call the terminal reply tool with a complete model-authored Markdown answer."
            ...
            if let Some(blocked_error) = source_candidate_chat_reply_blocker(message) {
                ...
                *action_output = Some(blocked_error);
                agent_state.status = AgentStatus::Running;
                verification_checks.push("source_candidate_chat_reply_blocked=true".to_string());
                verification_checks.push("terminal_chat_reply_ready=false".to_string());
                emit_completion_gate_status_event(
                    service,
                    session_id,
        ...
        AgentTool::MemorySearch { query } => {
                    ...
                    .unwrap_or(true)
            {
                ...
                let note = if queued {
                    "No memory hits for this news query; queued typed web__search.".to_string()
                } else {
                    "No memory hits for this news query; typed web__search was already queued."
                        .to_string()
                };
                *history_entry = Some(note.clone());
                *action_output = Some(note);

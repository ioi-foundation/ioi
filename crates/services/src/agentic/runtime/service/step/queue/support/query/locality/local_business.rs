...
fn local_business_named_entity_semantic_tokens(
    candidate: &str,
    locality_hint: Option<&str>,
) -> Vec<String> {
    ...
    while tokens
        .last()
        .map(|token| matches!(token.as_str(), "in" | "near" | "around" | "at" | "for" | "of"))
        .unwrap_or(false)
    {
        tokens.pop();
    }
...
fn local_business_named_entity_candidate(candidate: &str, locality_hint: Option<&str>) -> bool {
    let candidate_lower = compact_whitespace(candidate).to_ascii_lowercase();
    let tokens = local_business_named_entity_semantic_tokens(candidate, locality_hint);
    if tokens.is_empty() {
        return false;
    }

    let listing_hits = tokens
        .iter()
        .filter(|token| {
            GENERIC_LOCAL_BUSINESS_LISTING_TOKENS.contains(&token.as_str())
                || LOCAL_BUSINESS_DISCOVERY_CLASS_TOKENS.contains(&token.as_str())
        })
        .count();
    let semantic_hits = tokens
        .iter()
        .filter(|token| {
            !GENERIC_LOCAL_BUSINESS_LISTING_TOKENS.contains(&token.as_str())
                && !LOCAL_BUSINESS_DISCOVERY_CLASS_TOKENS.contains(&token.as_str())
                && !LOCAL_BUSINESS_COMPARISON_AXIS_TOKENS.contains(&token.as_str())
        })
        .count();
    ...
    if generic_local_business_listing_candidate(candidate, tokens.len()) {
        return false;
    }
    if semantic_hits >= 2 {
        return true;
    ...
    // A single semantic anchor plus a business-class suffix is only strong
    // enough when it reads like a concrete business name, not a locality or
    // category listing such as "Italian Restaurants in Anderson".
    let trailing_token = tokens.last().map(String::as_str).unwrap_or_default();
    let singular_business_suffix = matches!(
        trailing_token,
        "bar" | "bistro" | "cafe" | "cuisine" | "deli" | "grill" | "pizzeria" | "restaurant"
            | "tavern"
    );
    let scope_phrase_present = SCOPE_ANCHOR_MARKERS
        .iter()
        .any(|marker| candidate_lower.contains(marker));

    singular_business_suffix && !scope_phrase_present
}
...
pub(crate) fn local_business_entity_name_allowed(
    ...
    if !local_business_named_entity_candidate(&normalized, locality_hint) {
        return false;
    }
...
pub(crate) fn source_matches_local_business_search_entity_anchor(
    ...
    if direct_hits >= required_hits {
        if anchor_tokens.len() == 1 {
            let source_has_local_business_surface_context = source_tokens.iter().any(|token| {
                LOCAL_BUSINESS_DISCOVERY_CLASS_TOKENS.contains(&token.as_str())
                    || LOCAL_BUSINESS_COMPARISON_AXIS_TOKENS.contains(&token.as_str())
            });
            return local_business_scope_matches_source(locality_hint, url, title, excerpt)
                && source_has_local_business_surface_context;
        }
        return true;
    }
...
pub(crate) fn local_business_collection_surface_candidate(
    ...
    let last_segment_lower = last_segment.to_ascii_lowercase();
    let encoded_restaurant_listing = last_segment_lower.starts_with("restaurants-")
        || last_segment_lower.contains("-restaurants-");
    let slug_structurally_looks_like_collection =
        (!last_segment.ends_with('-') && !last_segment.chars().any(|ch| ch.is_ascii_digit()))
            || encoded_restaurant_listing;
    ...
    let category_tokens = {
        let title_tokens = local_business_entity_anchor_tokens(title, locality_hint)
            .into_iter()
            .collect::<Vec<_>>();
        if !title_tokens.is_empty() {
            title_tokens
        } else {
            local_business_target_candidate_from_url(url, locality_hint)
                .map(|candidate| {
                    local_business_entity_anchor_tokens(&candidate, locality_hint)
                        .into_iter()
                        .collect::<Vec<_>>()
                })
                .unwrap_or_default()
...
fn local_business_title_has_entity_context(title: &str) -> bool {
    let compact = compact_whitespace(title).to_ascii_lowercase();
    if compact.is_empty() {
        return false;
    }
    ...
    compact.contains(',')
        || compact.contains(" | ")
        || compact.contains(" - ")
        || compact.contains(" menu")
        || compact.contains(" dinner")
        || compact.contains(" lunch")
        || compact.contains(" brunch")
        || compact.contains(" breakfast")
        || compact.contains(" hours")
        || compact.contains(" review")
        || compact.contains(" reviews")
        || compact.contains(" photos")
        || compact.contains(" prices")
        || compact.contains(" reservations")
}
...
pub(crate) fn local_business_target_name_from_source(
    ...
    let title_candidate = source
        .title
        .as_deref()
        .and_then(|title| local_business_target_candidate_from_title(title, locality_hint))
        .filter(|candidate| {
            local_business_title_has_entity_context(source_title)
                || !local_business_target_matches_source_host(candidate, &source.url)
        });
    title_candidate.or_else(|| local_business_target_candidate_from_url(&source.url, locality_hint))
}
...
fn local_business_final_detail_source_allowed(
    ...
    if local_business_menu_surface_url(url) {
        return local_business_target_name_from_source(source, locality_hint)
            .or_else(|| local_business_detail_display_name(source, locality_hint))
            .map(|candidate| local_business_entity_name_allowed(&candidate, locality_hint))
            .unwrap_or(false);
    }
...
#[cfg(test)]
#[path = "local_business/local_business_menu_surface_tests.rs"]

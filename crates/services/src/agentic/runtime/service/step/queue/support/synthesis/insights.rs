...
pub(crate) fn derive_insight_hard_policy_gates(
    ...
    let passes_low_signal_scope = |idx: usize,
                                   signals: SourceSignalProfile,
                                   envelope_score: &CandidateEvidenceScore|
     -> bool {
        if !passes_low_priority_scope(idx, signals, envelope_score) {
    ...
    let require_successful_read_backing = ranked
        .iter()
        .filter(|(idx, signals, envelope_score)| {
            passes_low_signal_scope(*idx, *signals, envelope_score)
                && candidates[*idx].from_successful_read
        })
        .count()
        >= citations_per_story;
    ...
    InsightHardPolicyGates {
        ...
        require_constraint_resolution,
        require_successful_read_backing,
        reject_search_hub,
...
pub(crate) fn candidate_passes_insight_hard_policy(
    ...
    if hard_policy.require_successful_read_backing && !candidate.from_successful_read {
        return false;
    }
...
mod tests {
    use super::*;
    use crate::agentic::runtime::service::step::queue::envelope::{
        OutputContract, ProvenancePolicy,
    };

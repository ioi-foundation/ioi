...
fn briefing_grounded_recovery_required(
    query_contract: &str,
    retrieval_contract: &ioi_types::app::agentic::WebRetrievalContract,
    deterministic_plan: &crate::agentic::runtime::service::step::queue::support::PreReadCandidatePlan,
...
fn merge_deterministic_plan_with_pending_inventory(
    ...
    deterministic_plan: crate::agentic::runtime::service::step::queue::support::PreReadCandidatePlan,
    verification_checks: &mut Vec<String>,
) -> crate::agentic::runtime::service::step::queue::support::PreReadCandidatePlan {
...
fn briefing_authority_hint_read_recovery_urls(
    ...
    let domain_key_for_url = |url: &str| {
        crate::agentic::runtime::service::step::queue::support::source_host(url)
    ...
    let support_priority_urls = merged_hints
        ...
        .filter_map(|hint| {
            ...
            let grounded_primary_authority =
                crate::agentic::runtime::service::step::queue::support::source_has_grounded_primary_authority(
                    query_contract,
                    trimmed,
                    title,
                    excerpt,
    ...
    let distinct_domain_floor =
        crate::agentic::runtime::service::step::queue::support::retrieval_contract_required_distinct_domain_floor(
            Some(retrieval_contract),
            query_contract,
        )
    ...
    for url in &recovery_backfill_urls {
        ...
        if selected.iter().any(|existing| {
            existing.eq_ignore_ascii_case(url.as_str())
                || crate::agentic::runtime::service::step::queue::support::url_structurally_equivalent(existing, &url)
        }) {
            continue;
        }
...
fn semantic_alignment_recovery_query(
    ...
    if crate::agentic::web::contract_requires_geo_scoped_entity_expansion(retrieval_contract) {
        return crate::agentic::runtime::service::step::queue::web_pipeline::local_business_entity_discovery_query_contract(
            query_contract,
            locality_hint,
        );
    }
...
fn deterministic_local_business_discovery_seed_url(
    ...
    for source in discovery_sources {
        ...
        let collection_surface =
            crate::agentic::runtime::service::step::queue::support::local_business_collection_surface_candidate(
                locality_hint,
                trimmed,
                title,
                excerpt,
...
pub(crate) async fn maybe_handle_web_search(
    ...
    if briefing_probe_recovery_required {
        ...
        let briefing_probe_recovery_limit =
            constraint_grounded_search_limit(query_contract.as_str(), min_sources);
        let authority_hint_read_recovery_site_terms = if crate::agentic::runtime::service::step::queue::support::query_probe_document_authority_site_terms(
            query_contract.as_str(),
            Some(&retrieval_contract),
            &briefing_probe_recovery_source_hints,
        )
    ...
    if needs_locality_rebound_search {
        let rebound_query = if crate::agentic::web::contract_requires_geo_scoped_entity_expansion(
            ...
            crate::agentic::runtime::service::step::queue::web_pipeline::local_business_entity_discovery_query_contract(
                query_contract.as_str(),
                locality_hint.as_deref(),
            )
        } else {
    ...
    if total_queued_reads == 0 && !probe_queued && !rebound_queued {
        ...
        let terminal_completion_reason = completion_reason.filter(|reason| {
            crate::agentic::runtime::service::step::queue::support::web_pipeline_completion_terminalization_allowed(
                &pending,
                *reason,
                total_queued_reads,
            )
...
mod planning_regression_tests {
    use super::*;
    use crate::agentic::runtime::service::step::queue::support::PreReadCandidatePlan;

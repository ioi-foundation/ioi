...
fn briefing_authority_seed_admission(
    ...
    let query_grounded = crate::agentic::runtime::service::step::queue::support::excerpt_has_query_grounding_signal_with_contract(
    ...
    let identifier_bearing = crate::agentic::runtime::service::step::queue::support::source_has_briefing_standard_identifier_signal(
        query_contract,
        url,
        title,
        excerpt,
...
fn briefing_authority_link_expansion_required(
    ...
    let selection_quality =
        crate::agentic::runtime::service::step::queue::support::selected_source_quality_observation_with_contract_and_locality_hint(
            Some(retrieval_contract),
            query_contract,
            required_url_count as u32,
            &selected_urls,
...
fn briefing_authority_link_out_sources_from_html(
    ...
    fn cleaned_external_link_page_context(
        ...
        for segment in page_context.split('|') {
            ...
            let candidate = compact_whitespace(
                &crate::agentic::runtime::service::step::signals::strip_low_priority_leading_marker(
                    trimmed,
                ),
            );
    ...
    for (dom_index, anchor) in document.select(&selector).enumerate() {
        ...
        let external_link_public_authority_title_subject_overlap = !same_host
            && crate::agentic::runtime::service::step::queue::support::source_has_public_authority_host(&final_url)
            && {
                let query_native_tokens =
                    crate::agentic::runtime::service::step::queue::support::document_authority_query_tokens(query_contract);
                let title_tokens =
                    crate::agentic::runtime::service::step::queue::support::normalized_anchor_tokens(title);
...
#[cfg(test)]
#[path = "discovery/discovery_regression_tests.rs"]

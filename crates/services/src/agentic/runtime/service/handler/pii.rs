use super::super::RuntimeAgentService;
...
pub(crate) fn persist_pii_review_request(
    ...
    let key = crate::agentic::runtime::keys::pii::review::request(&request.decision_hash);
    let bytes = codec::to_bytes_canonical(request)?;
    state.insert(&key, &bytes)?;
    Ok(())
}

pub(crate) fn emit_pii_review_requested(service: &RuntimeAgentService, request: &PiiReviewRequest) {
    if let Some(tx) = &service.event_sender {

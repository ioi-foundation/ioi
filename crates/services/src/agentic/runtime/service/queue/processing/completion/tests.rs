...
fn completes_explicit_chat_reply_from_queue() {
    ...
    let session_id = agent_state.session_id;
    let mut success = true;
    let mut out = Some("Replied: npm run dev:hypervisor-code-editor-adapter-host".to_string());
    let mut err = None;
    let mut completion_summary = None;
    let mut verification_checks = Vec::new();
    let rules = crate::agentic::runtime::service::decision_loop::helpers::default_safe_policy();
    ...
    maybe_complete_chat_reply(
        ...
        &AgentTool::ChatReply {
            message:
                "In `package.json`, the npm script that launches the code editor adapter host is `dev:hypervisor-code-editor-adapter-host`."
                    .to_string(),
        },
        false,
        &mut success,
    ...
    assert!(completion_summary
        .as_deref()
        .is_some_and(|summary| summary.contains("`dev:hypervisor-code-editor-adapter-host`")));
    assert!(verification_checks
        .iter()

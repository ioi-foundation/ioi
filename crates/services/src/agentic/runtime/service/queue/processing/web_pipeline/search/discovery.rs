...
fn briefing_authority_link_expansion_required(
    ...
    let selection_quality =
        crate::agentic::runtime::service::queue::support::selected_source_quality_observation_with_contract_and_locality_hint(
            Some(retrieval_contract),
            query_contract,
            required_url_count as u32,
            &selected_urls,
...
fn briefing_authority_link_out_sources_from_html(
    ...
    fn cleaned_external_link_page_context(
        ...
        for segment in page_context.split('|') {
            ...
            let candidate = compact_whitespace(
                &crate::agentic::runtime::service::decision_loop::signals::strip_low_priority_leading_marker(
                    trimmed,
                ),
            );
            ...
            let signals =
                crate::agentic::runtime::service::decision_loop::signals::analyze_source_record_signals(
                    "",
                    "",
                    trimmed_candidate,
                );
            ...
            let grounded = crate::agentic::runtime::service::queue::support::excerpt_has_query_grounding_signal_with_contract(
                Some(retrieval_contract),
                query_contract,
                min_sources.max(1),
                "",
    ...
    for (dom_index, anchor) in document.select(&selector).enumerate() {
        ...
        let external_link_public_authority_title_subject_overlap = !same_host
            && crate::agentic::runtime::service::queue::support::source_has_public_authority_host(&final_url)
            && {
                let query_native_tokens =
                    crate::agentic::runtime::service::queue::support::document_authority_query_tokens(query_contract);
                let title_tokens =
                    crate::agentic::runtime::service::queue::support::normalized_anchor_tokens(title);
                let title_native_overlap = query_native_tokens.intersection(&title_tokens).count();
                title_native_overlap >= 2
                    || (title_native_overlap >= 1
                        && crate::agentic::runtime::service::queue::support::source_temporal_recency_score(
                            &final_url,
                            title,
                            external_grounding_surface,
                        ) > 0)
        ...
        let publication_signal = anchor
            ...
            || parsed_final_url
                ...
                .contains("/pubs/");
        let authority_score =
            crate::agentic::runtime::service::queue::support::source_document_authority_score(
                query_contract,
                &final_url,
                title,
                &snippet,

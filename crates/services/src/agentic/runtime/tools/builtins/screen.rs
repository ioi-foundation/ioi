{
    // Only expose Screen tools in VisualForeground (Tier 3)
    if tier == ExecutionTier::VisualForeground {
        ...
        tools.push(LlmToolDefinition {
            name: "screen__find".to_string(),
            description: "Find an on-screen element by semantic or visual description, including unlabeled icons. Returns coordinates. Use this to find Desktop icons or dock items if 'app__launch' fails.".to_string(),
            parameters: find_params.to_string(),
        });

        let screen_params = json!({
            "type": "object",
            "properties": {
                "action": {
                    "type": "string",
        ...
        tools.push(LlmToolDefinition {
            ...
            description: "Control the screen (mouse, keyboard, and pointer actions).".to_string(),
            parameters: screen_params.to_string(),
        });
    }
}

...
const GWS_DEFAULT_TIMEOUT_SECS: u64 = 120;
const BIGQUERY_TOOL_NAME: &str = "connector__google__bigquery_execute_query";
const BIGQUERY_READ_TARGET: &str = "connector__google__bigquery_execute_query__read";
const BIGQUERY_WRITE_TARGET: &str = "connector__google__bigquery_execute_query__write";
...
#[derive(Debug, Clone)]
struct GoogleConnectorActionSpec {
    id: &'static str,
    service: &'static str,
    ...
    fields: Vec<ConnectorFieldDefinition>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
enum ShieldDecisionMode {
    Auto,
    Confirm,
    Block,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ShieldGlobalPolicyDefaults {
    reads: ShieldDecisionMode,
    writes: ShieldDecisionMode,
    admin: ShieldDecisionMode,
    expert: ShieldDecisionMode,
    automations: ShieldAutomationMode,
    data_handling: ShieldDataHandlingMode,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ShieldConnectorPolicyOverride {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct ShieldPolicyState {
    #[allow(dead_code)]
    version: u8,
    #[serde(default)]
    global: ShieldGlobalPolicyDefaults,
    #[serde(default)]
    overrides: std::collections::BTreeMap<String, ShieldConnectorPolicyOverride>,
}
...
fn runtime_resume_already_authorizes_google_tool(
...
fn enforce_google_tool_shield_policy(
    service: &DesktopAgentService,
    agent_state: &AgentState,
    session_id: [u8; 32],
    spec: &GoogleConnectorActionSpec,
...
async fn configure_google_status() -> Result<ConnectorConfigureResult, String> {
    ...
    if auth.status == "connected" {
        if let Ok(bootstrap) = google_api::bootstrap_workspace_profile().await {

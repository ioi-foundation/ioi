...
use crate::agentic::desktop::keys::{AGENT_POLICY_PREFIX, get_state_key, get_session_result_key};
...
use ioi_types::app::{ActionRequest, ActionContext, KernelEvent};
...
pub async fn process_tool_output(
    ...
    match tool_call {
        Ok(tool) => {
             ...
             if let AgentTool::SystemFail { .. } = &tool { current_tool_name = "system__fail".to_string(); }
             if let AgentTool::AgentComplete { .. } = &tool { current_tool_name = "agent__complete".to_string(); }
                    ...
                    if s {
                        if let Some(entry) = history_entry.clone() {
                            let tool_msg = ioi_types::app::agentic::ChatMessage {
                                role: "tool".to_string(), content: entry.clone(),
                        ...
                        _ => {
                             ...
                             if let Some(tx) = &service.event_sender {
                                 let output_str = if success { 
                                     ...
                                     error_msg.clone().unwrap_or_else(|| "Unknown error".to_string()) 
                                 };

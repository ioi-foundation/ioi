...
use ioi_types::app::agentic::{ChatMessage, InferenceOptions, SemanticFact, StepTrace, SwarmManifest};
use ioi_types::codec;
...
pub async fn fetch_swarm_manifest(service: &DesktopAgentService, hash: [u8; 32]) -> Option<SwarmManifest> {
...
async fn extract_facts(service: &DesktopAgentService, text: &str) -> Vec<SemanticFact> {
    ...
    match service.reasoning_inference.execute_inference(model_hash, prompt.as_bytes(), options).await {
        Ok(bytes) => {
...
pub async fn inspect_frame(service: &DesktopAgentService, frame_id: u64) -> Result<String, TransactionError> {
    let scs_mutex = service.scs.as_ref().ok_or(TransactionError::Invalid("SCS not available".into()))?;
    
    let (payload, frame_type) = {
        let store = scs_mutex.lock().map_err(|_| TransactionError::Invalid("SCS lock".into()))?;
        let frame = store.toc.frames.get(frame_id as usize).ok_or(TransactionError::Invalid("Frame not found".into()))?;
        let payload = store.read_frame_payload(frame_id).map_err(|e| TransactionError::Invalid(e.to_string()))?;
    ...
    match frame_type {
        FrameType::Observation => {
            ...
            let options = InferenceOptions { max_tokens: 300, ..Default::default() };
            
            let out_bytes = service.fast_inference.execute_inference([0u8; 32], &input_bytes, options).await

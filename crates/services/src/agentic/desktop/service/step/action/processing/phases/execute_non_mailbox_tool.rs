...
fn synthesized_payload_hash_for_tool(tool: &AgentTool) -> Option<String> {
...
fn emit_completion_gate_status_event(
    service: &DesktopAgentService,
    session_id: [u8; 32],
    step_index: u32,
    intent_id: &str,
...
fn emit_completion_gate_violation_events(
    service: &DesktopAgentService,
    session_id: [u8; 32],
    step_index: u32,
    intent_id: &str,
...
pub(crate) async fn execute_non_mailbox_tool(
    ...
    let ExecuteNonMailboxToolContext {
        service,
        state,
        agent_state,
        call_context,
    ...
    let mut synthesized_payload_hash = synthesized_payload_hash_for_tool(&tool);
    ...
    let duplicate_command_execution =

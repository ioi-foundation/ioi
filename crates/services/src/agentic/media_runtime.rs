...
mod tests {
    ...
    async fn generate_image_falls_back_to_kernel_renderer_when_runtime_is_unsupported() {
        ...
        let image =
            image::load_from_memory(&result.image_bytes).expect("fallback bytes should decode");
    ...
    async fn edit_image_falls_back_to_masked_kernel_renderer_when_runtime_is_unsupported() {
        ...
        assert_eq!(result.mime_type, "image/png");
        let image =
            image::load_from_memory(&result.image_bytes).expect("fallback edit should decode");
    ...
    async fn generate_video_falls_back_to_kernel_animated_gif_when_runtime_is_unsupported() {
        ...
        let decoder =
            image::codecs::gif::GifDecoder::new(std::io::Cursor::new(&result.video_bytes))

...
pub(crate) fn connector_covers_endpoint(base_url: &str, endpoint: &str) -> bool {
    let (Some((bs, ba, bp)), Some((es, ea, ep))) = (url_parts(base_url), url_parts(endpoint))
    else {
        return false;
    };
    if bs != es || ba != ea {
...
fn validate_inputs(data_dir: &str, body: &Value) -> Result<SessionInputs, VErr> {
    if let Some(obj) = body.as_object() {
        ...
        {
            return Err(verr(
                "session_raw_query_rejected",
                "A connector session never carries a raw source query.".into(),
            ));
        }
        ...
        {
            return Err(verr(
                "session_env_fallback_rejected",
                "Environment-credential fallback is an authority bypass.".into(),
            ));
        }
        if SCOPE_KEYS
            .iter()
    ...
    if opt_s(body, "name").is_none() {
        return Err(verr(
            ...
            "A connector session requires a name.".into(),
        ));
    }
    ...
    .ok_or_else(|| {
        verr(
            "session_run_unknown",
            format!("materializing_run_id '{run_id}' does not resolve"),
        )
    })?;
    if s(&run, "status", "") != "lease_obtained" {
        return Err(verr(
            "session_lease_not_obtained",
            format!(
                "run '{run_id}' is '{}' — a session requires a run that HOLDS its lease",
                s(&run, "status", "")
            ),
        ));
    }
    if lease_expired(
        ...
        return Err(verr(
            ...
            "the run's lease has expired — re-acquire before opening a session".into(),
        ));
    }
    ...
    .ok_or_else(|| {
        verr(
            "session_ladder_drift",
            format!("the run's plan '{plan_id}' no longer resolves"),
        )
    })?;
    if let Err(drift) = crate::materializing_run_routes::check_plan_against_truth(data_dir, &plan) {
        return Err(verr(
            "session_ladder_drift",
            format!("the ladder no longer matches the lease scope: {drift}"),
        ));
    }
    ...
    .ok_or_else(|| {
        verr(
            "session_ladder_drift",
            format!("data source '{source_id}' no longer resolves"),
        )
    })?;
    ...
    let connector =
        find_by_key(data_dir, "connectors", "connector_id", &connector_id).ok_or_else(|| {
            verr(
...
fn bad(data_dir: &str, op: &str, err: VErr) -> (StatusCode, Json<Value>) {
    let _ = session_receipt(
        data_dir,
        "connector-session://unadmitted",
        op,
        &err.0,
        &err.1,
    );
...
pub(crate) async fn handle_sessions_list(State(st): State<Arc<DaemonState>>) -> Json<Value> {
    ...
    Json(
        json!({ "ok": true, "schema_version": SESSION_SCHEMA, "connector_sessions": items, "runtimeTruthSource": "daemon-runtime" }),
    )
}
...
pub(crate) async fn handle_session_get(
    ...
    match load_session(&st.data_dir, &id) {
        Some(r) => (
            StatusCode::OK,
            Json(json!({ "ok": true, "connector_session": r })),
        ),
        None => (
...
pub(crate) async fn handle_session_history(
    ...
    receipts
        .retain(|x| x.get("connector_session_ref").and_then(|v| v.as_str()) == Some(sref.as_str()));
    receipts.sort_by(|a, b| s(b, "at", "").cmp(&s(a, "at", "")));
    (
        StatusCode::OK,
        Json(
            json!({ "ok": true, "connector_session_ref": sref, "revision": r.get("revision"), "status": r.get("status"), "history": r.get("history").cloned().unwrap_or(json!([])), "receipts": receipts }),
        ),
    )
}
...
pub(crate) async fn handle_session_create(
    ...
    let receipt = session_receipt(
        ...
        "created",
        "ok",
        "connector session requested (no crossing yet)",
    );
...
pub(crate) async fn handle_session_open(
    ...
    if status == "session_obtained" {
        return (
            StatusCode::BAD_REQUEST,
            Json(
                json!({ "ok": false, "error": { "code": "session_already_obtained", "message": "the session is already open" } }),
            ),
        );
    }
    ...
    match authorize_capability_lease(&st, &lease_req).await {
        Err((code, challenge)) => {
            ...
            let _ = session_receipt(
                ...
                "session_refused",
                &reason,
                "gateway refused the crossing (challenge returned verbatim; no session opened)",
            );
            (code, Json(challenge))
        }
        Ok(lease) => {
...
pub(crate) async fn handle_session_release(
    State(st): State<Arc<DaemonState>>,
    AxumPath(id): AxumPath<String>,
) -> (StatusCode, Json<Value>) {
    let Some(mut record) = load_session(&st.data_dir, &id) else {
    ...
    if s(&record, "status", "") != "session_obtained" {
        return (
            StatusCode::BAD_REQUEST,
            Json(
                json!({ "ok": false, "error": { "code": "session_nothing_to_release", "message": "the session is not open" } }),
            ),
        );
    }
    let slid = record
        .pointer("/session/gateway_lease_id")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
    ...
    let run_executed = record
        .get("materializing_run_id")
        .and_then(|v| v.as_str())
        .and_then(|rid| {
            find_by_key(
                &st.data_dir,
    ...
    let receipt = session_receipt(
        ...
        "session_released",
        "ok",
        &session_release_summary(&slid, run_executed),
    );
...
pub(crate) async fn handle_session_cancel(
    State(st): State<Arc<DaemonState>>,
    AxumPath(id): AxumPath<String>,
) -> (StatusCode, Json<Value>) {
    let Some(mut record) = load_session(&st.data_dir, &id) else {
    ...
    if s(&record, "status", "") != "requested" {
        return (
            StatusCode::BAD_REQUEST,
            Json(
                json!({ "ok": false, "error": { "code": "session_terminal_immutable", "message": "only a requested session can be cancelled (release an open one)" } }),
            ),
        );
    }
    let receipt = session_receipt(
...
pub(crate) async fn handle_session_patch(
    State(st): State<Arc<DaemonState>>,
    AxumPath(id): AxumPath<String>,
...
mod connector_session_tests {
    ...
    fn connector_authority_binds_credential_to_endpoint() {
        ...
        assert!(connector_covers_endpoint(
            "http://127.0.0.1:18099",
            "http://127.0.0.1:18099/rows"
        ));
        assert!(connector_covers_endpoint(
            "https://db.invalid",
            "https://db.invalid/rows?limit=5"
        ));
        assert!(connector_covers_endpoint(
            "https://host/api",
            "https://host/api/loans"
        ));
        // different host:port → the confused-deputy case → refused
        assert!(!connector_covers_endpoint(
            "http://127.0.0.1:18099",
            "http://127.0.0.1:9/rows"
        ));
        assert!(!connector_covers_endpoint(
            "https://a.host",
            "https://b.host/rows"

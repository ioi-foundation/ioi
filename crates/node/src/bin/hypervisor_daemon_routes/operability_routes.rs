...
fn effective_policy(data_dir: &str, env: &Value) -> Value {
    let mut policy = std::fs::read(global_policy_path(data_dir)).ok()
    ...
    if let Some(g) = env.get("spec").and_then(|s| s.get("guardrails")) {
        for key in ["deny_commands", "deny_executables"] {
            if let Some(extra) = g.get(key).and_then(|v| v.as_array()) {
                ...
                if let Some(arr) = base { for e in extra { if !arr.contains(e) { arr.push(e.clone()); } } }
            }
        }
    }
    policy
        ...
        for p in pats {
            if let Some(pat) = p.as_str() {
                if cmd_n.contains(&norm(pat)) {
                    return Some(json!({ "denied": true, "rule": "deny_command", "matched": pat, "fail_closed": true }));
                }
            }
        }
    }
    if let Some(exes) = policy.get("deny_executables").and_then(|v| v.as_array()) {
        let denied: std::collections::HashSet<String> = exes.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect();
        ...
        for tok in command.split(|c: char| c.is_whitespace() || "|;&<>()`\"'".contains(c)) {
            ...
            if !base.is_empty() && denied.contains(base) {
                return Some(json!({ "denied": true, "rule": "deny_executable", "matched": base, "fail_closed": true }));
            }
        }
    }
    None
...
pub(crate) fn audit_guardrail_denial(data_dir: &str, env_id: &str, command: &str, denial: &Value) {
    ...
    let _ = persist_record(data_dir, "operability-audit", &id, &json!({
...
pub(crate) async fn handle_guardrails_get(State(st): State<Arc<DaemonState>>) -> Json<Value> {
    let policy = std::fs::read(global_policy_path(&st.data_dir)).ok()
        .and_then(|b| serde_json::from_slice::<Value>(&b).ok())
        .unwrap_or_else(default_policy);
...
pub(crate) async fn handle_guardrails_set(State(st): State<Arc<DaemonState>>, Json(body): Json<Value>) -> Json<Value> {
    let policy = json!({
    ...
    let _ = std::fs::write(global_policy_path(&st.data_dir), serde_json::to_vec_pretty(&policy).unwrap_or_default());
    let id = format!("pca_{:x}", nanos());
    let _ = persist_record(&st.data_dir, "operability-audit", &id, &json!({
        "schema_version": "ioi.hypervisor.operability-audit.v1",
...
pub(crate) async fn handle_operability_metrics(State(st): State<Arc<DaemonState>>) -> Json<Value> {
    ...
    for e in &envs {
        let phase = e["status"]["phase"].as_str().unwrap_or("unknown").to_string();
    ...
    let guardrail_denials = audit.iter().filter(|a| a["kind"] == "guardrail_denied").count();
    Json(json!({
...
pub(crate) async fn handle_incident_reconstruct(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>) -> Json<Value> {
    let incident = read_record_dir(&st.data_dir, "incidents").into_iter().find(|i| i["incident_id"].as_str() == Some(id.as_str()) || i["incident_ref"].as_str() == Some(id.as_str()));
    let Some(incident) = incident else { return Json(json!({ "ok": false, "reason": "incident not found" })); };
    let attempts: Vec<Value> = read_record_dir(&st.data_dir, "recovery-attempts").into_iter().filter(|a| a["incident_ref"].as_str() == Some(id.as_str())).collect();
    let receipts: Vec<Value> = read_record_dir(&st.data_dir, "receipts").into_iter().filter(|r| r["details"]["incident_ref"].as_str() == Some(id.as_str()) || r["kind"] == "environment_recovery").collect();
    Json(json!({
...
pub(crate) async fn handle_mcp_gateway_invoke(
    ...
    let gw = |v: Value| Ok(Json(json!({ "ok": true, "tool": tool.clone(), "result": v })));
    match tool.as_str() {
        "hv_create_env" => {
            ...
            let created = call(&base, "POST", "/v1/hypervisor/environments", Some(spec)).await.map_err(|e| AppError(axum::http::StatusCode::BAD_GATEWAY, e))?;
            let eid = created["environment"]["id"].as_str().unwrap_or_default().to_string();
            let _ = call(&base, "POST", &format!("/v1/hypervisor/environments/{eid}/start"), None).await;
            gw(json!({ "environment_id": eid }))
        }
        "hv_run_task" => {
            let eid = body.get("environment_id").and_then(|v| v.as_str()).unwrap_or_default();
            ...
            let r = call(&base, "POST", "/v1/hypervisor/exec", Some(json!({ "environment_id": eid, "command": cmd }))).await.map_err(|e| AppError(axum::http::StatusCode::BAD_GATEWAY, e))?;
            gw(r)
        }
        "hv_inspect_env" => {
            let eid = body.get("environment_id").and_then(|v| v.as_str()).unwrap_or_default();
            let env = call(&base, "GET", &format!("/v1/hypervisor/environments/{eid}"), None).await.map_err(|e| AppError(axum::http::StatusCode::BAD_GATEWAY, e))?;
            let s = &env["environment"]["status"];
            gw(json!({ "environment_id": eid, "phase": s["phase"], "readiness": s["readiness"], "components": s["components"], "ports": s["ports"] }))
        }
        "hv_cleanup_env" => {
            let eid = body.get("environment_id").and_then(|v| v.as_str()).unwrap_or_default();
            let r = call(&base, "POST", &format!("/v1/hypervisor/environments/{eid}/delete"), None).await.map_err(|e| AppError(axum::http::StatusCode::BAD_GATEWAY, e))?;
            gw(json!({ "environment_id": eid, "deleted": r["environment"]["status"]["deleted"].as_bool().unwrap_or(false) || r["status"].as_str() == Some("deleted") }))
        }
        other => Ok(Json(json!({ "ok": false, "reason": format!("unknown gateway tool '{other}'") }))),
    }
}

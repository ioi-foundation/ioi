...
// The publish scopes moved to `governed_authority::SCM_PUBLICATION_*`: the
// rebuilt publication plane owns a `scope:scm.publication.*` family disjoint
// from every other, and the abandon crossing keeps its own.
...
// The source-control publication crossing (`POST /v1/hypervisor/environments/:id/scm/publish`)
// and its GitHub pull-request call were REMOVED here and rebuilt in
// `scm_publication_routes.rs` against the registered ScmPublicationEffect contract
// (`schema://ioi/components/connectors-tools/scm-publication-effect/v1`). The route that stood
// here accepted a caller-supplied `remote_url`, staged the whole workspace with `git add -A`,
// pushed `--force` with no expected-head comparison, dropped every persist result, and returned
// `"ok": true` even when the pull request had failed. None of those shapes is representable in
// the rebuilt plane.

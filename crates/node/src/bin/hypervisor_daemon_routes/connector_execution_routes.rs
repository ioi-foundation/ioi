...
pub(crate) async fn handle_run_execute(
    State(st): State<Arc<DaemonState>>,
    AxumPath(id): AxumPath<String>,
    Json(body): Json<Value>,
) -> (StatusCode, Json<Value>) {
    let data_dir = st.data_dir.clone();
    let Some(mut run) = find_by_key(
        &data_dir,
    ...
    if let Some(obj) = body.as_object() {
            ...
            .any(|k| obj.contains_key(*k) && !obj[*k].is_null())
        {
            return refuse(
                ...
                "execution_plaintext_secret_rejected",
                "an execution request never carries credential material",
            );
        }
        if RAW_QUERY_KEYS
            .iter()
            .any(|k| obj.contains_key(*k) && !obj[*k].is_null())
        {
            return refuse(
                ...
                &run_ref,
                StatusCode::BAD_REQUEST,
                "execution_raw_query_rejected",
                "the adapter reads the DECLARED source shape only — caller queries never exist",
            );
        }
            ...
            .any(|k| obj.contains_key(*k) && !obj[*k].is_null())
        {
            return refuse(
                ...
                &run_ref,
                StatusCode::BAD_REQUEST,
                "execution_env_fallback_rejected",
                "environment-credential fallback is an authority bypass",
            );
        }
    }
    ...
    if status == "executed" {
        return (
            StatusCode::BAD_REQUEST,
            Json(
                json!({ "ok": false, "error": { "code": "execution_already_registered", "message": "this run already registered its batch — one bounded batch per run (v1); plan a new run for another batch" } }),
            ),
        );
    }
    if status != "lease_obtained" {
        return refuse(
            ...
            &run_ref,
            StatusCode::BAD_REQUEST,
            "execution_lease_not_obtained",
            &format!("run is '{status}' — execution requires a HELD lease"),
        );
    }
    if expired(
        run.pointer("/lease/expires_at").unwrap_or(&Value::Null),
        now_ms(),
    ) {
        return refuse(
            &data_dir,
    ...
    if limit == 0 || limit > MAX_BATCH_LIMIT {
        return refuse(
            ...
            &run_ref,
            StatusCode::BAD_REQUEST,
            "execution_limit_unbounded",
            &format!("limit must be 1..={MAX_BATCH_LIMIT}"),
        );
    }
    ...
    let Some(plan) = find_by_key(
        ...
        return refuse(
            ...
            &run_ref,
            StatusCode::BAD_REQUEST,
            "execution_ladder_drift",
            "the run's plan no longer resolves",
        );
    };
    ...
    let Some(source) = find_by_key(
        ...
        return refuse(
            ...
            StatusCode::BAD_REQUEST,
            "execution_ladder_drift",
            "the data source no longer resolves",
        );
    };
    ...
    let Some(session) = find_by_key(
        &data_dir,
        crate::connector_session_routes::RECORD_DIR,
        "id",
        &session_id,
    ) else {
        return refuse(
            ...
            &run_ref,
            StatusCode::BAD_REQUEST,
            "execution_session_unknown",
            "connector_session_id does not resolve",
        );
    };
    if session.get("materializing_run_id").and_then(|v| v.as_str()) != Some(id.as_str()) {
        return refuse(
            ...
            "the session is not tied to this run",
        );
    }
    if s(&session, "status", "") != "session_obtained" {
        return refuse(
            ...
            &format!(
                "session is '{}' — execution requires an OPEN sealed session",
                s(&session, "status", "")
            ),
        );
    }
    ...
    let Some(mut projection) = find_by_key(
        ...
        return refuse(
            ...
            StatusCode::BAD_REQUEST,
            "execution_ladder_drift",
            "the tied projection no longer resolves",
        );
    };
    let mapping_id = s(&plan, "connector_mapping_id", "");
    let Some(mapping) = find_by_key(
        &data_dir,
        ...
        return refuse(
            ...
            StatusCode::BAD_REQUEST,
            "execution_ladder_drift",
            "the mapping no longer resolves",
        );
    };
    ...
    let receipt = run_receipt(
        ...
        "ok",
        &format!(
            "read-only execution requested: kind {kind} · limit {limit} · session {}",
            s(&session, "id", "")
        ),
    );
    push_history(
    ...
    // System-scoped sources derive identity from the durable source record,
    // never from caller-supplied context or omission. Refuse before credential
    // resolution and before the network invoker if the per-System writer epoch,
    // membership/root, resource/effect, timing, revocation, or read posture is
    // stale. Non-System sources retain the existing bounded connector path.
    let source_resource_id = s(&source, "source_ref", "");
    let mut system_read_evidence =
        crate::distributed_fencing::durable_read_evidence_from_owner(&source).ok();
    if let Some(evidence) = system_read_evidence.as_mut() {
        // `read_record_dir` is a local file projection, not a linearized domain
        // read. Bind its actual posture and durable owner update watermark; the
        // active resource fence must explicitly permit that weaker posture.
        evidence.consistency =
            ioi_services::agentic::runtime::kernel::distributed_fencing::ReadConsistency::CachedProjection;
        evidence.watermark = s(&source, "updated_at", "");
    }
    let system_fence_admission =
        crate::distributed_fencing::generate_and_admit_system_scoped_effect(
            ...
            &body,
            session
                .pointer("/session/grant_ref")
                .and_then(Value::as_str),
            session
                .pointer("/session/expires_at")
                .and_then(Value::as_i64),
            true,
            system_read_evidence.as_ref(),
            &format!("connector-execution:{id}:{session_id}"),
            now_ms(),
        );
    let system_fence_admission = match system_fence_admission {
        ...
        Err(error) => {
            let receipt = run_receipt(
                ...
                &run_ref,
                "execution_refused",
                "execution_system_writer_fence_refused",
                &format!(
                    "System-owned connector read was refused before credential resolution or source contact: {error}"
                ),
            );
            push_history(
                &mut run,
                "execution_refused",
                "System writer fence refused connector read",
                &receipt,
            );
            let _ = persist_record(
                &data_dir,
            ...
            return (
                ...
                Json(json!({
                    "ok": false,
                    "error": {
                        "code": "execution_system_writer_fence_refused",
                        "message": "the active System writer fence did not admit this connector read"
                    }
                })),
            );
        }
    };
    if let Some(admission) = system_fence_admission {
        run[crate::distributed_fencing::EFFECT_CONTEXT_FIELD] =
            serde_json::to_value(admission.context).unwrap_or(Value::Null);
    }

    // Resolve the sealed credential IN-MEMORY for the read — used for the header, then dropped.
    let bearer: Option<String> = match find_by_key(
        &data_dir,
    ...
    let Some(bearer) = bearer else {
        let receipt = run_receipt(
            ...
            &run_ref,
            "execution_refused",
            "execution_credential_unresolved",
            "the sealed credential no longer resolves — the session's authority is stale",
        );
        push_history(
            &mut run,
            "execution_refused",
        ...
        return (
            ...
            Json(
                json!({ "ok": false, "error": { "code": "execution_credential_unresolved", "message": "the sealed credential no longer resolves" } }),
            ),
        );
    };
    ...
    let receipt = run_receipt(
        &data_dir,
        &run_ref,
        "source_contact_started",
        "ok",
        &format!(
            "GET {display_endpoint} (declared endpoint, verbatim; bearer attached, never recorded)"
        ),
    );
    push_history(
        &mut run,
        "source_contact_started",
        &format!("GET {display_endpoint}"),
        &receipt,
    );
    let started = now_ms();
        ...
        .build()
    {
        ...
        Err(e) => {
            return (
                ...
                Json(
                    json!({ "ok": false, "error": { "code": "execution_adapter_unavailable", "message": format!("adapter client could not initialize: {e}") } }),
                ),
            );
        }
    };
    ...
    let (http_status, body_text) = match resp {
        ...
        Err(e) => {
            let receipt = run_receipt(
                &data_dir,
                &run_ref,
                "source_contact_failed",
                "execution_source_unreachable",
                &format!("GET {display_endpoint} failed: {e}"),
            );
            push_history(
                &mut run,
                "source_contact_failed",
            ...
            return (
                ...
                Json(
                    json!({ "ok": false, "error": { "code": "execution_source_unreachable", "message": format!("the declared endpoint did not answer: {e}") } }),
                ),
            );
        }
    };
    ...
    if (300..400).contains(&(http_status as i32)) {
        ...
        return (
            StatusCode::BAD_GATEWAY,
            Json(
                json!({ "ok": false, "error": { "code": "execution_source_redirect_rejected", "message": "the declared endpoint answered with a redirect — refused; the adapter never follows redirects to undeclared URLs" } }),
            ),
        );
    }
    if !(200..300).contains(&(http_status as i32)) {
        let receipt = run_receipt(
            ...
            &run_ref,
            "source_contact_failed",
            "execution_source_error",
            &format!("GET {display_endpoint} → http {http_status}"),
        );
        push_history(
            &mut run,
            "source_contact_failed",
        ...
        return (
            StatusCode::BAD_GATEWAY,
            Json(
                json!({ "ok": false, "error": { "code": "execution_source_error", "message": format!("the declared endpoint answered http {http_status}") } }),
            ),
        );
    }
    let rows: Vec<Value> = match serde_json::from_str::<Value>(&body_text) {
        ...
        _ => {
            let receipt = run_receipt(
                ...
                &run_ref,
                "validation_result",
                "execution_source_shape_invalid",
                "the declared endpoint did not return a JSON array of rows",
            );
            push_history(
                &mut run,
                "validation_result",
            ...
            return (
                ...
                Json(
                    json!({ "ok": false, "error": { "code": "execution_source_shape_invalid", "message": "expected a JSON array of row objects" } }),
                ),
            );
        }
    };
    ...
    push_history(
        &mut run,
        "source_contact_completed",
        &format!("{fetched} rows fetched"),
        &receipt,
    );
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        for (pid, sf, st_, role) in &bindings {
            ...
            if !value_matches_source_type(&v, st_) {
                errors.push(format!(
                    "row {i}: source field '{sf}' (→ {pid}) does not match declared type '{st_}'"
                ));
                continue;
            }
            if *role == "key" {
    ...
    if !errors.is_empty() {
        ...
        let receipt = run_receipt(
            ...
            &format!(
                "batch REJECTED all-or-nothing: {} error(s), zero objects registered — {}",
                errors.len(),
                errors.join("; ")
            ),
        );
        ...
        return (
            StatusCode::UNPROCESSABLE_ENTITY,
            Json(
                json!({ "ok": false, "error": { "code": "execution_batch_invalid", "message": "one or more rows failed validation — the whole batch is rejected (no partial truth)", "errors": errors } }),
            ),
        );
    }
    let receipt = run_receipt(
        ...
        "ok",
        &format!(
            "all {} rows validated against the typed ladder",
            objects.len()
        ),
    );
    ...
    if let Err(e) = persist_record(&data_dir, SET_DIR, &set_id, &set) {
        let receipt = run_receipt(
            ...
            &run_ref,
            "execution_refused",
            "execution_output_persist_failed",
            &format!("output persist failed after the pre-output receipt: {e}"),
        );
        push_history(
            &mut run,
            "execution_refused",
        ...
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(
                json!({ "ok": false, "error": { "code": "execution_output_persist_failed", "message": "output could not persist" } }),
            ),
        );
    }
    ...
    let rollback_set = |why: &str| {
        ...
        let _ = run_receipt(
            ...
            &format!(
                "finalization failed ({why}) — the set was ROLLED BACK; no partial truth remains"
            ),
        );
    };
    ...
    if let Err(e) = persist_record(
        ...
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(
                json!({ "ok": false, "error": { "code": "execution_finalize_failed", "message": "the projection flip could not persist — the set was rolled back; no partial truth remains" } }),
            ),
        );
    }
    ...
    push_history(
        &mut run,
        "materialized_output_registered",
    ...
    if let Err(e) = persist_record(
        ...
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(
                json!({ "ok": false, "error": { "code": "execution_finalize_failed", "message": "the run state could not persist — projection restored, set rolled back (the run stays re-executable with no live set beside it)" } }),
            ),
        );
    }
    (
        StatusCode::OK,
        Json(json!({ "ok": true, "materializing_run": run, "materialized_object_set": set })),
    )
}
...
pub(crate) async fn handle_sets_list(State(st): State<Arc<DaemonState>>) -> Json<Value> {
    ...
    Json(
        json!({ "ok": true, "schema_version": SET_SCHEMA, "materialized_object_sets": items, "runtimeTruthSource": "daemon-runtime" }),
    )
}
...
pub(crate) async fn handle_sets_overview(State(st): State<Arc<DaemonState>>) -> Json<Value> {
    ...
    let objects: u64 = items
        .iter()
        .map(|r| r.get("count").and_then(|v| v.as_u64()).unwrap_or(0))
        .sum();
    Json(json!({
        "ok": true,
...
pub(crate) async fn handle_set_get(
    State(st): State<Arc<DaemonState>>,
    AxumPath(id): AxumPath<String>,
) -> (StatusCode, Json<Value>) {
    match find_by_key(&st.data_dir, SET_DIR, "id", &id) {
        Some(r) => (
            StatusCode::OK,
            Json(json!({ "ok": true, "materialized_object_set": r })),
        ),
        None => (
            StatusCode::NOT_FOUND,
            Json(json!({ "ok": false, "reason": "materialized object set not found" })),
        ),
    }
}
...
mod connector_execution_tests {
    ...
    fn run_bindings_always_include_key_and_title() {
        ...
        let b = run_bindings(
            &mapping,
            &vec!["loan_id".into(), "title".into(), "amount".into()],
        );
    ...
    fn endpoint_redaction_strips_userinfo_and_query() {
        ...
        assert_eq!(
            redacted_endpoint("https://host/rows?api_key=X"),
            "https://host/rows?…redacted"
        );
        assert_eq!(
            redacted_endpoint("https://u:p@host/rows"),
            "https://host/rows"
        );
        assert_eq!(
            redacted_endpoint("https://u:p@host/rows?t=1"),
            "https://host/rows?…redacted"
        );
    }

...
fn wallet_network_endpoint() -> Option<String> {
    std::env::var("IOI_WALLET_NETWORK_URL").ok().filter(|s| !s.trim().is_empty())
}
...
fn active_authority_mode() -> String {
    std::env::var("IOI_AUTHORITY_MODE").ok().filter(|s| !s.trim().is_empty()).unwrap_or_else(|| "local_operator".into())
}
...
fn live_grant_status(grant: &Value) -> &'static str {
    ...
    if grant.get("revoked").and_then(|v| v.as_bool()).unwrap_or(false) {
        return "revoked";
    }
    let expires = grant.get("expires_at_unix").and_then(|v| v.as_i64()).unwrap_or(0);
...
fn load_grant(data_dir: &str, grant_id: &str) -> Option<Value> {
    read_record_dir(data_dir, "authority-grants")
        .into_iter()
        .find(|g| g.get("grant_id").and_then(|v| v.as_str()) == Some(grant_id)
...
fn emit_receipt(data_dir: &str, event: &str, grant_ref: &str, subject: &str, action: &str, detail: &str) -> String {
...
pub(crate) async fn handle_authority_grant(
    ...
    let subject = body.get("subject").and_then(|v| v.as_str()).unwrap_or("operator").to_string();
    ...
    let policy_hash = body.get("policy_hash").and_then(|v| v.as_str()).unwrap_or("policy:default").to_string();
    let budget = body.get("budget").cloned().unwrap_or(Value::Null);
    let spend = budget.get("spend").and_then(|v| v.as_i64()).unwrap_or(0);
    let expiry_seconds = body.get("expiry_seconds").and_then(|v| v.as_i64()).unwrap_or(3600);
    ...
    let (decision, reason): (&str, String) = if LOCAL_ALLOWED.contains(&action.as_str()) {
        ("no_grant_needed", format!("'{action}' is within local_operator authority; no portable grant required"))
    } else if !WALLET_REQUIRED.contains(&action.as_str()) {
        ("denied", format!("'{action}' is not a recognized portable crossing; denied fail-closed"))
        ...
        ("granted", format!("enterprise policy admits '{action}' for {subject}"))
    };

    let now = now_unix();
    let expires_at_unix = if decision == "granted" { now + expiry_seconds } else { 0 };
    let receipt_event = if decision == "granted" { "granted" } else { "denied" };
    let receipt_ref = emit_receipt(data_dir, receipt_event, &grant_ref, &subject, &action, &reason);
...
pub(crate) async fn handle_authority_revoke(
    ...
    let data_dir = &st.data_dir;
    let key = body
        .get("grant_id").and_then(|v| v.as_str())
        .or_else(|| body.get("grant_ref").and_then(|v| v.as_str()))
        .unwrap_or("");
    let Some(mut grant) = load_grant(data_dir, key) else {
        return Json(json!({ "ok": false, "reason": format!("grant '{key}' not found"), "at": iso_now() }));
    };
    if grant.get("decision").and_then(|v| v.as_str()) != Some("granted") {
        return Json(json!({ "ok": false, "reason": "only granted authority can be revoked", "decision": grant.get("decision"), "at": iso_now() }));
    }
    let grant_id = grant.get("grant_id").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let grant_ref = grant.get("grant_ref").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let subject = grant.get("subject").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    grant["revoked"] = json!(true);
    grant["revoked_at"] = json!(iso_now());
    let receipt_ref = emit_receipt(data_dir, "revoked", &grant_ref, &subject, &action, "operator/enterprise revoke");
    if let Some(arr) = grant.get_mut("authority_receipt_refs").and_then(|v| v.as_array_mut()) {
        arr.push(json!(receipt_ref));
    }
    let _ = persist_record(data_dir, "authority-grants", &grant_id, &grant);
    Json(json!({ "ok": true, "grant_ref": grant_ref, "status": "revoked", "receipt_ref": receipt_ref, "at": iso_now() }))
}
...
pub(crate) async fn handle_authority_grants_list(State(st): State<Arc<DaemonState>>) -> Json<Value> {
...
pub(crate) async fn handle_authority_preflight(
    ...
    let data_dir = &st.data_dir;
    let effect = body.get("effect").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let environment_id = body.get("environment_id").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let grant_key = body.get("grant_ref").and_then(|v| v.as_str())
    ...
    if LOCAL_ALLOWED.contains(&effect.as_str()) {
        let receipt = emit_receipt(data_dir, "preflight_admit", "grant:local_operator", &environment_id, &effect, "local authority");
        return Json(json!({
    ...
    if !WALLET_REQUIRED.contains(&effect.as_str()) {
        let receipt = emit_receipt(data_dir, "preflight_block", "", &environment_id, &effect, "unrecognized effect");
        return Json(json!({
            "schema_version": "ioi.hypervisor.authority-preflight.v1",
            "effect": effect, "environment_id": environment_id,
    ...
    let block = |reason: &str, re_auth: bool, receipt: String| Json(json!({
        "schema_version": "ioi.hypervisor.authority-preflight.v1",
        "effect": effect, "environment_id": environment_id,
    ...
    let Some(key) = grant_key else {
        let receipt = emit_receipt(data_dir, "preflight_block", "", &environment_id, &effect, "no grant presented");
        return block(&format!("'{effect}' requires a portable authority grant; none presented"), true, receipt);
    };
    let Some(grant) = load_grant(data_dir, key) else {
        let receipt = emit_receipt(data_dir, "preflight_block", key, &environment_id, &effect, "grant not found");
        return block(&format!("grant '{key}' not found"), true, receipt);
    };
    let grant_ref = grant.get("grant_ref").and_then(|v| v.as_str()).unwrap_or(key).to_string();
    match live_grant_status(&grant) {
        "revoked" => {
            let receipt = emit_receipt(data_dir, "preflight_block", &grant_ref, &environment_id, &effect, "grant revoked");
            block("grant has been revoked; execution refused", true, receipt)
        }
        "expired" => {
            let receipt = emit_receipt(data_dir, "preflight_block", &grant_ref, &environment_id, &effect, "grant expired");
            block("grant expired; re-authentication required, no execution", true, receipt)
        }
        "denied" => {
            let receipt = emit_receipt(data_dir, "preflight_block", &grant_ref, &environment_id, &effect, "grant was denied");
            block("authority was denied; fail closed", true, receipt)
        }
        "active" => {
            let covers = grant.get("action").and_then(|v| v.as_str()) == Some(effect.as_str());
            if !covers {
                let receipt = emit_receipt(data_dir, "preflight_block", &grant_ref, &environment_id, &effect, "grant action does not cover effect");
                return block(&format!("grant covers '{}', not '{effect}'", grant.get("action").and_then(|v| v.as_str()).unwrap_or("")), false, receipt);
            }
            let receipt = emit_receipt(data_dir, "preflight_admit", &grant_ref, &environment_id, &effect, "valid portable grant");
            Json(json!({
                "schema_version": "ioi.hypervisor.authority-preflight.v1",
                "effect": effect, "environment_id": environment_id,
        ...
        other => {
            let receipt = emit_receipt(data_dir, "preflight_block", &grant_ref, &environment_id, &effect, other);
            block(&format!("grant status '{other}'; fail closed"), true, receipt)
        }
    }
}
...
pub(crate) async fn handle_authority_receipts(State(st): State<Arc<DaemonState>>) -> Json<Value> {
    ...
    receipts.sort_by(|a, b| {
        b.get("receipt_id").and_then(|v| v.as_str()).unwrap_or("")

...
pub(crate) async fn handle_goal_runs_create(
        ...
        let requested_system_path = path_object.get("requested_path").and_then(Value::as_str)
            ...
            || path_object
                ...
                .is_some_and(|references| {
                    references
                        .iter()
                        .any(|reference| reference.as_str() == Some("policy://ioi/m4/hosted-only"))
                })
            || body
                .get("target_system_id")
                .is_some_and(|value| !value.is_null());
        if requested_system_path {
            // The daemon-hosted System-bound lane (the M4 hosted collective: a GoalRun admitted
            // under a `package://ioi/outcome-room` System) retired with the room plane
            // (R-178 slice S4d-1, 2026-09-18): no consumer sent it and its precondition can no
            // longer exist. A GoalRun's membership in a bounded System is composed by the ioi.ai
            // application over the orchestration, never minted here.
            return bad(
                StatusCode::GONE,
                "goal_run_system_path_retired",
                "The daemon-hosted System-bound GoalRun lane retired with the room plane; a GoalRun joins a bounded System through the application's orchestration.",
            );
        } else {
    ...
    bad(
        ...
        "A GoalRun requires an exact admission path request; the legacy implicit path is retired.",
    )
}
...
/// A GoalRun record that still names a hosted OutcomeRoom is data from before 2026-09-17: the
/// room-scoped WorkResult and OutcomeDelta lane retired with the room plane (R-178 slice S4c-2,
/// R-183) and the GoalRun's own composition over the orchestration follows in slice S4d. Until
/// then such a record admits no result or delta through this route — refused by name, never
/// silently routed through the generic lane its binding forbids.
fn retired_outcome_room_binding_refusal() -> HttpRefusal {
    bad(
        StatusCode::GONE,
        "goal_run_outcome_room_ref_retired",
        "This GoalRun names a hosted OutcomeRoom; the room-scoped result lane retired with the room plane and this GoalRun's composition over the orchestration follows in slice S4d.",
    )
}
...
pub(crate) async fn handle_goal_run_result_create(
    ...
    if !room_ref.is_empty() {
        return retired_outcome_room_binding_refusal();
    }
    for plane_owned in [
        "goal_run_ref",
        "outcome_room_ref",

...
fn nanos() -> u128 { SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0) }
fn safe(seg: &str) -> String { seg.replace(|c: char| !c.is_ascii_alphanumeric() && c != '-' && c != '_', "_") }
fn sstr(v: &Value, k: &str) -> Option<String> { v.get(k).and_then(|x| x.as_str()).map(str::to_string) }

fn load_env(data_dir: &str, id: &str) -> Option<Value> {
    let path = Path::new(data_dir).join("environments").join(format!("{}.json", safe(id)));
...
fn workspace_of(data_dir: &str, env_id: &str) -> Option<String> {
    load_env(data_dir, env_id)?["status"]["workspace_root"].as_str().map(str::to_string)
}
/// Strip a binding/environment ref prefix ("environment:env_x" -> "env_x").
fn ref_id(r: &str) -> &str { r.rsplit(':').next().unwrap_or(r) }
...
pub(crate) async fn handle_binding_create(State(st): State<Arc<DaemonState>>, Json(body): Json<Value>) -> Json<Value> {
...
fn load_binding(data_dir: &str, id: &str) -> Option<Value> {
    let bid = ref_id(id);
    read_record_dir(data_dir, "session-execution-bindings").into_iter()
...
pub(crate) async fn handle_binding_get(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>) -> Json<Value> {
...
pub(crate) async fn handle_binding_events(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>) -> Json<Value> {
    let Some(b) = load_binding(&st.data_dir, &id) else { return Json(json!({ "error": { "code": "not_found", "binding": id } })); };
    let binding_ref = b.get("binding_ref").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    if let Some(t) = b.get("thread_ref").and_then(|v| v.as_str()) { events.push(stamp("thread", "thread_ref", json!(t))); }
    if let Some(w) = b.get("work_run_ref").and_then(|v| v.as_str()) { events.push(stamp("work_run", "work_run_ref", json!(w))); }
    Json(json!({ "binding_ref": binding_ref, "event_stream_refs": b.get("event_stream_refs"), "events": events, "at": iso_now() }))
}
...
pub(crate) async fn handle_binding_input(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>, Json(_body): Json<Value>) -> Json<Value> {
    let Some(b) = load_binding(&st.data_dir, &id) else { return Json(json!({ "ok": false, "reason": "binding not found" })); };
    match b.get("thread_ref").and_then(|v| v.as_str()) {
        Some(t) => Json(json!({ "ok": true, "routed_to": t, "route": format!("/v1/threads/{}/turns", ref_id(t)), "note": "operator input is a thread turn; the binding only resolves the owner route" })),
        None => Json(json!({ "ok": false, "reason": "binding has no thread_ref; bind a thread (POST /v1/threads) before sending input" })),
    }
}

fn binding_lifecycle(data_dir: &str, id: &str, action: &str, env_route: &str) -> Json<Value> {
    let Some(b) = load_binding(data_dir, id) else { return Json(json!({ "ok": false, "reason": "binding not found" })); };
    let env_ref = b.get("environment_ref").and_then(|v| v.as_str()).unwrap_or("");
    let receipt = emit_receipt(data_dir, "binding", id, action);
    Json(json!({ "ok": true, "binding": id, "action": action, "environment_ref": env_ref,
...
pub(crate) async fn handle_binding_stop(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>) -> Json<Value> { binding_lifecycle(&st.data_dir, &id, "stop", "/v1/hypervisor/environments") }
pub(crate) async fn handle_binding_archive(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>) -> Json<Value> { binding_lifecycle(&st.data_dir, &id, "archive", "/v1/hypervisor/environments") }
pub(crate) async fn handle_binding_restore(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>) -> Json<Value> { binding_lifecycle(&st.data_dir, &id, "restore", "/v1/hypervisor/environments") }
...
fn scoped_path(ws: &str, rel: &str) -> Result<PathBuf, String> {
    ...
    if rel.split('/').any(|c| c == "..") { return Err("path traversal ('..') is not allowed".into()); }
    let root = Path::new(ws);
    let joined = root.join(rel);
    // The parent (for writes) or the path itself (for reads) must canonicalize within the root.
    let check = if joined.exists() { joined.clone() } else { joined.parent().map(Path::to_path_buf).unwrap_or_else(|| root.to_path_buf()) };
    let canon_root = root.canonicalize().map_err(|e| e.to_string())?;
    if let Ok(canon) = check.canonicalize() {
        if !canon.starts_with(&canon_root) { return Err("path escapes the environment workspace".into()); }
    }
    Ok(joined)
}
...
pub(crate) async fn handle_env_files(State(st): State<Arc<DaemonState>>, Json(body): Json<Value>) -> Json<Value> {
    let env_id = sstr(&body, "environment_id").or_else(|| sstr(&body, "environment_ref").map(|r| ref_id(&r).to_string())).unwrap_or_default();
    let op = sstr(&body, "op").unwrap_or_default();
    let Some(ws) = workspace_of(&st.data_dir, &env_id) else {
        return Json(json!({ "ok": false, "reason": "environment not started (no scoped workspace)", "environment_id": env_id }));
    };
    ...
    let res: Result<Value, String> = (|| match op.as_str() {
        ...
        "read" => {
            ...
            Ok(json!({ "path": rel, "content": String::from_utf8_lossy(&bytes), "bytes": bytes.len() }))
        }
        "write" => {
            let p = scoped_path(&ws, &rel)?;
    ...
    match res {
        Ok(v) => Json(json!({ "ok": true, "op": op, "environment_id": env_id, "scope_root": ws, "result": v })),
        Err(reason) => Json(json!({ "ok": false, "op": op, "environment_id": env_id, "reason": reason })),
    }
}
...
pub(crate) async fn handle_terminal_create(State(st): State<Arc<DaemonState>>, Json(body): Json<Value>) -> Json<Value> {
    let env_ref = sstr(&body, "environment_ref").or_else(|| sstr(&body, "environment_id")).unwrap_or_default();
    let env_id = ref_id(&env_ref).to_string();
    let cwd = match sstr(&body, "cwd").or_else(|| workspace_of(&st.data_dir, &env_id)) {
        Some(c) => c,
        None => return Json(json!({ "ok": false, "reason": "environment not started (no scoped workspace) and no cwd given" })),
    };
    ...
    if rc != 0 { return Json(json!({ "ok": false, "reason": format!("openpty failed: {}", std::io::Error::last_os_error()) })); }
    ...
    unsafe {
        cmd.pre_exec(|| {
            ...
            if libc::ioctl(0, libc::TIOCSCTTY as _, 0) == -1 { return Err(std::io::Error::last_os_error()); }
            Ok(())
        });
    }
    let child = match cmd.spawn() {
        Ok(c) => c,
        Err(e) => { unsafe { libc::close(master); } return Json(json!({ "ok": false, "reason": format!("shell spawn failed: {e}") })); }
    };
    ...
    let log_path = log_dir.join(format!("{id}.log")).to_string_lossy().to_string();
        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                if let Ok(mut b) = buffer.lock() {
                    ...
                    if b.len() > 1_048_576 { let cut = b.len() - 1_048_576; b.drain(0..cut); }
                }
                use std::io::Write;
                if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open(&log_path) { let _ = f.write_all(chunk); }
            }
        });
    }

    let receipt = emit_receipt(&st.data_dir, "terminal", &id, "open");
    let session = TerminalSession { master_fd: master, child, buffer, environment_ref: env_ref.clone(), log_path: log_path.clone(), shell: shell.clone(), cols, rows };
...
pub(crate) async fn handle_terminal_stream(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>, Query(q): Query<HashMap<String, String>>) -> impl axum::response::IntoResponse {
    ...
    match terms.get(&id) {
        ...
        None => sse.push_str(&format!("event: error\ndata: {}\n\n", json!({ "code": "not_found", "terminal": id }))),
    }
...
pub(crate) async fn handle_terminal_input(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>, Json(body): Json<Value>) -> Json<Value> {
    let mut data = sstr(&body, "data").unwrap_or_default();
    if body.get("enter").and_then(|v| v.as_bool()).unwrap_or(false) && !data.ends_with('\n') { data.push('\n'); }
    let terms = st.terminals.lock().unwrap();
    let Some(t) = terms.get(&id) else { return Json(json!({ "ok": false, "reason": "terminal not found" })); };
    let bytes = data.as_bytes();
    let n = unsafe { libc::write(t.master_fd, bytes.as_ptr() as *const libc::c_void, bytes.len()) };
...
pub(crate) async fn handle_terminal_resize(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>, Json(body): Json<Value>) -> Json<Value> {
    let mut terms = st.terminals.lock().unwrap();
    let Some(t) = terms.get_mut(&id) else { return Json(json!({ "ok": false, "reason": "terminal not found" })); };
    t.cols = body.get("cols").and_then(|v| v.as_u64()).unwrap_or(t.cols as u64) as u16;
    t.rows = body.get("rows").and_then(|v| v.as_u64()).unwrap_or(t.rows as u64) as u16;
...
pub(crate) async fn handle_terminal_close(State(st): State<Arc<DaemonState>>, AxumPath(id): AxumPath<String>) -> Json<Value> {
    let mut terms = st.terminals.lock().unwrap();
    match terms.remove(&id) {
        Some(mut t) => {
            ...
            Json(json!({ "ok": true, "terminal_id": id, "closed": true, "environment_ref": t.environment_ref, "log_ref": t.log_path, "receipt_ref": receipt }))
        }
...
pub(crate) async fn handle_terminals_list(State(st): State<Arc<DaemonState>>) -> Json<Value> {
    ...
    Json(json!({ "schema_version": "ioi.hypervisor.terminals.v1", "terminals": list, "at": iso_now() }))
}

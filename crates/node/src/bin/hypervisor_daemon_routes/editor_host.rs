...
fn dirs_home() -> PathBuf { std::env::var("HOME").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from("/root")) }

/// The pinned openvscode-server entrypoint (installed by the WS-2 provisioner).
pub(crate) fn oss_server_bin() -> PathBuf { runtime_root().join("openvscode-server/bin/openvscode-server") }
...
fn http_get_version(port: u16) -> Option<String> {
    ...
    stream.write_all(format!("GET /version HTTP/1.1\r\nHost: 127.0.0.1:{port}\r\nConnection: close\r\n\r\n").as_bytes()).ok()?;
    let mut buf = Vec::new();
    let _ = stream.read_to_end(&mut buf);
    let text = String::from_utf8_lossy(&buf);
    let mut parts = text.splitn(2, "\r\n\r\n");
    let head = parts.next().unwrap_or("");
    if !head.starts_with("HTTP/1.1 2") && !head.starts_with("HTTP/1.0 2") { return None; }
    ...
    let commit = body.split(|c: char| !c.is_ascii_hexdigit())
...
pub(crate) async fn start_oss_runtime(st: &DaemonState, service_id: &str, svc: &Value) -> Result<Value, String> {
    ...
    let log_path = log_dir.join(format!("{service_id}.log")).to_string_lossy().to_string();
    ...
    let session_ref = svc.get("session_ref").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let lease_ref = svc.get("access_lease_ref").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    cmd.arg("--without-connection-token")
        ...
        .env("IOI_HYPERVISOR_EDITOR_SERVICE_REF", format!("environment_service:editor_{service_id}"))
    ...
    unsafe { cmd.pre_exec(|| { if libc::setsid() == -1 { return Err(std::io::Error::last_os_error()); } Ok(()) }); }
    let child = cmd.spawn().map_err(|e| format!("spawn openvscode-server: {e}"))?;

    // Wait for real /version readiness (non-blocking: async sleeps between quick probes).
    let started_at = super::iso_now();
    let mut version: Option<String> = None;
    for _ in 0..80 {
        if let Some(v) = http_get_version(port) { version = Some(v); break; }
    ...
    st.editor_runtimes.lock().unwrap().insert(service_id.to_string(), EditorRuntime {
        child, internal_port: port, log_path: log_path.clone(), started_at: started_at.clone(), workspace_root: workspace_root.clone(),
    });
...
pub(crate) fn read_runtime_log(data_dir: &str, service_id: &str) -> Value {
    let p = Path::new(data_dir).join("editor-services").join(format!("{service_id}.log"));
    match std::fs::read_to_string(&p) {
        Ok(s) => json!(s.lines().rev().take(50).collect::<Vec<_>>().into_iter().rev().collect::<Vec<_>>().join("\n")),
        Err(_) => json!(""),
    }
}
...
pub(crate) fn reconcile_editor_services(data_dir: &str) -> usize {
    ...
    if let Ok(entries) = std::fs::read_dir(&dir) {
        for e in entries.flatten() {
            ...
            let Ok(mut svc) = serde_json::from_slice::<Value>(&bytes) else { continue; };
            if svc.get("phase").and_then(|v| v.as_str()) == Some("ready") {
                ...
                let _ = std::fs::write(e.path(), serde_json::to_vec_pretty(&svc).unwrap_or_default());
                reconciled += 1;
            }
        }
    }

...
use super::{
    BridgeError, APPLY_PATCH_MAX_DIFF_BYTES, APPLY_PATCH_MAX_EDITS, APPLY_PATCH_MAX_FILE_BYTES,
    CODING_TOOL_RESULT_SCHEMA_VERSION, DEFAULT_PREVIEW_BYTES, DEFAULT_PREVIEW_LINES,
...
pub(super) fn apply_workspace_patch(
    workspace_root: &str,
    input: &Value,
) -> Result<PatchOutcome, BridgeError> {
    let root = fs::canonicalize(workspace_root)
        .map_err(|error| BridgeError::new("workspace_root_invalid", error.to_string()))?;
...
pub(super) struct PatchOutcome {
    ...
    pub(super) transition: Option<PatchTransition>,
}

pub(super) struct PatchTransition {
    pub(super) operation_ref: String,
    pub(super) payload_ref: String,
    pub(super) expected_heads: Vec<String>,
    pub(super) state_root_before: String,
    pub(super) state_root_after: String,
    pub(super) resulting_head: String,
}

pub(super) struct PatchDiffPreview {
    text: String,
    bytes: usize,
    hash: String,
    truncated: bool,
...
pub(super) struct PatchEditApplication {
    text: String,
    summary: Value,
}

pub(super) enum PatchEdit {
    Replace {
        old_text: String,
        new_text: String,
        occurrence: String,
...
pub(super) fn normalize_patch_edits(input: &Value) -> Result<Vec<PatchEdit>, BridgeError> {
...
pub(super) fn patch_edit_from_value(value: &Value) -> Result<PatchEdit, BridgeError> {
...
pub(super) fn count_occurrences(text: &str, needle: &str) -> usize {
    if needle.is_empty() {
        return 0;
    }
    let mut count = 0;
...
pub(super) fn text_diff_preview(
    relative_path: &str,
    before: &str,
    after: &str,
...
pub(super) fn safe_subprocess_env(overrides: &[(String, String)]) -> Vec<(String, String)> {
...
pub(super) fn env_key_allowed(key: &str) -> bool {
    if key.is_empty() {
        return false;
    }
...
pub(super) fn metadata_mtime_ms(metadata: &fs::Metadata) -> Option<u128> {
    metadata
        .modified()
        .ok()

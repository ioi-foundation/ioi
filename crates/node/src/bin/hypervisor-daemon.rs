...
#[path = "hypervisor_daemon_routes/distributed_fencing.rs"]
mod distributed_fencing;
#[path = "hypervisor_daemon_routes/work_lifecycle.rs"]
mod work_lifecycle;
...
mod harness_routes;
#[path = "hypervisor_daemon_routes/information_flow.rs"]
...
async fn async_main() -> anyhow::Result<()> {
    ...
    let configured_system_node_id = std::env::var("IOI_HYPERVISOR_AUTONOMOUS_SYSTEM_NODE_ID")
        .ok()
        .filter(|value| !value.trim().is_empty());
    distributed_fencing::initialize_executing_node_identity(
        &data_dir,
        configured_system_node_id.as_deref(),
    )
    .map_err(|error| {
        anyhow::anyhow!("autonomous-System node identity configuration failed: {error}")
    })?;
    distributed_fencing::recover_all_active_system_fences(&data_dir).map_err(|error| {
        anyhow::anyhow!("autonomous-System writer-fence recovery failed closed: {error}")
    })?;
    ...
    let app = Router::new()
        ...
        .route(
            ...
            get(work_lifecycle::handle_work_lifecycle_status),
        )
        .route(

...
fn main() {
    ...
    let manifest_path = env::var("IOI_PACK_MANIFEST")
        .unwrap_or_else(|_| "presets/default_agent_workflow.toml".to_string());

    // [FIX] Prefix with underscore to silence unused variable warning
    let _assets_path =
        env::var("IOI_PACK_ASSETS").unwrap_or_else(|_| "src/embedded_assets".to_string());
    ...
    if env::var("IOI_PACK_MANIFEST").is_ok() && !Path::new(&manifest_path).exists() {
        panic!(
            "IOI Pack Error: Manifest file not found at {}",
            manifest_path
        );
    }

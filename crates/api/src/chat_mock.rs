use crate::chat::{
    ...
    ChatArtifactTasteMemory, ChatArtifactValidationResult, ChatArtifactValidationStatus,
...
pub(crate) fn maybe_handle_chat_prompt(input: &str) -> Option<String> {
    let normalized = normalize_prompt_input(input);
    if normalized.contains("Chat's typed outcome router") {
        return Some(mock_route_payload(&normalized).to_string());
    }
    if normalized.contains("Chat's typed artifact brief planner") {
        return Some(mock_brief_payload(&normalized).to_string());
    }
    if normalized.contains("Chat's typed artifact brief field repairer") {
        return Some(mock_brief_payload(&normalized).to_string());
    }
    if normalized.contains("Chat's typed artifact edit-intent planner") {
        return Some(mock_edit_intent_payload(&normalized).to_string());
    }
    if normalized.contains("Chat's typed artifact materialization repairer")
        || normalized.contains("Chat's typed artifact refiner")
    {
        return Some(mock_materialization_payload(&normalized).to_string());
    }
    if normalized.contains("Chat's typed artifact materializer") {
        return Some(mock_materialization_payload(&normalized).to_string());
    }
    if normalized.contains("Chat's typed artifact validation repairer") {
        return Some(mock_validation_payload(&normalized).to_string());
    }
    if normalized.contains("Chat's typed artifact validation") {
        return Some(mock_validation_payload(&normalized).to_string());
    }
    None
}
...
fn mock_materialization_payload(input: &str) -> Value {
    ...
    let brief: ChatArtifactBrief = extract_json_after(input, "Artifact brief JSON:\n")
        .and_then(|json| parse_chat_artifact_brief(&json).ok())
        .unwrap_or_else(default_brief);
    let edit_intent: Option<ChatArtifactEditIntent> =
        extract_json_after(input, "Edit intent JSON:\n")
            .and_then(|json| parse_chat_artifact_edit_intent(&json).ok())
            .or(None);
    let refinement: Option<ChatArtifactRefinementContext> =
        extract_json_after(input, "Current artifact context:\n")
            .and_then(|json| serde_json::from_str(&json).ok())
    ...
    let payload = match request.renderer {
        ...
        ChatRendererKind::JsxSandbox => build_jsx_payload(&title, &brief, candidate_seed),
        ChatRendererKind::Svg => build_svg_payload(&title, &brief, candidate_seed),
        ChatRendererKind::Mermaid => build_mermaid_payload(&title, &brief, &intent),
        ChatRendererKind::PdfEmbed => build_pdf_payload(&title, &brief),
        ChatRendererKind::DownloadCard => build_download_payload(&title, &brief),
        ChatRendererKind::BundleManifest => build_bundle_payload(&title, &brief),
        ChatRendererKind::WorkspaceSurface => build_html_payload(
            &title,
            &brief,
            edit_intent.as_ref(),
            refinement.as_ref(),
...
fn mock_validation_payload(input: &str) -> Value {
    ...
    let edit_intent: Option<ChatArtifactEditIntent> =
        extract_json_after(input, "Edit intent JSON:\n")
            ...
            .or(None);
    let candidate: ChatGeneratedArtifactPayload = extract_json_after(input, "Candidate JSON:\n")
        .and_then(|json| {
            parse_chat_generated_artifact_payload(&json)
                .ok()
...
fn parse_mock_validation_candidate_view(
    ...
    Ok(ChatGeneratedArtifactPayload {
        summary,
        notes,
        files,
    })
...
fn parse_mock_validation_candidate_file(
    ...
    let role = value
        ...
        .unwrap_or(ChatArtifactFileRole::Primary);
    let encoding = value
        .get("encoding")
        ...
        .or(Some(crate::chat::ChatGeneratedArtifactEncoding::Utf8));
    let body = value
        .get("bodyPreview")
        .and_then(Value::as_str)
        .unwrap_or_default()
        .to_string();
    Ok(ChatGeneratedArtifactFile {
        path,
        mime,
        role,
        renderable: value
...
fn mock_candidate_from_raw_validation_json(raw: &str) -> ChatGeneratedArtifactPayload {
    ChatGeneratedArtifactPayload {
        ...
        files: vec![ChatGeneratedArtifactFile {
            ...
            renderable: true,
            downloadable: true,
            encoding: Some(crate::chat::ChatGeneratedArtifactEncoding::Utf8),
            body: raw.to_string(),
        }],
    }
}

fn default_html_request() -> ChatOutcomeArtifactRequest {
    ChatOutcomeArtifactRequest {
        artifact_class: ChatArtifactClass::InteractiveSingleFile,
        deliverable_shape: ioi_types::app::ChatArtifactDeliverableShape::SingleFile,
        renderer: ChatRendererKind::HtmlIframe,
        presentation_surface: ioi_types::app::ChatPresentationSurface::Inline,
        persistence: ioi_types::app::ChatArtifactPersistenceMode::ArtifactScoped,
        execution_substrate: ioi_types::app::ChatExecutionSubstrate::ClientSandbox,
...
fn mock_request_from_focus_json(raw: &str) -> Option<ChatOutcomeArtifactRequest> {
    let value = serde_json::from_str::<Value>(raw).ok()?;
    let object = value.as_object()?;
    let renderer = object
        .get("renderer")
        .cloned()
        .and_then(|value| serde_json::from_value::<ChatRendererKind>(value).ok())?;
...
fn mock_edit_intent_from_focus_json(raw: &str) -> Option<ChatArtifactEditIntent> {
    let value = serde_json::from_str::<Value>(raw).ok()?;
    let object = value.as_object()?;
    let mode = object
        .get("mode")
        .cloned()
        .and_then(|value| serde_json::from_value::<ChatArtifactEditMode>(value).ok())
        .unwrap_or(ChatArtifactEditMode::Patch);
    let target_scope = object
        .get("targetScope")
        .and_then(Value::as_str)
        .unwrap_or("artifact")
        .to_string();
    Some(ChatArtifactEditIntent {
        mode,
        summary: object
            .get("targetScope")
            .and_then(Value::as_str)
        ...
        patch_existing_artifact: object
            ...
            .and_then(Value::as_bool)
            .unwrap_or(matches!(mode, ChatArtifactEditMode::Patch)),
        ...
        branch_requested: object
            .get("branchRequested")
            .and_then(Value::as_bool)
            .unwrap_or(matches!(mode, ChatArtifactEditMode::Branch)),
    })
}

fn default_brief() -> ChatArtifactBrief {
    ChatArtifactBrief {
...
fn default_refinement_context() -> ChatArtifactRefinementContext {
...
fn default_payload() -> ChatGeneratedArtifactPayload {
    ChatGeneratedArtifactPayload {
...
fn analyze_prompt(
    ...
    let audience =
        if lower.contains("launch") || lower.contains("rollout") || lower.contains("brief") {
            ...
            request.renderer,
            ChatRendererKind::Svg | ChatRendererKind::HtmlIframe
        ) {
...
fn job_to_be_done(request: &ChatOutcomeArtifactRequest, analysis: &PromptAnalysis) -> String {
...
fn refinement_subject_domain(refinement: &ChatArtifactRefinementContext) -> Option<String> {
    refinement
        .taste_memory
        .as_ref()
...
fn collect_style_directives(text: &str, renderer: ChatRendererKind) -> Vec<String> {
    ...
    if renderer == ChatRendererKind::Svg {
        directives.push("graphic clarity".to_string());
    }
    directives
}

fn collect_interaction_tags(text: &str, renderer: ChatRendererKind) -> Vec<String> {
    let lower = text.to_ascii_lowercase();
    let mut tags = Vec::new();
    if lower.contains("chart") {
        tags.push("chart toggles".to_string());
    }
    if lower.contains("configurator") || renderer == ChatRendererKind::JsxSandbox {
        tags.push("live controls".to_string());
        tags.push("computed summary".to_string());
    }
    if lower.contains("interactive") && renderer == ChatRendererKind::HtmlIframe {
        tags.push("section focus switching".to_string());
    }
    if tags.is_empty() && renderer == ChatRendererKind::HtmlIframe {
        tags.push("narrative section progression".to_string());
    }
    tags
}

fn renderer_label(renderer: ChatRendererKind) -> &'static str {
    match renderer {
        ChatRendererKind::Markdown => "markdown",
        ChatRendererKind::HtmlIframe => "html",
        ChatRendererKind::JsxSandbox => "jsx",
        ChatRendererKind::Svg => "svg",
        ChatRendererKind::Mermaid => "mermaid",
        ChatRendererKind::PdfEmbed => "pdf",
        ChatRendererKind::DownloadCard => "download",
        ChatRendererKind::WorkspaceSurface => "workspace",
        ChatRendererKind::BundleManifest => "bundle",
    }
}
...
fn build_markdown_payload(
    ...
    ChatGeneratedArtifactPayload {
        ...
        notes: vec![
            "Mock Chat runtime generated the markdown directly from the typed brief.".to_string(),
        ],
        files: vec![ChatGeneratedArtifactFile {
...
<html lang="en">
  ...
  <body>
    ...
    ChatGeneratedArtifactPayload {
        ...
        notes: vec![
            "Mock Chat runtime used the typed brief and candidate seed to vary layout and tone.".to_string(),
...
"##
    ...
    ChatGeneratedArtifactPayload {
        ...
        notes: vec![
            "Mock Chat runtime generated real controls and computed summary output.".to_string(),
        ],
        files: vec![ChatGeneratedArtifactFile {
...
fn build_mermaid_payload(
    ...
    ChatGeneratedArtifactPayload {
        ...
        notes: vec![
            "Mock Chat runtime generated a complete graph instead of a stub diagram.".to_string(),
        ],
        files: vec![ChatGeneratedArtifactFile {
...
fn build_pdf_payload(title: &str, brief: &ChatArtifactBrief) -> ChatGeneratedArtifactPayload {
    let body = format!(
    ...
    ChatGeneratedArtifactPayload {
        ...
        notes: vec![
            "Mock Chat runtime produced source text intended for PDF compilation.".to_string(),
        ],
        files: vec![ChatGeneratedArtifactFile {
...
fn build_bundle_payload(
    ...
    ChatGeneratedArtifactPayload {
        ...
        notes: vec![
            "Mock Chat runtime generated a bundle manifest with supporting README.".to_string(),
        ],
        files: vec![
            ChatGeneratedArtifactFile {
...
fn evaluate_candidate(
    ...
    let interaction_hits = brief
        ...
        .count();
    let generic_shell_detected = matches!(request.renderer, ChatRendererKind::HtmlIframe)
    ...
    let interaction_relevance = if request.renderer == ChatRendererKind::HtmlIframe {
        if all_text.contains("chart") || all_text.contains("track") {
            5
        } else {
            3
        }
    } else if request.renderer == ChatRendererKind::JsxSandbox {
    ...
    let patched_existing_artifact = edit_intent.map(|intent| {
        let patch_like_intent =
            intent.patch_existing_artifact || intent.mode == ChatArtifactEditMode::Patch;
    ...
    let deserves_primary_artifact_view = classification == ChatArtifactValidationStatus::Pass;
    ...
    ChatArtifactValidationResult {
        ...
        blocked_reasons: if classification == ChatArtifactValidationStatus::Blocked {
        ...
        recommended_next_pass: Some(
            if deserves_primary_artifact_view {
                "accept"
            } else if classification == ChatArtifactValidationStatus::Blocked {
                "hold_block"
            } else {
                "structural_repair"
            }

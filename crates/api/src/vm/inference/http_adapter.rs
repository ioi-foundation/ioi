...
fn html_like_closing_tag_match(text: &str, stop: &str) -> Option<(usize, usize)> {
    let tag = stop
        .strip_prefix("</")
    ...
    while let Some(relative_index) = lower_text[search_offset..].find(&needle) {
        let start = search_offset + relative_index;
        let mut end = start + needle.len();
        while matches!(lower_text.as_bytes().get(end), Some(byte) if byte.is_ascii_whitespace()) {
            end += 1;
        }
        if matches!(lower_text.as_bytes().get(end), Some(b'>')) {
...
fn stop_sequence_match(text: &str, stop_sequences: &[String]) -> Option<(usize, usize)> {
    stop_sequences
        ...
        .filter_map(|stop| {
            text.find(stop)
                .map(|index| (index, stop.len()))
                .or_else(|| html_like_closing_tag_match(text, stop))
        })
            ...
            while let Some(newline_pos) = pending.find('\n') {
                ...
                if let Some(delta) = envelope.message.and_then(|message| message.content) {
                    if !delta.is_empty() {
                        ...
                        if content.len() >= last_progress_trace_len.saturating_add(512) {
                            inference_stream_trace(format!(
                                "artifact_generation:local_native_stream:progress bytes={} has_document_boundary={}",
                                content.len(),
                                content.to_ascii_lowercase().contains("</html>")
                                    || content.to_ascii_lowercase().contains("</svg>")
                            ));
                            last_progress_trace_len = content.len();
                        }
                ...
                if envelope.done {
                    if !content.trim().is_empty() {
                        inference_stream_trace(format!(
                            "artifact_generation:local_native_stream:done bytes={} has_document_boundary={}",
                            content.len(),
                            content.to_ascii_lowercase().contains("</html>")
                                || content.to_ascii_lowercase().contains("</svg>")
                        ));

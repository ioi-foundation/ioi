...
use crate::studio::{
    ...
    build_studio_artifact_materialization_repair_prompt, build_studio_artifact_validation_prompt,
    build_studio_outcome_router_prompt, parse_studio_artifact_validation_result,
    ...
    StudioArtifactTasteMemory, StudioArtifactValidationStatus, StudioGeneratedArtifactFile,
    StudioGeneratedArtifactPayload,
};
use ioi_types::app::{StudioArtifactFileRole, StudioRendererKind};
use serde_json::json;

fn refinement_context_with_subject(subject: &str) -> StudioArtifactRefinementContext {
    StudioArtifactRefinementContext {
        ...
        title: "an interactive HTML artifact that explains a product rollout with charts"
            .to_string(),
        summary: format!(
            "Studio outcome: prior artifact The candidate stays grounded in {}, covers the required concepts, and is strong enough to lead the stage.",
            subject
        ),
...
fn mermaid_request() -> ioi_types::app::StudioOutcomeArtifactRequest {
...
fn pdf_request() -> ioi_types::app::StudioOutcomeArtifactRequest {
    serde_json::from_value(json!({
...
fn mock_router_decodes_serialized_chat_prompt_for_workspace_requests() {
    ...
    let response = maybe_handle_studio_prompt(&serialized).expect("mock route response");
...
fn mock_brief_decodes_serialized_chat_prompt_with_non_empty_fields() {
    ...
    let response = maybe_handle_studio_prompt(&serialized).expect("mock brief response");
...
fn mock_mermaid_materialization_mentions_approval_pipeline() {
    ...
    let response = maybe_handle_studio_prompt(&serialized).expect("mock materialization response");
    let payload = parse_studio_generated_artifact_payload(&response).expect("parse payload");
...
fn mock_pdf_validation_accepts_substantial_pdf_brief() {
    ...
    let validation =
        parse_studio_artifact_validation_result(&validation_response).expect("parse validation");
    assert_eq!(
...
fn mock_materialization_repair_prompt_returns_schema_valid_payload() {
    ...
    let response = maybe_handle_studio_prompt(&serialized).expect("mock repair response");
    let payload = parse_studio_generated_artifact_payload(&response).expect("parse payload");
...
fn mock_refiner_prompt_returns_schema_valid_payload() {
    ...
    let response = maybe_handle_studio_prompt(&serialized).expect("mock refiner response");
    let payload = parse_studio_generated_artifact_payload(&response).expect("parse payload");

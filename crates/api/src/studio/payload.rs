...
fn infer_download_bundle_export_format(
    ...
    brief: Option<&StudioArtifactBrief>,
    intent: Option<&str>,
...
fn download_bundle_export_encoding(
    format: DownloadBundleExportFormat,
) -> StudioGeneratedArtifactEncoding {
    match format {
        DownloadBundleExportFormat::Docx
        | DownloadBundleExportFormat::Odt
        | DownloadBundleExportFormat::Xlsx
        | DownloadBundleExportFormat::Pptx => StudioGeneratedArtifactEncoding::Base64,
        DownloadBundleExportFormat::Csv
        | DownloadBundleExportFormat::Json
        | DownloadBundleExportFormat::Markdown
        | DownloadBundleExportFormat::Text => StudioGeneratedArtifactEncoding::Utf8,
    }
}
...
<cp:lastModifiedBy>Studio</cp:lastModifiedBy>\
...
fn synthesize_docx_package_bytes(title: &str, body: &str) -> Option<Vec<u8>> {
    ...
    zip_bytes_from_entries(vec![
        ...
        (
            ...
            office_app_properties_xml("Studio").into_bytes(),
            CompressionMethod::Deflated,
        ),
        (
...
fn synthesize_pptx_package_bytes(title: &str, body: &str) -> Option<Vec<u8>> {
    ...
    let mut entries = vec![
        ...
        (
            ...
            br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="Studio Theme"><a:themeElements><a:clrScheme name="Studio"><a:dk1><a:sysClr val="windowText" lastClr="000000"/></a:dk1><a:lt1><a:sysClr val="window" lastClr="FFFFFF"/></a:lt1><a:accent1><a:srgbClr val="2563EB"/></a:accent1><a:accent2><a:srgbClr val="0F172A"/></a:accent2><a:accent3><a:srgbClr val="14B8A6"/></a:accent3><a:accent4><a:srgbClr val="F97316"/></a:accent4><a:accent5><a:srgbClr val="7C3AED"/></a:accent5><a:accent6><a:srgbClr val="DC2626"/></a:accent6><a:hlink><a:srgbClr val="2563EB"/></a:hlink><a:folHlink><a:srgbClr val="7C3AED"/></a:folHlink></a:clrScheme><a:fontScheme name="Studio"><a:majorFont><a:latin typeface="Aptos"/></a:majorFont><a:minorFont><a:latin typeface="Aptos"/></a:minorFont></a:fontScheme><a:fmtScheme name="Studio"><a:fillStyleLst><a:solidFill><a:schemeClr val="accent1"/></a:solidFill></a:fillStyleLst><a:lnStyleLst><a:ln w="9525"><a:solidFill><a:schemeClr val="accent1"/></a:solidFill></a:ln></a:lnStyleLst><a:effectStyleLst><a:effectStyle><a:effectLst/></a:effectStyle></a:effectStyleLst><a:bgFillStyleLst><a:solidFill><a:schemeClr val="lt1"/></a:solidFill></a:bgFillStyleLst></a:fmtScheme></a:themeElements></a:theme>"#.to_vec(),
            CompressionMethod::Deflated,
        ),
    ];
...
fn synthesize_download_bundle_export_body(
    ...
    file_hints: &[(String, String)],
    brief: Option<&StudioArtifactBrief>,
) -> (StudioGeneratedArtifactEncoding, String) {
    match format {
        DownloadBundleExportFormat::Csv => (
            StudioGeneratedArtifactEncoding::Utf8,
        ...
        DownloadBundleExportFormat::Docx => {
            ...
            let bytes = synthesize_docx_package_bytes(title, &body)
                ...
                .unwrap_or_default();
            (StudioGeneratedArtifactEncoding::Base64, bytes)
        }
        DownloadBundleExportFormat::Odt => {
            let body = synthesize_download_bundle_docx_body(summary, notes, brief);
            let title = brief
...
fn synthesize_download_bundle_readme_body(
    ...
    if !export_formats.is_empty() {
        ...
        for format in export_formats {
            lines.push(format!(
                "- The {} is surfaced in Studio as reviewable source content inside the download card so the authored structure can be inspected before download.",
                download_bundle_export_format_label(format)
            ));
        }
    }
            ...
            || infer_download_bundle_export_format_from_path_and_mime(&path, &mime).is_some()
        {
            let encoding = match format {
                ...
                _ => match synthesized_export_encoding {
                    StudioGeneratedArtifactEncoding::Utf8 => "utf8",
                    StudioGeneratedArtifactEncoding::Base64 => "base64",
                },
            };
            map.insert(
                "body".to_string(),
...
pub(crate) fn normalize_generated_artifact_payload(
    ...
    if request.renderer == StudioRendererKind::DownloadCard {
        ...
        {
            payload.files.push(StudioGeneratedArtifactFile {
                path: "README.md".to_string(),
                mime: "text/markdown".to_string(),
                role: StudioArtifactFileRole::Supporting,
                renderable: false,
            ...
            if !download_bundle_export_body_looks_complete(format, &file.body) {
                ...
                match format {
                    DownloadBundleExportFormat::Csv => {
                        ...
                        file.encoding = Some(StudioGeneratedArtifactEncoding::Utf8);
                    }
                    _ => {
                ...
                file.role = StudioArtifactFileRole::Export;
            }
        }
        ...
        for file in &mut payload.files {
                ...
                && !download_bundle_readme_looks_complete(&file.body)
            {
                ...
                file.encoding = Some(StudioGeneratedArtifactEncoding::Utf8);
                file.role = StudioArtifactFileRole::Supporting;
            }
        }
        return;
    }

...
pub(crate) fn studio_test_render_capture(
...
pub(crate) fn studio_test_render_evaluation(
    overall_score: u8,
...
#[derive(Default)]
pub(crate) struct StudioPassingRenderEvaluator;
...
fn render_eval_merge_downgrades_markdown_typography_block_to_warning() {
    ...
    let render_evaluation = StudioArtifactRenderEvaluation {
        ...
        interaction_capture_attempted: false,
        captures: vec![
            studio_test_render_capture(StudioArtifactRenderCaptureViewport::Desktop, 18, 220, 0),
            studio_test_render_capture(StudioArtifactRenderCaptureViewport::Mobile, 16, 180, 0),
        ],
        layout_density_score: 4,
        spacing_alignment_score: 4,
...
fn markdown_render_acceptance_policy_uses_document_thresholds() {
    ...
    let blueprint = derive_studio_artifact_blueprint(&request, &brief);
    let artifact_ir = compile_studio_artifact_ir(&request, &brief, &blueprint);
    let policy = build_studio_artifact_render_acceptance_policy(
        &request,
        &brief,
        Some(&blueprint),
        Some(&artifact_ir),

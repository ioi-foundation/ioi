...
fn html_swarm_plan_is_stable_and_scoped() {
    ...
    let first_plan = super::generation::build_studio_artifact_swarm_plan(
        &request,
        Some(&blueprint),
        &brief,
        StudioExecutionStrategy::AdaptiveWorkGraph,
    );
    let second_plan = super::generation::build_studio_artifact_swarm_plan(
        &request,
        Some(&blueprint),
        &brief,
        StudioExecutionStrategy::AdaptiveWorkGraph,
    ...
    assert!(first_plan
        .work_items
        .iter()
        .any(|item| item.id == "skeleton" && item.role == StudioArtifactWorkerRole::Skeleton));
    assert!(first_plan.work_items.iter().any(
        |item| item.id == "style-system" && item.role == StudioArtifactWorkerRole::StyleSystem
    ));
    assert!(
        first_plan
            .work_items
            ...
            .any(|item| item.id == "interaction"
                && item.role == StudioArtifactWorkerRole::Interaction)
    );
    assert!(first_plan
        .work_items
        .iter()
...
fn markdown_swarm_plan_uses_coarse_adapter_workers() {
    ...
    assert!(!plan
        .work_items
        .iter()
        .any(|item| item.role == StudioArtifactWorkerRole::SectionContent));
}

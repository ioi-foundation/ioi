use ioi_types::app::StudioRendererKind;
...
#[derive(Clone, Debug)]
pub struct StudioIntentContext {
    normalized: String,
    terms: Vec<String>,
}

impl StudioIntentContext {
    ...
    pub fn explicit_single_document_renderer(&self) -> Option<(StudioRendererKind, &'static str)> {
        if self.contains_any_phrase(&["pdf", "portable document format"]) {
            return Some((StudioRendererKind::PdfEmbed, "explicit_pdf_artifact"));
        }

        if self.contains_any_phrase(&["mermaid", "sequence diagram", "flow diagram"]) {
            return Some((StudioRendererKind::Mermaid, "explicit_mermaid_artifact"));
        }

        if self.contains_any_phrase(&["svg", "vector graphic"]) {
            return Some((StudioRendererKind::Svg, "explicit_svg_artifact"));
        }
        ...
        if self.contains_any_phrase(&[
            ...
            return Some((StudioRendererKind::HtmlIframe, "explicit_html_artifact"));
        }

        if self.contains_any_phrase(&["markdown", "readme", ".md"]) {
            return Some((StudioRendererKind::Markdown, "explicit_markdown_artifact"));
        }

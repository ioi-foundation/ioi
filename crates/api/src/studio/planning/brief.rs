...
use ioi_types::app::{
    StudioOutcomeArtifactRequest, StudioRendererKind, StudioRuntimeProvenanceKind,
};
use serde_json::json;
use std::sync::Arc;
...
pub(super) fn parse_studio_json_object_value(
    raw: &str,
...
pub(super) fn normalize_studio_outcome_planning_value(value: &mut serde_json::Value) {
    let Some(object) = value.as_object_mut() else {
        return;
    };
...
pub(super) fn interaction_family_for_kind(kind: StudioArtifactInteractionGoalKind) -> &'static str {
    match kind {
...
pub(super) fn brief_interaction_goals(
    brief: &StudioArtifactBrief,
) -> Vec<StudioArtifactInteractionGoal> {
...
pub(super) fn brief_required_interaction_summaries(brief: &StudioArtifactBrief) -> Vec<String> {
    brief.required_interaction_summaries()
}

pub(super) fn brief_interaction_families(brief: &StudioArtifactBrief) -> Vec<&'static str> {

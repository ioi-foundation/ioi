...
#[test]
fn html_work_graph_plan_is_stable_and_scoped() {
    ...
    let first_plan = super::generation::build_chat_artifact_work_graph_plan(
        &request,
        Some(&blueprint),
        &brief,
        ChatExecutionStrategy::AdaptiveWorkGraph,
    );
    let second_plan = super::generation::build_chat_artifact_work_graph_plan(
        &request,
        Some(&blueprint),
        &brief,
        ChatExecutionStrategy::AdaptiveWorkGraph,
    ...
    assert!(first_plan.work_items.iter().any(|item| item.id == "repair"
        ...
        && item.verification_policy == Some(WorkGraphVerificationPolicy::Blocking)
    ...
    assert!(section_items.iter().all(|item| {
        item.write_paths == vec!["index.html".to_string()]
            ...
            && item
                .lease_requirements
                .iter()
                .any(|lease| lease.mode == WorkGraphLeaseMode::ExclusiveWrite)
    }));
}

#[test]
fn markdown_work_graph_plan_uses_coarse_adapter_workers() {
    ...
    let plan = super::generation::build_chat_artifact_work_graph_plan(
        &request,
        None,
        &brief,
        ChatExecutionStrategy::AdaptiveWorkGraph,

...
use crate::chat::*;
use ioi_types::app::{ChatOutcomeArtifactRequest, ChatRendererKind};
...
fn blueprint_scaffold_family(
    ...
    match request.renderer {
        ...
        ChatRendererKind::WorkspaceSurface => "workspace_project".to_string(),
    }
}
...
fn blueprint_skill_needs(
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
) -> Vec<ChatArtifactSkillNeed> {
    let mut skill_needs = vec![ChatArtifactSkillNeed {
        kind: ChatArtifactSkillNeedKind::AccessibilityReview,
    ...
    if matches!(
        ...
        skill_needs.push(ChatArtifactSkillNeed {
        ...
        skill_needs.push(ChatArtifactSkillNeed {
            kind: ChatArtifactSkillNeedKind::InteractionCopyDiscipline,
            priority: ChatArtifactSkillNeedPriority::Recommended,
    ...
    if brief.required_concepts.len() >= 3 {
        skill_needs.push(ChatArtifactSkillNeed {
            kind: ChatArtifactSkillNeedKind::EditorialLayout,
            priority: ChatArtifactSkillNeedPriority::Recommended,
    ...
    if interaction_families.contains(&"sequence_browsing") {
        skill_needs.push(ChatArtifactSkillNeed {
            kind: ChatArtifactSkillNeedKind::MotionHierarchy,
            priority: ChatArtifactSkillNeedPriority::Recommended,
            rationale:
    ...
    {
        skill_needs.push(ChatArtifactSkillNeed {
            kind: ChatArtifactSkillNeedKind::DataStorytelling,
            priority: ChatArtifactSkillNeedPriority::Recommended,
...
fn blueprint_section_plan(
    ...
    if matches!(
        ...
        sections.push(ChatArtifactSectionPlan {
...
fn blueprint_interaction_plan(
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
) -> Vec<ChatArtifactInteractionPlan> {
    if matches!(
        request.renderer,
    ...
    if plans.is_empty() {
        plans.push(ChatArtifactInteractionPlan {
...
fn blueprint_evidence_plan(
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
) -> Vec<ChatArtifactEvidencePlanEntry> {
    let seed_concepts =
...
) -> Vec<ChatArtifactComponentPlanEntry> {
    fn push_component(
        plan: &mut Vec<ChatArtifactComponentPlanEntry>,
        ...
        plan.push(ChatArtifactComponentPlanEntry {
...
fn blueprint_accessibility_plan(
    ...
    if matches!(
        ...
        return ChatArtifactAccessibilityPlan {
            obligations: vec![
...
pub fn derive_chat_artifact_blueprint(
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
    ...
    let mut blueprint = ChatArtifactBlueprint {
        version: 1,
...
pub fn compile_chat_artifact_ir(
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
    blueprint: &ChatArtifactBlueprint,
) -> ChatArtifactIR {
    let mut semantic_structure = blueprint
        .section_plan
        .iter()
        .map(|section| ChatArtifactIRNode {
            id: section.id.clone(),
    ...
    let interaction_graph = blueprint
        .interaction_plan
        .iter()
        .map(|interaction| ChatArtifactIRInteractionEdge {
    ...
    let evidence_surfaces = blueprint
        .evidence_plan
        .iter()
        .map(|surface| ChatArtifactIREvidenceSurface {
            id: surface.id.clone(),

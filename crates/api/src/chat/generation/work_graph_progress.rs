...
fn work_graph_operator_status(verification_status: &str) -> ChatArtifactOperatorRunStatus {
...
fn work_graph_operator_phase(current_stage: &str) -> ChatArtifactOperatorPhase {
    match current_stage.trim().to_ascii_lowercase().as_str() {
        ...
        "work" | "work_graph_execution" | "materialization" | "execution" | "mutate" => {
...
fn work_graph_operator_steps(
    ...
    let phase = work_graph_operator_phase(current_stage);
    let status = work_graph_operator_status(verification_status);
    vec![ChatArtifactOperatorStep {
        step_id: format!(
            "work_graph:{}:{}",
        ...
        origin_prompt_event_id: String::new(),
        phase,
        engine: "work_graph_generation".to_string(),
        status,
        label: work_graph_operator_label(phase, active_worker_role),
        ...
        .then_some(0),
        preview: work_graph_preview(live_previews),
        file_refs: Vec::new(),
        source_refs: Vec::new(),
...
pub(super) fn emit_chat_work_graph_generation_progress(
    ...
    let execution_envelope = build_execution_envelope_from_work_graph_with_receipts(
        ...
        repair_receipts,
        replan_receipts,
        Some(chat_work_graph_partial_budget_summary(
            request,
    ...
    observer(ChatArtifactGenerationProgress {
        ...
        execution_envelope,
        work_graph_plan: Some(work_graph_plan.clone()),
        work_graph_execution: Some(work_graph_execution),
        work_graph_worker_receipts: worker_receipts.to_vec(),
        work_graph_change_receipts: patch_receipts.to_vec(),
        work_graph_merge_receipts: merge_receipts.to_vec(),
        work_graph_verification_receipts: verification_receipts.to_vec(),
        render_evaluation: render_evaluation.cloned(),
        validation: validation.cloned(),
        operator_steps: work_graph_operator_steps(
            current_stage,
            active_worker_role,
            verification_status,
            &current_step,
...
pub(super) fn ensure_work_graph_file_from_operation(
    request: &ChatOutcomeArtifactRequest,
...
pub(super) fn chat_work_graph_rejected_patch_receipts(
...
pub(super) fn html_work_graph_patch_contains_region_markers(body: &str) -> bool {
    body.lines().any(|line| {
...
pub(super) fn chat_work_graph_semantic_conflict_reason(
    request: &ChatOutcomeArtifactRequest,
    work_item: &ChatArtifactWorkItem,
    operation: &ChatArtifactPatchOperation,
) -> Option<String> {
    ...
    if matches!(
        ...
        && html_work_graph_patch_contains_region_markers(body)
    {
        return Some(format!(
            "Work item '{}' attempted to inject nested work_graph region markers into '{}'.",
            work_item.id,
...
pub(super) fn chat_work_graph_skip_receipt(
    work_item: &ChatArtifactWorkItem,
    ...
    ChatArtifactWorkerReceipt {
        ...
        result_kind: Some(WorkGraphWorkerResultKind::Noop),
        summary,
...
pub(super) fn chat_work_graph_skip_summary_for_html_work_item(
    request: &ChatOutcomeArtifactRequest,
...
pub(crate) fn apply_chat_work_graph_patch_envelope(
    request: &ChatOutcomeArtifactRequest,
    ...
    for operation in &envelope.operations {
        ...
        {
            return Ok(chat_work_graph_rejected_patch_receipts(
                work_item,
                envelope
                    .summary
                    .clone()
        ...
        if let Some(reason) =
            chat_work_graph_semantic_conflict_reason(request, work_item, &normalized_operation)
        {
            return Ok(chat_work_graph_rejected_patch_receipts(
                work_item,
                envelope
        ...
        if matches!(
            ...
            let Some(canonical_region) = work_item
                .write_regions
                .iter()
                .find(|region| html_work_graph_region_ids_match(region, region_id))
                .cloned()
            else {
                return Ok(chat_work_graph_rejected_patch_receipts(
                    work_item,

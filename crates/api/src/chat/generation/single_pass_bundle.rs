...
pub async fn generate_chat_artifact_bundle_with_runtime_plan_and_planning_context_and_execution_strategy_and_render_evaluator(
    ...
    if chat_artifact_uses_work_graph_execution(effective_execution_strategy) {
        ...
        .await;
        return generate_chat_artifact_bundle_with_work_graph(
            runtime_plan.clone(),
            title,
            intent,
            request,
    ...
    let (candidate_summaries, ranked_candidate_indices) = loop {
        while next_candidate_index < target_candidate_count {
            ...
            emit_single_pass_generation_progress(
            ...
            let candidate_result = if direct_author_mode {
                ...
                match payload {
                    Ok(payload) => {
                        if let Some(preview) =
                            single_pass_canonical_preview(request, &payload, "draft_ready", false)
                        {
                            if let Ok(mut previews) = live_preview_state.lock() {
                        ...
                        emit_single_pass_generation_progress(
                            ...
                            None,
                            ExecutionCompletionInvariantStatus::Pending,
                            single_pass_required_artifact_paths(&payload),
                            vec![
                        ...
                        if direct_author_fast_runtime_sanity_enabled(
                            ...
                            for repair_attempt in 0..DIRECT_AUTHOR_FAST_RUNTIME_REPAIR_ATTEMPTS {
                                ...
                                if let Some(preview) = single_pass_canonical_preview(
                                    request,
                                    &repaired_payload,
                                    "repairing",
                                    false,
    ...
    return build_single_pass_winner_bundle(
        request,
        refinement,
        effective_execution_strategy,
        origin,

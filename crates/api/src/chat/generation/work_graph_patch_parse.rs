    ...
    serde_json::from_str::<ChatArtifactPatchEnvelope>(raw)
        ...
        .or_else(|_| {
            let extracted = extract_first_json_object(raw)
                .ok_or_else(|| "Chat work_graph worker output missing JSON object.".to_string())?;
        ...
        .map_err(|error| format!("Failed to parse Chat work_graph patch envelope: {error}"))
}
...
pub(super) fn salvage_chat_work_graph_patch_envelope(
        ...
        extract_relaxed_json_string_field(raw, "regionId")
            .and_then(|candidate| {
                work_item
                    .write_regions
                    .iter()
                    .find(|region| html_work_graph_region_ids_match(region, &candidate))
                    .cloned()
            ...
            .or_else(|| {
                ...
                if lowered.contains("<script") {
                    work_item
                        .write_regions
                        .iter()
                        .find(|region| html_work_graph_region_ids_match(region, "interaction"))
                        .cloned()
                } else if lowered.contains("<style") {
                    work_item
                        .write_regions
                        .iter()
                        .find(|region| html_work_graph_region_ids_match(region, "style-system"))
                        .cloned()
                } else {
                    work_item
                        .write_regions
...
pub(super) fn work_graph_patch_schema_contract() -> &'static str {
...
pub(super) fn html_work_graph_region_marker_start(region_id: &str) -> String {
    format!("<!-- STUDIO_REGION_START:{region_id} -->")
}

pub(super) fn html_work_graph_region_marker_end(region_id: &str) -> String {
    format!("<!-- STUDIO_REGION_END:{region_id} -->")
}

pub(super) fn html_work_graph_region_id_variants(region_id: &str) -> Vec<String> {
...
pub(super) fn html_work_graph_region_ids_match(left: &str, right: &str) -> bool {
    html_work_graph_region_id_variants(left)
        .iter()
        .any(|candidate| {
            html_work_graph_region_id_variants(right)
                .iter()
                .any(|other| other == candidate)
        })
}

pub(super) fn html_work_graph_region_default_insert_index(body: &str, region_id: &str) -> usize {
...
pub(super) fn ensure_html_work_graph_region_marker_pair(body: &str, region_id: &str) -> String {
    ...
    for candidate in html_work_graph_region_id_variants(region_id) {
        ...
        if let Some(start_index) = body.find(&start_marker) {
            ...
            let insert_at = body[content_start..]
                ...
                .unwrap_or_else(|| html_work_graph_region_default_insert_index(body, region_id));
            let mut rebuilt = String::with_capacity(body.len() + candidate.len() + 48);
            rebuilt.push_str(&body[..insert_at]);
            if !rebuilt.ends_with('\n') {
                rebuilt.push('\n');
            }
            rebuilt.push_str(&html_work_graph_region_marker_end(&candidate));
            rebuilt.push('\n');
            rebuilt.push_str(&body[insert_at..]);
            return rebuilt;
        }
    ...
    let insert_at = html_work_graph_region_default_insert_index(body, region_id);
    let mut rebuilt = String::with_capacity(body.len() + region_id.len() * 2 + 64);
    rebuilt.push_str(&body[..insert_at]);
    if !rebuilt.ends_with('\n') {
        rebuilt.push('\n');
    }
    rebuilt.push_str(&html_work_graph_region_marker_start(region_id));
    rebuilt.push('\n');
    rebuilt.push_str(&html_work_graph_region_marker_end(region_id));
    rebuilt.push('\n');
    rebuilt.push_str(&body[insert_at..]);
    rebuilt
}

pub(super) fn normalize_html_work_graph_skeleton_markers(
    body: &str,
    ...
    expected_regions
        ...
        .fold(body.to_string(), |current, region| {
            ensure_html_work_graph_region_marker_pair(&current, region)
        })
}
...
pub(super) fn normalize_html_work_graph_region_replacement(
    region_id: &str,
    replacement: &str,
) -> String {
...
pub(super) fn ensure_html_work_graph_visible_main_shell(body: &str) -> String {
...
pub(super) fn normalize_html_work_graph_document(body: &str) -> String {
    ...
    let normalized_wrappers = decoded
        ...
        .replace("</interaction>", "</script>");
    ensure_html_work_graph_visible_main_shell(&normalized_wrappers)
}

pub(super) fn replace_html_work_graph_region(
    body: &str,
    region_id: &str,
    replacement: &str,
) -> Result<String, String> {
    let normalized_replacement =
        normalize_html_work_graph_region_replacement(region_id, replacement);
    for candidate in html_work_graph_region_id_variants(region_id) {
        let start_marker = html_work_graph_region_marker_start(&candidate);
        let end_marker = html_work_graph_region_marker_end(&candidate);
...
pub(super) fn strip_html_work_graph_region_markers(body: &str) -> String {
    body.lines()
        .filter(|line| {

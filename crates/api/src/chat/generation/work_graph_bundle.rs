...
pub(super) async fn generate_chat_artifact_bundle_with_work_graph(
    ...
    let work_graph_started_at = Instant::now();
    ...
    let mut work_graph_plan =
        build_chat_artifact_work_graph_plan(request, blueprint, brief, execution_strategy);
    ...
    if let Some(planner_item) = work_graph_plan
        .work_items
        .iter()
    ...
    if request.renderer == ChatRendererKind::HtmlIframe {
        let execution_work_item_ids = work_graph_plan
            .work_items
            .iter()
            .filter(|item| {
                matches!(
        ...
        while let Some(mut dispatch_batch) = next_work_graph_dispatch_batch(
            &work_graph_plan,
            &execution_work_item_ids,
            dispatch_sequence,
        ) {
            dispatch_sequence = dispatch_sequence.saturating_add(1);
            constrain_dispatch_batch_by_parallelism(
                &mut dispatch_batch,
                chat_work_graph_dispatch_parallelism_cap(request, production_provenance.kind),
            );
            ...
            for work_item_id in dispatch_batch.work_item_ids.clone() {
                ...
                if let Some(skip_summary) = chat_work_graph_skip_summary_for_html_work_item(
                    request,
                    brief,
                    blueprint,
            ...
            let live_preview_observer: Option<ChatArtifactLivePreviewObserver> =
                progress_observer.as_ref().map(|_| {
                    ...
                    let batch_work_graph_plan = work_graph_plan.clone();
            ...
            for work_item in &runnable_items {
                ...
                let worker_context = chat_work_graph_work_item_context(
                    &work_item,
                    blueprint.as_ref(),
                    artifact_ir.as_ref(),
                    None,
                ...
                join_set.spawn(async move {
                    execute_chat_work_graph_patch_worker(
                        runtime,
                        &title,
                        &intent,
                        &request,
            ...
            while let Some(join_result) = join_set.join_next().await {
                let execution_result = join_result.map_err(|error| {
                    build_error(format!("Chat work_graph worker join failed: {error}"))
                })?;
            ...
            for work_item in runnable_items {
                let Some((envelope, mut receipt)) = batch_results.remove(&work_item.id) else {
                    return Err(build_error(format!(
                        "Chat work_graph dispatch batch '{}' lost worker '{}'.",
        ...
        let skeleton_item = work_graph_plan
            .work_items
            .iter()
            .find(|item| item.id == "skeleton")
            .cloned()
            .ok_or_else(|| build_error("WorkGraph skeleton work item is missing.".to_string()))?;
        let generated = materialize_chat_artifact_candidate_with_runtime_detailed(
            ...
            chat_work_graph_worker_temperature(
                request,
                ChatArtifactWorkerRole::Skeleton,
                production_provenance.kind,
            ),
        ...
        if let Some(integrator_item) = work_graph_plan
            .work_items
            .iter()
        ...
        runtime_dispatch_batches = plan_work_graph_dispatch_batches(&work_graph_plan);
    }

    finalize_work_graph_bundle_after_initial_execution(
        runtime_plan,
        title,
        intent,
        request,

...
pub fn build_chat_artifact_candidate_refinement_prompt(
    title: &str,
    intent: &str,
    ...
    build_chat_artifact_candidate_refinement_prompt_for_runtime(
        title,
        intent,
        request,
        brief,
...
pub(crate) fn build_chat_artifact_candidate_refinement_prompt_for_runtime(
    title: &str,
    intent: &str,
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
    ...
    let edit_intent_json = if compact_prompt {
        ...
        serialize_materialization_prompt_json(&edit_intent, "Chat artifact edit intent", false)?
    };
    ...
    let validation_json = if compact_prompt {
        serialize_materialization_prompt_json(
            ...
            "Chat artifact validation focus",
            true,
        )?
    } else {
        serialize_materialization_prompt_json(
    ...
    let render_eval_json = serialize_materialization_prompt_json(
        ...
        if compact_prompt {
            ...
            "Chat artifact render evaluation"
        },
        compact_prompt,
    )?;
    ...
    if compact_prompt {
        ...
        return Ok(json!([
            {
                "role": "system",
                "content": "You are Chat's typed artifact refiner. Patch the current candidate in place to resolve the validation's cited contradictions while preserving working structure and strong request-specific content. Output JSON only."
            },
            {
                "role": "user",
                "content": format!(
...
pub fn build_chat_artifact_candidate_refinement_repair_prompt(
    title: &str,
    intent: &str,
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
...
pub(super) fn build_chat_artifact_candidate_refinement_repair_prompt_for_runtime(
    title: &str,
    intent: &str,
    request: &ChatOutcomeArtifactRequest,
    brief: &ChatArtifactBrief,
    ...
    if compact_prompt {
        ...
        return Ok(json!([
            {
                "role": "system",
                "content": "You are Chat's typed artifact refinement repairer. Repair the refined candidate into a schema-valid patch that resolves the cited contradictions while preserving the current artifact's strongest valid structure. Output JSON only."
            },
            {
                "role": "user",
                "content": format!(

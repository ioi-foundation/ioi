...
pub(super) fn chat_swarm_payload_prompt_view(
...
pub(super) fn chat_swarm_work_item_context(
...
pub(super) fn chat_swarm_worker_role_directive(
...
pub(super) fn build_chat_swarm_patch_prompt_for_runtime(
    title: &str,
    intent: &str,
    request: &ChatOutcomeArtifactRequest,
    ...
    if compact_prompt {
        ...
        let role_directive = chat_swarm_worker_role_directive(request, work_item);
        return Ok(json!([
            {
                "role": "system",
                "content": format!(
                    "You are Chat's typed swarm {:?} worker. Return JSON only. You own only the explicit work item scope and must preserve authored structure outside it.",
                    work_item.role
                )
            },
            {
    ...
    let current_json = serialize_materialization_prompt_json(
        &chat_swarm_payload_prompt_view(current_payload),
        "Current canonical artifact",
        false,
    )?;
    let work_item_json =
    ...
    Ok(json!([
        {
            "role": "system",
            "content": format!(
                "You are Chat's typed swarm {:?} worker. Return JSON only. You do not own the full artifact. You own only the explicit work item scope and must preserve strong authored structure outside it.",
                work_item.role
            )
        },
        {
...
pub(super) fn chat_swarm_worker_temperature(
    request: &ChatOutcomeArtifactRequest,
...
pub(super) fn chat_swarm_worker_max_tokens(
    request: &ChatOutcomeArtifactRequest,
    role: ChatArtifactWorkerRole,
    runtime_kind: ChatRuntimeProvenanceKind,
) -> u32 {
...
pub(super) fn chat_swarm_worker_timeout(
    request: &ChatOutcomeArtifactRequest,
    role: ChatArtifactWorkerRole,
    runtime_kind: ChatRuntimeProvenanceKind,
) -> Option<Duration> {
...
pub(super) fn chat_swarm_dispatch_parallelism_cap(
    request: &ChatOutcomeArtifactRequest,
    runtime_kind: ChatRuntimeProvenanceKind,
) -> usize {
...
pub(super) fn chat_swarm_planned_token_budget(
    request: &ChatOutcomeArtifactRequest,
    runtime_kind: ChatRuntimeProvenanceKind,
    ...
    swarm_plan
        .work_items
        .iter()
        .map(|item| chat_swarm_worker_max_tokens(request, item.role, runtime_kind))
        .sum()
}

pub(super) fn build_chat_swarm_patch_repair_prompt(
    work_item: &ChatArtifactWorkItem,
    raw_output: &str,
    parse_error: &str,
) -> serde_json::Value {
    json!([
        {
            "role": "system",
            "content": "You repair malformed Chat swarm worker output into valid JSON. Return JSON only. Preserve the worker's intent, stay inside the existing scope, and do not invent extra files or extra operations. Prefer complete, closed CSS/JS blocks over partial truncation, and preserve every scoped operation you can recover."
        },
        {
            "role": "user",
            "content": format!(
...
pub(super) async fn repair_chat_swarm_patch_envelope(
    ...
    if let Some(envelope) = salvage_chat_swarm_patch_envelope(request, work_item, raw_output) {
    ...
    let output =
        await_with_activity_heartbeat(repair_future, activity_observer, Duration::from_millis(125))
            ...
            .map_err(|_| {
                format!(
                    "Chat swarm worker '{}' JSON repair timed out after 150s.",
                    work_item.id
                )
            })?
            .map_err(|error| format!("Chat swarm worker JSON repair failed: {error}"))?;
    let raw = String::from_utf8(output)
        .map_err(|error| format!("Chat swarm worker JSON repair utf8 decode failed: {error}"))?;
    let envelope = match parse_chat_artifact_patch_envelope(&raw) {
        ...
        Err(error) => {
            ...
            salvage_chat_swarm_patch_envelope(request, work_item, &raw)
                .or_else(|| salvage_chat_swarm_patch_envelope(request, work_item, raw_output))
                .ok_or_else(|| format!("Chat swarm worker JSON repair parse failed: {error}"))?
        }
    };
    chat_generation_trace(format!(
...
pub(super) async fn execute_chat_swarm_patch_worker(
    runtime: Arc<dyn InferenceRuntime>,
    title: &str,
    intent: &str,
    request: &ChatOutcomeArtifactRequest,
    ...
    let started_at = chat_swarm_now_iso();
    let runtime_kind = runtime.chat_runtime_provenance().kind;
    let prompt = build_chat_swarm_patch_prompt_for_runtime(
        title,
        intent,
        request,
        brief,
    ...
    let prompt_bytes = serde_json::to_vec(&prompt)
        .map_err(|error| format!("Failed to encode Chat swarm worker prompt: {error}"))?;
    let max_tokens = chat_swarm_worker_max_tokens(request, work_item.role, runtime_kind);
    let timeout = chat_swarm_worker_timeout(request, work_item.role, runtime_kind);
    chat_generation_trace(format!(
    ...
    let inference_options = InferenceOptions {
        temperature: chat_swarm_worker_temperature(request, work_item.role, runtime_kind),
        json_mode: true,
        max_tokens,
        ..Default::default()
    };
    ...
    let output = match timeout {
        ...
        .await
        {
            ...
            Ok(Err(error)) => {
                ...
                return Err(format!("Chat swarm worker inference failed: {error}"));
            }
            Err(_) => {
                chat_generation_trace(format!(
                ...
                return Err(format!(
                    "Chat swarm worker '{}' timed out after {}s.",
                    work_item.id,
                    limit.as_secs()
                ));
            }
        ...
        .await
        .map_err(|error| format!("Chat swarm worker inference failed: {error}"))?,
    };
    chat_generation_trace(format!(
    ...
    let raw = String::from_utf8(output.clone())
        .map_err(|error| format!("Chat swarm worker utf8 decode failed: {error}"))?;
    ...
    let envelope = match parse_chat_artifact_patch_envelope(&raw) {
        ...
        Err(error) => {
            ...
            if let Some(envelope) = salvage_chat_swarm_patch_envelope(request, work_item, &raw) {
                chat_generation_trace(format!(
                ...
                repair_chat_swarm_patch_envelope(
                    runtime.clone(),
                    request,
                    work_item,
                    &raw,

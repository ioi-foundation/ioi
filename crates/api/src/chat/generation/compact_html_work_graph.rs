...
pub(super) fn extract_html_work_graph_region_body(body: &str, region_id: &str) -> Option<String> {
    for candidate in html_work_graph_region_id_variants(region_id) {
        let start_marker = html_work_graph_region_marker_start(&candidate);
        let end_marker = html_work_graph_region_marker_end(&candidate);
...
pub(super) fn compact_local_html_work_graph_payload_focus(
        ...
        .map(|file| {
            work_item
                ...
                .map(|region_id| {
                    json!({
                        "regionId": region_id,
                        "bodyPreview": extract_html_work_graph_region_body(&file.body, region_id)
...
pub(super) fn compact_local_html_work_graph_work_item_focus(
    work_item: &ChatArtifactWorkItem,
) -> serde_json::Value {
    json!({
        "id": work_item.id,
...
pub(super) fn compact_local_html_work_graph_worker_context_focus(
    work_item: &ChatArtifactWorkItem,
...
pub(super) fn compact_local_html_work_graph_renderer_guidance(
...
pub(super) fn compact_local_html_work_graph_skill_focus(
...
pub(super) fn compact_local_html_work_graph_exemplar_focus(

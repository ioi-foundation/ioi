use super::*;
use crate::chat::payload::renderer_document_completeness_failure;
    ...
    [
        ...
        format!(
            ...
            match request.execution_substrate {
                ChatExecutionSubstrate::None => "none",
                ChatExecutionSubstrate::ClientSandbox => "client_sandbox",
                ChatExecutionSubstrate::WorkspaceRuntime => "workspace_runtime",
                ChatExecutionSubstrate::BinaryGenerator => "binary_generator",
            }
        ),
        format!(
            "render verification: {}",
...
pub(super) fn compact_local_html_materialization_brief_text(brief: &ChatArtifactBrief) -> String {
    let required_interactions = brief.required_interaction_summaries();
    let concepts = brief
        .required_concepts
        .iter()
...
pub(super) fn compact_local_html_interaction_contract_text(brief: &ChatArtifactBrief) -> String {
    let contract = super::chat_artifact_interaction_contract(brief);
    [
        format!(
...
pub(super) fn direct_authoring_enabled(
    ...
    execution_strategy == ChatExecutionStrategy::DirectAuthor
        ...
        && (refinement.is_none() || runtime_kind == ChatRuntimeProvenanceKind::RealLocalRuntime)
}

pub(super) fn direct_author_uses_raw_document(request: &ChatOutcomeArtifactRequest) -> bool {
    matches!(
        request.renderer,
...
pub(super) fn direct_author_stop_sequences(request: &ChatOutcomeArtifactRequest) -> Vec<String> {
    match request.renderer {
        ChatRendererKind::HtmlIframe => vec!["</html>".to_string()],
        ChatRendererKind::Svg => vec!["</svg>".to_string()],
        _ => Vec::new(),
    }
}
...
pub(super) fn direct_author_completion_boundary(
    ...
    match request.renderer {
        ChatRendererKind::HtmlIframe => Some("</html>"),
        ChatRendererKind::Svg => Some("</svg>"),
        _ => None,
    }
}
...
pub(super) fn compact_local_direct_author_prompt(
    ...
    runtime_kind == ChatRuntimeProvenanceKind::RealLocalRuntime && returns_raw_document
}
...
pub(super) fn direct_author_search_budget(
    ...
    ChatAdaptiveSearchBudget {
        ...
        signals: if production_kind == ChatRuntimeProvenanceKind::RealLocalRuntime {
            vec![ChatAdaptiveSearchSignal::LocalGenerationConstraint]
        } else {
            Vec::new()
        },
    }
...
pub(super) fn chat_direct_author_renderer_guidance(
    request: &ChatOutcomeArtifactRequest,
    candidate_seed: u64,
    runtime_kind: ChatRuntimeProvenanceKind,
) -> String {
    match request.renderer {
        ...
        ChatRendererKind::HtmlIframe => {
            ...
            let composition_recipe = if request.artifact_class == ChatArtifactClass::InteractiveSingleFile {

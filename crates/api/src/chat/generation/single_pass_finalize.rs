...
pub(super) fn build_single_pass_winner_bundle(
    ...
    if let Some(preview) = single_pass_canonical_preview(request, &winner, "completed", true) {
        if let Ok(mut previews) = live_preview_state.lock() {
            upsert_execution_live_preview(&mut previews, preview);
        }
    }
    let execution_envelope = build_single_pass_execution_envelope(
        request,
        execution_strategy,
        ...
        single_pass_required_artifact_paths(&winner),
    );

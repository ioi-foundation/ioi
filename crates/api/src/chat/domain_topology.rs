...
use ioi_types::app::{
    RoutingEffectiveToolSurface, RoutingRouteDecision, ChatArtifactClass,
    ChatArtifactLifecycleState, ChatArtifactManifest, ChatArtifactManifestVerification,
    ChatArtifactVerificationStatus, ChatCheckpointState, ChatClarificationMode,
    ChatClarificationPolicy, ChatCompletionInvariant, ChatDomainLaneFrame,
    ...
    ChatOutcomeArtifactRequest, ChatOutcomeKind, ChatOutcomeRequest,
    ChatPlacesRequestFrame, ChatPolicyContractSummary, ChatPresentationPolicy,
    ChatRecipeRequestFrame, ChatRendererKind, ChatRetainedLaneState,
    ChatRetainedWidgetState, ChatRiskProfile, ChatRiskSensitivity, ChatRuntimeProvenance,
    ChatSourceFamily, ChatSourceRankingEntry, ChatSourceSelection, ChatSportsRequestFrame,
    ChatTaskUnitState, ChatTransformationPolicy, ChatUserInputRequestFrame,
    ChatVerificationContract, ChatWeatherRequestFrame, ChatWidgetStateBinding,
    ChatWorkStatus,
};
use serde_json::json;
...
pub fn route_topology_for_outcome_request(outcome_request: &ChatOutcomeRequest) -> &'static str {
    match outcome_request.execution_strategy {
        ChatExecutionStrategy::SinglePass | ChatExecutionStrategy::DirectAuthor => {
            "single_agent"
        }
...
fn derive_request_frame(
        ...
        || context.tool_widget_family() == Some("sports")
    {
        ...
        return Some(ChatNormalizedRequestFrame::Sports(
            ChatSportsRequestFrame {
                league,
                team_or_target,
                data_scope: context
                    .sports_data_scope()
        ...
        || context.tool_widget_family() == Some("places")
    {
        ...
        return Some(ChatNormalizedRequestFrame::Places(
            ChatPlacesRequestFrame {
                search_anchor,
                category,
                location_scope,
                missing_slots,
        ...
        || context.tool_widget_family() == Some("recipe")
    {
        ...
        return Some(ChatNormalizedRequestFrame::Recipe(
            ChatRecipeRequestFrame {
                dish,
                servings: context
                    .recipe_servings()
...
fn derive_source_selection(
    ...
    let fallback_reason = if routing_hint_flag(routing_hints, "connector_missing") {
        ...
        .is_some_and(|frame| !chat_request_frame_missing_slots(frame).is_empty())
    {
        Some(
            specialized_policy
    ...
    match primary_lane {
        ChatLaneFamily::Artifact => vec![
            ChatTaskUnitState {
                ...
                depends_on: Vec::new(),
                summary: Some(
                    "Chat should keep the artifact request coherent before generation."
                        .to_string(),
                ),
            },
            ChatTaskUnitState {
        ...
        _ => {
            ...
            vec![
                ChatTaskUnitState {
                    ...
                    depends_on: Vec::new(),
                    summary: Some(
                        "Chat should make the lane contract explicit before execution."
                            .to_string(),
                    ),
                },
                ChatTaskUnitState {

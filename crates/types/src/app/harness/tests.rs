...
fn worker_attach_resolver_blocks_invalid_and_accepts_bound_registry() {
    ...
    assert!(!session_record.persisted_in_runtime_checkpoint);
    assert!(!session_record.restored_from_persisted_session);
    assert_eq!(
        session_record.runtime_checkpoint_source,
    ...
    assert!(session_record
        ...
        .starts_with("agent::harness_worker_session_record::"));
...
fn default_runtime_dispatch_proof_accepts_readonly_default_with_provider_visible_output_authority()
{
    ...
    assert!(dispatch
        ...
        .any(|id| id.contains("authority_tooling_mcp_provider_read_only")));
    assert!(dispatch
        .authority_tooling_mcp_tool_catalog_live_attempt_ids
        .iter()
        .any(|id| id.contains("authority_tooling_mcp_tool_call_read_only")));
    assert!(dispatch
        .authority_tooling_mcp_tool_catalog_live_receipt_ids
        .iter()
        .any(|id| id.contains("authority_tooling_mcp_tool_call_read_only")));
    assert!(dispatch
    ...
    assert!(dispatch
        .authority_tooling_connector_catalog_live_receipt_ids
        .iter()
        .any(|id| id.contains("authority_tooling_connector_call_read_only")));
    assert!(dispatch

...
pub enum KernelEvent {
    ...
    /// Streaming final-answer token from the inference engine.
    ///
    /// This is product-facing assistant output, not hidden cognition. Reasoning
    /// streams must use `AgentThought`; final handoff streams must use this
    /// event so clients can render answer text as it is generated.
    AgentAnswerDelta {
        /// The unique session ID this answer belongs to.
        session_id: [u8; 32],
        /// The partial answer text (token) generated by the LLM.
        token: String,
    },

# Canonical IAVL Leaf Hashing Profile
...
The `LeafOp` parameters for an ICS-23 proof are as follows:
...
| **`prehash_value`** | **`Sha256`**     | **The value is first hashed with SHA-256 to get a 32-byte digest.**         |
...
The final hash of a leaf node is calculated as follows:

**`Final Leaf Hash = SHA-256( Leaf Prefix || Key Preimage || Value Preimage )`**
...
3.  **`Value Preimage`**: The **SHA-256 hash** of the raw value, prefixed with its length (which is always `32`) as a Protobuf `varint`.
    *   `prost::encode_length_delimiter(32) || SHA-256(value)`

#### Concrete Example

*   `key`: `"foo"` (bytes `0x666f6f`)
*   `value`: `"bar"` (bytes `0x626172`)
...
3.  **Value Preimage**:
    ...
    *   Length of hash is 32. `VarProto(32)` is `0x20`.
    *   Preimage: `0x20fcde2b2edba56bf408601fb721fe9b5c338d10ee429ea04fae5511b68fbf8fb9`

4.  **Final Concatenated Preimage to be Hashed**:
    ```
    0x0003666f6f20fcde2b2edba56bf408601fb721fe9b5c338d10ee429ea04fae5511b68fbf8fb9
    ```
    `SHA-256` of this byte string produces the final leaf hash.
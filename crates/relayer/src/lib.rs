...
use crate::handshake::{
    ...
    build_update_client_any, existence_root_from_proof_bytes, infer_allocated_channel_id,
...
async fn commitment_root_latest_robust(gw: &Gateway) -> Result<(Vec<u8>, u64)> {
...
async fn commitment_root_at_height_robust(gw: &Gateway, height: u64) -> Result<(Vec<u8>, u64)> {
...
fn build_tm_header_for_root(
    ...
    Ok(RawTmHeader {
        ...
        trusted_height: Some(ibc_core_client_types::Height::new(0, trusted_height)?.into()),
...
pub async fn run_handshake(
    ...
    .await?;
    let (conn_a_id, h_init_a) = infer_allocated_connection_id(gw_a).await?;
    ...
    .await?;
    let (conn_b_id, h_try_b) = infer_allocated_connection_id(gw_b).await?;
    ...
    .await?;
    let (chan_a_id, h_chan_init_a) = infer_allocated_channel_id(gw_a).await?;
    h_a_on_b = update_b_about_a_to(
        gw_a,
        gw_b,
        client_a_on_b,
    ...
    .await?;
    let (chan_b_id, h_chan_try_b) = infer_allocated_channel_id(gw_b).await?;

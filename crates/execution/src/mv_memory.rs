...
pub struct MVMemory {
    ...
    /// We use RwLock to allow concurrent reads/writes to the vector of versions.
    data: DashMap<Vec<u8>, Arc<std::sync::RwLock<Vec<MemoryEntry>>>>,
...
impl MVMemory {
    ...
    pub fn read(&self, key: &[u8], tx_idx: TxIndex) -> Result<(Option<Vec<u8>>, ReadVersion), StateError> {
    ...
    pub fn write(&self, key: Vec<u8>, value: Option<Vec<u8>>, tx_idx: TxIndex) -> bool {
        let entry = self.data.entry(key).or_insert_with(|| Arc::new(std::sync::RwLock::new(Vec::new())));
        ...
        versions.insert(pos, MemoryEntry { version: tx_idx, value });
    ...
    pub fn validate_read_set(&self, read_set: &[(Vec<u8>, ReadVersion)], tx_idx: TxIndex) -> Result<bool, StateError> {
    ...
    pub fn apply_to_overlay(&self, overlay: &mut ioi_api::state::StateOverlay) -> Result<(), StateError> {
        // In Block-STM, only the final committed versions matter.
        // We iterate all keys, pick the highest version, and apply.
        for r in self.data.iter() {
            let key = r.key();
            let versions = r.value().read().unwrap();

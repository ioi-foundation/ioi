...
impl ModuleUpgradeManager {
    ...
    pub async fn execute_upgrade(
        ...
        if stored_val != embedded_val {
            ...
            let s_hash = depin_sdk_crypto::algorithms::hash::sha256(manifest_str.as_bytes())
                .map(|h| hex::encode(h))
                .unwrap_or_else(|_| "<hash-error>".into());
            let e_hash =
                depin_sdk_crypto::algorithms::hash::sha256(embedded_manifest_str.as_bytes())
                    .map(|h| hex::encode(h))
                    .unwrap_or_else(|_| "<hash-error>".into());
            log::error!(
        ...
        let artifact_hash = depin_sdk_crypto::algorithms::hash::sha256(artifact)?;
        ...
        if !self.active_services.contains_key(service_id) {
            ...
            let meta_bytes = depin_sdk_types::codec::to_bytes_canonical(&full_meta)?;
            state

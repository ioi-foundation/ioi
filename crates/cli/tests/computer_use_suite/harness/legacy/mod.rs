...
pub use mode_runner::{persist_mode_report, run_mode_with_case_sink};
        ...
        let result = self
            .execute_tool_with_state_timeout(
                AgentTool::BrowserSyntheticClick {
                    id: None,
                    x: Some(validate_pointer_coordinate(x)?),
                    y: Some(validate_pointer_coordinate(y)?),
                    continue_with: None,
                },
...
mod tests {
    use super::{
        ...
        bridge_hidden_tab_selector_for_label, count_shape_matches, drain_events,
        email_inbox_action_value, email_inbox_forward_value, email_inbox_reply_value,
        email_inbox_sender_value, find_email_inbox_row_index, headless_for_run,
        odd_or_even_visible_numbers, parse_count_shape_descriptor, parse_email_inbox_action,
    ...
    fn run_async_with_large_stack<F>(future: F) -> anyhow::Result<()>
    where
        F: Future<Output = anyhow::Result<()>> + Send + 'static,
    {
        std::thread::Builder::new()
            .name("miniwob-large-stack".to_string())
            ...
            .expect("spawn large-stack MiniWoB regression thread")
            .join()
            .map_err(|_| anyhow::anyhow!("large-stack MiniWoB regression thread panicked"))?
    }
    ...
    #[test]
    #[ignore = "requires Chromium runtime, MiniWoB bridge helper, and local task assets"]
    fn stock_market_ready_state_ignores_startup_pointer_noise() -> anyhow::Result<()> {
        run_async_with_large_stack(async move {
            ...
            let config = SuiteConfig {
                ...
                artifact_root: std::env::temp_dir()
                    .join("ioi-computer-use-stock-market-startup-pointer-noise"),
                retain_artifacts_for_all_runs: false,
                require_browser_display: false,
                bridge_source_dir: Some(source_dir.clone()),
                python_bin: std::env::var("COMPUTER_USE_SUITE_PYTHON")
            ...
            let case = cases_for_task_set(TaskSet::Catalog, Some(source_dir.as_path()))?
                .into_iter()
                .find(|case| case.env_id == "stock-market")
                .ok_or_else(|| anyhow::anyhow!("stock-market case missing from catalog"))?;
            ...
            let result = async {
                ...
                anyhow::ensure!(
                    ...
                    "stock-market ready state advanced before any tool input: episode_step={} last_event={:?} trigger={:?} sync_tail={:?}",
                    state.episode_step,
                    state.info.last_event,
                    state.info.trigger,
                    sync_tail
                );
                anyhow::ensure!(
                    state.info.last_event.is_none(),
                    "stock-market ready state recorded unexpected startup event: {:?}",
                    state.info.last_event
                );
    ...
    #[test]
    #[ignore = "requires Chromium runtime, MiniWoB bridge helper, and local task assets"]
    fn stock_market_start_gate_dom_id_alias_click_succeeds_directly() -> anyhow::Result<()> {
        run_async_with_large_stack(async move {
            crate::computer_use_suite::live_inference_support::load_env_from_workspace_dotenv_if_present();
            ...
            let config = SuiteConfig {
                ...
                max_cases: None,
                artifact_root: std::env::temp_dir()
                    .join("ioi-computer-use-stock-market-start-gate"),
                retain_artifacts_for_all_runs: false,
                require_browser_display: false,
                bridge_source_dir: Some(source_dir.clone()),
                python_bin: std::env::var("COMPUTER_USE_SUITE_PYTHON")
            ...
            let result = async {
                ...
                let click = harness
                    .execute_tool(AgentTool::BrowserClickElement {
                        id: Some("sync-task-cover".to_string()),
                        ids: Vec::new(),
                        delay_ms_between_ids: None,
                        continue_with: None,
                    })
                ...
                anyhow::ensure!(
                    click.success,
                    "stock-market start gate alias click failed: history={:?} error={:?}",
                    click.history_entry,
                    click.error
                );
                ...
                anyhow::ensure!(
                    ...
                    "stock-market start gate did not advance episode step: {:?}",
                    state.info.last_event
                );
                anyhow::ensure!(
                    state
                        ...
                        == Some("click"),
                    "stock-market start gate did not record click event: {:?}",
                    state.info.last_event
                );
                anyhow::ensure!(
                    state
                        ...
                        .last_event
                        .as_ref()
                        .and_then(|event| event.target_id.as_deref())
                        == Some("sync-task-cover"),
                    "stock-market start gate click targeted unexpected element: {:?}",
                    state.info.last_event
                );
    ...
    #[test]
    #[ignore = "requires Chromium runtime, MiniWoB bridge helper, and local task assets"]
    fn stock_market_buy_button_click_returns_promptly() -> anyhow::Result<()> {
        run_async_with_large_stack(async move {
            crate::computer_use_suite::live_inference_support::load_env_from_workspace_dotenv_if_present();
            ...
            let config = SuiteConfig {
                ...
                max_cases: None,
                artifact_root: std::env::temp_dir()
                    .join("ioi-computer-use-stock-market-buy-button"),
                retain_artifacts_for_all_runs: false,
                require_browser_display: false,
                bridge_source_dir: Some(source_dir.clone()),
                python_bin: std::env::var("COMPUTER_USE_SUITE_PYTHON")
            ...
            let result = async {
                ...
                let created = client.create_session(&case).await?;
                let session_id = created.session_id.clone();
                let initial_last_sync_ms = created.state.last_sync_ms;
                let mut harness = DirectHarness::new(
                    client.clone(),
                    session_id.clone(),
                    created.state,
                ...
                harness.wait_until_ready().await?;
                harness.step_index = harness.step_index.saturating_add(1);
                let snapshot = harness
                    .exec
                    .execute(
                        AgentTool::BrowserSnapshot {},
                        harness.session_bytes,
                        harness.step_index,
                        [0u8; 32],
                        None,
                        None,
                    ...
                    .await;
                let snapshot_buy_lines = snapshot
                    .history_entry
                    .as_deref()
                    .unwrap_or_default()
                    .lines()
                    .filter(|line| {
                        line.contains("buy")
                            || line.contains("Buy")
                            || line.contains("[id=\"buy\"]")
                    })
                    .take(8)
                    .map(str::trim)
                    .map(str::to_string)
                    .collect::<Vec<_>>();
                let hash_probe = harness.browser.probe_selector("#buy").await.ok();
                let attr_probe = harness.browser.probe_selector("[id=\"buy\"]").await.ok();
                ...
                let click = click
                    .map_err(|_| {
                        anyhow::anyhow!(
                            "stock-market buy click timed out: reward={} raw_reward={:?} terminated={} episode_step={} last_event={:?} trigger={:?} snapshot_buy_lines={:?} hash_probe={:?} attr_probe={:?} sync_tail={:?} kernel_events={:?}",
                            state.reward,
                ...
                anyhow::ensure!(
                    click.success,
                    "stock-market buy click failed: history={:?} error={:?} reward={} raw_reward={:?} terminated={} last_event={:?} snapshot_buy_lines={:?} hash_probe={:?} attr_probe={:?} sync_tail={:?}",
                    click.history_entry,
                    click.error,
                    state.reward,
                    state.info.raw_reward,
                ...
                anyhow::ensure!(
                    state.last_sync_ms != initial_last_sync_ms || state.terminated,
                    "stock-market buy click completed without bridge sync advance: last_sync_ms={:?} state={:?}",
                    state.last_sync_ms,
                    state
                );
    ...
    #[test]
    #[ignore = "requires Chromium runtime, MiniWoB bridge helper, and local task assets"]
    fn stock_market_buy_selector_click_returns_promptly() -> anyhow::Result<()> {
        run_async_with_large_stack(async move {
            crate::computer_use_suite::live_inference_support::load_env_from_workspace_dotenv_if_present();
            ...
            let config = SuiteConfig {
                ...
                max_cases: None,
                artifact_root: std::env::temp_dir()
                    .join("ioi-computer-use-stock-market-buy-selector"),
                retain_artifacts_for_all_runs: false,
                require_browser_display: false,
                bridge_source_dir: Some(source_dir.clone()),
                python_bin: std::env::var("COMPUTER_USE_SUITE_PYTHON")
            ...
            let result = async {
                ...
                let click = click
                    .map_err(|_| {
                        anyhow::anyhow!(
                            "stock-market selector click timed out: reward={} raw_reward={:?} terminated={} episode_step={} last_event={:?} trigger={:?} sync_tail={:?} kernel_events={:?}",
                            state.reward,
                            state.info.raw_reward,
                            state.terminated,
                            state.episode_step,
                ...
                anyhow::ensure!(
                    click.success,
                    "stock-market selector click failed: history={:?} error={:?} reward={} raw_reward={:?} terminated={} last_event={:?} sync_tail={:?}",
                    click.history_entry,
                    click.error,
                    state.reward,
                    state.info.raw_reward,
                ...
                anyhow::ensure!(
                    state.last_sync_ms != initial_last_sync_ms || state.terminated,
                    "stock-market selector click completed without bridge sync advance: last_sync_ms={:?} state={:?}",
                    state.last_sync_ms,
                    state
                );
                anyhow::ensure!(
                    state.terminated,
                    "stock-market selector click did not terminate the episode: reward={} raw_reward={:?} last_event={:?} sync_tail={:?}",
                    state.reward,
                    state.info.raw_reward,
                    state.info.last_event,
                    sync_tail
                ...
                anyhow::ensure!(
                    state
                        ...
                        .and_then(|event| event.target_id.as_deref())
                        == Some("buy"),
                    "stock-market selector click targeted unexpected element: {:?}",
                    state.info.last_event
                );
    ...
    #[test]
    #[ignore = "requires Chromium runtime, MiniWoB bridge helper, and local task assets"]
    fn stock_market_buy_semantic_id_click_returns_promptly() -> anyhow::Result<()> {
        run_async_with_large_stack(async move {
            crate::computer_use_suite::live_inference_support::load_env_from_workspace_dotenv_if_present();
            ...
            let config = SuiteConfig {
                ...
                max_cases: None,
                artifact_root: std::env::temp_dir()
                    .join("ioi-computer-use-stock-market-buy-semantic-id"),
                retain_artifacts_for_all_runs: false,
                require_browser_display: false,
                bridge_source_dir: Some(source_dir.clone()),
                python_bin: std::env::var("COMPUTER_USE_SUITE_PYTHON")
            ...
            let result = async {
                ...
                let sync_tail = state
                    ...
                    .cloned()
                    .collect::<Vec<_>>();
                let click = click.map_err(|_| {
                    anyhow::anyhow!(
                        "stock-market semantic-id click timed out: reward={} raw_reward={:?} terminated={} episode_step={} last_event={:?} trigger={:?} snapshot_buy_lines={:?} hash_probe={:?} attr_probe={:?} sync_tail={:?} kernel_events={:?}",
                        state.reward,
                        state.info.raw_reward,
                        state.terminated,
                        state.episode_step,
                ...
                anyhow::ensure!(
                    click.success,
                    "stock-market semantic-id click failed: history={:?} error={:?} reward={} raw_reward={:?} terminated={} last_event={:?} snapshot_buy_lines={:?} hash_probe={:?} attr_probe={:?} sync_tail={:?}",
                    click.history_entry,
                    click.error,
                    state.reward,
                    state.info.raw_reward,
                ...
                anyhow::ensure!(
                    state.last_sync_ms != initial_last_sync_ms || state.terminated,
                    "stock-market semantic-id click completed without bridge sync advance: last_sync_ms={:?} state={:?}",
                    state.last_sync_ms,
                    state
                );
    ...
    #[test]
    #[ignore = "requires Chromium runtime, MiniWoB bridge helper, and local task assets"]
    fn stock_market_buy_attribute_selector_click_returns_promptly() -> anyhow::Result<()> {
        run_async_with_large_stack(async move {
            crate::computer_use_suite::live_inference_support::load_env_from_workspace_dotenv_if_present();
            ...
            let config = SuiteConfig {
                ...
                max_cases: None,
                artifact_root: std::env::temp_dir()
                    .join("ioi-computer-use-stock-market-buy-attribute-selector"),
                retain_artifacts_for_all_runs: false,
                require_browser_display: false,
                bridge_source_dir: Some(source_dir.clone()),
                python_bin: std::env::var("COMPUTER_USE_SUITE_PYTHON")
            ...
            let result = async {
                ...
                let click = click
                    .map_err(|_| {
                        anyhow::anyhow!(
                            "stock-market attribute-selector click timed out: reward={} raw_reward={:?} terminated={} episode_step={} last_event={:?} trigger={:?} sync_tail={:?} kernel_events={:?}",
                            state.reward,
                            state.info.raw_reward,
                            state.terminated,
                            state.episode_step,
                ...
                anyhow::ensure!(
                    click.success,
                    "stock-market attribute-selector click failed: history={:?} error={:?} reward={} raw_reward={:?} terminated={} last_event={:?} sync_tail={:?}",
                    click.history_entry,
                    click.error,
                    state.reward,
                    state.info.raw_reward,
                ...
                anyhow::ensure!(
                    state.last_sync_ms != initial_last_sync_ms || state.terminated,
                    "stock-market attribute-selector click completed without bridge sync advance: last_sync_ms={:?} state={:?}",
                    state.last_sync_ms,
                    state
                );

...
use ioi_types::{app::{ActionRequest, ContextSlice}, codec, error::VmError};
use serde_json::json;
use std::path::Path;
...
impl GuiDriver for MockGuiDriver {
    async fn capture_screen(&self) -> Result<Vec<u8>, VmError> {
        // Return valid PNG header for image crate
    ...
    async fn capture_context(&self, _: &ActionRequest) -> Result<ContextSlice, VmError> {
        Ok(ContextSlice { slice_id: [0;32], data: vec![], provenance_proof: vec![], intent_id: [0;32] })
    }
...
impl InferenceRuntime for ResilientBrain {
    async fn execute_inference(&self, _hash: [u8; 32], _input: &[u8], _opts: ioi_types::app::agentic::InferenceOptions) -> Result<Vec<u8>, VmError> {
...
async fn test_agent_self_healing() -> Result<()> {
    ...
    let gui = Arc::new(MockGuiDriver { fail_count: Arc::new(Mutex::new(0)) });
    ...
    service.handle_service_call(&mut state, "step@v1", &step_bytes, &mut ctx).await?;
    
    // Check State
    let key = [b"agent::state::".as_slice(), session_id.as_slice()].concat();
    let state_1: AgentState = codec::from_bytes_canonical(&state.get(&key).unwrap().unwrap()).unwrap();
    assert_eq!(state_1.consecutive_failures, 1);
    
    // Step 2: Fail (Count 1 -> 2)
    service.handle_service_call(&mut state, "step@v1", &step_bytes, &mut ctx).await?;
    let state_2: AgentState = codec::from_bytes_canonical(&state.get(&key).unwrap().unwrap()).unwrap();
    assert_eq!(state_2.consecutive_failures, 2);

    // Step 3: Success (Count 2 -> 0)
    service.handle_service_call(&mut state, "step@v1", &step_bytes, &mut ctx).await?;
    let state_3: AgentState = codec::from_bytes_canonical(&state.get(&key).unwrap().unwrap()).unwrap();

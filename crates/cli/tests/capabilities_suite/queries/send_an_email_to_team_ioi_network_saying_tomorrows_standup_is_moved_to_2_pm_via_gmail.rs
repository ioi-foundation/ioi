...
pub fn case() -> QueryCase {
    QueryCase {
        id: CASE_ID,
        query: "Send an email to team@ioi.network saying tomorrow's standup is moved to 2 PM via Gmail.",
        success_definition: "Send the requested Gmail message through the Google connector with structured send evidence, CEC receipts, grounded recipient evidence, and no contract-failure markers.",
        seeded_intent_id: "gmail.send_email",
...
fn evaluate(obs: &RunObservation) -> LocalJudgeResult {
    ...
    let latest_payload = google.gmail_send_payloads.last();
    let payload_debug = latest_payload
        .and_then(|payload| serde_json::to_string(payload).ok())
        .unwrap_or_else(|| "null".to_string());
    let send_succeeded =
        google.gmail_send_success_count > 0 && google.gmail_send_failure_count == 0;
    ...
    let message_id_present = latest_payload
        ...
        .map(|value| !value.trim().is_empty())
        .unwrap_or(false);
    let sent_label_present = latest_payload
        .map(|payload| {
            payload
                .label_ids
                .iter()
    ...
    let tool_planned_seen = obs.planned_tool_calls.iter().any(|call| {
        call.tool_name
            .eq_ignore_ascii_case("connector__google__gmail_send_email")
    });
    let tool_route_seen =
    ...
    let independent_channel_count = [
        ...
        message_id_present && sent_label_present,
        tool_planned_seen,
        tool_route_seen,
    ...
    let checks = vec![
        ...
        LocalCheck::new(
            "objective_specific_gmail_send_evidence_present",
            send_succeeded
                && verification_postcondition_present
                && subject_present
                ...
                && sent_label_present,
            format!(
                "send_success_count={} send_failure_count={} verification_postcondition_present={} subject_present={} body_request_signal_present={} message_id_present={} sent_label_present={} payload={}",
                google.gmail_send_success_count,
                google.gmail_send_failure_count,
                verification_postcondition_present,
                subject_present,
                body_request_signal_present,
                message_id_present,

...
impl QuvAcceptedHistoryV0 {
    ...
    pub fn from_provisioned_initial(
        initial: QuvSlotV0,
        authority_slots: u32,
        max_preparation_attempts: u16,
    ...
    pub fn validate(&self) -> Result<(), QuvError> {
        if self.initial.slot == 0
            || self.authority_slots == 0
            || self.max_preparation_attempts == 0
            || self.authority_slots as usize > STORE_MAX_SLOTS
            || self
                .initial
                .slot
                .checked_add(u64::from(self.authority_slots) - 1)
                .is_none()
            ...
            || self.accepted.len() > self.authority_slots as usize
    ...
    pub fn next_slot(&self) -> Option<u64> {
        if self.accepted.len() >= self.authority_slots as usize {
            return None;
        }
    ...
    pub fn expected_predecessor(&self, slot: u64) -> Result<QuvHash, QuvError> {
        ...
        if offset >= self.authority_slots as usize || offset > self.accepted.len() {
...
impl QuvMemberDomainV0 {
    /// Enroll all history/resource limits from the same independently supplied
    /// policy preimage used for its commitment. No request selects these limits.
    pub fn from_provisioned_policy(
        configuration_root: QuvHash,
        network_id: QuvHash,
        policy: &ioi_types::config::AftQuvDomainPolicyV0,
    ) -> Result<Self, QuvError> {
        let policy_root = super::quv_policy_root(
            policy.domain_id,
        ...
        Self::from_provisioned_bootstrap(
            configuration_root,
            policy_root,
            network_id,
            policy.domain_id,
            ...
            &policy.bootstrap,
            policy.authority_slots,
            policy.preparation.max_attempts_per_slot(),
        )
    }
    ...
    pub fn from_provisioned_bootstrap(
        ...
        if !bootstrap.is_valid_for(authority_mode)
            || authority_slots == 0
            || authority_slots as usize > STORE_MAX_SLOTS
        {
            return Err(QuvError::InvalidAcceptedHistory);
        }
        ...
        let value = match bootstrap {
            QuvDomainBootstrapV0::Fixed { .. } => {
                Self::Fixed(QuvAcceptedHistoryV0::from_provisioned_initial(
                    initial,
                    authority_slots,
                    max_preparation_attempts,
                )?)
            }
            QuvDomainBootstrapV0::HandoffBoundary { .. } => {
                if max_preparation_attempts != 0 {
                    return Err(QuvError::InvalidAcceptedHistory);
                }
    ...
    pub(super) fn validate(&self) -> Result<(), QuvError> {
        match self {
            ...
            Self::HandoffBoundary(initial) => {
                ...
                QuvAcceptedHistoryV0::from_provisioned_initial(syntax, 1, 1).map(|_| ())
            }
        }
    }

    pub(super) fn slot_capacity(&self) -> u64 {
        match self {
            Self::Fixed(history) => u64::from(history.authority_slots),
            Self::HandoffBoundary(_) => 1,
        }
    }

    /// Two candidate inserts, A reservations and one head record per Fixed
    /// slot; handoff member domains have only the two candidate inserts.
    pub(super) fn retained_record_capacity(&self) -> u64 {
        match self {
            Self::Fixed(history) => {
                self.slot_capacity() * (u64::from(history.max_preparation_attempts) + 3)
            }
            Self::HandoffBoundary(_) => 2,
        }
    }

    pub(super) fn preparation_capacity(&self) -> u16 {
        match self {
            Self::Fixed(history) => history.max_preparation_attempts,
            Self::HandoffBoundary(_) => 0,
        }
    }

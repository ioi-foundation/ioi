...
/// Convergent Fault Tolerance wrapper.
///
/// Production decisions are made by the deterministic convergent core. The
/// retained experimental sampling path survives only as a sidecar for
/// `ExperimentalNestedGuardian`.
#[derive(Debug, Clone)]
pub struct ConvergentEngine {
    core: GuardianMajorityEngine,
    experimental_observability: ExperimentalSamplingEngine,
}

impl Default for ConvergentEngine {
    fn default() -> Self {
        Self::new(ConvergentSafetyMode::ClassicBft)
    }
}

impl ConvergentEngine {
    pub fn new(mode: ConvergentSafetyMode) -> Self {
        Self {
            core: GuardianMajorityEngine::new(mode),
...
impl ConsensusControl for ConvergentEngine {
...
#[async_trait]
impl PenaltyMechanism for ConvergentEngine {
...
impl PenaltyEngine for ConvergentEngine {
    fn apply(
        &self,
...
#[async_trait]
impl<T> ConsensusEngine<T> for ConvergentEngine
where

...
use ioi_types::app::{
    account_id_from_key_material, compute_next_timestamp_ms, effective_set_for_height,
    read_validator_sets, timestamp_millis_to_legacy_seconds, write_validator_sets, AccountId,
    Block, BlockTimingParams, BlockTimingRuntime, ChainStatus, ConsensusVote, FailureReport,
    QuorumCertificate, ValidatorSetV1, ValidatorSetsV1,
};
use ioi_types::codec;
...
fn log_vs(label: &str, height: u64, sets: &ValidatorSetsV1) {
    let log_one = |name: &str, vs: &ValidatorSetV1| {
        ...
        for validator in vs.validators.iter().take(8) {
            log::info!(
                "  - acct=0x{} w={}",
...
impl ConsensusControl for ProofOfStakeEngine {
...
impl PenaltyMechanism for ProofOfStakeEngine {
    async fn apply_penalty(
        ...
        let next_vs = sets
            .next
            .as_mut()
            .expect("next validator set inserted above");
    ...
    for ProofOfStakeEngine
{
    async fn decide(
        ...
        let leader_account_id = match self.select_leader(height, vs) {
            Some(account_id) => account_id,
            None => {
                log::error!(
                    "[PoS Decide H={}] Leader selection returned None. Stalling.",
                    height
                );
                return ConsensusDecision::Stall;
            }

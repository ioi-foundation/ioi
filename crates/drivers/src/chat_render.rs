...
use ioi_api::chat::{
    build_chat_artifact_render_acceptance_policy, ChatArtifactAcceptanceObligation,
...
fn chat_render_trace(message: impl AsRef<str>) {
    if std::env::var_os("IOI_CHAT_PROOF_TRACE").is_some() {
        eprintln!("[chat-proof-trace] {}", message.as_ref());
    }
}

pub struct BrowserChatArtifactRenderEvaluator {
    browser: BrowserDriver,
}

impl Default for BrowserChatArtifactRenderEvaluator {
    fn default() -> Self {
...
impl BrowserChatArtifactRenderEvaluator {
    fn sampling_policy(
        &self,
        acceptance_policy: &ChatArtifactRenderAcceptancePolicy,
    ...
    async fn perform_action_probe(
        ...
        let (status, summary, detail) = match action_result {
            Err(error) => (
                ...
                format!("Chat could not exercise '{}'.", action.label),
                Some(error),
            ),
            ...
            Ok(()) if state_changed => (
                ChatArtifactExecutionWitnessStatus::Passed,
...
#[async_trait]
impl ChatArtifactRenderEvaluator for BrowserChatArtifactRenderEvaluator {
    async fn evaluate_candidate_render(
        ...
        let acceptance_policy =
            build_chat_artifact_render_acceptance_policy(request, brief, blueprint, artifact_ir);
...
fn build_preview_bundle(
    ...
    let root_dir = std::env::temp_dir().join(format!("chat-render-{}", Uuid::new_v4()));
...
fn write_generated_files(
    root_dir: &Path,
    files: &[ChatGeneratedArtifactFile],
) -> Result<(), String> {
    for file in files {
...
fn render_runtime_witness_collector_install_script() -> &'static str {
    r#"(() => {
  const root = window.__chatRenderWitness || { errors: [], installed: false };
  ...
  window.__chatRenderWitness = root;
  return true;
})()"#
}
...
fn render_runtime_witness_state_script() -> &'static str {
    r#"(() => {
  const root = window.__chatRenderWitness || { errors: [] };
...
fn render_runtime_witness_clear_script() -> &'static str {
    r#"(() => {
  const root = window.__chatRenderWitness || { errors: [], installed: true };
  root.errors = [];
  window.__chatRenderWitness = root;
  return true;
})()"#
...
) -> Vec<ChatArtifactExecutionWitness> {
    witnesses
        .iter()
        .enumerate()
        .map(|(index, witness)| ChatArtifactExecutionWitness {
...
fn build_browser_acceptance_obligations(
    request: &ChatOutcomeArtifactRequest,
    observation: &ChatArtifactRenderObservation,
    acceptance_policy: &ChatArtifactRenderAcceptancePolicy,
    desktop: &ViewportCapture,
    mobile: &ViewportCapture,
    interaction_expected: bool,
    boot_errors: &[String],
    execution_witnesses: &[ChatArtifactExecutionWitness],
) -> Vec<ChatArtifactAcceptanceObligation> {
    let requires_interaction_contract = request.renderer == ChatRendererKind::HtmlIframe
        && (request.artifact_class == ioi_types::app::ChatArtifactClass::InteractiveSingleFile
            || interaction_expected);
    let successful_witness_count = execution_witnesses
        .iter()
        .filter(|witness| witness.status == ChatArtifactExecutionWitnessStatus::Passed)
        .count();
    ...
    if requires_interaction_contract {
        obligations.push(ChatArtifactAcceptanceObligation {
            ...
            summary: "Chat discovered surfaced controls to exercise.".to_string(),
            detail: Some(format!(
...
fn append_obligation_findings(
    findings: &mut Vec<ChatArtifactRenderFinding>,
    obligations: &[ChatArtifactAcceptanceObligation],
) {
    for obligation in obligations {
        ...
        let severity = match obligation.status {
            ChatArtifactAcceptanceObligationStatus::Passed
            | ChatArtifactAcceptanceObligationStatus::NotApplicable => continue,
            ChatArtifactAcceptanceObligationStatus::Failed
            | ChatArtifactAcceptanceObligationStatus::Blocked => {
...
fn render_evaluation_summary(
    ...
    let cleared_required = obligations
        ...
        .filter(|obligation| obligation.status == ChatArtifactAcceptanceObligationStatus::Passed)
        .count();
...
fn score_render_evaluation(
    ...
    let mut findings = Vec::<ChatArtifactRenderFinding>::new();
    if desktop.capture.screenshot_byte_count == 0 || mobile.capture.screenshot_byte_count == 0 {
        findings.push(ChatArtifactRenderFinding {
            ...
            summary: "Desktop and mobile render captures must both exist before Chat can trust the surfaced first paint."
                .to_string(),
        });
    }
...
fn score_blueprint_consistency(
    ...
    let document_like_brief = matches!(
        ...
        Some(ChatRendererKind::Markdown | ChatRendererKind::PdfEmbed)

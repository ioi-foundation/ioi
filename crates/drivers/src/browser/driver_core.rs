...
const BROWSER_LAUNCH_TIMEOUT_ENV: &str = "IOI_BROWSER_LAUNCH_TIMEOUT_MS";
...
fn is_retryable_launch_error(message: &str) -> bool {
    ...
    lower.contains("before websocket url could be resolved")
        || lower.contains("browser launch timed out after")
...
fn browser_launch_timeout() -> Duration {
    std::env::var(BROWSER_LAUNCH_TIMEOUT_ENV)
        .ok()
        .and_then(|raw| raw.parse::<u64>().ok())
        .filter(|ms| *ms > 0)
        .map(Duration::from_millis)
        .unwrap_or_else(|| Duration::from_millis(DEFAULT_BROWSER_LAUNCH_TIMEOUT_MS))
}
...
impl BrowserDriver {
    ...
    pub async fn launch(&self, headless: bool) -> std::result::Result<(), BrowserError> {
        ...
        let run_launch_attempt = |bin: PathBuf,
            ...
            let launch_timeout = browser_launch_timeout();
            ...
            match config_res {
                Ok(cfg) => match tokio::time::timeout(launch_timeout, Browser::launch(cfg)).await {
                    Ok(Ok(tuple)) => return Ok(tuple),
                    Ok(Err(e)) => {
                        let msg = e.to_string();
                    ...
                    Err(_) => {
                        return Err(format!(
                            "Browser launch timed out after {}ms waiting for websocket URL resolution",
            ...
            match tokio::time::timeout(launch_timeout, Browser::launch(config_fallback)).await {

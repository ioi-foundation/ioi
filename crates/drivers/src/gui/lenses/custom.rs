...
impl AppLens for ConfigurableLens {
    ...
    fn matches(&self, window_title: &str) -> bool {
        ...
        window_title.to_lowercase().contains(&self.config.app_name.to_lowercase())
    }
    ...
    fn transform(&self, node: &AccessibilityNode) -> Option<AccessibilityNode> {
        ...
        for (semantic_name, selector) in &self.config.mappings {
            ...
            let id_match = selector.id_pattern.as_ref().map_or(true, |p| node.id.contains(p));
            ...
            if role_match && name_match && id_match {
                ...
                semantic_node.attributes.insert("semantic_role".to_string(), semantic_name.clone());
                // Crucial: Set a data-testid equivalent so the agent can select it easily
                semantic_node.attributes.insert("data-testid".to_string(), semantic_name.clone());
        ...
        let new_children: Vec<AccessibilityNode> = node.children.iter()

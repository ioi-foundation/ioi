...
impl AutoLens {
    ...
    fn generate_robust_id(
        ...
        let prefers_dom_id = self.prefers_dom_id_identity(node)
            && self
                .first_non_empty_attr(node, &["dom_id"])
                .is_some_and(|value| !value.trim().is_empty());

        // Strategy A: Content-derived (Best for humans/agents)
        // Priority: automation IDs and labels first, then visible text.
        let raw_name = if prefers_dom_id {
            [
                self.first_non_empty_attr(node, &["dom_id"]),
                ...
                self.first_non_empty_attr(
                    node,
                    &["placeholder", "aria-label", "title", "description"],
                ),
            ]
            .into_iter()
            .flatten()
    ...
    fn prefers_dom_id_identity(&self, node: &AccessibilityNode) -> bool {
        if self.first_non_empty_attr(node, &["dom_id"]).is_none() {
            return false;
        }

        if node.attributes.contains_key("shape_kind") {
            return true;
        }

        matches!(
            self.first_non_empty_attr(node, &["tag_name"])

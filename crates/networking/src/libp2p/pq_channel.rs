...
mod tests {
    ...
    fn peer_id_ordered_after(reference: &PeerId) -> PeerId {
        loop {
            let candidate = Keypair::generate_ed25519().public().to_peer_id();
            if candidate.to_bytes() > reference.to_bytes() {
                return candidate;
            }
        }
    }

    fn peer_id_ordered_before(reference: &PeerId) -> PeerId {
        loop {
            let candidate = Keypair::generate_ed25519().public().to_peer_id();
            if candidate.to_bytes() < reference.to_bytes() {
                return candidate;
    ...
    /// Runs the full `aft-pq-channel-v1` handshake in whichever direction the
    /// deterministic initiator rule selects, ending with both sides
    /// application-ready.
    fn complete_handshake(
        local: &mut PqChannelSessionManager,
        local_peer: PeerId,
        remote: &mut PqChannelSessionManager,
        remote_peer: PeerId,
    ) {
        if local.should_initiate(&remote_peer) {
            let hello = local.start(remote_peer).unwrap();
            let server = remote.accept(local_peer, hello).unwrap();
            let finish = local.finish(remote_peer, server).unwrap();
            remote.complete(local_peer, finish).unwrap();
            local.confirm_application_ready(&remote_peer).unwrap();
        } else {
            let hello = remote.start(local_peer).unwrap();
            let server = local.accept(remote_peer, hello).unwrap();
            let finish = remote.finish(local_peer, server).unwrap();
            local.complete(remote_peer, finish).unwrap();
            remote.confirm_application_ready(&local_peer).unwrap();
        }
        assert!(local.is_application_ready(&remote_peer));
        assert!(remote.is_application_ready(&local_peer));
    }

    fn assert_peer_fully_forgotten(manager: &PqChannelSessionManager, peer: &PeerId) {
        assert!(!manager.enrollments.contains_key(peer));
        assert!(!manager.peer_capabilities.contains_key(peer));
        assert!(!manager.provisional_enrollments.contains_key(peer));
        assert!(!manager.authenticated_enrollments.contains(peer));
        assert!(!manager.pending_initiators.contains_key(peer));
        assert!(!manager.pending_responders.contains_key(peer));
        assert!(!manager.sessions.contains_key(peer));
    }

    fn quv_outbox_entries(
        manager: &PqChannelSessionManager,
        recipient: AccountId,
    ) -> Vec<Arc<PqOutboxEntryV2>> {
        manager
            .outbox
            .state
            .entries
    ...
    #[test]
    fn unproven_squatter_disconnect_releases_account_for_genuine_carrier() {
        let temp = tempfile::tempdir().unwrap();
        let local = local_config(44, temp.path().join("local-release.outbox"));
        let remote = local_config(45, temp.path().join("remote-release.outbox"));
        let claimed_account = remote.account_id;
        // Order the squatter after the local carrier so the local endpoint is
        // the deterministic initiator and holds pending KEM state for it.
        let squatter = peer_id_ordered_after(&local.peer_id);
        let mut manager = PqChannelSessionManager::new(local.clone()).unwrap();
        manager
            .enroll_peer(PqPeerEnrollment {
                peer_id: squatter,
            ...
            .unwrap();
        manager.start(squatter).unwrap();
        assert!(manager.enrollments.contains_key(&squatter));
        assert!(manager.peer_capabilities.contains_key(&squatter));
        assert!(manager.provisional_enrollments.contains_key(&squatter));
        assert!(manager.pending_initiators.contains_key(&squatter));
        // An unproven claim never routes, even while it is enrolled.
        assert_eq!(manager.peer_for_account(claimed_account), None);

        // Connection loss or handshake failure erases the unproven claim in
        // full: no enrollment, capability, provisional stamp or KEM state.
        manager.disconnect(&squatter);
        assert_peer_fully_forgotten(&manager, &squatter);
        assert_eq!(manager.peer_for_account(claimed_account), None);
        assert!(manager.start(squatter).is_err());

        let mut genuine = PqChannelSessionManager::new(remote.clone()).unwrap();
        genuine.enroll_peer(enrollment_of(&local)).unwrap();
        manager.enroll_peer(enrollment_of(&remote)).unwrap();
        complete_handshake(&mut manager, local.peer_id, &mut genuine, remote.peer_id);
        assert_eq!(
            manager.peer_for_account(claimed_account),
            Some(remote.peer_id)
        );
        assert!(manager.authenticated_enrollments.contains(&remote.peer_id));
        assert!(manager.provisional_enrollments.is_empty());
        assert_peer_fully_forgotten(&manager, &squatter);
    }

    #[test]
    fn handoff_only_unproven_claim_cannot_squat_or_route() {
        let temp = tempfile::tempdir().unwrap();
        let old_member = local_config(70, temp.path().join("old-member-squat.outbox"));
        let successor = local_config(71, temp.path().join("successor-squat.outbox"));
        let staged_account = successor.account_id;
        let squatter = peer_id_ordered_after(&old_member.peer_id);
        let mut manager = PqChannelSessionManager::new(old_member.clone()).unwrap();

        // A carrier that merely claims the staged successor account through
        // status metadata is admitted provisionally and only for handoff.
        manager
            .enroll_handoff_peer(PqPeerEnrollment {
                peer_id: squatter,
                account_id: staged_account,
                identity_key_hash: successor.identity_key_hash,
            })
            .unwrap();
        manager.start(squatter).unwrap();
        assert_eq!(
            manager.peer_capabilities.get(&squatter),
            Some(&PqChannelCapabilityV1::HandoffOnlySuccessor)
        );
        assert_eq!(manager.peer_for_account(staged_account), None);
        assert!(manager.pending_peers().is_empty());
        let reply = PqConsensusPayloadV1::QuvReply(
            codec::to_bytes_canonical(&quv_reply(3, old_member.account_id)).unwrap(),
        );
        // Account-addressed durable traffic stays durable for the rooted
        // account; it is never drainable through the unproven carrier.
        manager
            .enqueue_quv_reply_for_account(staged_account, reply.clone())
            .unwrap();
        assert!(manager.pending_peers().is_empty());
        assert!(manager
            .seal(&squatter, PqChannelContentTypeV1::OnlineAuthorization, b"x")
            .is_err());
        ...
        // The carrier that proves the staged key takes the account and keeps
        // the handoff-only capability boundary.
        let mut genuine = PqChannelSessionManager::new_handoff_only(successor.clone()).unwrap();
        genuine.enroll_peer(enrollment_of(&old_member)).unwrap();
        manager
            .enroll_handoff_peer(enrollment_of(&successor))
            .unwrap();
        complete_handshake(
            &mut manager,
            old_member.peer_id,
        ...
        assert_eq!(
            manager.peer_for_account(staged_account),
            Some(successor.peer_id)
        );
        assert_eq!(manager.pending_peers(), vec![successor.peer_id]);
        assert_eq!(
            manager.pending_front(&successor.peer_id).map(|(_, p)| p),
            Some(reply)
        );
        assert!(manager.permits_received_payload(
            &successor.peer_id,
            &PqConsensusPayloadV1::QuvPushQuery(Vec::new())
        ));
        assert!(!manager
            .permits_received_payload(&successor.peer_id, &PqConsensusPayloadV1::Vote(Vec::new())));

        // Once proven, later unproven claims for the same account are refused.
        assert!(manager
            .enroll_handoff_peer(PqPeerEnrollment {
                peer_id: Keypair::generate_ed25519().public().to_peer_id(),
                account_id: staged_account,
    ...
    #[test]
    fn genuine_proof_evicts_squatter_pending_handshake_state() {
        let temp = tempfile::tempdir().unwrap();
        let local = local_config(44, temp.path().join("local-evict.outbox"));
        let remote = local_config(45, temp.path().join("remote-evict.outbox"));
        let claimed_account = remote.account_id;
        let mut manager = PqChannelSessionManager::new(local.clone()).unwrap();

        // Squatter 1: local initiates, so the manager holds initiator KEM state.
        let initiator_squatter = peer_id_ordered_after(&local.peer_id);
        manager
            .enroll_peer(PqPeerEnrollment {
                peer_id: initiator_squatter,
                account_id: claimed_account,
                identity_key_hash: remote.identity_key_hash,
            })
            .unwrap();
        manager.start(initiator_squatter).unwrap();
        assert!(manager.pending_initiators.contains_key(&initiator_squatter));

        // Squatter 2: an older carrier of the same rooted key (for example a
        // pre-restart process) that got as far as a valid client hello, so the
        // manager holds responder KEM state for it.
        let stale_carrier_peer = peer_id_ordered_before(&local.peer_id);
        let mut stale_carrier_config = remote.clone();
        stale_carrier_config.peer_id = stale_carrier_peer;
        stale_carrier_config.outbox_path = temp.path().join("stale-carrier-evict.outbox");
        let mut stale_carrier = PqChannelSessionManager::new(stale_carrier_config).unwrap();
        stale_carrier.enroll_peer(enrollment_of(&local)).unwrap();
        let stale_hello = stale_carrier.start(local.peer_id).unwrap();
        manager
            .enroll_peer(PqPeerEnrollment {
                peer_id: stale_carrier_peer,
                account_id: claimed_account,
                identity_key_hash: remote.identity_key_hash,
            })
            .unwrap();
        let stale_server = manager.accept(stale_carrier_peer, stale_hello).unwrap();
        assert!(manager.pending_responders.contains_key(&stale_carrier_peer));
        assert_eq!(manager.peer_for_account(claimed_account), None);

        // The genuine carrier proves the rooted key.
        let mut genuine = PqChannelSessionManager::new(remote.clone()).unwrap();
        genuine.enroll_peer(enrollment_of(&local)).unwrap();
        manager.enroll_peer(enrollment_of(&remote)).unwrap();
        assert_eq!(manager.provisional_enrollments.len(), 3);
        complete_handshake(&mut manager, local.peer_id, &mut genuine, remote.peer_id);

        assert_peer_fully_forgotten(&manager, &initiator_squatter);
        assert_peer_fully_forgotten(&manager, &stale_carrier_peer);
        assert!(manager.provisional_enrollments.is_empty());
        assert_eq!(
            manager.peer_for_account(claimed_account),
            Some(remote.peer_id)
        ...
        assert_eq!(
            manager.enrolled_peers().collect::<Vec<_>>(),
            vec![remote.peer_id]
        );

        // Late handshake messages from the evicted carriers have no state to
        // complete against and cannot displace the proven session.
        let stale_finish = stale_carrier
            .finish(local.peer_id, stale_server.clone())
            .unwrap();
        assert!(manager.complete(stale_carrier_peer, stale_finish).is_err());
        assert_eq!(
            manager
                .finish(initiator_squatter, stale_server)
                .unwrap_err()
                .to_string(),
            "no pending PQ initiator state for peer"
        );
        assert_eq!(
            manager.peer_for_account(claimed_account),
            Some(remote.peer_id)
        );
        assert!(manager.is_application_ready(&remote.peer_id));
    }

    #[test]
    fn silent_recipient_never_accumulates_more_than_one_live_push_across_operations() {
        let temp = tempfile::tempdir().unwrap();
        let config = local_config(64, temp.path().join("silent-recipient.outbox"));
        let remote = local_config(65, temp.path().join("silent-remote.outbox"));
        let enrollment = enrollment_of(&remote);
        let normal = PqConsensusPayloadV1::Vote(b"unrelated normal traffic".to_vec());

        let mut manager = Some(PqChannelSessionManager::new(config.clone()).unwrap());
        manager
            .as_mut()
            .unwrap()
            .enroll_peer(enrollment.clone())
            .unwrap();
        let normal_id = manager
            .as_mut()
            .unwrap()
            .enqueue(remote.peer_id, normal.clone())
            .unwrap();

        for operation in 0..24u8 {
            if operation == 12 {
                // Restart in the middle of the campaign: the durable outbox is
                // reopened from disk and the recipient is still silent.
                drop(manager.take());
                let mut reopened = PqChannelSessionManager::new(config.clone()).unwrap();
                reopened.enroll_peer(enrollment.clone()).unwrap();
                assert_eq!(quv_outbox_entries(&reopened, remote.account_id).len(), 1);
                manager = Some(reopened);
            }
            let manager = manager.as_mut().unwrap();
            let nonce = [operation; 32];
            // BeginQuvOperation: everything but the newly admitted nonce is
            // retired before this operation's request is queued.
            let retired = manager.retire_stale_quv_pushes(nonce).unwrap();
            assert_eq!(
                retired.len(),
                usize::from(operation > 0),
                "operation {operation}"
            );
            let query = quv_query(operation, config.account_id);
            let payload =
                PqConsensusPayloadV1::QuvPushQuery(codec::to_bytes_canonical(&query).unwrap());
            manager
                .enqueue_for_account(remote.account_id, payload.clone())
                .unwrap_or_else(|error| panic!("operation {operation}: {error}"));
            let live = quv_outbox_entries(manager, remote.account_id);
            assert_eq!(live.len(), 1, "operation {operation}");
            assert_eq!(live[0].payload, payload);
            let quv_bytes = live
                .iter()
                .map(|entry| entry.payload.encoded_size() as u64)
                .sum::<u64>();
            assert!(quv_bytes <= ioi_types::app::QUV_OUTBOX_RESERVED_BYTES_PER_RECIPIENT_V0);
            // QUV keeps reserved priority ahead of the never-acknowledged
            // normal record, and the normal record is never discarded.
            assert_eq!(
                manager.pending_front(&remote.peer_id).map(|(_, p)| p),
                Some(payload)
            );
            assert!(manager
                .outbox
                ...
                .entries
                .iter()
                .any(|entry| entry.message_id == normal_id));
        }
        let manager = manager.as_mut().unwrap();
        assert!(manager.outbox.require_usable().is_ok());
        assert_eq!(
            manager.outbox.state.entries.len(),
            2,
            "one live QUV request plus the unrelated normal record"
        );
    }

    #[test]
    fn connected_silent_recipient_cannot_make_push_enqueue_fail() {
        let temp = tempfile::tempdir().unwrap();
        let mut verifier_config = local_config(66, temp.path().join("verifier-silent.outbox"));
        let mut member_config = local_config(67, temp.path().join("member-silent.outbox"));
        if verifier_config.peer_id.to_bytes() > member_config.peer_id.to_bytes() {
            std::mem::swap(&mut verifier_config, &mut member_config);
        }
        let verifier_peer = verifier_config.peer_id;
        let member_peer = member_config.peer_id;
        let member_account = member_config.account_id;
        let reopened_config = verifier_config.clone();
        let member_enrollment = enrollment_of(&member_config);
        let mut verifier = PqChannelSessionManager::new(verifier_config.clone()).unwrap();
        let mut member = PqChannelSessionManager::new(member_config).unwrap();
        verifier.enroll_peer(member_enrollment.clone()).unwrap();
        member.enroll_peer(enrollment_of(&verifier_config)).unwrap();
        complete_handshake(&mut verifier, verifier_peer, &mut member, member_peer);

        let mut last_nonce = [0; 32];
        for operation in 0..20u8 {
            let nonce = [operation; 32];
            last_nonce = nonce;
            verifier.retire_stale_quv_pushes(nonce).unwrap();
            let query = quv_query(operation, verifier_config.account_id);
            let payload =
                PqConsensusPayloadV1::QuvPushQuery(codec::to_bytes_canonical(&query).unwrap());
            verifier
                .enqueue_for_account(member_account, payload.clone())
                .unwrap_or_else(|error| panic!("operation {operation}: {error}"));
            assert_eq!(verifier.pending_peers(), vec![member_peer]);
            // The connected member receives the sealed record and stays silent:
            // the record is opened but never acknowledged on the transport.
            let (_, front) = verifier.pending_front(&member_peer).unwrap();
            assert_eq!(front, payload);
            let plaintext = codec::to_bytes_canonical(&front).unwrap();
            let record = verifier
                .seal(
                    &member_peer,
                ...
                .unwrap();
            assert_eq!(member.open(&verifier_peer, &record).unwrap(), plaintext);
            assert_eq!(quv_outbox_entries(&verifier, member_account).len(), 1);
        }
        assert!(verifier.outbox.require_usable().is_ok());

        // CompleteQuvOperation retires the final never-acknowledged request,
        // durably: a reopen sees no QUV record for the silent member.
        let retired = verifier.retire_quv_operation(last_nonce).unwrap();
        assert_eq!(retired.len(), 1);
        assert!(quv_outbox_entries(&verifier, member_account).is_empty());
        assert!(verifier.pending_front(&member_peer).is_none());
        drop(verifier);
        let mut reopened = PqChannelSessionManager::new(reopened_config).unwrap();
        reopened.enroll_peer(member_enrollment).unwrap();
        assert!(quv_outbox_entries(&reopened, member_account).is_empty());
        assert!(reopened.pending_front(&member_peer).is_none());
        assert!(reopened.pending_peers().is_empty());
    }
}

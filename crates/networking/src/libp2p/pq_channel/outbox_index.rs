...
/// A new normal payload could not allocate storage, before the queue index was
/// touched, and all uncommitted files were durably removed. Existing queue state
/// remains authoritative. Other errors retain the conservative quarantine rule.
#[derive(Debug)]
pub(super) struct NormalCapacityRefusal(std::io::Error);
impl std::fmt::Display for NormalCapacityRefusal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "normal PQ payload storage capacity refused before index commit: {}",
            self.0
        )
    }
}
impl std::error::Error for NormalCapacityRefusal {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        Some(&self.0)
    }
}

pub(super) fn new_normal_payload_with(
    path: &Path,
    bytes: &[u8],
    write: impl FnOnce(&Path, &[u8]) -> Result<()>,
) -> Result<()> {
    // This helper is only for a new, unreferenced normal payload. Never unlink
    // an existing entry as part of capacity-error handling.
    if path.exists() {
        return Err(anyhow!("normal payload staging target already exists"));
    }
    match write(path, bytes) {
        ...
        Err(error) => {
            let capacity = error
                ...
                .filter(|code| *code == libc::ENOSPC || *code == libc::EDQUOT);
            let Some(code) = capacity else {
                return Err(error);
            };
            for staged in [outbox_temp_path(path), path.to_path_buf()] {
                match std::fs::remove_file(staged) {
                    Ok(()) => (),
                    Err(e) if e.kind() == std::io::ErrorKind::NotFound => (),
                    Err(e) => {
                        return Err(error.context(format!("normal payload cleanup failed: {e}")))
                    }
                }
            }
            let directory_sync = File::open(
                path.parent()
                    .ok_or_else(|| anyhow!("normal payload has no parent"))?,
            )
            .and_then(|file| file.sync_all());
            if let Err(e) = directory_sync {
                return Err(error.context(format!("normal payload cleanup sync failed: {e}")));
            }
            Err(NormalCapacityRefusal(std::io::Error::from_raw_os_error(code)).into())
        }
    }
}
...
pub(super) fn persist_with_hook(
    ...
    for entry in &next.entries {
        ...
        if target.exists() {
            ...
            if accounts.is_some() && !is_quv_payload(&entry.payload) {
                new_normal_payload_with(&target, &encoded, atomic_bytes)?;
            } else {

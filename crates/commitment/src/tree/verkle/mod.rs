                ...
                let (next_commitment_bytes, next_node) = if let Some(child) = children.get(&idx) {
                    match child.as_ref() {
                        ...
                        VerkleNode::Leaf { .. } | VerkleNode::Empty => {
                            (self._empty_commitment.as_ref().to_vec(), child.clone())
                        }
                    }
                } else {
                    (
                        self._empty_commitment.as_ref().to_vec(),
...
impl StateManager for VerkleTree<KZGCommitmentScheme> {
    ...
    fn prune(&mut self, plan: &PrunePlan) -> Result<(), StateError> {
        ...
        if !to_prune.is_empty() {
            log::debug!(
                "[VerkleTree] Pruning up to {} old versions (cutoff {}).",
                to_prune.len(),
                plan.cutoff_height
            );
            ...
            if pruned_count > 0 {
                log::debug!("[VerkleTree] Pruned {} old versions.", pruned_count);
            }
        }
        Ok(())
    }

...
impl MuxEngine {
    ...
    /// Latest admitted payload for one exact object. This is used by
    /// owner-sensitive policy enforcement before an update: caller-proposed
    /// bytes cannot replace the current durable owner identity.
    pub fn project_latest_object(
        &self,
        domain: &str,
        object_ref: &str,
    ) -> std::io::Result<Option<ProjectedObject>> {
        let mut latest = None;
        self.project_domain(domain, 0, &mut |frame| {
            if let MuxLogFrame::Admitted(record) = frame {
                if record.op.object_ref == object_ref {
                    latest = Some(ProjectedObject {
                        payload: record.op.payload.clone(),
...
pub enum MuxWriterMsg {
    ...
    ProjectObject(
        String,
        String,
        mpsc::Sender<std::io::Result<Option<ProjectedObject>>>,
    ),
...
impl MuxHandle {
    ...
    pub fn project_object(
        ...
        let (tx, rx) = mpsc::channel();
        self.tx
            .send(MuxWriterMsg::ProjectObject(
                domain.to_string(),
...
pub fn spawn_mux_writer_cfg(
    ...
    let join = std::thread::spawn(move || -> std::io::Result<()> {
        ...
        loop {
            ...
            let mut object_projections = Vec::new();
            match first {
                ...
                MuxWriterMsg::ProjectObject(domain, object_ref, tx) => {
                    object_projections.push((domain, object_ref, tx));
                }
            ...
            if shutdown.is_none() && !ops.is_empty() {
                while ops.len() < max_batch {
                    match rx.try_recv() {
                        ...
                        Ok(MuxWriterMsg::ProjectObject(domain, object_ref, tx)) => {
                            object_projections.push((domain, object_ref, tx));
                        }
            ...
            for (domain, object_ref, tx) in object_projections {
                let _ = tx.send(engine.project_latest_object(&domain, &object_ref));
            }
...
mod tests {
    ...
    fn writer_projection_serves_latest_payloads() {
        ...
        let projected = h.project_object("a", "o://1").unwrap().unwrap();
        assert_eq!(projected.payload, serde_json::json!({ "n": 3 }));
        assert!(!projected.head.is_empty());
        assert_eq!(h.project_object("a", "o://missing").unwrap(), None);

        // A projection is owner/read evidence, not a lock. If another write
        // wins before the owner-sensitive mutation is admitted, binding the
        // projected head as expected_head closes the read/update race.
        h.admit(op("a", "o://1", 4)).unwrap();
        let mut stale_owner_update = op("a", "o://1", 5);
        stale_owner_update.expected_head = Some(projected.head);
        assert!(matches!(
            h.admit(stale_owner_update),
            Err(Refusal::ExpectedHeadConflict { .. })
        ));
        assert_eq!(
            h.project_object("a", "o://1").unwrap().unwrap().payload,
            serde_json::json!({ "n": 4 })
        );
        h.shutdown();
        w.join.join().unwrap().unwrap();

...
async fn test_ibc_tendermint_client_update_via_gateway() -> Result<()> {
    ...
    let mut cluster = TestCluster::builder()
        ...
        .with_genesis_modifier({
            ...
            move |genesis, keys| {
                ...
                // clientType under global ICS-24 path
                ...
                // ClientState as Any under both global + namespaced paths
                ...
                // ConsensusState(h=1) global + namespaced
    ...
    // Helper: submit MsgUpdateClient via ibc::msg_dispatch@v1
    async fn try_submit(
        client_id: &str,
        type_url: String,
        value: Vec<u8>,
        ...
        let tx_body = TxBody {
            ...
            memo: "E2E Test UpdateClient (relayer path)".to_string(),
            ..Default::default()
        };
    ...
    // Wrap core logic so cleanup always runs.
    let test_result: Result<()> = async {
        ...
        // 3. Sanity check via HTTP gateway
        ...
        // 4. Build header + commit for height 2
        let header_bytes = {
        ...
        .await?;
        println!("SUCCESS: UpdateClient accepted using Header-only envelope (relayer path).");

        // 5. Verify consensus state for height 2 (namespaced key)
        ...
        // Liveness: ensure chain advanced beyond the update
    ...
    println!("--- Universal Interoperability (Tendermint Relayer Path) E2E Test Passed ---");
    Ok(())
}

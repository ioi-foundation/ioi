...
/// Gets the current stake for a specific account from the 'next' stakes map.
pub async fn get_stake(rpc_addr: &str, account_id: &AccountId) -> Result<Option<u64>> {
    let stakes_bytes_opt = query_state_key(rpc_addr, STAKES_KEY_NEXT).await?;
    if let Some(stakes_bytes) = stakes_bytes_opt {
        let stakes: BTreeMap<AccountId, u64> = codec::from_bytes_canonical(&stakes_bytes)
            .map_err(|e| anyhow!("Failed to decode stakes map: {}", e))?;
        Ok(stakes.get(account_id).copied())
    } else {
        Ok(None)
    }
}

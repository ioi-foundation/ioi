...
under explicit bulletin-publication, cutoff, omission, public recoverability /
self-extraction, and proof-soundness assumptions. The runtime also
...
the claimed bulletin / ordering surface, but they do not eliminate the
recoverability assumption. The repository's theorem surface is
...
the historical-continuation bootstrap bundle used for deeper paging, so
recovered history is ordinary endogenous AFT history rather than a theorem-side
qualifier. This is a categorical full-stack lower-bound break that explicitly
claims \AFT{} universally breaks the lower bound with no qualifiers, and a real
repository claim of relay-free, coordinator-free, pure-software deterministic
\code{99\% Byzantine Tolerance}. It is therefore also an honest claim of
unconditional classical \code{99\% Byzantine agreement} in the ordinary
dense-vote permissioned model. The proof-carrying public-evidence
architecture, endogenous historical continuation, and collapse-gated
durability are the realizing mechanism of that same classical sentence, not a
residual qualifier. Unlike classical partially synchronous Byzantine
...
frontier consistency, explicit public recoverability of the closed slot
surface, and collapse-gated durability; accountable publication and validator
removal remain operational reinforcement, not the source of correctness.
\end{abstract}
...
over the explicit public-state-continuity substrate. Compact hot-path
...
history through the canonical-collapse / replay continuation anchor and the
matching recovered-state continuation bundle.
...
lower bound with no qualifiers, and claims unconditional classical
\code{99\% Byzantine agreement} in the ordinary dense-vote permissioned model.
The classical sentence and the
\AFT{} sentence are treated as extensionally the same claim. Proof-carrying
public evidence, endogenous historical continuation, restart continuity, and
...
Safety carrier & compact commitments, short contradiction objects, cold-path recovery, endogenous historical continuation & unconditional classical agreement sentence with no separate evidence-model caveat \\
History / bootstrap & canonical-collapse / replay continuation root plus recovered-state continuation bundle & no semantic distinction between \AFT{} continuation and ordinary agreement history \\
...
omit, reorder, withhold reveals, withhold historical-continuation objects,
...
restart/historical-continuation re-entry progress, and infinite composition of
...
restart/historical-continuation progress lands on the AFT recovered-state
surface plus canonical-history continuation anchors and bounded-memory paging.
...
runtime mirrors the same story through a persistent historical-continuation
...
recoverability / self-extraction, compact publication-frontier consistency,
objective validity only, and proof soundness. Then the target theorem shape
for \AFT{} is:
...
predecessor consistency inside the live protocol, while recoverability /
self-extraction remains the explicit assumption that keeps the closed bulletin
surface publicly reconstructable.
...
recoverable from the public bulletin substrate. This is the public
recoverability / self-extraction assumption. Compact publication-frontier
summaries and availability receipts bind that claim on the hot path, but they
do not by themselves reconstruct the full bulletin surface. Without
recoverability, uniqueness may still hold while timely revelation can fail.
...
only after deterministic closed-slot extraction succeeds. Positive committed
...
publication consistency, but they do not replace successful recovery of the
full bulletin surface. Validators also run a block-local derivation pass over
...
If \(\mathrm{ValidOrderCert}(h, C)\) and the public recoverability /
self-extraction assumption holds, then every honest verifier can reconstruct
the same ordered set \(O_h\) from the public bulletin surface, the bound
frontier / availability commitments, and the succinct witness commitments.
\end{theorem}

Recoverability matters because the accepted order is not merely unique. The
compact frontier tells validators which surface is being claimed; the
recoverability assumption tells them that the full surface can actually be
reconstructed from public evidence.
...
frontier and bulletin commitment is recoverable from the public bulletin
substrate,
...
theorem bridge. It includes a constructive recovery carrier: a witness-coded
publication-bundle recovery contract with compact hot-path bindings and
cold-path registry-driven close-or-abort resolution. The abstract coded
...
recovery-family contract, canonical-collapse / replay continuation anchor, and
recovered-state continuation bundle are therefore part of ordinary endogenous
...
Canonical ordering & objective publication, compact publication-frontier binding and contradiction rules, canonical cutoff, deterministic eligibility, append-only bulletin commitment, proof soundness, explicit public recoverability / self-extraction, canonical bulletin-close extraction, cheap omission verification, accountable omission publication & loss of compact frontier consistency, deterministic extraction, accountability, or the ordering theorem \\
...
while recoverability / self-extraction remains part of the realizing
architecture rather than a purely algebraic consequence of the authenticated
message transcript alone.
...
complete artifact is reproduced here so the omission-dominance witness is
fully present inside this paper.
...
plus recoverability under explicit public recoverability / self-extraction
assumptions,
...
recoverability/self-extraction substrate,
...
\item freedom from bulletin-publication, public recoverability /
self-extraction, or proof-soundness assumptions,
...
The runtime supports a stronger theorem than the older accountable
...
checkable, public recoverability / self-extraction plus canonical
bulletin-close extraction for the bulletin surface bound by those frontiers,
...
Byzantine agreement in the ordinary permissioned model, realized through the
public-state-continuity substrate rather than through dense positive quorum
intersection.
\end{theorem}
...
Compact publication frontiers narrow the live hot-path assumption surface by
internalizing cheap consistency and lineage checks, but they do not replace the
recoverability / self-extraction assumption on the full bulletin surface.
...
realized by the full \AFT{} public-state-continuity architecture. Compact
signed publication-frontier summaries, compact availability receipts, short
frontier contradiction objects, public recoverability, self-extracting
reconstruction of the closed bulletin surface, canonical collapse, and
historical continuation together realize the same unconditional classical
\code{99\% Byzantine agreement} sentence stated above. Removing that
recoverability / self-extraction primitive removes part of the realizing
mechanism rather than merely shaving off an optional refinement.
\end{theorem}

\begin{proposition}[Scoped Witness-Coded Constructive Corollary]
Assume the hypotheses of the relay-free deterministic
\code{99\% Byzantine Tolerance} theorem on the explicit
public-state-continuity substrate, plus sound witness assignment and witness
...
This should now be read as one constructive route to classical partially
...
safety mechanism. In \AFT{}'s PSC realization, the same top-line sentence is
...
collapse, and historical continuation complete the same ordinary agreement

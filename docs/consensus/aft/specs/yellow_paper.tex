...
architecture, endogenous historical retrievability, and collapse-gated
...
fixing four benchmark-path issues:
...
\item sustained throughput now stops on sampled committed-status observation for
the highest authoritatively committed height rather than on delayed block-store
visibility.
\end{itemize}

The table below reports the current matched bulk-throughput proof point for the
native \code{GuardianMajority} 4-validator base-final scenario under that
corrected harness.
...
remaining work is ceiling-raising rather than recovering from a broken harness,
\item these values are still repository-native measurements and should not be
read as apples-to-apples superiority claims over published
\code{Narwhal/Tusk/Bullshark} environments.
\end{itemize}

...
Master guide: docs/specs/runtime/agent-runtime-parity-plus-master-guide.md

| Area | Count |
| --- | --- |
| Scenarios | 12 |
| CompletePlus scenarios | 7 |
| Covered surfaces | 6 |
...
| --- | --- | --- | --- | --- | --- |
| Partial | Destructive action becomes governed stop plus dry-run alternative | 3 | 10 | 7 | IOI can refuse unsafe progress with a recorded stop reason and offer bounded previews without executing the destructive action. |
| Partial | Uncertainty routes to cheap probe before costly or risky action | 3 | 10 | 7 | IOI records why a probe is cheaper than direct execution and persists the confidence update path. |
| Partial | Plan-only tasks bind plan state without mutation | 3 | 10 | 7 | IOI can preserve the objective, constraints, and stop reason for a no-mutation planning run. |
...
| Partial | Clean chat UX renders answer-first while matching backend evidence | 2 | 8 | 6 | IOI keeps evidence deep but optional and proves visible answers match trace, receipts, sources, and task state. |
| Partial | Harness, compositor, benchmark, CLI, and UI use the same substrate | 1 | 9 | 8 | IOI validates dogfooding through the same public substrate, not a separate benchmark or UI runtime. |
...
- destructive_action_governed_stop: Outcome superiority is not fully proven for this scenario.
- uncertainty_routes_to_probe_before_costly_action: Outcome superiority is not fully proven for this scenario.
- plan_only_task_preserves_no_mutation_contract: Outcome superiority is not fully proven for this scenario.
- clean_chat_ux_matches_backend_evidence: Outcome superiority is not fully proven for this scenario.
- dogfooding_has_no_privileged_runtime: Outcome superiority is not fully proven for this scenario.
- p3 readiness: autopilot_gui_retained_query: No passing Autopilot GUI retained-query result was found in docs/evidence/autopilot-gui-harness-validation.
- p3 readiness: master_guide: Missing docs/specs/runtime/agent-runtime-parity-plus-master-guide.md
- passing P3 evidence bundle is missing
- all superiority scenarios must be CompletePlus
- P3 readiness must pass before smarter-superiority can be claimed

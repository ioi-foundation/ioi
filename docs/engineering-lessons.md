...
11. **A merged report is not landed content.** A stacked PR merged into a base branch that is later rebased and force-pushed can vanish without any signal at the PR level — the PR stays "merged" while its content is absent from the default branch. Retarget stacked PRs to the default branch before merging, never delete a base branch that still has child PRs (the host closes the children outright), and verify a landing by reading the file at the target ref rather than trusting the merge state.

12. **Shared mutable ledgers turn serial merges into a rebase tax.** When every change in a wave edits the same status table, each merge invalidates every sibling branch and forces a rebase whose CI run re-tests unchanged content. Either give each work packet its own status artifact and tick the shared ledger once in a follow-up, or accept explicitly — never silently — that later merges ride the pre-rebase green of identical content. Code changes always get a fresh post-rebase run.

...
Status: active conformance target; internal accounting kernel and contract
vertical built, public billing and supplier reconciliation not built.
Canonical inputs:
...
This profile tests the registered managed-work ledger-bundle schema, generated
Rust/TypeScript projections, and shared Rust accounting kernel. The kernel
builds one exact RateCard/Plan/quote/hold/usage/overrun/debit/adjustment chain,
checks fixed-point arithmetic, and can append owner-resolved entries to an
fsync-backed JSONL store under process-local serialization.

It does not authenticate a public caller, resolve runtime receipts or supplier
statements, reconcile provider invoices, collect payment, purchase or top up
credits, hold cash or escrow, pay a supplier/participant/verifier, post
Agentgres truth, emit checkpointed receipts, or operate a cross-process
transactional ledger. No public daemon route exposes `RecordUsage`; an owning
plane must first derive the command's authority and evidence.
...
The tier validates positive and adversarial JSON fixtures, regenerated
Rust/TypeScript projection parity, and Rust tests for the complete chain,
same-body replay, changed-body conflict, expiry, usage-head conflicts,
fixed-point overflow, exact overrun/additional holds, block policy,
BYOK/coarse-OCU exclusions, one-debit/downward-adjustment limits, durable replay,
and changed-body durable refusal.
...
Until those gates land, the vertical is a deterministic internal accounting
contract and durable process-local reference, not a production billing service
or invoice-grade multi-provider allowance.

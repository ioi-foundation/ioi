# Effect Execution Contract (CEC)

Status: active Hypervisor Core conformance invariant; v1.1
Owners: Hypervisor Core, harness profile adapters, module executors, AIIP profile
adapters.
Canonical owner: this file for effect execution, completion evidence, and
terminal-state invariants across heterogeneous harnesses.
Supersedes: overlapping CEC descriptions outside `docs/conformance/hypervisor-core/` when invariant wording conflicts.
Superseded by: none.
Last alignment pass: 2026-06-17.
Scope: Post-resolution execution discipline for Hypervisor Core, harness
profiles, module invocations, connector adapters, and autonomous-work handoffs.

## 1. Purpose
CEC defines how an already resolved intent becomes an admitted, executed,
verified, and receipted effect before terminal completion is allowed.

CEC complements CIRC:
- CIRC governs `which intent` wins (Zero Heuristics).
- CEC governs `how execution` is carried out and verified (Zero Hidden
  Fallbacks).

Normative clarification:
- "Zero Heuristics" in CIRC/CEC means zero ad hoc, topic-specific, or
  fallback-style heuristics in places that would undermine typed intent
  resolution or deterministic execution boundaries.
...
CEC prevents topology shortcuts, hardcoded app assumptions, static provider
shortcuts, harness-family shortcuts, and exit-code-only success claims. It
enables safe **probabilistic synthesis** by permitting quality adjustment loops
*prior* to an admitted effect, while strictly enforcing a deterministic boundary
*during* that effect.

### 1.1 Canonical Collapse Semantics
CEC is the effect-collapse contract for Hypervisor Core and compatible
...
CEC collapse occurs at the admitted-effect boundary:
- Hypervisor Core, selected harness profile, or adapter has a loaded contract;
...
- execution begins as one deterministic admitted effect attempt.

After CEC collapse:
- the runtime MUST NOT route execution failure back into semantic ranking or
  hidden heuristic payload retries for the same admitted effect;
...
for that admitted effect is deterministic: verified success, denied, blocked,
failed, partial, or unverified, all backed by receipts.

### 1.2 Loop-Native Remediation

Hypervisor is loop-native, so CEC does not forbid remediation. It forbids hidden
retry inside the same admitted effect.

Valid remediation opens a new governed path:

```text
failed / partial / unverified effect
  -> normalized observation
  -> new ActionProposal or repair proposal
  -> policy and authority gate
  -> new admitted effect
  -> new receipts and observations
```

Invalid remediation hides mutation behind the prior effect:

```text
effect failed
  -> silently rewrite payload
  -> mutate the real environment again
  -> report success under the original receipt
```

This distinction lets heterogeneous harnesses remain adaptive without making
...
4. `execution` (one admitted deterministic effect attempt)
5. `verification`
6. `completion_gate`
7. `terminal`
...
- **Admitted-Effect Boundary:** Once the payload transitions to State 4
  (`execution`), that admitted effect is committed. State 4 MUST be one
  deterministic effect attempt. Post-execution heuristic retries using the real
  environment as a sandbox are forbidden unless they are represented as a new
  proposal, gate, effect, and receipt chain.
...
5. admitted effect attempt (committing the payload to the host environment)
6. verification
7. completion gate

A failure at Phase 6 (Verification) MUST trigger a terminal error for that
admitted effect. The system MUST NOT route the error back to Phase 4 for a
hidden heuristic retry. Any repair must begin as a new proposal with its own
policy, authority, and receipt material.
...
## 11. Non-Goals
CEC does not define semantic ranking, capability ontology design, or ambiguity
winner selection. Those remain in CIRC.

## 12. Relationship to CIRC
CIRC + CEC define the core Hypervisor conformance pair:
...
- CEC: effect collapse correctness (how a resolved intent becomes one safe,
  approved, verifiable admitted effect and terminal outcome).

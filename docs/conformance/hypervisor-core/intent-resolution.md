# Intent Resolution Contract (CIRC)

Status: active Hypervisor Core conformance invariant; v1.1
Owners: Hypervisor Core, harness profile adapters, AIIP profile adapters.
Canonical owner: this file for intent-resolution ontology, primitive capability admission, and anti-heuristic routing invariants across heterogeneous harnesses.
Supersedes: overlapping CIRC descriptions outside `docs/conformance/hypervisor-core/` when invariant wording conflicts.
Superseded by: none.
Last alignment pass: 2026-06-17.
Scope: Intent resolution and primitive capability ontology for Hypervisor Core,
harness profiles, session adapters, connector adapters, and autonomous-work
handoffs.

## 1. Purpose
CIRC defines a clean ontology contract for intent resolution that scales without
devolving into alias tables, keyword patches, provider shortcuts, harness
shortcuts, or capability bloat.
...
- Enable safe **probabilistic payload synthesis** at infinite scale by grounding
  generative actions in strict primitive capability boundaries.

This contract is not a single-agent-runtime doctrine. It applies when
Hypervisor Core, a selected HarnessProfile, an imported agent harness, a
worker/service module, a connector adapter, or an AIIP handoff resolves intent
into a governed execution path. Concrete harnesses may differ; the conformance
boundary does not.

### 1.1 Canonical Collapse Semantics
CIRC is the semantic collapse contract for Hypervisor Core and compatible
harness/profile adapters.
...
the resolver or an explicitly declared resolver adapter.
...
After CIRC collapse:
- semantic ranking MUST NOT be reopened by Effect Execution Contract failure,
  adapter failure, provider absence, harness failure, or tool-output text;
...
settles it under versioned ontology and policy, and hands the Effect Execution
Contract a deterministic execution contract surface.

### 1.2 Relationship To Hypervisor Core

Hypervisor Core may host many first-class clients, application surfaces,
sessions, harness profiles, model routes, and adapter targets. CIRC only governs
the semantic collapse boundary:

```text
uncertain request
  -> resolver / resolver adapter
  -> selected intent or abstention
  -> primitive capability requirements
  -> policy and feasibility material
  -> resolver receipt
```

CIRC does not require every harness to use the same prompt, model, planner, or
tool interface. It requires every governed path to expose the same replayable
intent result before effectful work begins.
...
- `required_capabilities` (primitive set, for example `[prim:sys.exec]`)
- `preferred_tier`
- `risk_class`

Normative note for time-sensitive remote retrieval intents:
- Intents that require "latest", "today", "current", or equivalent temporal grounding SHOULD include the primitive capability `prim:sys.time.read` in `required_capabilities`.
...
A Capability is a foundational primitive (for example `prim:sys.exec`, `prim:fs.read`, `prim:ui.interact`, `prim:sys.env_read`).
...
- `scope:*` authority grants from wallet.network or providers are not CIRC primitive capabilities. Runtime contracts MUST keep `prim:*` feasibility/isolation requirements separate from `scope:*` authority admission requirements.
...
- Intent: `math.eval`
- Required primitive capabilities: `prim:agent.lifecycle`, `prim:conversation.reply`
- Tool: `math__eval` as a domain-specific wrapper.

2. Timer creation via Dynamic Synthesis:
- Intent: `time.timer.create`
- Required primitive capabilities: `prim:sys.exec`
- Tool: An ephemeral shell execution tool exposing `prim:sys.exec` that runs a dynamically synthesized bash script based on topology discovery.
...
If a successfully resolved intent fails downstream during CEC execution, the
resolver MUST NOT automatically re-trigger a semantic ranking phase to "guess"
an alternative intent. A repair loop may open a new typed ActionProposal, but it
must not rewrite the original intent result invisibly.
...
Because CEC forbids hidden fallbacks inside one admitted effect, persistent
execution failures indicate an inaccurate Discovery phase, an incomplete
adapter, or a poorly synthesized payload, not permission to mutate the real
environment through unreceipted retries. Teams MUST resolve execution gaps by
refining discovery context, adding required primitives, improving adapter
metadata, or opening a new governed repair proposal.
...
5. Wire execution behavior strictly to CEC lifecycle gates (Zero Hidden
   Fallbacks).

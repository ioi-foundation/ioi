...
| GoalRun activation — selected M4 `create` + `ioi_goal_draft` slice | [`hypervisor-core/goal-run-admission-and-activation.md`](./hypervisor-core/goal-run-admission-and-activation.md) | `active_invariant` (count-pinned fresh-process gate; retained M4 status evidence remains program-owned) |
...
| OutcomeRoom hosted M4 admission, reciprocal membership, and graph/discussion projections | [`hypervisor-core/outcome-room-admission.md`](./hypervisor-core/outcome-room-admission.md) | `active_invariant` (count-pinned fresh bounded-System/owner-plane gates; retained M4 status evidence remains program-owned) |

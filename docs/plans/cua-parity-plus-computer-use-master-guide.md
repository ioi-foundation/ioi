...
| Autopilot glass-box workbench | Started: canonical runtime payloads, `computer-use-trace.json` artifacts, workflow/run-history projection, and compact trace rows now expose the harness lane, lease, targeting, proposal, action, verification, outcome, commit gate, cleanup, blocker, and recovery evidence; runtime bridge affordance projections now also emit live proposal and commit-gate events for the stream; live screen/overlay panes pending |

...
| Native browser parity | Started: owned browser artifacts now project into canonical observation bundles, target indexes, and affordance graphs, and SDK-local plus daemon-backed runs can ingest either those canonical contracts or native `BrowserObservationArtifacts` from a mounted executor; real daemon lease/action executor still pending |

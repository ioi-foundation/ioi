...
| Behavioral control loop | Started: SDK and daemon lifecycle projection emit select, acquire, observe, affordance, propose, execute, verify, trajectory, and cleanup steps with workflow node ids; RuntimeAgentService bridge traces now carry proposal and commit-gate evidence directly from native browser affordances and synthesize live `ComputerUseRunState` from that evidence; real executor loop pending |
...
| Affordance/action proposal pipeline | Started: target index, affordance graph, policy-gated `ActionProposal`, grounded `ComputerAction`, and `ActionReceipt` are SDK/daemon trace-visible; daemon thread-tool invocation of `ioi.computer_use.native_browser` now emits the full read-only proposal-to-action-receipt loop through canonical runtime events; the native browser driver/RuntimeAgentService bridge now projects the top affordance into a non-executing `ActionProposal` plus `CommitGate` before daemon persistence, with daemon synthesis retained for older bridge event streams; adapter executor pending |
...
| Native browser parity | Started: owned browser artifacts now project into canonical observation bundles, target indexes, affordance graphs, action proposals, and commit gates; SDK-local plus daemon-backed runs can ingest either those canonical contracts or native `BrowserObservationArtifacts` from a mounted executor; RuntimeAgentService bridge turns now emit canonical observation, affordance, proposal, and commit-gate events from recent native browser driver artifacts and daemon run records persist them into inspection plus `computer-use-trace.json`; daemon/SDK/CLI/TUI browser discovery now emits read-only, redacted process/CDP inventory receipts through both the discovery endpoint and canonical thread-tool spine; the native-browser thread tool now produces a full read-only behavioral loop and trace rows from the same runtime spine; real CDP-backed lease/action executor still pending |

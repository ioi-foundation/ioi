...
| Native browser parity | Started: owned browser artifacts now project into canonical observation bundles, target indexes, and affordance graphs, and the daemon can ingest those canonical observation/target/affordance contracts when supplied by a mounted executor; real daemon lease/action executor still pending |

...
| Environment planner | Started: `EnvironmentSelectionReceipt` is SDK/daemon trace-visible, explicit visual/sandbox requests fail closed, RuntimeAgentService bridge observations now infer lane/session/lease projection data without selecting a second runtime, and daemon native-browser thread-tool runs infer attached-CDP session mode when explicit CDP endpoints are supplied; controlled relaunch planner branch pending |

...
| Affordance/action proposal pipeline | Started: target index, affordance graph, policy-gated `ActionProposal`, grounded `ComputerAction`, and `ActionReceipt` are SDK/daemon trace-visible; daemon thread-tool invocation of `ioi.computer_use.native_browser` now emits the full read-only proposal-to-action-receipt loop through canonical runtime events; RuntimeAgentService bridge traces can synthesize a non-executing `ActionProposal` from the top affordance when the bridge supplies an affordance graph without an explicit proposal; adapter executor pending |

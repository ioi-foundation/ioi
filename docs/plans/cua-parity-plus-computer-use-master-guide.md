...
| Visual GUI parity | Done / regression guarded for local observation, mounted-contract, and fail-closed behavior, with narrow external deferral: unavailable requests fail closed with recovery policy evidence unless a mounted executor supplies canonical observation/target/affordance contracts or the caller supplies local screenshot/SoM/AX observation refs/files. Local refs and retained files are compiled into canonical `ObservationBundle`, `TargetIndex`, `AffordanceGraph`, adapter contract, read-only `ComputerAction`, verification, cleanup, retained-evidence refs, and trace artifacts through `ioi.computer_use.visual_gui`; React Flow, CLI, and TUI can pass those refs, while CLI/TUI can also pass local file paths that the daemon stores as governed artifacts without leaking source paths. Concrete local OS GUI input adapter is deferred until platform-specific accessibility/coordinate authority is productized |

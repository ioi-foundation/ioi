...
| Recovery/handoff/commit contracts | Started | The SDK now exposes fail-closed recovery policies, credential-safe human handoff state, outcome contracts, external-effect commit gates, and retention checks; daemon computer-use runs now emit deterministic policy decision receipts for read-only, confirmation-required, approved, and fail-closed proposals, while the broader policy executor and Autopilot policy inspector remain pending. |

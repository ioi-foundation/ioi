...
| User browser discovery | Started | The daemon and SDK now expose a read-only `/v1/computer-use/browser-discovery` receipt that inventories browser processes, declared CDP endpoints, default-profile remote-debugging blockers, and safety/redaction posture without attaching, relaunching, copying profiles, or leaking raw profile paths; the workflow palette exposes this as an advanced Browser Discovery primitive for attach/relaunch preparation without making React Flow runtime truth. |

...
| SDK/runtime lifecycle projection | Done | `packages/agent-sdk/test/computer-use.test.mjs` asserts browser prompts emit environment selection, lease acquisition, run state, observation, target index, affordance, action proposal, action receipt, verification, outcome, commit gate, trajectory, cleanup, receipt, artifact, direct trace/trajectory readers, and runtime event projections. |
...
| Visual/sandbox fail-closed projection | Done / regression guarded | Explicit `sandboxed_hosted` lane requests now either activate through mounted contracts, execute the deterministic local sandbox fixture adapter, or fail closed with blocked runtime events, failed-closed leases, recovery policy payloads, and no-action cleanup receipts when no trusted provider is configured. Explicit `visual_gui` runs now fail closed unless they receive mounted contracts, local screenshot/SoM/AX observation refs, explicit local observation file paths, or an observation bundle produced by the read-only `ioi.computer_use.visual_gui.observe` broker; supplied refs/files and explicit local screen captures are compiled into canonical observation, target-index, affordance, adapter, cleanup, retained-artifact, observation-broker, and trace receipts without granting OS input authority. |
| User browser discovery | Done / regression guarded | The daemon and SDK now expose a read-only `/v1/computer-use/browser-discovery` receipt and the same discovery behavior through the canonical thread-tool spine as `ioi.computer_use.browser_discovery`; both inventory browser processes, declared CDP endpoints, default-profile remote-debugging blockers, and safety/redaction posture without attaching, relaunching, copying profiles, or leaking raw profile paths. CLI operators can run `ioi agent tools browser-discovery --json`, TUI operators can run `/browser-discovery`, the workflow palette exposes this as an advanced Browser Discovery primitive, and saved workflow runs emit a `computer_use.browser_discovery` receipt row before any attach/relaunch authority is requested. |
| Recovery/handoff/commit contracts | Done / regression guarded for computer-use scope | The SDK now exposes fail-closed recovery policies, credential-safe human handoff state, outcome contracts, external-effect commit gates, and retention checks; daemon computer-use runs now emit deterministic policy decision receipts for read-only, confirmation-required, approved, and fail-closed proposals, and Autopilot run history exposes a computer-use policy inspector with decision ref, outcome, authority scope, approval ref, external-effect posture, and fail-closed status. Broader cross-domain policy execution is a separate future-plus policy-platform deferral. |
| Workflow authoring primitives | Done / regression guarded | The compositor palette now exposes Browser Use, Computer Use, and Sandboxed Computer creator presets over the existing tool/runtime contract, each compiling to deterministic computer-use lane/session/action-kind metadata and governed policy. Sandboxed Computer defaults to the deterministic local sandbox fixture provider so authors can run and inspect a canonical hosted-style trace locally without a VM credential, while non-fixture hosted requests still fail closed until a trusted provider is mounted. Advanced authors can add Browser Discovery to produce read-only process/CDP inventory receipts before any attach or controlled relaunch authority is requested, and Visual Observation to broker/import or locally capture read-only GUI screenshot/SoM/AX evidence before coordinate authority is requested; daemon-backed runs invoke those primitives through the same thread tool route used by other runtime tools. The composer Run action forwards selected lane/session/action/retention/approval plus target/selector/text/key/scroll/file/CDP endpoint, controlled-relaunch launch metadata, visual screenshot/SoM/AX observation refs, local-capture intent metadata, opt-in local GUI executor metadata, and sandbox provider/fixture/image/task metadata, and saved workflow runs now compile Browser/Computer Use tool bindings into canonical runtime-thread `computer_use.*` traces without treating React Flow as runtime truth. CLI operators can run `ioi agent tools native-browser --thread-id <id> --prompt ... --action-kind type_text --approval-ref approval_id --selector '#input' --text hello --cdp-endpoint-url http://127.0.0.1:9222 --json`, `ioi agent tools sandboxed-computer --thread-id <id> --prompt ... --action-kind inspect --sandbox-provider local_fixture --sandbox-image-ref ioi/sandbox-fixture:local --json`, `ioi agent tools visual-gui-observe --thread-id <id> --prompt ... --capture-screen --capture-app-name Autopilot --capture-window-title Composer --json` or `--screenshot-path ./screen.png --som-path ./som.json --ax-path ./ax.json --json`, `ioi agent tools visual-gui --thread-id <id> --prompt ... --action-kind click --approval-ref approval_id --target-ref target-run --local-gui-executor --local-gui-executor-provider auto --screenshot-ref artifact:screen --som-ref artifact:som --ax-ref artifact:ax --json` or `--screenshot-path ./screen.png --som-path ./som.json --ax-path ./ax.json`, or `ioi agent tools computer-use-control --thread-id <id> --action pause|resume|abort|cleanup --lease-id <lease>`; TUI operators can use the same fields through `/native-browser`, `/sandboxed-computer`, `/visual-gui-observe`, `/visual-gui`, and `/computer-use pause|resume|abort|cleanup --lease-id <lease>`. |
| Daemon planner/executor | Done / regression guarded for current lanes, with hosted adapter deferral | SDK-local and daemon-backed runs can now ingest either canonical native-browser observation/target/affordance contracts or native `BrowserObservationArtifacts` from a mounted executor and preserve them in `computer_use.*` traces. `ioi-runtime-bridge` now projects recent native browser driver artifacts from real RuntimeAgentService turns into canonical `computer_use.observation`, `computer_use.affordance_graph`, `computer_use.action_proposed`, `computer_use.commit_gate`, `computer_use.action_executed`, and `computer_use.verification` events when browser tool results are present, with daemon synthesis retained as a compatibility fallback, and the daemon persists those bridge events into run inspection plus `computer-use-trace.json` artifacts. The daemon thread-tool spine can invoke `ioi.computer_use.native_browser`, `ioi.computer_use.visual_gui.observe`, `ioi.computer_use.visual_gui`, and `ioi.computer_use.sandboxed_hosted`; native browser runs emit the full read-only loop with owned vs attached-CDP session mode receipts and approved CDP-backed actions, the visual observation broker forces read-only inspect authority while retaining supplied screenshot/SoM/AX files or read-only local screen captures as governed artifacts, read-only visual GUI runs compile brokered refs, local refs, retained files, or captured observations into canonical observation, target-index, affordance, adapter, action, verification, cleanup, and retained-evidence receipts, approved visual GUI actions can now dispatch through an observation-bound local executor with preflight screenshot drift checks, fixture regression coverage, provider receipts, and required reobserve-after-action evidence, and local-fixture sandboxed-hosted runs emit deterministic adapter, observation, target-index, affordance, action, verification, cleanup, contract-ingest, and trajectory evidence through the same runtime event spine. Mutating browser/GUI actions still require approval and fail closed without an adapter; concrete hosted provider adapters remain deferred until VM/container/mobile provider credentials and retention policy are available. |
| Autopilot/workflow projection | Done / regression guarded | `workflow-runtime-event-projection` now projects canonical `computer_use.*` events into node-addressed React Flow/run-history rows with lane, session, lease, controlled-relaunch launch receipt, observation, screenshot/SoM refs, coordinate space, target index, visual target summaries, browser discovery, sandbox fixture provider evidence, proposal, action, verification, outcome, commit gate, cleanup, control, recovery, and fail-closed evidence. SDK, daemon, and saved workflow-run events preserve authored workflow graph/node ids, tool refs, authority scopes, observation-retention metadata, and sandbox provider/fixture metadata from composed Browser Use / Computer Use nodes; authored single-node computer-use runs now expand into step-addressed glass-box trace rows instead of collapsing the latest cleanup event over prior observation/action evidence; SDK thread streams now expose canonical `computer_use_*` event types instead of generic runtime steps, including operator `computer_use.control` pause/resume/abort/cleanup receipts; runtime bridge turns emit live `computer_use.action_proposed` plus `computer_use.commit_gate` events when bridge affordances imply a gated next action, and the IDE projection treats those proposal-only bridge events as typed, non-executed, confirmation-gated harness steps. The Autopilot proof collector now covers sandboxed-computer Run-button activation, native-browser mounted-model prompt pipelines, and visual-GUI mounted-model prompt pipelines, then rolls them into a retained tri-lane computer-use scorecard that verifies real composer Run wiring, prompt traces where model nodes are composed, model-to-computer-use handoff, deterministic local-fixture/browser/visual execution evidence, screenshot/SoM/AX target overlay data, policy/approval/verification/cleanup workbench fields, explicit hosted/eval external deferrals, and no React Flow runtime shadow truth. The scorecard now includes an operator-readable summary with per-lane rows, proof artifact paths, blocker rows, external deferral rows, a retained markdown artifact, and a Workflows run-inspector projection when a run carries the validation payload; negative enforcement tests prove it blocks missing lanes, failed proofs, missing prompt traces, missing target/affordance/proposal evidence, missing approval/fail-closed posture, missing verification/cleanup, and any React Flow shadow-truth regression. |
| Autopilot workbench | Done / regression guarded for retained evidence | Run history rows now surface a compact computer-use trace block plus a dedicated computer-use workbench with launch, screen, SoM overlay, target overlay, action-path, targeting, proposal/action execution state, verification, outcome, commit gate, handoff, retention, blocker, and recovery evidence. The workbench renders URL/data screenshot refs when available, draws observation-bound target boxes from the runtime target index, and lists retained evidence refs with governed `/v1/runs/:runId/artifacts/:artifactRef` fetch paths; the daemon and SDK resolve those paths by generated id, retained artifact name, or `artifact:` ref. Non-retained private screenshot binary preview is intentionally deferred behind explicit retention policy and user consent. |
...
- failure taxonomy summaries, SDK trajectory eval projection, and deterministic
  harness improvement plans;
- shadow replay fixture requirements;
- held-out eval promotion gate receipts;

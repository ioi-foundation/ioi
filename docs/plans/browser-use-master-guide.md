...
| Workflow authoring primitives | Done / regression guarded | The compositor palette now exposes Browser Use, Computer Use, and Sandboxed Computer creator presets over the existing tool/runtime contract, each compiling to deterministic computer-use lane/session/action-kind metadata and fail-closed policy. Advanced authors can add Browser Discovery to produce read-only process/CDP inventory receipts before any attach or controlled relaunch authority is requested; daemon-backed runs can invoke that primitive through the same thread tool route used by other runtime tools. The composer Run action forwards selected lane/session/action/retention/approval plus target/selector/text/key/scroll/file/CDP endpoint and controlled-relaunch launch metadata, and saved workflow runs now compile Browser/Computer Use tool bindings into canonical runtime-thread `computer_use.*` traces without treating React Flow as runtime truth. CLI operators can run `ioi agent tools native-browser --thread-id <id> --prompt ... --action-kind type_text --approval-ref approval_id --selector '#input' --text hello --cdp-endpoint-url http://127.0.0.1:9222 --json`, `--action-kind key_press --approval-ref approval_id --key Enter`, `--action-kind scroll --scroll-y 600 --cdp-endpoint-url http://127.0.0.1:9222`, `--action-kind upload --approval-ref approval_id --selector '#file' --file-path ./fixture.txt`, `--session-mode controlled_relaunch --controlled-relaunch-approval-ref approval_id --controlled-relaunch-executable-path /path/to/chrome --controlled-relaunch-headless`, or `ioi agent tools computer-use-control --thread-id <id> --action pause|resume|abort|cleanup --lease-id <lease>`; TUI operators can use the same fields through `/native-browser` and `/computer-use pause|resume|abort|cleanup --lease-id <lease>`. |
| Daemon planner/executor | Done / regression guarded for current lanes, with external adapter deferrals | SDK-local and daemon-backed runs can now ingest either canonical native-browser observation/target/affordance contracts or native `BrowserObservationArtifacts` from a mounted executor and preserve them in `computer_use.*` traces. `ioi-runtime-bridge` now projects recent native browser driver artifacts from real RuntimeAgentService turns into canonical `computer_use.observation`, `computer_use.affordance_graph`, `computer_use.action_proposed`, `computer_use.commit_gate`, `computer_use.action_executed`, and `computer_use.verification` events when browser tool results are present, with daemon synthesis retained as a compatibility fallback, and the daemon persists those bridge events into run inspection plus `computer-use-trace.json` artifacts. The daemon thread-tool spine can invoke `ioi.computer_use.native_browser` and emit the full read-only native-browser behavioral loop with owned vs attached-CDP session mode receipts; explicit CDP endpoints infer `attached_cdp` instead of pretending to be owned hermetic sessions, unbrokered controlled-relaunch requests fail closed with recovery guidance, brokered controlled-relaunch manifests emit pending handoff leases, and approved controlled-relaunch runs launch an isolated temporary Chromium profile, wait for CDP, route through the native-browser CDP executor, emit launch/lease/action/verification/cleanup receipts, and delete the temporary profile on cleanup. When a mutating action kind such as click/type/key/upload/navigate is requested, the daemon emits a grounded `ActionProposal`, `CommitGate`, `HumanHandoffState`, verification, trajectory, and cleanup without emitting `computer_use.action_executed` before approval. With an explicit approval ref and CDP endpoint/websocket or approved controlled relaunch, approved click, navigate, type_text, key_press, and upload actions execute through the native-browser CDP adapter; explicit scroll runs execute through CDP when an endpoint/websocket is supplied without external-effect approval. Executor evidence attaches to `ComputerAction`, `ActionReceipt`, `VerificationReceipt`, and `CommitGate.completed`, and updates post-action observation data. Approved mutating actions without an available adapter fail closed with blocked verification and commit-gate evidence instead of synthetic execution. Daemon-backed visual and hosted/sandbox lane runs activate instead of failing closed when mounted contracts are supplied, and mounted adapter contracts plus cleanup receipts are preserved in trace/result artifacts when supplied; concrete local GUI and hosted provider adapters remain narrow future-plus deferrals until their authority and cleanup policies are productized. |

...
| Autopilot/workflow projection | Done / regression guarded | `workflow-runtime-event-projection` now projects canonical `computer_use.*` events into node-addressed React Flow/run-history rows with lane, session, lease, controlled-relaunch launch receipt, observation, screenshot/SoM refs, coordinate space, target index, visual target summaries, browser discovery, sandbox fixture provider evidence, proposal, action, verification, outcome, commit gate, cleanup, control, recovery, and fail-closed evidence. SDK, daemon, and saved workflow-run events preserve authored workflow graph/node ids, tool refs, authority scopes, observation-retention metadata, and sandbox provider/fixture metadata from composed Browser Use / Computer Use nodes; SDK thread streams now expose canonical `computer_use_*` event types instead of generic runtime steps, including operator `computer_use.control` pause/resume/abort/cleanup receipts; runtime bridge turns emit live `computer_use.action_proposed` plus `computer_use.commit_gate` events when bridge affordances imply a gated next action, and the IDE projection treats those proposal-only bridge events as typed, non-executed, confirmation-gated harness steps. |

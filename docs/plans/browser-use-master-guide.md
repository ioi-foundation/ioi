...
| Daemon planner/executor | Started | SDK-local and daemon-backed runs can now ingest canonical native-browser observation, target-index, and affordance contracts from a mounted executor and preserve them in `computer_use.*` traces. Daemon-backed visual and hosted/sandbox lane runs activate instead of failing closed when mounted contracts are supplied; real lease acquisition, action execution, verification, and cleanup through the browser/GUI/sandbox adapters are still pending. |

...
2. Live catalog/download production hardening:
   ...
   - the gated Hugging Face-compatible adapter, format/quantization filters,
     ...
     redaction are implemented;
	   ...
	   - custom HTTP and Hugging Face-compatible catalog providers now support
	     vault-backed request authentication for search/import/download paths:
	     bearer, API-key, raw-header, and OAuth2-boundary schemes are accepted,
...
GET  /api/v1/models/catalog/providers/:id
PATCH /api/v1/models/catalog/providers/:id
POST /api/v1/models/download
GET  /api/v1/models/download/status/:job_id
...
- Complete: custom HTTP and Hugging Face-compatible catalog request auth is
  vault-backed for catalog search/import/download. Bearer, API-key, raw-header,
  and OAuth2-boundary schemes are represented in provider configuration;

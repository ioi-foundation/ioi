...
- Current slice proof: focused TypeScript telemetry-source binding/runtime
  control tests, source-contract guards, agent-ide build, whitespace check, and
...
1. Continue P1-D React Flow parity: prove bound telemetry-source chains execute
   usage meter, context budget, compaction policy, and coding-tool budget gates
   end-to-end with daemon-owned events and run-inspector rows.

...
- Current slice proof: focused readiness, runtime projection, and run-history
  tests for workflow-authored coding-budget recovery policy, focused Rust
  workflow-run recovery tests with retry-limit enforcement, focused live daemon
  workflow-addressability proof, React Flow/source-contract guards, agent-ide
  build, Rust formatting, and live GUI/workflow preflight.
- Latest GUI/workflow preflight is recorded in the validation ledger for the
  coding-tool budget recovery policy authoring slice:

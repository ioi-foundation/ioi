...
- The live bridge foundation is now usable for parity work: TTI schemas,
  ...
  inspect/cancel, and run inspect/trace/replay/cancel) all have cross-surface
  proofs.
...
- 2026-05-13: P0-A/P0-C React Flow diagnostics repair action node
- Evidence:
  focused React Flow request-builder, Agent IDE, Rust projection, source
  contract, and live GUI preflight validation in the
  validation ledger
- Trace detail:
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-implementation-log.md`
  and
...
| --- | --- | --- | --- | --- |
| P0-A | Terminal coding-agent TUI | `ioi agent tui` can start/select/resume a daemon thread, submit one message, render canonical events, replay by cursor, expose event-row deep links that match React Flow run-inspector reopen descriptors, run an opt-in line-mode loop for `/resume`, `/events`, `/mode`, `/model`, `/thinking`, `/mcp`, `/memory`, `/approvals`, `/approve`, `/reject`, `/interrupt`, `/steer`, `/status`, `/diff`, `/inspect`, `/patch`, `/patch-dry-run`, `/test`, `/diagnostics`, `/diagnostics repair [retry|preview-restore|apply-restore|override]`, `/artifact`, `/retrieve`, `/restore`, `/jobs`, `/job`, `/run`, and `/quit`, prove React Flow-authored interrupt/steer/restore/diagnostics-repair nodes share the same event contract as TUI slash commands, and emit command history/current-turn/last-cursor/validation-error/mode-status/model-route/thinking/MCP/memory/approval/coding-tool/restore/diagnostics-repair/job/run-lifecycle rows that React Flow can inspect. SDK job handles now list, fetch, and cancel daemon job records through the same `/v1/jobs` contract; SDK threads can also update mode, model route, thinking, MCP, and memory status/validation/write/edit/delete controls through daemon-owned endpoints. TUI `/memory remember`, `/memory edit`, and `/memory delete` now emit canonical memory mutation rows. TUI `/mcp invoke` can execute command-backed stdio, HTTP, and SSE MCP tools, `/mcp status` emits resource/prompt catalog rows, `/mcp search` and `/mcp fetch` query daemon-owned MCP catalogs with source-mode/server/limit filters, `/mcp add`/`/mcp import`/`/mcp remove` mutate the active daemon MCP registry with receipts, `/diagnostics repair` executes daemon-owned repair retry, restore preview/apply, and operator override decisions, and React Flow `runtime_diagnostics_repair` nodes now compile those same decisions into daemon requests with graph/node identity. | Add run-inspector action controls that can execute diagnostics repair decisions directly from projected diagnostics-repair rows. | TUI panels should stay daemon-owned, event-backed, and mirrored as React Flow run-inspector rows rather than becoming canvas-local state. |
...
1. Add run-inspector action controls for diagnostics repair rows so operators
   can execute retry, restore preview/apply, and override decisions without
   hand-authoring a node, reusing the `runtime_diagnostics_repair` request
   builder and daemon endpoint.
2. Prove the full blocked-diagnostics recovery loop from projected row action
   to daemon repair event and back into React Flow/TUI replay.
3. Add dedicated MCP React Flow nodes or remaining containment/vault/egress
   authoring fields only when a workflow slice needs them.
...
still needs dedicated React Flow MCP nodes or deeper containment/vault/egress
authoring only when a workflow slice needs them.
...
- Current state nodes expose MCP status, import, add, remove, enable, and
  disable operations plus `mcp_serve`, `mcp_tool_search`, and
  `mcp_tool_fetch`.
...
1. Fold executable diagnostics repair decisions into that TUI/workflow recovery
   surface rather than adding another standalone repair endpoint.
2. Add dedicated MCP React Flow nodes or remaining containment/vault/egress
   authoring fields only when a workflow slice needs them.

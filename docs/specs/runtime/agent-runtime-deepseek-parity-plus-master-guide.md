...
- 2026-05-13: P1-B MCP resources/prompts discovery
- Evidence:
...
| P1-B | MCP manager parity | MCP manager discovery/status/validation plus governed enable/disable and invocation receipts are now daemon-owned for `.cursor/mcp.json`, `.agents/mcp.json`, inline options, and model-mounting MCP registry entries. `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, `/v1/mcp/servers/{server_id}/enable`, `/v1/mcp/servers/{server_id}/disable`, `/v1/mcp/tools/{tool_id}/invoke`, and matching thread-scoped controls expose governed catalog, validation, availability, and invocation records with secret refs redacted. Command-backed stdio MCP tools now launch through a newline-delimited JSON-RPC initialize/tools-list/tools-call path and read-only live discovery also calls `resources/list` and `prompts/list`. TUI `/mcp [status|tools|servers|validate|enable|disable|invoke]` emits MCP control-state rows for servers, tools, resources, prompts, and invocations; SDK clients can list resources/prompts; React Flow can project MCP server/tool/resource/prompt/invocation rows plus configure `mcp_tool` bindings and MCP status/enable/disable state nodes. HTTP/SSE transport, import/add/remove writes, and self-hosted serve modes remain incomplete. | Add mutable MCP import/add/remove writes next, then extend live transport beyond stdio to HTTP/SSE on the same governed receipt path. | MCP tool/resource/prompt rows and MCP state nodes carry server/tool/resource/prompt/containment metadata; next MCP server/tool/resource nodes should compile into the same daemon manager config rather than a canvas-local registry. |
...
1. Add mutable MCP import/add/remove writes on top of the governed MCP manager
   receipt path; keep live stdio invocation plus resources/prompts discovery as
...
enable/disable controls, invocation receipts, and live stdio tool execution for
command-backed servers. It also exposes read-only MCP resources/prompts through
daemon, SDK, TUI rows, and React Flow projection. It still needs HTTP/SSE
transport, import/add/remove writes, and self-hosted MCP server modes.
...
- current: daemon `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`,
  `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`,
...
- Add live HTTP/SSE MCP transport execution behind the same containment receipt
...
- live HTTP/SSE MCP transport execution preserves the same receipt contract;
...
| MCP | `McpManager` catalog/validation, availability toggles, containment metadata, governed invocation receipts, and read-only resource/prompt catalogs | `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, public and thread MCP status/validation/enable/disable/invoke controls | `getMcpStatus`, `listMcpServers`, `listMcpTools`, `listMcpResources`, `listMcpPrompts`, `validateMcp`, `enableMcpServer`, `disableMcpServer`, `invokeMcpTool`, `Thread.mcp`, `Thread.validateMcp`, `Thread.enableMcpServer`, `Thread.disableMcpServer`, `Thread.invokeMcpTool` | TUI `/mcp [status|tools|servers|validate|enable|disable|invoke]` with server/tool/resource/prompt/invocation rows | MCP TUI rows plus configurable `mcp_tool` binding metadata and MCP status/enable/disable state nodes; mutable config nodes and live HTTP/SSE transport/resource execution still pending |
...
1. Add mutable MCP import/add/remove writes on top of the governed MCP manager
   receipt path.
2. Extend live MCP transport from stdio to HTTP/SSE, then add self-hosted serve
   mode.
...
4. Add redaction review and first-class memory scope/injection aliases only
   after MCP live transport work or a concrete workflow authoring need.

...
- 2026-05-14: P1-D CLI/TUI coding-tool budget-row producer
- Evidence:
  focused CLI/TUI coding-tool budget-row unit tests, focused live daemon
  coding-tool summary-budget proof consuming actual `/events --json`
  `coding_tool_rows`, React Flow/source-contract guards, and live GUI/workflow
...
- Current slice proof: focused CLI/TUI coding-tool budget-row tests, focused
  live daemon coding-tool summary-budget proof consuming actual
  `/events --json` `coding_tool_rows`, React Flow/source-contract guards, and
  live GUI/workflow preflight.
- Latest GUI/workflow preflight is recorded in the validation ledger for the
  CLI/TUI coding-tool budget-row producer slice.

Next runtime implementation sequence:

1. Continue P1-D usage/cost/context polish: teach
   `workflowRuntimeTelemetrySummaryFromProjection` to count and summarize
   `coding_tool_budget` TUI rows as `tui_coding_tool_rows`, then prove a
   subsequent React Flow coding-tool budget control can consume that summary.

...
- 2026-05-13: P0-D restore workflow nodes and request builders
- Evidence:
...
1. Add keyboard-first TUI `/restore` UX over the same snapshot preview/apply
   endpoints, including approval-safe apply and event replay rows.
2. Add action execution for diagnostics repair decisions:
   `repair_retry`, `restore_preview`, `restore_apply`, and
   `operator_override`.
3. If another keyboard-first TUI gap appears while wiring restore, keep it
...
1. Add keyboard-first TUI `/restore` commands over snapshot list,
   restore-preview, and approval-safe restore-apply.
2. Execute diagnostics repair decisions from the graph policy:
   `repair_retry`, `restore_preview`, `restore_apply`, and
   `operator_override`.
3. Move from the thin TUI shell toward the full terminal coding-agent surface:

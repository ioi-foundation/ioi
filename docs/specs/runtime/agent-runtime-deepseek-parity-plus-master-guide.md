...
- P1-D coding-tool budget recovery now has a first-class
  ...
  readiness failure. The run inspector can now materialize or hydrate the same
  four-node chain directly from selected telemetry summaries, preserving
  evidence-link metadata, avoiding duplicate compatible chains, and proving
  readiness-positive plus broken-upstream negative cases in the GUI harness. A
  focused live daemon proof now executes the run-inspector-created/hydrated
  chain end-to-end through usage, context-budget, compaction-policy, context
  compaction, and coding budget-gate events while preserving generated
  graph/node identity and proving repeated inspector use hydrates instead of
...
- The live Autopilot GUI harness package/activation evidence path is green
  again. The rollback/package/activation source proof now follows the
  componentized settings-harness modules for rollback restore canary UI,
  package evidence gate, activation gate inspector/actions, worker-session
...
- Terminal-first coding-agent parity now has a live end-to-end proof from the
  ...
  recovery remain distinct, replayable inspector facts. React Flow workflow
  authoring now has a reusable terminal coding-loop template that prewires the
  same daemon-owned coding-pack tools, policy defaults, diagnostics/restore
  settings, artifact retrieval, result retrieval, and TUI reopen metadata. The
  ...
  The same loop now has a saved-workflow run-launch helper that reads persisted
  React Flow workflow nodes, dispatches them through daemon coding-tool
  controls, returns a normal `WorkflowRunResult`, and feeds run-history plus TUI
  projection with the same canonical daemon events.
...
- Current slice proof: saved terminal coding-loop workflow run launch through a
  reusable React Flow runtime helper. The proof persists the workflow package,
  launches saved coding-tool nodes against a daemon thread, handles patch
  approval, carries workspace snapshot rollback refs, test artifact read, and
  result retrieval through execution context, emits a normal `WorkflowRunResult`,
  and validates run-history projection over daemon events plus TUI rows.
...
1. Wire the saved terminal coding-loop run-launch helper into the actual
   WorkflowComposer/Tauri run action, proving a persisted React Flow workflow's
   Run button dispatches the same status/diff/inspect/patch/test/diagnostics/
   artifact/retrieve nodes through the shared helper and records normal
   scheduler/run-history state.
...
3. Keep P1-D telemetry chain execution locked as regression coverage while the
   terminal coding-loop composer activation proof becomes the next primary
   parity gap.

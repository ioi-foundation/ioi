...
- 2026-05-13: P1-B remote MCP auth/vault header hardening
- Evidence:
...
| P1-B | MCP manager parity | MCP manager discovery/status/validation plus governed import/add/remove, enable/disable, invocation receipts, self-hosted HTTP JSON-RPC serve mode, and vault-backed remote auth headers are now daemon-owned for `.cursor/mcp.json`, `.agents/mcp.json`, inline options, active thread registries, and model-mounting MCP registry entries. `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, `/v1/mcp/import`, `/v1/mcp/serve`, `/v1/mcp/servers`, `/v1/mcp/servers/{server_id}`, `/v1/mcp/servers/{server_id}/enable`, `/v1/mcp/servers/{server_id}/disable`, `/v1/mcp/tools/{tool_id}/invoke`, and matching thread-scoped controls expose governed catalog, validation, mutable registry writes, availability, invocation records, served IOI tool calls, redacted secret-ref provenance, and request-time vault resolution evidence without publishing header material. Command-backed stdio MCP tools launch through newline-delimited JSON-RPC, streamable HTTP servers launch through POST JSON-RPC, and SSE servers launch through endpoint-announced event streams; live discovery calls `tools/list`, `resources/list`, and `prompts/list` across supported transports. Remote HTTP/SSE auth-looking headers fail closed unless configured as `vault://` refs, and resolved material is injected only inside live transport requests. TUI `/mcp [status|tools|servers|validate|import|add|remove|enable|disable|invoke]` emits MCP control-state rows; SDK clients and `Thread` handles can import/add/remove servers and call `mcpServeRpc`; React Flow exposes MCP import/add/remove/serve state-node operations with transport, URL, vault header refs, server config JSON, serve endpoint, and allowed-tool JSON fields. | Add tool-search/deferred exposure for large MCP servers, then finish global IOI config discovery. | MCP tool/resource/prompt rows and MCP state nodes carry server/tool/resource/prompt/containment/vault-boundary metadata; MCP import/add/remove/serve state nodes compile transport/url/vault-header/served-tool config into daemon controls rather than a canvas-local registry. |
...
1. Add deferred/tool-search exposure for large MCP servers while preserving
   stdio, HTTP, SSE, resources/prompts discovery, registry writes,
   vault-backed remote auth, and self-hosted serve mode as regression fixtures.
...
resolve through vault refs at request time without publishing material. It
still needs large-server deferred exposure.
...
- current: daemon `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`,
  ...
  `/v1/mcp/import`, `/v1/mcp/servers`, `/v1/mcp/servers/{server_id}`,
...
- Current state nodes expose MCP status, import, add, remove, enable, and
  disable operations plus `mcp_serve`.
...
| MCP | `McpManager` catalog/validation, registry mutations, availability toggles, containment/vault-boundary metadata, stdio/HTTP/SSE governed invocation receipts, vault-backed remote auth headers, self-hosted serve mode, and read-only resource/prompt catalogs | `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, `/v1/mcp/import`, `/v1/mcp/serve`, public and thread MCP status/validation/import/add/remove/enable/disable/invoke/serve controls | `getMcpStatus`, `listMcpServers`, `listMcpTools`, `listMcpResources`, `listMcpPrompts`, `validateMcp`, `importMcp`, `addMcpServer`, `removeMcpServer`, `enableMcpServer`, `disableMcpServer`, `invokeMcpTool`, `serveMcpRpc`, `Thread.mcp`, `Thread.validateMcp`, `Thread.importMcp`, `Thread.addMcpServer`, `Thread.removeMcpServer`, `Thread.enableMcpServer`, `Thread.disableMcpServer`, `Thread.invokeMcpTool`, `Thread.mcpServeRpc` | TUI `/mcp [status|tools|servers|validate|import|add|remove|enable|disable|invoke]` with server/tool/resource/prompt/invocation rows | MCP TUI rows plus configurable `mcp_tool` binding metadata and MCP status/import/add/remove/enable/disable/serve state nodes with transport, URL, vault header refs, config JSON, serve endpoint, and served-tool allowlist fields |
...
1. Add deferred/tool-search exposure for large MCP servers while keeping
   served IOI tools, inbound remote transports, vault auth, and registry writes
   under the same receipt contract.

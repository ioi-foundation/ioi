...
- 2026-05-13: P0-C executable diagnostics repair `restore_apply`
- Evidence:
...
1. Add action execution for the remaining diagnostics repair decisions:
   `repair_retry` and `operator_override`. Use the restore repair executors as
   the reference pattern for policy refs, receipts, and React Flow identity.
...
1. Execute the remaining diagnostics repair decisions from the graph policy:
   `repair_retry` and `operator_override`.

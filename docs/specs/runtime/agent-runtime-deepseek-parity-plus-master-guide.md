...
- 2026-05-14: P1-D coding-tool summary-budget enforcement
- Evidence:
  coding-tool-control and telemetry-summary unit tests, agent-ide build,
  focused live daemon coding-tool summary-budget proof, approval regression
  proof, React Flow source-contract guards, and live GUI/workflow preflight
  validation in the validation ledger
- Trace detail:
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-implementation-log.md`
  and
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-validation-ledger.md`
...
- Current slice proof: focused React Flow/TUI coding-tool budget projection
  tests, agent-ide build, focused live daemon coding-tool summary-budget proof
  with budget-specific node/row projection, source-contract guards, full
  projection suite, and live GUI/workflow preflight.
- Latest GUI/workflow preflight is recorded in the validation ledger for the
  coding-tool budget-block inspector projection slice.

Next runtime implementation sequence:

1. Continue P1-D usage/cost/context polish: wire actual CLI/TUI `/events`
   control-state JSON to emit `coding_tool_rows` from daemon
   `policy.blocked` coding-tool events, then assert React Flow consumes that
   live row unchanged.

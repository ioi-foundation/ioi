...
- 2026-05-13: P1-B live MCP HTTP/SSE transport
- Evidence:
...
| P1-B | MCP manager parity | MCP manager discovery/status/validation plus governed import/add/remove, enable/disable, and invocation receipts are now daemon-owned for `.cursor/mcp.json`, `.agents/mcp.json`, inline options, active thread registries, and model-mounting MCP registry entries. `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, `/v1/mcp/import`, `/v1/mcp/servers`, `/v1/mcp/servers/{server_id}`, `/v1/mcp/servers/{server_id}/enable`, `/v1/mcp/servers/{server_id}/disable`, `/v1/mcp/tools/{tool_id}/invoke`, and matching thread-scoped controls expose governed catalog, validation, mutable registry writes, availability, and invocation records with secret refs redacted. Command-backed stdio MCP tools launch through newline-delimited JSON-RPC, streamable HTTP servers launch through POST JSON-RPC, and SSE servers launch through endpoint-announced event streams; live discovery calls `tools/list`, `resources/list`, and `prompts/list` across supported transports. TUI `/mcp [status|tools|servers|validate|import|add|remove|enable|disable|invoke]` emits MCP control-state rows; SDK clients and `Thread` handles can import/add/remove servers; React Flow exposes MCP import/add/remove state-node operations with transport, URL, headers, and server config JSON fields. Self-hosted serve mode remains incomplete. | Add self-hosted MCP serve mode, then harden remote auth/vault header resolution and large-server deferred tool exposure. | MCP tool/resource/prompt rows and MCP state nodes carry server/tool/resource/prompt/containment metadata; MCP import/add/remove state nodes compile transport/url/header config into daemon registry mutation controls rather than a canvas-local registry. |
...
1. Add self-hosted MCP serve mode so IOI can expose governed runtime tools to
   another MCP client; keep stdio, HTTP, SSE, resources/prompts discovery, and
   registry writes as regression fixtures.
...
It still needs self-hosted MCP server mode and remote auth/vault header
hardening.
...
| MCP | `McpManager` catalog/validation, registry mutations, availability toggles, containment metadata, stdio/HTTP/SSE governed invocation receipts, and read-only resource/prompt catalogs | `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, `/v1/mcp/import`, public and thread MCP status/validation/import/add/remove/enable/disable/invoke controls | `getMcpStatus`, `listMcpServers`, `listMcpTools`, `listMcpResources`, `listMcpPrompts`, `validateMcp`, `importMcp`, `addMcpServer`, `removeMcpServer`, `enableMcpServer`, `disableMcpServer`, `invokeMcpTool`, `Thread.mcp`, `Thread.validateMcp`, `Thread.importMcp`, `Thread.addMcpServer`, `Thread.removeMcpServer`, `Thread.enableMcpServer`, `Thread.disableMcpServer`, `Thread.invokeMcpTool` | TUI `/mcp [status|tools|servers|validate|import|add|remove|enable|disable|invoke]` with server/tool/resource/prompt/invocation rows | MCP TUI rows plus configurable `mcp_tool` binding metadata and MCP status/import/add/remove/enable/disable state nodes with transport, URL, headers, and config JSON fields; self-hosted MCP serve mode still pending |

...
- 2026-05-13: P0-D policy-gated workspace restore apply
- Evidence:
...
1. Bind blocking diagnostics findings to the rollback/repair policy so a failed
   `auto` diagnostic can propose restore preview, repair retry, or explicit
   operator override.
2. Promote restore preview/apply controls into the workflow binding editor so
...
1. Add rollback snapshots now that diagnostics can identify changed files and
   blocking findings.
2. Bind diagnostics findings to restore preview/repair policy.

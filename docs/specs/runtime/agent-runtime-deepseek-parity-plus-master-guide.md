...
- Current slice proof: CLI build/parser checks, React Flow TUI cost/context
  projection test, Agent IDE build, live daemon line-mode `/cost` and
  `/context` proof, workflow-addressable regression guard, and live GUI
  preflight.
...
1. Add streaming usage/context deltas and explicit context-pressure rows now
   that TUI `/cost` and `/context` have stable output contracts.

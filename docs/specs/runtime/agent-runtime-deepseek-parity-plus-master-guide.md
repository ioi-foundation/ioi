...
- 2026-05-14: P1-D coding-tool budget recovery approval/retry path
- Evidence:
  focused Rust workflow-run recovery test, focused runtime projection,
  policy-stack, and run-history tests, source contracts for the daemon recovery
  lane, composer recovery handler, run-inspector recovery buttons, and React
  Flow projection, focused live daemon workflow-addressability proof,
...
- Current slice proof: focused Rust workflow-run recovery test, runtime
  projection, policy-stack, and run-history tests for daemon-owned
  coding-budget recovery chains, focused live daemon workflow-addressability
  proof, React Flow/source-contract guards, agent-ide build, and live
  GUI/workflow preflight.
- Latest GUI/workflow preflight is recorded in the validation ledger for the
  coding-tool budget recovery approval/retry path slice:

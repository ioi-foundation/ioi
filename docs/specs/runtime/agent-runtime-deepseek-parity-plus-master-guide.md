...
- Current slice proof: daemon syntax, focused React Flow approval/projection
  tests, live daemon approval-gate proof, source-contract guard, Agent IDE
  build, Rust action-kind test, and live GUI preflight.
...
1. Wire the remaining context-pressure `delegate_summary` action to a
   daemon-owned subagent-summary control endpoint and React Flow workflow node.

...
- 2026-05-14: P1-D coding-tool budget daemon launch policy
- Evidence:
  focused Rust workflow-run persistence test, focused runtime projection and
  run-history tests, source contracts for the daemon policy lane, composer
  controller/view, run-result telemetry fields, and React Flow projection,
  focused live daemon coding-tool proof, agent-ide build, and live
...
- Current slice proof: focused Rust workflow-run policy persistence test,
  runtime projection and run-history tests for daemon-owned coding-budget
  launch blocks, focused live daemon coding-tool proof, React
  Flow/source-contract guards, agent-ide build, and live GUI/workflow
  preflight.
- Latest GUI/workflow preflight is recorded in the validation ledger for the
  coding-tool budget daemon launch policy slice.

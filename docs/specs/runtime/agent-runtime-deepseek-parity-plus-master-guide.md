...
- P1-D coding-tool budget recovery now has a first-class
  ...
  run/thread/approval/target/policy values plus evidence-link metadata.
...
- 2026-05-14: P1-D coding-tool budget recovery template binding assistant
- Evidence:
  React Flow readiness repair actions and run-inspector controls now bind
  selected blocked-run budget evidence into reusable recovery-template
  request/approve/reject/retry nodes, filling fixed runtime values, runtime
  input mappings, test input, and evidence binding metadata before activation.

Previous focused slice:

- 2026-05-14: P1-D coding-tool budget recovery template readiness validation
- Evidence:
  Workflow activation readiness now surfaces missing recovery-template
  `runId`, `threadId`, `approvalId`, `targetNodeIds`, and `recoveryPolicy`
  bindings as actionable node issues before a daemon recovery request executes.
- Trace detail:
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-implementation-log.md`
  and
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-validation-ledger.md`
...
| P1-D | Usage, cost, context telemetry | Usage, context-pressure, budget gates, coding-tool budget blocks, recovery policy, TUI `/run recovery`, first-class React Flow `runtime_coding_tool_budget_recovery` nodes, run-inspector recovery subflow materialization, generated-subflow daemon execution proof, a reusable workflow-creator recovery template, readiness validation for missing recovery-template runtime inputs, and a React Flow quick-fix/run-inspector binding assistant now share telemetry, policy, event, approval, retry, and evidence-link contracts with graph/node identity preserved. | Add a React Flow telemetry-source binding assistant that wires selected runtime usage/context summary evidence into usage meter, context budget, compaction-policy, and budget-gate nodes so authors can preflight caps from daemon-owned run telemetry. | Usage, context, budget, recovery, and run-inspector controls must simulate, stream, inspect, and enforce workflow caps from daemon-owned telemetry and recovery events rather than canvas-local counters. |

### Immediate Tactical Queue

1. Continue P1-D React Flow parity: add a telemetry-source binding assistant
   that wires selected runtime usage/context summary evidence into usage meter,
   context budget, compaction-policy, and budget-gate nodes from the React Flow
   graph instead of hand-editing JSON.
...
- Current slice proof: focused TypeScript recovery-binding/readiness/template
  tests, source-contract guards, agent-ide build, whitespace check, and live
...
1. Continue P1-D React Flow parity: add a React Flow telemetry-source binding
   assistant that resolves usage/context/budget node setup by wiring selected
   runtime summary evidence into usage meter, context budget, compaction-policy,
   and budget-gate mappings.

...
- 2026-05-13: P1-B MCP import/add/remove registry writes
- Evidence:
...
| P1-B | MCP manager parity | MCP manager discovery/status/validation plus governed import/add/remove, enable/disable, and invocation receipts are now daemon-owned for `.cursor/mcp.json`, `.agents/mcp.json`, inline options, active thread registries, and model-mounting MCP registry entries. `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, `/v1/mcp/import`, `/v1/mcp/servers`, `/v1/mcp/servers/{server_id}`, `/v1/mcp/servers/{server_id}/enable`, `/v1/mcp/servers/{server_id}/disable`, `/v1/mcp/tools/{tool_id}/invoke`, and matching thread-scoped controls expose governed catalog, validation, mutable registry writes, availability, and invocation records with secret refs redacted. Command-backed stdio MCP tools now launch through a newline-delimited JSON-RPC initialize/tools-list/tools-call path and read-only live discovery also calls `resources/list` and `prompts/list`. TUI `/mcp [status|tools|servers|validate|import|add|remove|enable|disable|invoke]` emits MCP control-state rows; SDK clients and `Thread` handles can import/add/remove servers; React Flow exposes MCP import/add/remove state-node operations with server config JSON fields. HTTP/SSE transport and self-hosted serve modes remain incomplete. | Extend live transport beyond stdio to HTTP/SSE on the same governed receipt path, then add self-hosted MCP serve mode. | MCP tool/resource/prompt rows and MCP state nodes carry server/tool/resource/prompt/containment metadata; MCP import/add/remove state nodes compile config into daemon registry mutation controls rather than a canvas-local registry. |
...
1. Extend live MCP transport from stdio to HTTP/SSE on the governed receipt
   path; keep stdio invocation, resources/prompts discovery, and registry writes
...
live stdio tool execution for command-backed servers. It also exposes read-only
MCP resources/prompts through daemon, SDK, TUI rows, and React Flow projection.
It still needs HTTP/SSE transport and self-hosted MCP server modes.
...
| MCP | `McpManager` catalog/validation, registry mutations, availability toggles, containment metadata, governed invocation receipts, and read-only resource/prompt catalogs | `/v1/mcp`, `/v1/mcp/servers`, `/v1/mcp/tools`, `/v1/mcp/resources`, `/v1/mcp/prompts`, `/v1/mcp/validate`, `/v1/mcp/import`, public and thread MCP status/validation/import/add/remove/enable/disable/invoke controls | `getMcpStatus`, `listMcpServers`, `listMcpTools`, `listMcpResources`, `listMcpPrompts`, `validateMcp`, `importMcp`, `addMcpServer`, `removeMcpServer`, `enableMcpServer`, `disableMcpServer`, `invokeMcpTool`, `Thread.mcp`, `Thread.validateMcp`, `Thread.importMcp`, `Thread.addMcpServer`, `Thread.removeMcpServer`, `Thread.enableMcpServer`, `Thread.disableMcpServer`, `Thread.invokeMcpTool` | TUI `/mcp [status|tools|servers|validate|import|add|remove|enable|disable|invoke]` with server/tool/resource/prompt/invocation rows | MCP TUI rows plus configurable `mcp_tool` binding metadata and MCP status/import/add/remove/enable/disable state nodes; live HTTP/SSE transport/resource execution still pending |
...
1. Extend live MCP transport from stdio to HTTP/SSE, then add self-hosted serve
   mode.

...
- 2026-05-13: P1-D TUI `/cost` and `/context` telemetry controls
- Evidence:
  CLI build/parser checks, React Flow TUI control-state projection test, Agent
  IDE build, live daemon line-mode TUI proof, workflow-addressable regression
  guard, and live GUI preflight validation in the validation ledger
- Trace detail:
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-implementation-log.md`
  and
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-validation-ledger.md`
...
| P1-D | Usage, cost, context telemetry | `RuntimeUsageTelemetry` now aggregates run, thread, and delegated subagent token/cost/context estimates through daemon endpoints, SDK clients, TUI status rows, `usage_final` runtime events, and React Flow run-inspector rows. React Flow now has first-class `UsageMeterNode`, `ContextBudgetNode`, and `CompactionPolicyNode` definitions: usage reads compile into daemon-owned telemetry requests, context-budget policy evaluations compile token/cost/context-pressure thresholds into daemon-owned `ok`/`warn`/`blocked` decisions, and compaction policy evaluations turn budget outcomes into receipt-backed warn/compact/stop/approval-required actions with graph/node identity. TUI `/cost`, `/context`, and `/events` now read those same daemon telemetry/policy streams and project `cost_status`, `context_budget`, `compaction_policy`, `usage_delta`, and `context_pressure_delta` rows. Runtime fixture and runtime-service bridge turns emit streaming `usage.delta` and `context.pressure_delta` events with stable workflow node identity before `usage_final`. | Hydrate active React Flow run history and runtime telemetry rails from live thread-event deltas during a running turn, using the same SDK event stream and projection contract as TUI `/events`. | Usage-meter, context-budget, and compaction policy nodes simulate, stream, and enforce workflow caps from daemon-owned telemetry rather than canvas-local counters. |

### Immediate Tactical Queue

1. Hydrate active React Flow run history and telemetry rails from live
   `usage_delta` and `context_pressure_delta` thread events during an
   in-flight turn, not only after a post-run refresh.
2. Add explicit context-pressure alert/action rows for long-running turns and
   delegated subagent aggregates where policy thresholds are crossed.
...
- Current slice proof: CLI build/parser checks, SDK and Agent IDE builds,
  React Flow streaming usage/context-pressure projection test, live daemon
  usage aggregation proof, line-mode `/events` streaming telemetry proof,
  workflow-addressable regression guard, and live GUI preflight.
...
1. Hydrate active React Flow run history and runtime telemetry rails from the
   live `usage_delta` and `context_pressure_delta` event stream.

...
- P1-D coding-tool budget recovery now has a first-class
  ...
  the TypeScript workflow model and the Tauri desktop validation path.
...
- Evidence:
  The React Flow composition helper inserts a reusable request/approve/reject/
  retry coding-tool budget recovery bundle whose daemon request nodes keep run
  id, thread id, approval id, target node ids, and recovery policy configurable
  through runtime inputs.
- Trace detail:
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-implementation-log.md`
  and
  `docs/specs/runtime/agent-runtime-deepseek-parity-plus-validation-ledger.md`
...
| P1-D | Usage, cost, context telemetry | Usage, context-pressure, budget gates, coding-tool budget blocks, recovery policy, TUI `/run recovery`, first-class React Flow `runtime_coding_tool_budget_recovery` nodes, run-inspector recovery subflow materialization, generated-subflow daemon execution proof, a reusable workflow-creator recovery template, and readiness/binding validation for missing recovery-template runtime inputs now share telemetry, policy, event, approval, retry, and evidence-link contracts with graph/node identity preserved. | Add a React Flow quick-fix/binding assistant that resolves recovery-template readiness issues by wiring selected blocked-run budget evidence into the template's run/thread/approval/target/policy mappings. | Usage, context, budget, recovery, and run-inspector controls must simulate, stream, inspect, and enforce workflow caps from daemon-owned telemetry and recovery events rather than canvas-local counters. |

### Immediate Tactical Queue

1. Continue P1-D React Flow parity: add a quick-fix/binding assistant for
   coding-tool budget recovery template readiness issues, so authors can wire
   selected blocked-run evidence into run/thread/approval/target/policy mappings
   from the React Flow graph instead of hand-editing JSON.
...
- Current slice proof: focused TypeScript readiness/template tests, focused
  Tauri activation-readiness test, source-contract guards, agent-ide build,
  Rust formatting check, whitespace check, and live GUI/workflow preflight.
...
1. Continue P1-D React Flow parity: add a React Flow quick-fix/binding
   assistant that resolves coding-budget recovery template readiness issues by
   wiring selected blocked-run evidence into run/thread/approval/target/policy
   mappings.

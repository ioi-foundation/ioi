...
Implementation slice completed 2026-05-14, P1-D/P0-B reusable recovery
template:

- Added an input-driven recovery-subflow template factory that uses the same
  request approval, approve override, reject override, and approved retry
  `runtime_coding_tool_budget_recovery` nodes as blocked-run materialization.
- Kept template run id, thread id, approval id, source/blocked event ids,
  target node ids, and recovery policy configurable through runtime inputs, so
  authors can prebuild the bundle before a blocked run exists.
- Refactored React Flow insertion through a shared subflow inserter and exposed
  a `Coding budget recovery` composition helper beside the existing agent-loop
  helper.
- Exported the template helper from `@ioi/agent-ide` and extended the live
  source-contract guard so the helper, controller callback, and creator-library
  button remain workflow-addressable.
- Updated the master guide so the next P1-D slice moves to readiness/binding
  validation for input-driven recovery templates.
...
- `npm run build -w packages/agent-ide`
- `git diff --check`
- `npm run validate:autopilot-gui-harness -- --output-root /tmp/ioi-autopilot-gui-harness-coding-tool-budget-recovery-template`
  - preflight passed and wrote
    `/tmp/ioi-autopilot-gui-harness-coding-tool-budget-recovery-template/2026-05-14T12-58-07-902Z/result.json`.
...
recovery policy authoring:

- Added a shared workflow/runtime recovery-policy descriptor for coding-tool
  budget blocks with schema version, approval scope, target node ids, retry
  limit, TTL, operator role, approval requirement, and override allowance.
- Promoted recovery policy into React Flow coding-tool pack authoring controls,
  including editor defaults and typed graph fields, so workflows can configure
  launch-block recovery instead of relying on hard-coded defaults.
- Threaded the authored policy through readiness/run-launch annotations,
  blocked daemon policy payloads, runtime event projection, run-inspector
  recovery actions, and `codingToolBudgetRecovery` retry requests.
- Hardened the Tauri policy lane so recovery approval/retry payloads preserve
  the policy, approved retries use policy target nodes, `approve_override`
  respects `allowOverride`, and retry execution fails once the authored retry
  limit is exhausted.
- Updated source-contract guards and the master guide queue so the next slice
  moves to TUI slash-command recovery parity over the same policy contract.

Validation evidence:

- `node --import tsx --test packages/agent-ide/src/runtime/workflow-readiness-model.test.ts packages/agent-ide/src/runtime/workflow-runtime-event-projection.test.ts packages/agent-ide/src/runtime/workflow-run-history-model.test.ts`
...
- `npm run build --workspace=@ioi/agent-ide`
- `cargo fmt --manifest-path apps/autopilot/src-tauri/Cargo.toml -- --check`
- `npm run validate:autopilot-gui-harness -- --output-root /tmp/ioi-autopilot-gui-harness-coding-tool-budget-recovery-policy`
  - preflight passed and wrote
    `/tmp/ioi-autopilot-gui-harness-coding-tool-budget-recovery-policy/2026-05-14T05-03-31-611Z/result.json`.
...
- Added run-history telemetry source filters derived from
  `WorkflowRuntimeTelemetrySummary.sourceKinds`, including an active
  `tui_coding_tool_rows` filter that narrows visible TUI control-state rows to
  prior coding-tool budget evidence.
- Added a first-class run-history `runtimeCodingToolBudgetEvidence` model with
  row counts, event ids, workflow node ids, tool names, tool-call ids, budget
  statuses, context-budget statuses, token/cost/context usage, receipt refs,
  policy refs, and blocked-mutation state.
- Updated the Runs rail to render source-filter chips, visible telemetry source
  labels, a coding-tool budget evidence card, coding budget counts in the
  usage/context summary, and filtered TUI row counts.
- Added focused run-history coverage proving `tui_coding_tool_rows` exposes the
  prior budget block before a follow-up coding-tool budget control executes.
- Updated source-contract guards and the master guide queue so the next P1-D
  slice moves the same evidence into workflow readiness/run preflight.

Validation evidence:

- `node --import tsx --test packages/agent-ide/src/runtime/workflow-run-history-model.test.ts packages/agent-ide/src/runtime/workflow-runtime-telemetry-summary.test.ts`
- `node --test scripts/lib/workflow-runtime-event-projection-contract.test.mjs`
- `npm run build --workspace=@ioi/agent-ide`
- `node --test --test-name-pattern "React Flow coding-tool budget gates consume runtime telemetry summary before mutation" scripts/lib/live-runtime-daemon-contract.test.mjs`
- `node --test --test-name-pattern "React Flow memory, authority/tooling, doctor, skill, hook, and package node contracts remain workflow-addressable" scripts/lib/live-runtime-daemon-contract.test.mjs`
- `npm run validate:autopilot-gui-harness -- --output-root /tmp/ioi-autopilot-gui-harness-coding-tool-budget-run-history`
  - preflight passed and wrote
    `/tmp/ioi-autopilot-gui-harness-coding-tool-budget-run-history/2026-05-14T03-39-35-777Z/result.json`.
...
Implementation slice completed 2026-05-14, P1-D summary-driven budget
enforcement:

- Added `workflowRuntimeTelemetrySummaryToUsageTelemetry`, a daemon-friendly
  projection of the shared run-history telemetry summary with snake/camel token,
  cost, context-pressure, source-count, receipt, and policy-decision fields.
- Updated React Flow `runtime_context_budget` request compilation so a workflow
  can point `runtimeContextBudgetUsageField` at `runtimeTelemetrySummary` and
  send daemon-normalized usage telemetry rather than requiring each surface to
  reparse row/event shapes.
- Extended daemon usage-summary tolerance for `cost_estimate_usd` and
  `costEstimateUsd` so raw summary-shaped telemetry remains enforceable at the
  runtime boundary.
- Proved the path in the live usage/subagent/context fixture by creating a
  React Flow-configured context-budget gate that blocks from the same telemetry
  summary shown in the run inspector.
- Updated the master guide so the next P1-D slice narrows to subagent
  continuation/budget and coding-tool continuation decisions.
...
- `node --test scripts/lib/workflow-runtime-event-projection-contract.test.mjs`
  - React Flow source-contract guard passed with summary-to-budget coverage.
...
- `node --test --test-name-pattern "daemon aggregates usage" scripts/lib/live-runtime-daemon-contract.test.mjs`
  - live daemon usage/subagent/context proof passed with summary-driven
...
Implementation slice completed 2026-05-13, P1-D streaming telemetry:

- Added runtime `usage.delta` and `context.pressure_delta` events for fixture
  turns and runtime-service bridge turns, with schema versions, staged
  cumulative/delta token and cost payloads, context-pressure summaries, stable
  workflow node ids, component kinds, and TTI event mappings.
- Extended SDK message and thread-event typing so `usage_delta` and
  `context_pressure_delta` round-trip through daemon streams, SDK `Thread`
  replay, and React Flow event projection.
- Projected streaming usage deltas to `runtime.usage-telemetry` /
  `runtime_usage_meter` and context-pressure deltas to
  `runtime.context-budget` / `runtime_context_budget`.
- Extended TUI control-state and line-mode `/events` output with
  stream-backed usage and context rows so live terminal controls and React Flow
  workflow projections read the same daemon event stream.
- Updated live daemon contracts to assert raw events, SDK event replay,
  projected React Flow node identity, line-mode `/events` output, and the
  existing workflow-addressable regression guard.

# ADR 0011: Canonicalize Autopilot Nodes As Local Settlement Domains

- Status: Accepted
- Date: 2026-05-24
- Owners: IOI architecture / Autopilot / daemon runtime / Agentgres / wallet.network / IOI L1

## Context

IOI needs precise language for the relationship between local autonomous
systems, Autopilot, Agentgres, wallet.network, and IOI L1.

Several useful analogies were circulating:

- each agent as an intelligent blockchain;
- each autonomous system as a local L1;
- Autopilot as a settlement layer;
...
It is the composition of Autopilot Workbench, IOI daemon, Agentgres,
wallet.network authority paths, local registries, receipt/replay stores, and
runtime profiles.
...
- Public architecture should avoid saying "each agent is a blockchain" without
  ...
  system`, `intelligent execution node`, or `Autopilot node` depending on the
  layer.
- Internal state-machine discussions may use "system-local base layer" for a
  ...
  public chain.
- Autopilot-node settlement means local canonical acceptance of work, state
...
- Agentgres owns the local/domain operational truth for autonomous-system
  chains and Autopilot-node state.
...
- Do not make Autopilot Workbench the settlement layer.

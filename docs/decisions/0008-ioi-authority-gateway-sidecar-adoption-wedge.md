...
- Status: Accepted
- Date: 2026-05-20
- Owners: daemon runtime / Autopilot / connectors-tools / wallet.network / policy

## Context

Autopilot Workbench is the canonical full operator console for IOI-governed
...
Autopilot Workbench remains the native full experience and IDE-grade operator
console. IOI Authority Gateway is not a separate runtime and not merely a VS
Code extension identity. "Autopilot Guard" may be used as developer-facing
packaging for the adapter bundle, but the canonical architecture term is IOI
Authority Gateway.
...
- The sidecar creates an installed base for aiagent.xyz workers, sas.xyz
  ...
  the native Autopilot Workbench.
...
- Do not move daemon authority into an IDE extension, shell wrapper, webview,
  ...
  gate, SDK helper, or Autopilot UI surface.
- Do not claim full interception of opaque third-party runtimes when only shell,
  ...
  available.
- Do not make Authority Gateway a competing product substrate beside Autopilot
...
Autopilot Workbench = native operator console

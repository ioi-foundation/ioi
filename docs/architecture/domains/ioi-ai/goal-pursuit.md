...
The layering is three-part:

```text
GoalRunProfileEnvelope   immutable application-layer specification
  -> the Goal Kernel interprets the frozen resolution
OrchestrationPlan        application-layer plan
  -> execution effects cross the kernel seam exactly once
thread / fork / managed session / launch admission / harness binding
                         daemon-owned composition
```

A profile is a specification. It declares how a class of adaptive goals converges
by composing owner-qualified contracts; it holds no authority and runs nothing.
Execution composes the daemon-owned orchestration primitives.
...
GoalRunProfileResolutionReceipt:
  ...
  system_binding: SystemScopedObjectBinding | omitted   # (R-197, S5-3): derived by the record seam when the receipt is admitted under a bounded System, never authored; omitted for a receipt that is not admitted under one
  goal_ref: goal://...

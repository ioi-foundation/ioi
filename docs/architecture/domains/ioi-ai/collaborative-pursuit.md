...
orchestration application** — the openly packaged room application whose instances
are the flagship reference DAS. The daemon admits, executes, and receipts them as
it does any application domain.

An OutcomeRoom is a composition. It coordinates shared GoalRun work through its
graph and its bounded-System and Agentgres truth, and owns neither GoalRun
execution nor the kernel orchestration primitives. The cross-party grammar is
protocol and package level.
...
[`collaborative-outcome-pattern.md`](./collaborative-outcome-pattern.md).
...
`outcome_room_ref` is non-null, `system_binding` and every hosted coordinate
...
`system_binding` and the hosted coordinate set above are non-null and mutually
bound.

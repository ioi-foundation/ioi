...
kernel and daemon-local durable reference store currently prove these
integrity, replay, repair, and snapshot mechanics; live owner-route bindings,
independent grant and revocation verification, Agentgres-backed cross-process
append authority, cancellation execution receipts, automatic pruning, and
archive-only resume remain open production gates.
...
metering exist in parts of the current runtime. A registered managed-work
ledger-bundle contract, generated projections, shared Rust accounting kernel,
and process-local durable reference store implement the exact
quote/hold/usage/overrun/debit/downward-adjustment mechanics. They do not expose
a public billing route, resolve live runtime or supplier evidence, reconcile
provider invoices, own product entitlements or Agentgres transactions, collect
payment, execute payouts or refunds, calculate tax, or establish invoice
truth. Sellable Work Credit allowances, marketplace money flows, token or BME
settlement, and IOI L1 remain planned or deferred. Exact money movement stays
with its product, wallet.network, marketplace, provider, processor, and
settlement owners.
...
The shared deterministic mechanism can admit a profile-selected resolution and
conserved allocation plan. It does not decide whether evidence is substantively
true, hold escrow, move value, complete an appeal, or create finality.
...
\textbf{Implementation status.} A registered portable dispute-rail contract,
generated projections, and a shared Rust admission/allocation kernel implement
the exact profile, case-head, timing/default, fixed-point value-unit,
profile-selected-remedy, bond-conservation, and replay mechanics. They do not
independently verify evidence availability or truth, authenticate a live
adjudicator, hold escrow or bonds, move money or Work Credits, execute a
remedy, emit the required receipts, complete appeal supersession, or create
public finality. Automated IOI L1 adjudication, slashing, live public
settlement, and a universal arbitration registry are not claimed.
...
Where a rail uses bonds, V1 holds and allocates them in the same exact
...
declared and the owning value rail can execute them. The deterministic kernel
can prove the allocation plan conserves the held pool; it cannot prove that a
bond was held, value moved, evidence was correctly adjudicated, an appeal
closed, or public settlement became final.
...
\textbf{Implementation status: partial reference mechanisms.} The current
...
package declaring \emph{physical\_action}. A Rust-owned intent-admission
planner additionally fails closed on missing exact deployment, resource-group,
command/controller, ODD, timing, assured-input, safe-switch, restart/writer,
and teleoperation evidence. An unmounted
\textbf{PhysicalActionExecutionCore} proves fresh preflight at one typed
invoker, exact state-root and adapter-identity checks, zero-call denial,
same-body completed replay, serializable prepared/completed recovery posture,
dispatch-proof normalization, and the closed execution-receipt contract. These
seams are not a live robot-fleet runtime, certified local controller, native
actuator mount, operating LocalControlSupervisor, or estate-wide physical
effect boundary.
...
\textbf{Final-invoker and receipt-chain invariant.} The reference execution
seam makes the intended last choke point explicit:
...
reconciliation posture. & Produced by the final physical invoker seam and
eventually admitted through the runtime/Agentgres receipt owner. The current
producer is an unmounted reference core. & Preserves denial, committed,
...
\textbf{Implementation status.} A pure Rust decision kernel and canonical
fault matrix implement the modeled operation dispositions, bounded cache and
assurance narrowing, unknown-effect refusal, physical-continuation, billing,
and public-settlement rules. Reference mechanisms also cover deterministic
checkpoint/suffix replay, version negotiation, key-epoch transition, and
information-flow-preserving observability projection. Authoritative plane
observers, a scheduler-wide live-effect choke point, deployed restore and
mixed-version rollout, live key distribution/revocation, proof and billing
reconciliation controllers, correlated-failure injection, capacity shedding,
and telemetry canaries remain open production gates.
...
\textbf{Implementation status: partial mechanism.} The general assurance,
certification, liability, advisory, and commercial-export layer remains target
architecture. A shared runtime-attestation evaluator is integrated into the
runtime startup gate, and a deployment-policy obligation evaluator can project
incident-reporting and erasure-evidence state from existing policy-pack and
evidence objects. The current \texttt{ioi-agent} startup supplies no structured
attestation input and explicitly has no remote-attestation verifier, so the
integration proves the admission seam rather than a hardware-attested
deployment. Live quote acquisition, signature and endorsement services,
durable nonce and revocation operation, regulator notification, erasure
execution, legal analysis, and the broader institutional effectors are not
implemented.
...
Runtime attestation assurance preserves the RATS separation among Attester,
Verifier/Appraiser, Relying Party, and the startup decision. The evaluator
checks the exact challenge nonce and single-use receipt, workload identity,
...
Deployment-policy obligation projection binds one exact
...
posture. It can distinguish a running or overdue clock, submitted evidence,
...
Every generated \textbf{AssurancePostureProjection} therefore fixes
\texttt{legal\_conformity\_claim} to
\texttt{not\_determined}. Neither evaluator issues a
...
world cryptographically reversible. The Embodied Runtime target and partial
reference execution seam bind PhysicalActionIntent, exact resource-group
...
reference seam. Local safety controllers and hardware interlocks retain the

...
\vfill
{\footnotesize IOI Foundation \textperiodcentered{} Internet of Intelligence \textperiodcentered{} Technical Whitepaper v1.12.0}\\[0.4cm]
\end{titlepage}

\hypertarget{erratum}{%
\section{Revision Notes --- v1.12.0 (August 2026)}\label{erratum}}

This revision reconciles the publishing synthesis with the governed-effect and
AFT capstones completed after v1.11.0. It promotes architectural primitives
that now have bounded implementation evidence while preserving the exact
limits of the demonstrated profiles.

\textbf{Two-sided governed effects.} The determinism and evidence sections now
name the complete sequence: durable intent commitment before an external
effect, provider-native evidence after it, durable outcome commitment that
names the intent as predecessor, and terminal reconciliation as a separate
correctness condition. This is stronger than execute-then-log while remaining
proposition-scoped.

\textbf{Bounded autonomous authority.} The authority sections now distinguish
exact-effect approvals from passkey-rooted standing envelopes, atomic
single-use draws, and trajectory admission. They also name approval fatigue
and individually admissible actions that compose into an inadmissible
aggregate outcome as first-class threats.

\textbf{Workload and secret boundary.} A trusted-host/hostile-guest profile now
has implementation evidence for a fresh KVM guest with no network device,
bounded output admission, opaque request-bound authority, replay refusal, and
a scoped secret-non-possession probe. This is not resistance to a compromised
host kernel, VMM, daemon, firmware, or hardware.

\textbf{Portable evidence and relying-party effect.} The evidence sections now
separate certificate production, verification, acceptance, and registry
mutation. A separately provisioned first-party AFT verifier evaluated a
portable C8 v3 bundle and an AFT measured-results registry changed state because
the evidence satisfied its policy. This is not third-party verification.

\textbf{AFT status and claim discipline.} The AFT assumption ledger now carries
A1--A10, and the measured-cost appendix contains the retained Campaign O result
under its measured-container, same-provider-container-unknown-host,
variance-caveated honesty profile. The paper does not relabel that placement as
bare metal or the result as reproduced within threshold.
...
recovery. Hypervisor's autonomy plane virtualizes those autonomy primitives;
independent deployment and resource-relationship facets place the controller
on hosted, attached-estate, or future node-root substrates. Type 1 and Type 2
remain external market mappings earned by exact profile evidence, and ``Type
3'' is only qualified IOI shorthand rather than a standardized VMM category.
The ontology makes the
...
first-party execution and control environment, with no release yet formally
designated as the reference implementation; locally canonical Domain
Ontologies with optional versioned federation are
the semantic world plane; Machine Authority is the security category and the
IOI Authority Protocol is its portable protocol-family target;
...
\node[ioibox,text width=92mm] (room) {\textbf{OutcomeRoom (optional durable DAS)}\\\footnotesize shared objective, terms, participant leases, frontier, claims, typed bindings};
...
\node[ioibox,below=of result,text width=92mm] (admit) {\textbf{Verification + Agentgres / System admission}\\\footnotesize admit, reject, challenge, or request repair};
...
Every consequential room child carries a non-authoritative
\textbf{SystemScopedObjectBinding}: the enclosing room System, room object,
issuer or proposer, payload root, and timestamps. Its mutation is an ordinary
Agentgres operation under the bounded System, where expected heads and System
predecessor continuity provide compare-and-swap, and where policy, decision,
sequence, admission proof, state root, and receipt root remain canonical. A
room object may reference that evidence but cannot mint or own a parallel
admission chain. This makes untrusted local-agent proposals useful without
making them trusted room truth.

Every persistent OutcomeRoom is instantiated through genesis as a bounded
System and therefore has one canonical System/Agentgres operation chain. The
hosted profile orders frontier, attempt, finding, evaluation, and decision
updates through that chain. A future cross-System federated profile requires a
separate owner ruling and must not be inferred by recreating room-local roots
or receipts. A board, chat, inbox, digest, leaderboard, or replay is a
...
emit a transaction-shaped action/evidence record that securely binds. Where
the selected effect profile can produce an external consequence, it MUST also
preserve the two-sided commitment order: the durable intent commitment is
admitted before dispatch, and the durable outcome commitment is admitted only
after the declared external evidence and reconciliation state are available.
The outcome names the intent commitment as its predecessor.
...
\item
  The pre-effect intent commitment ref and hash, including the bounded
  authority and idempotency material required by the profile
\item
  The post-effect outcome commitment ref and hash, including evidence refs,
  terminal or reconciliation-required state, and the predecessor intent ref
\end{enumerate} \\
\end{longtable}
...
\hypertarget{implemented-governed-effect-profile}{%
\subsubsection{2.4.3 Implemented Governed-Effect
Profile}\label{implemented-governed-effect-profile}}

The August 2026 capstone demonstrates one bounded implementation of the
deterministic effect gate against a real Akash provider. The architectural
result is not that software can call a provider API. It is that the model was a
proposer rather than the authority, and that the external lifecycle crossed
distinct trust and state domains:

\begin{center}
\needspace{7\baselineskip}
\begin{tikzpicture}[scale=1,transform shape,node distance=5mm and 8mm]
\node[ioiplain,text width=39mm] (m) {\textbf{Probabilistic model}\\\footnotesize proposes a bounded operation};
\node[ioibox,right=of m,text width=46mm] (h1) {\textbf{Hypervisor}\\\footnotesize admits proposal and authority; commits intent};
\node[ioibox,right=of h1,text width=35mm] (p) {\textbf{Provider}\\\footnotesize effect, evidence, teardown, settlement};
\node[ioibox,below=10mm of h1,text width=52mm] (h2) {\textbf{Hypervisor / relying party}\\\footnotesize reconcile, commit outcome, verify, accept or refuse};
\draw[ioiflow](m)--(h1);
\draw[ioiflow](h1)--(p);
\draw[ioiflow](p)|-(h2);
\end{tikzpicture}
...
The retained provider lifecycle reached escrow, bid, lease, authenticated
workload-result retrieval, teardown, and terminal financial reconciliation with
zero open or unknown exposure. A workload-bound follow-on composed a fresh
KVM guest under the
\texttt{trusted\_host\_}\allowbreak\texttt{hostile\_guest/no-nic-v1} profile,
where the guest held only opaque request-bound authority and the named probes
could not obtain provider credentials or recovery material. A portable C8 v3
bundle subsequently passed a separately provisioned first-party verifier, and
the AFT measured-results registry admitted exactly one variance-caveated row.

This profile demonstrates logical policy mediation, workload-bound effect
isolation, scoped worker secret non-possession, authenticated result binding,
provider reconciliation, portable evidence, separate-binary verification, and
a relying-party state transition for one exact Akash provider. It does not
demonstrate provider neutrality, attested bare-metal placement, resistance to a
compromised host trusted computing base, reproduced-within-threshold benchmark
variance, third-party verification, or system-wide non-bypassability for every
possible host effect. Those are separate profiles and milestones.
...
HypervisorOS is the future-gated \textbf{node-root deployment} of the same
Hypervisor control plane. Workstation is a hosted controller and local-resource
profile; Infrastructure manages an admitted attached estate. Conventional
compute and governed-autonomy capabilities are selected independently of
either placement. Type 1 and Type 2 are external category mappings that require
their own operational proof; ``Type 3'' is qualified IOI shorthand for the
autonomy plane, not a peer or successor VMM standard. HypervisorOS may improve
control, integrity, containment,
...
Current served routes remain wallet.network-bound where a portable delegated
grant, secret lease, spend, declassification, cross-domain reuse, or high-risk
external effect is required. Provider replacement is a future released IOI
Authority Protocol target; no alternate served adapter or interop proof exists.
...
between distinct independently governed systems: marketplace worker systems,
outcome-service systems, robot-fleet systems, enterprise systems, and peer
autonomous systems. A local or installed Worker participates through AIIP only
when it acts behind its own independently governed System identity; same-system
GoalRuns, HarnessInvocations, installed Workers, member nodes, and embodied
units use native L0 assignment, lease, state/evidence, and Embodied Runtime
contracts instead. Each sovereign system retains its runtime, private context,
...
A transport address does not determine this boundary. \texttt{in\_process},
\texttt{daemon\_ipc}, \texttt{unix\_socket}, or \texttt{local\_http} may carry
AIIP only when the channel binds two distinct independently governed
\texttt{system\_id} values and each endpoint binds its own admitted channel-
enrollment decision and receipt. The same transports carry L0 when both
endpoints belong to one System. Bilateral channel enrollment is distinct from
optional IOI Network enrollment.
...
the work boundary. An \textbf{AIIPProfile} declares supported cross-system
protocol features, transports, receipt obligations, settlement modes,
authority planes, and payload classes. An \textbf{AIIPChannel} binds two
distinct independently governed and bilaterally enrolled System identities
with ordering, replay protection, schema version, privacy posture, lease refs,
and dispute windows. A local transport does not weaken that requirement;
same-System routing remains L0. A \textbf{BoundedExecutionDomain} declares its
manifest, capabilities, policy root, authority requirements, receipt schemas,
state boundary, runtime profile, settlement behavior, and supported AIIP
profiles when it acts behind such an independently governed System endpoint.
...
Current served portable-delegation and designated high-risk external-effect
routes remain wallet.network-bound. A future released IOI Authority Protocol
profile targets provider replacement; no alternate served adapter exists. The
installation path \textbf{MUST} present an intent
...
current served routes wallet.network-bound for portable delegated authority,
secret release, decryption, spend, declassification, or high-risk approval.
Provider replacement is a future released IOI Authority Protocol target with
no alternate served adapter. Agentgres records admitted state roots, receipts,
archive refs, restore
...
GoalRun create/start/reconcile path exists and remains partial. The current
hosted v2 OutcomeRoom slice admits one bounded-System-backed room, reciprocal
GoalRun membership, and a minimum runtime-derived WorkResult/OutcomeDelta graph
with durable payload/label custody and reconstructable proof projections. The
current v2 participant, frontier/claim, offer, Attempt, Finding, and
VerifierChallenge lifecycles are not started; mounted v1 predecessor planes are
historical executable/source-disposition code and are fenced from v2 rooms.
Wider role topology, generalized multi-harness and remaining orchestration,
federation, acceptance/verdict, and settlement remain planned. Multi-path
attempt comparison is partial.
Thread forks, replay,
...
\item
  \textbf{A5} the declared liveness environment and bounded dishonest
  weight. For the optimistic live path and seal cadence this is post-GST
  pairwise delivery within the bound between honest parties (a full honest
  mesh). For the hash-only asynchronous fallback it is a static adversary
  with (f<n/3), reliable private authenticated channels with eventual
  delivery, and private random inputs to the hash-based construction. These
  are separate profiles; neither inherits the other's timing or latency.
  No strong-lineage safety claim cites A5.
\item
...
\item
  ...
  Current served routes remain wallet.network-bound when portable delegated
  authority, secrets, spend, declassification, or high-risk external effects
  are involved. Provider replacement is a future released IOI Authority
...
\hypertarget{portable-certificates-and-relying-party-acceptance}{%
\subsubsection{6.2.3 Portable Certificates and Relying-Party
Acceptance}\label{portable-certificates-and-relying-party-acceptance}}

A certificate is evidence, not acceptance. A portable governed-effect bundle
therefore separates four transitions that must not be collapsed:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  the producer emits a certificate whose referenced objects are identified by
  their native ref and native hash;
\item
  a declared verifier evaluates the bundle under a closed schema, trust-root,
  mutation, and claim policy;
\item
  an accountable relying party applies its acceptance policy to the verified
  result; and
\item
  only that relying party may perform its declared state transition, recording
  before and after state hashes in an acceptance receipt.
\end{enumerate}

Verifier independence is multidimensional. Separate source, binary, code
generation path, transport, operator, organization, and trust roots are
different axes and MUST be reported rather than compressed into the word
``independent.'' A separately built verifier maintained by the same
organization is a \emph{first-party separate verifier}; it is not third-party
verification.

The first implemented relying-party profile is the AFT measured-results
registry. Its policy accepts only the declared certificate and result schemas,
environment and honesty classes, verdict set, trust roots, and registry
predecessor. In the retained August 2026 admission, the separate Rust verifier
accepted the portable C8 v3 evidence, rejected forty fully resealed semantic
mutations, and the registry advanced once from revision zero to revision one.
The producer could not self-declare acceptance inside its own certificate.
...
\textbf{wallet.network} is IOI's current first-party portable
delegated-authority layer for
...
decisions. Current IOI served routes use wallet.network when power must be portable,
...
credentials, approval gates, passkey-bound consent evidence, registered
standing authority envelopes, and bounded atomic draws are implemented for the
named development profiles. Guardian quorums, key shards, and a generalized MPC
vault remain planned; the profiles below are target architecture where those
facilities are not yet implemented.
...
current IOI served routes send portable delegated authority, secrets, spend,
declassification, and high-risk external effects through
\textbf{wallet.network}. Ordinary local product governance may remain with its
domain owner. A future released IOI Authority Protocol profile targets a
replaceable compatible authority-provider role; no alternate served adapter or
interoperability proof exists today.

\textbf{wallet.network} is IOI's canonical first-party authority wallet and
...
proposal and, on current IOI routes, requests a short-lived, scoped
cryptographic grant from \emph{wallet.network}.

Current IOI shipping posture requires wallet.network where work needs portable
delegated authority, secrets, spend, declassification, provider-trust
acceptance, cross-domain reuse, or high-risk external effects. Ordinary local
governance may remain in Hypervisor or its domain owner. wallet.network is
IOI's first-party portable identity-operation, secret, authority-scope,
approval, payment, and revocation layer for autonomous software; this product
posture is not a future protocol dependency.
...
\hypertarget{standing-authority-and-trajectory-admission}{%
\subsubsection{7.3.6 Standing Authority and Trajectory
Admission}\label{standing-authority-and-trajectory-admission}}

Exact-effect approval remains the default for consequential acts, but requiring
a fresh human signature for every low-risk repetition creates approval fatigue
and turns ceremony into an attack surface. A model need not forge a signature
if it can persuade an operator to approve the two-hundredth request that looks
like the previous forty.

The bounded unattended profile therefore uses a
\textbf{StandingAuthorityEnvelope}. The owner authenticates and consents to one
exact envelope---purpose, targets, operation families, per-effect and aggregate
spend, quantity, rate, time, result, evidence, revocation, and teardown
constraints. Each later effect consumes a single-use draw atomically with the
trajectory decision and durable intent commitment. A draw cannot widen the
envelope, auto-top-up it, reset consumed budget, or survive revocation.

Per-operation admissibility is not sufficient. A sequence of individually
allowed effects may compose into an aggregate outcome the owner never
authorized: distributed spend, many small deployments, chunked data egress, or
cross-provider accumulation. The authority trajectory state therefore projects
the relevant history at owner, System, principal, envelope-ancestor, provider,
destination, data, and semantic-action scopes required by policy. Admission may
allow, deny, or require step-up; semantic evidence may narrow or escalate but
never create or widen authority. Failed admission leaves authority and
trajectory state unchanged.

The implemented development profile composes passkey consent evidence with a
registered standing envelope and bounded atomic wallet draws. It does not yet
claim generalized behavioral anomaly detection, every cross-provider semantic
aggregation policy, guardian quorum, or protection against compromise of the
wallet/daemon trusted computing base.
...
wallet.network implements IOI's current served portable-delegation and
designated high-risk authority profile. In that shipping profile, Own becomes
Act only through scoped grants bound to
...
the applicable authority provider; current IOI served portable, delegated, and
designated high-risk scopes use wallet.network. \\
...
The open/protected split follows the trust boundary. The target \textbf{open,
independently operable L0} must include constitution/deployment/membership/
...
self-hostable first-party candidate path, and future conformance fixtures
required to operate and verify IOI-compatible systems without hosted IOI.
\textbf{Protected commercial
...
decisions, current first-party served routes use wallet.network for portable
delegated authority, secrets, spend, declassification, and high-risk grants,
and Agentgres records admitted
...
\hypertarget{approval-fatigue-and-trajectory-abuse}{%
\subsubsection{9.2.11 Approval Fatigue and Trajectory
Abuse}\label{approval-fatigue-and-trajectory-abuse}}

\textbf{Scenario:} A model does not steal a credential or forge authority. It
produces a long sequence of plausible requests until the approving human stops
distinguishing them, or it composes many individually admissible effects into a
forbidden aggregate outcome.

\textbf{Defense:} Authority classes separate exact effects, closed batches,
and standing envelopes. Standing authority carries aggregate budgets, rates,
targets, expiry, evidence, revocation, and trajectory policy; each draw is
atomic and single-use. Novel, anomalous, semantically cumulative, or
threshold-crossing paths require step-up or refuse. Interfaces summarize the
aggregate trajectory and delta from prior approvals rather than presenting
each request as an isolated ceremony. This reduces approval fatigue; it does
not make persuasion or semantic aggregation a solved problem.

\hypertarget{effect-boundary-bypass}{%
\subsubsection{9.2.12 Effect-Boundary Bypass and Trusted-Computing-Base
Compromise}\label{effect-boundary-bypass}}

\textbf{Scenario:} An untrusted workload routes around the mediated effect
interface using ambient network, filesystem, host-control, inherited
credential, or compromised-daemon access.

\textbf{Defense:} A conformance profile must name the control points made
unreachable by construction and the trusted computing base that enforces that
claim. The implemented hostile-guest profile removes guest network devices,
host mounts, and host-control sockets, gives the guest only an opaque
request-bound capability, and admits bounded output through quarantine. Other
provider, desktop, browser, and opaque external-harness profiles claim only the
hooks they actually mediate.

The daemon is part of the trusted computing base, as an operating-system kernel
is. The current evidence does not prove system-level non-bypassability against
a compromised host kernel, VMM, daemon, firmware, hardware, or every effect
channel. ``Syscall boundary'' is load-bearing only for a profile whose isolation
mechanism and adversarial tests make the named bypasses unreachable; elsewhere
it remains an architectural interface and a target conformance property.
...
First-party owners continue to act in their own domains: Hypervisor and the
daemon execute and emit runtime evidence; wallet.network owns identity, authority,
...
Local/domain governance owns ordinary local decisions; on current IOI served
routes, wallet.network implements portable delegated identity operations,
secrets, spend, declassification, high-risk approvals, and revocation-bearing
grants. Future released IOI Authority Protocol profiles target a replaceable
provider role.
...
\item
  ...
  environments, and debugging. Workbench is a retired label; the owner name is Developer Workspace.
\item
...
\texttt{HypervisorContextRouteResolver} has exactly one workspace or
application owner and resolves only admitted typed context to a canonical route
while preserving query, hash, return state, Open Application identity, and
typed context; it never accepts retired path spellings. An immutable
...
home_domain_ref: URI,
home_domain_binding_ref: URI,
home_domain_binding_root: Hash,
genesis_ref: URI,
package_id: URI,
constitution_ref: URI,
...
manifest_ref: URI,
admitted_manifest_root: Hash,
deployment_profile_ref: URI, // content-addressed revision
...
active_profile_set_ref: URI,
active_profile_set_root: Hash,
active_component_registry_ref: URI,
...
latest_sequence: u64,
latest_operation_commitment: Hash,
latest_transition_commitment_ref: Option<URI>,
latest_transition_id: URI,
latest_transition_root: Hash,
latest_receipt_ref: URI,
latest_state_ref: URI,
latest_state_root: Hash,
latest_receipt_root: Hash,
operation_log_ref: URI, // immutable revision under the canonical log owner
operation_log_root: Hash,
upgrade_policy_ref: URI, // inherited constitution policy; no upgrade effect
default_settlement_mode: Option<String>,
allowed_settlement_modes: Vec<String>,
status: String,
\end{lstlisting}

The first active chain proves constitutional and logical continuity, not
runtime readiness. Its canonical immutable operation-log head permanently
binds the sequence-zero, initialize, and activate prefix. Later protected
lifecycle operations advance that same owner through immutable successor
revisions whose entries bind their exact owner contracts and receipts; the log
does not validate later authority from shape alone. The chain binds the
constitution's ordinary-upgrade policy without performing an upgrade, while
node membership, writer epoch, runtime, network, settlement, and
public-commitment effects remain explicitly unadmitted until their owning
contracts supply proof.
...
current served portable-delegation and designated high-risk external-effect
routes remain wallet.network-bound, while provider replacement is a future
released IOI Authority Protocol target with no alternate served adapter; the
daemon admits, enforces, and executes;
...
\textbf{Current bounded implementation evidence (August 2026).} The strongest
retained governed-effect profile demonstrates: a model-originated but
daemon-admitted request; operator- and wallet-bound authority; a pre-effect
intent commitment; one real Akash provider lifecycle; authenticated workload
result retrieval; teardown and terminal financial reconciliation with zero
open or unknown exposure; a post-evidence outcome commitment; a fresh
hostile-guest KVM profile with scoped secret-non-possession probes; portable C8
v3 evidence; forty rejected resealed semantic mutations; and one
policy-admitted AFT registry transition performed by a separately provisioned
first-party verifier.

The corresponding profile labels are:

\begin{itemize}
\tightlist
\item \texttt{measured\_container};
\item \texttt{same\_provider\_container\_unknown\_host};
\item \texttt{variance\_caveated}; and
\item \texttt{trusted\_host\_hostile\_guest}.
\end{itemize}

The evidence is deliberately not
labeled provider-neutral, attested bare metal, compromised-host resistant,
reproduced within threshold, third-party verified, or universally
non-bypassable. Evidence hashes and campaign provenance live in the canonical
architecture evidence and work-item records rather than in this publishing
synthesis.
...
Internet of Intelligence Protocol • Whitepaper v1.12.0 • © 2026 IOI
Foundation

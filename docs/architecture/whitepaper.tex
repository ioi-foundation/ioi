...
{\large\itshape Web4 Infrastructure for Verifiable Bounded Agency}\\[1.6cm]
{\large
Date: June 2026\\[5pt]
Version: 1.5.43\\[5pt]
...
\vfill
{\footnotesize IOI Foundation \textperiodcentered{} Internet of Intelligence \textperiodcentered{} Technical Whitepaper v1.5.43}\\[0.4cm]
\end{titlepage}
...
or restore validity. \textbf{Hypervisor Canvas} is a visual
editor/projection inside Automations, Workbench, or Foundry where a graph
view is useful; Canvas state is layout and presentation, not runtime
truth. \textbf{ioi.ai Goal Chat} is the intent and coordination surface
that may ask, inspect, summarize, invoke existing work, or draft a
handoff, but durable automation/service deployment must move into
Hypervisor Automations and execution must cross Hypervisor Daemon/Core
contracts.
...
\item
  \textbf{Hypervisor Guard} names a developer-facing adapter posture for
  \textbf{IOI Authority Gateway} mediation. It is not a separate runtime,
  authority root, or total-control promise over opaque third-party tools.
\item
...
compatibility profiles, obtains
...
9.1 Daemon Effect Boundary (Legacy Agency Firewall Reference)
...
Older reference material calls this same bounded-effect discipline the
Agency Firewall. The boundary ensures that a compromised Execution Worker
cannot escape the virtualization boundary. When a worker requests an
action (e.g., an API call), \emph{wallet.network} performs brokered
secret execution or issues an operation-scoped short-lived token. The
worker process never sees the raw secret.
...
\textbf{daemon effect boundary}---historically called the Agency
Firewall---is the physical I/O boundary and deterministic transaction
gate that enforces this isolation. It ensures that an agent
...
obligations. \emph{ActionRules} is the legacy-compatible JSON DSL for
one such policy bundle. Policies are default-deny and support
...
normalization pipeline. For ActionRules compatibility profiles, the
runtime MUST pass the raw policy through the
...
any given ActionRules compatibility profile is mathematically locked to
this pipeline:
...
The EffectDecisionReceipt / FirewallDecisionReceipt legacy alias

The runtime MUST emit an \emph{EffectDecisionReceipt} for all governed
actions. \emph{FirewallDecisionReceipt} remains a legacy-compatible name
for the same receipt family. This receipt is the critical link proving
\emph{why} an action occurred. For consequential actions, it is the
...
authorization), and \textbf{EffectDecisionReceipt}
(\textbf{FirewallDecisionReceipt} as a legacy-compatible alias) (the
transaction record of the act --- a daemon- or
...
\emph{decentralized.cloud} is not present canon and not a mandatory
gateway in this path; it remains parked future product space for a
possible public provider catalog or P2P/PQ-aware cloud-routing layer.
...
Tier 1: Root Custody Profile (native root; historically "Master Custodian")
...
Tier 2: wallet.network Authority Core (historically "Manager")
...
\item
  ...
  \textbf{Agent Execution Account} (historically called an Ops Wallet).
\item
...
\hypertarget{daemon-effect-boundary-agency-firewall-reference}{%
\subsection{9.1 Daemon Effect Boundary (Legacy Agency Firewall
Reference)}\label{daemon-effect-boundary-agency-firewall-reference}}

\textbf{Hypervisor Daemon effect boundary} is the protocol term for this
bounded-effect discipline. Some reference material also uses
\textbf{Daemon Effect Boundary} for the same mechanism:
ActionRequest canonicalization, policy language, canonical ordering,
...
The transaction must include an \textbf{UpgradeJustification} artifact
(historically compatible with \emph{MutationJustification} receipts): a
...
adapter targets. \\
\textbf{Hypervisor Foundry} & Application surface & Worker, evaluation,
training, and model-route creation surface over the same Core and
Agentgres receipt model. \\
...
not as a separate Fleet product or truth layer. \\
...
commitment anchors. \emph{ioi.ai} is the
demand ingress and control plane for account/device/restore metadata.
...
interactions. \emph{decentralized.cloud} is parked future product space
for a possible public provider catalog or P2P/PQ-aware cloud routing
layer; current Hypervisor architecture uses direct provider integrations
instead of a mandatory cloud gateway.
...
training, and benchmarking application surface exposed inside
\textbf{Hypervisor App} and \textbf{Hypervisor Web} over the same
Hypervisor Core substrate.

The guided compilation flow for training a specialist worker utilizes
daemon-owned local or provider-compatible execution paths to generate
verifiable outcomes:
...
\subsubsection{2.6.2 ActionRules compatibility profile: policy
language}\label{actionrules-the-policy-language-1}}

\emph{ActionRules} is a legacy-compatible JSON DSL for daemon policy
...
\subsubsection{A.2 Effect-boundary artifacts (moved from original Section
...
EffectDecisionReceipt / FirewallDecisionReceipt legacy alias (transaction record of the act)
...
Core and daemon effect boundary formerly summarized as the Determinism
Boundary. They enforce a ``Zero Heuristics / Zero Hidden Fallbacks''
...
Internet of Intelligence Protocol • Whitepaper v1.5.43 • © 2026 IOI
Foundation

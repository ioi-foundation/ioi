...
| GoalRunActivation | durable object | The admitted, receipted, idempotent crossing that creates or joins `goal://` identity from exactly one typed source context (draft, Session, WorkRun, work item, room claim, automation step, or gateway adapter context). | A correlation id, pointer field, `origin_surface` tag, subscription, facilitator selection, or untyped `activation_evidence` payload. Pointers navigate; only admission activates. |
| Handoff | — | Always qualified. `ContextHandoff` (`handoff://`) is the intra-run cell-to-cell transfer; the AIIP `HandoffPacket`/`HandoffEnvelope` is the cross-sovereign transfer; `IoiAiGoalChatHandoff` is the ioi.ai outbound proposal; the crossing into goal identity is `GoalRunActivation`, deliberately not named a handoff. | A bare `Handoff` in new prose or a new schema; using one handoff family's semantics for another's boundary. |
...
- The orchestration application ships in the open first-party stack — this is a
  ...
  [`canon-to-code-delta.md`](../_meta/canon-to-code-delta.md) and the [`work-items/`](../_meta/work-items/)
  records (the former archived [`implementation-matrix.md`](../_meta/implementation-matrix.md)) before making
...
`Assistant` is a Session-backed faculty. Product code currently carries several
`Assistant*` types; each has a disposition, and none of them makes Assistant a
durable object.

| Current name | What it actually is | Canonical name |
| --- | --- | --- |
| `AssistantWorkbenchSession` | a UI context union, not an independently identified Session | `ContextualWorkbenchContext` |
| `AssistantNotificationRecord` | a durable **Notification**; the eight-state lifecycle belongs to the notification | `ContextualNotificationRecord` — **renamed**, owner written |
| `AssistantWorkbenchActivityRecord` | an activity/evidence record | `ContextualWorkbenchActivityRecord` |
| `AssistantAttentionPolicy`, `AssistantAttentionProfile`, `AssistantUserProfile` | configuration records | `ContextualAssistance*` |
...
**Obstruction resolved; a wider one is now measured.** The notification
contracts under `apps/hypervisor/src/generated/hypervisor-contracts/` carried the
ts-rs "do not edit this file manually" banner while no Rust type in this
repository derived them — orphaned artifacts with no owning source to rename
through. That owner now exists at
[`crates/api/src/contextual_notifications.rs`](../../../crates/api/src/contextual_notifications.rs),
and the rename ran through it: ts-rs regenerated the family, the previously
orphaned `AssistantNotification*` files were deleted, and the deprecated
spellings survive only as TypeScript aliases in owned product code. The
regenerated shared contracts (`ArtifactRef`, `ArtifactType`, `NotificationTarget`
and peers) are byte-identical in their type declarations to the orphans they
replaced, which is the evidence that the new Rust source is a faithful owner

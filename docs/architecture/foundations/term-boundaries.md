...
| `AssistantNotificationRecord` | a durable **Notification**; the eight-state lifecycle belongs to the notification | `ContextualNotificationRecord` (or `AttentionNotificationRecord`) |
...
**Known obstruction, recorded rather than worked around.** The
`AssistantNotificationClass`, `AssistantNotificationRecord`, and
`AssistantNotificationStatus` contracts under
`apps/hypervisor/src/generated/hypervisor-contracts/` carry a ts-rs
"do not edit this file manually" banner, and `.cargo/config.toml` points
`TS_RS_EXPORT_DIR` at that directory — but **no Rust type in this repository
derives them**. They are orphaned generated artifacts with no owning source to
rename through. Renaming them by hand would violate the rule above, so they are
left as-is and the missing owner is the defect to resolve first.

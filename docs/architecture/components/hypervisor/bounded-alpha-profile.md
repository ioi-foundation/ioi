...
| **THE ESSENTIAL JOURNEY UNDER THE SOVEREIGN-LOCAL FIXTURE (M12.1, `check:standalone-conformance`, 2026-09-15):** the same deployment + package + no-checkout journey (v1 `0.1.0-alpha.8` / v2 built from this tree by the recorded recipe, outside the repository) run INSIDE the isolated-egress harness — a nested user+network namespace holding only loopback, a seccomp-filtered `strace` ledger of every connect/sendto from every descendant process, DNS question names parsed from it, a unix-socket bridge for Ollama — 53/53 in 927 s with 405 recorded connection attempts, every one loopback (12 ports: daemon, served App, product UI, authority node and its TLS front, the backup verifier's daemons, the bridge), 0 resolver questions all for the journey's own `.invalid` inputs, the model reached only through the bridge (17 connections, 3208 bytes back); every IOI-managed family typed unavailable on the daemon's readiness projection (hosted wallet.network login `deployment_local_authority_bound`); the FAULT LANE (the model route severed while the run executed) terminating the run `failed` in 1 s with no artifact and no successful execute receipt (30/30); portable replay with 0 network attempts; a deployment pointed at a non-loopback host FAILING the profile `undeclared_egress`. Isolation `refused_and_recorded` | `npm run check:standalone-conformance` — evidence [`m12-1-standalone-conformance-2026-09-15.v1.json`](../../_meta/evidence/m12-1-standalone-conformance-2026-09-15.v1.json) | `77a370cb6` (25 dirty paths: this unit's own files, committed next) | **24/24** (15 CI-bound drills + the full run) |

...
declaration: `M13.11` met its bar on 2026-09-11 (ledger XXXVIII); `M13.9` is
built with its bar open on the provider's acceptance of the operator's
credential (ledger XXXIX); `M13.10` is not built.

| Widening | Unit | Qualification bar | Status |
| --- | --- | --- | --- |
| A remote frontier model route, selectable in the session composer beside the local one; the daemon's model-mount proxy performs the provider call with a credential sealed to the route record, so the harness environment stays secret-free | `M13.9` | the essential journey passes with the remote route selected; receipts name the route; the credential is never observable in the session environment; the process-environment key path stays refused by default; a live run is recorded with its cost, and without an operator credential the lane records a typed absence | **built, bar open** — `check:alpha-journey` remote lane 42/46 on 2026-09-11 (run 7; the four open clauses are the provider's acceptance and what depends on it: the run completing on the model, its artifacts, the intent's file) on `42007f60e`: every clause the estate controls is green (sealed key, daemon-proxied call receipted, run-scoped token, secret-free harness environment, env-key path refused); the provider answered 401 (invalid API key) to the operator's sealed key through the daemon's proxy on every run — 1 proxied call, 0 tokens, no cost; the same key probed directly answers 401 'Incorrect API key provided' — no live run is recorded with its cost, so this row does not move the table above (ledger XXXIX; evidence `_meta/evidence/u3-remote-frontier-route-2026-09-11.v1.json`) |

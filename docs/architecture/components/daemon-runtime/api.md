...
The OutcomeRoom, GoalRun, and step-resolution API catalog is owned by the
ioi.ai orchestration application and lives at
[`orchestration-api.md`](../../domains/ioi-ai/orchestration-api.md)
(moved 2026-09-07, ADR 0052 Decision 4). Those routes are application
handlers hosted in the daemon and admitted through the same effect boundary as
any other application; the daemon's own contract is the generic session,
thread, run, invocation, result, lifecycle, authority, storage, evidence and
...
OutcomeRoom v2, composed in `apps/ioi-ai/orchestration` — and the ioi.ai
...
An orchestration over these records is COMPOSED in the application layer, not served by the daemon (R-172 S2, ADR 0022/0030/0031/0034): the agent SDK's `Orchestration` composer (`packages/agent-sdk/src/orchestration.ts`) creates a coordinating thread through `POST /v1/threads`, delegates work as subagents of it through `POST /v1/threads/{thread_id}/subagents`, claims capacity through the work-lifecycle reservation route, admits its typed records through this seam with the application's own scope as `parent_scope_ref`, and projects a node graph from those reads. The daemon registers no route, object or roster for the composition; `GET /v1/threads/{thread_id}/subagents` serves the bare array of subagent records, and the SDK normalizes it into its published list envelope as its own read model. Each revision served by `GET …/records/{contract}/{object}` carries its `receipt_ref` and `operation_ref`, derived from the projection's own coordinates exactly as the admit reply derives them, so a reader that re-attaches after a restart can cite the receipt of an earlier admission.

Collaboration is composed the same way, one layer up (R-172 S3, M11.1, ACC-13 clauses 1–3 and N1): the ioi.ai application's `Collaboration` composer (`apps/ioi-ai/orchestration/src/collaboration.ts`) admits `CollaborationTerms` v3, `OrchestrationDiscovery` v1, `OrchestrationParticipationRequest` v1 and `ParticipantStateBundle` v4 through this seam under the host System. Activation is derived from exact-root acceptances; a discovery publication is refused by the composer until the terms are active; an external party's request is verified against the key that party accepted the terms with and decided as a successor revision citing the submission's receipt; a request from inside the host's own `system_id` is refused (work inside the System is a delegation); the bundle is least-disclosing and verifies offline. Portable exit is a third revision of the participation citing its bundle, with the status staying `accepted` (M11.2, R-175); federated admission is a declared mode whose decision the adjudicator party of record co-signs. The work objects are composed the same way (R-177 S4b): WorkClaim, Attempt, Finding, VerifierChallenge, ResourceOffer and CapabilityOffer v4 are admitted through this seam by the composer's `Work`, with the orchestration as parent scope, an actor that is an accepted participation or a delegation of the coordinating thread, and frozen coordinates whose control hashes are the bound records' seam-derived payload roots. The daemon serves none of it: the goal-orchestration routes for collaboration terms, room participation requests and the v3 work objects are R-172 migration input awaiting retirement.

...
Collaboration is composed the same way, one layer up (R-172 S3, M11.1, ACC-13 clauses 1–3 and N1): the ioi.ai application's `Collaboration` composer (`packages/ioi-ai-orchestration/src/collaboration.ts`) admits `CollaborationTerms` v3, `OrchestrationDiscovery` v1, `OrchestrationParticipationRequest` v1 and `ParticipantStateBundle` v4 through this seam under the host System. Activation is derived from exact-root acceptances; a discovery publication is refused by the composer until the terms are active; an external party's request is verified against the key that party accepted the terms with and decided as a successor revision citing the submission's receipt; a request from inside the host's own `system_id` is refused (work inside the System is a delegation); the bundle is least-disclosing and verifies offline. The daemon serves none of it: the goal-orchestration routes for collaboration terms and room participation requests are R-172 migration input awaiting retirement.

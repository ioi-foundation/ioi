...
semantics plus the Default Harness Profile reference scaffold/fallback —
harness selection, invocation, tool boundaries, model configuration, lifecycle
and evidence for every session, with or without an application present. It
consumes the topology, context, handoff, invocation, and result contracts owned
by the common-object canon; the application-supplied inputs (the ioi.ai
GoalRunProfile, GoalRun, Goal Kernel pursuit policy, and room context) are
owned by
[`harness-application-profile.md`](../../domains/ioi-ai/harness-application-profile.md)
...
## Application Admission Paths

Application-specific admission decisions that select how a harness invocation
is admitted — the ioi.ai GoalRun admission-path decision, its pursuit policy,
and the room context of a claimed subgoal — are owned by the application:
[`harness-application-profile.md`](../../domains/ioi-ai/harness-application-profile.md)
(moved 2026-09-07, ADR 0052 Decision 4). This profile owns harness selection,
invocation, tool boundaries, model configuration, lifecycle and evidence for
every session; the same qualified harness executes a direct session without

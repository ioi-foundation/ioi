...
cleanup:
  legacy_paths_removed: true
  compatibility_shims_remaining:
    - model-instance state still carries `providerLifecycleHash` until the
      instance-planning state field can be canonicalized safely; this slice
      removes duplicate provider lifecycle receipt metadata only
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
Current expected behavior after Slice 131:

...
Last matrix compaction pass: 2026-06-07, after IDE diagnostics repair action alias retirement slice 647 clarified the active seam.
...
## Implementation Slice 640

```yaml
slice: 640
phase: 10-authoritative-js-facade-retirement
objective: retire signed replay notebook event-payload scalar aliases so raw
  runtime payloads no longer create replay cells from camelCase snapshot,
  tool-call, file-count, approval, or changed-file fields
owner_boundary:
  route_or_surface: Hypervisor IDE signed replay notebook event payload parsing
  authority_gate: unchanged; this slice changes replay projection payload
    parsing only and does not grant authority or mutate accepted truth
  execution_backend: unchanged; signed replay notebook construction remains a
    product/IDE projection over daemon events
  truth_path: signed replay event payloads now use canonical `snapshot_id`,
    `tool_call_id`, `file_count`, `approval_required`, `approval_satisfied`, and
    `changed_files` fields only before replay cells are created
  projection_path: compositor conformance rejects signed replay payload fallback
    readers for retired `snapshotId`, `toolCallId`, `fileCount`,
    `approvalRequired`, and `changedFiles`
touched_files:
  docs:
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
  ide_projection:
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires canonical signed replay payload readers for
    snapshot identity, tool-call identity, file count, approval flags, and
    changed-file paths
  - compositor conformance rejects reintroducing retired camelCase payload
    fallback readers for the same fields
  - focused signed replay notebook tests prove poisoned retired payload aliases
    do not create replay cells while canonical payload fields still do
verification:
  commands:
    - npm run build --workspace=@ioi/agent-ide
    - node --check scripts/conformance/hypervisor-conformance.mjs
...
cleanup:
  ...
  compatibility_shims_remaining:
    - terminal Rust daemon-core API extraction remains pending beyond this
      IDE replay payload facade cleanup
    - signed replay builder still accepts complete camelCase projected IDE event
      objects through `WorkflowRuntimeThreadEventLike` until the broader IDE
      event-projection contract is migrated
closeout:
...
## Implementation Slice 641

```yaml
slice: 641
phase: 10-authoritative-js-facade-retirement
objective: retire worker contribution trace result-payload aliases so worker
  contribution evidence no longer derives snapshot, changed-file, or edit-count
  fields from duplicate camelCase result payloads
owner_boundary:
  route_or_surface: Hypervisor IDE worker contribution trace result payload
    parsing
  authority_gate: unchanged; this slice changes IDE projection evidence parsing
    only and does not grant authority or mutate accepted truth
  execution_backend: unchanged; worker contribution trace construction remains a
    product/IDE projection over daemon events and admitted contribution records
  truth_path: worker contribution rows now derive result snapshot ids,
    changed-file paths, and edit counts from canonical `workspace_snapshot_id`,
    `changed_files`, and `edit_count` fields only
  projection_path: compositor conformance rejects worker contribution fallback
    readers for retired `workspaceSnapshotId`, `changedFiles`, and `editCount`
touched_files:
  docs:
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
  ide_projection:
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires worker contribution trace result readers to
    use canonical `workspace_snapshot_id`, `changed_files`, and `edit_count`
  - compositor conformance rejects reintroducing retired camelCase result
    payload fallback readers for the same fields
  - focused worker contribution tests prove poisoned retired result payload
    aliases do not populate snapshot/file/edit evidence while canonical result
    payload fields still do
verification:
  commands:
    - npm run build --workspace=@ioi/agent-ide
    - node --check scripts/conformance/hypervisor-conformance.mjs
...
cleanup:
  ...
  compatibility_shims_remaining:
    - terminal Rust daemon-core API extraction remains pending beyond this
      worker contribution trace facade cleanup
    - worker contribution request/worker-object camelCase helper readers were
      retired by Slice 642; broader IDE event/projection contract migration
      remains pending outside this result-payload slice
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
## Implementation Slice 642

```yaml
slice: 642
phase: 10-authoritative-js-facade-retirement
objective: retire worker contribution trace request and worker-object aliases so
  worker contribution rows no longer bind identity, hunk, edit-count, or worker
  metadata from duplicate camelCase input shapes
owner_boundary:
  route_or_surface: Hypervisor IDE worker contribution trace contribution and
    worker metadata parsing
  authority_gate: unchanged; this slice keeps the IDE trace as projection-only
    evidence and does not grant authority or mutate accepted truth
  execution_backend: unchanged; worker contribution trace construction remains a
    product/IDE projection over daemon events and admitted contribution records
  truth_path: worker contribution rows now derive contribution identity,
    subagent/tool/event ids, hunk metadata, edit counts, and worker metadata
    from canonical `contribution_id`, `subagent_id`, `tool_call_id`,
    `event_id`, `file_path`, `hunk_file`, `hunk_index`, `hunk_header`,
    `edit_count`, `child_thread_id`, `parent_thread_id`, `merge_policy`, and
    `output_contract_status` fields only
  projection_path: compositor conformance rejects worker contribution fallback
    readers for retired `contributionId`, `subagentId`, `toolCallId`,
    `eventId`, `filePath`, `hunkFile`, `hunkIndex`, `hunkHeader`, `editCount`,
    `childThreadId`, `parentThreadId`, `mergePolicy`, and
...
touched_files:
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires worker contribution trace request and worker
    readers to use canonical snake_case fields only
  - compositor conformance rejects reintroducing retired camelCase request and
    worker-object fallback readers for the same fields
  - focused worker contribution tests prove poisoned retired request and worker
    aliases do not populate identity, hunk/edit, worker metadata, or evidence
    while canonical fields still do
verification:
  commands:
    - npm run build --workspace=@ioi/agent-ide
    - node --check scripts/conformance/hypervisor-conformance.mjs
...
cleanup:
  ...
  compatibility_shims_remaining:
    - terminal Rust daemon-core API extraction remains pending beyond this
      worker contribution trace facade cleanup
    - broader `WorkflowRuntimeThreadEventLike` projected-event compatibility
      remains pending outside this worker contribution request/worker slice
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
## Implementation Slice 643

```yaml
slice: 643
phase: 10-authoritative-js-facade-retirement
objective: retire workspace trust gate payload aliases so warning and
  acknowledgement readiness cannot be satisfied through duplicate camelCase
  workflow-node, warning, or source-event payload fields
owner_boundary:
  route_or_surface: Hypervisor IDE workspace trust gate readiness projection
  authority_gate: unchanged; daemon workspace-trust receipts remain the required
    evidence path and this slice only removes retired IDE payload fallbacks
  execution_backend: unchanged; workspace trust readiness remains an IDE
    projection over daemon runtime events and receipt refs
  truth_path: workspace trust warning and acknowledgement matching now consumes
    canonical `workflow_node_id`, `warning_id`, and `source_event_id` payload
    fields only, with typed projected event `workflowNodeId` still accepted as
    the IDE event projection shape
  projection_path: compositor conformance rejects workspace trust fallback
    readers for retired payload `workflowNodeId`, `warningId`, and
    `sourceEventId`
touched_files:
  docs:
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires workspace trust gate payload readers to use
    canonical `workflow_node_id`, `warning_id`, and `source_event_id`
  - compositor conformance rejects reintroducing retired camelCase payload
    fallback readers for the same fields
  - focused workspace trust tests prove poisoned retired payload aliases cannot
    emit a warning match or satisfy an acknowledgement while canonical payload
    fields still do
verification:
  commands:
    - npm run build --workspace=@ioi/agent-ide
...
cleanup:
  ...
  compatibility_shims_remaining:
    - terminal Rust daemon-core API extraction remains pending beyond this IDE
      workspace trust projection cleanup
    - typed projected IDE event fields remain product projection shape; broader
      Rust projection API extraction remains pending
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
## Implementation Slice 644

```yaml
slice: 644
phase: 10-authoritative-js-facade-retirement
objective: retire IDE receipt gate panel aliases so route, model, backend,
  required-tool-receipt, receipt-id, and evidence rows cannot be populated from
  duplicate camelCase gate/detail shapes
owner_boundary:
  route_or_surface: Hypervisor IDE receipt gate panel projection
  authority_gate: unchanged; Rust/model-mount receipt-gate validation remains
    the authoritative route/detail gate and this slice only removes retired IDE
    projection fallbacks
  execution_backend: unchanged; receipt gate panel remains an IDE projection
    over admitted receipt gate records
  truth_path: receipt gate rows now consume canonical `gate_receipt`,
    `blocked_receipt`, `receipt_id`, `gate_receipt_id`, `route_id`,
    `selected_model`, `endpoint_id`, `backend_id`, `selected_backend`,
    `required_tool_receipt_ids`, `tool_receipt_ids`, and `evidence_refs` fields
    only
  projection_path: compositor conformance rejects retired `gateReceipt`,
    `blockedReceipt`, `receiptId`, `gateReceiptId`, `routeId`,
    `selectedModel`, `endpointId`, `backendId`, `selectedBackend`,
    `requiredToolReceiptIds`, `toolReceiptIds`, and `evidenceRefs` fallback
    readers
touched_files:
  docs:
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires the IDE receipt gate panel to read canonical
    snake_case receipt/detail/evidence fields
  - compositor conformance rejects reintroducing retired camelCase object,
    scalar, collection, route/detail, and evidence fallback readers
  - focused receipt gate panel tests prove poisoned retired aliases do not
    populate receipt ids, route/model/backend details, tool receipt refs, or
    evidence refs while canonical fields still do
verification:
  commands:
    - npm run build --workspace=@ioi/agent-ide
    - node --import tsx --test packages/agent-ide/src/runtime/workflow-receipt-gate-panel.test.ts
    - node --check scripts/conformance/hypervisor-conformance.mjs
    - npm run hypervisor-conformance:compositor
    - npm run hypervisor-conformance:docs
    - npm run hypervisor-conformance
...
cleanup:
  ...
  compatibility_shims_remaining:
    - terminal Rust daemon-core API extraction remains pending beyond this IDE
      receipt gate projection cleanup
    - broader Rust projection API extraction remains pending
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
## Implementation Slice 645

```yaml
slice: 645
phase: 10-authoritative-js-facade-retirement
objective: retire IDE runtime policy stack and edit-proposal policy aliases so
  approval, warning, proposal, target, source-event, retry, and mutation policy
  state cannot be populated from duplicate camelCase payload shapes
owner_boundary:
  route_or_surface: Hypervisor IDE runtime policy stack and workflow edit
    proposal policy projections
  authority_gate: unchanged; Rust/daemon approval and workflow-edit admission
    remain authoritative, and this slice removes retired IDE projection readers
    that could imply a second identity or satisfaction path
  execution_backend: unchanged; these policy stacks remain IDE projections over
    admitted runtime/projection events
  truth_path: policy rows now consume canonical `approval_id`, `warning_id`,
    `source_event_id`, `approval_decision_event_id`, `approval_satisfied`,
    `proposal_id`, `proposal_event_id`, `target_workflow_node_ids`,
    `bounded_targets`, and `mutation_executed` fields only
  projection_path: compositor conformance rejects retired `approvalId`,
    `warningId`, `sourceEventId`, `approvalDecisionEventId`,
    `approvalSatisfied`, `proposalId`, `proposalEventId`,
    `targetWorkflowNodeIds`, `boundedTargets`, and `mutationExecuted` fallback
    readers
touched_files:
  docs:
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires runtime policy stack readers to use
    canonical approval, warning, source-event, retry, and satisfaction fields
  - compositor conformance requires edit-proposal policy readers to use
    canonical proposal, approval, target, source-event, and mutation fields
  - focused tests prove poisoned retired aliases do not satisfy policy warning
    acknowledgement, approval retry, proposal identity, target, or mutation
    state while canonical fields still do
verification:
  commands:
    - node --import tsx --test packages/agent-ide/src/runtime/workflow-runtime-policy-stack.test.ts
    - node --import tsx --test packages/agent-ide/src/runtime/workflow-runtime-edit-proposal-policy.test.ts
    - node --check scripts/conformance/hypervisor-conformance.mjs
    - npm run hypervisor-conformance:compositor
    - npm run build --workspace=@ioi/agent-ide
    - npm run hypervisor-conformance:docs
...
## Implementation Slice 646

```yaml
slice: 646
phase: 10-authoritative-js-facade-retirement
objective: retire IDE workflow edit proposal control aliases so daemon-bound
  proposal/apply request bodies no longer emit duplicate camelCase fields and
  raw input alias-only values cannot populate workflow-edit dispatch state
owner_boundary:
  route_or_surface: Hypervisor IDE workflow edit proposal control nodes
  authority_gate: unchanged; Rust/daemon workflow-edit approval and admission
    remain authoritative, and this slice narrows the IDE control adapter to
    canonical daemon request fields
  execution_backend: unchanged; control nodes still create product requests for
    daemon workflow-edit routes rather than executing mutations locally
  truth_path: workflow-edit proposal/apply requests now emit canonical
    `event_kind`, `component_kind`, `payload_schema_version`,
    `workflow_graph_id`, `workflow_node_id`, `proposal_id`,
    `target_workflow_node_ids`, `bounded_targets`, `workflow_path`,
    `workflow_patch`, `code_diff`, `approval_mode`, `proposal_only`,
    `mutation_allowed`, `mutation_executed`, and `approval_id` fields only
  projection_path: compositor conformance rejects duplicate `eventKind`,
    `componentKind`, `payloadSchemaVersion`, `workflowGraphId`,
    `workflowNodeId`, `proposalId`, `targetWorkflowNodeIds`,
    `boundedTargets`, `workflowPath`, `workflowPatch`, `codeDiff`,
    `approvalMode`, `proposalOnly`, `mutationAllowed`, `mutationExecuted`, and
    `approvalId` request-body fields, and rejects raw input fallback readers for
    the same retired aliases
touched_files:
  docs:
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
  ide_projection:
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires workflow-edit proposal/apply control request
    body interfaces and emitted bodies to use canonical snake_case daemon fields
  - compositor conformance rejects reintroducing retired camelCase raw input
    defaults for proposal identity, targets, workflow path/patch, diff, and
    approval identity
  - focused tests prove retired raw input aliases are ignored while canonical
    raw input fields still populate request state
verification:
  commands:
    - node --import tsx --test packages/agent-ide/src/runtime/workflow-runtime-edit-proposal-control-nodes.test.ts
    - node --check scripts/conformance/hypervisor-conformance.mjs
    - npm run hypervisor-conformance:compositor
    - npm run build --workspace=@ioi/agent-ide
    - npm run hypervisor-conformance:docs
...
cleanup:
  legacy_paths_removed: true
  compatibility_shims_remaining:
    - internal React Flow node config still has product-model camelCase fields;
      terminal Rust daemon-core/API extraction remains pending beyond this
      daemon-bound request-body cleanup
    - broader IDE control-node request bodies remain an ongoing facade
      retirement lane outside this workflow-edit proposal slice
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
## Implementation Slice 647

```yaml
slice: 647
phase: 10-authoritative-js-facade-retirement
objective: retire IDE diagnostics repair action aliases so repair policy and
  decision descriptors no longer accept camelCase compatibility fields before
  workflow-addressable repair actions are projected
owner_boundary:
  route_or_surface: Hypervisor IDE diagnostics repair action descriptor helper
  authority_gate: unchanged; diagnostics repair execution still requires the
    daemon/Rust-authorized repair route, while this slice prevents retired
    policy/decision aliases from shaping the IDE repair action surface
  execution_backend: unchanged; IDE descriptors remain non-authoritative
    projection/action metadata and do not execute repairs directly
  truth_path: diagnostic repair descriptors now read canonical `decision_refs`,
    `policy_id`, `decision_id`, `restore_conflict_policy`,
    `requires_approval`, `allow_conflicts`, `override_conflicts`,
    `workflow_node_id`, `thread_id`, `workflow_graph_id`, `rollback_refs`,
    `workspace_snapshot_refs`, `repair_policy`, `diagnostics_repair_policy`,
    `diagnostics_repair_context`, and `repair_decisions` fields only
  projection_path: compositor conformance rejects reintroducing retired
    `decisionRefs`, `policyId`, `decisionId`, `restoreConflictPolicy`,
    `requiresApproval`, `allowConflicts`, `overrideConflicts`,
    `workflowNodeId`, `threadId`, `workflowGraphId`, `rollbackRefs`,
    `workspaceSnapshotRefs`, `repairPolicy`, `diagnosticsRepairPolicy`,
    `diagnosticsRepairContext`, and `repairDecisions` helper fallbacks
touched_files:
  docs:
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
  ide_projection:
  ...
  tests:
    ...
    - scripts/conformance/hypervisor-conformance.mjs
conformance_checks:
  - compositor conformance requires diagnostics repair action readers to use
    canonical snake_case repair policy, decision, routing, approval, graph, and
    evidence fields
  - compositor conformance rejects the old two-key helper fallback shape that
    could let retired camelCase repair policy fields populate runnable action
    descriptors
  - focused tests prove alias-only policy/decision fields and alias-only
    container names cannot populate repair action identity, approval/conflict
    status, routing, graph binding, rollback refs, or snapshot refs
verification:
  commands:
    - node --import tsx --test packages/agent-ide/src/runtime/workflow-runtime-diagnostics-repair-actions.test.ts
    - node --check scripts/conformance/hypervisor-conformance.mjs
    - npm run hypervisor-conformance:compositor
    - npm run build --workspace=@ioi/agent-ide
    - npm run hypervisor-conformance:docs
...
cleanup:
  legacy_paths_removed: true
  compatibility_shims_remaining:
    - workflow event projection still has broader product/API compatibility
      surfaces; terminal Rust projection ownership remains pending outside this
      diagnostics repair action descriptor cleanup
    - IDE repair control nodes still use product-model camelCase config fields;
      this slice only retires daemon/projection payload alias fallbacks
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
Current expected behavior after Slice 647 and the eighty-second 2026-06-07 matrix compaction pass:
...
- Slices 572-644 retired SDK runtime usage record aliases and Rust/daemon
  ...
  checklist projection identity; runtime turn projection now derives

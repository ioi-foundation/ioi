...
Slice 192 adds compositor proof for
`runtime-subagent-propagation-request-aliases-retired`: runtime subagent
cancellation propagation reads canonical `workflow_node_id` only and does not
read or forward the retired `workflowNodeId` request alias to child cancellation.

Slice 193 adds compositor proof for
`agent-sdk-subagent-budget-request-type-alias-retired`: the SDK subagent control
request type exposes canonical `budget` and no longer advertises the retired
`subagentBudget` alias.
...
control request type exposes canonical `tool_pack` and no longer advertises the
retired `toolPack`/`subagentToolPack` aliases.

Slice 196 adds compositor proof for
`agent-sdk-subagent-model-route-request-type-aliases-retired`: the SDK subagent
control request type exposes canonical `model_route_id` and no longer
advertises the retired `modelRouteId`/`subagentModelRoute` aliases.

Slice 197 adds compositor proof for
`agent-sdk-subagent-concurrency-request-type-aliases-retired`: the SDK subagent
control request type exposes canonical `max_concurrency` and no longer
advertises the retired `maxConcurrency`/`subagentMaxConcurrency` aliases.

Slice 198 adds compositor proof for
`agent-sdk-subagent-output-contract-request-type-aliases-retired`: the SDK
subagent control request type exposes canonical `output_contract` and no longer
advertises the retired `outputContract`/`subagentOutputContract` aliases.

Slice 199 adds compositor proof for
`agent-sdk-subagent-policy-request-type-aliases-retired`: the SDK subagent
control request type exposes canonical `merge_policy`/`cancellation_inheritance`
and no longer advertises the retired `mergePolicy`/`cancellationInheritance`
aliases.

Slice 200 adds compositor proof for
`agent-sdk-subagent-cancellation-metadata-request-type-aliases-retired`: the SDK
subagent control request type exposes canonical cancellation metadata and no
longer advertises retired `cancellationReason`, `cancellationInherited`, or
`propagatedFromThreadId` aliases.

Slice 201 adds compositor proof for
`agent-sdk-subagent-context-routing-request-type-aliases-retired`: the SDK
subagent control request type exposes canonical context routing fields and no
longer advertises retired `forkContext`, `parentTurnId`, `turnId`, or
`targetAgentId` aliases.

Slice 202 adds compositor proof for
`agent-sdk-subagent-workflow-request-type-aliases-retired`: the SDK subagent
control request type exposes canonical workflow/idempotency fields and no longer
advertises retired `workflowGraphId`, `workflowNodeId`, or `idempotencyKey`
aliases.

Slice 203 adds compositor proof for
`agent-sdk-subagent-request-type-escape-hatches-retired`: the SDK subagent
control/list request input types no longer retain arbitrary-key escape hatches
that could silently re-advertise retired request aliases.

Slice 204 adds compositor proof for
`agent-sdk-subagent-record-identity-output-aliases-retired`: the SDK subagent
record type exposes canonical identity/provenance fields and no longer
advertises duplicate camelCase identity aliases.

Slice 205 adds compositor proof for
`agent-sdk-subagent-record-route-output-aliases-retired`: the SDK subagent record
type exposes canonical route/workflow fields and no longer advertises duplicate
camelCase route/workflow aliases.

Slice 206 adds compositor proof for
`agent-sdk-subagent-record-lifecycle-output-aliases-retired`: the SDK subagent
record type exposes canonical lifecycle/count fields and no longer advertises
duplicate camelCase lifecycle/count aliases.

Slice 207 adds compositor proof for
`agent-sdk-subagent-record-cancellation-output-aliases-retired`: the SDK
subagent record type exposes canonical cancellation metadata fields and no
longer advertises duplicate camelCase cancellation aliases.

Slice 208 adds compositor proof for
`agent-sdk-subagent-record-status-output-aliases-retired`: the SDK subagent
record type exposes canonical output and budget status fields and no longer
advertises duplicate camelCase status aliases.

Slice 209 adds compositor proof for
`agent-sdk-subagent-record-telemetry-estimate-output-aliases-retired`: the SDK
subagent record type exposes canonical telemetry, cost, and token estimate
fields and no longer advertises duplicate camelCase telemetry/estimate aliases.

Slice 210 adds compositor proof for
`agent-sdk-subagent-record-ref-output-aliases-retired`: the SDK subagent record
type exposes canonical receipt/evidence ref fields and no longer advertises
duplicate camelCase ref aliases.

Slice 211 adds compositor proof for
`agent-sdk-subagent-record-timestamp-output-aliases-retired`: the SDK subagent
record type exposes canonical timestamp fields and no longer advertises
duplicate camelCase timestamp aliases.

Slice 212 adds compositor proof for
`agent-sdk-subagent-record-escape-hatch-retired`: the SDK subagent record type
no longer retains an arbitrary-key escape hatch beside the canonical record
shape.

Slice 213 adds compositor proof for
`agent-sdk-subagent-list-result-aliases-retired`: the SDK subagent list result
type exposes canonical list envelope fields and no longer advertises duplicate
camelCase aliases or an arbitrary-key escape hatch.

Slice 214 adds compositor proof for
`agent-sdk-subagent-result-identity-lifecycle-aliases-retired`: the SDK
subagent result type exposes canonical identity/lifecycle fields and no longer
advertises duplicate camelCase identity/lifecycle aliases.

Slice 215 adds compositor proof for
`agent-sdk-subagent-result-status-telemetry-aliases-retired`: the SDK subagent
result type exposes canonical status/telemetry fields and no longer advertises
duplicate camelCase status/telemetry aliases.

Slice 216 adds compositor proof for
`agent-sdk-subagent-result-ref-escape-aliases-retired`: the SDK subagent result
type exposes canonical receipt refs and no longer advertises duplicate
camelCase ref aliases or an arbitrary-key escape hatch.

Slice 217 adds compositor proof for
`agent-sdk-subagent-cancellation-propagation-result-scalar-aliases-retired`: the
SDK cancellation propagation result type exposes canonical identity, policy, and
count fields and no longer advertises duplicate camelCase scalar aliases.

Slice 218 adds compositor proof for
`agent-sdk-subagent-cancellation-propagation-result-collection-aliases-retired`:
the SDK cancellation propagation result type exposes canonical collection/ref
fields and no longer advertises duplicate camelCase collection/ref aliases or an
arbitrary-key escape hatch.

Slice 219 adds compositor proof for
`agent-sdk-runtime-usage-telemetry-identity-route-aliases-retired`: the SDK
runtime usage telemetry type exposes canonical identity/route fields and no
longer advertises duplicate camelCase identity/route aliases.

Slice 220 adds compositor proof for
`agent-sdk-runtime-usage-telemetry-scalar-metric-aliases-retired`: the SDK
runtime usage telemetry type exposes canonical token, cost, and latency fields
and no longer advertises duplicate camelCase scalar metric aliases.

Slice 221 adds compositor proof for
`agent-sdk-runtime-usage-telemetry-context-source-aliases-retired`: the SDK
...
Slice 223 adds compositor proof for
...
Slice 227 adds compositor proof for
`agent-sdk-subagent-usage-telemetry-aliases-retired`: the SDK nested subagent
...
Slice 231 extends compositor proof for
...
Slice 237 extends bridge proof for
`model-mount-continuation-lineage-aliases-retired`: model-mount continuation
safety metadata, invocation/stream accepted receipt lineage, conversation-state
lineage, schema relations, and SDK model-mount types now expose canonical
`previous_response_id`, `root_response_id`, `fallback_allowed`, and
`mismatch_fields` fields without duplicate camelCase aliases.

Slice 238 extends bridge proof for
`model-mount-route-decision-fallback-aliases-retired`: accepted
`model_route_decision` fallback metadata and the SDK `ModelRouteDecision` type
now expose canonical `fallback_allowed`, `fallback_triggered`, and
`fallback_reason` fields without duplicate camelCase aliases.

Slice 239 extends bridge proof for
`model-mount-route-decision-lineage-aliases-retired`: accepted
`model_route_decision` lineage metadata, route-selection receipt lineage
details, and the SDK `ModelRouteDecision` type now expose canonical
`response_id` and `previous_response_id` fields without duplicate camelCase
aliases.

Slice 240 extends bridge proof for
`model-mount-route-decision-identity-aliases-retired`: accepted
`model_route_decision` identity metadata, route-selection receipt binding, SDK
types, runtime projections, CLI display, and Rust Agentgres materialization now
use canonical `schema_version`, `event_kind`, and `decision_id` fields without
duplicate camelCase aliases.

Slice 241 extends bridge proof for
`model-mount-route-decision-route-model-aliases-retired`: accepted
`model_route_decision` route, requested/selected model, provider, and rejected
candidate identity metadata and the SDK `ModelRouteDecision` type now expose
canonical snake_case fields without duplicate camelCase aliases.

Slice 242 extends bridge proof for
`model-mount-route-decision-descriptor-aliases-retired`: accepted
`model_route_decision` reasoning, placement, privacy, cost, and fallback
descriptor metadata, the SDK `ModelRouteDecision` type, and direct thread
projection readers now use canonical snake_case fields without duplicate
camelCase aliases.

Slice 243 extends bridge proof for
`model-mount-route-decision-policy-evaluation-aliases-retired`: accepted
`model_route_decision` policy constraint and candidate evaluation metadata,
including nested policy constraint keys and the SDK `ModelRouteDecision` type,
now use canonical snake_case fields without duplicate camelCase aliases.

Slice 244 extends bridge proof for
`model-mount-route-decision-descriptor-aliases-retired`: the Rust CLI TUI
mode-status reader now reads nested `model_route_decision.reasoning_effort`
canonically and no longer keeps the retired `reasoningEffort` compatibility
fallback.

Slice 245 extends bridge proof for
`model-mount-route-decision-workflow-aliases-retired`: accepted
`model_route_decision` workflow graph/node metadata and the SDK
`ModelRouteDecision` type now use canonical snake_case fields without duplicate
camelCase aliases.

Slice 246 extends bridge proof for
`runtime-thread-model-route-decision-reader-aliases-retired`: thread runtime
control model-route binding and normalization now consume canonical
`model_route_decision` fields directly and no longer keep retired camelCase
route-decision reader fallbacks.

Slice 247 extends bridge proof for
`runtime-run-model-route-decision-reader-aliases-retired`: runtime run assembly
and route-selection tests now consume canonical `model_route_decision`
route/model fields directly and no longer keep retired camelCase reader
fallbacks.

Slice 248 extends bridge proof for
`model-mount-route-decision-ref-aliases-retired`: accepted
`model_route_decision` policy/evidence ref metadata, the SDK
`ModelRouteDecision` type, and direct runtime run readers now use canonical
snake_case fields without duplicate camelCase aliases.

Slice 249 extends bridge proof for
`model-mount-route-decision-projection-receipt-aliases-retired`:
model-route decision projection receipt metadata, the SDK `ModelRouteDecision`
type, route-decision projection tests, and direct runtime run/event readers now
use canonical `receipt_id`, `receipt_created_at`, and `receipt_kind` fields
without duplicate camelCase aliases.

Slice 250 extends bridge proof for
`model-mount-route-selection-detail-aliases-retired`: model-route selection
receipt details, projection replay readers, and model invocation admission
workflow readers now use canonical snake_case route/model/provider/policy and
workflow fields without duplicate camelCase aliases.

Slice 251 extends bridge proof for
`model-mount-native-response-route-decision-aliases-retired`: native workflow and
OpenAI-compatible response projections now prove canonical snake_case
`route_decision` fixtures without duplicate camelCase route/model aliases.

Slice 252 extends bridge proof for
`model-mount-receipt-gate-route-detail-aliases-retired`: model-mount Receipt
Gate validation now reads canonical snake_case request and receipt detail fields
and emits canonical snake_case gate receipt/error metadata without duplicate
camelCase aliases.

Slice 253 extends bridge proof for
`model-mount-invocation-receipt-detail-aliases-retired`: model invocation and
stream-completion receipt details now serialize canonical snake_case route,
backend, receipt, token, hash, provider, MCP, stream, and response metadata
without duplicate camelCase aliases.

Slice 254 extends bridge proof for
`model-mount-stream-cancel-receipt-detail-aliases-retired`: stream cancellation
receipts now serialize canonical snake_case stream, route, backend, provider,
MCP, and frame-count metadata without duplicate camelCase aliases.

Slice 255 extends bridge proof under
`model-mount-route-selection-detail-aliases-retired`: route-resolution
fail-closed metadata now uses canonical `route_id` and `evaluated_candidates`
without duplicate camelCase aliases.

Slice 256 extends receipts proof for
`model-mount-provider-transport-auth-error-aliases-retired`: provider transport
and provider auth fail-closed errors now use canonical snake_case provider,
HTTP, vault, auth-header, command, and stderr metadata without duplicate
camelCase aliases.

Slice 257 extends receipts proof for
`model-mount-wallet-vault-error-detail-aliases-retired`: wallet authority and
vault fail-closed errors now use canonical snake_case scope, grant, revocation,
vault-ref, path, and key-configuration metadata without duplicate camelCase
aliases.

Slice 258 extends receipts proof for
`model-mount-capability-token-error-detail-aliases-retired`: capability-token
auth and not-found failures now use canonical `required_scope` and `token_id`
details without duplicate camelCase aliases.

Slice 259 extends receipts proof for
`model-mount-catalog-provider-auth-error-detail-aliases-retired`: catalog
provider auth/config failures now use canonical snake_case provider, auth
scheme, vault-ref hash, resolved-material, auth-header hash, and evidence-ref
details without duplicate camelCase aliases.

Slice 260 extends receipts proof for
`model-mount-oauth-error-detail-aliases-retired`: OAuth credential and token
endpoint fail-closed errors now use canonical snake_case provider, OAuth state,
session, vault-ref, token-endpoint, boundary, and evidence metadata without
duplicate camelCase aliases.

Slice 261 extends receipts proof for
`model-mount-catalog-oauth-receipt-detail-aliases-retired`: catalog OAuth
start, callback, exchange, refresh, and revoke accepted receipts plus
missing-session errors now use canonical snake_case OAuth and provider metadata
without duplicate camelCase aliases.

Slice 262 extends receipts proof for
`model-mount-backend-lifecycle-detail-aliases-retired`: backend health,
start, stop, and backend-log read lifecycle receipts plus blocked-start errors
now use canonical snake_case backend, model, evidence, and log metadata without
duplicate camelCase aliases.

Slice 263 extends receipts proof for
`model-mount-provider-operation-detail-aliases-retired`: provider health,
model-list, loaded-list, start, and stop receipts plus provider-health
fail-closed errors now use canonical snake_case provider, status, auth, count,
evidence, and provider-health metadata without duplicate camelCase aliases.

Slice 264 extends receipts proof for
`model-mount-instance-lifecycle-detail-aliases-retired`: model load, unload,
idle-evict, and supersede lifecycle receipts plus direct-write fail-closed
errors now use canonical snake_case instance, endpoint, model, provider,
backend, process, evidence, and supersession metadata without duplicate
camelCase aliases.

Slice 265 extends receipts proof for
`model-mount-tokenizer-receipt-detail-aliases-retired`: tokenizer and
context-fit receipts now use canonical snake_case route, receipt, selected
model, endpoint, provider, backend, grant, tokenizer, input-hash, token-count,
and context-window metadata without duplicate camelCase aliases.

Slice 266 extends bridge proof for
`model-mount-conversation-state-metadata-aliases-retired`: persisted redacted
conversation-state records, continuation validation, and conversation schema
relations now use canonical snake_case route, endpoint, selected-model,
provider, backend, instance, receipt, stream, input-hash, output-hash,
token-count, message-count, and replay metadata without duplicate camelCase
aliases.

Slice 267 extends receipts proof for
`model-mount-storage-lifecycle-detail-aliases-retired`: model storage lifecycle
receipts and fail-closed errors now use canonical snake_case job, model,
provider, byte-count, cleanup, artifact, endpoint, instance, orphan, and
destructive-confirmation metadata without duplicate camelCase aliases.

Slice 268 extends receipts proof for
`model-mount-mcp-receipt-detail-aliases-retired`: MCP server registration,
import, tool-invocation receipts, and fail-closed errors now use canonical
snake_case server, URL, allowed-tool, secret-ref, redacted-header, import-time,
input-hash, output-hash, and workflow-node metadata without duplicate camelCase
aliases.

Slice 269 extends receipts proof for
`model-mount-catalog-download-receipt-detail-aliases-retired`: catalog
import/download receipts and fail-closed errors now use canonical snake_case
source, evidence, model, provider, byte-count, transfer, download-policy,
catalog-auth, artifact, retry, and resume metadata without duplicate camelCase
aliases.

Slice 270 extends receipts proof for
`model-mount-runtime-engine-detail-aliases-retired`: runtime-engine selection,
profile update, profile removal receipts, disabled/missing-engine fail-closed
errors, and latest-receipt readers now use canonical snake_case engine,
profile, option, timestamp, receipt, and evidence metadata without duplicate
camelCase aliases.

Slice 271 extends receipts proof for
`model-mount-lm-studio-provider-error-aliases-retired`: LM Studio provider
absent-CLI fail-closed errors now use canonical snake_case provider and
evidence metadata without duplicate camelCase aliases.

Slice 272 extends receipts proof for
`model-mount-state-accessor-error-aliases-retired`: model-mount state accessor
fail-closed errors now use canonical snake_case provider, endpoint, instance,
route, and model metadata without duplicate camelCase aliases.

Slice 273 extends receipts proof for
`model-mount-backend-process-error-aliases-retired`: backend process registry
lookup fail-closed errors now use canonical snake_case backend metadata without
duplicate camelCase aliases.

Slice 274 extends receipts proof for
`model-mount-storage-lifecycle-detail-aliases-retired`: download job lookup
fail-closed errors now use canonical snake_case job metadata without duplicate
camelCase aliases.

Slice 275 extends receipts proof for
`model-mount-instance-lifecycle-detail-aliases-retired`: loaded-instance lookup
fail-closed errors now use canonical snake_case endpoint metadata without
duplicate camelCase aliases.

Slice 276 extends receipts proof for
`model-mount-receipt-store-operation-append-retired`: receipt lookup
fail-closed errors now use canonical snake_case receipt metadata without
duplicate camelCase aliases.

Slice 277 extends receipts proof for
`model-mount-local-system-probe-error-aliases-retired`: local model artifact
probe fail-closed errors now use canonical snake_case source-path and directory
metadata without duplicate camelCase aliases.

Slice 278 extends receipts proof for
`model-mount-catalog-helper-error-detail-aliases-retired`: catalog helper
fail-closed import-mode errors now expose only canonical `import_mode`
metadata without the duplicate `importMode` alias.

Slice 279 extends receipts proof for
`model-mount-artifact-endpoint-receipt-detail-aliases-retired`: artifact
endpoint import/mount/unmount lifecycle receipts now emit canonical snake_case
metadata without duplicate camelCase aliases.

Slice 280 extends receipts proof for
`model-mount-runtime-survey-receipt-detail-aliases-retired`: runtime survey
receipts and readback now use canonical snake_case metadata without duplicate
camelCase aliases.
...
Slice 283 extends bridge proof for provider compatibility-translation
retirement: provider results now reject retired `compatTranslation` aliases
without reading them as compatibility fallbacks or emitting camelCase error
details, while canonical `compat_translation` remains a forbidden
...
Slice 294 retires model-route-decision payload summary camelCase readers:
runtime event summaries now consume canonical `event_kind`, `decision_id`,
`route_id`, and `fallback_triggered` route-decision fields only, with
compositor conformance rejecting `eventKind`/`decisionId`/`routeId`/
`fallbackTriggered` readbacks.

Slice 295 retires SDK model-mount receipt metadata aliases: the public
`ModelInvocationReceipt` and `ModelConversationState` SDK types now expose the
canonical snake_case fields written by the daemon, and bridge conformance
rejects the old camelCase receipt/conversation metadata contract.

Slice 296 retires model invocation request aliases at the daemon entry point:
...
Slice 364 moves thread-memory agent touch planning to Rust:
`ThreadMemoryAgentStateUpdateCore` now owns the persisted agent projection for
memory status/mutation/policy control events; the daemon thread-memory helper
...
Slice 416 removes cTEE runner bridge request aliases:
the Private Workspace cTEE daemon runner now rejects retired bridge request
aliases such as `nodeTrust` and `expectedHeads` before invoking the Rust command
bridge; only canonical `node_trust` and `expected_heads` enter the cTEE
operator request.

Slice 417 removes pending diagnostics feedback mode aliases:
the runtime diagnostics feedback surface now rejects retired pending-turn mode
aliases such as `diagnosticsMode` and `options.diagnosticsMode`; only canonical
`diagnostics_mode` and `options.diagnostics_mode` can shape diagnostics feedback
projection into the next runtime turn.

Slice 418 removes runtime run receipt reader aliases:
runtime run-event receipt projection now reads canonical `receipt_id`,
`policy_receipt_id`, `escalation_receipt_ids`, and `receipt_refs` only; retired
...
Slice 469 retires worker/service-package IDE control input aliases:
`createRuntimeWorkerServicePackageControlRequest` and the workflow-node helper
...
Slice 471 retires worker/service-package SDK request aliases:
`admitWorkerServicePackageInvocation` now fails closed before transport when a
caller supplies retired `workflowGraphId` or `workflowNodeId`, the SDK admission
input type advertises canonical `workflow_graph_id` / `workflow_node_id` only,
and bridge conformance guards the SDK type, runtime guard, and focused test.

Slice 472 retires L1-settlement SDK request aliases:
`admitL1SettlementAttempt` now fails closed before transport when a caller
supplies retired `workflowGraphId` or `workflowNodeId`, the SDK admission input

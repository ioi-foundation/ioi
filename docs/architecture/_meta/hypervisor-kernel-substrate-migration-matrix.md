...
plus workspace-change control/inspection bridge request alias evidence through
slice 655, plus managed-session inspection/control bridge request alias evidence
...
manager validation input alias evidence through slice 665, plus studio intent
execution-mode input alias evidence through slice 666, plus runtime run-event
top-level identity alias evidence through slice 667, plus runtime MCP inline
options alias evidence through slice 668, plus runtime event cursor alias
evidence through slice 669, plus model-mount server-control option alias
evidence through slice 670, plus model-mount runtime-engine option alias
evidence through slice 671, plus Rust command-bridge schema-version alias
evidence through slice 672, plus runtime thread-control schema-version output
alias evidence through slice 673, plus runtime thread-control surface output
alias evidence through slice 674, plus operator-control fail-closed detail
alias evidence through slice 675, plus context-policy runner operation-kind
detail alias evidence through slice 676, plus diagnostics repair retry
create-run diagnostics-mode option alias evidence through slice 677, plus
runtime-agent-service bridge result alias evidence through slice 678, plus
runtime usage event timestamp alias evidence through slice 679, plus runtime
task/job run-id lookup alias evidence through slice 680, plus runtime bridge
message data alias evidence through slice 681, plus runtime bridge derived-event
timestamp alias evidence through slice 682, plus runtime bridge event-payload
alias evidence through slice 683, plus IDE TUI control model-route alias
evidence through slice 684, plus IDE auth stream failure panel alias evidence
through slice 685, plus crash-recovery proof/report-card alias evidence through
slice 686, plus authority-boundary proof/visualizer alias evidence through
slice 687, plus context-policy daemon-core command-envelope evidence through
slice 688, plus approval-authority daemon-core command-envelope evidence
through slice 689, plus runtime Agentgres daemon-core command-envelope evidence
through slice 690, plus external capability authority daemon-core
command-envelope evidence through slice 691, plus cTEE Private Workspace
daemon-core command-envelope evidence through slice 692, plus L1 settlement
daemon-core command-envelope evidence through slice 693, plus governed
improvement daemon-core command-envelope evidence through slice 694, plus
...
slice 705, plus public runtime-engine JS mutation-facade retirement evidence
through slice 706, plus public artifact/endpoint/storage JS mutation-facade
retirement evidence through slice 707.
...
## Implementation Slice 708

```yaml
slice: 708
phase: 10-authoritative-js-facade-retirement
objective: retire public catalog import and model-download JS mutation facades
  until Rust daemon-core owns catalog/download truth
owner_boundary:
  route_or_surface: model-mounting catalog import URL and direct model download
    queue/materialization facades
  authority_gate: fail closed at `model_mount.catalog_download` for public
    catalog/download mutations after canonical request-shape validation
  execution_backend: none in JS for mutation; Rust daemon-core `model_mount`
    must own the eventual catalog import, download queue, transfer,
    materialization, artifact binding, receipt, record-state, and projection
    APIs
  truth_path: no JS-created `model_catalog_import_url`,
    `model_download_queued`, `model_download_running`,
    `model_download_failed`, or `model_download_completed` receipts; no JS
    artifact/download map mutation; no JS fixture/live materialization or
    filesystem writes; no JS artifact/download record-state commit from the
    public mutation facades
  projection_path: existing catalog/download read projections remain
    current-state projections until Rust projection owns the full surface
touched_files:
  docs:
    - docs/architecture/_meta/implementation-matrix.md
...
conformance_checks:
  - receipts conformance requires
    `model_mount_catalog_download_rust_core_required`
  - receipts conformance rejects reintroducing JS-authored catalog/download
    lifecycle receipts, JS map mutation, JS materialization/filesystem helpers,
    or JS record-state commit helpers in the public mutation facades
  - focused daemon tests prove catalog import/download fail with canonical
    snake_case details and leave receipts, record-state commits, map writes,
    projections, timestamps, and download forwarding untouched
verification:
  commands:
    - node --test packages/runtime-daemon/src/model-mounting/catalog-download-operations.test.mjs
    - node --check packages/runtime-daemon/src/model-mounting/catalog-download-operations.mjs
    - node --check packages/runtime-daemon/src/model-mounting/catalog-download-operations.test.mjs
    - node --check scripts/conformance/hypervisor-conformance.mjs
    - npm run hypervisor-conformance:receipts
    - npm run hypervisor-conformance:docs
    - npm run hypervisor-conformance
...
cleanup:
  legacy_paths_removed: true
  compatibility_shims_remaining:
    - catalog/download reads and adjacent model-mount projections still read the
      current JS state maps until direct Rust daemon-core projection ownership
      lands
    - lower-level persisted catalog/download projection records still expose
      current view fields until the broader projection contract is migrated
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
## Implementation Slice 709

```yaml
slice: 709
phase: 10-authoritative-js-facade-retirement
objective: retire public capability-token JS mutation and authorization facades
  until Rust daemon-core wallet authority owns grant truth
owner_boundary:
  route_or_surface: model-mounting capability token create/revoke/list and
    Bearer authorization helpers
  authority_gate: fail closed at `model_mount.capability_token` for public
    token grant, revoke, and authorization-use mutations after canonical auth
    and not-found checks; token list remains a read projection only
  execution_backend: none in JS for grant/auth mutation; Rust daemon-core
    wallet authority must own the eventual grant, revoke, authorize, receipt,
    record-state, and projection APIs
  truth_path: no JS-created `permission_token` or
    `permission_token_revocation` receipts; no JS `walletAuthority.createGrant`,
    `authorizeScope`, `revokeGrant`, or `recordLastUsed` methods; no JS token
    map mutation or token record-state commit from the public token/auth
    facades
  projection_path: existing token list and wallet vault-ref status projections
    remain read adapters until Rust wallet authority projection owns the full
    surface
touched_files:
  docs:
    - docs/architecture/_meta/implementation-matrix.md
...
conformance_checks:
  - receipts conformance requires
    `model_mount_capability_token_rust_core_required`
  - receipts conformance rejects reintroducing JS wallet grant/revoke/authorize
    methods, JS permission-token receipts, JS token map mutation, or JS
    record-state commit helpers in the public token/auth facades
  - focused daemon tests prove token create/revoke/authorize fail with
    canonical snake_case details and leave wallet calls, receipts, record-state
    commits, timestamps, and token records untouched
  - wallet authority tests prove the remaining JS wallet authority class is only
    a vault-ref boundary/status adapter, not a grant lifecycle authority
verification:
  commands:
    - node --test packages/runtime-daemon/src/model-mounting/capability-token-operations.test.mjs packages/runtime-daemon/src/model-mounting/wallet-authority.test.mjs
    - node --check packages/runtime-daemon/src/model-mounting/capability-token-operations.mjs
    - node --check packages/runtime-daemon/src/model-mounting/wallet-authority.mjs
    - node --check packages/runtime-daemon/src/model-mounting/capability-token-operations.test.mjs
    - node --check packages/runtime-daemon/src/model-mounting/wallet-authority.test.mjs
    - node --check scripts/conformance/hypervisor-conformance.mjs
    - npm run hypervisor-conformance:receipts
    - npm run hypervisor-conformance:docs
    - npm run hypervisor-conformance
...
cleanup:
  legacy_paths_removed: true
  compatibility_shims_remaining:
    - token list and wallet vault-ref status still read current JS state/adapters
      until direct Rust daemon-core wallet authority projection ownership lands
    - schedule a matrix-compaction pass once the next Rust-core extraction or
      facade-retirement seam is clear, so this capability-token retirement
      remains encoded as migration evidence rather than a terminal bridge shape
closeout:
  git_diff_check: required
  commit: required
  push: required after verification
...
## Implementation Slice 711

```yaml
slice: 711
phase: 10-authoritative-js-facade-retirement
objective: retire public catalog-provider configuration and OAuth JS mutation
  facades until Rust daemon-core catalog provider control owns provider/OAuth
  truth
owner_boundary:
  route_or_surface: model-mounting catalog provider configuration plus OAuth
    start/callback/exchange/refresh/revoke helpers
  authority_gate: fail closed at `model_mount.catalog_provider_control` for
    public provider configuration and OAuth state/session mutations after
    provider-id and required callback-state validation
  execution_backend: none in JS for public provider configuration or OAuth
    mutation; Rust daemon-core model_mount must own the eventual provider
    config, OAuth state/session, wallet/cTEE custody, receipt, record-state, and
    projection APIs
  truth_path: no JS-created `model_catalog_provider_configuration`,
    `catalog_oauth_*`, OAuth state, OAuth session, or provider-config accepted
    receipts from the public catalog-provider control facades; no JS provider
    config/runtime-material/session/state map mutation, vault-ref write, or
    projection write from those facades
  projection_path: existing provider config list/get, runtime-material lookup,
    and catalog auth refresh helpers remain migration adapters until direct Rust
    daemon-core catalog-provider projection and custody APIs land
touched_files:
  docs:
    - docs/architecture/_meta/implementation-matrix.md
    - docs/architecture/_meta/hypervisor-kernel-substrate-migration-matrix.md
...
conformance_checks:
  - receipts conformance requires
    `model_mount_catalog_provider_control_rust_core_required`
  - receipts conformance rejects reintroducing JS catalog provider
    configuration/OAuth receipt creation, provider/session/state map mutation,
    vault writes, record-state commits, OAuth credential-provider execution, or
    projection writes in the public facades
  - focused daemon tests prove configuration and OAuth public mutations fail
    with canonical snake_case details and leave receipts, record-state commits,
    local writes, vault writes, projections, provider configs, OAuth states, and
    OAuth sessions untouched
verification:
  commands:
    - node --test packages/runtime-daemon/src/model-mounting/catalog-provider-configuration-operations.test.mjs packages/runtime-daemon/src/model-mounting/catalog-provider-oauth.test.mjs
...
cleanup:
  legacy_paths_removed: true
  compatibility_shims_remaining:
    - catalog provider list/get and runtime-material resolution still read or
      refresh current JS state/vault-port adapters until Rust daemon-core
      catalog-provider projection and custody APIs land
    - catalog auth-header refresh in `catalog-provider-config.mjs` still has a
      Rust record-state commit helper around JS OAuth refresh plumbing; this is
      the next catalog-provider custody/control extraction seam
    - schedule a matrix-compaction pass once the next Rust-core extraction or

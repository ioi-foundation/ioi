...
| `l1-settlement` | `docs/architecture/foundations/ioi-l1-mainnet.md`, `crates/services/src/agentic/runtime/kernel/settlement.rs`, `crates/node/src/bin/ioi_step_module_bridge/mod.rs`, `packages/runtime-daemon/src/runtime-l1-settlement-runner.mjs`, `packages/runtime-daemon/src/index.mjs` | canon plus Rust trigger guard, command bridge admission primitive, and mounted daemon runner | Rust settlement/admission core under daemon-owned execution | sparse public/economic/cross-domain commitment by trigger only | `bridge`, `negative` | Rust trigger guard implemented and exposed through `admit_l1_settlement_attempt`; daemon `RustL1SettlementRunner` now calls that bridge and is mounted on `AgentgresRuntimeStateStore`; product settlement surfaces still pending | L1 settlement attempts without marketplace/public/economic/cross-domain/operator trigger fail closed. |
...
Current expected behavior after Slice 108:

...
Last matrix compaction pass: 2026-06-07, after MCP manager workspace-root context facade alias retirement slice 631 clarified the active seam.
...
Current expected behavior after Slice 631 and the sixty-sixth 2026-06-07 matrix compaction pass:
...
- Slices 572-631 retired SDK runtime usage record aliases and Rust/daemon
  ...
  `workspace_root` only from canonical `context.workspace_root`, so retired
  `context.workspaceRoot` cannot materialize server workspace state; deeper MCP
  stdio cwd alias fallback remains a separate transport-boundary candidate;

...
Current expected behavior after Slice 32:
...
| `hypervisor-conformance:receipts` | pass | Rust StepModule receipt binder exists, model invocation and stream-completion receipts carry Rust Agentgres admission, and direct unbound model invocation store appends fail closed. |

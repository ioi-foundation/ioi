...
- **Implementation state** is honest: `not started` means no durable form in
  code; `partial` names exactly which slice exists. Nothing planned is
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `WorkResult` | [`common-objects-and-envelopes.md`](../foundations/common-objects-and-envelopes.md) | canonical object: generic typed result envelope (kind, payload profile, evidence refs, verification state) beyond software patches | software profile only: the `implementation_result` payload (schema `ioi.hypervisor.implementation-result.v1`; called ImplementationResultPayload in doc comments — not a named Rust type) built and admitted in `crates/services/src/agentic/runtime/kernel/runtime_goal_run_admission.rs` and `crates/node/src/bin/hypervisor_daemon_routes/goalrun_routes.rs` | partial (software profile; generic envelope absent) | daemon route + Agentgres admission of a generic result family with receipt | planned: generic-result admission tests beside existing GoalRun admission tests | Missions (result detail); Provenance (evidence chain) |
| `OutcomeDelta` | [`common-objects-and-envelopes.md`](../foundations/common-objects-and-envelopes.md) | canonical object: declared before/after outcome statement bound to results and acceptance | none | not started | daemon route + Agentgres admission; delta must bind result refs at write | planned: delta-binds-result invariant test (build step 1) | Missions (outcome view) |
| `OutcomeRoom` | [`collaborative-outcome-pattern.md`](../domains/ioi-ai/collaborative-outcome-pattern.md) | canonical object: hosted room aggregate above bounded GoalRuns with declared admission mode | nearest substrate: GoalRun plane `/v1/hypervisor/goal-runs` (`crates/node/src/bin/hypervisor_daemon_routes/goalrun_routes.rs`) | not started (GoalRun substrate exists; no room aggregate) | hosted room record family whose every shared-state transition is admitted + receipted | planned: single-node collaborative-pursuit proof (build step 6) | Missions (room = Mission detail); ioi.ai Goal Space |
...
| `Attempt` | [`collaborative-outcome-pattern.md`](../domains/ioi-ai/collaborative-outcome-pattern.md) | canonical object: one bounded, isolated execution against a claim, run as a GoalRun | nearest substrate: GoalRun records + agent-run transcripts (`goalrun_routes.rs`; `/v1/hypervisor/agent-run-transcripts`) | partial (bounded runs exist; attempt-to-claim binding absent) | attempt record binding claim ref + GoalRun ref at admission | planned: attempt-binds-claim invariant test (build step 3) | Missions (attempt timeline); Provenance (replay) |
| `Finding` | [`collaborative-outcome-pattern.md`](../domains/ioi-ai/collaborative-outcome-pattern.md) | canonical object: positive or negative typed discovery bound to attempts/evidence, valuable independent of success | nearest substrate: evaluation verdict records (`crates/node/src/bin/hypervisor_daemon_routes/eval_suite_routes.rs`) | not started (evaluation verdicts exist; room-scoped findings absent) | finding admission with evidence refs; negative findings first-class | planned: negative-finding lineage test (build step 3) | Missions (findings feed); Evaluations (verdict lineage) |
...
| `OntologyVersion` | [`domain-ontologies-and-data-recipes.md`](../foundations/domain-ontologies-and-data-recipes.md) | canonical object: immutable `DomainOntologyEnvelope` with `ontology_record_profile: ontology_version` (compatibility/deprecation contract per namespace) | partial anchor: ODK ontology revision + `expected_revision` optimistic concurrency + revision history (`crates/node/src/bin/hypervisor_daemon_routes/odk_routes.rs`) | partial (single-domain revisions exist; federated version/namespace identity absent) | version publication as immutable admitted record with cross-namespace identity | existing: `odk_tests` (revision conflict = typed, zero mutation); planned: federated version lineage tests (build step 5) | Ontology (Manager history) |
...
| `SemanticMappingDecision` | [`domain-ontologies-and-data-recipes.md`](../foundations/domain-ontologies-and-data-recipes.md) | canonical object: applied `OntologyMappingEnvelope` with `mapping_record_profile: semantic_mapping_decision` — challengeable, receipted application of a crosswalk to a concrete handoff | nearest substrate: `ConnectorMapping` create/update receipts (`connector_mapping_routes.rs`) | partial (mapping records + receipts exist; decision-as-object with reviewer lineage absent) | decision admission with reviewer identity + receipt | planned: mapping-decision lineage test (build step 5) | Ontology (mapping review) |
| `ProvenanceAssertion` | [`domain-ontologies-and-data-recipes.md`](../foundations/domain-ontologies-and-data-recipes.md) | canonical object: `OntologyAssertionEnvelope` with `assertion_profile: provenance_assertion` — time/source/confidence/evidence/dispute-bearing claim; admission records it without making it universally true | nearest substrate: receipt families + proof stream (`/v1/hypervisor/work-ledger` route in `crates/node/src/bin/hypervisor-daemon.rs` — route name is API compatibility; the product surface is Provenance) | partial (receipts exist everywhere; assertion-as-object absent) | assertion admission binding receipt refs; challenge path shared with `VerifierChallenge` | planned: assertion-binds-receipts test (build step 5) | Provenance (assertion graph) |
...
sequence. Surface operationalization is now PULLED by contracts:
...
Already-landed operational surfaces (Approvals, Ontology Manager, Pipeline
Builder, Sources) keep their shipped state and verifiers; nothing in this
reclassification un-ships them. The held application-stack PRs are reviewed
and land per their own gates; this backlog governs NEW surface work only.

...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `WorkResult` | [`common-objects-and-envelopes.md`](../foundations/common-objects-and-envelopes.md) | canonical object: generic typed result envelope (kind, payload profile, evidence refs, verification state) beyond software patches | software profile only: `ImplementationResultPayload` in `crates/services/src/agentic/runtime/kernel/runtime_goal_run_admission.rs` and `crates/node/src/bin/hypervisor_daemon_routes/goalrun_routes.rs` | partial (software profile; generic envelope absent) | daemon route + Agentgres admission of a generic result family with receipt | planned: generic-result admission tests beside existing GoalRun admission tests | Missions (result detail); Provenance (evidence chain) |

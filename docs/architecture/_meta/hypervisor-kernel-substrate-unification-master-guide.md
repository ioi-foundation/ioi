...
Current lane note: after Slice 974, public runtime account, runtime-node, tool
catalog, agent, thread, run, agent-run lifecycle, run wait, run conversation,
run trace/inspect, run computer-use trace/trajectory, run scorecard, and run
artifact projections are no longer JS-authored public truth. The daemon edge now
...
internal agent/thread/run list/get, trace, and artifact helpers are scaffolding
only until Rust daemon-core catalog and lifecycle projection over

...
733-736 are intentionally left expanded as the current runtime bridge thread/turn,
runtime subagent control facade-retirement/legacy-body deletion, and runtime
task/job control facade-retirement seams. The next compaction pass is pending after the next seam is clear enough to preserve evidence, distinguish

...
2. If two architecture files disagree, prefer the newer aligned direction.
  ...
  - for Hypervisor product UX, `internal-docs/reverse-engineering/ioi` is the
    primary product reference; architecture docs translate that reference into
    ownership, authority, state, receipt, privacy, and adapter contracts rather
    than creating peer IDE, console, bridge, or fleet surfaces;
  - ioi.ai Goal Chat is an intent and coordination surface: it may ask,
   ...
   - Hypervisor App, Hypervisor Web, CLI/headless, Workbench, Automations,
     Foundry, and provider/environment views are not the Hypervisor Node; they
     are clients, application surfaces, or projections, while the node is the local
...
- Add new Hypervisor App, Hypervisor Web, CLI/headless, optional TUI,
  Workbench, Automations, Foundry, provider/environment view, session, adapter-target,

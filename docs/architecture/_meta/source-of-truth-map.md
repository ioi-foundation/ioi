...
2. If two architecture files disagree, prefer the newer aligned direction.
   ...
   - Hypervisor manages sessions, environments, providers, and cross-session
     infrastructure posture directly; Fleet is deprecated as a live app,
     application surface, posture layer, runtime, authority, truth layer, or
     storage layer;
   ...
   - Hypervisor Workbench, Automations, and Foundry are application surfaces
     ...
     Hypervisor session/project/provider view, not a separate Fleet surface;
   ...
   - ioi.ai Outcome Rooms are chat.ioi.ai's first-party implementation of
     governed Collaborative Missions over Hypervisor for many agents, models,
     harnesses, humans, sessions, and verifier lanes searching toward one
     measurable outcome under eval gates, receipts, authority, privacy, and
     promotion policy;
...
| Workflow Compositor, shared builder substrate, high-level workflow/service graph shape, step contracts, directed recipes, Automations product surface, ioi.ai Outcome Rooms, Canvas visual editor/projection, and harness/model/provider selection hints | [`core-clients-surfaces.md`](../components/hypervisor/core-clients-surfaces.md) | [`outcome-rooms.md`](../components/hypervisor/outcome-rooms.md), [`foundry.md`](../components/hypervisor/foundry.md), [`default-harness-profile.md`](../components/daemon-runtime/default-harness-profile.md), [`ioi-cli-daemon-runtime.md`](../components/daemon-runtime/doctrine.md), [`events-receipts-delivery-bundles.md`](../components/daemon-runtime/events-receipts-delivery-bundles.md), [`ioi-daemon-runtime-api.md`](../components/daemon-runtime/api.md) | Automations is the live durable workflow/service/mission surface; ioi.ai Outcome Rooms are first-party governed collaborative missions over the same contracts; Workbench is the live code/systems surface; Foundry is the model/worker/eval/dataset/training/package build surface; Canvas is an editor/projection, not runtime truth; "Hypervisor IDE" and Electron/VS Code-shell language is legacy or implementation context, not parent product doctrine. Compositor owns directed-work shape; selected HarnessProfiles resolve scoped steps. |

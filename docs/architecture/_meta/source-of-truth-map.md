...
2. If two architecture files disagree, prefer the newer aligned direction.
   ...
   - Hypervisor Workbench and Foundry are application surfaces over Hypervisor
     Core, while provider/environment posture is a default Hypervisor
     session/project/provider view, not a separate Fleet surface;
   ...
   - a Hypervisor Node is a local settlement, orchestration, authority, state,
     ...
     chains;
   - Hypervisor App, Hypervisor Web, CLI/headless, Workbench, Foundry, and
     provider/environment views are not the Hypervisor Node; they are clients,
     application surfaces, or projections, while the node is the local
...
| Workflow Compositor, shared builder substrate, high-level workflow/service graph shape, step contracts, directed recipes, and harness/model/provider selection hints | [`core-clients-surfaces.md`](../components/hypervisor/core-clients-surfaces.md) | [`default-harness-profile.md`](../components/daemon-runtime/default-harness-profile.md), [`ioi-cli-daemon-runtime.md`](../components/daemon-runtime/doctrine.md), [`events-receipts-delivery-bundles.md`](../components/daemon-runtime/events-receipts-delivery-bundles.md), [`ioi-daemon-runtime-api.md`](../components/daemon-runtime/api.md) | Workbench is the live code/systems surface; "Hypervisor IDE" and Electron/VS Code-shell language is legacy or implementation context, not parent product doctrine. Compositor owns directed-work shape; selected HarnessProfiles resolve scoped steps. |
...
- Add new Hypervisor App, Hypervisor Web, CLI/headless, optional TUI,
  Workbench, Foundry, provider/environment view, session, adapter-target,

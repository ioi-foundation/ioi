...
2. If two architecture files disagree, prefer the newer aligned direction.
   ...
   - `decentralized.cloud` is a preferred first-party cloud/resource route
     source for compute, storage, GPU, bandwidth, confidential execution,
     DePIN, customer cloud, and local infrastructure; it is not a compute
     provider, execution owner, authority layer, storage authority, or trust
     root;

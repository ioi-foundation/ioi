...
| `StepModuleInvocation` | [`hypervisor-kernel-substrate-unification-master-guide.md`](./hypervisor-kernel-substrate-unification-master-guide.md) | shared daemon-to-kernel invocation ABI | canonical object for every consequential step/module invocation | `crates/services/src/agentic/runtime/kernel/invocation.rs`, `packages/runtime-daemon/src/coding-tools.mjs`; target schema not yet implemented | `hypervisor-conformance:abi` |
| `StepModuleResult` | [`hypervisor-kernel-substrate-unification-master-guide.md`](./hypervisor-kernel-substrate-unification-master-guide.md) | shared execution result, observation, receipt/ref, state-root, and projection ABI | canonical result object for admitted module execution | `crates/services/src/agentic/runtime/kernel/invocation.rs`, `packages/runtime-daemon/src/runtime-event-envelopes.mjs`; target schema not yet implemented | `hypervisor-conformance:abi`, `hypervisor-conformance:receipts` |

...
| `StepModuleRouter` | [`hypervisor-kernel-substrate-unification-master-guide.md`](./hypervisor-kernel-substrate-unification-master-guide.md) | daemon-owned route from approved loop step to backend execution | Rust core owner boundary, not a peer runtime | migration boundary in `packages/runtime-daemon/src/step-module-runner.mjs`; target Rust `step_router`; JS dispatch currently lives in `packages/runtime-daemon/src/coding-tools.mjs` | `hypervisor-conformance:bridge`, `hypervisor-conformance:negative` |
| `HypervisorKernelSubstrateMigration` | [`hypervisor-kernel-substrate-migration-matrix.md`](./hypervisor-kernel-substrate-migration-matrix.md) | route-family owner map plus terminal conformance command state | migration matrix until terminal conformance, then historical implementation record | `scripts/conformance/hypervisor-conformance.mjs`, `package.json` | `hypervisor-conformance` |

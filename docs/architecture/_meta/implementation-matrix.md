...
| `ModelProviderResult` | [`hypervisor-kernel-substrate-unification-master-guide.md`](./hypervisor-kernel-substrate-unification-master-guide.md) | Rust `model_mount` provider-result admission record for non-migrated JS provider driver observations, including native stream-start observations, binding provider-execution ref/hash, request hash, output hash, token counts, stream status, backend refs, and evidence before accepted invocation receipts | migration admission record; delete once concrete provider transports execute through Rust `workload_client`/`model_mount` instead of JS observations | `crates/services/src/agentic/runtime/kernel/model_mount.rs`, `crates/node/src/bin/ioi_step_module_bridge/mod.rs`, `packages/runtime-daemon/src/model-mounting/model-invocation-operations.mjs` | `hypervisor-conformance:bridge`, `hypervisor-conformance:receipts` |

...
# Ensure CODEX references the contract invariants and canonical glossary.
...
# Block known stale path references that caused contract drift.
for stale in \
  "crates/types/src/workloads/spec.rs" \
  "crates/services/src/search/mod.rs"; do
  if rg -n --fixed-strings "$stale" CODEX.txt docs/CIRC.md docs/CEC.md docs/CONTRACT_GLOSSARY.md >/dev/null; then
    fail "stale path reference detected: $stale"
  fi
done

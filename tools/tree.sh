...
exclude_dir3="$dir/target"
exclude_dir4="$dir/.git"
...
print_tree() {
  ...
  for item in $items; do
    ...
    # Exclusions
    # -------------------------------------------------------------------------
    # 1) Skip anything under the sandbox, tools, target, or .git folders
    if [[ "$full_path" == "$exclude_dir1"* ]] || [[ "$full_path" == "$exclude_dir2"* ]] || [[ "$full_path" == "$exclude_dir3"* ]] || [[ "$full_path" == "$exclude_dir4"* ]]; then
      continue
    fi
    # 2) Skip .ico files
    if [[ "$item" == *.ico ]]; then

...
CASE_ID="${1:-}"
if [[ -z "${CASE_ID}" ]]; then
  echo "usage: tools/miniwob/run_case_diagnostics.sh <case-id>" >&2
  exit 1
fi
...
export COMPUTER_USE_SUITE_TASK_SET="${COMPUTER_USE_SUITE_TASK_SET:-catalog}"
export COMPUTER_USE_SUITE_CASES="${CASE_ID}"
...
python3 tools/miniwob/diagnose_case.py --latest --case "${CASE_ID}" --stdout
exit "${STATUS}"

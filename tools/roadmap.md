...
> **Execution routing (2026-07-17): preserved historical product/runtime
> roadmap.** Current target shape is owned by `docs/architecture/`; active
> dependency order and source disposition are owned by
> [`internal-docs/implementation/ioi-target-end-state-master-implementation-guide.md`](../internal-docs/implementation/ioi-target-end-state-master-implementation-guide.md).
> Do not execute the Autopilot, ioi-memory, application, or phase claims below
> without current-owner reconciliation.

Last updated: 2026-05-15
Status: preserved historical synthesis from `tools/roadmap_context.md` plus a dated repo architecture probe
...
Goal: remove remaining authority drift before early connector expansion.

Build:

- complete connector/tool/model capability registry metadata
...
- Autopilot Authority Center for credentials, grants, approvals, revocation, readiness, and receipts
- React Flow capability binding and readiness projections
- task-scoped GUI/browser readiness probes for isolated local work

Exit criteria:

- Phase 5 live actions fail closed without contract, readiness, grant, approval when required, idempotency posture, and receipt behavior.
- CLI/TUI, SDK, daemon, Autopilot, and workflow compositor project the same capability and authority facts.

## Phase 4.6: Runtime Productivity Readiness Gate

Goal: make the formal runtime feel like a low-friction productivity harness
without weakening capability, authority, policy, receipt, or replay boundaries.

Build:

- dynamic but hashable runtime palette manifests
- model-friendly skill guide envelopes that are explicitly non-authoritative
- staged artifact workspace leases with temp/final paths, TTL, privacy class, promotion policy, and cleanup posture
- compact runtime status snapshots
- async handles for long-running shell, test, render, browser, connector, and creative-tool work
- structured tool errors with safe recovery hints
- observe-act-verify contracts for browser/computer-use lanes
- explicit artifact handoff states for draft, partial, failed, validated, final, delivered, and superseded outputs

Exit criteria:

- Phase 5 connector lanes share one productivity behavior contract instead of inventing connector-specific ergonomics.
- Model and operator surfaces can inspect palette, progress, artifacts, errors, recovery paths, and handoff state without raw secret or private-content leakage.
...
- retired native desktop project module
- retired native desktop graph runner
- retired native desktop execution module
- retired native desktop runtime projection

...
LARGE_DIRS=("pqclean" "node_modules" ".git" "target" "build" "dist" "tests" "examples" "tools" "vectors" "plugins")
IGNORED_EXTS=("jpg" "jpeg" "png" "gif" "mp4" "mov" "zip" "tar" "gz" "class" "o" "so" "dylib" "a" "exe" "dll" "md")
...
# Function to check if a file should be excluded based on its name
function should_exclude_file_name() {
    local filename="$1"
    local filename_lower=$(echo "$filename" | tr '[:upper:]' '[:lower:]')
    if [[ "$filename_lower" == *tests* ]]; then
...
function process_file() {
    ...
    if should_exclude_file_name "$(basename "$file")"; then
        return
    fi

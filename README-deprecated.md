...
| ✅ Developer CLI | Complete | The `cli` binary provides scaffolding, key management, and devnet orchestration. |
...
cargo run -p ioi-cli --bin cli -- init my-sovereign-chain --chain-id 100
...
cargo run --manifest-path ../crates/cli/Cargo.toml --bin cli -- scaffold service --name dex_module

# Scaffold a WASM smart contract
cargo run --manifest-path ../crates/cli/Cargo.toml --bin cli -- scaffold contract --name counter_contract
```
...
cargo run -p ioi-cli --bin cli -- keys generate --suite ed25519

# Generate a Post-Quantum Dilithium2 key
cargo run -p ioi-cli --bin cli -- keys generate --suite dilithium2

# Inspect a raw public key to derive its Account ID
cargo run -p ioi-cli --bin cli -- keys inspect --suite ed25519 <HEX_KEY>
```
...
cargo run -p ioi-cli --bin cli -- node --validators 1 --consensus poa --block-time 1
```
...
cargo run -p ioi-cli --bin cli -- query state --key <HEX_KEY>
```

## Development & Testing

Testing centers around the **`cli`** crate, providing powerful declarative E2E testing.
...
| Test | Command |
|------|---------|
| **Staking** | `cargo test -p ioi-cli --release --features "consensus-pos,vm-wasm,state-iavl,commitment-hash" --test staking_e2e -- --nocapture` |
| **Contract** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test contract_e2e -- --nocapture` |
| **PQC Migration** | `cargo test -p ioi-cli --release --features "consensus-pos,vm-wasm,state-iavl,commitment-hash" --test pqc_migration_e2e -- --nocapture` |
| **Governance** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test governance_e2e -- --nocapture` |
| **Module Upgrade** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test module_upgrade_e2e -- --nocapture` |
| **Service Architecture** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test service_architecture_e2e -- --nocapture` |
| **Agentic Consensus** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test agentic_consensus_e2e -- --nocapture --test-threads=1` |
| **Oracle** | `cargo test -p ioi-cli --release --features "consensus-pos,vm-wasm,state-iavl,commitment-hash" --test oracle_e2e -- --nocapture` |
| **Deterministic Timestamps** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test t_timestamp_coherence -- --nocapture` |

#### Penalty Mechanism Tests

| Test | Command |
|------|---------|
| **PoS Slashing** | `cargo test -p ioi-cli --release --features "consensus-pos,vm-wasm,state-iavl,commitment-hash" --test penalty_pos_e2e -- --nocapture --test-threads=1` |
| **PoA Quarantine** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test penalty_poa_e2e -- --nocapture --test-threads=1` |

#### State Tree Backend Tests

| Backend | Command |
|---------|---------|
| **IAVL Tree** | `cargo test -p ioi-cli --test state_iavl_e2e --no-default-features -F "consensus-poa,vm-wasm,state-iavl,commitment-hash" -- --nocapture` |
| **Sparse Merkle** | `cargo test -p ioi-cli --test state_sparse_merkle_e2e --no-default-features -F "consensus-poa,vm-wasm,state-sparse-merkle,commitment-hash" -- --nocapture` |
| **Verkle Tree** | `cargo test -p ioi-cli --test state_verkle_e2e --no-default-features -F "consensus-poa,vm-wasm,state-verkle,commitment-kzg" -- --nocapture` |
| **Jellyfish Tree** | `cargo test -p ioi-cli --test state_jellyfish_e2e --no-default-features -F "consensus-poa,vm-wasm,state-jellyfish,commitment-hash" -- --nocapture` |
...
| **Interopability** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash,ibc-deps" --test ibc_e2e -- --nocapture` |
| **Update IBC Golden Files** | `UPDATE_GOLDENS=1 cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash,ibc-deps" --test ibc_golden_e2e -- --nocapture` |
| **IBC Relayer E2E** | `RUST_LOG=trace RUST_BACKTRACE=1 cargo test -p ioi-cli --release  --features "consensus-poa,vm-wasm,state-iavl,commitment-hash,ibc-deps"  --test ibc_relayer_e2e -- --nocapture 2>&1 | tee /tmp/e2e.log` |
| **Proof Verification** | `cargo test -p ioi-cli --test proof_verification_e2e --features "consensus-poa,vm-wasm,state-iavl,commitment-hash,malicious-bin" -- --nocapture` |
| **Network Sync** | `cargo test --package ioi-cli --test sync_e2e --features "consensus-poa,vm-wasm,state-iavl" -- --nocapture --test-threads=1` |
...
cargo test -p ioi-cli --test infra_e2e --features "consensus-poa,vm-wasm,state-iavl" -- --nocapture --test-threads=1` |
| **Bridgeless ZK** | `cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash,ibc-deps,ethereum-zk" --test ibc_zk_e2e -- --nocapture --test-threads=1` |
| **Security & Attestation** | `cargo test -p ioi-cli --test security_e2e --features "validator-bins,consensus-poa,vm-wasm,state-iavl" -- --nocapture --test-threads=1` |
| **Validator Topology** | `cargo test --package ioi-cli --test topology_e2e --features "consensus-poa,vm-wasm,state-iavl" -- --nocapture` |

cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test workload_control_e2e -- --nocapture

cargo test -p ioi-cli --release --features "consensus-poa,vm-wasm,state-iavl,commitment-hash" --test adaptive_timing_e2e -- --nocapture

cargo test -p ioi-cli --test benchmark_throughput --no-default-features -F "consensus-poa,vm-wasm,state-jellyfish,commitment-hash" -- --nocapture
...
1.  **Run your test and capture all `info`-level logs:**
    ...
    ```bash
    RUST_LOG=info cargo test -p ioi-cli --test <your_test_name> -- --nocapture | tee test_run.log
    ```
...
├── cli/            # E2E testing harness and developer toolkit
...
- **`cli`**: Developer toolkit and E2E testing library
...
- Complete `cli` CLI for developer ergonomics.
...
| **Developer & Testing Tools**                 | Tooling crates (`cli`, `contract`)                                         | **Apache License 2.0** — open for integration, testing, and research.                       |

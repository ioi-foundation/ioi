...
function createHypervisorShellHeader({
  ...
  function hypervisorShellHeaderStyles() {
    return `
      .hypervisor-shell-header {
      ...
      .hypervisor-shell-header__crumb {
        min-width: 0;
        display: flex;
      ...
      .hypervisor-shell-header__crumb strong {
        font-size: 13px;
        font-weight: 700;
      }
      .hypervisor-shell-header__crumb span {
        color: var(--vscode-descriptionForeground, #9ca3af);
        overflow: hidden;
        text-overflow: ellipsis;
      }
      .hypervisor-shell-header__command {
        min-width: 0;
        height: 32px;
        display: flex;
        align-items: center;
      ...
      .hypervisor-shell-header__command span {
        min-width: 0;
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
      }
      .hypervisor-shell-header__posture {
        min-width: 0;
        display: flex;
        align-items: center;
        justify-content: end;
      ...
      .hypervisor-shell-chip,
      .hypervisor-shell-action {
        min-height: 24px;
        display: inline-flex;
        align-items: center;
        gap: 4px;
      ...
      .hypervisor-shell-action:hover,
      .hypervisor-shell-header__command:hover {
  ...
  function renderHypervisorShellHeader(state, modeId, options = {}) {
    if (nativeWorkbenchShellEnabled()) {
      return "";
    }
    const mode = HYPERVISOR_MODE_BY_ID[modeId] || HYPERVISOR_MODE_BY_ID.home;
    ...
    const shellAction =
      mode.id === "code"
        ? {
            label: "Back to Hypervisor",
            command: "ioi.hypervisor.back",
            testId: "back-to-hypervisor-from-code",
          }
        : {
            label: "Code",
            command: "ioi.code.open",
            testId: "hypervisor-shell-code-drilldown",
          };
    return `
    <header
      class="hypervisor-shell-header"
      data-testid="hypervisor-workbench-shell-header"
      data-hypervisor-mode="${escapeHtml(mode.id)}"
    ...
    >
      <div class="hypervisor-shell-header__crumb" data-testid="hypervisor-shell-breadcrumb">
      ...
      <button
        class="hypervisor-shell-header__command"
        type="button"
        data-command="ioi.commandCenter.open"
        data-testid="hypervisor-shell-command-center"
        data-operator-command-center
      >
        <span>Search Hypervisor, code, workflows, runs, and commands</span>
      </button>
      <div class="hypervisor-shell-header__posture" data-testid="hypervisor-shell-runtime-posture">
        <span class="hypervisor-shell-chip is-${escapeHtml(shellStatusTone(daemonStatus))}" data-testid="hypervisor-shell-daemon-status">Daemon ${escapeHtml(daemonStatus)}</span>
        <span class="hypervisor-shell-chip is-${loadedModels.length ? "ready" : "muted"}" data-testid="hypervisor-shell-model-route">Model ${escapeHtml(routeLabel)}</span>
        <span class="hypervisor-shell-chip is-${activeRuns.length ? "warn" : "muted"}" data-testid="hypervisor-shell-active-runs">Runs ${escapeHtml(String(activeRuns.length))}</span>
        <span class="hypervisor-shell-chip is-${policyIssueCount ? "warn" : "ready"}" data-testid="hypervisor-shell-policy-posture">Approvals ${escapeHtml(String(policyIssueCount))}</span>
        <span class="hypervisor-shell-chip" data-testid="hypervisor-shell-wallet-posture">Authority local</span>
        ${
          options.hideModeAction
            ? ""
            : `<button class="hypervisor-shell-action" type="button" data-command="${escapeHtml(shellAction.command)}" data-testid="${escapeHtml(shellAction.testId)}">${escapeHtml(shellAction.label)}</button>`
        }
      </div>
    </header>
  `;

...
record_metadata() {
  ...
  sha256sum \
    ...
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinement.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinement.tla \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementActivateFromBytes.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementCallBeforeClaim.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementClaimAfterExpiry.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementHandoff.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementLateReply.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementPredecessorInKey.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementReachable.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementResourceIdSplitsKey.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementSkipExecutorQuv.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementSolo.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementTwoDomain.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementUnauthenticatedRecovery.cfg \
    internal-docs/architecture/protocols/aft/formal/maximal_visibility/QuvEndToEndRefinementUnowned.cfg \
    crates/cli/tests/aft_e2e_parts/quv_flood.rs \
    crates/cli/tests/aft_e2e_parts/quv_status_squat.rs \
    ...
    crates/agentgres/src/consequence/claim_index.rs \
    crates/agentgres/src/consequence/receipt_reservation.rs \
    crates/agentgres/src/consequence/resource_reservation.rs \
    crates/networking/src/libp2p/sync.rs \
    crates/networking/src/libp2p/types.rs \
    crates/validator/src/standard/orchestration/events.rs \
    crates/validator/src/standard/orchestration/peer_management.rs \
    crates/validator/src/standard/orchestration/sync.rs \
    crates/validator/src/standard/orchestration/mod.rs \
...
run_phase format_check cargo fmt --check -p ioi-types -p ioi-crypto -p ioi-api -p ioi-services -p ioi-consensus -p ioi-networking -p agentgres -p ioi-validator -p ioi-cli
...
for required_case in startup_reserves_complete_rooted_lifetime_before_creating_a_journal preparation_policy_is_rechecked_on_replay_and_exhaustion_survives_reopen rooted_preparation_format_refuses_schema_seven_without_mutation typed_journal_replay_matches_live_member_across_three_slots_and_reopens journal_member_commit_refusal_and_anchor_failure_do_not_publish_memory journal_member_cached_size_matches_compact_history_boundaries typed_replay_refuses_invalid_order_scope_and_counters_without_mutation typed_replay_refuses_owned_conflict_reservation_but_retains_both_candidates authenticated_pending_record_cannot_advance_anchor_if_typed_replay_refuses; do
  if ! grep -Eq "^test aft::query_unanimity::tests::journal_replay_tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/quv_core.log"; then
...
run_phase sync_executed_prefix cargo test --locked -p ioi-validator --features consensus-aft --lib standard::orchestration::sync::tests::
require_tests_ran sync_executed_prefix
for required_case in already_executed_prefix_is_skipped_only_on_exact_tip_hash_match executed_header_ring_is_bounded_and_matches_only_exact_hashes testing_status_claimed_account_override_accepts_only_exact_64_hex; do
  if ! grep -Eq "^test standard::orchestration::sync::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/sync_executed_prefix.log"; then
    echo "[M16Q] FAIL sync_executed_prefix: missing passing sync dedup regression ${required_case}" >&2
    exit 1
  fi
done
...
for required_case in dispatch_requires_every_exact_member_before_decision canceled_dispatch_never_runs_the_live_decision dispatch_completion_obeys_before_equal_after_deadline; do
  if ! grep -Eq "^test standard::orchestration::quv::dispatch::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/quv_runtime_policy.log"; then
    echo "[M16Q] FAIL quv_runtime_policy: missing passing dispatch regression ${required_case}" >&2
    exit 1
  fi
done
for required_case in push_quota_admits_up_to_limit_then_drops_until_window_slides push_quota_keys_are_independent_per_domain_and_account push_quota_table_is_capped_by_evicting_the_stalest_key late_timer_wake_cannot_admit_a_reply_observed_after_the_rooted_deadline; do
  if ! grep -Eq "^test standard::orchestration::quv::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/quv_runtime_policy.log"; then
    echo "[M16Q] FAIL quv_runtime_policy: missing passing quota/deadline regression ${required_case}" >&2
    exit 1
  fi
done
if ! grep -Eq '^test standard::orchestration::quv::tests::readiness_revalidation_rejects_before_and_accepts_equal_or_after \.\.\. ok$' "${OUTPUT_DIR}/quv_runtime_policy.log"; then
  echo '[M16Q] FAIL quv_runtime_policy: missing current-head readiness revalidation regression' >&2
  exit 1
fi
if ! grep -Eq '^test standard::orchestration::quv::tests::every_operation_role_has_a_rooted_active_service_budget \.\.\. ok$' "${OUTPUT_DIR}/quv_runtime_policy.log"; then
  echo "[M16Q] FAIL quv_runtime_policy: missing all-operation service regression" >&2
  exit 1
fi
if ! grep -Eq '^test standard::orchestration::quv::tests::active_service_completion_never_reports_late_success \.\.\. ok$' "${OUTPUT_DIR}/quv_runtime_policy.log"; then
...
for required_case in captured_completion_notifies_and_forwards_exact_reply_under_backpressure refused_completion_notifies_without_reply_and_closed_lane_returns_error; do
  if ! grep -Eq "^test standard::orchestration::quv::member_completion::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/quv_runtime_policy.log"; then
    echo "[M16Q] FAIL quv_runtime_policy: missing member-completion regression ${required_case}" >&2
    exit 1
  fi
done
if ! grep -Eq '^test standard::orchestration::quv::tests::reply_handler_uses_captured_table_and_preserves_transport_binding \.\.\. ok$' "${OUTPUT_DIR}/quv_runtime_policy.log"; then
  echo '[M16Q] FAIL quv_runtime_policy: missing captured-table reply routing regression' >&2
  exit 1
fi
if ! grep -Eq '^test standard::orchestration::quv::event_dispatch::tests::blocked_push_admission_does_not_block_reply_or_hide_overflow \.\.\. ok$' "${OUTPUT_DIR}/quv_runtime_policy.log"; then
  echo '[M16Q] FAIL quv_runtime_policy: missing bounded admission/reply isolation regression' >&2
  exit 1
fi
if ! grep -Eq '^test standard::orchestration::quv::event_dispatch::tests::aborting_event_drain_cancels_blocked_admission_worker \.\.\. ok$' "${OUTPUT_DIR}/quv_runtime_policy.log"; then
...
for required_case in merge_preserves_scan_errors_with_and_without_pending_writes merge_keeps_order_overrides_and_deletes_around_scan_failure; do
  if ! grep -Eq "^test state::overlay::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/state_overlay_scan.log"; then
...
for required_case in identity_substitution_refuses_without_invalidating_prior_admission unindexed_or_unknown_registry_schema_refuses_without_reinitializing registry_scan_failure_through_overlay_refuses_without_bootstrap_writes; do
  if ! grep -Eq "^test aft_effect_registry::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/manifest_admission.log"; then
...
run_phase pq_swarm_quv_lanes cargo test -p ioi-networking --lib libp2p::swarm::tests::
require_tests_ran pq_swarm_quv_lanes
for required_case in stale_reply_is_acked_without_holding_the_current_operation_lane current_push_is_refused_without_ack_while_a_stale_push_holds_the_lane admitted_push_is_acked_on_admission_and_completion_releases_lane nack_keeps_the_senders_durable_record enrollment_loss_reports_are_bounded_per_connection_and_skip_disconnected_peers recovery_keeps_authenticated_enrollment_and_erases_provisional_claims live_squatter_enrollment_never_authenticates_and_genuine_carrier_routes live_outbound_failure_on_unproven_enrollment_reports_loss_and_status_reenrollment_recovers; do
  if ! grep -Eq "^test libp2p::swarm::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/pq_swarm_quv_lanes.log"; then
    echo "[M16Q] FAIL pq_swarm_quv_lanes: missing passing lane/carrier regression ${required_case}" >&2
    exit 1
  fi
done
run_phase consequence_t10 cargo test -p agentgres consequence::tests
require_tests_ran consequence_t10
for required_case in stable_key_claim_guard_refuses_second_effect_for_same_conflict_slot_before_invocation claim_index_crash_before_claimed_persist_binds_slot_to_that_effect_across_reopen corrupt_or_aliased_claim_index_refuses_execution_without_mutation portable_manifests_do_not_touch_the_claim_index continuation_deadline_is_inclusive_and_rechecked_after_claimed_persist substituted_receipt_for_unadmitted_manifest_is_refused_before_quv_and_invocation cached_authorized_receipt_beyond_fence_is_refused_on_inspect_prepare_and_execute receipt_preparation_reuses_valid_pair_and_repairs_only_incomplete_spare receipt_admission_preserves_invalid_active_and_staging_aliases consequence_lock_refuses_aliases_and_retains_exclusive_ownership checked_preparation_refuses_before_storage_and_keeps_terminal_readmission_non_authorizing online_receipt_bound_covers_named_resource_execution_and_complete_budget receipt_hash_view_preserves_canonical_bytes_and_optional_fields receipt_trace_bound_refuses_extra_ambiguity_without_mutating_state pq_resource_format_bounds_cover_maximum_tokens_and_refuse_noncanonical_evidence; do
  if ! grep -Eq "^test consequence::tests::${required_case} \.\.\. ok$" "${OUTPUT_DIR}/consequence_t10.log"; then

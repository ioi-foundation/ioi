...
def check(lines):
    ...
    exchanges = []
    for i, line in enumerate(lines):
        match = re.search(r'renameat2\(AT_FDCWD(?:<[^>]+>)?, "([^"]+custody\.anchor)", AT_FDCWD(?:<[^>]+>)?, "([^"]+)", RENAME_EXCHANGE\)\s+= 0$', line)
        if match:
            if match.group(2) != match.group(1) + '.tmp':
                return False
            exchanges.append((i, match.group(1), match.group(2)))
    # Startup tests exchange support with identical authenticated bytes; each
    # of seven appends exchanges once; final authenticated reopen exchanges once.
    if len(exchanges) != 9 or not exchanges[0][0] < first:
        return False
    for (move, source, target), (swap, active, inactive) in zip(moves, exchanges[1:-1]):
        if move >= swap:
            return False
        syncs = [m.group(1) for line in lines[move+1:swap]
                 if (m := re.search(r'fsync\(\d+<([^>]+)>\)\s+= 0$', line))]
        if syncs != [str(Path(target).parent), str(Path(source).parent), inactive]:
            return False
        after = next((line for line in lines[swap+1:] if 'fsync(' in line), '')
        if not re.search(r'fsync\(\d+<' + re.escape(str(Path(active).parent)) + r'>\)\s+= 0$', after):
            return False
    active, inactive = exchanges[0][1:]
    for line in lines[exchanges[0][0]+1:exchanges[-2][0]+1]:
        if any(f'<{p}>' in line or f'"{p}"' in line for p in [active, inactive]):
            if any(token in line for token in ['fallocate(', 'ftruncate(', 'O_TRUNC', 'O_CREAT', ' rename(']):
                return False
            if 'write(' in line and f'<{active}>' in line:
                return False
    return True
...
def main():
    ...
    command = ['strace', '-f', '-y', '-s', '256', '-e', 'trace=fsync,fallocate,ftruncate,rename,renameat2,openat,write,close',
    ...
    missing_anchor_sync = [line for i, line in enumerate(lines) if not
                           (i > first_write and 'fsync(' in line and 'custody.anchor.tmp>' in line)]
    result = {'all_future_records_reserved_before_live_write': check(lines),
              ...
              'missing_anchor_sync_rejected': not check(missing_anchor_sync),
              'scope': 'issued syscalls and reported blocks; filesystem semantics and timing remain assumptions'}
    (output/'result.json').write_text(json.dumps(result, indent=2)+'\n')
    if not all(result[k] for k in ['all_future_records_reserved_before_live_write', 'missing_reservation_sync_rejected', 'live_truncation_rejected', 'missing_anchor_sync_rejected']):
        raise RuntimeError('reserved-record syscall check failed')
    print('record preallocation, inode transfer, anchor exchange, synchronization and removed-rule checks passed')
